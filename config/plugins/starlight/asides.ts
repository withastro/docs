...
/** Build an mdast paragraph carrying `data.hName` / `data.hProperties` hints. */
...
/**
 * Build an mdast paragraph that renders to an SVG element via data hints.
 *
 * Bypasses `hastscript`'s `s()` normalization (kebab → camelCase) because
 * Sätteri's HAST serializer doesn't reverse the conversion on output, so
 * `fill-rule` would render as `fillrule` instead of `fill-rule`. Numbers
 * are coerced to strings for the same reason — Sätteri drops non-string
 * property values.
 */
function s(el: string, attrs: Record<string, string | number> = {}, children: unknown[] = []): P {
	...
	return {
		type: 'paragraph',
		data: { hName: el, hProperties: properties },
		children: children as P['children'],
	};
}

// Default icon SVG paths (24×24 viewBox), matching Starlight's built-in icons.
...
/**
 * Sätteri MDAST plugin that converts blocks delimited with `:::` into styled
 * asides (a.k.a. "callouts", "admonitions", etc.). Mirrors Starlight's
 * `remarkAsides` essentially 1:1 — emits an mdast paragraph carrying
 * `data.hName` / `data.hProperties` hints, which Sätteri's MDAST→HAST
 * converter expands into the corresponding HTML element while leaving the
 * children alone.
 *
 ...
 * ```
 */
export function asidesPlugin(options?: RemarkRehypePluginOptions) {
	return defineMdastPlugin({
		...
		containerDirective(node, ctx) {
			...
			let title = t ? t(`aside.${variant}`) : defaultTitles[variant];
