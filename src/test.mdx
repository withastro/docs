...
Cài đặt tích hợp [`@astrojs/db`](/en/guides/integrations-guide/db/) bằng lệnh `astro add` tích hợp sẵn:
...
<ReadMore>Xem [tham chiếu cấu hình bảng](/en/guides/integrations-guide/db/#table-configuration-reference) để có tham chiếu đầy đủ về các tùy chọn bảng.</ReadMore>
...
<ReadMore>Xem [tham chiếu cột bảng](/en/guides/integrations-guide/db/#table-configuration-reference) để biết thêm chi tiết.</ReadMore>
...
Bạn có thể truy vấn cơ sở dữ liệu từ bất kỳ [trang Astro](/en/basics/astro-pages/#astro-pages), [endpoint](/en/guides/endpoints/), hoặc [action](/en/guides/actions/) nào trong dự án của bạn bằng cách sử dụng ORM `db` và query builder được cung cấp.
...
Bạn cũng có thể sử dụng [Astro actions](/en/guides/actions/) để chèn dữ liệu vào bảng Astro DB. Ví dụ sau chèn một hàng vào bảng `Comment` bằng một action:
...
Xem [tham chiếu CLI](/en/guides/integrations-guide/db/#astro-db-cli-reference) để có danh sách đầy đủ các lệnh.

</ReadMore>

## Xây dựng tích hợp Astro DB

[Tích hợp Astro](/en/reference/integrations-reference/) có thể mở rộng các dự án người dùng với các bảng Astro DB và dữ liệu seed bổ sung.
