...
- **내부 링크:** Astro 공식 문서 페이지에 대한 링크는 항상 "/en"을 "/ko"로 업데이트합니다. 또한 특정 제목으로 이동하는 링크의 ID 역시 한국어 버전으로 업데이트해야 합니다. 만약 해당 페이지가 아직 번역되지 않은 경우에는 "/en"은 "/ko"로 수정하되, ID는 영어 버전을 유지합니다. 다른 페이지에서 링크된 페이지를 번역하는 경우에는 PR에 관련된 모든 페이지 업데이트를 포함시킵니다.
