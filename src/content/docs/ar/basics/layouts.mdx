...
- [**`<Slot />`**](/ar/basics/astro-components/#slots) لتحديد المكان الذي يجب إدراج محتوى الصفحة فيه.

{/* TODO: Link to /ar/guides/{framework-components, client-side-scripts, styling} (does not exist yet)*/}

ومع ذلك، فإن مكونات التخطيط ليست شيئًا مميزًا! يمكنها [قبول الخصائص](/ar/basics/astro-components/#props-komponenteneigenschaften) و[استيراد واستخدام مكونات أخرى](/ar/basics/astro-components/#komponentenstruktur) مثل أي مكون أسترو آخر. يمكن أن تحتوي على [مكونات إطار عمل واجهة المستخدم](/en/guides/framework-components/) و[برامج نصية من جانب العميل](/en/guides/client-side-scripts/). ليست بحاجة لتوفير واجهة صفحة كاملة، بل يمكن استخدامها بدلاً من ذلك كقوالب واجهة مستخدم جزئية.

ومع ذلك، إذا كانت مكونة التخطيط تحتوي على غطاء صفحة، يجب أن يكون عنصر `<html>` هو العنصر الأب لجميع العناصر الأخرى في المكون. يجب أن تكون جميع عناصر [`<style>`](/en/guides/styling/#styles-stile-in-astro) أو [`<script>`](/en/guides/client-side-scripts/#verwendung-von-script-in-astro) محاطة بعلامات `<html>`.
...
<ReadMore>تعرف على المزيد حول [الفتحات](/ar/basics/astro-components/#slots).</ReadMore>
...
لا يتم التعرف على هذه الخاصية كخاصية خاصة عند استخدام [مجموعات المحتوى](/en/guides/content-collections/) للاستعلام عن المحتوى وعرضه.
...
يمكنك تعيين نوع[`Props` type](/en/guides/typescript/#component-props)  للتخطيط باستخدام مساعد `MarkdownLayoutProps`:
...
- **`url`** - عنوان URL للصفحة (على سبيل المثال: `/en/guides/markdown-content`).
...
:::note
سيكون لتخطيط Markdown الوصول إلى جميع خصائص ملف Markdown [المتاحة](/en/guides/markdown-content/#available-properties) من `Astro.props` **مع فرقين رئيسيين:**
...
{/* TODO: Link to /ar/guides/markdown-content (does not exist yet)*/}
<ReadMore>تعرف أكثر عن دعم Astro لـ Markdown و MDX في [دليل Markdown](/en/guides/markdown-content/).</ReadMore>
