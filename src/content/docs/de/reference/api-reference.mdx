...
`Astro.props` ist ein Objekt, das alle Werte enthält, die als [Komponentenattribute](/de/basics/astro-components/#props-komponenteneigenschaften) übergeben wurden. Layoutkomponenten für `.md` und `.mdx`-Dateien erhalten Frontmatter-Werte als Props.
...
`Astro.cookies` enthält Helfer zum Lesen und Bearbeiten von Cookies im Modus [Server-Side-Rendering](/de/guides/on-demand-rendering/).
...
Im Modus [server-side-rendering](/de/guides/on-demand-rendering/) kannst du mit `Astro.redirect()` zu einer anderen Seite umleiten.
