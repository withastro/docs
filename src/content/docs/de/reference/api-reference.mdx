...
Bei statischen Builds sind dies die `Parameter`, die von `getStaticPaths()` zurückgegeben und für das Prerendering von [dynamischen Routen](/de/basics/routing/#dynamische-routen) verwendet werden.
...
Die [Endpunktfunktionen](/de/basics/endpoints/) erhalten ein Kontextobjekt als ersten Parameter. Es spiegelt viele der globalen Eigenschaften von `Astro` wider.
...
Sie kann auch in statischen Datei-Endpunkten für [dynamisches Routing] verwendet werden (/de/basics/endpoints/#params-und-dynamisches-routing).
