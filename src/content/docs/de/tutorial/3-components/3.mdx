...
Da deine Website auf verschiedenen Geräten angesehen wird, ist es an der Zeit, eine Seitennavigation zu erstellen, die auf mehrere Bildschirmgrößen reagieren kann!
...
1. Ersetze auf jeder Seite deine bestehende `<Navigation/>`-Komponente durch deine neue `<Header/>`-Komponente.
    ...
    <details>
    <summary>Zeig mir den Code!</summary>
...
2. Überprüfe deine Browser-Vorschau und stelle sicher, dass deine Kopfzeile auf jeder Seite angezeigt wird. Es wird noch nicht anders aussehen, aber wenn du deine Vorschau mit Entwicklertools untersuchst, wirst du sehen, dass du nun Elemente wie `<header>` und `<nav>` rund um deine Navigationslinks hast.
</Steps>
</Box>
...
<Steps>
1. Aktualisiere `Navigation.astro` mit der CSS-Klasse, um deine Navigationslinks zu steuern. Umfasse die vorhandenen Navigationslinks mit einem `<div>` mit der Klasse `nav-links`.
...
2. Kopiere die CSS-Styles unten in `global.css`. Diese Styles:
    ...
    Beginne damit zu definieren, was auf kleinen Bildschirmgrößen passieren soll! Kleinere Bildschirmgrößen erfordern einfachere Layouts. Passe dann deine Styles an, um größere Geräte zu berücksichtigen. Wenn du zuerst ein kompliziertes Layout entwirfst, musst du späterdaran feilen, um es wieder einfach zu machen.
    :::
...
- [Komponentenbasiertes Design](https://www.droptica.com/blog/component-based-design/) <Badge>extern</Badge>

- [Semantische HTML-Tags](https://www.dofactory.com/html/semantics) <Badge>extern</Badge>

- [Mobile-First Design](https://www.mobileapps.com/blog/mobile-first-design) <Badge>extern</Badge>
