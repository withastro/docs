...
Standardmäßig wird eine [UI-Framework-Komponente im Client nicht hydratisiert](/de/basics/framework-components/#interaktive-komponenten-hydratisieren). Wenn keine `client:*`-Direktive angegeben wird, wird ihr HTML-Code ohne JavaScript auf der Seite gerendert.
