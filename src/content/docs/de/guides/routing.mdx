...
1. Du musst einen oder mehrere dynamische Parameter in `[eckigen Klammern]` im Datei- oder Verzeichnisnamen enthalten.

2. Du musst eine `getStaticPaths()`-Funktion exportieren, die genau angibt, welche Routen von Astro vorgerendert werden sollen.
