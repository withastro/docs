---
titel: Veröffentliche deine Astro-Site auf Netlify
...
4. Bestätige deinen Build-Befehl (`astro build`)
    ...
    Das CLI erkennt automatisch die Build-Einstellungen (`astro build`) und das Veröffentlichungsverzeichnis (`dist`) und bietet an, automatisch [eine `netlify.toml`-Datei](#netlifytoml-file) mit diesen Einstellungen zu erzeugen.
