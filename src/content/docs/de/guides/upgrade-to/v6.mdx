---
title: Aktualisiere auf Astro v6
...
Bei Bedarf kannst du deine Astro-Integrationen auch [manuell aktualisieren](/de/guides/integrations-guide/#manual-upgrading), und möglicherweise musst du auch andere Abhängigkeiten in deinem Projekt aktualisieren.

:::note[Möchtest du fortfahren?]
...
Astro v6.0 enthält [möglicherweise kompatibilitätsbeeinträchtigende Änderungen](#kompatibilitätsbrechende-änderungen) sowie die Entfernung und Abkündigung einiger Funktionen.

Falls dein Projekt nach dem Upgrade auf Version 6.0 nicht wie erwartet funktioniert, findest du in dieser Anleitung einen Überblick über alle inkompatiblen Änderungen sowie Anweisungen zur Aktualisierung deiner Codebasis.
...
Wenn du Vite-spezifische Plugins, Konfigurationen oder APIs verwendest, schau dir den [Vite-Migrationsleitfaden](https://vite.dev/guide/migration) an, um dich über kompatibilitätsbrechende Änderungen zu informieren, und aktualisiere dein Projekt bei Bedarf.
...
Astro v6.0 bringt wesentliche Änderungen daran mit sich, wie Astro verschiedene Laufzeitumgebungen (Client, Server und Prerender) verwaltet, nachdem intern eine Umstrukturierung vorgenommen wurde, um die [neue Environments-API von Vite](https://vite.dev/guide/api-environment) zu nutzen.
...
### Legacy: Rückwärtskompatibilität bei Inhaltssammlungen

In Astro 5.x konnten Projekte das Upgrade auf die neue Content-Layer-API, die für Content-Sammlungen eingeführt wurde, hinauszögern, da eine gewisse automatische Abwärtskompatibilität bestand, die zuvor nicht hinter einem Flag verborgen war. Das bedeutete, dass ein Upgrade von Astro 4 auf Astro 5 möglich war, ohne deine Content-Sammlungen zu aktualisieren, selbst wenn du das Flag `legacy.collections` nicht aktiviert hattest. Die Projekte ließen sich weiterhin erstellen, und es wurden keine Fehler oder Warnungen angezeigt.

Astro v6.0 entfernt diese automatische Unterstützung für ältere Inhaltssammlungen sowie [das Flag `legacy.collections`](#entfernt-legacy-inhaltssammlungen). Alle Inhaltssammlungen müssen nun [die in Astro v5.0 eingeführte Content-Layer-API](https://astro.build/blog/content-layer-deep-dive/) nutzen, auf der alle Inhaltssammlungen basieren.
...
<ReadMore>Ausführliche Anweisungen zum Upgrade von älteren Sammlungen auf die neue Content Layer API findest du im [Astro v5-Upgrade-Leitfaden](/de/guides/upgrade-to/v5/#legacy-v20-content-collections-api).</ReadMore>
...
Dieser Flag bewahrt einige ältere Funktionen der Inhaltssammlungen aus Version 4:
...
- Verwendet pfadbasierte Entry-IDs anstelle von Slug-basierten IDs

**Dies ist eine vorübergehende Migrationshilfe.** Migrier deine Sammlungen so bald wie möglich zur Content Layer API und deaktiviere dieses Flag anschließend.
...
<ReadMore>Erfahre mehr über die [`astro:config`-Virtual-Module](/de/reference/modules/astro-config/).</ReadMore>
...
### Veraltet: exposed `astro:transitions` internals
...
### Veraltet: session driver string signature
...
```js title="meine-adapter/server.js" del={1-12} ins={13-22}
...
```js title="meine-adapter/server.js" del={1-5} ins={6-8}
...
Wenn du ein Adapter-Autor mit einem öffentlichen Repository bist und [das Schlüsselwort `astro-adapter` in deine `package.json` einfügst](/de/reference/publish-to-npm/#categories), wird das Astro-Kernteam versuchen, direkt einen PR an dein Repository zu senden, um dir bei der Migration deines Codes zu helfen, falls du die folgenden Schritte noch nicht befolgt hast.

Falls du Warnmeldungen erhältst, weil du einen Community-Adapter verwendest, der noch nicht aktualisiert wurde, wende dich bitte direkt an den Autor des Adapters, um ihn darauf hinzuweisen. Letztendlich liegt es in seiner Verantwortung, seine Adapter zu aktualisieren. Du kannst auch das Astro-Kernteam im [`#integrations`-Kanal unseres Discord-Servers](https://astro.build/chat) darüber informieren, und wir werden versuchen, dem Adapter-Autor bei der Aktualisierung zu helfen.
...
1. Aktualisiere deine `setAdapter()`-Methode: Setze `entrypointResolution: "auto"`, entferne `exports` und `args`
    ...
    ```js title="meine-adapter.mjs" ins={3} del={4-5}
    setAdapter({
      // ...
...
2. Aktualisiere deinen Server-Einstiegspunkt, um alle erforderlichen Exporte ohne `createExports()` bereitzustellen:
    ...
    ```js title="meine-adapter/server.js" del={1-11} ins={12-18}
...
3. Wenn dein Adapter eine `start()`-Funktion bereitstellt, passe deinen Server-Einstiegspunkt so an, dass der Code direkt aufgerufen wird:
    ...
    ```js title="meine-adapter/server.js" del={1-9} ins={10-16}
    import { App } from 'astro/app';
...
4. Wenn du dich auf `args` verlassen hast, [erstelle ein virtuelles Modul, um die Konfiguration zur Erstellungszeit zu übergeben](/de/reference/adapter-reference/#passing-build-time-configuration) und importiere sie stattdessen aus dem virtuellen Modul:
    ...
    ```js title="meine-adapter/server.js" del={1-3} ins={4}
    export function createExports(manifest, { assets }) {
      // ...
    }
    import { assets } from 'virtual:@example/meine-adapter:config';
    ```
...
### Entfernt: Legacy Inhaltssammlungen
...
Falls du deine Sammlungen noch nicht auf Astro v5.0 aktualisiert hast, stelle außerdem sicher, dass deine Inhaltssammlungen für die neue API **vollständig aktualisiert** sind.

Astro v5.x bot eine gewisse automatische Abwärtskompatibilität, damit Inhaltssammlungen auch dann weiterhin funktionierten, wenn sie nicht auf die neue API aktualisiert worden waren. Daher können deine v5-Sammlungen eine oder mehrere ältere Funktionen enthalten, die für v6 auf die neuere API aktualisiert werden müssen, selbst wenn dein Projekt zuvor fehlerfrei war.
...
<details>
<summary>keine Konfigurationsdatei für Inhaltssammlungen hast</summary>
Erstelle die Datei `src/content.config.ts` und [definieren deine Sammlungen](/de/guides/content-collections/#defining-build-time-content-collections) darin.
</details>
...
<details>
<summary>eine Sammlung hast, für die kein `loader` definiert ist ([`ContentCollectionMissingALoaderError`](/de/reference/errors/content-collection-missing-loader/))</summary>

Importiere [Astro’s integrierten `glob()`-Loader](/de/guides/content-collections/#the-glob-loader) und definiere `pattern` und `base` für deine Sammlungseinträge:
...
<details>
<summary>eine Sammlung hast, die einen Sammlungstyp definiert (`type: 'content'` oder `type: 'data'`) / ([`ContentCollectionInvalidTypeError`](/de/reference/errors/content-collection-invalid-type/))</summary>
Es gibt keine verschiedenen Arten von Sammlungen mehr. Dies muss aus deiner Sammlungsdefinition gelöscht werden.
...
const blog = defineCollection({
  // Für die Inhaltsschicht definierst du keinen `type` mehr
  type: 'content',
  loader: glob({ pattern: '**/[^_]*.{md,mdx}', base: "./src/data/blog" }),
  schema: z.object({
    title: z.string(),
...
<details>
<summary>die veralteten Abfragemethoden für Sammlungen `getDataEntryById()` und `getEntryBySlug()` verwendest / ([`GetEntryDeprecationError`](/de/reference/errors/get-entry-deprecation-error/))</summary>
Ersetze beide Methoden durch [`getEntry()`](/de/reference/modules/astro-content/#getentry).

</details>

<details>
<summary>ältere Methoden zum Abfragen und Darstellen von Sammlungen hast, die von einer `slug`-Eigenschaft abhängen / ([`ContentSchemaContainsSlugError`](/de/reference/errors/content-schema-contains-slug-error/))</summary>
...
<ReadMore>Weitere Informationen zur Abwärtskompatibilität von älteren Sammlungen in Astro v5 sowie eine vollständige Schritt-für-Schritt-Anleitung für das Upgrade älterer Sammlungen auf die neue Content Layer API findest du im [Astro v5-Upgrade-Leitfaden](/de/guides/upgrade-to/v5/#legacy-v20-content-collections-api).</ReadMore>
...
Erwäge gegebenenfalls die Verwendung von [Inhaltssammlungen](/de/guides/content-collections/), um deine Inhalte zu organisieren. Diese verfügen über eigene, neuere und leistungsfähigere Abfragefunktionen.
...
<ReadMore>Erfahre mehr über [Übergänge mit Formularen](/de/guides/ansichtsübergänge/#transitions-with-forms).</ReadMore>
...
Wenn du einen Content-Loader entwickelst und eine Funktion verwendest, um eine `schema`-Eigenschaft der Sammlung dynamisch zurückzugeben, musst du deine bestehende Funktion entfernen und stattdessen die neue `createSchema()`-Eigenschaft verwenden, um dein Schema zu definieren.
...
<ReadMore>Erfahre mehr über die [Astro-Konfigurationsdatei](/de/guides/astro-konfigurieren/#die-astro-konfigurationsdatei).</ReadMore>
...
- `liveContentCollections` (Siehe die aktualisierte [Dokumentation zu Content Collections](/de/guides/content-collections/), um mehr über Live-Sammlungen zu erfahren.)
- `failOnPrerenderConflict` (Siehe die neue Konfigurationsoption [`prerenderConflictBehavior`](/de/reference/configuration-reference/#prerenderconflictbehavior).)
...
<ReadMore>Erfahre mehr über spannende neue Funktionen und vieles mehr im [Blogbeitrag zu Version 6.0](https://astro.build/blog/astro-6-beta/).</ReadMore>
...
In Astro v5.5 wurde das Flag `experimental.preserveScriptOrder` eingeführt, um mehrere `<style>`- und `<script>`-Tags in derselben Reihenfolge darzustellen, in der sie im Quellcode deklariert wurden. Astro 5.x kehrte ihre Reihenfolge in der generierten HTML-Ausgabe um. Dies konnte zu unerwarteten Ergebnissen führen, beispielsweise dazu, dass CSS-Stile beim Erstellen deiner Website durch früher definierte Style-Tags überschrieben wurden.

Astro 6.0 hebt dieses experimentelle Flag auf und macht dies zum neuen Standardverhalten in Astro: Skripte und Stile werden nun in der Reihenfolge gerendert, die in deinem Code definiert ist.
...
### Geändert: Wie responsive Bildstile ausgegeben werden

<SourcePR number="15407" title="support responsive images"/>

In Astro 5.x wurden Bilder zur Laufzeit berechnet, und die responsiven Bildstile `fit` und `pos` wurden in ein `style`-Attribut eingefügt. Dies war aus vielen Gründen nicht mit Astros Content Security Policy (CSP) kompatibel.

Astro 6 generiert Bildstile zur Erstellungszeit innerhalb eines virtuellen Moduls auf Basis der Projektkonfiguration, was zu einer Hash-Klasse und `data-*`-Attributen führt, mit denen du responsives Styling auf deine Bilder anwenden kannst.
...
Falls du dich jedoch auf die zuvor für deine Bilder generierten Inline-Stile verlassen hast:

```html
<img style="--fit: <value>; --pos: <value>" >
```

dann musst du deinen Projektcode anpassen, um stattdessen die neuen `data-*`-Attribute zu berücksichtigen:
...
Die folgenden Änderungen gelten in Astro v6.0 als grundlegende Änderungen. Grundlegende Änderungen bieten unter Umständen vorübergehende Abwärtskompatibilität, unter Umständen aber auch nicht. Wenn du diese Funktionen genutzt hast, musst du deinen Code möglicherweise entsprechend den Empfehlungen in den einzelnen Einträgen anpassen.

### Geändert: endpoints with a file extension cannot be accessed with a trailing slash
...
Rufe stattdessen auf dem Server `getImage()` auf und übergebe das resultierende `src` an den Client:
...
<ReadMore>Ein vollständiges Beispiel findest du unter [Bilder mit `getImage()` generieren](/de/guides/bilder/#bilder-mit-getimage-generieren).</ReadMore>
...
Wenn du manuelle Links zu Überschriften hast, musst du möglicherweise den Wert des Ankerlinks um einen neuen abschließenden Bindestrich ergänzen. Zum Beispiel die folgende Markdown-Überschrift:

```md
## `<Picture />`
```

erzeugt nun den folgenden HTML-Code mit einem abschließenden Bindestrich in der Überschrift `id`:
...
```markdown ins="-"
Weitere Informationen findest du unter [der Bildkomponente](/de/guides/images/#picture-).
```
...
<ReadMore>Erfahre mehr über [Überschriften-IDs](/de/guides/markdown-inhalt/#heading-ids).</ReadMore>
...
Astro 6.0 nutzt die neue Environment-API von Vite für die Build-Konfiguration und die Interaktion mit dem Entwicklungsserver. Das ermöglicht vor allem den Dev-Modus in Laufzeiten wie workerd, bedeutet aber auch, dass sich einige Integrations-Hooks und HMR-Zugriffsmuster geändert haben.
...
```ts del={2} ins={3}
// Um dasselbe Format (z. B. "file:///path/to/src") beizubehalten, nimm folgende Änderung vor:
...
In Astro 5.x wurden die Typen für Inhaltssammlungen mithilfe von `zod-to-ts` generiert, wenn sie von einem Inhaltslader bereitgestellt und nicht durch ein benutzerdefiniertes Schema festgelegt wurden.
