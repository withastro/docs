...
In Astro v2.x wurden standardmäßig alle Projekt-Styles als Link-Tags gesendet. Du konntest wählen, sie jedes Mal in `<style>`-Tags einzufügen, indem du `"always"` verwendest, oder nur Styles unter einer bestimmten Größe mit `"auto"` inlining durch die Konfiguration [`build.inlineStylesheets`](/de/reference/configuration-reference/#buildinlinestylesheets). Die Standardvoreinstellung war `"never"`.
