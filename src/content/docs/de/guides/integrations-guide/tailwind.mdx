...
Diese **[Astro integration][astro-integration]** erlaubt dir, [Tailwinds](https://tailwindcss.com) CSS Utility-Klassen in jeder `.astro` Datei und [UI-Framework-Komponenten](/de/guides/framework-components/) deines Projekts zu verwenden.

## Warum Tailwind?

Tailwind ermöglicht es dir, Utility-Klassen anstelle von CSS zu verwenden. Diese Utility-Klassen entsprechen größtenteils einer bestimmten CSS-Eigenschaftseinstellung: Zum Beispiel entspricht das Hinzufügen von `text-lg` zu einem Element dem Setzen von `font-size: 1.125rem` in CSS. Du findest es möglicherweise einfacher, deinen Stile mithilfe dieser vordefinierten Utility-Klassen zu schreiben und zu pflegen!

Wenn dir diese vordefinierten Einstellungen nicht gefallen, kannst du die [Tailwind-Konfigurationsdatei](https://tailwindcss.com/docs/configuration) deiner projektbezogenen Designanforderungen anpassen. Wenn zum Beispiel der "große Text" in deinem Design tatsächlich `2rem` beträgt, kannst du [die `lg` fontSize-Einstellung](https://tailwindcss.com/docs/font-size#customizing-your-theme) auf `2rem` ändern.

Tailwind ist auch eine großartige Wahl, um Stile zu React-, Preact- oder Solid-Komponenten hinzuzufügen, die kein `<style>`-Tag in der Komponentendatei unterstützen.
...
Solltest du dabei Probleme entdecken, [melde sie gerne bei uns auf GitHub](https://github.com/withastro/astro/issues) und versuche stattdessen die manuelle Installation.
...
Füge die Integration dann deiner Astro Konfigurationsdatei im Feld `integrations` hinzu:
...
Erstelle als nächstes eine `tailwind.config.mjs` Datei in deinem Projektstamm&shy;verzeichnis. Du kannst folgenden Befehl ausführen, um eine Konfigurationsdatei mit Basiseinstellungen für dich erstellen zu lassen:
...
Wenn du diese Integration installierst, kannst du die Utility-Klassen von Tailwind sofort verwenden. Gehe zur [Tailwind Dokumentation](https://tailwindcss.com/docs/utility-first), um zu lernen, wie man Tailwind verwendet. Wenn du eine Utility-Klasse findest, die du ausprobieren möchtest, füge sie einfach einem HTML-Element in einem Projekt hinzu!

[Autoprefixer](https://github.com/postcss/autoprefixer) wird auch automatisch eingerichtet, wenn im Entwicklungsmodus und für Produktions-Builds gearbeitet wird, sodass Tailwind-Klassen in älteren Browsern funktionieren.

## Konfiguration

### Tailwind konfigurieren

Wenn du die automatische Installation verwendet hast und jede Aufforderung mit Ja beantwortet hast, siehst du in deinem Projektstamm&shy;verzeichnis eine `tailwind.config.mjs` Datei. In dieser Datei kannst du Änderungen an deiner Tailwind Konfiguration vornehmen. Du kannst [in der Tailwind Dokumentation](https://tailwindcss.com/docs/configuration) lernen, wie man Tailwind konfiguriert.

Wenn du keine Konfigurationsdatei findest, füge deine eigene `tailwind.config.(js|cjs|mjs|ts|mts|cts)` Datei dem Projektstamm&shy;verzeichnis hinzu und die Integration with sie verwenden. Dies kann hilfreich sein, wenn du Tailwind bereits in einem anderen Projekt konfiguriert hast und diese Einstellungen auf dieses übertragen möchtest.
...
Früher bekannt als `config.path` in `@astrojs/tailwind` v3. Sehe dir dazu das [v4 Änderungsprotokoll](https://github.com/withastro/astro/blob/main/packages/integrations/tailwind/CHANGELOG.md#400) an, um deine Konfiguration zu aktualisieren.

Wenn du eine andere Tailwind Konfigurationsdatei als die übliche `tailwind.config.(js|cjs|mjs|ts|mts|cts)` Datei verwenden möchtest, musst du den Ort dieser Datei der `configFile` Option der Integration bekannt. Wenn du einen relativen Pfad verwendest, wird der Pfad relativ zum aktuellen Arbeitsverzeichnis berechnet.

:::caution
Es ist nicht empfohlen, eine andere Datei zu verwenden, da dies zu Problemen mit anderen Werkzeugen, die dir bei der Arbeit mit Tailwind behilflich sein können, wie zum Beispiel der offiziellen Tailwind VS Code Erweiterung, führen kann.
:::
...
Früher bekannt als `config.applyBaseStyles` in `@astrojs/tailwind` v3. Sehe dir dazu das [v4 Änderungsprotokoll](https://github.com/withastro/astro/blob/main/packages/integrations/tailwind/CHANGELOG.md#400) an, um deine Konfiguration zu aktualisieren.
...
Um dieses Standardverhalten auszuschalten, setze `applyBaseStyles` auf `false`. Dies kann hilfreich sein, wenn du deine eigene `base.css` Datei definieren musst (zum Beispiel um die [`@layer` Direktive](https://tailwindcss.com/docs/functions-and-directives#layer) zu verwenden). Nützlich ist das auch, wenn du nicht willst, dass `base.css`in jeder Seite deines Projekts importiert wird.
...
Du kannst jetzt [deine eigene `base.css` Datei als lokales Stylesheet importieren](/en/guides/styling/#import-a-local-stylesheet).

#### nesting

Setze es auf `true` um das [`tailwindcss/nesting` PostCSS plugin](https://tailwindcss.com/docs/using-with-preprocessors#nesting) zu verwenden. So kannst du auch verschachtelte CSS Deklarationen neben dem Tailwind Syntax verwenden. Diese Option ist standardmäßig auf `false` gesetzt.
...
Wenn du die `@apply` Direktive in dem `<style>` Tag einer Astro, Vue, Svelte oder anderer Komponenten verwendest, kann es vorkommen, dass ein Fehler wegen deiner definierten Tailwind Klasse, die nicht existiert, generiert wird, was den Build zum Absturz bringt.
...
Du kannst dich auch die [Astro Integration Dokumentation][astro-integration] für weitere Integrationen durchlesen.
