...
In Astro passieren die meisten Prozesse auf dem Server statt in dem Browser. Dies macht deine Seite oder App grundsätzlich schneller als durch reines client-seitiges Rendering, vor allem auf weniger performanten Geräten oder bei schlechter Internetverbindung. Serverseitig gerendertes HTML ist schnell, SEO-freundlich, und standardmäßig barrierefrei.
...
Standardmäßig sind Astro-Projekte so konfiguriert, dass sie beim Erzeugungszeitpunkt vorgerendert werden (also statisch generiert), um die schlankste Browser-Erfahrung zu gewährleisten. Der Browser muss nicht darauf warten, dass das HTML erst erstellt werden muss, weil der Server keine Seiten bei Bedarf erstellen muss. Deine Webseite ist nicht von der Leistung deines Servers abhängig, und wenn das HTML einmal erstellt wurde, wird deine Webseite als statische Seite für deine Besucher verfügbar sein, solange dein Server funktioniert.

Statische Seite können [Astro-Inseln](/de/concepts/islands/) für interaktive UI-Komponenten (oder sogar ganze eingebettete, clientseitig gerenderte Apps), geschrieben in einem [UI-Framework deiner Wahl](/de/guides/framework-components/), beinhalten.
...
:::tip
Astros standardmäßiger `static`-Modus ist eine mächtige, moderne Wahl für Seiten mit vielen Inhalten, die nur selten ändern, und die den gleichen Inhalt an alle Besucher anzeigen.
:::

### Bei Bedarf gerendert

Mit [einem SSR-Adapter](/de/guides/server-side-rendering/) können Astros beiden anderen Ausgabemodi konfiguriert werden, um **Rendering bei Bedarf (on-demand) auf einigen oder all deinen Seiten, Routen oder API-Endpunkten** zu ermöglichen:
...
Da sie bei jedem Besuch neu generiert werden, können diese Routen für jeden Besucher personalisiert werden. So können zum Beispiel on-demand gerenderte Seiten einem eingeloggten deren Profilinformationen angezeigt werden, oder auch frisch aktualisierte Daten, ohne die Notwendigkeiten eines erneuten und vollständig Erzeugungsschritt. On-demand Rendering auf dem Server ist auch als **serverseitiges Rendering (SSR)** bekannt.
  
<ReadMore>Lies mehr über [mögliche Features mit on-demand Rendering](/de/guides/server-side-rendering), wie zum Beispiel Cookies, Response- und Request-Objekte, HTML-Streaming, und mehr.</ReadMore>

[Erwäge den `server`- oder `hybrid`-Modus](/de/guides/server-side-rendering/#on-demand-rendering-features) in deinem Astro-Projekt, wenn du das folgende benötigst:
