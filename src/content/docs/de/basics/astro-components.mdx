...
// Zugriff auf übergebene Komponenteneigenschaften (Props), wie z.B. `<X title="Hallo, Welt!" />`
...
Hier ist ein Beispiel für eine Komponente, die eine `greeting`-Eigenschaft und eine `name`-Eigenschaft empfängt. Beachte, dass die zu empfangenden Eigenschaften aus dem globalen `Astro.props`-Objekt destrukturiert werden.
...
Diese Komponente, wenn in anderen Astro-Komponenten, Layouts oder Seiten importiert und gerendert, kann diese Eigenschaften als Attribute weitergeben:
