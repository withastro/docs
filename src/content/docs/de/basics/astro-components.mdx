...
Um mehrere HTML-Elemente ohne ein umschließendes `<div>` in den `<slot/>` Platzhalter einer Komponente zu übergeben, verwende das `slot=""`-Attribut auf [Astros <Fragment/>-Komponente](/de/reference/api-reference/):
