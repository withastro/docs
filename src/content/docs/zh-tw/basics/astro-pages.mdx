...
為了在每個頁面避免重複使用到相同的 HTML 元素，你可以將一般的 `<head>` 和 `<body>` 移到你自己的[版面 components](/zh-tw/basics/layouts/)。你可以隨自己喜歡或多或少使用這些版面 components。
...
擁有 `.html` 副檔名的檔案可以放置在 `src/pages/` 內，並直接作為你網站的頁面。但要注意的是，有些關鍵的 Astro 功能在 [HTML Components](/zh-tw/basics/astro-components/#html-元件) 中是不被支援的。
