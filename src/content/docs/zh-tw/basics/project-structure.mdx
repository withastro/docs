...
- [Astro components](/zh-tw/basics/astro-components/)
- [前端 components （React 等）](/zh-tw/guides/framework-components/)
...
**Components** 是你 HTML 網頁中可重複使用的元件程式碼。這些元件可以是 [Astro components](/zh-tw/basics/astro-components/) 或是 [前端 components](/zh-tw/guides/framework-components/) 如 React 或 Vue。一般來說會在這個資料夾中組合和組織你專案所有的 components。

這是在 Astro 專案中普遍的慣例作法，但並非必須。就你自己喜好，自由自在地組織你的 components 吧！

### `src/layouts`

[版面](/zh-tw/basics/layouts/)是一個特殊的 component 其會將一些內容包進一個更大的網頁版面。這些很常被用來定義網頁如 [Astro 頁面](/zh-tw/basics/astro-pages/) 和 [Markdown 頁面](/zh-tw/guides/markdown-content/)的版面。

就如 `src/components` 一樣，這個目錄是一個普遍慣例作法並非必須。

### `src/pages`

[Pages](/zh-tw/basics/astro-pages/) are a special kind of component used to create new pages on your site. A page can be an Astro component, or a Markdown file that represents some page of content for your site.
