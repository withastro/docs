...
加入[支援的檔案類型](/zh-tw/basics/astro-pages/#supported-page-files#支援的頁面檔案類型) 到這個資料夾就可以建立網站的頁面路由。
...
`src/content/` 是保留給 Astro 的目錄，用來儲存[內容合集](/zh-tw/guides/content-collections/)和設定檔。其他檔案不可放在此目錄底下。
...
查看[設定 Astro 指南](/zh-tw/guides/configuring-astro/)來了解更多在設定上的細節。
