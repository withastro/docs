...
Astro 會處理、最佳化並將你的 `src/` 打包建立最後瀏覽器會看到的網站。不像靜態的 `public/` 資料夾，Astro 會幫你建立並處理你 `src/` 的檔案。

一些檔案（如 Astro 元件）會被算繪成靜態的 HTML 而不會就所寫的內容傳到瀏覽器上。其他檔案（如 CSS）會被傳到瀏覽器上，但可能會為了效能表現而先被最佳化或打包進其他 CSS 檔案。
...
**元件** 是你 HTML 網頁中可重複使用的程式碼。它們可以是 [Astro 元件](/zh-tw/basics/astro-components/) 或是 [UI 框架元件](/zh-tw/guides/framework-components/) 如 React 或 Vue。一般來說會在這個資料夾中組合和組織你專案所有的元件。
...
Astro 的 JavaScript 組態檔支援幾個檔案格式：`astro.config.js`、`astro.config.mjs` 以及 `astro.config.ts`。我們推薦在多數情況下使用 `.mjs`，或者如果你想要用 TypeScript 寫組態檔的話就用 `.ts`。
