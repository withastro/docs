...
Astro 元件最重要的一點是：**不在客戶端算繪**。它們只在建置階段或[伺服器端算繪（SSR）](/zh-tw/guides/on-demand-rendering/)需要時轉換為 HTML。元件的 frontmatter 可放入 JavaScript 程式碼，它們不會送至使用者的瀏覽器。預設不包含任何 JavaScript 足跡，因此你的網站變得更快了。
