...
- 供頁面內容嵌入的插槽 [`<slot />`](/zh-tw/basics/astro-components/#slots)

但其實版面元件沒什麼特別的！它們和其他 Astro 元件一樣，可以[接受參數](/zh-tw/basics/astro-components/#component-props)、[匯入並使用其他元件](/zh-tw/basics/astro-components/#component-structure)，也能包含 [UI 框架元件](/zh-tw/guides/framework-components/)和[客戶端腳本](/zh-tw/guides/client-side-scripts/)。甚至可當作局部 UI 模板，不需要提供整個頁面。
...
<ReadMore>進一步了解[插槽](/zh-tw/basics/astro-components/#slots)。</ReadMore>
...
2. 預設的 [`<slot />`](/zh-tw/basics/astro-components/#slots)，指名頁面的 Markdown/MDX 內容要在哪個位置渲染。
