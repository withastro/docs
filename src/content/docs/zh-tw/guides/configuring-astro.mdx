...
要自訂 Astro 怎麽運作可以在 Astro 專案加入 `astro.config.mjs` 檔案。這個在 Astro 專案是一個常見的檔案，所有官方的範例模板和主題預設都帶有一個。
...
Astro 會自動嘗試在專案根目錄解析叫做 `astro.config.mjs` 的設定檔案。如果在專案根目錄找不到設定檔案，會使用 Astro 的預設選項。
