...
你可以透過 [GitHub Actions](https://github.com/features/actions) 將 Astro 網站自動搭建並部署到 GitHub Pages 網站上。為了這麼做，請將網站的原始碼託管至 GitHub。

Astro 維護了官方的 `withastro/action` 工具幫助部署專案。按照下方的說明就能快速的將 Astro 網站部署進 GitHub pages。如果需要更多資訊時，可以檢視[這個 README](https://github.com/withastro/action)。
...
你可能需要為 `base` 設定內容，這樣 Astro 才會將儲存庫名稱（例如 `/my-repo`）視為網站的根目錄。
...
為了設定 Astro 在 GitHub Pages 上使用你的自訂網域，請將你的網域名稱設定成 `site` 的值，並請不要設定 `base`：
