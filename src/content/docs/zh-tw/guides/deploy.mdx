...
Astro 網站可透過儀錶板或 CLI 快速構建並部署到數個主機上。
...
1. 選擇支援**持續部署**的服務主機（例如 [Netlify](/zh-tw/guides/deploy/netlify/) 或 [Vercel](/zh-tw/guides/deploy/vercel/)），接著引入你的 Git 程式庫作為新網站或專案。

    許多常見的服務主機都能辨識出 Astro 網站，並依此選擇適當的設定以構建及部署，如下所示（若不同的話，也可修改這些設定）：

    :::note[部署設定]
    - **構建指令：** `astro build` 或 `npm run build`
    - **發布目錄：** `dist`
    :::

1. 點選「Deploy」，接著你的新網站就會在服務主機上建立並分配到專屬的 URL（例如 `new-astro-site.netlify.app`）。
...
3. 構建網站並部署到服務主機
    ...
    其他服務主機則會要求你先[在本地端構建你的網站](#在本地端構建你的網站)，接著用命令列部署。

## 在本地端構建你的網站

許多服務主機像是 Netlify 和 Vercel 會為你構建網站並發布建立好的輸出檔到網路上。但有些網站會要求你在本地端構建，接著執行部署指令或上傳你建立好的輸出檔。

你可能也會希望先在本地端構建、預覽你的網站，或在自己的環境下找出任何潛在的錯誤或警告。

執行 `npm run build` 指令以構建 Astro 網站。
...
構建後的輸出檔預設會放在 `dist/`，目錄位置可透過 [`outDir` 設定](/zh-tw/reference/configuration-reference/#outdir)調整。

## 為 SSR 新增配接器

:::note
以 [SSR（伺服器端渲染）](/zh-tw/guides/server-side-rendering/)部署 Astro 網站之前，請先確定以下事項：

- 在專案相依模組安裝[適當的配接器](/zh-tw/guides/server-side-rendering/)（手動安裝，或用配接器的 `astro add` 指令，例如 `npx astro add netlify`）。
