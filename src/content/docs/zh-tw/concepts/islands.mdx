...
Astro 以首個內建選擇性水合作用的主流 JavaScript web 框架之姿而流行，運用的正是 Sylor-Miller 最初提出的同一個元件群島模式。此後，我們在 Sylor-Miller 的原創成果上持續擴展與演進，這有助於啟發以類似的元件群島方式來處理動態伺服器端算繪內容。

## 什麼是群島？

在 Astro 中，群島是原本靜態的 HTML 頁面上的增強 UI 元件。

[**客戶端群島**](#客戶端群島)是一個可互動的 JavaScript UI 元件，獨立於頁面其餘部分進行注水；而[**伺服器群島**](#伺服器群島)則是一個 UI 元件，獨立於頁面其餘部分在伺服器端算繪其動態內容。

這兩種群島都以元件為單位，獨立執行較耗費資源或較慢的程序，以優化頁面載入。
...
頁面的主要內容可以先以預留內容（例如通用頭像）立即算繪，直到島嶼本身的內容準備就緒。有了伺服器群島，含有少量個人化內容的小元件不會延遲原本靜態頁面的算繪。
...
另一個優勢是提供優秀的訪客體驗。伺服器群島經過優化且載入迅速，通常甚至在瀏覽器繪製頁面之前就已完成。而在島嶼算繪所需的短暫時間內，你可以顯示自訂的備用內容，防止任何版面位移。
