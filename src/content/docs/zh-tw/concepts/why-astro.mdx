...
- **[伺服器優先](/zh-tw/basics/rendering-modes/)**：避免在客戶端執行耗費資源的算繪。
...
1. **[以內容為主](#content-driven)**：Astro 專為內容豐富的網站設計。
2. **[伺服器優先](#server-first)**：由伺服器算繪 HTML 的網站會執行得更加快速。
3. **[預設即迅速](#fast-by-default)**：不太可能用 Astro 寫出緩慢的網站。
4. **[簡單易使用](#easy-to-use)**：你不需要是專家，也能用 Astro。
5. **[注重開發者](#developer-focused)**：你應該享有成功打造網站的所需資源。

### 以內容為主 \{#content-driven\}
...
**Astro 盡可能使用[伺服器端算繪](/zh-tw/basics/rendering-modes/)而非客戶端算繪**。這和傳統的伺服器端框架（PHP、WordPress、Laravel 與 Ruby on Rails 等）過去幾十年的使用方法相同。你毋須再學習另一套伺服語言，因為 Astro 依然使用 HTML、CSS 以及 JavaScript（或是 TypeScript，如果你比較喜歡的話）。
