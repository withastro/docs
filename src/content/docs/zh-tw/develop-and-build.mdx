...
要檢查網站在建置時會建立的版本，請離開開發伺服器（<kbd>Ctrl</kbd> + <kbd>C</kbd>）並在終端機上執行你使用的套件管理器對應的指令：To check the version of your site that will be created at build time, quit the dev server (<kbd>Ctrl</kbd> + <kbd>C</kbd>) and run the appropriate build command for your package manager in your terminal:
