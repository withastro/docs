...
要修改專案，請在程式碼編輯器上開啟專案資料夾。在有開發伺服器運作的開發模式下做事讓你可以看到程式碼修改對網站的更新。To make changes to your project, open your project folder in your code editor. Working in development mode with the dev server running allows you to see updates to your site as you edit the code.

你也可以自訂開發環境的樣子，例如設定 TS 或安裝官方 Astro。You can also [customize aspects of your development environment](#configure-your-dev-environment) such as configuring TypeScript or installing the official Astro editor extensions.

### 啟動 Astro 開發伺服器

Astro 附帶內建的開發伺服器，有專案開發所需的所有東西。指令會啟動本地的開發伺服器，Astro comes with a built-in development server that has everything you need for project development. The `astro dev` CLI command will start the local development server so that you can see your new website in action for the very first time.
