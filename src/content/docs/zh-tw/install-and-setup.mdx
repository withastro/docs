...
你可以在啟動新 Astro 專案的同時，在 `create astro` 指令中傳遞 `--add` 參數來安裝任何[官方整合](/zh-tw/guides/integrations/)或支援 `astro add` 指令的社群整合。
...
2. 安裝 Astro
    ...
    :::note[重要]
    Astro 必須在本地安裝，而非全域安裝。請確保你**沒有**執行 `npm install -g astro`、`pnpm add -g astro`、`bun add -g astro` 或 `yarn add global astro`。
    :::
