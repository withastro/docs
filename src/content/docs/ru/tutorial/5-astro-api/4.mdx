...
4. Перейдите на `localhost:4321/rss.xml` и убедитесь, что вы видите (неформатированный) текст на странице с `item` для каждого из ваших файлов `.md`. Каждый элемент должен содержать информацию о посте в блоге, такую как `title`, `url` и `description`.
