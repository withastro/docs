...
2. Проверьте предварительный просмотр в браузере по адресу `localhost:3000/posts/post-1`. Теперь вы должны видеть содержимое, отрендеренное:
...
5. Снова проверьте предварительный просмотр в браузере по адресу `localhost:3000/posts/post-1` и убедитесь, что эта строка больше не отображается и что ваш заголовок отображается только один раз. Внесите все необходимые корректировки, чтобы убедиться, что у вас нет дублирующегося контента.
...
- [Вкладывание макетов в Astro](/en/core-concepts/layouts/#nesting-layouts)
