...
Используйте хелпер Astro `getViteConfig()` в вашем конфигурационном файле [`vitest.config.ts`](https://vitest.dev/config/), чтобы настроить Vitest с учётом настроек вашего проекта Astro:
...
Cypress — это инструмент для фронтального тестирования, созданный для современного веба. Cypress позволяет писать сквозные тесты для вашего сайта Astro.
...
Вы можете запустить тесты с помощью [расширения NightwatchJS для VSCode](https://marketplace.visualstudio.com/items?itemName=browserstackcom.nightwatch) или используя приведённые ниже шаги CLI:
...
Вы можете начать и запускать ваши тесты с помощью [расширения VSCode](https://playwright.dev/docs/getting-started-vscode).
