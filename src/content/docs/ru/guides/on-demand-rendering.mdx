...
Вы можете подключить любой из [официальных адаптеров, поддерживаемых Astro](/ru/guides/integrations/#official-integrations) с помощью команды `astro add`. Она автоматически установит адаптер и внесет все необходимые изменения в файл `astro.config.mjs`.
...
Вы также можете [добавить адаптер вручную, установив NPM-пакет](/ru/guides/integrations/#installing-an-npm-package) (например, `@astrojs/netlify`) и самостоятельно обновив файл `astro.config.mjs`.
