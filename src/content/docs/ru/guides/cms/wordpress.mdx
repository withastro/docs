...
2. **Сайт WordPress** - Your site's REST API is `[YOUR_SITE]/wp-json/wp/v2/` и он доступен по умолчанию на любом сайте WordPress. Также можно [настроить WordPress на локальном компьютере](https://wordpress.org/support/article/installing-wordpress-on-your-own-computer/).
...
Получайте данные WordPress через уникальный REST API URL вашего сайта и маршрут для вашего контента. (Для блога это обычно будет `posts`.) После этого можно отобразить свойства данных, с помощью директивы Astro `set:html={}`.
...
Через этот API вам доступно большое количество данных, поэтому, вероятно, вы захотите получить только определенные поля. Вы можете ограничить ответ, добавив параметр [`_fields`](https://developer.wordpress.org/rest-api/using-the-rest-api/global-parameters/#_fields) к API URL, например: `[YOUR-SITE]/wp/v2/posts?_fields=author,id,excerpt,title,link`
...
В этом примере получение данных осуществляется через общедоступный WordPress API [https://norian.studio/dinosaurs/](https://norian.studio/dinosaurs/). Этот сайт WordPress сохраняет информацию о каждом отдельном динозавре в маршруте `dinos`, точно так же, как блог хранит отдельные записи блога в маршруте `posts`.

Этот пример показывает, как воспроизвести структуру этого сайта в Astro: индексную страницу, которая перечисляет динозавров со ссылками на динамически создаваемые отдельные страницы динозавров.
...
В этом примере показывается получение данных с сайта WordPress, типы записей которого уже настроены и доступны для REST API.
:::

### Отображение списка постов WordPress

Страница `src/pages/index.astro` перечисляет каждого динозавра с описанием и ссылкой на его собственную страницу.
...
Получение данных через API возвращает объект, внутри которого есть свойства:
...
Страница `src/pages/dinos/[slug].astro` [динамически создает страницу](/ru/core-concepts/routing/#dynamic-routes) для каждого динозавра.
...
Возвращается свойство `_embedded['wp:featuredmedia']['0'].media_details.sizes.medium.source_url`, которое можно использовать для отображения изображения на каждой странице динозавра. (Замените `medium` на нужный вам размер изображения.)
...
- [Soft Hard System](https://softhardsystem.com/) от Рафида Мухюмина Вафи — [исходный код на GitHub](https://github.com/RafidMuhymin/softhardsystem)
- [Dinos!](https://wc-dinos.netlify.app/) от Аниндо Неел Дутты — [исходный код на GitHub](https://github.com/leen-neel/astro-wordpress)
