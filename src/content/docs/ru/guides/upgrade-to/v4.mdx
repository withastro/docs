...
## Обновление Astro \{#upgrade-astro\}
...
Astro v4.0 включает [потенциально разрушающие изменения](#breaking-changes), а также [удаление некоторых ранее устаревших функций](#previously-deprecated-features-now-removed). 
...
## Удалены экспериментальные флаги Astro v4.0 \{#astro-v40-experimental-flags-removed\}
...
### Обновлено: Vite 5.0 \{#upgraded-vite-50\}
...
### Обновлено: зависимости unified, remark и rehype \{#upgraded-unified-remark-and-rehype-dependencies\}
...
### Переименовано: `entrypoint` (API интеграций) \{#renamed-entrypoint-integrations-api\}
...
#### Что мне делать? \{#what-should-i-do-2\}
...
### Изменено: сигнатура `app.render` в API интеграций \{#changed-apprender-signature-in-integrations-api\}
...
+ app.render(request, { routeData, locals })
```
### Изменено: адаптеры теперь должны указывать поддерживаемые функции \{#changed-adapters-must-now-specify-supported-features\}
...
#### Что мне делать? \{#what-should-i-do-4\}
...
### Удалено: свойство `path` языка Shiki \{#removed-shiki-language-path-property\}
...
## Утратило актуальность \{#deprecated\}
...
### Утратило актуальность: `handleForms` для событий `submit` Анимаций Переходов \{#deprecated-handleforms-for-view-transitions-submit-events\}
...
## Ранее устаревшие функции теперь удалены \{#previously-deprecated-features-now-removed\}
...
### Удалено: возврат простых объектов из конечных точек \{#removed-returning-simple-objects-from-endpoints\}
...
### Удалено: `build.split` и `build.excludeMiddleware` \{#removed-buildsplit-and-buildexcludemiddleware\}
...
### Удалено: `Astro.request.params` \{#removed-astrorequestparams\}
...
### Удалено: использование `rss` в `getStaticPaths()` \{#removed-using-rss-in-getstaticpaths\}
...
#### Что делать? \{#what-should-i-do-12\}
...
### Удалено: имена HTTP-методов в нижнем регистре \{#removed-lowercase-http-method-names\}
...
#### Что делать? \{#what-should-i-do-13\}
...
### Удалено: 301 редирект при отсутствии префикса `base` \{#removed-301-redirects-when-missing-a-base-prefix\}
...
#### Что делать? \{#what-should-i-do-14\}
...
### Удалено: автоконверсия `astro/client-image` \{#removed-astroclient-image-auto-conversion\}
...
#### Что делать? \{#what-should-i-do-15\}
