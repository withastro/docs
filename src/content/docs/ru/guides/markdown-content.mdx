...
Astro предоставляет страницам Markdown и MDX специальное свойство frontmatter `layout`, которое может указывать относительный путь (или [псевдоним](/ru/guides/aliases/)) к компоненту [макета Astro](/ru/basics/layouts/#markdownmdx-макеты).
...
<ReadMore>Узнайте больше о [Макетах Markdown](/ru/basics/layouts/#markdownmdx-макеты).</ReadMore>
...
MDX поддерживает использование операторов `export` для добавления переменных в содержимое MDX. Эти переменные доступны как в самом шаблоне, так и в виде именованных свойств при [импорте файла](#importing-markdown) куда-либо еще.
...
Вы можете импортировать одну конкретную страницу с помощью оператора `import` или несколько страниц с помощью [`Astro.glob()`](/ru/guides/imports/#astroglob).
...
Когда вы импортируете Markdown и MDX файлы в компонент Astro, вы получаете объект, содержащий их [экспортированные свойства](#exported-properties).
...
Посмотрите [свойства, экспортируемые в компонент макета Astro](/ru/basics/layouts/#markdown-layout-props), когда используется особый [макет frontmatter](#frontmatter-layout) Astro.
:::
...
Вместо того чтобы помещать файлы Markdown/MDX в каталог `src/pages/` для создания маршрутов страниц, вы можете [генерировать страницы динамически] (/ru/guides/routing/#dynamic-routes).
...
При рендеринге импортированного MDX-контента [пользовательские компоненты](#assigning-custom-components-to-html-elements) могут быть переданы через свойство `components`.
...
Astro внедряет атрибут `id` во все элементы заголовков (от `<h1>` до `<h6>`) в файлах Markdown и MDX и предоставляет утилиту `getHeadings()` для извлечения этих идентификаторов в [экспортируемых свойствах Markdown](#exported-properties).
...
Вы можете добавить свойства frontmatter во все ваши файлы Markdown и MDX с помощью плагина [remark или rehype](#markdown-plugins).
...
Теперь каждый файл Markdown или MDX будет содержать `customProperty` в своем frontmatter, что делает его доступным при [импорте вашего markdown](#importing-markdown) и из [свойства `Astro.props.frontmatter` в ваших макетах](#frontmatter-layout).

<RecipeLinks slugs={["ru/recipes/reading-time"]} />

### Расширение конфигурации Markdown из MDX

Интеграция Astro в MDX по умолчанию расширяет [существующую конфигурацию Markdown вашего проекта](/en/reference/configuration-reference/#markdown-options). Чтобы отменить отдельные опции, вы можете указать их эквивалент в конфигурации MDX.
...
- содержимого внутри [встроенного компонента `<Code />`](/ru/reference/api-reference/#code-) (powered by Shiki).
- содержимое компонента [`<Prism />`](/ru/reference/api-reference/#prism-) (работает на Prism).
...
3. Загрузите ее в `<head>` вашей страницы через [компонент макета](/en/basics/layouts/) с помощью тега `<link>`. (См. [Базовое использование Prism](https://prismjs.com/#basic-usage).)
