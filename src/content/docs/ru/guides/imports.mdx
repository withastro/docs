...
Astro использует ESM, те же [`import`](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Statements/import#syntax) и [`export`](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Statements/export) операторы, которые поддерживаются в браузерах.
...
Astro включает встроенную поддержку TypeScript. Вы можете импортировать `.ts` и `.tsx` файлы напрямую в ваш проект Astro и даже писать TypeScript-код прямо внутри [скрипта компонента Astro](/ru/basics/astro-components/#the-component-script) и любых [тегах скриптов](/ru/guides/client-side-scripts/).
