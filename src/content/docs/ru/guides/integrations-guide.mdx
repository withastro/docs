...
Astro включает команду `astro add` для автоматической установки официальных интеграций. Некоторые плагины сообщества также могут быть добавлены с помощью этой команды. Пожалуйста, проверьте в документации к каждой интеграции, поддерживается ли `astro add`, или вы должны [установить вручную](#manual-installation).
...
Ознакомьтесь со справочником [пл API Интеграций](/ru/reference/integrations-reference/), чтобы узнать, что могут делать интеграции и как написать свою собственную.