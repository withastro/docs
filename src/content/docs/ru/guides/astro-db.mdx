---
title: 'Astro DB'
description: Узнайте, как использовать Astro DB, полностью управляемую SQL-базу данных, специально разработанную для Astro.
...
Добавьте Astro DB в новый или существующий проект Astro (требуется `astro@4.5` или более поздняя версия) с помощью интеграции [`@astrojs/db`](/ru/guides/integrations-guide/db/) (`v0.8.0` или более поздняя версия). Astro включает встроенную команду `astro add` для автоматизации процесса установки.
...
При желании вы можете [установить `@astrojs/db` вручную](/ru/guides/integrations-guide/db/#manual-installation).

## Определите вашу базу данных
...
Таблицы обеспечивают автоматическую поддержку TypeScript для вашего контента в Astro. Когда вы определяете таблицу, Astro автоматически генерирует и применяет к ней интерфейс TypeScript. В результате при запросе данных вы получаете полную поддержку TypeScript, включая автозаполнение свойств и проверку типов.

Чтобы настроить отдельную таблицу базы данных, импортируйте и используйте утилиты `defineTable()` и `column` из `astro:db`.

В этом примере настраивается таблица `Comment` с обязательными текстовыми столбцами для `author` и `body`, а затем она становится доступной для вашего проекта через экспорт `defineDb()`.
...
<ReadMore>См. [справочник по конфигурации таблиц](/ru/guides/integrations-guide/db/#table-configuration-reference) для полного описания параметров таблиц.</ReadMore>
...
Вы можете определить эти таблицы отношений и сохранить их в схеме базы данных с помощью **столбцов ссылок**. Чтобы установить отношения, вам понадобятся:
...
Вы можете запросить вашу базу данных с любой [страницы Astro](/ru/basics/astro-pages/#astro-pages) или [конечной точки](/ru/guides/endpoints/) в вашем проекте, используя предоставленные `db` ORM и конструктор запросов.
...
1. с помощью [Astro Studio CLI](/ru/reference/cli-reference/#astro-studio-cli) с использованием команды `astro link`
...
См. [Справочник по CLI](/ru/guides/integrations-guide/db/#astro-db-cli-reference) для полного списка команд.
...
Файлы конфигурации [config](#define-your-database) и [seed](#seed-your-database) для интеграции следуют тому же формату, что и их пользовательские аналоги.
