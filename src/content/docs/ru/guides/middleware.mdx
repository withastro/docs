...
2. Внутри этого файла экспортируйте функцию [`onRequest()`](/en/reference/modules/astro-middleware/#onrequest), которая может передавать [объект `context`](#объект-context) и функцию `next()`. Это не должно быть экспортом по умолчанию.
