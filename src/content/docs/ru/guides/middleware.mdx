...
2. Внутри этого файла экспортируйте функцию [`onRequest()`](/ru/reference/api-reference/#onrequest), которой можно передать объект [`context`](#the-context-object) и функцию `next()`. Это не должен быть экспорт по умолчанию.
...
Объект [`context`](/ru/reference/api-reference/#endpoint-context) содержит информацию, которая будет доступна другому промежуточному ПО, маршрутам API и маршрутам `.astro` в процессе рендеринга.
