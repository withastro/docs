...
Настроив маршрутизацию i18n, вы теперь можете вычислять ссылки на страницы вашего сайта, используя вспомогательные функции, такие как [`getRelativeLocaleURL()`](/ru/reference/api-reference/#getrelativelocaleurl), доступные из [`astro: i18n`](/ru/reference/api-reference/#internationalization-astroi18n). Эти сгенерированные ссылки всегда будут обеспечивать правильный локализованный маршрут и могут помочь вам правильно использовать или проверять URL-адреса на вашем сайте.
...
Вы можете отказаться от такого поведения, [установив `redirectToDefaultLocale: false`](/ru/ссылка/configuration-reference/#i18nroutingredirecttodefaultlocale). Это позволяет вам иметь домашнюю страницу сайта, которая существует за пределами настроенной вами структуры папок локали.
...
## Пути к пользовательским локалям
...
При использовании функций из [виртуального модуля `astro: i18n`](/ru/reference/api-reference/#internationalization-astroi18n) для вычисления допустимых URL-путей на основе вашей конфигурации (например, `getRelativeLocaleUrl()`), [используйте `path` как значение `locale`](/ru/reference/api-reference/#getlocalebypath).
...
- Функция адаптера [`functionPerRoute`](/ru/reference/adapter-reference/#functionperroute) не поддерживается.
