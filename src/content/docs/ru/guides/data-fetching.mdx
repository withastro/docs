...
Ваш задеплоенный Astro сайт выполнит запрос на получение данных **один раз, во время сборки**. Во время разработки, вы увидите запросы данных когда компонент обновляется. Если вам нужно обновлять данные несколько раз на стороне клиента, используйте [компоненты фреймворка](/ru/basics/framework-components/) или [клиентские скрипты](/ru/guides/client-side-scripts/) в Astro компоненте.
:::


## `fetch()` в компонентах фреймворка

Функция `fetch()` доступна глобально в любом [компоненте фреймворка](/ru/basics/framework-components/):
...
Astro компоненты могут получать данные с вашей CMS и затем рендерить контент на странице. Используя [динамические роуты](/ru/basics/routing/#динамические-переадресации), компоненты даже могут генерировать страницы основываясь на контенте с вашей CMS.
