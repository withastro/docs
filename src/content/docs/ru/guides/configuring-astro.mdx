...
Каждый стартовый проект Astro включает файл `tsconfig.json`. [Скрипты компонентов Astro](/ru/basics/astro-components/#the-component-script) используют TypeScript, что обеспечивает работу инструментов редактора и позволяет вам при необходимости добавить проверку типов для вашего кода.
