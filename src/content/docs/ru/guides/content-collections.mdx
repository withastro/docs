...
При использовании `type: 'content'` каждая запись контента генерирует свойство `slug`, удобное для URL, из своего [file `id`](/ru/reference/modules/astro-content/#id). Slug используется для запроса записи непосредственно из вашей коллекции. Он также полезен при создании новых страниц и URL-адресов из вашего контента.
...
Astro предоставляет две функции для запроса коллекции и возврата одной (или нескольких) записей содержимого: [`getCollection()`](/ru/reference/modules/astro-content/#getcollection) и [`getEntry()`](/ru/reference/modules/astro-content/#getentry).
...
Обе функции возвращают записи содержимого, определенные типом [`CollectionEntry`](/ru/reference/modules/astro-content/#collectionentry).
