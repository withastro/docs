...
Посмотрите, как преобразовать базовый пример блога из `src/pages/posts/` в `src/content/posts` в нашем [пошаговом руководстве](/rututorial/6-islands/4/), которое использует кодовую базу из [готового проекта учебного пособия Создать Блог](https://github.com/withastro/blog-tutorial-demo).
