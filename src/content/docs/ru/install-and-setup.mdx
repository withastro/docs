...
Вы можете создать новый Astro-проект и сразу установить любые [официальные интеграции](/ru/guides/integrations/) или интеграции от сообщества, которые поддерживают команду `astro add`, передав аргумент `--add` в команду `create astro`.
...
2. Установите Astro
    ...
    :::note[Важно]
    Astro должен быть установлен локально, а не глобально. Убедитесь, что вы *не* запускаете `npm install -g astro`, `pnpm add -g astro`, `bun add -g astro` или `yarn add global astro`.
    :::
