...
**Когда:** При инициализации, перед тем как [Vite](https://vitejs.dev/config/) или [конфигурация Astro ](/ru/reference/configuration-reference/) будут разрешены.

**Зачем:** Для расширения конфигурации проекта. Это включает обновление [конфигурации Astro](/ru/reference/configuration-reference/), применение [плагинов Vite](https://vitejs.dev/guide/api-plugin.html), добавление рендеров компонентов и внедрение скриптов на страницу.
```ts
'astro:config:setup'?: (options: {
  config: AstroConfig;
  command: 'dev' | 'build' | 'preview';
...
Например, предположим, что вам нужно добавить в проект пользователя плагин [Vite](https://vitejs.dev/):
...
Параметр **`stage`** указывает на то, как должен быть вставлен этот скрипт (`content`). Некоторые этапы позволяют вставлять скрипты без модификации, другие - оптимизировать во время [шага сборки Vite](https://vitejs.dev/guide/build.html):
...
**Когда:**  Сразу после создания сервера Vite в режиме "dev", но перед событием `listen()`. [См. API createServer Vite](https://vitejs.dev/guide/api-javascript.html#createserver) для получения дополнительной информации.
...
**Тип:** [`ViteDevServer`](https://vitejs.dev/guide/api-javascript.html#vitedevserver)
...
Адрес, семейство и номер порта, предоставленные [модулем Net NodeJS](https://nodejs.org/api/net.html).
...
**Тип:** [`RouteData[]`](#routedata-type-reference)
