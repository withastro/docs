...
`Astro.props` — это объект, содержащий все значения, которые были переданы в качестве [атрибутов компонента](/ru/basics/astro-components/#component-props). Компоненты макета для файлов `.md` и `.mdx` получают в качестве реквизитов значения frontmatter.
...
<ReadMore>Узнайте больше о том, как [макеты Markdown и MDX](/ru/guides/markdown-content/#frontmatter-layout) обрабатывают свойства.</ReadMore>

<ReadMore>Узнайте, как добавить [определения типов TypeScript для ваших свойств](/ru/guides/typescript/#component-props).</ReadMore>

### `Astro.params`

`Astro.params` — это объект, содержащий значения сегментов динамического маршрута, найденных для данного запроса.

В статических сборках это будут `params`, возвращаемые `getStaticPaths()`, используемые для пререндеринга [динамических маршрутов](/ru/guides/routing/#dynamic-routes).
...
:::note
Это для продвинутых случаев использования! В большинстве случаев проще выводить содержимое слота с помощью [элемента `<slot />`](/ru/basics/astro-components/#slots).
:::
...
См. также: [Передача данных с `props`](#data-passing-with-props)
...
- `Заголовки` — Сгенерированный список заголовков, [зеркальное отражение утилиты Astro `getHeadings()`](/ru/guides/markdown-content/#exported-properties) при импорте Markdown и MDX.
- `remarkPluginFrontmatter` — Измененный объект frontmatter после применения любых [плагинов remark или rehype](/ru/guides/markdown-content/#modifying-frontmatter-programmatically). Устанавливается в тип `any`.
...
Низкоуровневый API для создания [`APIContext`](#endpoint-context) для передачи в функцию промежуточного ПО Astro `onRequest()`.
...
Подробнее об [использовании фрагментов](/ru/basics/astro-syntax/#fragments) смотрите в разделе синтаксис Astro.
