...
`Astro.props` — это объект, содержащий все значения, которые были переданы в качестве [атрибутов компонента](/ru/basics/astro-components/#входные-параметры-компонента). Компоненты макета для файлов `.md` и `.mdx` получают в качестве реквизитов значения frontmatter.
...
<ReadMore>Узнайте, как добавить [определения типов TypeScript для ваших свойств](/ru/guides/typescript/#входные-параметры-компонента).</ReadMore>
...
- `remarkPluginFrontmatter` — Измененный объект frontmatter после применения любых [плагинов remark или rehype](/ru/guides/markdown-content/#внедрение-frontmatter). Устанавливается в тип `any`.
