...
**Тип:** <code>(key: string, options?: <a href="#astrocookiegetoptions">AstroCookieGetOptions</a>) => <a href="#astrocookie-type">AstroCookie</a> | undefined</code>
</p>

Получает куки как объект [`AstroCookie`](#astrocookie-type), который содержит `value` и утилиты для преобразования куки в другие типы, отличные от строки.
...
Как только куки «удалён» (истёк), `Astro.cookies.has()` возвращает `false`, а `Astro.cookies.get()` — [`AstroCookie`](#astrocookie-type) с `value`, равным `undefined`. Доступные опции при удалении куки: `domain`, `path`, `httpOnly`, `sameSite` и `secure`.
