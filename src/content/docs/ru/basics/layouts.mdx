...
**Макеты** - это [компоненты Astro](/ru/basics/astro-components/), используемые для обеспечения повторного использования структуры пользовательского интерфейса (UI), такой как шаблон страницы.
...
Макеты страниц особенно полезны для [Markdown и MDX файлов](/ru/guides/markdown-content/#markdown-и-mdx-страницы), которые иначе не имели бы какого-либо форматирования. 
...
:::note
Макет Markdown/MDX будет иметь доступ ко всем [экспортированным свойствам](/ru/guides/markdown-content/#exported-properties) своего файла  из `Astro.props` **с некоторыми ключевыми отличиями:**.
...
*   Значения, определенные вне frontmatter (например, операторы `export` в MDX), недоступны. Вместо этого рассмотрите [импорт макета](#importing-layouts-manually-mdx).
:::
...
---
макет: ../../layouts/BaseLayout.astro
