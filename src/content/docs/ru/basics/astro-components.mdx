...
Чтобы передать несколько HTML-элементов в заполнитель `<slot/>` компонента без обёртывающего `<div>`, используйте атрибут `slot=""` на компоненте [`<Fragment/>` Astro](/ru/basics/astro-syntax/#fragments):
