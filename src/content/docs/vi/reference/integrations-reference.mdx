...
This reference page is for anyone writing their own integration. To learn how to use an integration in your project, check out our [Using Integrations](/vi/guides/integrations-guide/) guide instead.
...
- **Libraries:** [`partytown`](/vi/guides/integrations-guide/partytown/)
- **Features:** [`sitemap`](/vi/guides/integrations-guide/sitemap/)
...
**When:** On initialization, before either the [Vite](https://vite.dev/config/) or [Astro config](/vi/reference/configuration-reference/) have resolved.

**Why:** To extend the project config. This includes updating the [Astro config](/vi/reference/configuration-reference/), applying [Vite plugins](https://vite.dev/guide/api-plugin.html), adding component renderers, and injecting scripts onto the page.
...
A read-only copy of the user-supplied [Astro config](/vi/reference/configuration-reference/). This is resolved _before_ any other integrations have run. If you need a copy of the config after all integrations have completed their config updates, [see the `astro:config:done` hook](#astroconfigdone).
...
A callback function to update the user-supplied [Astro config](/vi/reference/configuration-reference/). Any config you provide **will be merged with the user config + other integration config updates,** so you are free to omit keys!
...
Adds a [custom client directive](/vi/reference/directives-reference/#custom-client-directives) to be used in `.astro` files.
...
Adds a [custom dev toolbar app](/vi/reference/dev-toolbar-app-reference/).
...
Adds [middleware](/vi/guides/middleware/) to run on each request. Takes the `entrypoint` module that contains the middleware, and an `order` to specify whether it should run before (`pre`) other middleware or after (`post`).
...
A callback function to inject routes into an Astro project. Injected routes can be [`.astro` pages](/vi/basics/astro-pages/) or [`.js` and `.ts` route handlers](/vi/guides/endpoints/#static-file-endpoints).
...
**Why:** To set options for a route at build or request time, such as enabling [on-demand server rendering](/vi/guides/on-demand-rendering/#enabling-on-demand-rendering).
...
The `prerender` property is used to configure [on-demand server rendering](/vi/guides/on-demand-rendering/#enabling-on-demand-rendering) for a route. If the route file contains an explicit `export const prerender` value, the value will be used as the default instead of `undefined`.
...
If the final value after running all the hooks is `undefined`, the route will fall back to a prerender default based on the [`output` option](/vi/reference/configuration-reference/#output): prerendered for `static` mode, and on-demand rendered for `server` mode.
...
A read-only copy of the user-supplied [Astro config](/vi/reference/configuration-reference/). This is resolved _after_ other integrations have run.
...
Makes the integration an adapter. Read more in the [adapter API](/vi/reference/adapter-reference/).
...
A mutable instance of the Vite server used in "dev" mode. For instance, this is [used by our Partytown integration](/vi/guides/integrations-guide/partytown/) to inject the Partytown server as middleware:
...
An object providing callback functions to interact with the [dev toolbar](/vi/reference/dev-toolbar-app-reference/):
...
The loader can then access the `refreshContextData` property to get the webhook body. See the [`refreshContextData`](/vi/reference/content-loader-reference/#refreshcontextdata) property for more information.
...
**Why:** To set up any global objects or clients needed during a production build. This can also extend the build configuration options in the [adapter API](/vi/reference/adapter-reference/).
...
Exposes the [middleware](/vi/guides/middleware/) file path.
...
:::caution
This property is deprecated since v5.0. Check the [migration guide](/vi/guides/upgrade-to/v5/#deprecated-routes-on-astrobuilddone-hook-integration-api).
:::
...
An instance of the Astro logger, useful to write logs. This logger uses the same [log level](/vi/reference/cli-reference/#--verbose)
configured via CLI.
...
Determines whether the route use [on demand rendering](/vi/guides/on-demand-rendering/). The value will be `true` for projects configured with:
...
Allows you to access the route `params`. For example, when a project uses the following [dynamic routes](/vi/guides/routing/#dynamic-routes) `/pages/[lang]/[...slug].astro`, the value will be `['lang', '...slug']`.
...
For regular routes, the value will be the URL pathname where this route will be served. When the project uses [dynamic routes](/vi/guides/routing/#dynamic-routes) (ie. `[dynamic]` or `[...spread]`), the pathname will be undefined.
...
[The `astro add` command](/vi/reference/cli-reference/#astro-add) allows users to easily add integrations and adapters to their project. If you want _your_ integration to be installable with this tool, **add `astro-integration` to the `keywords` field in your `package.json`**:
