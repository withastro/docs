...
Astro is designed to make it easy to deploy to any cloud provider for on-demand rendering, also known as server-side rendering (SSR). This ability is provided by __adapters__, which are [integrations](/vi/reference/integrations-reference/). See the [on-demand rendering guide](/vi/guides/on-demand-rendering/) to learn how to use an existing adapter.

## What is an adapter?

An adapter is a special kind of [integration](/vi/reference/integrations-reference/) that provides an entrypoint for server rendering at request time. An adapter does two things:
...
An adapter is an [integration](/vi/reference/integrations-reference/) and can do anything that an integration can do.
...
The client IP address that will be made available as [`Astro.clientAddress`](/vi/reference/api-reference/#clientaddress) in pages, and as `ctx.clientAddress` in API routes and middleware.
...
The [`context.locals` object](/vi/reference/api-reference/#locals) used to store and access information during the lifecycle of a request.

The example below reads a header named `x-private-header`, attempts to parse it as an object and pass it to `locals`, which can then be passed to any [middleware function](/vi/guides/middleware/).
...
Provide a value for [`integrationRouteData`](/vi/reference/integrations-reference/#integrationroutedata-type-reference) if you already know the route to render. Doing so will bypass the internal call to [`app.match`](#appmatch) to determine the route to render.
...
[The `astro add` command](/vi/reference/cli-reference/#astro-add) allows users to easily add integrations and adapters to their project. If you want _your_ adapter to be installable with this tool, **add `astro-adapter` to the `keywords` field in your `package.json`**:
...
Then, consume the hook [`astro:build:ssr`](/vi/reference/integrations-reference/#astrobuildssr), which will give you a `middlewareEntryPoint`, an `URL` to the physical file on the file system.
