...
This experimental server-side API unlocks a variety of potential future uses, but is currently scoped to allow [testing of `.astro` component output](/vi/guides/testing/#vitest-and-container-api) in `vite` environments such as `vitest`.
...
Enables rendering components using [HTML streaming](/vi/guides/on-demand-rendering/#html-streaming).

### `renderers` option

**Type:** `AddServerRenderer[]`

A list of loaded client renderers required by the component. Use this if your `.astro` component renders any [UI framework components](/vi/guides/framework-components/) or MDX using an official Astro integration (e.g. React, Vue, etc.).
...
For [on-demand rendered applications](/vi/guides/on-demand-rendering/), or cases where the container is called at runtime or inside other "shells" (e.g. PHP, Ruby, Java, etc.), renderers must be manually imported.
...
Server renderers are required to build your project, and must be stored in the container for every framework used. Client renderers are additionally needed to any hydrate client-side components using [`client:*` directives](/vi/reference/directives-reference/#client-directives).
...
An option to pass content to be rendered with [`<slots>`](/vi/basics/astro-components/#slots).
...
An option to pass [properties](/vi/basics/astro-components/#component-props) for Astro components.
...
An object to pass information about the path parameter to an Astro component responsible for [generating dynamic routes](/vi/guides/routing/#dynamic-routes).
...
An option to pass information from [`Astro.locals`](/vi/reference/api-reference/#locals) for rendering your component.
...
An option available when using `renderToResponse` to specify that you are rendering an [endpoint](/vi/guides/endpoints/):
...
Whether or not the Container API renders components as if they were [page partials](/vi/basics/astro-pages/#page-partials). This is usually the behavior you want when rendering `components.boolean` so you can render components without a full page shell.
