...
The following reference covers all supported configuration options in Astro. To learn more about configuring Astro, read our guide on [Configuring Astro](/vi/guides/configuring-astro/).
...
Deploy to your favorite server, serverless, or edge host with build adapters. Import one of our first-party adapters ([Cloudflare](/vi/guides/integrations-guide/cloudflare/), [Netlify](/vi/guides/integrations-guide/netlify/), [Node.js](/vi/guides/integrations-guide/node/), [Vercel](/vi/guides/integrations-guide/vercel/)) or explore [community adapters](https://astro.build/integrations/2/?search=&categories%5B%5D=adapters) to enable on-demand rendering in your Astro project.

See our [on-demand rendering guide](/vi/guides/on-demand-rendering/) for more on Astro's server rendering options.
...
Read our [Integrations Guide](/vi/guides/integrations-guide/) for help getting started with Astro Integrations.
...
See [the sessions guide](/vi/guides/sessions/) for more information.
...
The Unstorage driver to use for session storage.  The [Node](/vi/guides/integrations-guide/node/#sessions),
[Cloudflare](/vi/guides/integrations-guide/cloudflare/#sessions), and
[Netlify](/vi/guides/integrations-guide/netlify/#sessions) adapters automatically configure a default driver for you,
...
Set `session.ttl` to add a default expiration period for your session values. Passing a `ttl` option to [`session.set()`](/vi/reference/api-reference/#set) will override the global default
...
See the [Prefetch guide](/vi/guides/prefetch/) for more information.
...
See the [code syntax highlighting guide](/vi/guides/syntax-highlighting/) for usage and examples.
...
- `prism` - use the [Prism](https://prismjs.com/) highlighter and [provide your own Prism stylesheet](/vi/guides/syntax-highlighting/#add-a-prism-stylesheet)
...
See our guide for more information on [internationalization in Astro](/vi/guides/internationalization/)
...
When [`i18n.fallback`](#i18nfallback) is configured to avoid showing a 404 page for missing page routes, this option controls whether to [redirect](/vi/guides/routing/#redirects) to the fallback page, or to [rewrite](/vi/guides/routing/#rewrites) the fallback page's content in place.
...
With the following configuration, if you have the file `src/pages/vi/about.astro` but not `src/pages/fr/about.astro`, the `astro build` command will generate `dist/fr/about.html` with the same content as the `dist/vi/about.html` page.
...
Both page routes built and URLs returned by the `astro:i18n` helper functions [`getAbsoluteLocaleUrl()`](/vi/reference/modules/astro-i18n/#getabsolutelocaleurl) and [`getAbsoluteLocaleUrlList()`](/vi/reference/modules/astro-i18n/#getabsolutelocaleurllist) will use the options set in `i18n.domains`.

See the [Internationalization Guide](/vi/guides/internationalization/#domains) for more details, including the limitations of this feature.
...
See our guide for more information on [environment variables in Astro](/vi/guides/environment-variables/).
