...
These directives control how [UI Framework components](/vi/guides/framework-components/) are hydrated on the page.
...
A client directive can only be used on a UI framework component that is directly imported into a `.astro` component.  Hydration directives are not supported when using [dynamic tags](/vi/reference/astro-syntax/#dynamic-tags) and [custom components passed via the `components` prop](/vi/guides/integrations-guide/mdx/#custom-components-with-imported-mdx).
...
Visit the [`addClientDirective` API](/vi/reference/integrations-reference/#addclientdirective-option) page to learn more about creating a custom client directive.
...
<ReadMore>See more about using [server island components](/vi/guides/server-islands/).</ReadMore>
...
<ReadMore>See the [Styling & CSS](/vi/guides/styling/#global-styles) page for more details about how global styles work.</ReadMore>
...
The one exception is using the [`define:vars` directive](/vi/reference/directives-reference/#definevars) on the `<style>` tag, which does not automatically imply `is:inline`.
:::
...
<ReadMore>See how [client-side scripts](/vi/guides/client-side-scripts/) work in Astro components.</ReadMore>
...
For scripts, try [passing variables to scripts manually](/vi/guides/client-side-scripts/#pass-frontmatter-variables-to-scripts) instead.
:::
