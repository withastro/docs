...
**Layouts** là các [component Astro](/en/basics/astro-components/) được sử dụng để cung cấp cấu trúc UI có thể tái sử dụng, chẳng hạn như một template trang web.

Chúng ta thường sử dụng thuật ngữ "layout" cho các thành phần Astro cung cấp các phần tử UI chung được chia sẻ trên nhiều trang như header, thanh điều hướng và footer. Một thành phần layout Astro điển hình cung cấp cho [các trang Astro, Markdown hoặc MDX](/en/basics/astro-pages/) với:
- một **page shell** (các thẻ `<html>`, `<head>` và `<body>`)
- một [**`<slot />`**](/en/basics/astro-components/#slots) để chỉ định nơi nội dung trang riêng lẻ sẽ được chèn vào.

Tuy nhiên, không có gì đặc biệt về một thành phần layout! Chúng có thể [nhận props](/en/basics/astro-components/#component-props) và [import và sử dụng các thành phần khác](/en/basics/astro-components/#component-structure) như bất kỳ thành phần Astro nào khác. Chúng có thể bao gồm [các thành phần UI framework](/en/guides/framework-components/) và [client-side scripts](/en/guides/client-side-scripts/). Chúng thậm chí không cần phải cung cấp một page shell đầy đủ, và thay vào đó có thể được sử dụng như các template UI một phần.
...
Các thành phần layout thường được đặt trong thư mục `src/layouts` trong dự án của bạn để tổ chức, nhưng điều này không bắt buộc; bạn có thể chọn đặt chúng ở bất cứ đâu trong dự án của mình. Bạn thậm chí có thể đặt các thành phần layout cùng với các trang của mình bằng cách [thêm tiền tố `_` vào tên layout](/en/guides/routing/#excluding-pages).
...
<ReadMore>Tìm hiểu thêm về [slots](/en/basics/astro-components/#slots).</ReadMore>
...
Astro cung cấp một thuộc tính frontmatter đặc biệt `layout` dành cho [các tệp `.md` riêng lẻ nằm trong `src/pages/` sử dụng file-based routing](/en/guides/markdown-content/#individual-markdown-pages) để chỉ định thành phần `.astro` nào sẽ được sử dụng làm page layout. Thành phần này cho phép bạn cung cấp nội dung `<head>` như meta tags (ví dụ: `<meta charset="utf-8">`) và styles cho trang Markdown. Theo mặc định, thành phần được chỉ định này có thể tự động truy cập dữ liệu từ tệp Markdown.

Điều này không được nhận dạng là một thuộc tính đặc biệt khi sử dụng [content collections](/en/guides/content-collections/) để truy vấn và render nội dung của bạn.
...
2. Một [`<slot />`](/en/basics/astro-components/#slots) mặc định để chỉ ra nơi nội dung Markdown của trang sẽ được render.
...
Bạn có thể thiết lập [`Props` type](/en/guides/typescript/#component-props) của layout với helper `MarkdownLayoutProps`:
...
:::note
Một Markdown layout sẽ có quyền truy cập vào tất cả [các thuộc tính có sẵn](/en/guides/markdown-content/#available-properties) của tệp Markdown từ `Astro.props` **với hai điểm khác biệt chính:**
...
<ReadMore>Tìm hiểu thêm về hỗ trợ Markdown và MDX của Astro trong [hướng dẫn Markdown](/en/guides/markdown-content/) của chúng tôi.</ReadMore>
