...
<ReadMore>Tìm hiểu thêm trong [tài liệu tham khảo cú pháp Astro](/en/reference/astro-syntax/).</ReadMore>
:::
...
Khi Astro component của bạn cần tương tác phía client, bạn có thể thêm [thẻ `<script>` HTML tiêu chuẩn](/en/guides/client-side-scripts/) hoặc [UI Framework components](/en/guides/framework-components/#hydrating-interactive-components) dưới dạng "client islands".

Đối với các components cần render nội dung được cá nhân hóa hoặc dynamic, bạn có thể trì hoãn việc server rendering của chúng bằng cách thêm [server directive](/en/reference/directives-reference/#server-directives). Các "server islands" này sẽ render nội dung của chúng khi có sẵn, mà không làm chậm việc tải toàn bộ trang.
...
<ReadMore>Đọc thêm về [hỗ trợ TypeScript tích hợp sẵn](/en/guides/typescript/) của Astro.</ReadMore>
:::
...
Tuy nhiên, [cú pháp component template của Astro](/en/reference/astro-syntax/) cũng hỗ trợ **JavaScript expressions**, thẻ Astro [`<style>`](/en/guides/styling/#styling-in-astro) và [`<script>`](/en/guides/client-side-scripts/#using-script-in-astro), **imported components**, và [**special Astro directives**](/en/reference/directives-reference/). Data và values được định nghĩa trong component script có thể được sử dụng trong component template để tạo ra HTML được tạo động.
...
Pattern này là cơ sở của một [Astro layout component](/en/basics/layouts/): toàn bộ trang nội dung HTML có thể được "wrapped" với thẻ `<SomeLayoutComponent></SomeLayoutComponent>` và gửi đến component để render bên trong các page elements chung được định nghĩa ở đó.
...
Để truyền nhiều HTML elements vào placeholder `<slot/>` của component mà không có `<div>` bọc ngoài, sử dụng attribute `slot=""` trên [Astro's `<Fragment/>` component](/en/reference/astro-syntax/#fragments):
...
:::tip
Named slots cũng có thể được truyền đến [UI framework components](/en/guides/framework-components/)!
:::
...
- Bất kỳ thẻ `<script>` nào đều được để unbundled, được xử lý như thể chúng có [directive `is:inline`](/en/reference/directives-reference/#isinline).
- Chúng chỉ có thể [tham chiếu assets nằm trong thư mục `public/`](/en/basics/project-structure/#public).

:::note
Một [element `<slot />`](/en/basics/astro-components/#slots) bên trong HTML component sẽ hoạt động như nó sẽ làm trong Astro component. Để sử dụng element [HTML Web Component Slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) thay thế, thêm `is:inline` vào element `<slot>` của bạn.
:::

## Các bước tiếp theo

<ReadMore>Đọc thêm về việc sử dụng [UI framework components](/en/guides/framework-components/) trong dự án Astro của bạn.</ReadMore>