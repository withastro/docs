...
:::tip
Để có thêm chức năng, chẳng hạn như bao gồm các component và biểu thức JSX trong Markdown, hãy thêm tích hợp [`@astrojs/mdx`](/en/guides/integrations-guide/mdx/) để viết nội dung Markdown của bạn bằng [MDX](https://mdxjs.com/).
:::
...
Markdown cục bộ có thể được nhập vào các component `.astro` bằng lệnh `import` cho một tệp duy nhất và [Vite's `import.meta.glob()`](/en/guides/imports/#importmetaglob) để truy vấn nhiều tệp cùng một lúc. [Dữ liệu được xuất từ các tệp Markdown này](#importing-markdown) sau đó có thể được sử dụng trong component `.astro`.

Nếu bạn có các nhóm tệp Markdown liên quan, hãy xem xét [định nghĩa chúng như collections](/en/guides/content-collections/). Điều này mang lại cho bạn một số lợi thế, bao gồm khả năng lưu trữ các tệp Markdown ở bất kỳ đâu trên hệ thống tệp hoặc từ xa.
...
<ReadMore>Xem thêm về [khi nào nên sử dụng content collections](/en/guides/content-collections/#when-to-create-a-collection) thay vì nhập tệp.</ReadMore>
...
Hàm [`render()`](/en/reference/modules/astro-content/#render) trả về nội dung body Markdown của bạn, danh sách các tiêu đề được tạo, cũng như đối tượng frontmatter đã được sửa đổi sau khi bất kỳ plugin remark hoặc rehype nào được áp dụng.

<ReadMore>Đọc thêm về [sử dụng nội dung được trả về bởi truy vấn collections](/en/guides/content-collections/#using-content-in-astro-templates).</ReadMore>
...
Bạn cũng có thể [render nội dung HTML của một mục content collection Markdown](/en/guides/content-collections/#rendering-body-content) bằng cách render component `<Content />`.
...
Bạn có thể tùy chỉnh cách remark phân tích Markdown của bạn trong `astro.config.mjs`. Xem danh sách đầy đủ [các tùy chọn cấu hình Markdown](/en/reference/configuration-reference/#markdown-options).

### Thêm plugin remark và rehype

Astro hỗ trợ thêm các plugin [remark](https://github.com/remarkjs/remark) và [rehype](https://github.com/rehypejs/rehype) của bên thứ ba cho Markdown. Các plugin này cho phép bạn mở rộng Markdown với các khả năng mới, như [tự động tạo mục lục](https://github.com/remarkjs/remark-toc), [áp dụng nhãn emoji dễ truy cập](https://github.com/florianeckerstorfer/remark-a11y-emoji), và [styling Markdown của bạn](/en/guides/styling/#markdown-styling).
...
Tích hợp MDX của Astro sẽ mở rộng [cấu hình Markdown hiện có của dự án bạn](/en/reference/configuration-reference/#markdown-options) theo mặc định. Để ghi đè các tùy chọn riêng lẻ, bạn có thể chỉ định tương đương của chúng trong cấu hình MDX của mình.
...
Để tránh mở rộng cấu hình Markdown từ MDX, đặt [tùy chọn `extendMarkdownConfig`](/en/guides/integrations-guide/mdx/#extendmarkdownconfig) (được bật theo mặc định) thành `false`:
...
Astro coi [bất kỳ tệp được hỗ trợ nào bên trong thư mục `/src/pages/`](/en/basics/astro-pages/#supported-page-files) như một trang, bao gồm `.md` và các loại tệp Markdown khác.
...
Để giúp với chức năng hạn chế của các trang Markdown riêng lẻ, Astro cung cấp thuộc tính frontmatter `layout` đặc biệt là đường dẫn tương đối đến [component layout Markdown](/en/basics/layouts/#markdown-layouts) của Astro. `layout` không phải là thuộc tính đặc biệt khi sử dụng [content collections](/en/guides/content-collections/) để truy vấn và render nội dung Markdown của bạn, và không được đảm bảo hỗ trợ bên ngoài trường hợp sử dụng dự định của nó.
...
Component layout này là một component Astro thông thường với [các thuộc tính cụ thể tự động có sẵn](/en/basics/layouts/#markdown-layout-props) thông qua `Astro.props` cho template Astro của bạn. Ví dụ, bạn có thể truy cập các thuộc tính frontmatter của tệp Markdown thông qua `Astro.props.frontmatter`:
...
Khi sử dụng thuộc tính frontmatter `layout`, bạn phải bao gồm thẻ `<meta charset="utf-8">` trong layout của mình vì Astro sẽ không thêm nó tự động nữa. Bây giờ bạn cũng có thể [style Markdown của mình](/en/guides/styling/#markdown-styling) trong component layout.

<ReadMore>Tìm hiểu thêm về [Layout Markdown](/en/basics/layouts/#markdown-layouts).</ReadMore>

## Lấy Markdown từ xa

Astro không bao gồm hỗ trợ tích hợp cho Markdown từ xa bên ngoài [content collections](/en/guides/content-collections/).
