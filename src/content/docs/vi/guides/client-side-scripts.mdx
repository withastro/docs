...
<ReadMore>Xem trang [tham khảo directives](/en/reference/directives-reference/#script--style-directives) của chúng tôi để biết thêm thông tin về các directives có sẵn trên thẻ `<script>`.</ReadMore>
...
Trong các component Astro, code trong [frontmatter](/en/basics/astro-components/#the-component-script) giữa các `---` fences chạy trên server và không có sẵn trong trình duyệt. Để gửi variables từ server đến client, chúng ta cần một cách để lưu trữ các variables và sau đó đọc chúng khi JavaScript chạy trong trình duyệt.
...
Các elements được render bởi UI framework có thể chưa có sẵn khi thẻ `<script>` thực thi. Nếu script của bạn cũng cần xử lý [UI framework components](/en/guides/framework-components/), việc sử dụng custom element được khuyến nghị.