...
Adapter này cho phép Astro triển khai [các tuyến đường và tính năng render theo yêu cầu](/en/guides/on-demand-rendering/) của bạn lên [Vercel](https://www.vercel.com/), bao gồm [server islands](/en/guides/server-islands/), [actions](/en/guides/actions/), và [sessions](/en/guides/sessions/).
...
Tìm hiểu cách triển khai trang web Astro của bạn trong [hướng dẫn triển khai Vercel](/en/guides/deploy/vercel/) của chúng tôi.
...
<ReadMore>Tìm hiểu thêm về [triển khai dự án của bạn lên Vercel](/en/guides/deploy/vercel/).</ReadMore>
...
Các thuộc tính `domains` và `remotePatterns` sẽ tự động được điền bằng [cài đặt `image` tương ứng của Astro](/en/reference/configuration-reference/#image-options).
...
Khi được bật, một [Image Service](/en/reference/image-service-reference/) được hỗ trợ bởi API Tối ưu hóa Hình ảnh của Vercel sẽ được tự động cấu hình và sử dụng trong production. Trong development, image service được chỉ định bởi [`devImageService`](#devimageservice) sẽ được sử dụng thay thế.
...
Lưu ý rằng các request ISR function không bao gồm search params, tương tự như [requests](/en/reference/api-reference/#request) trong static mode.
...
Khi sử dụng sessions trên Vercel, bạn cần [cấu hình driver](/en/reference/configuration-reference/#sessiondriver) cho lưu trữ session. Bạn có thể cài đặt storage provider từ [Vercel marketplace](https://vercel.com/marketplace?category=storage).
