...
:::caution[Removed]
`@astrojs/prefetch` has been replaced by the [built-in `prefetch` feature](/vi/guides/prefetch/) introduced in Astro 3.5. See the [migration guide](/vi/guides/prefetch/#migrating-from-astrojsprefetch) for instructions on updating an older project.
