...
Với Astro Sitemap, bạn không phải lo lắng về việc tự tạo tệp XML này: tích hợp Astro Sitemap sẽ crawl các tuyến đường được tạo tĩnh của bạn và tạo tệp sitemap, bao gồm [dynamic routes](/en/guides/routing/#dynamic-routes) như `[...slug]` hoặc `src/pages/[lang]/[version]/info.astro` được tạo bởi `getStaticPaths()`.

Tích hợp này không thể tạo sitemap entries cho dynamic routes trong [SSR mode](/en/guides/on-demand-rendering/).
...
Thêm URL trang web của bạn như tùy chọn [`site`](/en/reference/configuration-reference/#site) trong `astro.config.mjs`. Điều này phải bắt đầu với `http://` hoặc `https://`.
