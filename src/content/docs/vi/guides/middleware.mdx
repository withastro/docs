...
2. Trong file này, export một hàm [`onRequest()`](/en/reference/modules/astro-middleware/#onrequest) có thể nhận một [đối tượng `context`](#đối-tượng-context) và hàm `next()`. Đây không được là default export.
...
Đối tượng [`context`](/en/reference/api-reference/) bao gồm thông tin sẽ được cung cấp cho các middleware khác, API routes và `.astro` routes trong quá trình hiển thị.
...
Đối tượng `locals` này được chuyển tiếp qua quá trình xử lý yêu cầu và có sẵn như một thuộc tính cho [`APIContext`](/en/reference/api-reference/#locals) và [`AstroGlobal`](/en/reference/api-reference/#locals). Điều này cho phép dữ liệu được chia sẻ giữa các middleware, API routes và trang `.astro`. Điều này hữu ích để lưu trữ dữ liệu cụ thể cho yêu cầu, chẳng hạn như dữ liệu người dùng, qua bước hiển thị.

:::tip[Thuộc tính integration]
[Integrations](/en/guides/integrations-guide/) có thể thiết lập thuộc tính và cung cấp chức năng thông qua đối tượng `locals`. Nếu bạn đang sử dụng một integration, hãy kiểm tra tài liệu của nó để đảm bảo bạn không ghi đè bất kỳ thuộc tính nào của nó hoặc thực hiện công việc không cần thiết.
:::
...
Nhiều middleware có thể được kết hợp theo thứ tự được chỉ định bằng [`sequence()`](/en/reference/modules/astro-middleware/#sequence):
...
`APIContext` hiển thị một phương thức được gọi là `rewrite()` hoạt động giống như [Astro.rewrite](/en/guides/routing/#rewrites).

Sử dụng `context.rewrite()` bên trong middleware để hiển thị nội dung của một trang khác mà không [chuyển hướng](/en/guides/routing/#dynamic-redirects) khách truy cập của bạn đến một trang mới. Điều này sẽ kích hoạt một giai đoạn hiển thị mới, khiến bất kỳ middleware nào được thực thi lại.
...
Hàm `next()` chấp nhận cùng payload của [hàm `Astro.rewrite()`](/en/reference/api-reference/#rewrite). Vị trí của đường dẫn rewrite có thể được cung cấp dưới dạng string, URL hoặc `Request`.
