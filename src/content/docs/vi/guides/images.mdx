...
<ReadMore>Xem tài liệu tham khảo API đầy đủ cho các component [`<Image />`](/en/reference/modules/astro-assets/#image-) và [`<Picture />`](/en/reference/modules/astro-assets/#picture-).</ReadMore>
...
Bạn cũng có thể chọn lưu trữ hình ảnh từ xa, trong [hệ thống quản lý nội dung (CMS)](/en/guides/cms/) hoặc nền tảng [quản lý tài sản kỹ thuật số (DAM)](/en/guides/media/).
...
Component `<Image />` chấp nhận [nhiều thuộc tính component](/en/reference/modules/astro-assets/#image-properties) cũng như bất kỳ thuộc tính nào được chấp nhận bởi thẻ HTML `<img>`.
...
<ReadMore>Xem chi tiết về [thuộc tính component `<Picture />`](/en/reference/modules/astro-assets/#picture-properties) trong tài liệu tham khảo `astro:assets`.</ReadMore>

### Hiển thị hình ảnh chưa xử lý với thẻ HTML `<img>`

[Cú pháp template Astro](/en/reference/astro-syntax/) cũng hỗ trợ viết thẻ `<img>` trực tiếp, với toàn quyền kiểm soát output cuối cùng. Những hình ảnh này sẽ không được xử lý và tối ưu. Nó chấp nhận tất cả thuộc tính thẻ HTML `<img>`, và thuộc tính duy nhất bắt buộc là `src`.

Hình ảnh cục bộ phải được import từ đường dẫn tương đối từ file `.astro` hiện tại, hoặc bạn có thể cấu hình và sử dụng [alias import](/en/guides/imports/#aliases). Sau đó, bạn có thể truy cập `src` và các thuộc tính khác của hình ảnh để sử dụng trong thẻ `<img>`.
...
Tham chiếu default import của bất kỳ file `.svg` cục bộ nào. Vì import này được coi như một component Astro, bạn phải sử dụng các quy ước tương tự (ví dụ viết hoa) như khi [sử dụng dynamic tags](/en/reference/astro-syntax/#dynamic-tags).
...
Sử dụng cú pháp Markdown tiêu chuẩn `![alt](src)` trong file `.md`. Cú pháp này hoạt động với [Image Service API](/en/reference/image-service-reference/) của Astro để tối ưu hình ảnh cục bộ được lưu trong `src/` và hình ảnh từ xa. Hình ảnh được lưu trong thư mục `public/` không bao giờ được tối ưu.
...
Các component `<Image />` và `<Picture />` không khả dụng trong file `.md`. Nếu bạn cần kiểm soát nhiều hơn thuộc tính hình ảnh, chúng tôi khuyến nghị sử dụng [tích hợp MDX của Astro](/en/guides/integrations-guide/mdx/) để thêm hỗ trợ cho định dạng file `.mdx`. MDX cho phép [tùy chọn hình ảnh bổ sung khả dụng trong MDX](#hình-ảnh-trong-file-mdx), bao gồm kết hợp component với cú pháp Markdown.
...
Nhưng bạn có thể truyền nội dung tĩnh được tạo bởi `<Image />` đến một component framework bên trong file `.astro` [như children](/en/guides/framework-components/#passing-children-to-framework-components) hoặc sử dụng [named `<slot/>`](/en/guides/framework-components/#can-i-use-astro-components-inside-my-framework-components):
...
[Sharp](https://github.com/lovell/sharp) là dịch vụ hình ảnh mặc định được sử dụng cho `astro:assets`. Bạn có thể cấu hình thêm dịch vụ hình ảnh bằng tùy chọn [`image.service`](/en/reference/configuration-reference/#imageservice).
...
Thư mục cache mặc định là `./node_modules/.astro`, tuy nhiên điều này có thể được thay đổi bằng cài đặt cấu hình [`cacheDir`](/en/reference/configuration-reference/#cachedir).
