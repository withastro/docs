...
[Component trang](/en/basics/astro-pages/) `.astro` cũng như các file Markdown và MDX (`.md`, `.mdx`) trong thư mục `src/pages/` **tự động trở thành các trang trên website của bạn**. Route của mỗi trang tương ứng với đường dẫn và tên file trong thư mục `src/pages/`.
...
:::tip
Không có "cấu hình routing" riêng để duy trì trong dự án Astro! Khi bạn thêm file vào thư mục `src/pages/`, một route mới sẽ được tự động tạo. Trong static build, bạn có thể tùy chỉnh định dạng file output bằng tùy chọn cấu hình [`build.format`](/en/reference/configuration-reference/#buildformat).
:::
...
Tham số có thể được bao gồm trong các phần riêng biệt của đường dẫn. Ví dụ, file `src/pages/[lang]/[version]/info.astro` với cùng `getStaticPaths()` ở trên sẽ tạo các route `/en/v1/info` và `/fr/v2/info`.
...
<ReadMore>Tìm hiểu thêm về [`getStaticPaths()`](/en/reference/routing-reference/#getstaticpaths).</ReadMore>
...
Trong ví dụ sau, tham số rest (`[...slug]`) và tính năng [`props`](/en/reference/routing-reference/#data-passing-with-props) của `getStaticPaths()` tạo trang cho slug ở các độ sâu khác nhau.
...
Bạn có thể chỉ định mapping các chuyển hướng vĩnh viễn trong cấu hình Astro với giá trị [`redirects`](/en/reference/configuration-reference/#redirects).
...
Việc viết lại cho phép bạn phục vụ route khác mà không chuyển hướng trình duyệt đến trang khác. Trình duyệt sẽ hiển thị địa chỉ gốc trong thanh URL, nhưng thay vào đó sẽ hiển thị nội dung của URL được cung cấp cho [`Astro.rewrite()`](/en/reference/api-reference/#rewrite).
...
<ReadMore>Xem [tham khảo API `Astro.rewrite()`](/en/reference/api-reference/#rewrite) để biết thêm thông tin.</ReadMore>
...
- `_server_islands/`: Phục vụ các component động được trì hoãn vào [server island](/en/guides/server-islands/).
- `_actions/`: Phục vụ bất kỳ [action](/en/guides/actions/) được định nghĩa nào.
...
<ReadMore>Tìm hiểu thêm về [prop `page` phân trang](/en/reference/routing-reference/#the-pagination-page-prop).</ReadMore>
