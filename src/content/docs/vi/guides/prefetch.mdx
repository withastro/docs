...
Chiến lược prefetch mặc định khi thêm thuộc tính `data-astro-prefetch` là `hover`. Để thay đổi nó, bạn có thể cấu hình [`prefetch.defaultStrategy`](/en/reference/configuration-reference/#prefetchdefaultstrategy) trong file `astro.config.mjs`:
...
Nếu bạn muốn prefetch tất cả liên kết, bao gồm cả những liên kết không có thuộc tính `data-astro-prefetch`, bạn có thể đặt [`prefetch.prefetchAll`](/en/reference/configuration-reference/#prefetchprefetchall) thành `true`:
...
Với flag thử nghiệm [`clientPrerender`](/en/reference/experimental-flags/client-prerender/) được kích hoạt, bạn có thể sử dụng tùy chọn `eagerness` trên `prefetch()` để gợi ý cho trình duyệt mức độ tích cực nó nên prefetch/prerender các mục tiêu liên kết.
...
Khi bạn sử dụng [View Transitions](/en/guides/view-transitions/) trên một trang, prefetching cũng sẽ được kích hoạt theo mặc định. Nó đặt cấu hình mặc định là `{ prefetchAll: true }` cho phép [prefetching cho tất cả liên kết](#prefetch-tất-cả-liên-kết-theo-mặc-định) trên trang.
...
Nó cũng hoàn toàn hỗ trợ `<script type="speculationrules">` từ [Speculation Rules API](https://developer.mozilla.org/en-US/docs/Web/API/Speculation_Rules_API), có thể được sử dụng để mô tả thêm [các chiến lược và quy tắc prefetching](#eagerness), nâng cao trải nghiệm người dùng cho người dùng Chrome của bạn. Bạn sẽ cần kích hoạt thử nghiệm [`clientPrerender`](/en/reference/experimental-flags/client-prerender/) để sử dụng chức năng này với `prefetch()`
