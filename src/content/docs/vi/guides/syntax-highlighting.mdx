...
Bạn có thể [thêm Prism stylesheet và chuyển sang highlighting của Prism](#add-a-prism-stylesheet), hoặc vô hiệu hóa syntax highlighting của Astro hoàn toàn, với tùy chọn cấu hình [`markdown.syntaxHighlight`](/en/reference/configuration-reference/#markdownsyntaxhighlight).

<ReadMore>Xem tham chiếu đầy đủ [`markdown.shikiConfig`](/en/reference/configuration-reference/#markdownshikiconfig) để biết tập hợp đầy đủ các tùy chọn syntax highlighting cho Markdown khi sử dụng Shiki.</ReadMore>
...
```
<ReadMore>Xem tham chiếu đầy đủ [Shiki config](/en/reference/configuration-reference/#markdownshikiconfig) để biết tập hợp đầy đủ các tùy chọn cho Markdown code blocks.</ReadMore>
...
Bạn có thể tham chiếu `Props` của các component này bằng cách sử dụng utility type [`ComponentProps`](/en/guides/typescript/#componentprops-type).
...
2. Thêm stylesheet này vào [thư mục `public/` của dự án bạn](/en/basics/project-structure/#public).

3. Load nó vào `<head>` của trang trong [layout component](/en/basics/layouts/) thông qua thẻ `<link>`. (Xem [Prism basic usage](https://prismjs.com/#basic-usage).)
</Steps>
