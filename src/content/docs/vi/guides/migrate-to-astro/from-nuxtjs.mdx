...
- Các dự án Astro cũng có thể là SSG hoặc [SSR với prerendering cấp trang](/en/guides/on-demand-rendering/).
- Astro sử dụng routing dựa trên file, và [cho phép các trang có tên đặc biệt tạo routes động](/en/guides/routing/#dynamic-routes).
- Astro [dựa trên component](/en/basics/astro-components/), và cấu trúc markup của bạn sẽ tương tự trước và sau khi di chuyển.
- Astro có [tích hợp chính thức để sử dụng các component Vue](/en/guides/integrations-guide/vue/).
- Astro hỗ trợ [cài đặt các package NPM](/en/guides/imports/#npm-packages), bao gồm các thư viện Vue. Bạn có thể giữ một số hoặc tất cả các component Vue và dependencies hiện tại.
...
- [Page Routing](/en/basics/astro-pages/#file-based-routing): Nuxt sử dụng `vue-router` cho SPA routing, và `vue-meta` để quản lý `<head>`. Trong Astro, bạn sẽ tạo các route trang HTML riêng biệt và kiểm soát `<head>` của trang trực tiếp, hoặc trong component layout.

- [content-driven](/en/concepts/why-astro/#content-driven): Astro được thiết kế để showcase nội dung của bạn và cho phép bạn chọn tương tác chỉ khi cần thiết. Ứng dụng Nuxt hiện tại có thể được xây dựng cho tương tác client-side cao. Astro có khả năng tích hợp để làm việc với nội dung của bạn, chẳng hạn như tạo trang, nhưng có thể cần các kỹ thuật Astro nâng cao để bao gồm các mục khó sao chép hơn bằng các component `.astro`, chẳng hạn như dashboards.
...
Bạn có thể truyền đối số `--template` cho lệnh `create astro` để bắt đầu dự án Astro mới với một trong những starter chính thức của chúng tôi (ví dụ: `docs`, `blog`, `portfolio`). Hoặc, bạn có thể [bắt đầu dự án mới từ bất kỳ repository Astro hiện có nào trên GitHub](/en/install-and-setup/#install-from-the-cli-wizard).
...
Bạn có thể thấy hữu ích khi cài đặt một số [integrations tùy chọn của Astro](/en/guides/integrations-guide/) để sử dụng khi chuyển đổi dự án Nuxt sang Astro:
...
Bạn có thể thấy hữu ích khi bắt đầu bằng cách chuyển đổi layouts và templates của Nuxt thành [component layout Astro](/en/basics/layouts/).
...
Trong NuxtJS, [trang](/en/basics/astro-pages/) của bạn nằm trong `/pages`. Trong Astro, tất cả nội dung trang của bạn phải nằm trong `src/` trừ khi bạn đang sử dụng [content collections](/en/guides/content-collections/).
...
Các [trang `.astro`](/en/basics/astro-pages/) này phải được đặt trong `src/pages/` và sẽ có các routes trang được tạo tự động dựa trên đường dẫn file của chúng.
...
Khi là một phần của [content collection](/en/guides/content-collections/), bạn sẽ [tạo trang động](/en/guides/content-collections/#generating-routes-from-content) từ các mục nội dung của bạn.
...
Xem [hướng dẫn testing của Astro](/en/guides/testing/) để biết thêm.
...
Nếu cần thiết, cập nhật bất kỳ [file imports](/en/guides/imports/) nào để tham chiếu đường dẫn file tương đối chính xác. Điều này có thể được thực hiện bằng cách sử dụng [import aliases](/en/guides/typescript/#import-aliases), hoặc bằng cách viết ra đường dẫn tương đối đầy đủ.
...
- [Sử dụng SSR](/en/guides/on-demand-rendering/).
- Export hàm `getStaticPaths()` trong frontmatter của trang Astro để báo cho framework biết [static routes để tạo động](/en/guides/routing/#dynamic-routes).
...
[Astro hỗ trợ các CSS preprocessors phổ biến nhất](/en/guides/styling/#css-preprocessors) bằng cách cài đặt chúng như dev dependency. Ví dụ, để sử dụng SCSS:
...
Chuyển đổi bất kỳ [component `<nuxt-img/>` hoặc `<nuxt-picture/>` của Nuxt](https://image.nuxtjs.org/components/nuxt-img) thành [component image riêng của Astro](/en/guides/images/) trong file `.astro` hoặc `.mdx`, hoặc thành [thẻ HTML `<img>`](/en/guides/images/#images-in-ui-framework-components) hoặc `<picture>` tiêu chuẩn tùy theo trong các component Vue của bạn.

Component `<Image />` của Astro chỉ hoạt động trong file `.astro` và `.mdx`. Xem [danh sách đầy đủ các thuộc tính component](/en/reference/modules/astro-assets/#image-properties) và lưu ý rằng một số sẽ khác với các thuộc tính của Nuxt.
...
Bạn có thể tìm hiểu thêm về [sử dụng images trong Astro](/en/guides/images/) trong Images Guide.
...
Nếu bạn tìm thấy (hoặc tạo!) video hoặc bài blog hữu ích về việc chuyển đổi trang Nuxt sang Astro, [thêm nó vào danh sách này](https://github.com/withastro/docs/edit/main/src/content/docs/en/guides/migrate-to-astro/from-nuxtjs.mdx)!
:::