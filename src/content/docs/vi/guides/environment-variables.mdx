...
- `import.meta.env.BASE_URL`: URL cơ sở mà trang web của bạn được phục vụ từ đó. Điều này được xác định bởi [tùy chọn cấu hình `base`](/en/reference/configuration-reference/#base).
- `import.meta.env.SITE`: Được đặt thành [tùy chọn `site`](/en/reference/configuration-reference/#site) được chỉ định trong `astro.config` của dự án.
- `import.meta.env.ASSETS_PREFIX`: Tiền tố cho các liên kết asset được tạo bởi Astro nếu tùy chọn cấu hình [`build.assetsPrefix`](/en/reference/configuration-reference/#buildassetsprefix) được đặt. Điều này có thể được sử dụng để tạo các liên kết asset không được xử lý bởi Astro.
...
Các lệnh `astro dev` và `astro build` mặc định tương ứng với chế độ `"development"` và `"production"`. Bạn có thể chạy các lệnh này với [cờ `--mode`](/en/reference/cli-reference/#--mode-string) để truyền giá trị khác cho `mode` và tải file `.env` tương ứng.
...
<ReadMore>Đang phát triển adapter? Xem cách [làm cho adapter tương thích với `astro:env`](/en/reference/adapter-reference/#envgetsecret).</ReadMore>
...
- **Biến server bí mật**: Các biến này không phải là một phần của bundle cuối cùng và có thể được truy cập trên server thông qua module `astro:env/server`:
   ...
   Theo mặc định, secrets chỉ được xác thực tại runtime. Bạn có thể bật xác thực biến private khi khởi động bằng cách [cấu hình `validateSecrets: true`](/en/reference/configuration-reference/#envvalidatesecrets).
...
<ReadMore>Để xem danh sách đầy đủ các trường xác thực, xem [tham chiếu API `envField`](/en/reference/configuration-reference/#envschema).</ReadMore>
...
<ReadMore>Tìm hiểu thêm trong [tham chiếu API](/en/reference/modules/astro-env/#getsecret).</ReadMore>
