...
Cần nâng cấp dự án cũ hơn lên v4 trước? Xem [hướng dẫn di chuyển cũ hơn](/en/guides/upgrade-to/v4/) của chúng tôi.
...
Bạn cũng có thể [nâng cấp các integration Astro thủ công](/en/guides/integrations-guide/#manual-upgrading) nếu cần, và bạn cũng có thể cần nâng cấp các dependency khác trong dự án.
...
<ReadMore>Đọc thêm về [syntax highlighting trong Astro](/en/guides/syntax-highlighting/).</ReadMore>
...
<ReadMore>Đọc thêm về [Markdown layouts](/en/basics/layouts/#markdown-layouts).</ReadMore>
...
<ReadMore>Đọc thêm về [sử dụng Markdown plugins trong Astro](/en/guides/markdown-content/#markdown-plugins).</ReadMore>
...
<ReadMore>Đọc thêm về [thiết lập endpoint để sử dụng cho tối ưu hóa hình ảnh](/en/reference/configuration-reference/#imageendpoint).</ReadMore>
...
<ReadMore>Đọc thêm về [tùy chọn cấu hình `build` trong Astro](/en/reference/configuration-reference/#build-options).</ReadMore>
...
<ReadMore>Đọc thêm về [cài đặt cấu hình Vite trong Astro](/en/reference/configuration-reference/#vite).</ReadMore>
...
<ReadMore>Đọc thêm về [pagination trong Astro](/en/guides/routing/#pagination).</ReadMore>
...
<ReadMore>Đọc thêm về [sử dụng HTML attributes trong Astro](/en/reference/astro-syntax/#dynamic-attributes).</ReadMore>
...
<ReadMore>Xem thêm về [lưu trữ dữ liệu trong `context.locals`](/en/guides/middleware/#storing-data-in-contextlocals).</ReadMore>
...
<ReadMore>Đọc thêm về [tạo dynamic routes với `params`](/en/guides/routing/#static-ssg-mode).</ReadMore>
...
<ReadMore>Xem [API reference cho `IntegrationRouteData`](/en/reference/integrations-reference/#integrationroutedata-type-reference).</ReadMore>
...
<ReadMore>Xem [Adapter API reference cho `renderOptions`](/en/reference/adapter-reference/#renderoptions).</ReadMore>
...
<ReadMore>Đọc thêm về [chỉ định các tính năng Astro được hỗ trợ trong adapter](/en/reference/adapter-reference/#astro-features).</ReadMore>
...
<ReadMore>Đọc thêm về [phát triển dev toolbar app cho Astro bằng Dev Toolbar API](/en/reference/dev-toolbar-app-reference/).</ReadMore>
...
Biết một tài nguyên tốt cho Astro v5.0? [Chỉnh sửa trang này](https://github.com/withastro/docs/edit/main/src/content/docs/en/guides/upgrade-to/v5.mdx) và thêm link bên dưới!
...
Nếu bạn đang sử dụng MDX server renderer với [Astro Container API](/en/reference/container-reference/) thử nghiệm, bạn phải cập nhật import để phản ánh vị trí mới:
...
<ReadMore>Tìm hiểu thêm về [sử dụng MDX trong dự án của bạn](/en/guides/integrations-guide/mdx/).</ReadMore>
...
<ReadMore>Tìm hiểu thêm về [content collections](/en/guides/content-collections/) được cập nhật.</ReadMore>
...
Nếu bạn chưa sẵn sàng cập nhật các collections hiện có, bạn có thể bật flag [`legacy.collections`](/en/reference/legacy-flags/) và các collections hiện có của bạn sẽ tiếp tục hoạt động như trước.
...
Thay thế tất cả việc sử dụng `Astro.glob()` bằng `import.meta.glob()`. Lưu ý rằng `import.meta.glob()` không còn trả về `Promise`, vì vậy bạn có thể phải cập nhật code cho phù hợp. Bạn không cần cập nhật [glob patterns](/en/guides/imports/#glob-patterns).
...
Khi thích hợp, hãy xem xét sử dụng [content collections](/en/guides/content-collections/) để sắp xếp nội dung của bạn, có các function truy vấn mới hơn, hiệu suất cao hơn.

Bạn cũng có thể muốn xem xét sử dụng các gói glob từ NPM, chẳng hạn như [`fast-glob`](https://www.npmjs.com/package/fast-glob).

<ReadMore>Tìm hiểu thêm về [import files với `import.meta.glob`](/en/guides/imports/#importmetaglob).</ReadMore>
...
<ReadMore>Tìm hiểu thêm về [Adapter API](/en/reference/adapter-reference/) để xây dựng adapter integrations.</ReadMore>
...
<ReadMore>Tìm hiểu thêm về [Integration API hook `astro:routes:resolved`](/en/reference/integrations-reference/#astroroutesresolved) để xây dựng integrations.</ReadMore>
...
<ReadMore>Tìm hiểu thêm về [integrations chính thức của Astro](/en/guides/integrations-guide/).</ReadMore>
...
<ReadMore>Tìm hiểu thêm về [on-demand rendering trong Astro](/en/guides/on-demand-rendering/).</ReadMore>
...
Nếu adapter của bạn không hỗ trợ tối ưu hóa hình ảnh Sharp tích hợp của Astro, bạn có thể [cấu hình dịch vụ hình ảnh no-op passthrough](/en/guides/images/#configure-no-op-passthrough-service) để cho phép bạn sử dụng các component `<Image />` và `<Picture />`.
...
<ReadMore>Đọc thêm về [cấu hình dịch vụ hình ảnh mặc định](/en/guides/images/#default-image-service).</ReadMore>
...
<ReadMore>Đọc thêm về [quy tắc thứ tự ưu tiên route](/en/guides/routing/#route-priority-order).</ReadMore>
...
<ReadMore>Đọc thêm về [sử dụng thẻ `script` trong Astro](/en/guides/client-side-scripts/#using-script-in-astro).</ReadMore>
...
<ReadMore>Đọc thêm về [view transitions và client-side routing trong Astro](/en/guides/view-transitions/).</ReadMore>
...
<ReadMore>Đọc thêm về [cấu hình TypeScript trong Astro](/en/guides/typescript/#setup).</ReadMore>
...
Để giải quyết hộp thoại "confirm form resubmission?" khi refresh, hoặc để bảo tồn kết quả action qua các sessions, bây giờ bạn có thể [tùy chỉnh xử lý kết quả action từ middleware](/en/guides/actions/#advanced-persist-action-results-with-a-session).

Chúng tôi khuyến nghị sử dụng session storage provider [như được mô tả trong ví dụ Netlify Blob của chúng tôi](/en/guides/actions/#advanced-persist-action-results-with-a-session). Tuy nhiên, nếu bạn thích hành vi cookie forwarding từ 4.X và chấp nhận giới hạn kích thước 4 KB, bạn có thể triển khai pattern như được hiển thị trong đoạn mã mẫu này:
...
<ReadMore>Đọc thêm về [function `compiledContent()`](/en/guides/markdown-content/#importing-markdown) để trả về compiled Markdown.</ReadMore>
...
<ReadMore>Đọc thêm về [API `astro:content`](/en/reference/modules/astro-content/).</ReadMore>
