...
Need to upgrade an older project to v3? See our [older migration guide](/vi/guides/upgrade-to/v3/).
...
You can also [upgrade your Astro integrations manually](/vi/guides/integrations-guide/#manual-upgrading) if needed, and you may also need to upgrade other dependencies in your project.
...
In Astro v3.0, `build.split` and `build.excludeMiddleware` build config options were deprecated and replaced with [adapter configuration options](/vi/reference/adapter-reference/#adapter-features) to perform the same tasks.
...
Please see the v3 migration guide to [update these deprecated middleware properties](/vi/guides/upgrade-to/v3/#deprecated-buildexcludemiddleware-and-buildsplit) with adapter configurations.
...
Update all occurrences to [`Astro.params`](/vi/reference/api-reference/#params), which is the supported replacement.
...
If you are using the unsupported method for generating RSS feeds, you must now use the [`@astrojs/rss` integration](/vi/recipes/rss/) for a complete RSS setup.
...
Please see the v3 migration guide [for guidance using uppercase HTTP request methods](/vi/guides/upgrade-to/v3/#changed-http-request-methods-case).
...
When using the Astro preview server, all of your static asset imports and URLs from the public directory must have [the base value](/vi/reference/configuration-reference/#base) prefixed to the path.
...
Know a good resource for Astro v4.0? [Edit this page](https://github.com/withastro/docs/edit/main/src/content/docs/vi/guides/upgrade-to/v4.mdx) and add a link below!
