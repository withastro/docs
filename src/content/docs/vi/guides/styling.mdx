...
<ReadMore>Xem trang [tham khảo directives](/en/reference/directives-reference/#classlist) để tìm hiểu thêm về `class:list`.</ReadMore>
...
<ReadMore>Xem trang [tham khảo directives](/en/reference/directives-reference/#definevars) để tìm hiểu thêm về `define:vars`.</ReadMore>
...
<ReadMore>Đọc thêm về việc sử dụng [static assets](/en/guides/imports/) nằm trong `public/` hoặc `src/`.</ReadMore>
...
Bạn có thể import stylesheets trong Astro component frontmatter của bạn bằng cách sử dụng ESM import syntax. CSS imports hoạt động như [bất kỳ ESM import nào khác trong một component Astro](/en/basics/astro-components/#the-component-script), đó là nên được tham chiếu **relative to the component** và phải được viết ở **top** của component script của bạn, với bất kỳ imports nào khác.
...
:::note
Đây là một [Vite-specific setting](https://vite.dev/config/ssr-options.html#ssr-noexternal) mà _không_ liên quan đến (hoặc yêu cầu) [Astro SSR](/en/guides/on-demand-rendering/).
:::
...
Tùy thuộc vào giá trị bạn chọn cho [`scopedStyleStrategy`](/en/reference/configuration-reference/#scopedstylestrategy), scoped styles có thể hoặc không thể tăng [CLASS column specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_cascade/Specificity#class_column).
...
:::tip
Một pattern phổ biến trong Astro là import global CSS bên trong một [Layout component](/en/basics/layouts/). Hãy đảm bảo import Layout component trước các imports khác để nó có độ ưu tiên thấp nhất.
:::
...
Bất kỳ phương pháp styling Astro nào đều có sẵn cho một [Markdown layout component](/en/basics/layouts/#markdown-layouts), nhưng các phương pháp khác nhau sẽ có các hiệu ứng styling khác nhau trên trang của bạn.
...
Bạn cũng có thể thêm CSS integrations bao gồm [Tailwind](/en/recipes/tailwind-rendered-markdown/). Nếu bạn đang sử dụng Tailwind, [typography plugin](https://tailwindcss.com/docs/typography-plugin) có thể hữu ích để tạo style cho Markdown.
