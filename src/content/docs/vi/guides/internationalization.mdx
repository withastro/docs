...
Astro sử dụng [middleware](/en/guides/middleware/) để triển khai logic định tuyến. Hàm middleware này được đặt ở [vị trí đầu tiên](/en/guides/middleware/#chaining-middleware) nơi nó chờ đợi mọi `Response` đến từ bất kỳ middleware bổ sung nào và mỗi route trang trước khi cuối cùng thực thi logic của chính nó.
...
Tổ chức các thư mục nội dung của bạn với nội dung được bản địa hóa theo ngôn ngữ. Tạo các thư mục `/[locale]/` riêng lẻ ở bất cứ đâu trong `src/pages/` và [định tuyến dựa trên file](/en/guides/routing/) của Astro sẽ tạo các trang của bạn tại các đường dẫn URL tương ứng.

Tên thư mục của bạn phải khớp chính xác với các mục trong `locales`. Chỉ bao gồm thư mục được bản địa hóa cho `defaultLocale` của bạn nếu bạn cấu hình `prefixDefaultLocale: true` để hiển thị đường dẫn URL được bản địa hóa cho ngôn ngữ mặc định của bạn (ví dụ: `/en/about/`).
...
Tùy chọn định tuyến này xác định xem URL của ngôn ngữ mặc định của bạn có sử dụng tiền tố ngôn ngữ hay không (ví dụ: `/en/about/`).
...
Cài đặt này cũng xác định nơi các file trang cho ngôn ngữ mặc định của bạn phải tồn tại (ví dụ: `src/pages/about/` hoặc `src/pages/en/about`) vì cấu trúc file và cấu trúc URL phải khớp cho tất cả các ngôn ngữ.
...
Bạn có thể chọn không sử dụng hành vi này bằng cách [đặt `redirectToDefaultLocale: false`](/en/reference/configuration-reference/#i18nroutingredirecttodefaultlocale). Điều này cho phép bạn có trang chủ trang web tồn tại bên ngoài cấu trúc thư mục locale được cấu hình của bạn.
...
Bạn có thể chạy `middleware` với [tùy chọn định tuyến](#routing) kết hợp với middleware của riêng bạn, sử dụng tiện ích [`sequence`](/en/reference/modules/astro-middleware/#sequence) để xác định thứ tự:
...
Tùy chọn định tuyến này cho phép bạn tùy chỉnh domain theo từng ngôn ngữ cho các dự án được render `server` sử dụng adapter [`@astrojs/node`](/en/guides/integrations-guide/node/) hoặc [`@astrojs/vercel`](/en/guides/integrations-guide/vercel/) với `site` được cấu hình.
...
Tất cả `locales` không được ánh xạ sẽ tuân theo cấu hình `prefixDefaultLocales` của bạn. Tuy nhiên, ngay cả khi giá trị này là `false`, các file trang cho `defaultLocale` của bạn cũng phải tồn tại trong thư mục được bản địa hóa. Đối với cấu hình trên, thư mục `/en/` là bắt buộc.
...
- File `/en/about.astro` sẽ tạo URL `https://example.com/about`.
...
Chiến lược dự phòng của bạn bao gồm hai phần: chọn ngôn ngữ nào nên dự phòng cho ngôn ngữ nào khác ([`i18n.fallback`](/en/reference/configuration-reference/#i18nfallback)) và chọn có thực hiện [chuyển hướng](/en/guides/routing/#redirects) hay [viết lại](/en/guides/routing/#rewrites) để hiển thị nội dung dự phòng ([`i18n.routing.fallbackType`](/en/reference/configuration-reference/#i18nroutingfallbacktype) được thêm trong Astro v4.15.0).
...
Khi sử dụng các hàm từ [module ảo `astro:i18n`](/en/reference/modules/astro-i18n/) để tính toán đường dẫn URL hợp lệ dựa trên cấu hình của bạn (ví dụ: `getRelativeLocaleUrl()`), [sử dụng `path` làm giá trị cho `locale`](/en/reference/modules/astro-i18n/#getlocalebypath).
...
- [`Astro.preferredLocale`](/en/reference/api-reference/#preferredlocale): Astro có thể tính toán **locale ưa thích** cho khách truy cập của bạn nếu locale ưa thích của trình duyệt được bao gồm trong mảng `locales` của bạn. Giá trị này không được xác định nếu không có sự khớp như vậy.

- [`Astro.preferredLocaleList`](/en/reference/api-reference/#preferredlocalelist): Một mảng tất cả các locale vừa được trình duyệt yêu cầu vừa được trang web của bạn hỗ trợ. Điều này tạo ra danh sách tất cả các ngôn ngữ tương thích giữa trang web của bạn và khách truy cập của bạn. Giá trị là `[]` nếu không có ngôn ngữ được trình duyệt yêu cầu nào được tìm thấy trong mảng `locales` của bạn. Nếu trình duyệt không chỉ định bất kỳ ngôn ngữ ưa thích nào, thì giá trị này sẽ là [`i18n.locales`].

- [`Astro.currentLocale`](/en/reference/api-reference/#currentlocale): Locale được tính toán từ URL hiện tại, sử dụng cú pháp được chỉ định trong cấu hình `locales` của bạn. Nếu URL không chứa tiền tố `/[locale]/`, thì giá trị sẽ mặc định là [`i18n.defaultLocale`](/en/reference/configuration-reference/#i18ndefaultlocale).
