...
:::note
Các dự án có thể tiếp tục sử dụng legacy Content Collections API được giới thiệu trong Astro v2.0. Tuy nhiên, chúng tôi khuyến khích bạn [cập nhật các collection hiện có](/en/guides/upgrade-to/v5/#legacy-v20-content-collections-api) khi có thể.
:::
...
Tương tự như tạo Astro integration hoặc Vite plugin, bạn có thể [phân phối loader như NPM package](/en/reference/publish-to-npm/) để người khác sử dụng trong dự án của họ.

<ReadMore>Xem đầy đủ [Content Loader API](/en/reference/content-loader-reference/) và ví dụ về cách xây dựng loader riêng.</ReadMore>
...
- [`getCollection()`](/en/reference/modules/astro-content/#getcollection) tải toàn bộ collection và trả về một mảng entries.
- [`getEntry()`](/en/reference/modules/astro-content/#getentry) tải một entry duy nhất từ collection.
...
<ReadMore>Xem danh sách đầy đủ các thuộc tính được trả về bởi [kiểu `CollectionEntry`](/en/reference/modules/astro-content/#collectionentry).</ReadMore>
...
Sau khi được truy vấn, bạn có thể render các entry Markdown và MDX thành HTML bằng thuộc tính hàm [`render()`](/en/reference/modules/astro-content/#render). Gọi hàm này cho phép bạn truy cập nội dung HTML được render, bao gồm cả component `<Content />` và danh sách tất cả heading được render.
...
Bạn có thể sử dụng utility [`CollectionEntry`](/en/reference/modules/astro-content/#collectionentry) để type đúng props của component bằng TypeScript. Utility này nhận đối số chuỗi khớp với tên schema collection và sẽ kế thừa tất cả thuộc tính của schema collection đó.
...
Bạn sẽ cần tự tạo một [dynamic route](/en/guides/routing/#dynamic-routes) mới nếu muốn tạo trang HTML cho mỗi entry collection, như các bài blog riêng lẻ. Dynamic route sẽ ánh xạ tham số request đến (ví dụ: `Astro.params.slug` trong `src/pages/blog/[...slug].astro`) để tải entry đúng cho mỗi trang.
...
Nếu bạn đang xây dựng website tĩnh (hành vi mặc định của Astro), sử dụng hàm [`getStaticPaths()`](/en/reference/routing-reference/#getstaticpaths) để tạo nhiều trang từ một page component duy nhất (ví dụ: `src/pages/[slug]`) trong quá trình build.
...
:::note
Nếu custom slugs của bạn chứa ký tự `/` để tạo URL với nhiều path segments, bạn phải sử dụng [rest parameter (ví dụ: `[...slug]`)](/en/guides/routing/#rest-parameters) trong tên file `.astro` cho trang dynamic routing này.
:::
...
- Một [Content Loader API](/en/reference/content-loader-reference/) để lấy nội dung cung cấp cả built-in loaders và quyền truy cập API cấp thấp. Có một số loader của bên thứ ba và cộng đồng khả dụng, và bạn có thể xây dựng loader tùy chỉnh để tải dữ liệu từ bất cứ đâu.
...
- Bạn chỉ có một hoặc số lượng nhỏ các trang khác nhau. Hãy xem xét [tạo các page components riêng lẻ](/en/basics/astro-pages/) như `src/pages/about.astro` với nội dung trực tiếp thay thế.
- Bạn đang hiển thị các file không được xử lý bởi Astro, như PDFs. Đặt các static assets này trong [thư mục `public/`](/en/basics/project-structure/#public) của dự án thay thế.
