...
Các trang Astro sử dụng phần mở rộng file `.astro` và hỗ trợ các tính năng giống như [Astro components](/en/basics/astro-components/).
...
Để tránh lặp lại các phần tử HTML giống nhau trên mọi trang, bạn có thể di chuyển các phần tử `<head>` và `<body>` chung vào [layout components](/en/basics/layouts/) của riêng mình. Bạn có thể sử dụng nhiều hoặc ít layout components tùy ý.
...
<ReadMore>Đọc thêm về [layout components](/en/basics/layouts/) trong Astro.</ReadMore>

## Trang Markdown/MDX

Astro cũng coi bất kỳ file Markdown (`.md`) nào bên trong `src/pages/` là trang trong website cuối cùng của bạn. Nếu bạn có [MDX Integration được cài đặt](/en/guides/integrations-guide/mdx/#installation), nó cũng xử lý các file MDX (`.mdx`) theo cách tương tự.

:::tip
Hãy xem xét việc tạo [content collections](/en/guides/content-collections/) thay vì trang cho các thư mục của các file Markdown liên quan có cùng cấu trúc tương tự, chẳng hạn như bài đăng blog hoặc sản phẩm.
:::

Các file Markdown có thể sử dụng thuộc tính frontmatter đặc biệt `layout` để chỉ định [layout component](/en/basics/layouts/) sẽ bao bọc nội dung Markdown trong tài liệu trang `<html>...</html>` đầy đủ.
...
Các file có phần mở rộng `.html` có thể được đặt trong thư mục `src/pages/` và sử dụng trực tiếp làm trang trên trang web của bạn. Lưu ý rằng một số tính năng chính của Astro không được hỗ trợ trong [HTML Components](/en/basics/astro-components/#html-components).

## Trang lỗi 404 tùy chỉnh

Để có trang lỗi 404 tùy chỉnh, bạn có thể tạo file `404.astro` hoặc `404.md` trong `src/pages`.

Điều này sẽ build thành trang `404.html`. Hầu hết [dịch vụ triển khai](/en/guides/deploy/) sẽ tìm và sử dụng nó.

## Trang lỗi 500 tùy chỉnh

Để có trang lỗi 500 tùy chỉnh hiển thị cho các trang được [render theo yêu cầu](/en/guides/on-demand-rendering/), tạo file `src/pages/500.astro`. Trang tùy chỉnh này không khả dụng cho các trang được prerender và không thể được prerender.
...
Partials, khi kết hợp với thư viện rendering, cung cấp một giải pháp thay thế cho [Astro islands](/en/concepts/islands/) và [thẻ `<script>`](/en/guides/client-side-scripts/) để xây dựng nội dung động trong Astro.

Các file trang có thể export giá trị cho [`partial`](/en/reference/routing-reference/#partial) (ví dụ: `.astro` và `.mdx`, nhưng không phải `.md`) có thể được đánh dấu là partials.
