...
SPAs có vị trí của chúng, thậm chí [được nhúng bên trong một trang Astro](/en/guides/migrate-to-astro/from-create-react-app/). Nhưng, SPAs thiếu khả năng hydrate một cách có chọn lọc và chiến lược, khiến chúng trở thành lựa chọn quá nặng nề cho hầu hết các dự án trên web ngày nay.
...
Thậm chí tốt hơn, bạn có thể yêu cầu Astro chính xác cách và khi nào render mỗi component. Nếu image carousel đó thực sự đắt đỏ để tải, bạn có thể đính kèm một [client directive](/en/reference/directives-reference/#client-directives) đặc biệt yêu cầu Astro chỉ tải carousel khi nó hiển thị trên trang. Nếu người dùng không bao giờ nhìn thấy nó, nó không bao giờ tải.
...
<ReadMore>Đọc thêm về [sử dụng các thành phần JavaScript framework](/en/guides/framework-components/) trong dự án của bạn.</ReadMore>
...
Thêm directive [`server:defer`](/en/reference/directives-reference/#server-directives) vào bất kỳ thành phần Astro nào trên trang của bạn để biến nó thành server island riêng của nó:
...
<ReadMore>Đọc thêm về [sử dụng server islands](/en/guides/server-islands/) trong dự án của bạn.</ReadMore>