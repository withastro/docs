...
This allows your page's critical content to be rendered immediately as a shell, while deferring your most dynamic UI elements, such as user avatars or product reviews. This also means that a slower island, such as one connected to a legacy backend, won’t delay the rest of the page's personalized content from being available.
