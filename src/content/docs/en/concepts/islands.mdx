...
Astro pioneered and popularized a frontend architecture called **Islands: interactive UI framework components that render independently from the rest of your page**.

Islands architecture results in better frontend performance by helping you avoid monolithic JavaScript patterns and by stripping all non-essential JavaScript from the page automatically.

{/* I REALLY want something right upfront that makes it obvious to non-Lighthouse Score junkies why they should care, and why this is important. */}
Whether you are closely monitoring core web vitals and Lighthouse scores, or simply don't want to make your visitors wait and burden their devices with a lot of unnecessary JavaSript, islands architecture is the key to building fast, lightweight, performant, *respectful* sites and apps.
...
Selective hydration stands in contrast to how most JavaScript-based web frameworks choose to hydrate & render an entire website as one large JavaScript application (also known as a single-page application, or SPA). 

SPAs provide simplicity and power but suffer from page-load performance problems due to heavy client-side JavaScript usage. SPAs have their place, even [embedded inside an Astro page](/en/concepts/embedded-web-apps/). But, SPAs lack the native ability to selectively and strategically hydrate, making them a heavy-handed choice for most projects on the web today.
...
Turning any static UI component into an interactive island only requires a `client:*` directive. Astro wires up everything for you automatically and automatically builds & bundles your client-side JavaScript for optimized performance.
...
And because interaction is configured at the component-level, you can handle different loading priorities for each component based on its usage. For example, `client:idle` tells a component to load when the browser becomes idle, and `client:visible` tells a component to load only once it's visible. 
