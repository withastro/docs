...
Islands architecture results in better frontend performance by stripping all non-essential JavaScript from the page automatically. Javascript will be sent to the browser only if, and when, an individual component requires it.

This is important whether you are closely monitoring Core Web Vitals and Lighthouse scores, or simply don't want to make your visitors wait and burden their devices. Islands architecture is the key to building fast, lightweight, performant sites.
...
- Incrementally convert an existing site to another framework with no down time.
:::
