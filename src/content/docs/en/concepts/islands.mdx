...
Server islands provide an optimized rendering solution for pages that combine content that is:

- Completely static and never changing.
...
**Server islands are an extension of Astro's pioneering Island Architecture philosophy, bringing performance optimizations to your server-rendered code.**