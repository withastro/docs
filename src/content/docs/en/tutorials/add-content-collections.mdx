...
6. You might run into a TypeScript error `Cannot find module 'astro:content' or its corresponding type declarations.ts(2307)`. In order to resolve it quit the dev server and run the following command: `npx astro sync`. Restart the dev server when you want to view your site in development mode again.
