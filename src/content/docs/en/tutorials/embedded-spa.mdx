...
Astro can render an **embedded single-page application (SPA)** from any UI framework as an [Astro island](/en/concepts/islands/). This allows a JavaScript app to run entirely at the root of an Astro project in just a few lines of code. 
...
And as a **multi-page application**, Astro can also generate additional pages and API routes for more traditional page content at other routes... or even more embedded SPAs! This powerful embedded SPA pattern allows you to use one Astro project to house:
...
Over time, you may find that instead of simply embedding, you want to migrate a single-page application [(e.g. Create-React-App)](/en/guides/migrate-to-astro/from-create-react-app/) by rewriting some or all of it as an Astro app. The embedded SPA pattern allows you to update incrementally: first getting your app running in your Astro project, and then refactoring to leverage the benefits of working in Astro.
...
You will modify the script code from the sample repo to create a component and embed an entire React app directly in an Astro project.
