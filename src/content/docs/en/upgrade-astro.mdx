...
- **Improvements to the documentation** (e.g. reference and error messages). They are built from source from the `docs` repository. This allows Astro to quickly update docs fixes and improvements in the cases where documentation source content is stored in the main `astro` repo.
