...
:::note
Minor releases are meant to include only non-breaking changes. However, *in rare cases* Astro will include an upgrade of the minimum required Node.js version in a minor release (e.g. from `v18.4.0` to `v18.7.0`).

Astro is committed to adhering to semantic versioning and will include an upgrade to a new version of Node.js in a minor release **only after careful analysis of a variety of factors** including, but not limited to, the number of Astro projects running on incompatible versions, the security risk to developers of supporting an older version of Node.js, and the impact on Astro's core functionalities.
:::
...
- Upgrade of minimum required Node.js version (minor or major), e.g. from `v18.4.0` to `v20.0.0`.
...
- **Improvements to the documentation** (e.g. reference and error messages). They are built from source from the `docs` repository. This allows Astro to quickly update docs fixes and improvements in the cases where documentation source content is stored in the main `astro` repository.
