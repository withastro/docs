...
1. **What file types are supported in the `src/pages/` directory of an Astro project?**

    Astro supports `.astro`, `.md`, `.mdx`, and `.html` page files, and `.js` / `.ts` endpoints. UI frameworks are not supported. ([supported page / endpoint file types](#supported-page-files))
