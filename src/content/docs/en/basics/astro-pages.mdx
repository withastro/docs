...
:::caution
Once deployed, Astro will only use `500.astro` when using `output: 'hybrid'` for pages that opt-out from prerendering or `output: 'server'`.
:::

If an error occurs in this file, it will default to the host 500 error page.
...
If an error occurs in this file, it will default to the error overlay.
