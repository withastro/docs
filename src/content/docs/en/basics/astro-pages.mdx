...
1. **Can I use framework components like `.jsx`, `.svelte`, or `.vue` files as pages?**

    Astro supports `.astro`, `.md`, `.mdx`, and `.html` page files, and `.js` / `.ts` endpoints. React, Svelte and other framework components can be imported and used in `.astro` and `.mdx` pages, but should not be placed in `src/pages/` directly. ([supported page / endpoint file types](#supported-page-files))

2. **How do I add a new page to my site?**
    Astro uses file-based routing. The file structure in the `src/pages/` directory maps directly to the URL structure of your site. ([file-based routing](#file-based-routing))

3. **How can I link between pages in an Astro project?**

    Use the HTML `<a>` element to link between pages in your project. There is no special built-in `<Link />` routing component.([linking between pages](#linking-between-pages))

### Community question and answers

Find people asking and answering questions about pages in Astro:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="pages"
  stackOverflow="Search for questions about Astro pages on Stack Overflow"
  reddit="Search for questions about Astro pages on Reddit"
