...
1. **What goes in the `src/` directory of my Astro project?**

    The `src/` directory is where most of your project source code lives. This includes pages, layouts, Astro components, UI framework components, styles, and Markdown. ([`src/`](#src))

2. **What goes in the `public/` directory of my Astro project?**
    The `public/` directory is used for your non-code, unprocessed assets like fonts, icons, etc. ([file-based routing](#public))

3. **I am not writing TypeScript. Do I still need the `tsconfig.json` file?**

    Yes. This file is used by Astro to support auto-completion, Intellisense, content collections, and more. You can configure whether or not your own project code will also be checked for TypeScript errors, but you should keep this file.([`tsconfig.json`](#tsconfigjson))


### Community question and answers

Find people asking and answering questions about project structure in Astro:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="project structure"
  stackOverflow="Search for questions about Astro project structure on Stack Overflow"
  reddit="Search for questions about Astro project structure on Reddit"
