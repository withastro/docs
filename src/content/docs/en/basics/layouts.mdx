...
1. **Is a layout like a theme?**

    A layout is an individual Astro component used to provide a reusable UI structure, such as a template for one kind of page on your site. A theme or template is a complete Astro project that may have only one site layout, or different layouts for different page types (e.g. blog posts and a contact page).([`page overview`](#_top))

2. **When should I use the frontmatter `layout` property in Markdown and MDX pages?**

    You can specify an Astro component to use as a layout directly in Markdown and MDX page files located within `src/pages/`. This property cannot be used with content collections. ([Markdown/MDX layouts](#markdownmdx-layouts))

3. **Which props are automatically available to a layout component?**

    In `.astro` page components, your layout will have access to any props passed directly to your `<Layout>` component. Markdown/MDX page components using the frontmatter `layout` property have access to all frontmatter values and other page metadata by default. ([`tsconfig.json`](#markdown-layout-props))

### Community question and answers

Find people asking and answering questions about layouts in Astro:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="layouts"
  stackOverflow="Search for questions about Astro layouts on Stack Overflow"
  reddit="Search for questions about Astro layouts on Reddit"
