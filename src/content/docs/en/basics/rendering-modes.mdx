...
1. **Is Astro an SSG? Does it do SSR?**
    ...
    Yes, and yes! Astro pages, routes, and API endpoints can be either pre-rendered statically at build time or rendered on demand by a server when a route is requested. ([`page overview`](#_top))

2. **Do I need to enable an on-demand rendering mode to have dynamically-generated HTML or page routes?**

    No. An Astro static site using the default `static` output mode will run all your dynamic code once, at build time to generate the HTML for your site. Adding an adapter for on-demand rendering is used to display pages that need to update between site builds or access personalized content. ([on-demand rendered](#pre-rendered))
