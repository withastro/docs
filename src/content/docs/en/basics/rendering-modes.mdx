...
1. **Is Astro an SSG? Does it do SSR?**

    Yes, and yes! Astro pages, routes, and API endpoints can be either pre-rendered statically at build time or rendered on demand by a server when a route is requested. ([overview](#_top))

2. **Do I need to enable an on-demand rendering mode to have dynamically-generated HTML or page routes?**

    No. An Astro static site using the default `static` output mode will run all your dynamic code once, at build time to generate the HTML for your site. Adding an adapter for on-demand rendering is used to display pages that need to update between site builds or access personalized content. ([prerendered](#prerendered))

3. **How do I choose between `server` and `hybrid` mode?**

    Use `output: 'server'` for highly dynamic sites because in this mode, all your routes will be rendered on-demand by default. Use `output: 'hybrid'` to keep your site static by default and opt out of prerendering only for the exact routes you need to be rendered upon request. ([on-demand rendered](#on-demand-rendered))

### Community question and answers

Find people asking and answering questions about server rendering modes in Astro:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="server modes"
  stackOverflow="Search for questions about server modes on Stack Overflow"
  reddit="Search for questions about server modes on Reddit"
