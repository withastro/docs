...
The most important thing to know about Astro components is that they **don't render on the client**. They render to HTML at build time by default. You can include JavaScript code inside of your component frontmatter which runs only to build your component. Then, all of it will be stripped from the final page sent to your users' browsers. The result is a faster site, with zero JavaScript footprint added by default.

Within a page, you can defer the rendering of individual components by adding a [server directive](/en/reference/directives-reference/#server-directives). These "server islands" allow your page to wait for personalized or dynamic content rendered on demand, without delaying the entire page load.
...
Server islands allow you to prerender or server-render a page shell while allowing individual, dynamic, personalized "islands" of content within it to be rendered on-demand. This allows your visitors to see the most critical parts of your page sooner, and allows your main content to be more aggressively cached, providing faster performance.

A server island component is a normal Astro component that is directed to delay rendering and display placeholder fallback content until its own content is ready. These components can do anything you normally would in an on-demand rendered page, such as fetch content, and access cookies.
...
### Creating a server island on a page

Add [the `server:defer` directive](/en/reference/directives-reference/#server-directives) to any component imported and rendered on your page in order to turn it into its own island:
...
The page will be rendered without the contents of the island, and the island component's contents will instead be fetched on the client.
...
- Loading indicators such as spinners or progress bars
...
In most cases you, can get information about the page rendering your server island by [passing props](#component-props) like in normal components. 
