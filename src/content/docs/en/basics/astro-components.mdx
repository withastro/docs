...
:::note
Fallback content will not show if an empty slot is passed into a component.
:::

```astro title="ComponentA.astro"
<slot name="foo">
  <p>Default Content</p>
</slot>
```

```astro title="ComponentB.astro"
---
import ComponentA from ""./ComponentA.astro";
---

<ComponentA>
  <slot name="bar" slot="foo" />
</ComponentA>
```
...
Here we are not passing a slot into `ComponentB` from `index.astro`. But `ComponentB` is still passing an empty slot into `ComponentA`. Therefore `ComponentA` will not render it's default content.

This is because a component checks that a slot exists - not if it's empty.
