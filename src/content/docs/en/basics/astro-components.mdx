...
1. **What are Astro components and how are they used?**

    Astro components are the basic building blocks of any Astro project. They are HTML-only templating components with no client-side runtime. ([page overview](#_top))

2. **What is the structure of an Astro component?**
    An Astro component is made up of two main parts: the Component Script and the Component Template. ([component structure](#component-structure))

3. **How do I separate my server-side code?**

    JavaScript written in the script frontmatter (in between the code fences (`---`)) is always run on the server only. ([the component script](#the-component-script))

3. **How can I pass data to Astro components?**

    An Astro component can define and accept props. These props then become available to the component template for rendering HTML. ([component props](#component-props))

4. **How can I render HTML from my other files in Astro components?**
    
    HTML from another component can be rendered inside a default or named `<slot />` placeholder element. ([slots](#slots))

### Community question and answers

Find people asking and answering questions about components in Astro:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="components"
  stackOverflow="Search for questions about Astro components on Stack Overflow"
  reddit="Search for questions about Astro components on Reddit"
