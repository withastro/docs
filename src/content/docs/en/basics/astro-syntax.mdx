...
1. **Does Astro use JSX?**

    No. The Astro component templating language is a superset of HTML. The syntax for writing dynamic HTML is similar to JSX, but it is not identical. ([`page overview`](#_top))

2. **What is the main differences between the Astro templating language and JSX?**

    Astro uses curly braces `{ }` in the component template when writing JavaScript expressions to produce dynamically-generated HTML elements. Without these braces, your code will be interpreted as HTML. ([differences between Astro template syntax and JSX](#differences-between-astro-and-jsx))

3. **Where do I define variables in an Astro file?**

    Define local JavaScript variables inside of the frontmatter component script between the two code fences (`---`) of an Astro component. You can then inject these variables into the component’s HTML template using JSX-like expressions. ([JSX-like expressions](#jsx-like-expressions))

### Community question and answers

Find people asking and answering questions about templating syntax in Astro:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="astro syntax"
  stackOverflow="Search for questions about the Astro syntax on Stack Overflow"
  reddit="Search for questions about the Astro syntax on Reddit"
