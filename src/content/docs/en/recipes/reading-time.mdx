...
Create a [remark plugin](https://github.com/remarkjs/remark) which adds a reading time property to your Markdown or MDX files's frontmatter. Use this property to display the reading time for each page.
...
Use the `minutesRead` frontmatter property from `Astro.props` to add the reading time to a [Markdown layout](/en/guides/markdown-content/#markdown-and-mdx-pages). The example layout below displays the reading time above the page content:
