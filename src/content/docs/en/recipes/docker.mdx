...
#### Multi-stage build
Here's an example of a more advanced Dockerfile that, thanks to Docker's [multi-stage builds](https://docs.docker.com/build/building/multi-stage/), optimizes the build process for your site, sometimes reducing the build time even by minutes.
By utilizing multiple build stages, we ensure that the npm dependencies are not unnecessarily reinstalled when **only the source code** changes.
