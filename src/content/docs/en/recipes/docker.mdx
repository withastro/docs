...
# copy only the package.json and package-lock.json to avoid re-installing dependencies when only the source code changes
...
# by copying the app's whole source code here, the `-deps` steps will be skipped if only the source code changes
COPY . .
RUN npm run build
