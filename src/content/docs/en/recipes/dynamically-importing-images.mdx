...
## Extension: use a data file

Instead of manually defining the props for each individual component, you can import the props for your `<MyCustomCardComponent />` component from a data file. This is helpful to display a card for every person.

<Steps>

1. Create a new `data` folder under the `src` directory. Add a new file `src/data/people.json` with the following content:

    ```json title="src/data/people.json" 
    [
      {
        "imagePath": "/src/images/avatar-1.jpg",
        "name": "Priya",
        "altText": "A headshot of Priya against a brick wall background.",
        "age": {25}
      },
      {
        "imagePath": "/src/images/avatar-2.jpg",
        "name": "Tobbe",
        "altText": "Tobbe holding his cat.",
        "age": {47}
      },
      {
        "imagePath": "/src/images/avatar-3.jpg",
        "name": "Alejandro",
        "altText": "Alejandro playing the piano.",
        "age": {19}
      }
    ]
    ```

2. In `src/pages/index.astro`, import the data file `people.json`.

    ```astro title="src/pages/index.astro" ins={3}
    ---
    import MyCustomCardComponent from '../components/MyCustomCardComponent.astro';
    import People from '../data/people.json';
    ---
    ```
3. Replace individual card components with a mapping of all entries from your `people.json` data file.

    ```astro title="src/pages/index.astro" del={5-10}
    ---
    import MyCustomCardComponent from '../components/MyCustomCardComponent.astro';
    import People from '../data/people.json';
    ---
