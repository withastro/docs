...
1. Create an endpoint (e.g. `src/pages/api/hello.ts`):
...
2. On any Astro page, import the `get()` method from the endpoint. Call it with the [`Astro` global](https://docs.astro.build/en/reference/api-reference/#astro-global) to provide the request context, and use the response on the page:
