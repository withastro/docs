...
2. Create a Remark Plugin
   ...
   There are two ways to get the modified time of a file; using Git, or using the filesystem.

   :::note
   If you use Git, it is highly recommended to use it to get the modified time of the files. Git operations (like cloning) will result in the file system modified time being updated.
   :::

   - Using Git

     This plugin uses `execSync` to run a git command that returns the commit timestamp in ISO 8601 format. The timestamp is then added to the frontmatter of the file.
