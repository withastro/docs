...
2. In `my-integration.ts`, add the following code to provide the name of your integration and run the function to add your dev toolbar app with the `astro:config:setup` hook:
...
3. To select a new message each time your toolbar app is toggled, use the `onAppToggled` hook provided by the `app` argument:
    ...
    With this change, the a new random message will be selected every time you toggle the app on, providing you with an infinite source of motivation.

</Steps>

For more information on building toolbar apps, see the [Astro Toolbar API documentation](/en/reference/dev-toolbar-app-reference/).
