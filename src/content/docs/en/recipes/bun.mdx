...
## Add on-demand rendering with Bun using `@astrojs/node`

Bun features [Node.js API compatibility](https://bun.sh/docs/runtime/nodejs-apis). Using the [`@astrojs/node`](/en/guides/integrations-guide/node/) adapter you can add [on-demand rendering](/en/core-concepts/rendering-modes/#on-demand-rendered) to your Astro project using Bun's runtime as a replacement for Node.
