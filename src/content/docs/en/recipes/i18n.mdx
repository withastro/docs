...
If you want serve the default language at top-level, [click here](/en/recipes/i18n/#set-up-default-language-at-top-level).
...
### Set up default language at top-level

1. Create a directory only for each language that is not the default language that you want to support. For example, `en/` and `fr/` if you support English as the default language and French:
...
2. Add a line to the `src/i18n/ui.ts` file to possibly toggle the feature:
...
3. Create a helper function to translate paths based on the current user interface language in `src/i18n/utils.ts`:
...
4. Import the helpers where needed and use them to translate paths to the current language. For example, a nav component might look like:
...
5. Let users switch between languages
