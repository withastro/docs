...
If you don't want the default language to show up in the URL like the other languages, you can use the [instructions](/en/recipes/i18n/#hide-default-language-in-the-url) below.
...
1. Create a directory only for each language that is not the default language. For example, store your default language pages direclty in `pages/` and `fr/` if you also support French:
...
2. Add one line to the `src/i18n/ui.ts` file to possibly toggle the feature:
...
3. Add helper function to the `src/i18n/utils.ts` file to translate paths based on the current language:
...
5. Helper could also be used to translate paths for specific language. For example, when users switch between languages:
...
1. Add route mappings to the `src/i18n/ui.ts` file:
...
2. Update the `useTranslatedPath` helper function in `src/i18n/utils.ts` to add some more route translation logic.
...
4. Helper could be used to get a translated route. For example, when users switch languages, they don't have to come back from the home page. However, if no route is defined, you will be taken to the home page
