...
2. Set up `src/pages/index.astro` to redirect to your default language.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        If you are using an SSR adapter, you can use [`Astro.redirect`](/en/basics/routing/#dynamic-redirects) to redirect to the default language on the server.
...
3. Use [dynamic routes](/en/basics/routing/#dynamic-routes) to fetch and render content based on a `lang` and a `slug` parameter.
    ...
    <ReadMore>Read more about [dynamic routing](/en/basics/routing/#dynamic-routes).</ReadMore>
