...
2. Create a helper function to detect the page language based on the current URL and a helper to let us get translations strings for different parts of our UI in `src/i18n/utils.ts`:
    ...
    :::note[Did you notice?]
    In step 1, the `nav.twitter` string was not translated to French. The `useTranslations` helper here will return the deault language’s value if a key is not translated so that French users will still see “Twitter”.
    :::
