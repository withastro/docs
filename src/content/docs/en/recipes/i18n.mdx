...
3. Use [dynamic routes](/en/guides/routing/#dynamic-routes) to fetch and render content based on a `lang` and a `slug` parameter.
    ...
    :::note[Default locale without a URL prefix]
        If your default language is served **without** a locale prefix (for example, `/blog/...` instead of `/en/blog/...`), use a rest-parameter route such as `src/pages/[...lang]/blog/[...slug].astro` and include an `undefined` entry for the default locale in `getStaticPaths()`:
...
:::tip[Using these helpers with Astro’s built-in i18n routing]
If your project is using Astro’s i18n routing feature, you can usually skip parsing the URL and use `Astro.currentLocale` instead. For example: `const t = useTranslations(Astro.currentLocale)`.
:::
