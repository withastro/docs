...
2. Set up `src/pages/index.astro` to redirect to your default language.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="server">
        If you are using an adapter, you can use [`Astro.redirect`](/en/core-concepts/routing/#dynamic-redirects) to redirect to the default language on the server.
...
3. Use [dynamic routes](/en/core-concepts/routing/#dynamic-routes) to fetch and render content based on a `lang` and a `slug` parameter.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="server">
        In [an on-demand rendering mode](/en/core-concepts/rendering-modes/#on-demand-rendered), fetch the requested entry directly:
