...
1. Create a directory for each language you want to support. For example, `en/` and `fr/` if you are supporting English and French:
    ...
    :::tip[RTL languages]
    If you are including right-to-left (RTL) languages such as Arabic, then you will also need to [add supporting styles](https://rtlstyling.com/posts/rtl-styling).
    :::
