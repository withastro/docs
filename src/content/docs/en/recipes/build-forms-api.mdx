...
1. Create a form component using your UI framework. Each input should have a `name` attribute that describes the value of that input.
  ...
  2. Create the `post` API endpoint that will recieve the form data. Use `request.formData()` to process it. Be sure to validate the form values before you use them. 
  
        This example sends a JSON object with a message back to the client.

      ```ts title="src/api/feedback.ts" "request.formData()" "post"
