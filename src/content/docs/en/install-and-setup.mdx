...
2. Install Astro
    ...
    :::note[Important]
    Astro must be installed locally, not globally. Make sure you are *not* running `npm install -g astro` `pnpm add -g astro` `yarn add global astro` or `bun add -g astro`.
    :::
