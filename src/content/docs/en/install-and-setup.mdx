...
- **Node.js** - `v18.17.1` or `v20.3.0` or higher. ( `v19` is not supported.) Or [Deno](https://deno.com/) `v2.1` or higher
...
2. Run the following command in your terminal, substituting the official Astro starter template name, or the GitHub username and repository of the theme you want to use:
    ...
    <PackageManagerTabs>
      ...
      ```shell
      # create a new project with an official example
      deno -A npm:create-astro@latest --template <example-name>
      
      # create a new project based on a GitHub repository’s main branch
      deno -A npm:create-astro@latest --template <github-username>/<github-repo>
      ```
      </Fragment>
    </PackageManagerTabs>
...
2. Install Astro
    ...
    Then, replace any placeholder "scripts" section of your `package.json` (or "tasks" section of your `deno.json` file, if using Deno) with the following:
    ...
    If using Deno to install and manage your dependencies, you'll need to allow those dependecies to execute lifecycle scripts by:

    - Adding `"nodeModulesDir": "auto"` to your `deno.json` file,
    - Installing dependencies with the `allow-scripts` flag like this: `deno install --allow-scripts`
