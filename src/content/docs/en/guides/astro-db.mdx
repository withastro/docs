...
As you begin to add more tables, your data will likely start to form relationships. For example, an author can have multiple posts, and an article can have many sources.

You can relate tables to each other using reference columns. You will need:
...
You may need to seed related tables based on a generated id. You can use the `.returning()` function to return the inserted row, and use any generated columns for later inserts.

This example seeds an author and their comments using the generated Author `id`:
...
You can query related data from multiple tables using a SQL join. To author a join query, you can extend your `db.select()` statement with a join operator. Each function accepts a table to join with and a condition to match rows between the two tables.

This example uses an `innerJoin()` function to join `Comment`s with their related `Author`s based on the `authorId` column. This returns an array of objects with each `Author` and `Comment` as top-level properties:
...
**Astro recommends adding a layer of authentication** before accepting user input. [See the Authentication guide](https://docs.astro.build/en/guides/authentication/) for setup instructions.
...
You can build your Astro project from either local development data or a remote database connection.
...
Adding the `--remote` flag will switch to a remote connection. This flag is recommended for production deploys to [persist inserted data](#insert-data-from-a-server-route).
...
## Link your project to Astro Studio
...
- `astro login` - Authenticate with Astro Studio to run all database management commands.

- `astro link` - Link your local project to a remote database. This will prompt you to select a project from the Studio dashboard or create a new one.

- `astro db push` - Safely push database configuration changes to your remote DB. This will check for any risk of data loss and guide you on recommended migration steps. If a breaking schema change must be made, use the `--force-reset` flag to reset all production data.
