...
When you're ready to deploy, see our [Deploy with a Studio Connection guide](/en/guides/studio/#deploy-with-a-studio-connection).

</ReadMore>

## Set up the Astro DB CI Action

Astro DB can be configured to automatically push schema changes to your Studio database. This verifies changes can be made safely, and keeps your configuration up-to-date whenever you merge to `main`.

Before getting started, you will need to configure an app token as a GitHub secret. Retrieve your app token [using the environment secret section above](#set-your-environment-secret). Then [follow GitHub's documentation](https://docs.github.com/en/actions/security-guides/using-secrets-in-github-actions#creating-secrets-for-a-repository) to configure the secret in your repository using the name `ASTRO_STUDIO_APP_TOKEN`. 

Once your secret is configured, create a new GitHub Actions workflow file in your project's `.github/workflows` directory. This should checkout the repository and install Node.js as steps, and use the `withastro/action-studio` action to sync schema changes. 
...
This example pushes changes whenever the `main` branch is updated:
