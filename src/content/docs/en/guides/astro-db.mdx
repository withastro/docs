...
**Important Note**: Currently, there is a known bug related to the `db.batch()` method [link](https://github.com/withastro/astro/issues/10462), which results in inconsistent types issues in the underlying `drizzle` library that spreads to Astro's database API. This bug may affect the execution of batch operations as intended. The issue has been acknowledged and a fix is anticipated. Once the issue is resolved, the following example code can be used as is to seed multiple rows with a single network request.
