...
:::caution
This is an advanced feature that requires deploying without overriding an existing database. For a fully managed solution, try [connect to the Astro Studio platform](/en/recipes/studio/) instead.
:::

If you are deploying to a self-managed host such as a [virtual private server](https://en.wikipedia.org/wiki/Virtual_private_server) and do not want to connect to Astro Studio, you can connect to your database using the `ASTRO_DATABASE_FILE` environment variable. This pattern __does not__ work well with serverless hosts, as data is not retained in those environments.

This path should be to your `.db` file on the host. It's important that you *do not* override this file on deployment, ever, or you will lose your production data. Additionally, any schema migrations must be managed manually using this method.
