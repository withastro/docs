...
Astro DB is a complete solution to configuring, developing and querying your data. A local database is created in the `.astro` directory whenever you run `astro dev` to manage your data without the need for Docker or a network connection.
...
You can either use the CLI or the web interface to create and rethrieve the values we need.
...
2. [Login or signup](https://docs.turso.tech/cli/authentication) to Turso. For this example the username for the account will be Houston.

3. Let's create a new database. In this case the database name is `andromeda`, but you can change it to any other name.

   ```sh "andromeda"
   turso db create andromeda
   ```

4. Now we are going to get the database URL of the newly created database.

   ```sh "andromeda"
   turso db show andromeda
   ```

    Use the value in the row `URL:` as the environment variable for `ASTRO_DB_REMOTE_URL`.

   ```env title=".env" "libsql://andromeda-houston.turso.io"
   ASTRO_DB_REMOTE_URL=libsql://andromeda-houston.turso.io
   ```

5. To get the second value that we need, let's create a bearer token to authenticate requests to the database.

   ```sh "andromeda"
   turso db tokens create andromeda
   ```

    Use the output of the command as the environment variable for `ASTRO_DB_APP_TOKEN`
