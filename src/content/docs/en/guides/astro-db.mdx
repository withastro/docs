...
<p><Since pkg="@astrojs/db" v="0.13.0" /></p>
...
- `libsql+http:` will use a remote server through HTTP. `libsql+https:` can be used to enable a secure connection. The host is the same as for `libsql:`.
...
- `file:path/to/file.db?encryptionKey=your-encryption-key`
- `libsql://your.server.io?encryptionKey=your-encryption-key`

### `syncUrl`

Passing a separate connection URL on this search parameter to turn the DB into an embedded replica of another of another database. This should only be used with the schemes `file:` and `memory:`. The parameter must be URL encoded.
...
For example, to have an in-memory embedded replica of a database on `libsql://your.server.io`, you can set the connection URL to `memory:?syncUrl=libsql%3A%2F%2Fyour.server.io`.
