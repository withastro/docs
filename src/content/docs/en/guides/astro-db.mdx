...
You will not have access to production data during development unless you [specify it yourself](#connecting-to-remote-databases). This protects your data while allowing you to test and develop with type-safety using simulated data.
...
- `ASTRO_DB_APP_TOKEN`: the auth token to your libSQL server. This is required for remote databases, not needed for [local DBs like files or in-memory](#url-scheme-and-host) databases
...
The [Actions feature](/en/guides/actions/) provides a more straightforward approach for inserting data into Astro DB. Here's an example of how to perform a database insert using Actions:
