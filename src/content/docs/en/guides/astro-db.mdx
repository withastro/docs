...
Astro DB can connect to any LibSQL server from any platform that exposes the Hrana protocol of the server, or host it yourself.
...
The [commands for deploying and pushing changes](#deploy-with-a-studio-connection) to your database are the same when using LibSQL  
 as when connecting to a Studio hosted database. However, both of your environment variables need to be set locally when running commands with the `--remote` option like `astro build` and `astro db push`.
