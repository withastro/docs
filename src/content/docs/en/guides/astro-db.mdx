...
Astro DB is a complete solution to local and remote database development. A local database is created whenever you run `astro dev`, using LibSQL to manage your data without the need for Docker or a network connection. 
...
Tables also power Astro's automatic TypeScript typings for your content. When you define a table, Astro will automatically generate and apply a TypeScript interface to it. The result is full TypeScript support when you query your data, including property autocompletion and type-checking.
