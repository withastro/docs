...
Define your tables in your `db/config.ts` file by providing the structure of the data in your existing libSQL database, or the data you will collect in a new database.
...
:::caution
Table names in Astro DB are case-sensitive. Using a different case (e.g., `comment` instead of `Comment`) would result in creating a new, separate table rather than referencing the existing one.
:::
...
To seed development data for testing and debugging into your Astro project, create a `db/seed.ts` file. Import both the `db` object and any configured table from `astro:db`. Use the `db` handle (the same as given to your application) to insert the initial data into the development DB. This development data should match the form of your database schema and production data.
...
- `ASTRO_DB_REMOTE_URL`: the connection URL to your libSQL DB
- `ASTRO_DB_APP_TOKEN`: the auth token to your libSQL server. This is required for remote databases, not needed for local DBs like files or in-memory databases

The way you get these values varies between services, but the next section will show you how you can get started with [Turso](https://turso.tech).

Turso is the company behind [libSQL](https://github.com/tursodatabase/libsql), the open-source fork of SQLite that powers Astro DB. They created and maintain the libSQL project, making them the perfect choice to provide a fully managed libSQL database platform.

### Getting started with Turso

You can either use the CLI or the web interface to create and retrieve the values you need.
The steps that are shown below will walk you through the process of installing the Turso CLI, logging in (or signing up), creating a new database, getting the two values we need and pushing our schema to the remote database.
...
3. Next, create a new database. In this example the database name is `andromeda`, but you can change it to any other name.

   ```sh "andromeda"
   turso db create andromeda
   ```

4. You can see the information about the newly created DB with the following command:

   ```sh "andromeda"
   turso db show andromeda
   ```

   Use the value from the row `URL:` as the environment variable `ASTRO_DB_REMOTE_URL`.
   
   :::tip
   To read more about .env files and environment variables, check out the [Using environment variables](https://docs.astro.build/en/guides/environment-variables) guide.
   :::
...
5. To authenticate requests to the database, create a new token for it:

   ```sh "andromeda"
   turso db tokens create andromeda
   ```

   Use the output of the command as the environment variable `ASTRO_DB_APP_TOKEN`
...
This was a quick guide on how to get started with Turso, read on to learn more on how to use Astro DB.

You can deploy your Astro DB project with a live connection to your libSQL-compatible database. This is possible with any deployment platform using any static or server rendering mode.

Configure your build command to connect to your libSQL-compatible database using the `--remote` flag. This example applies the flag to a `"build"` script in your project's `package.json`. If your deployment platform accepts a build command, ensure this is set to `npm run build`.
...
### Seed from your remote database
...
To connect to your hosted remote database, you can add the `--remote` flag. Use this flag for production deploys to have both readable and writable access to your remote database. This will allow you to [accept and persist user data](#insert).
