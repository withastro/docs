...
If you deploy your site to a self-managed host such as a [virtual private server](https://en.wikipedia.org/wiki/Virtual_private_server), you can choose to use a database file instead of connecting to Astro Studio.

:::caution
Using a database file is an advanced feature, and care should be taken when deploying to prevent overriding your database. This method will also not work in serverless deployments, as the file system is not persisted in those environments. For a fully managed solution, try [connect to the Astro Studio platform](/en/recipes/studio/) instead.
:::

To use a database file, set the `ASTRO_DATABASE_FILE` environment variable. This path should point to your `.db` file within the host environment. It's important that you *do not* override this file on deployment, ever, or you will lose your production data. Additionally, any schema migrations must be managed manually using this method.

If you're comfortable with the risks and are able to manage deployment yourself, you can set the `ASTRO_DATABASE_FILE` variable during your build and omit:
