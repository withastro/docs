...
To use a remote connection, you will need an app token to authenticate with your database server. Refer to your database provider's documentation for token creation and setup instructions.

### TODO: maybe explain the process for turso? using both the CLI and the web ui

### Deploy your project with a live connection

You can deploy your Astro DB project with a live connection to your libSQL-compatible database. This is possible with any deployment platform using static builds or an [SSR adapter](/en/guides/server-side-rendering/).

First, configure your build command to connect to your libSQL-compatible database using the `--remote` flag. This example applies the flag to a `"build"` script in your project's `package.json`. If your deployment platform accepts a build command, ensure this is set to `npm run build`.
...
Your table schema will change over time as your project grows. You can safely test configuration changes locally and push to your remote database when you deploy.
