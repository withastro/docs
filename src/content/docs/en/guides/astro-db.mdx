...
When deploying to serverless environments that don't support all features of Node.js, such as Cloudflare Workers or Deno, you can switch to the web libSQL driver by setting the `mode` setting in the `astro.config.mjs` file, allowing for Astro DB to continue functioning in these environments.

<ReadMore>For more information, see the [`@astrojs/db` documentation](/en/guides/integrations-guide/db/#mode).</ReadMore>
