...
In both modes, most of the processing occurs on the server, which improves loading time and makes your site or app faster than client-side rendering when viewed on less-powerful devices or on slower internet connections. Server-rendered HTML is fast, SEO friendly, and accessible.


## SSG by default

All Astro projects are SSG by default to provide the most lightweight browser experience. The client does not need to wait for any HTML to build because the server does not need to generate your pages. Your site is not dependent on the performance of a backend data source, and once built, will remain available to visitors as long as your server is functioning.

SSG sites can include [Astro islands](/en/concepts/islands/) for interactive UI components (or even entire embedded client-side rendered apps!) written in the [UI framework of your choice](/en/core-concepts/framework-components/) in an otherwise static, server-rendered page.

Also available in SSG mode are Astro's [View Transitions API](/en/guides/view-transitions/) for animation and state persistence across page navigation, as well as [middleware](/en/guides/middleware/) to intercept and transform response data from a request. This makes Astro SSG mode a powerful, modern-feeling choice for content-heavy sites that update infrequently, and display the same page content to all visitors.

## Opt in to SSR

SSR brings the advantages of fresh page content and [several other features](#ssr-features), but at a potential performance cost compared to SSG. Pages must be generated on demand before they can be served to the client. If data is fetched from external sources, then their speed also becomes a factor in your page load time. And, even if no data has changed, a page must still be re-generated server-side before rendering and so a caching strategy may be required.

Despite the extra complexity and potential performance issues compared to SSG, SSR mode enables app-like behaviour that is more performant and more accessible than traditional single-page JavaScript applications (SPA). Astro's SSR mode still allows you to pre-render individual pages, achieving the performance benefits of SSG for the routes that do not need to be generated on-demand. 

SSR apps can also include [Astro islands](/en/concepts/islands/) for interactivity (or even entire embedded client-side rendered apps!) in your choice of [UI frameworks](/en/core-concepts/framework-components/). With [middleware](/en/guides/middleware/) and Astro's [View Transitions API](/en/guides/view-transitions/) for animations and preserving state across route navigations, SSR in Astro provides a true app experience without the JavaScript overhead of a traditional SPA.
...
### Configure an SSR mode

Astro provides two SSR modes to ensure that you can take still advantage of SSG performance by pre-rendering whenever possible, whether you have an entirely server-rendered app or a mostly static site that needs SSR only for select routes. Both server modes require you to [add an adapter](#adding-an-adapter) to carry out the server functions.

Each mode has a different default setting: [pages](/en/core-concepts/astro-pages/) and [server endpoints](/en/core-concepts/endpoints/#server-endpoints-api-routes) can either be server-rendered or pre-rendered by default. Both modes allow individual routes to opt-out of the default accordingly:
...
To enable SSR, update your `output` configuration to one of the two SSR modes. 
...
You can then override your SSR mode's default behavior for any page or route by exporting a value for `prerender`, either `true` or `false` as appropriate:
...
```
See more examples of [specifying rendering behavior on a per-page basis](#per-page-prerendering) for these two SSR options.
