...
**Static site generation (SSG)** refers to generating HTML pages during the build process, before they are requested by the client. The same page is sent to the browser for every visitor, and a full-site rebuild is required to update the contents of the page.

**Server-side rendering (SSR)** refers to generating HTML pages on the server on-demand just before sending them to the client. Since they are generated per visit, these pages can be customized for each viewer. For example, this page can show a logged in user their account information, or can display updated data without requiring a full-site rebuild.

In both modes, the processing occurs on the server, which improves loading time and makes your site or app faster than client-side rendering when viewed on older devices or on slower connections.

## SSG by default

All Astro projects are SSG by default to provide the most lightweight browser experience. The client does not need to wait for any HTML to build, and the server does not need to generate your pages. Your site or app is not dependent on the performance of a back end data source, and once built, will remain available to visitors as long as your server is functioning.

SSG sites can still take advantage of Astro islands for client-side interactivity, as well as middleware to intercept and transform response data from a request. This makes Astro SSG mode a powerful choice for sites that update infrequently, and display the same page to all visitors.

## Opt in to SSR

SSR brings the advantages of fresh page content and [several other features](#ssr-features) at a potential performance cost. Pages must be generated on demand and are not ready to be served immediately. If data is fetched from external sources, then their speed also becomes a factor in your page load time. And, even if no data has changed, the page must still be re-generated server-side before rendering and so a caching strategy may be required.

Astro provides several optimized adapter integrations to perform these server actions. Despite the extra configuration and potential performance issues compared to SSG, SSR mode enables app-like behaviour that is more performant than traditional single-page JavaScript applications (SPA). Combined with Astro's [view transitions API](/en/guides/view-transitions/), SSR in Astro provides a true app-like experience without the JavaScript overhead of a traditional SPA.

Astro provides [two server-rendered options]() to ensure that you can take still advantage of SSG performance by pre-rendering whenever possible in SSR mode: a mostly server-rendered app, and a mostly static site that needs SSR only for select routes. Both server modes require you to [add an adapter]() to carry out the server functions.
...
To enable SSR features for production deployments, update your `output` configuration to one of the two available SSR modes: `'server'` or `'hybrid'`. 

Both of these modes control which [pages](/en/core-concepts/astro-pages/) or [server endpoints](/en/core-concepts/endpoints/#server-endpoints-api-routes) should be server-rendered. Each configuration option has a different default behavior, and allows individual routes to opt-out of the default accordingly:
...
For example, to enable an SSR app, add `output: server` to your Astro config: 
...
```
See more usage examples of [configuring individual routes](#configuring-individual-routes) for these two SSR options.
...
### Opting-out of server-rendering in `server` mode:
