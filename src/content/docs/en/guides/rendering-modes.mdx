...
SSG sites can still take advantage of Astro islands for client-side interactivity, as well as middleware to intercept and transform response data from a request. This makes Astro SSG mode a powerful choice for sites that update infrequently, and display the same page content to all visitors.
...
Despite the extra configuration and potential performance issues compared to SSG, SSR mode enables app-like behaviour that is more performant than traditional single-page JavaScript applications (SPA). Combined with Astro's [view transitions API](/en/guides/view-transitions/) during navigation, SSR in Astro provides a true app experience without the JavaScript overhead of a traditional SPA.
...
Astro provides [two server-rendered options](#choose-an-ssr-mode) to ensure that you can take still advantage of SSG performance by pre-rendering whenever possible in SSR mode: a mostly server-rendered app, and a mostly static site that needs SSR only for select routes. Both server modes require you to [add an adapter](#adding-an-adapter) to carry out the server functions.
