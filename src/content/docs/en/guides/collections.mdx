...
Existing content and data collections using the original Content Collections API will continue to function alongside collections using the Content Layer API for a transition period. No change to your existing collections is necessary until you decide to update them. You can even have both kinds of collections defined in your content configuration file at the same time.
