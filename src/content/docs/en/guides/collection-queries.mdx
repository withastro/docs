...
Use [`getCollection()`](/en/reference/api-reference/#getcollection) to query your entire collection, and [`getEntry()`](/en/reference/api-reference/#getentry) to query a single entry. These return a unique `id`, a `data` object with all defined properties, and a `body` containing the raw, uncompiled body of a Markdown/MDX document.
...
```astro title="src/pages/posts/[slug].astro" "{ slug: post.id }"
---
import { getCollection } from 'astro:content';
