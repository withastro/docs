...
- [What happens if some of my pages don't exist in one of my supported languages?](#fallback)

1. **How can I prevent my home page from redirecting to my default language?**

      Set `redirectToDefaultLocale: false` in your routing config object. This allows you to have a site home page that exists outside of your configured locale folder structure. ([`redirectToDefaultLocale`](#redirecttodefaultlocale))

2. **Can I support regional variations of a language?**

    Yes. To create one URL path for all varities of a language, list each variation in your site's supported `locales` and map them to a custom URL path. ([custom locale paths](#custom-locale-paths))

3. **Can I access the browser settings for language preference of my visitors?**

    Yes, for pages rendered on demand. `Astro.preferredLocale` and `Astro.preferredLocaleList` have access to the browser’s `Accept-Language` header and will compare them to your supported `locales` to determine which language to show your visitor. ([browser language detection](#browser-language-detection))


### Community question and answers

Find people asking and answering questions about internationalization (i18n) in Astro:

<CommunityLinks
  query="i18n"
  stackOverflow="Search for questions about internationalization (i18n) on Stack Overflow"
  reddit="Search for questions about internationalization (i18n) on Reddit"
