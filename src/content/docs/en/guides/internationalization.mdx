...
The configuration `i18n.fallbackType` allows to change the behavior of the fallback strategy. By default, Astro will redirect to the fallback locale when a page is not found. You can change this behavior to show the content of the fallback locale without redirecting.

When `i18n.fallbackType` is set to `"rewrite"`, Astro will show the content of the fallback locale without redirecting. This means that the URL will remain the same, but the content will be from the fallback locale.
...
export default defineConfig({
  i18n: {
    ...
    fallbackType: "rewrite" // the value `"redirect"` is the default behaviour and value
  }
})
```
