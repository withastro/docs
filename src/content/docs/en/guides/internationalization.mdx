...
This routing option allows you customize your domains on a per-language basis for `server` rendered projects using the [`@astrojs/node` adapter](/en/guides/integrations-guide/node/) with a `site` configured.
...
```js title="astro.config.mjs" {3-7} ins={12-19}
import { defineConfig } from "astro/config"
export default defineConfig({
