...
This routing option allows you to customize your domains on a per-language basis for `server` rendered projects using the [`@astrojs/node` adapter](/en/guides/integrations-guide/node/) with a `site` configured.
