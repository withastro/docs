...
:::tip[You don’t have to duplicate every page per locale]
There are two common ways to structure i18n routes:

1. **Per-locale folders** (e.g. `src/pages/fr/...`) — great when each locale has its own page files.
2. **A single dynamic locale route** (e.g. `src/pages/[...lang]/...`) — useful when your default locale has **no URL prefix** (so `lang` can be `undefined`), or when pages are generated dynamically (e.g. from content collections).
:::

#### Alternative: dynamic locale routes with `[...lang]`
...
:::note[Default locale without a URL prefix]
When `routing.prefixDefaultLocale` is `false` (the default), your default language does **not** have a `/<lang>/` segment in the URL.

If you use a rest-parameter route like `src/pages/[...lang]/...`, include an `undefined` entry in `getStaticPaths()` so Astro also builds the unprefixed default-locale routes:
...
You can then use `Astro.currentLocale` in your templates for a consistent locale string.
:::
...
Routing localizes URLs, but you still need to translate UI labels (navigation, buttons, etc.). One lightweight approach is to create a dictionary of UI strings and a helper function that picks the right string based on the current locale.
...
In static mode, a language picker is often the simplest way to let users switch locales. Generate locale-aware links with `getRelativeLocaleUrl()` so you don’t have to handle prefixes manually:

```astro title="src/components/LanguagePicker.astro"
---
import { getRelativeLocaleUrl } from "astro:i18n"

// Use the same values you configured in `i18n.locales`
const locales = ["en", "fr"]
---
<ul>
  {locales.map((locale) => (
    <li><a href={getRelativeLocaleUrl(locale)}>{locale}</a></li>
  ))}
</ul>
```
