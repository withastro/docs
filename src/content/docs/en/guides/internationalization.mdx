...
Astro uses a [middleware](/en/guides/middleware/) to implement its routing logic. This middleware function is placed in the [first position](/en/guides/middleware/#chaining-middleware), even before the user's middleware, and it checks every `Response` coming from each route.

This means two things:
- the i18n middleware business logic comes into play **after** a route is rendered;
- any operation (redirects, etc.) done in the users middleware are checked by the i18n middleware;
