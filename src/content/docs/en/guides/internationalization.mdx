...
This routing option allows you customize your domains on a per-language basis for `server` rendered projects using the [`@astrojs/node` adapter](/en/guides/integrations-guide/node/) and a `site` configured.
