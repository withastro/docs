...
2. Choose and configure a [`routingStrategy`](#routingstrategy) based on the desired URL path for your `defaultLocale`:
  ...
  - [`"prefix-other-locales"`](#prefix-other-locales) (default): URLs in your default language will **not** have a `/[locale]/` prefix. All other locales will.

  - [`"prefix-always"`](#prefix-always): All URLs, including your default language, will have a `/[locale]/` prefix.

  - [`"domain"`](#domain): URLs can be configured to support multiple domains (or sub-domains) instead of using a `/[locale]/` prefix.
...
You must also specify the URL domain pattern to use in a `domains` configuration object. For any `locales` not configured in `domains`, the fallback URLs created will follow [`prefix-other-locales`](#prefix-other-locales), and create a `/[locale]/` path for all non-default languages.
