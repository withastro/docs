...
**SSR only**, use this strategy when you need to support multiple domains. 
...
The `/locale/` folders **must** be placed under the `pages/` folder.
...
So make sure that your server/hosting platform is able to provide such information. Failing to retrieve these headers will result in a 404 (status code) page.
