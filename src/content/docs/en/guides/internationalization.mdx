...
2. Organize your content folders with localized content by language. Your folder names must match the items in `locales` exactly, and your folder organization must match the URL paths chosen for your [`routingStrategy`](#routing-strategy). Include a localized folder for your `defaultLocale` only if you configure `prefix-always` to show a localized URL path.
    <FileTree>
    - src
      - pages
        - about.astro
...
Your `/[locale]/` folder names must match exactly the `locales` in the list, and every supported language must have its own localized folder. 
...
This is the **default** value. With this option, URLs in your default language will **not** have a `/[locale]/` prefix.
...
With this option, all routes will have their locale code (prefix) in their URL. All page content files, including those for your `defaultLocale`, must exist in a localized folder to match the URL structure of the chosen strategy.
...
- `src/pages/[locale]/blog.astro` will all produce routes of the form `example.com/[locale]/blog/`.
