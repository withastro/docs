...
1. **Does prefetch work in all browers?**

    Astro's prefetching works differently in each browser depending on its supported features, but most common browsers are supported. Astro's prefetching uses `<link rel="prefetch">` if supported by the browser, and falls back to the `fetch()` API otherwise. ([browser support](#browser-support))

2. **How does prefetch work with view transitions?**

    When you use View Transitions on a page, prefetching will also be enabled by default. You can customize the prefetch configuration in `astro.config.mjs` to override the default. ([finding more integrations](#finding-more-integrations))

3. **Can I still use prefetch for other links like inside buttons?**

    You can prefetch programmatically with the `prefetch()` API from the `astro:prefetch` module. This can be useful for navigation that might not always appear as `<a>` links. ([prefetch programmatically](#prefetch-programmatically))

### Community question and answers

Find people asking and answering questions about prefetching in Astro:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="prefetching"
  stackOverflow="Search for questions about prefetching on Stack Overflow"
  reddit="Search for questions about prefetching on Reddit"
