...
If you deploy to Netlify, Vercel, or Cloudflare, use the adapter's [CDN cache provider](#adapter-cache-providers) instead of the in-memory provider.

## Adapter cache providers

Astro's first-party adapters for Netlify, Vercel, and Cloudflare each provide a CDN cache provider that maps cache directives to the platform's native cache headers and invalidation API. Rather than storing responses in memory, these push your caching directives to the host's edge network, and cache hits are served straight from the CDN without invoking your server function.

For now, you configure these providers manually, as shown below. In a future release, the Netlify and Vercel providers will be enabled automatically by their adapters. The Cloudflare provider requires a feature that is currently in private beta.
...
The Cloudflare provider requires the Cloudflare Workers Cache feature, which is currently in private beta. Without beta access it does not work and should not be used: Cloudflare Workers run in front of the cache, so cached responses are never served, and calling [`cache.invalidate()`](/en/reference/api-reference/#cacheinvalidate) throws an error. Only configure this provider if your account has access to the beta.
:::
