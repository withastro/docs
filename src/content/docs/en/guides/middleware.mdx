...
The `APIContext` exposes a method called `rewrite`, which works the same way as [Astro.rewrite](/en/guides/routing/#rewrite).
...
When you have multiple middleware functions chained via [sequence()](#chaining-middleware), submitting a path to `next()` will rewrite the `Request` in place and **it _won't_ execute again the middleware**. The next middleware function in the chain will receive the new `Request`:
...
async function first(_, next) {
  ...
  // Return updated `ctx` which is passed to next function
  return next("/")
}
