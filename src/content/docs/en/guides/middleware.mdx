...
A low-level API to create an [`APIContext`](https://docs.astro.build/en/reference/api-reference/#endpoint-context), that can be used by the Astro middleware.

This function can be used by integrations/adapters to programmatically execute the Astro middleware.

### `trySerializeLocals`

A low-level API that takes in any value and tries to return a serialized version - a **string** - of it. If the value can't be serialized, the function will throw a **runtime error**.