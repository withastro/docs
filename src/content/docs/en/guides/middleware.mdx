...
export function onRequest (context, next) {
  if (!isLoggedIn(context) {
    // The user is not logged in, then we rewrite the current `Request` with a new one, and create an header
    // The header will tell to the `/login` where the user should be sent after a successfully login.
...
The `next` function accepts the same payload of the [`Astro.redirect() function`](/en/reference/api-reference/#astrorewrite).

In case you have multiple middleware functions chained via [sequence](#chaining-middleware), using `next("/")` with  a payload will rewrite the request **in place** which means that the next middleware function will receive the **new `Request`**:

```js
// This is the first middleware function that will be triggered by Astro 
async function first(_, next) {
  // Let's suppose the current URL is https://exmpale.com/blog
  console.log(ctx.url.pathname) // this will log "/blog"
  // We rewrite to a new route, the homepage 
  return next("/")
}

// This is the second middleware function that will be triggered by Astro
async function second(ctx, next) {
  // `ctx` will contain new information, computed by the previous rewrite
  console.log(ctx.url.pathname) // this will log  "/"    
  return next()
}
...
:::caution
Contrary to `Astro.redirect()` and `APIContext.rewrite`, `next(payload)` **won't** execute a new rendering phase, hence the middleware won't be called again.
:::
