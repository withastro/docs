...
1. **How can I customize page content for each user?**

    Middleware allows you to store data in a `locals` object that is available in all Astro components and API endpoints before a page is rendered. This allows you to modify page content before it is shown to your visitor. For example, you can show logged-in users a different version of the page. ([storing data in `context.locals`](#storing-data-in-contextlocals))

2. **Can I use Astro middleware with static SSG sites?**

    Yes. Astro middleware will run at build time to intercept requests and responses for prerendered pages. However some data, such as cookies and page headers, is only available when pages rendered on-demand. ([overview](#_top))

3. **Is it possible to define a prop in an Astro component that can be accessed by Middleware?**

    No. The flow of data is the other way around. In Astro, you define `locals` in middleware and then you can access this object in your Astro components. ([storing data in `context.locals`](#storing-data-in-contextlocals))

### Community question and answers

Find people asking and answering questions about middleware in Astro:

<CommunityLinks
  query="middleware"
  stackOverflow="Search for questions about middleware on Stack Overflow"
  reddit="Search for questions about middleware on Reddit"
