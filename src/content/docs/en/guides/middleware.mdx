...
In `dev` mode, Astro perform checks and will throw an error if `locals` are be overridden.
The value of locals needs to be an object, and it can't be overridden at runtime. Doing so would risk wiping out all the information stored by the user.
