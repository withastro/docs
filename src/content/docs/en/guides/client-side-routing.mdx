...
Support for client-side routing in Astro projects can be enabled behind an experimental flag. It is powered by the new [View Transitions](https://developer.chrome.com/docs/web-platform/view-transitions/) browser API, with fallback options for non-supporting browsers.

Using the View Transitions support you can have seamless animations between pages. Astro comes with:

- A `<ViewTransitions />` component that does the routing for you.
...
- Fallback support for browsers that do not yet support the View Transition APIs.
...
For example, you might have a head component that you can update with:
