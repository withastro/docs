...
If you changed [Astro's default (`strict`) TypeScript setting](https://docs.astro.build/en/guides/typescript/#tsconfig-templates) to `base` because you are not writing TypeScript in your project, you will need to also add the following `compilerOptions` in your `tsconfig.json` to use content collections:

```json title="tsconfig.json" ins={4-7}
{
  "extends": "astro/tsconfigs/base",
  // only needed if you changed the default to `base`
  "compilerOptions": {
...
Astro provides two built-in loaders (`glob()` and `file()`) for fetching your local content at build time. To fetch remote data at build time, you can [build a custom loader](#custom-build-time-loaders) or use [community-published loader integrations](https://astro.build/integrations/2/?search=&categories%5B%5D=loaders) for common data sources such as Obsidian vaults, GitHub repositories, or Bluesky posts.
...
You can also transform each collection entry `id` programmatically using the [`generateID()` helper function](/en/reference/content-loader-reference/#generateid) when you define your build-time collection:
...
Support for parsing JSON, YAML, and TOML files into collection entries is built-in (unless you have a [nested JSON document](#nested-json-documents)). To load your collection from unsupported file types, such as `.csv`, you will need to create a [parser function](/en/reference/content-loader-reference/#parser).
...
You can build a custom loader using the [Content Loader API](/en/reference/content-loader-reference/) to fetch remote content from any data source, such as a CMS, a database, or an API endpoint.
...
This is useful for simple loaders that don't need to manually control how the data is loaded and stored. Whenever the loader is called, it will clear the store and reload all the entries.
...
The returned entries are stored in the collection and can be [queried using the `getCollection()` and `getEntry()` functions](#querying-build-time-collections).

#### Object loaders

For more control over the loading process and the data store, you can use the Content Loader API to create a custom [object loader](/en/reference/content-loader-reference/#object-loaders). This calls a `load()` method at build time to fetch data and update the data store. For example, you can create a loader that allows entries to be updated incrementally or clears the store only when necessary.

Import and define your custom object loader in your collections config, passing any required values.
...
A live loader is an object with two methods: [`loadCollection()`](/en/reference/content-loader-reference/#loadcollection) and [`loadEntry()`](/en/reference/content-loader-reference/#loadentry). These methods handle errors gracefully and return either data or an [Error](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error) object.
...
export function articleLoader(config: { apiKey: string }): LiveLoader<Article> {
  return {
    ...
    loadEntry: async ({ filter }) => {
      try {
        // filter will be { id: "some-id" } when the second parameter of getLiveEntry() is called with a string
...
To render live content using [the same `render()` function and `<Content />` component](/en/guides/content-collections/#rendering-body-content) available to build-time collections, your live loader must return [a `rendered` property](/en/reference/content-loader-reference/#rendered) in the entry. 
...
If your live loader [returns a `rendered` property](#providing-rendered-content), you can use [the `render()` function and `<Content />` component](#rendering-body-content) to render your content directly in your pages, using the same method as build-time collections.
