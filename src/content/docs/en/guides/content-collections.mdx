...
The `file()` loader will automatically detect and parse a single array of objects from JSON, YAML, and TOML files (based on their file extension) with no need for a `parser` unless you have a [nested JSON document](#nested-json-documents). To use other files, such as `.csv`, you will need a to create a parser function.
