...
The exact method for generating routes will depend on your build [`output`](/en/reference/configuration-reference/#output) mode: `static` (the default) or one of Astro's [on-demand rendering modes](/en/core-concepts/rendering-modes/#on-demand-rendered) (`server` or `hybrid`).
...
### Building for on-demand server rendering

If you are building a website or app that displays your pages dynamically upon request, you will not generate paths ahead of time using `getStaticPaths()`. Instead, your page will examine the request (using `Astro.request` or `Astro.params`) to find the `slug` on-demand, and then fetch it using [`getEntry()`](/en/reference/api-reference/#getentry).
