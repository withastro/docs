...
Collection entries can also "reference" other related entries. A common example is a blog post that references an author stored in an `authors` collection, or other related posts stored in the same collection.

You can define these relationships using the `reference()` utility from your collection schema. This accepts a collection name, and validates the entry identifier(s) specified in your content frontmatter or data file.
...
const oneBlogPost = await getEntry('authors', 'grace-hopper');
```
