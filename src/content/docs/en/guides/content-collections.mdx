...
7. Replace `Astro.glob()` with [`getCollection()`](/en/reference/api-reference/#getcollection) to fetch content and metadata from your Markdown files. You will also need to update references to the returned post object, since you will now find your frontmatter values on the `data` property.
    ...
    ```js title="src/pages/tags/[tag].astro" "post.data" "getCollection(\"posts\")" "post.data.title"
    ...
    export async function getStaticPaths() {
      const allPosts = await getCollection("posts");
...
8. Update the logic of extracting publish date in the `layouts/MarkdownPostLayout.astro` file.

    Previously, `pubDate` was a string. Now, after introducing types for our posts' frontmatter, `pubDate` is a `Date`.
    Thanks to that, we can apply `Date` methods, like `toDateString()` on `pubDate`.
    ...
    Without that change, our blog posts would not render, since `slice` method does not exist on `Date`.
