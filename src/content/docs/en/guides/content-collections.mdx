...
If you are working with data collections, you may wish to generate JSON Schema files to get better IntelliSense and type-checking in your editor. Astro provides a flag to auto-generate a JSON Schema for content collections of `type: 'data'` which can be used as the `$schema` value. This feature generates JSON Schema files for each data collection in your project, using a library called [zod-to-json-schema](https://github.com/StefanTerdell/zod-to-json-schema#known-issues)

To enable this feature, add the flag to your `contentCollection` configuration:
...
This feature requires you to manually reference the schema, which you can either do in each file or in your editor settings:
...
Each editor has it's own place to configure this, but in VSCode you can use the [`json.schemas` setting](https://code.visualstudio.com/docs/languages/json#_json-schemas-and-settings):
