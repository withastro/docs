...
If you are working with large Content Collections, it is possible to enable cached builds with the new [`experimental.contentCollectionCache`](/en/reference/configuration-reference/#experimentalcontentcollectioncache) flag. This experimental feature optimizes Astro's build process, enabling unchanged Content Collections to be stored and reused between builds.
