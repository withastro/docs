...
If the custom slug for your content contains `/` character, meaning it has multiple levels, you must use [rest parameters](/en/core-concepts/routing/#rest-parameters) for the dynamic route that generates it. Refer to the example code [here](/en/guides/content-collections/#building-for-static-output-default).
