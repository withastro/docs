...
You can also transform all entry `id`s for a collection programmatically after they are slugified. Define your transformation using the [`generateID()` helper function](/en/reference/content-loader-reference/#generateid) when you define your build-time collection. For example, you may wish to revert the default behavior of converting uppercase letters to lowercase:
...
You can build a custom loader using the [Content Loader API](/en/reference/content-loader-reference/) to fetch remote content from any data source, such as a CMS, a database, or an API endpoint. A loader can be a single inline async function that fetches and returns data. Custom [object loaders](#object-loaders) offer more precise control over data handling, and also interact with the persistent data store.
