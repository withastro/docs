...
Call `getCollection()` inside of `getStaticPaths()` to [query your content or data collection](https://docs.astro.build/en/guides/content-collections/#querying-collections). Then, create your new URL paths using the `slug` property (content collections) or `id` property (data collections) of each content entry.
