...
// 2. check if the slug identifier matches the filename
if (slug === undefined) {
    throw new Error("slug/filename mismatch");
}
// 3. Query for the entry directly using the request slug
const entry = await getEntryBySlug('blog', slug);
// 4. redirect if the entry does not exist
if (entry === undefined) {
    return Astro.redirect("/404");
}
// 5. (Optional) render the entry to HTML in the template
const { Content } = await entry.render();
---
<h1>{entry.data.title}</h1>
<Content />
