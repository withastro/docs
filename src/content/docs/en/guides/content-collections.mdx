...
1. **How do I use frontmatter values from my collection items in my HTML?**

    An entry's frontmatter values, as well as full content, are available when you call the `render()` function. Destructure the frontmatter properties you want to use, and then access them from the entry's `data` object. ([rendering content to html](#rendering-content-to-html))

2. **Why am I not getting full TyepScript support like autocompletion for my content collections?**

    Content collections requires Astro's `strict` or `strictest` TypeScript setting. To continue using the `base` setting, you must enable `"strictNullChecks": true` in your `tsconfig.json`. ([setting up TypeScript](#setting-up-typescript))

3. **How do I make draft blog posts?**

    Add a `draft` property to the frontmatter of your post, then use `getCollection()` with a callback function to filter out your draft posts. Use this filtered query result in your dynamic routing `[page].astro` file to build the pages for your site. ([filtering collection queries](#filtering-collection-queries))

4. **What if I already have an existing Astro blog but want to switch to content collections?**

    You can follow a step-by-step tutorial to move your Markdown files from `src/pages/` to `src/content` and create a schema for your blog collection. Additionally, the tutorial will show you how create your own page routes for your content. ([migrating from file-based routing](#migrating-from-file-based-routing))

### Community question and answers

Find people asking and answering questions about content collections in Astro:

<CommunityLinks
  query="content collections"
  stackOverflow="Search for questions about content collections on Stack Overflow"
  reddit="Search for questions about content collections on Reddit"
