...
Как только вы создадите коллекцию, вы сможете начать [запрашивать содержимое](#querying-collections), используя встроенные API содержимого Astro.
...
### Использование схем коллекций сторонних производителей
...
Обе функции возвращают записи содержимого, определенные типом [`CollectionEntry`](/ru/reference/api-reference/#collection-entry-type).

### Доступ к данным по ссылкам

Любые [ссылки, определенные в вашей схеме](#defining-collection-references), должны запрашиваться отдельно после первого запроса к элементу коллекции. Вы можете повторно использовать функцию `getEntry()` или `getEntries()`, чтобы получить ссылающуюся запись из возвращаемого объекта `data`. 
...
Информацию о рендеринге контента в HTML см. в разделе [Рендеринг контента в HTML](/ru/guides/content-collections/#rendering-content-to-html) ниже.
...
В этом случае вы можете использовать утилиту [`CollectionEntry`](/en/reference/api-reference/#collection-entry-type) для корректного задания свойств компонента с помощью TypeScript.  Эта утилита принимает строковый аргумент, соответствующий имени схемы вашей коллекции, и наследует все свойства схемы этой коллекции.
...
Коллекции контента хранятся вне каталога `src/pages/`. Это означает, что по умолчанию для элементов коллекции не генерируются маршруты. Вам нужно вручную создать новый [динамический маршрут](/ru/guides/routing/#dynamic-routes), чтобы генерировать HTML-страницы из записей вашей коллекции. Ваш динамический маршрут будет отображать входящий параметр запроса (например, `Astro.params.slug` в `src/pages/blog/[...slug].astro`) для получения нужной записи в коллекции.
...
Вызовите [`getCollection()`](/ru/reference/api-reference/#getcollection) внутри `getStaticPaths()` для запроса содержимого. Затем создайте новые URL-пути, используя свойство `slug` каждой записи контента.
...
:::note
Если ваши пользовательские slug содержат символ `/` для создания URL с несколькими сегментами пути, вы должны использовать [rest-параметр (`[...slug]`)](/ru/guides/routing/#rest-parameters) в имени файла `.astro` для этой страницы динамической маршрутизации.
:::
...
Посмотрите, как преобразовать базовый пример блога из `src/pages/posts/` в `src/content/posts` в нашем [пошаговом руководстве](/ru/tutorials/add-content-collections/), которое использует кодовую базу из [готового проекта руководства Build a Blog](https://github.com/withastro/blog-tutorial-demo).

## Включение кэширования сборки

<p><Since v="3.5.0" /><Badge>Experimental</Badge></p>

Если вы работаете с большими коллекциями, вы можете включить кэширование сборок с помощью флага [`experimental.contentCollectionCache`](/ru/reference/configuration-reference/#experimentalcontentcollectioncache). Эта экспериментальная функция оптимизирует процесс сборки Astro, позволяя сохранять неизменные коллекции и повторно использовать их между сборками.
...
:::caution
**Не рекомендуется.** Плагины Remark и rehype обращаются к _сырому_ фронтматеру Markdown или MDX-документа. Это означает, что фронтматер `remarkPluginFrontmatter` обрабатывается отдельно от вашей безопасной для типов `схемы`, и не будет отражать никаких изменений или настроек по умолчанию, примененных через Astro. Используйте на свой страх и риск!
:::

Astro поддерживает плагины remark или rehype, которые [изменяют ваш frontmatter напрямую](/ru/guides/markdown-content/#modifying-frontmatter-programmatically). Вы можете получить доступ к этому измененному frontmatter внутри записи контента, используя свойство `remarkPluginFrontmatter`, возвращаемое из `render()`:
