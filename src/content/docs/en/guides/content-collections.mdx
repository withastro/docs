...
The correct date format you should use in your frontmatter should be `yyyy-mm-dd` (e.g. `2021-07-28`). 
...
You can also choose to truncate the string in your template and render only the `YYYY-MM-DD` from the full UTC timestamp. 
...
```
If you are interested in having more formatting options over the day, month, and year, then please look at this example `<FormattedDate />` [component](https://github.com/withastro/astro/blob/latest/examples/blog/src/components/FormattedDate.astro) in the official Astro blog template. 
