...
The `<Image />` component generates an `<img>` tag. This component can transform an image's dimensions, file type and quality to produce an optimized image that avoids Content Layout Shift **(CLS)**.
...
`quality` is an optional prperty that can either be a preset (`low`, `medium`, `high`, `max`) that is automatically normalized between formats or a number from `0` to `100` (interpreted differently between formats).
...
In addition to the properties above, the `<Image />` component accepts all properties accepted by the HTML `<img>` tag. For example, you can provide a `class` to the final `img` element.
...
`getImage()` takes all the same parameters as the Image component (except `alt`).
