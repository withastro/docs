...
Use the built-in `<Image />` Astro component to display optimized versions of your images.
...
`<Image />` can transform an image's dimensions, file type, and quality to produce an optimized `<img>` tag to avoid **Cumulative Layout Shift (CLS)**.
...
The source location of your image file depends on where your image file is located:
...
However, both of these properties are required for remote images and images stored in your `public/` folder.
...
The [Astro template syntax](/en/core-concepts/astro-syntax/) also supports writing an `<img>` tag directly, with full control over its final output.
...
- in `.mdx` files, you can use:
  - the above methods for `.astro` files, both [the HTML `<img>` element](#img) and [the Astro `<Image />` component](#image--astroassets). Remember that the image must be imported (AFTER any YAML frontmatter), and that the import is used differently in each case (e.g. img uses `src={image.src}` and Image uses `src={image}` directly.)
