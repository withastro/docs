...
This allows you to move your images from the `public/` directory to the new, reserved `src/assets/` directory (See the URL path of these built images in the example below.) or anywhere near your Markdown files.
...
2. Migrate any existing `<Image />` components.
    ...
    Remove any component attributes from your `<Image /> components that are not [currently supported image asset properties](#properties). For example `aspectRatio` is no longer supported, as it is now automatically inferred from the `width` and `height` attributes.
