...
We **strongly recommend** storing all images to be optimized in the `src/assets/` directory. This location is optional for now, but may become the required location for all asset types in the future.
...
- a preset (`low`, `medium`, `high`, `max`) that is automatically normalized between formats.
