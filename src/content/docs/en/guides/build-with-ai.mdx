...
- **Use `astro add` for Integrations**: Ask AI tools to use `astro add` for official integrations (`astro add tailwind`, `astro add react`). For other packages, intall using the package manager rather than editing `package.json` directly.
- **Verify Current APIs**: AI tools may use outdated patterns. Ask them to check the latest documentation, especially for newer features like Sessions, Actions and Content Collections, and particularly for experimental features which may have changed and may no longer be experimental.
