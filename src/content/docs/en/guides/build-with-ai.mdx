...
**Conversations with the chatbot are public, and are subject to the same server rules for language and behavior as the rest of our channels**, but they are not actively visited by our volunteer support members.
...
- **Use `astro add` for integrations**: Ask AI tools to use `astro add` for official integrations (`astro add tailwind`, `astro add react`). For other packages, install using the package manager rather than editing `package.json` directly.
