...
The `astro preview` command uses the same background mode as `astro dev`. When an AI coding agent is detected, or when you pass the [`--background` flag](/en/reference/cli-reference/#--background-1), the preview server starts as a detached background process and writes a lock file (`.astro/preview.json`) recording its URL, port, and PID.

To opt out of automatic background mode, set the `ASTRO_PREVIEW_BACKGROUND` environment variable before running `astro preview`:

```shell
ASTRO_PREVIEW_BACKGROUND=0 astro preview
```

<ReadMore>See the [CLI reference](/en/reference/cli-reference/#astro-preview) for the full list of `astro preview` flags and subcommands.</ReadMore>
