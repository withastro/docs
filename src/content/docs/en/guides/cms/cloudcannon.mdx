...
5. Configure how CloudCannon looks and behaves for editors by adding configuration to your CloudCannon config file. You can use CloudCannon's [configuration mode](https://cloudcannon.com/documentation/articles/what-is-configuration-mode/) to add config, without needing to write any code, or you can add config manually yourself in a code editor.
6. Your configuration file will be saved back to your repository with the rest of your files.

</Steps>

You can now explore your Site Dashboard to see your Astro site's content files. With no further setup required, edit any `.md` and `.mdx` files with our WYSIWYG Content Editor, and any `.yml`, `.json`, or other common data file types with our Data Editor. Later on we'll learn how to set up Editable Regions for use in CloudCannon's visual editor, allowing more flexibility and options for how and what we can edit in CloudCannon. Importantly, this will allow us to open and edit `.astro` files in the Visual Editor.
...
CloudCannon offers free hosting as part of all of it's plans, and is using Cloudflare under the hood. However, you can host with almost any hosting provider - the main requirement is that the hosting provider can deploy from a git repository.
...
When CloudCannon first builds your site, a unique `adjective-noun.cloudvent.net` placeholder url will be given to your site. This placeholder site is no-indexed by search engines by default. When you are ready, you can add a custom domain.
