...
In Astro's default `static` rendering mode, use [`getStaticPaths()`](/en/reference/api-reference/#getstaticpaths) to fetch your list of articles from Strapi.
...
If you've [opted into an on-demand rendering mode](/en/core-concepts/rendering-modes/#on-demand-rendered) with `output: 'server'` or `output: 'hybrid'`, [generate your dynamic routes](/en/core-concepts/routing/#on-demand-rendering-mode) using the following code:
