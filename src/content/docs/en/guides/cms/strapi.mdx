...
1. Create a Strapi server in the `backend` directory:
<PackageManagerTabs>
  <Fragment slot="npm">
  ```shell
...
3. In the future you can run the server with the following command:
...
```
:::note
In some cases using `localhost:1337` will not work, so just to be sure use the ip address
:::
...
### Optional: Create the interface(s)

If you are using typescript we will need to create the corresponding interface to the Strapi content types. Create the file `src/interfaces/article.ts` with the content below. Don't forget to modify this interface or even create more to fit your actual needs.
...
Now we are going to create (or modify) the `src/pages/blog/index.astro` file. Import our wrapper, then the interface and fetch the articles. The api returns a list containing the articles. We are going to use 2 parameters for the `fetchApi` function:
...
In this case the api call will make the request to the following url: `http://localhost:1337/api/articles`, and the data will be a list of jsons, similar to the one following:
```
[
  {
    id: 1,
...
```
Now we can move on to rendering the posts in a list.
...
:::tip
For the time of development you can add the `output: "server"` property to the `astro.config.mjs` file to enable SSR mode.
:::
...
Since we are using a redirect to `/404`, create a 404 page in `src/pages`: 
