...
Create the file `src/pages/blog/[slug].astro` to [dynamically generate a page](/en/basics/routing/#dynamic-routes) for each article.
...
If you've [opted into SSR mode](/en/guides/server-side-rendering/) with `output: server` or `output: hybrid`, [generate your dynamic routes](/en/basics/routing/#server-ssr-mode) using the following code:
