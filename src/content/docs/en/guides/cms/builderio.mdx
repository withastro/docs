...
To add your Builder API key to Astro, create a `.env` file in the root of your project (if one does not already exist) and add the following variable:
...
With the setup above, you can now create a blog with Astro and Builder.

### Creating a model for a blog post


In the Builder app create the model that will represent a blog post: go to the **Models** tab and click the **Create Model** button to create a model with the following fields and values:

- **Type:** Section
- **Name:** "blogpost"
- **Description:** "This model is for a blog post"

:::note[Why a section model?]
...
1. Before you can create new content in Builder, make sure [the preview URL for your model is working](#setting-up-the-preview).
2. Open the visual editor for a content entry of our `blogpost` model. If you've followed [the previous section](#testing-the-preview-url-setup), you can use that entry. Otherwise, create a new one: Go to the **Content** tab in your Builder space and click on **New**.
...
6. When you're ready click the **Publish** button in the upper right corner.
