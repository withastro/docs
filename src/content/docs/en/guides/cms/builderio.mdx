...
[] follow the tutorial: build blog ("part 2")
* problem with the flow of the instructions: there is no mention of the preview URL. idea 1: leave part 1 to be a content section model, then just assume that and build on that. idea 2: link back to the preview URL / mention it again. idea 3: give the instructions again
* i feel the sections could be a little more unified. this might make it easier to follow. things that stick out: <FileTree> is used sometimes, sometimes not; steps are sometimes numbered, sometimes not. 
[] dynamic preview URL??????
