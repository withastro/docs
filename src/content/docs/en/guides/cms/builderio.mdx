...
This component tells Builder where to insert the content from its CMS - it represents the portion of the page that's editable in the Builder app. In this guide the `<builder-component>` represents the content of a blog post and the `builder-preview.astro` page recreates how it will be embedded into the site later on - sitting between a <header> and a <footer>.

#### Setting the new route as the preview URL
1. Run your Astro project (e.g. start the dev server) and go to the route corresponding to the file you just created. Copy the URL to your clipboard. (It might be something like `https://localhost:3000/builder-preview`. The exact URL will depend on your project environment.)

2. Go to the **Models** tab in your Builder space, pick the model  you've created and paste the URL in the **Preview URL** field. Make sure the URL is complete and includes the protocol, for example `https://`.
...
1. Create a new content entry for your `blogpost` model and pick it to open the visual editor. If You've followed the steps under [testing it](/#testing-the-preview-url-set-up), you can use that content entry. Otherwise create it now: Go to the **Content** tab in your Builder space and click on *+ New*. You should see the Builder editor now:
...
[] go through code -- AI
  ...
  [] preview site and edit diffs to highlight appropiate lines
...
[] add links
[] add screenshots 
[] preview site - pay attention to how headings are being rendered in table of contents --- are they too long?
[] revise again --- AI
