...
Create a layout for a single blog post. Here we are using the `BaseLayout` as we left it at the end of our getting started with Astro tutorial [xxx].
...
:::note
This is one workflow among many. It demonstrates how You could split up the development of a site - the writers can design posts using Builder's visual interface, while other teams can handle more perennial or technical stuff. 
Feel free to experiment with other set ups and find one that suits your team's needs!
:::
...
Follow [Builder's official tutorial](https://www.builder.io/blog/creating-blog) for creating a blog post model - it haas gifs!
...
In that new model add some fields. This will be the data related to a blog post (without the written content). You can add whatever fields You like. For this tutorial we'll keep it simple:
...
xxx now you can go to /localhost/pages/foo and should be able to see your blog posts!
...
// 💥 TO REVIEWER: This is a lot of repeated code since we're making the same API call as in Blog.astro. 
