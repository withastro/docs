...
:::note[For visual types 👀]
You can find a short video showing this process in one of Builder's official tutorials [here](https://www.builder.io/blog/creating-blog#create-a-blog-article).
:::
...
[x] revise, simplify, clarify -- AI === info boxes
[x] code: query of the Qwik API. what if slug is not unique? it seems safer to query based on id. -> meh, either way if the slug isnt unique the result is fkkd
[x] prettify code: copy paste code into code editor to unify formatting and tab-spacing
[] fix typos -- AI
