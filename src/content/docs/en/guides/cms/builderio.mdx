...
:::note
The official Builder quickstart guide is [here](https://www.builder.io/c/docs/quickstart#step-1-add-builder-as-a-dependency) (in the list of frameworks pick 'REST API' on the far right). 
...
* **A Builder API key** - This public key will be used to fetch your content from Builder. Builder's guide on how to find it is [here](https://www.builder.io/c/docs/using-your-api-key#finding-your-public-api-key). 

### Foundational knowledge
This is some of the stuff that this guide assumes that you know and won't be explaining.

**You don't have to be an expert in any of these to finish this guide. When in doubt, just start hacking! If you get stuck, you can always ask for help on [our super friendly Discord 💜](https://astro.build/chat)**.
...
* **A basic knowledge of how Astro works** - Relevant topics for this guide: [basic structure of an `.astro` file](https://docs.astro.build/en/core-concepts/astro-pages/#astro-pages), [dynamic routes](https://docs.astro.build/en/core-concepts/routing/#dynamic-routes).

* **How to use a code editor** - You'll be adding files and code to your Astro project and starting a dev server. You can read a beginner-friendly introduction to these topics [here](https://www.tatianamac.com/posts/beginner-eleventy-tutorial-partii/#2.-source-code-editor).
...
You can find some gifs illustrating this procedure in one of Builder's official tutorials [here](https://www.builder.io/blog/creating-blog#creating-a-blog-article-model).
:::
...
:::note[For the visual types]
You can find a short video illustrating this process in one of Builder's official tutorials [here](https://www.builder.io/blog/creating-blog#create-a-blog-article).
:::

0. Before you can create new content in Builder, make sure [the preview URL for your model is working](#setting-up-the-preview-url). 

1. Open the visual editor for a content entry of our 'blogpost' model. If you've followed [the previous section](#testing-the-preview-url-set-up), you can use that entry. Otherwise create a new one: Go to the **Content** tab in your Builder space and click on **+ New**. 

1. In the panel on the right under the **options** tab, you should see the data fields `title` and `slug`. These are the fields you've created when you [set up your model](#creating-a-model-for-a-blogpost). Fill them in with the following values:
...
[x] try to understand the dynamic preview URL, again
...
[x] remove screenshot mentions
[] revise, simplify, clarify -- AI === info boxes
[] code: query of the Qwik API. what if slug is not unique? it seems safer to query based on id. 
[] prettify code: copy paste code into code editor to unify formatting and tab-spacing
[] fix typos -- AI
