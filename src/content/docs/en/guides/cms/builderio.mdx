...
4. A basic understanding of Builder - models, sections, preview URL and the REST API. mhmmmm [xxx] [xxx]
...
### Connecting models to components

To connect your newly created blog model to your Astro project, create a new folder named `pages` in your `src` directory, a `posts` folder within that and add the following files:

`[...slug].astro` stands for each blog post. It uses Astro's spread syntax() and creates a new route for each blog post. First, it fetches a list of all the slugs using Builder's ... API. Then it takes each slug and queries the API for the HTML of the matching blog post. This HTML is the stuff you put on the page when you used Builder's drag and drop editor!    
