...
:::note
The HackMD API client requires Node.js 22 or later.
:::
...
`getNoteList()` provides note metadata for the index and routes. `getNote()` retrieves the full Markdown content.

:::caution
Astro's `set:html` directive inserts an HTML string without escaping it. This example first passes the note through `markdown-it` with raw HTML disabled. If you enable the `html` option for trusted authors, sanitize the rendered result before passing it to `set:html`.
:::
