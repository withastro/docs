...
You can also start a new project from [the clean Sanity + Astro starter template][starter] that includes a basic setup.
...
This command will add the Sanity and React configuration to your `astro.config.mjs` file. This is where you will tell Astro what your Sanity project ID is, as well as the name of your dataset (most likely `production`).

The `@astrojs/react` dependency is only needed if you plan to embed the Studio on a route.

If you want to integrate [block content][portabletext] and [images from the Sanity CDN][asset-pipeline], these libraries will come in handy:
...
If you are using TypeScript, you'll also have to install the following libraries:
...
To initialise a new Studio and its dependencies with a new or existing Sanity project, run the following command:

```shell
npm create sanity@latest
```

When using the embedded Studio, run the following command to initalize a Sanity project and store the `projectId` and `dataset` variables in an `.env` file.

```shell
npx sanity@latest init --env
```

If you are unsure about the details of your project, go to [sanity.io/manage][manage] to find information about your projects.

### Client configuration

Copy and paste the `projectId` and `dataset` values from the `.env` file into the Astro configuration file. [Check out the environment variables guide][env-config] if you want to use the `.env file` for configuration.
...
If you're using Server Side Rendering, then set `useCdn` to `true` for performance and cost. You can also override this setting if you run the site in hybrid, for example `useSanityClient.config({useCdn: false}).fetch(*[_type == "liveBlog"])`.

### Configuration with embedded Studio:

Sanity Studio is where you can edit and manage your content. It‘s a Single Page Application, easy to configure, and can be customized in a lot of ways. It‘s up to you if you want to keep the Studio in a separate repository, in its own folder (as a monorepo), or embedded into your Astro website. For simplicity, this guide will show you how to embed the Studio on its own route.


To add Studio on `yoursite.com/admin`, set the following configuration:
