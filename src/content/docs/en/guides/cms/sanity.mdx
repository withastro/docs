...
You can use [Sanity](http://sanity.io) as a headless content management system (CMS) to drive content to Astro websites. It comes with a hosted document and asset store called Content Lake, a fully customizeable editing enviroment called Sanity Studio, and APIs that lets you integrate your content to pretty much anything.

## Quick start

The easiest way to get started is with the official [`@sanity/astro` integration][astro-sanity]. You can add it to an existing project or start with a new one.
