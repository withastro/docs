...
1. **An Astro project**- If you’d like to start with a fresh Astro project, read the [installation guide](/en/install/auto/). This guide uses the [Astro Headless CMS Theme](https://astro.build/themes/details/cosmic-cms-blog/) with [Tailwind CSS](https://tailwindcss.com/) for styling.
...
Start by using the [Cosmic JavaScript SDK](https://github.com/cosmicjs/cosmic-sdk-js) to fetch data from the Cosmic Bucket.

### Configuring API Keys

API keys are the bridge between Cosmic and an Astro project. In the top navigation menu of the Cosmic dashboard, navigate to **API keys** to retrieve both the **Bucket slug** and **Bucket read key**. Paste these into the **.env** file in your Astro project.
...
Astro's ecosystem works with dependencies that enhance its capabilities. To seamlessly pull content from Cosmic, add the [Cosmic JavaScript SDK](https://www.npmjs.com/package/@cosmicjs/sdk) and two more packages: [date-fns](https://www.npmjs.com/package/date-fns) (to format the date), and [react-markdown](https://www.npmjs.com/package/react-markdown) (to render markdown content from Cosmic).
...
Since we'll be using [react-markdown](https://www.npmjs.com/package/react-markdown) to render the content coming from Cosmic, add the [Astro React Integration](/en/guides/integrations-guide/react/).
...
Once you've installed the Cosmic JavaScript SDK and have the necessary API keys, initiate the Cosmic instance within the Astro project. Designing queries to fetch specific content types or items will be instrumental. As a quick test, create a simple Astro component to display the fetched data.

Place the functions to fetch Cosmic content in a file called `cosmic.js`. Place this file inside of the `src/lib` folder in your project. Place the following function inside of this file which then can be utilized in Astro components.
...
This function utilizes the chaining method `.find` from the Cosmic SDK. here, `.props` is used to fetch the desired metadata from the Object in Cosmic. Read more about [queries in the Cosmic documentation](https://www.cosmicjs.com/docs/api/queries).

The following example, shows fetching a list of blog posts from Cosmic directly inside an Astro component.
