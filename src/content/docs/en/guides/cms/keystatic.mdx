...
This guide assumes you already have **an existing Astro project** in SSR running on [`output: 'hybrid'`](<>). If you don’t, you can follow the instructions in our [Installation guide](/en/install/auto/).
...
## Setting up Keystatic Admin UI dashboard

{/* In our `pages` directory, every route within the `/keystatic` segment will become a Keystatic Admin UI route. */}

{/* So that you can import and mount this app here with React, create a new file outside of the pages directory. */}

The Keystatic Admin UI is a React app that runs in your Astro project that lives in your `pages` folder. The following steps will set it up in your Astro project.

Create a `keystatic.page.ts` in the project root, alongside your Keystatic config file.
...
Create a new page called `src/pages/keystatic/[...params].astro` and mount the component on the client side only:
...
To read and write data, you need to create some `API routes` for Keystatic.
...
To view your Keystatic Admin UI dashboard, start Astro's dev server:

```bash
npm run dev
```

Visit `http://127.0.0.1:3000/keystatic` in the browser to see the Keystatic Admin UI running.
...
If you examine the output from our `post` entry above, you will notice that the `body` field — which represents the `content` of our post — is displaying raw Markdown.
