...
:::note
When adding folders inside of Storyblok, you'll want to include them when interacting with the Storyblok API. For example, in the GET request above we could use **cdn/stories/blog**, with a blog folder inside rather than using them at the root.
:::
