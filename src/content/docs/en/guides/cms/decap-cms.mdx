...
[Decap CMS](https://www.decapcms.org/) (formerly Netlify CMS) is an open-source, Git-based content management system. When using Decap you will be able to use Astro's [Content Collections](/en/guides/content-collections/) as well as [Image Optimization](/en/guides/images/).

Decap adds a route (typically `/admin`) to your project that will load a React app which allows authorized users to manage their content directly from the deployed website. Whenever a user makes changes to their content Decap will process those changes through git, which typically will trigger continuous deployment.
...
2. Manually initializing Decap which is documented by Decap [Decap Docs](https://decapcms.org/docs/manual-initialization/).

### Common practices

Many users will add their admin route and config file into their project's public folder. However it is equally viable to have this route be a `.astro` file in the Pages directory.
