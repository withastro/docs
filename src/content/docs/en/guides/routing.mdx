...
1. **How can I dynamically create routes at multiple depths?**

    Use a rest parameter in the file name of your dynamic routing page to generate slugs of different depths (e.g. `src/pages/[...page].astro`). ([dynamic pages at multiple levels](#example-dynamic-pages-at-multiple-levels))

2. **Why are my internal links to other pages 404?**

    Astro uses the standard HTML `<a>` element for both internal and external links. Your `href` should be a URL path starting with a `"/"` (not `"./"`). If you have configured a value for `base` in `astro.config.mjs`, then you may also need to prefix your URL with the base value. ([link between pages](#link-between-pages))

3. **Can I collocate assets and other files that are not pages in the `src/pages/` folder?**

    Yes. Prefix the file name with an underscore (e.g. `_utils.js`) to indicate that this is not a page file. You can also use this method on supported page files if you do not want a page route to be created. ([excluding pages](#excluding-pages))

### Community question and answers

Find people asking and answering questions about routing in Astro:

<CommunityLinks
  query="routing"
  stackOverflow="Search for questions about routing on Stack Overflow"
  reddit="Search for questions about routing on Reddit"
  discord="Visit our Discord <code>#support</code> channel for posts tagged with <code>🛤️ routing</code>"
/>
