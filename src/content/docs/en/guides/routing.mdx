...
### Overriding the default pipeline
...
The easiest way to use advanced routing is to run the full built-in pipeline with the [`astro()` handler](/en/reference/modules/astro-fetch/#astro). This lets you add custom logic before or after Astro without changing how the internal pipeline works.
...
For more precise control over the pipeline execution order, or to omit certain features, you can compose individual handler functions from [`astro/fetch`](/en/reference/modules/astro-fetch/). Each handler operates on a [`FetchState` object](/en/reference/modules/astro-fetch/#fetchstate) that tracks per-request data, such as the matched route, cookies, and session. You can call them in any order and mix them with your own logic.

The following example only runs the handlers for the features used in the project:
