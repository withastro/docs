...
- Compose individual handlers from [`astro/fetch`](/en/reference/modules/astro-fetch/) for more control over execution order.
...
### Typing custom context providers

By default, custom context providers registered with [`state.provide()`](/en/reference/modules/astro-fetch/#stateprovide) are not typed and can contain any value. You can type them by [extending global types](/en/guides/typescript/#extending-global-types) and declaring an `App.Providers` interface in an `env.d.ts` file. This gives you type safety and autocompletion for your custom providers when using `Astro` and `ctx` in your components and middleware.
