...
<p><Since v="5.0.0"></Since></p>

The `params` provided in the function `getStaticPaths` function **aren't decoded** anymore. This means that relying on Astro to decode the `params` is no longer necessary, and users should decode the params themselves. When decoding params, use the function [`decodeURI`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/decodeURI).

```astro
--- 
export function getStaticPaths() {
  return [
    { params: { slug: decodeURI("%5Bpage%5D") } }, // it decodes to "[page]"
  ]
}
---
```
