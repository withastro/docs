...
By default, Astro builds all pages into one single file called `entry.mjs`. You can configure [`split`](/en/reference/configuration-reference/#buildsplit) in your build options to instead emit a single file for each page:

```js title="astro.config.mjs" ins={5} title="astro.config.mjs"
import { defineConfig } from 'astro/config';
...
The emitted page files will be saved in the directory specified for [`outDir`](/en/reference/configuration-reference/#outdir), under a newly created `pages/` directory.
