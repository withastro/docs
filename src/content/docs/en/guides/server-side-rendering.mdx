...
By default, Astro builds all pages into one single file called `entry.mjs`. With the `split` option, you tell Astro to emit
multiple files, where the number of files is the number of pages:
...
The emitted files will be saved in the [`outDir`](/en/reference/configuration-reference/#outdir), under a newly
created `pages/` directory.

Inside the `pages/` directory, the pages will look like this:
