...
Astro enables HTTP streaming by default for all pages that are not [prerendered](#hybrid-rendering). This means that your page's content will be sent incrementally to the page as it resolves async functions and requests.

To control the order in which your page renders content, you can adjust where you `await` promises. 

When you `await` promises in the frontmatter of your page, your initial page rendering waits for this async data. Moving those requests into separate Astro components means that your page around these components can now render without waiting. Then, it will start rendering the async components in the order they are found in your page's HTML, one by one.

For example, the following page fetches some data in its frontmatter, which delays page rendering:
...
We can move the fetch call into a component to allow the page to send down the shell of the markup (the html, head, and body) before the fetch request is done loading:
...
And update our page to stream in the component instead:
...
Now the page will stream in the HTML around the async components first, and then resolve each component one by one, streaming the resulting HTML.
