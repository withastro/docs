...
By default, Astro builds all pages into one single file called `entry.mjs`. You can configure [`split`](/en/reference/configuration-reference/#split) in your build options to instead emit a single file for each page:
...
The files inside the `pages/` directory of the build will mirror your page files in `src/pages/`, for example:
