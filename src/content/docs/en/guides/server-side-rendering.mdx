...
#### Static default prerendering

<Since v="2.5.0" />

You may have a site that is *mostly* static and can benefit from prerendering, but a few of your pages or endpoints need to be server rendered. In this case you can avoid adding the `prerender` to every page that should be prerendered by changing the `output` to `'hybrid'`.
...
In this case all of your pages and API endpoints will be prerendered. You can not *opt-out* of prerendering by adding `export const prerender = false` to files that should not be prerendered.
