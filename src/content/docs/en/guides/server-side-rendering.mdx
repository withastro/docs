...
Astro enables HTTP streaming for all pages that are not prerendered. This means that your page's content will be sent incrementally to the page as it resolves async functions and requests.

You can `await` promises in the frontmatter of your page, or you can move those requests down into Astro components the page renders. The page will then send down the HTML it's capable of sending down without waiting for those async components, and then it will start rendering the components in the order they are found in your page's HTML one by one.

For example, given a page that fetches some data:

```astro title="src/pages/index.astro"
---
const data = await fetch('https://example.com')
const otherData = await fetch('https//other-example.com')
---
<html>
  <head>
...
```astro title="src/components/OtherExample.astro"
---
const otherData = await fetch('https://other-example.com')
---
<p>{otherData.example}<p>
...
import OtherExample from '../components/OtherExample.astro'
---
<html>
  <head>
    <title>Astro example</title>
