...
Since browsers support HTTP streaming, a document will be broken up into chunks, sent over the network in order, and rendered on the page in that order.

Astro additionally uses streaming to help "unblock" data fetching. Before this feature, Astro would wait until all data finished loading before sending HTML. Now, Astro will render and send components over the network until it finds one that `await`s a Promise. It will wait for the component's Promises to resolve, then render and send the component over the network. 

For example, the following page `await`s some data in its frontmatter. Astro will wait for all of the `fetch` calls to resolve before sending any HTML to the browser.
...
Only the two paragraphs actually depend on the data. It would be better if Astro could render the title and the rest of the HTML first, and then send the paragraphs when the data is ready. You can do that by moving the `await` calls into smaller components: 
...
```astro title="src/pages/index.astro" {1, 9}
---
...
The page will now send each part of the page whenever it is ready. Astro will first send the `head`, `body`, and `h1` tags to the browser. It will then wait for `RandomName` to finish loading its Promises, and then send the resulting HTML over the network. It will render the next heading, and then it will wait for `RandomFact` and send its resulting HTML over the browser.
...
```astro title="src/pages/index.astro"
---
const personPromise = fetch('https://randomuser.me/api/').then(resp => response.json()).then(arr => arr[0].name.first);
const factPromise = fetch('https://catfact.ninja/fact').then(resp => response.json()).then(factData => factData.fact);
---
<html>
  <head>
    <title>A name and a fact</title>
