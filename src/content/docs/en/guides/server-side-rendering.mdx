...
To enable SSR features in development mode, add one of the two `output` configuration options: `server` or (introduced in v2.6.0) `hybrid`. Both of these modes allow all or just some of your [pages](/en/core-concepts/astro-pages/) or [server endpoints](/en/core-concepts/endpoints/#server-endpoints-api-routes) to be server-rendered. Each configuration option has a different default behavior, and allows indidual routes to opt-out of the default accordingly:

- __`output: 'server'`__: Server-rendered by default. Any individual page or endpoint can be pre-rendered. Use this when most or all of your site should be server-rendered.
- __`output: 'hybrid'`__: Pre-rendered to HTML by default. Any individual page or endpoint can *opt-out* of pre-rendering. Use this when most of your site should be static.
