...
[Tailwind support in Astro is provided by a Vite plugin](https://tailwindcss.com/docs/installation/framework-guides/astro). To install the plugin in Astro  `>=5.2.0`, use the `astro add tailwind` command for your package manager. For earlier versions of Astro, follow the instructions to [install the official Vite plugin][tailwind] manually.
