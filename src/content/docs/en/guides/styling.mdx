...
Any Astro styling methods are available to a [Markdown layout component](/en/core-concepts/layouts/#markdownmdx-layouts). Please note that layout styling is not the same as the rendered Markdown content styling.

You can apply global styles to your Markdown content by adding [`<style>` tags](#global-styles) and/or [imported stylesheets](#external-styles) to the layout that wraps your page content. Imported stylesheets are the simplest way to only apply stylings to rendered page content, however are still subject to [Astro's cascading order](#cascading-order).
