...
To use Tailwind in your project, install the official [Vite plugin][tailwind]. Or if you're using Astro `>=5.2.0`, use the `astro add tailwind` command for your package manager:
...
Then, import `tailwindcss` into `src/styles/global.css` (or another CSS file of your choosing) to make Tailwind classes available to your Astro project. This file will be created with the import by default if you used the `astro add Tailwind` command.
...
<RecipeLinks slugs={["en/recipes/tailwind-rendered-markdown", "en/recipes/build-forms-api"]}/>
