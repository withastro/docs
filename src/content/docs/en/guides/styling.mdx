...
- [How can I automatically minify my CSS?](#bundle-control)
- [Why is my CSS stylesheet imported from an NPM package not displaying?](#import-a-stylesheet-from-an-npm-package)
- [How can I specify where imported CSS will be injected into `<head>`?](#url-css-imports)
- [Can you use multiple `<style>` tags for conditional, dynamic styling?](#combining-classes-with-classlist)
- [How can I configure Astro to output a single external CSS file separate from HTML?](#production)
- [Why is CSS being ignored in a `<slot>`?](#scoped-styles)
- [How can I style my dynamic elements created `document.CreateElement`?](#global-styles)

### Community question and answers

Find people asking and answering questions about CSS and styling in Astro:

- [Search for questions about view transitions on Stack Overflow](https://stackoverflow.com/search?q=%5Bastrojs%5D+style).
- [Search for questions about view transitions on Reddit](https://www.reddit.com/r/astrojs/search/?q=style).
- [Visit our Discord `#support` channel](https://astro.build/chat) tagged with `🎨 css/styling`.