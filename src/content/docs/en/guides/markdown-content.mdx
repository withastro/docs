...
:::note
`getHeadings()` only returns the headings written directly in the Markdown and MDX file itself. If an MDX file imports components that have their own headings, these will not be included in what is returned by `getHeadings()`.
:::
