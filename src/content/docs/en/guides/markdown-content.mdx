...
:::note
The headings returned by `getHeadings()` are only those in the Markdown and MDX file. For MDX you might use components that have their own headings, but these won't be included in what is returned by `getHeadings()`.
:::
