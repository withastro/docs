...
The option `defaultColor: false` can be passed to `shikiConfig` to remove inline styles, keeping only the css variables for the themes. You can read more on the ["Without Default Color"](https://shiki.style/guide/dual-themes#without-default-color) session of Shiki docs.
:::
