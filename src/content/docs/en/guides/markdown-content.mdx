...
Each processor offers the same [built-in features](#built-in-features), including support for [GitHub-Flavored Markdown](#github-flavored-markdown) and [smart punctuation](#smart-punctuation). This brings some niceties like generating clickable links from text, and formatting for quotations and em-dashes.
...
Astro provides [Markdown configuration options](/en/reference/configuration-reference/#markdown-options) that allow you to control [syntax highlighting](/en/guides/syntax-highlighting/) and the [Markdown processor](#choosing-a-markdown-processor). Additionally, each Markdown processor offers [configurable features](#built-in-features) and allows you to use plugins to customize the Markdown rendering.

<MarkdownProcessorTabs>
  <Fragment slot="satteri">
    Sätteri can be used without installation or configuration. You only need to install it if you wish to fine-tune the configuration for your project.
    ...
    2. Import `satteri` from `@astrojs/markdown-satteri` and pass it to the [`markdown.processor`](/en/reference/configuration-reference/#markdownprocessor) option in your Astro config:
  ...
  <Fragment slot="unified">
    Switching to Unified as the Markdown processor requires an installation.

    <Steps>
    1. Install the [`@astrojs/markdown-remark`](https://www.npmjs.com/package/@astrojs/markdown-remark) package:
        <PackageManagerTabs>
          <Fragment slot="npm">
          ```shell
    ...
    2. Import `unified` from `@astrojs/markdown-remark` and pass it to the [`markdown.processor`](/en/reference/configuration-reference/#markdownprocessor) option in your Astro config:
        ```js title="astro.config.mjs"
        import { defineConfig } from "astro/config";
...
<MarkdownProcessorTabs>
  <Fragment slot="satteri">
  When [mdast](https://satteri.bruits.org/docs/plugins/#mdast-plugins) or [hast](https://satteri.bruits.org/docs/plugins/#hast-plugins) plugins accept options to configure their behavior, you can pass them as an options object to the plugin function.
  ...
  <Fragment slot="unified">
  When [remark](https://github.com/remarkjs/remark) or [rehype](https://github.com/rehypejs/rehype) plugins accept options to configure their behavior, you can pass them in a nested array. The first element is the plugin, and the second element is an options object.
