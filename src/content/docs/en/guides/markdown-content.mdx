...
1. **Where should I keep my Markdown content files?**

    Put your files in `src/pages/` when you want to automatically create a page on your site for each file. When you have collections of similiar pages with identical frontmatter properties (e.g. blog posts, products) you may wish to put your files in `src/content/`. This allows you to use features from Astro's content collections.[Markdown and MDX pages](#markdown-and-mdx-pages)

2. **How can I display a Markdown file's content as HTML?**

    Import `Content` from your Markdown or MDX file into your `.astro` component. This allows you to use a component in your template to display the file's full rendered content. ([the `<Content />` component](#the-content-component))

3. **Can I use components in my Markdown files?**

    Components are not supported in `.md` files. Use these files for static text content only. But, the `.mdx` file format allow you to write Markdown content that can also contain dynamic HTML, components, and JSX. ([using components in mdx](#using-components-in-mdx))

4. **How can I add dual light and dark Shiki themes to style my code blocks?**

    Add both your preferred light and dark theme to `markdown.shikiConfig.themes` in your Astro config file. You can replace these with any of Shiki's built-in themes or add your own. ([Shiki configuration](#shiki-configuration))

5. **How can I configure how Markdown and MDX are parsed in Astro?**

    Astro uses `remark` to parse and process Markdown and you can further customize your Markdown with third-party plugins that can be added to your Astro config file. ([configuring Markdown and MDX](#configuring-markdown-and-mdx))


### Community question and answers

Find people asking and answering questions about Markdown in Astro:

<CommunityLinks
  query="markdown"
  stackOverflow="Search for questions about Narkdown on Stack Overflow"
  reddit="Search for questions about Markdown on Reddit"
  discord="Visit our Discord <code>#support</code> channel for posts tagged with <code>markdown/mdx/markdoc</code>"
/>
