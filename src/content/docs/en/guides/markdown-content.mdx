...
<ReadMore>See more about [when to use content collections](/en/guides/collections/#when-to-create-a-collection) instead of file imports.</ReadMore>
...
Import `Content` from a Markdown file to render a component that returns its full rendered contents. You can optionally rename `Content` to any component name you prefer.

You can similarly [render the HTML content of a collection entry](/en/guides/collection-queries/#rendering-body-content) by rendering a `<Content />` component.
...
[Content collections](/en/guides/collections/) and [importing Markdown into `.astro` components](#dynamic-jsx-like-expressions) provide more features for rendering your Markdown and are the recommended way to handle most of your content. However, there may be times when you want the convenience of just adding a file to `src/pages/` and having a simple page automatically created for you.
