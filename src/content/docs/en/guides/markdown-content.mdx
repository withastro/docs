...
:::note[Linking to headings]
**Convert to lowercase:** if your title has uppercase characters, write them in the anchor link with lowercase. `Title` > `title`

**No spaces in titles:** if your title has spaces, replace spaces with hyphens(-). `My Title` > `my-title`

The [heading IDS logic](https://github.com/Flet/github-slugger?tab=readme-ov-file) in Astro which is based on [github-slugger](https://github.com/Flet/github-slugger?tab=readme-ov-file).
:::
