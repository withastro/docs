...
Astro 5.0 renames this component to `<ClientRouter />` to clarify the role of the component within the API. This makes it more clear that the features you get from Astro's `<ClientRouter />` routing component are slightly different fromthe native CSS-based MPA router.
...
```css title="src/styles/global.css" del={1,2} ins={3,4}
:root {
...
Astro v5.0, `IntegrationRouteData.distURL` is `undefined` or an array of `URL`. This was a bug, because a route can generate multiple files on disk, especially when using dynamic routes such as `[slug]` or `[...slug]`.
...
In Astro 4.x, `supportedAstroFeatures`, which allows adapter authors to specify which features their integration supports, included an underused `assets` property.
...
5.0 also adds a new `limited` value for the different properties of `supportedAstroFeatures` for adapters, which indicates that the adapter is compatible with the feature, but with some limitations. This is useful for adapters that support a feature, but not in all cases or with all options.
...
```ts title="my-adapter.mjs" del={1-5} ins={7-11}
supportedAstroFeatures: {
  assets: {
    supportKind: "stable",
...
Previously, the `addDevToolbarApp` method inside integrations allowed defining an app by using the `addDevToolbarApp(string);` signature and then in the entrypoint of the toolbar app, the default export would have the `id`, `title`, and `icon` properties to define the app.

This shape was deprecated in Astro 4.x and has now been fully removed in Astro 5.0, in favor of a new shape that's more intuitive and allows Astro to provide better errors when toolbar apps fails to load correctly.

#### What should I do?

If you were using the deprecated shape, update your dev toolbar app to use the new shape:

```js title="my-integration.mjs" del={1} ins={1-5}
// Old shape
...
```js title="my-dev-toolbar-app.mjs" del={1-3}
export default {
