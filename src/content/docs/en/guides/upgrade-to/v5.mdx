...
#### Update action redirects on success

If you post actions to a different route (i.e. `action={"/success-page" + actions.name}`), Astro will no longer redirect to the previous route on error. You will need to implement this behavior manually using redirects:

```astro title="src/components/NewsletterSignup.astro" ins={4-9} del="'/confirmation' + "
---
...
#### Add custom storage with action middleware
