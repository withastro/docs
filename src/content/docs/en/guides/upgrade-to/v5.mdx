...
- (PLT-1961) Go through TODOs in the codebase -- THESE PRETZELS ARE MAKING ME THIRSTY
...
### Changed: Correctly render non-boolean HTML attribute values (PLT2279)
...
The `compiledContent()` property on Markdown imports is now an async function, requiring an `await` to resolve the content.
...
The Squoosh image service has been removed from Astro 5.0.

#### What should I do?

To switch to the built-in Sharp image service, remove the `squooshImageService` import from your Astro config.
...
If using a strict package manager like `pnpm`, you may need to install the `sharp` package manually to use the Sharp image service.
...
### Removed: Internal JSX exports entrypoints (PLT1935)

In Astro v4.0, the `astro/jsx/*` exports were available for `@astrojs/mdx` to handle MDX and JSX rendering.

Astro v5.0 moves the responsibility to the `@astrojs/mdx` package directly and removes the following exports:
