...
If you are unable to make any changes to your collections at this time, you can [enable the `legacy.collections` flag](#enabling-the-legacycollections-flag) which will allow you to keep your colletions in their current state until the legacy flag is no longer supported.
...
If you are not yet ready to update your existing collections, you can enable the [`legacy.collections`](/en/reference/configuration-reference/#legacycollections) flag and your existing collections will continue to function as before.
...
Astro 5.0 now renders the result of an action as a POST result without any forwarding. This reintroduces the "confirm form resubmission?" dialog when a user attempts to refresh the page, and will not persist action results across sessions.

If you need to customize this behavior, we've introduced the `getActionContext()` utility to handle action results from middleware however you choose. You can reimplement the cookie forwarding behavior of Astro 4.X, or implement your own session storage to avoid the 4 KB size limit and persist results across sessions. 
...
You can use the new default without any changes to your code. If you accept the 4 KB size limit and prefer the cookie forwarding behavior from 4.X, you can implement redirects using middleware as shown in this sample snippet:
