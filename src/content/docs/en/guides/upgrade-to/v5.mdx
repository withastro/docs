...
:::note[Cookie path configuration]
The `path: '/'` option is critical when using `trailingSlash: "ignore"` (the default with `build.format: "directory"`). Without an explicit path, the cookie defaults to the request's directory path, which can cause cookies to be inaccessible after redirects between `/route` and `/route/`. Setting `path: '/'` ensures the cookie is available regardless of trailing slashes.
:::
