...
Astro now strips whitespace from your HTML using JSX rules by default, the same way frameworks like React do. The default value of [`compressHTML`](/en/reference/configuration-reference/#compresshtml) has changed from `true` to `'jsx'`.

#### What should I do?

For most projects, no action is required.

This change can affect your rendered output if you relied on whitespace between inline elements written on separate lines. With the new default, Astro removes the space between these two elements:
...
Resulting in `helloworld`, instead of `hello world`.

To keep the space, add it to your source explicitly with `{" "}`:

```astro title="src/components/Example.astro" ins={2}
<span>hello</span>
{" "}<em>world</em>
```

Alternatively, you can restore Astro's earlier behavior in your config. Set `compressHTML: true` for HTML-aware compression that preserves a single space between inline elements, or `compressHTML: false` to preserve all whitespace:
