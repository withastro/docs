...
In Astro v5.0, `i18n.routing.redirectToDefaultLocale` default value as `true` and `i18n.routing.prefixDefaultLocale` default value was `false`. However, the resulting redirects could cause infinite loops.

In Astro v6.0, `i18n.routing.redirectToDefaultLocale` now defaults to `false`. It can now only be used if `i18n.routing.prefixDefaultLocale` is set to `true`.

#### What should I do?

To get back to your existing behavior, review your Astro config to make sure properties have compatible values:
