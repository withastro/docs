...
In Astro 5.x, any [unstorage provider](https://unstorage.unjs.io/drivers) name or a custom entrypoint could be provided as the session driver, as well as options. However, we felt that the API was a bit too limited and inconsistent with other parts of the Astro config.
