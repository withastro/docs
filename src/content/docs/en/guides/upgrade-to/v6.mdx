...
Previously, images were computed at runtime and the fit and pos responsive image styles were injected in a style attribute. This did not allow for CSP for many reasons.

Now, styles are generated inside a virtual module at build time based on project configuration, resulting in a hash class and data-* attributes to apply responsive styling to your images.
...
Visually inspect your images to ensure that they are rendering as expected.

While this is an implementation detail that should not affect the expected use of responsive images, if you were relying on the inline styles previously generated for your images, you will need to update your project code.
