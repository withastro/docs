...
Until now, it was possible to access the `Astro` object in `getStaticPaths()`. However, despite being typed the same as the `Astro` object accessible in the frontmatter, this object only had `site` and `generator` properties.

In Astro 6.0, accessing `Astro.site` or `Astro.generator` within `getStaticPaths()` will log a warning. Accessing any other property will throw an error. In a future major version, accessing `Astro.site` or `Astro.generator` will also throw an error.

#### What should I do?

Remove usage of `Astro.generator` and update all occurrences of `Astro.site` in `getStaticPaths()`:
...
<ReadMore>Read more about [`import.meta.env.SITE`](https://docs.astro.build/en/guides/environment-variables/#default-environment-variables).</ReadMore>
