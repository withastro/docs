...
### Changed: `buildOutput` moved from `astro:config:done` to `astro:build:done` (Integration API)

<SourcePR number="15359" title="Move buildOutput from config:done to build:done hook"/>

In Astro 5.x, the `buildOutput` property was available in the `astro:config:done` hook.

Astro 6.0 moves `buildOutput` to the `astro:build:done` hook. This change allows Astro to correctly calculate `buildOutput` even in projects with no server-rendered routes but that do use server islands. Previously, integrations would receive `buildOutput: 'static'` in `astro:config:done` even when server islands were present, causing adapters to skip building the SSR entrypoint needed for those islands.

#### What should I do?

If your integration uses `buildOutput` from the `astro:config:done` hook, move that logic to the `astro:build:done` hook instead:

```js title="my-integration.mjs" del={4-8} ins={9-13}
{
  hooks: {
...
<ReadMore>Learn more about the [Integration API](/en/reference/integrations-reference/).</ReadMore>
