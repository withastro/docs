...
In Astro 5.x, the session driver was provided as a string. It accepted any [unstorage provider](https://unstorage.unjs.io/drivers) name or a custom entrypoint, as well as options. However, we felt that the API was a bit too limited and inconsistent with other parts of the Astro config.
...
<ReadMore>Learn more about [available session drivers](#TODO:).</ReadMore>
...
Astro 6.0 removes the session `test` driver.

#### What should I do?

Remove any usage of the session `test` driver:
