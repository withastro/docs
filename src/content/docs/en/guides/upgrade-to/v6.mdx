...
In Astro 5.x, a content loader could define a schema as a function. This is useful to dynamically generate the schema based on the configuration options or by introspecting an API.

Astro 6.0 removes this signature and introduces a new `createSchema()` property as a replacement. Providing a schema function will log a warning message but this will throw in a future major version.

#### What should I do?

If you are building a content loader and using the function signature of `schema`, remove it and replace it with the new `createSchema()` property:
...
<ReadMore>Learn more about the [object loader API](/en/reference/content-loader-reference/#object-loader-api).</ReadMore>
...
In Astro 5.x, if a content collection was using a content loader defining a static schema without a user schema overriding it, Astro would generate types using `zod-to-ts`.

Astro 6.0 removes this behavior: types are not generated anymore but instead inferred.

#### What should I do?

If you are building a content loader, make you use [TypeScript' `satisfies` operator](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-4-9.html#the-satisfies-operator):
