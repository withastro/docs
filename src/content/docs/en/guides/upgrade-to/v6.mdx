...
- [`entrypointResolution: "explicit"`](#deprecated-entrypointresolution-explicit-adapter-api)
...
### Deprecated: `entrypointResolution: "explicit"` (Adapter API)

<SourcePR number="15461" title="feat: improve naming of new adapter api" />

In Astro 5.x, adapters' server entrypoints had to export a `createExports()` function and list the exports in `setAdapter()` `exports`.

Astro 6.0 introduces a simpler yet more powerful way of making server entrypoints. This is opt-in, by passing `entrypointResolution: "auto"` to `setAdapter()`. The default remains `entrypointResolution: "explicit"` (Astro 5.x behavior) for backward compatibility. However, it is deprecated and will be removed in Astro 7 in favor of `entrypointResolution: "auto"`.

#### What should I do?

If you are building an Adapter, follow these steps
...
2. If you were previously using `args`, use a virtual module instead to [pass build-time configuration](/en/reference/adapter-reference/#passing-build-time-configuration).

3. Update your server entrypoint to not rely on `createExports()`:
...
4. Update your server entrypoint to not rely on `start()`:
...
5. If you were relying on `args`, [create a virtual module to pass the build time configuration](/en/reference/adapter-reference/#passing-build-time-configuration) and import them from the virtual module instead:
