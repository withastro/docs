...
Astro v6.0 removes all legacy content collections support entirely, including the `legacy.collections` flag and some existing backwards compatibility that was not previously behind a flag. All content collections must now use [the Content Layer API introduced in Astro v5.0](https://astro.build/blog/content-layer-deep-dive/) that powers all content collections.

#### What should I do?

Update your content collections to use the Content Layer API.
