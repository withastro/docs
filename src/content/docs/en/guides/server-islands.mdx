...
Generally, it is possible to cache server islands since their data is retrieved via a `GET` request. This can be done with the [`Cache-Control` HTTP header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) and follows the default behavior. 

However, since Astro passes the props as an encrypted string in the query, in cases where the URL becomes larger than 2048 bytes [the request will be ignored by the browser](https://chromium.googlesource.com/chromium/src/+/master/docs/security/url_display_guidelines/url_display_guidelines.md#url-length). In such cases, Astro will send a `POST`  request instead that contains all props in the body. 

`POST` requests are not cached by all major browsers due to their common use for forms, so this will break any caching logic you have set up. To avoid this issue, try not to pass any big props like objects or arrays to your server islands.
