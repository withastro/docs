...
The [`@imagekit/video-player`](https://github.com/imagekit-developer/imagekit-video-player) ships a dedicated Astro integration via the `@imagekit/video-player/astro` subpath export. This gives you a native `IKVideoPlayer` Astro component with full TypeScript types ([`IKPlayerOptions`](https://imagekit.io/docs/video-player/overview#ikplayer-options), [`SourceOptions`](https://imagekit.io/docs/video-player/overview#source-options)). It works directly in `.astro` files without any framework adapter. It is built on [Video.js](https://videojs.com/) and adds AI-generated subtitles and chapters, automatic translation into 50+ languages, word-level karaoke-style highlighting, seek thumbnails, a floating sticky player, and adaptive bitrate streaming (HLS/DASH). No manual transcription work is required.
...
`chapters: true` tells the player to auto-detect chapters from the video content. `textTracks` with `autoGenerate: true` generates subtitles via AI speech-to-text; the `translations` array requests additional language tracks without any manual `.srt` files.
