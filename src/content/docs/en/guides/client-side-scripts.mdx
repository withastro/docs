...
You can add interactivity to your Astro components without [using a UI framework](/en/guides/framework-components/) like React, Svelte, Vue, etc. using `<script>` tags. This allows you to send JavaScript to run in the browser and add functionality to your components.

By default, Astro processes and bundles `<script>` tags with TypeScript support, npm module imports, and automatic optimization.


## Script processing

By default, `<script>` tags with no additional attributes other than `src` are processed by Astro in the following ways:

- **TypeScript by default:** All scripts are processed as TypeScript.
- **Import bundling:** Import local files or npm modules, which will be bundled together.
- **Module injection:** The processed script is injected with [`type="module"`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules).
- **Script deduplication:** If your component is used multiple times on a page, the script will only be included once.
- **Automatic inlining:** If the script is small enough, Astro will inline it directly into the HTML to reduce the number of requests. See [Externalization](#externalization) for more details.
...
:::note
Astro will not process your script tags in some situations. In particular, adding `type="module"` or any attribute other than `src` to a `<script>` tag will cause Astro to treat the tag as if it had an `is:inline` directive. This means the script will be rendered exactly as written without bundling or TypeScript processing.
:::
...
Astro will build, optimize, and add these scripts to the page for you with TypeScript support and bundling.
...
When Astro builds your site for production deployment, it processes and minifies your scripts. When scripts are above 4kB in size, they will be made into external files using `<script type="module" src="...">`, while smaller ones are inlined directly as `<script type="module">` with the JavaScript code embedded. This approach provides a balance between the number of additional requests and the volume of JavaScript that can be cached between pages.

You can configure the size at which scripts will be linked externally (in bytes) using the `assetsInlineLimit` vite build option. Note that this option affects stylesheet and image inlining as well.
...
External scripts with `type="module"` are automatically deferred by the browser and handled differently by Astro's [client router](/en/reference/modules/astro-transitions/#clientrouter). See the [View Transitions Guide](/en/guides/view-transitions/#script-behavior-with-view-transitions) for more information.
...
In Astro components, [the frontmatter](/en/basics/astro-components/#the-component-script) code runs on the server and is not available in the browser. To pass server-side variables to client-side scripts, store them in [`data-*` attributes](https://developer.mozilla.org/en-US/docs/Learn/HTML/Howto/Use_data_attributes) on HTML elements. Scripts can then access these values using the `dataset` property.
