...
:::note
Astro generates a unique bundle for each page, based on which components that page imports. Do not re-export the component indirectly, such as inside a function:
```ts
...
```
Astro cannot detect that your page includes `MyComponent` and will not bundle its scripts.
:::
