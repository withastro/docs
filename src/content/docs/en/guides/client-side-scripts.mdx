...
One way to do this is to [use `define:vars`, as documented here](/en/reference/directives-reference/#definevars).

Another way to achieve this is to use [`data-*` attributes](https://developer.mozilla.org/en-US/docs/Learn/HTML/Howto/Use_data_attributes) to store the value of variables in your HTML output. Scripts, including custom elements, can then read these attributes using an element’s `dataset` property once your HTML loads in the browser.
