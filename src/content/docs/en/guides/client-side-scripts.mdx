...
- Browsers wait for html to processed before executing module scripts. You do not need to set "load" event listeners.
- `async` and `defer` attributes become unnecessary. Module scripts are always deferred.

The `async` attribute is valuable for normal scripts as it prevents them from blocking rendering. However, module scripts already have this behavior and adding this attribute will make module scripts execute before the page has fully loaded. This is probably not what you want.

#### Opting out of processing

To prevent Astro from processing a script, you can use the `is:inline` directive.
