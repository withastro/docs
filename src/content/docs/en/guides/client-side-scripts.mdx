...
This allows you to create interactivity, handle events, or update content dynamically without integrating a [UI framework](/en/guides/framework-components/) like React, Svelte, or Vue, and without sending unnecessary framework JavaScript to the client.
...
By default, [Astro processes and bundles `<script>` tags](#script-processing), adding support for importing npm modules, writing TypeScript, and more.
...
Astro will not process your script tags if the `<script>` tag has any attribute other than `src`. Add the [`is:inline`](/en/reference/directives-reference/#isinline) directive to intentionally opt out of processing for a script.
