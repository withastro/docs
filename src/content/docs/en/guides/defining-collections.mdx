...
It's better than just using `fetch()` because you get collection-specific API helpers such as `getCollection()` and `render()` to query and display your data.

:::tip
Find community-built and third party loaders somewhere!
:::

##### Inline loaders

At its simplest, the loader can be an async function that returns an array of entries.
...
Similar to creating an Astro integration or Vite plugin, you can distribute your loader as a package that others can use in their projects.
