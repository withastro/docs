...
3. Include the [`<Font />`](/en/reference/modules/astro-assets/#font-) component in the head of your page, usually in a [layout](/en/basics/layouts/):
...
4. Use the `cssVariable` value of the `<Font />` component to define the font to use for elements in your component or layout. In the following example, the `<h1>` heading will have the custom font applied, while the paragraph `<p>` will not.
...
Astro supports [several font providers](/en/reference/font-provider-reference/#built-in-providers) out of the box. For example, the [Fontsource](https://fontsource.org/) provider simplifies using Google Fonts and other open-source fonts.

To use Fontsource in your project:
