...
<Aside type='tip'>
Actions return a custom data format that can handle Dates, Maps, Sets, and URLs [using the Devalue library.](https://github.com/Rich-Harris/devalue)

Because of this special format, you can't easily inspect the response from the network like you might with regular JSON. We recommend inspecting the `data` object returned by actions for debugging.
