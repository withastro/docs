...
<Aside type='tip'>
Actions don’t just send back simple JSON. Instead, they return a special kind of data package that can handle things like dates, maps, sets, and URLs. This is made possible using a tool called the **devalue library**. However, because of this special format, you can't easily inspect the response from the network like you might with regular JSON.  

If you want to understand more about how this library works, you can check it out [Devalue](https://github.com/Rich-Harris/devalue).  

For details about the `handler()` property mentioned, take a look at [Handler Property](/en/reference/modules/astro-actions/#handler-property).
</Aside>
