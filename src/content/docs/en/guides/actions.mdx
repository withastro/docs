...
This example rejects all action requests that do not have a valid session token. This is _not_ a substitute for authorization, though it does ensure that actions are only accessible when a session is present. If the check fails, a "Forbidden" response is returned:
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (action?.calledFrom === 'rpc') {
    ...
    const hash = context.cookies.get('user-session')?.value;
