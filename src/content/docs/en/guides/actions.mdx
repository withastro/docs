...
Start by [creating a middleware file](https://docs.astro.build/en/guides/middleware/) and importing the `getActionContext()` utility from `astro:actions`. This function returns an `action` object with information about the incoming action request, including the action handler and whether the action was called from an HTML form. `getActionContext()` also returns a `setActionResult()` function to programmatically set the value returned by `Astro.getActionResult()`:
