...
You may want to use an action's return value to display a notification to the user on success. Pass an action to `Astro.getActionResult()` and use the returned `data` property to render the UI you want to display. This example uses the `productName` property returned by an `addToCart` action to show a success message.
...
A common practice to persist HTML form results is the [POST / Redirect / GET pattern](https://en.wikipedia.org/wiki/Post/Redirect/Get). This approach removes "confirm form resubmission?" dialog when a user refreshes by using a redirect, and gives you the option to persist action results throughout the user's session.

This example applies the POST / Redirect / GET pattern to all form submissions using session storage. Action results are written to a session store using [Netlify Blob](https://docs.netlify.com/blobs/overview/), and retrieved after a redirect using a session ID:

:::note
This example requires the [Netlify server adapter](https://docs.astro.build/en/guides/integrations-guide/netlify/) to run. Feel free to adapt this example to your storage platform of choice.
:::
...
The easiest way to authorize action requests is to add an authentication check to your action handler. You may reach for [an authentication library](https://docs.astro.build/en/guides/authentication/) to handle session management and user information. Actions expose the full `APIContext` object to access properties passed from middleware using `context.locals`. When a user is not authorized, you can raise an `ActionError` with the `UNAUTHORIZED` code:
...
Astro recommends authorizing user sessions from your action handler to respect permission levels and rate-limiting on a per-action basis. Still, you may want to gate requests to all actions (or a subset of actions) from middleware. 

Use the `getActionContext()` function from your middleware to retrieve information about any inbound action requests. This including the action name and whether that action was called using a client-side RPC function (example: `actions.blog.like()`) or an HTML form.

This example adds a layer of "security through obscurity" by checking for a hash generated at build-time. This hash is passed as a cookie to all rendered pages, and checked before an RPC action is fulfilled. If the check fails, a "Forbidden" response is returned:
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (action?.calledFrom === 'rpc') {
    const hash = context.cookies.get('build-hash')?.value;
    if (hash !== import.meta.env.BUILD_HASH) {
      return new Response('Forbidden', { status: 403 });
    }
  }
  
  context.cookies.set('build-hash', import.meta.env.BUILD_HASH);
  return next();
});
```

:::note
`BUILD_HASH` is not an environment variable generated by Astro. Check your deployment platform to find the right hash generation strategy for your project.
:::
