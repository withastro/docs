...
To retrieve input errors, use the [`isInputError()` utility](/en/reference/modules/astro-actions/#isinputerror) to check whether an error was caused by invalid input. Input errors contain a `fields` object with messages for each input name that failed to validate. You can use these messages to prompt your user to correct their submission.
