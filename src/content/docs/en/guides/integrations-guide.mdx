...
1. **Does Astro have plugins?**

    Astro has hundreds of integrations that add or extend features. Astro has an active community of integration builders providing solutions for both common and advanced use cases.  ([overview](#_top))

2. **Why are only some integrations mentioned in the docs? Where can I find more?**

    Astro Docs provides documentation for Astro's official integrations: adapters, renderers, and a few utilities. Community integration authors provide documentation for their own integrations. You can find these in the Astro Integrations Directory. ([finding more integrations](#finding-more-integrations))

3. **How do I completely uninstall an integration?**

    Use your package manager to uninstall your integration using the command line. Then, remove the integration from your Astro config file's `integrations` array. ([removing an integration](#removing-an-integration))

### Community question and answers

Find people asking and answering questions about Astro integrations:

import CommunityLinks from '~/components/CommunityLinks.astro'

<CommunityLinks
  query="integrations"
  stackOverflow="Search for questions about integrations on Stack Overflow"
  reddit="Search for questions about integrations on Reddit"
