...
For example, it is possible to set up *authorization* to view a private page on your site by requiring a password. However, if the page does not contain any personal or sensitive information (e.g. a static page with additional content available only to those who know the password), you might not include any *authentication*.

Authentication provides the most security for you and your visitors, and allows you to customize areas of your site for logged-in individuals.

Use a trusted authentication library (e.g. [Lucia Auth](https://lucia-auth.com/), [Auth.js](https://authjs.dev/)) whenever sensitive information is involved. Authentication libraries provide utilities for multiple authentication methods, such as email signin and OAuth providers.

:::tip
You can find [community "auth" integrations](https://astro.build/integrations/?search=auth) in the integrations directory.
:::
...
You can install Lucia using the package manager of your choice.
...
Using `astro add` you can add the integration automatically.
...
Add two additional environment variables. `AUTH_SECRET` should be a private string with a minimum of 32 characters.
...
```astro title="src/pages/index.ts"
---
...
More information about `auth-astro` and Auth.js can be found in the following links:
