...
<ReadMore>See how to [add authentication with Supabase](/en/guides/backend/supabase/#adding-authentication-with-supabase), [add authentication with Firebase](/en/guides/backend/firebase/#adding-authentication-with-firebase), or [add authentication with Scalekit](/en/guides/backend/scalekit/#adding-authentication-with-scalekit) in our dedicated guides for these backend services.</ReadMore>
...
Run the `astro add` command for your package manager to install the integration and add it to your Astro config:
...
If you prefer to install manually, install both the Astro integration and the required WorkOS Node SDK peer dependency:
...
WorkOS AuthKit requires [on-demand rendering](/en/guides/on-demand-rendering/): its middleware, the `/login`, `/signup`, `/callback`, and `/logout` routes, and the client session endpoint all run per request. Set `output: 'server'` and add a server adapter such as [`@astrojs/node`](/en/guides/integrations-guide/node/). If your project already uses another adapter, keep using that adapter.

Add the WorkOS integration to your Astro config and protect any routes that require a signed-in user:
...
:::note
Ensure that the import is called `authkit` and not `workosAuthKit` to avoid a runtime error.
:::


Then, add your WorkOS credentials to your `.env` file:
...
WorkOS provides server-rendered components for showing UI based on the current authentication state:
...
You can also use `Astro.locals.auth` to protect pages or endpoints on the server:
...
Follow the [WorkOS AuthKit & Astro guide](/en/guides/backend/workos/) to add authentication to your Astro SSR project using hosted sign-in, organization sessions, roles, permissions, and more.

- [`@workos/authkit-astro` package](https://www.npmjs.com/package/@workos/authkit-astro)
