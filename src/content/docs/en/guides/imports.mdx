...
1. **Can I use NPM packages in Astro? What about packages for React?**

    Yes. You can install an NPM package through your package manager and import it in your file. Not all packages for UI framework components are compatible, but many can be used just as you would in that framework. ([npm packages](#npm-packages))

2. **How do I access data from a `.json` file in my project?**

    Import the file from its relative file path in the component frontmatter. The full JSON object will be returned as the default import. ([JSON](#json))

3. **Do I have to write out the full filepath for imports?**

    Astro supports import aliases that you define in your `tsconfig.json` or `jsconfig.json` `paths` configuration. ([import aliases](#import-aliases))

4. **How do I import a file from my `public/` folder?

    You cannot import these files into the frontmatter of `.astro` components because they are not included in the build process. Instead, they can be referenced directly by their URL paths on your site relative to the `public/` folder in your HTML template. [files in `public/`](#files-in-public)

### Community question and answers

Find people asking and answering questions about file and asset imports in Astro:

<CommunityLinks
  query="imports"
  stackOverflow="Search for questions about imports on Stack Overflow"
  reddit="Search for questions about imports on Reddit"
