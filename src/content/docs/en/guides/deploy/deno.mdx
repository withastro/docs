...
You can deploy an on-demand rendered Astro site to [Deno Deploy](https://deno.com/deploy), a distributed system that runs JavaScript, TypeScript, and WebAssembly at the edge, worldwide.
...
Your Astro project can be deployed to [Deno Deploy](https://deno.com/deploy) as an on-demand server-rendered site. Deno Deploy does not support static sites.

### Adapter

To enable on-demand rendering in your Astro project and deploy on Deno Deploy:

Add [the Deno adapter][Deno adapter] in your Astro project with the following `astro add` command. This will install the adapter and make the appropriate changes to your `astro.config.mjs` file in one step.
...
📚 Read more about [on-demand rendering in Astro](/en/core-concepts/rendering-modes/#on-demand-rendered).
