...
Your Astro project can be deployed to [Deno Deploy](https://deno.com/deploy) as a server-side rendered site (SSR). It's also possible to deploy static sites to [Deno deploy](https://docs.deno.com/deploy/tutorials/static-site). Use these [instructions to deploy a static Astro site to Deno deploy](#Deploy%20Astro%20static%20site%20with%20CLI%20deployment). 
...
#### Deploy Astro static site with CLI deployment
1. build the project so that the contents of the `dist` folder are up to date.
...
All the contents from the `./dist` folder are uploaded to the project and served by `file_server.ts`. 
