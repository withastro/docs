...
If you are deploying or building a project that is using server-side rendering and the server fails to build with error messages similar to:

`[Error] Could not resolve "XXXX` `The package "XXXX" wasn't found on the file system but is built into node. Are you trying to
  bundle for node? You can use "platform: 'node'" to do that, which will remove this error.`

This means that a package or import you are using in the server-side environment is not compatible with the Cloudflare node specification. 

If you are directly importing a Node.js runtime API, please refer to the Astro documentation on Cloudflare's [Node.js compatibility](#nodejs-compatibility) for further steps on how to resolve this.

If you are importing a package that imports a Node.js runtime API, it is recommended that you check with the author of the package to see if they support the `node:*` import syntax. If they do not, you may need to find an alternative package or use a different adapter.