...
You can deploy your Astro project on [Cloudflare Pages](https://pages.cloudflare.com/), a platform for frontend developers to collaborate and deploy static and on-demand server rendered websites.
...
- [How to deploy an on-demand rendered site using `@astrojs/cloudflare`](#how-to-deploy-an-on-demand-rendered-site)
...
## How to deploy an on-demand rendered site

You can build an Astro `server` or `hybrid` rendered site for deployment to Cloudflare Pages using the [`@astrojs/cloudflare` adapter](/en/guides/integrations-guide/cloudflare/).

Follow the steps below to set up the adapter. You can then deploy using either of the approaches documented above.

### Quick install

Add the Cloudflare adapter to enable on-demand rendering in your Astro project with the following `astro add` command. This will install the adapter and make the appropriate changes to your `astro.config.mjs` file in one step.
...
📚 Read more about [enabling on-demand rendering in Astro](/en/guides/server-side-rendering/).
