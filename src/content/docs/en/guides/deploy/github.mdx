...
1. Set the [`site`](/en/reference/configuration-reference/#site) and, if needed, [`base`](/en/reference/configuration-reference/#base) options in `astro.config.mjs`.
    ...
    :::tip[GitHub Organizations]
    If you're not using custom domains for your [private pages](https://docs.github.com/en/enterprise-cloud@latest/pages/getting-started-with-github-pages/changing-the-visibility-of-your-github-pages-site),
    GitHub will create a random URL for you. 

    ```text
    https://<random-string>.pages.github.io/
    ```

    Make sure you configure the `site` key with the given URL - no `base` needed.
    :::
