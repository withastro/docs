...
[Clever Cloud](https://clever-cloud.com) is a European Platform as a Service that provides hightly automated and scalabale services to enhance developers experience at industrial levels. 
...
4. Set up the minimal size for youy instance and scalability options: Astro can be deployed in a **Nano** instance as minimal size. Instance sizes depends of your application specifiactions and dependencies. We suggest you start with the smallest instance and then adjust accordingly as you watch the metrics from the **Overview** page.
...
Enable auto-scalability in **Scalability** option, to automatically allocate ressources to the server according to it's consumption. This is especially recommended if your Astro app uses Server-Side Rendering (SSR).

#### Dedicated build instance

Aditionnally, enabling a dedicated build instance is available from the **Information** otpion in your application menu. Once the build succesful, your application runs in the minimal instance size you've set.
...
To connect a service to your Astro application, create an add-on and and connect it to your application directly from the Console. See all available add-ons on [Clever Cloud documentation](https://developers.clever-cloud.com/doc/addons/).
