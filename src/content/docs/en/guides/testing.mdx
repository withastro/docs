...
2. Create an `index.spec.js` file in the `cypress/e2e` folder. Use the following test into the file to verify that the page title and header are correct.
...
First, start the dev server so Cypress can access your live site. To run our test from the previous example using the command line, execute the following command:
