...
You can run the tests with the [NightwatchJS VSCode Extension](https://marketplace.visualstudio.com/items?itemName=browserstackcom.nightwatch) or using the CLI commands below:
...
- To see the full HTML Test Report, open it using the following command:
    ...
    ```
- To run the tests against a specific browser use the `--environment` or `-e` CLI argument. If you don't have the relevant browser installed, Nightwatch will attempt to set it up for you using [Selenium Manager](https://www.selenium.dev/blog/2022/introducing-selenium-manager/)
