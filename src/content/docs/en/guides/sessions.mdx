...
Sessions require a storage driver to store the session data. The Node, Cloudflare, and Netlify adapters automatically configure a default driver for you, but other adapters currently require you to [specify a driver manually](#configuring-a-session-driver). You can use [any supported driver from unstorage](https://unstorage.unjs.io/drivers/).
