...
`lang` specifies the primary language getting highlighted.

**Embedded languages**

<p><Since v="6.0.0" /></p>

`embeddedLangs` can be used to register additional languages (for example, `jsx` in `vue`), but isn't necessary for more standard combinations (such as `ts` in `svelte`).
:::
