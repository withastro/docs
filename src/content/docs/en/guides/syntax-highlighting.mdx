...
A Markdown code block is indicated by a block with three backticks \`\`\` at the start and end. You can indicate the programming language being used after the opening backticks.
...
You can [switch to Prism](#add-a-prism-stylesheet), or disable Astro's syntax highlighting entirely, with the [`markdown.syntaxHighlighting`](/en/reference/configuration-reference/#markdownsyntaxhighlight) configuration option.

<ReadMore>See the full [`markdown.shikiConfig` reference](/en/reference/configuration-reference/#markdownshikiconfig) for the complete set of Markdown syntax highlighting options.</ReadMore>
...
Then, add [Shiki's dark mode CSS variables](https://shiki.style/guide/dual-themes#query-based-dark-mode) to your project for site-wide styling. Replace the `.shiki` class in the examples with `.astro-code`:
...
This component This component is powered internally by Shiki. It supports all popular Shiki themes and languages as well as several other Shiki options such as custom themes, languages, [transformers](#transformers), and default colors.
