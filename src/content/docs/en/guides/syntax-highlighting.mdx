...
`lang` specifies the primary language getting highlighted. `embeddedLangs` can be used to register additional languages (for example, `jsx` in `vue`), but isn't necessary for more standard combinations (such as `ts` in `svelte`).

If your `{code}` includes multiple languages and some of them are rendered as plaintext, add the secondary languages via `embeddedLangs`.
