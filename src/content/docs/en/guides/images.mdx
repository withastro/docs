...
To inline local `.svg` files directly in your HTML, import the SVG file to use as an Astro component. This process assigns an `<svg>` HTML tag to a component, and requires the same conventions (e.g. capitalization) as when [using dynamic tags](/en/basics/astro-syntax/#dynamic-tags).
