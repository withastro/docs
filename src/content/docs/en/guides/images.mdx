...
## Inline `.svg` files in the HTML (experimental)
...
To use this feature, import an SVG file to use as an Astro component, passing any common SVG attributes. Astro also provides a `size` attribute to set equal `height` and `width` properties. This process assigns an `<svg>` HTML tag to a component, and requires the same conventions (e.g. capitalization) as when [using dynamic tags](/en/basics/astro-syntax/#dynamic-tags).
