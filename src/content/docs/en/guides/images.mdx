...
## Using `.svg` files as components (experimental)
...
To use this feature, reference the default import of any local `.svg` file. Since this import is treated as an Astro component, it requires using the same conventions (e.g. capitalization) as when [using dynamic tags](/en/basics/astro-syntax/#dynamic-tags).
