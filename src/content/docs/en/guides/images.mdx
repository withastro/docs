...
Images from authorized sources may be cached during the site build, enabling faster subsequent builds. The cache is based on [HTTP Caching](https://developer.mozilla.org/en-US/docs/Web/HTTP/Caching), and respects the [Cache-Control header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) returned by the remote server.
...
<ReadMore>See the configuration reference for [`cacheDir`](/en/reference/configuration-reference/#cachedir)</ReadMore>

##### Revalidation

<p><Since v="5.1.0" /></p>

Once a cached image expires, it is revalidated by using a stored [Last-Modified](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Last-Modified) time or [Etag (entity tag)](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/ETag) to save bandwidth and build time, otherwise it is redownloaded.
