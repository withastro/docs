...
Store your images in the `public/` folder only if you want to avoid any processing.
...
Import and use Astro's built-in `<Image />` component for optimized images. Alternatively, Astro syntax supports writing an HTML `<img>` tag directly.
...
`<Image />` can transform a local or authorized remote image's dimensions, file type, and quality to produce an optimized `<img>` tag to avoid **Cumulative Layout Shift (CLS)**.
...
Use standard Markdown `![alt](src)` syntax in your `.md` files. This syntax works with Astro's Image Services API to optimize your local images and authorized remote images.
...
`astro:assets` is no longer experimental in Astro v3.0 and `<Image />` is now a built-in component and the previous `@astrojs/image` integration has been removed. These and other accompanying changes in using images in Astro may cause some breaking changes when you upgrade your Astro project from an earlier version.
...
```md title="src/pages/posts/post-1.md" "/_astro" ".hash" "../../assets/" del={10}
# My Markdown Page
