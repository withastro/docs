...
When using local images or [inferSize](#infersize) with remote images in their original aspect ratio, the `width` and `height` can be automatically inferred from the source file and are optional.
...
A boolean option that will allow Astro to get the width and height of remote images when set to `true`.

By default this is set to false, because it will add extra blocking time to an on-demand rendered page. 
