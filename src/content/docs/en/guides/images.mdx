...
In `.astro` files, **a local image must be imported from its relative path** to the `.astro` file. This import provides the `src` value for your image.
...
Use the built-in `<Image />` Astro component to display optimized versions of your **local images in the `src/` folder and [configured remote images](#authorizing-remote-images)**.
...
<ReadMore>See details about [the `<Image />` component properties](/en/reference/modules/astro-assets/#image-properties) in the `astro:assets` reference.</ReadMore>

Additionally, the `<Image />` component accepts all properties accepted by the HTML `<img>` tag.
...
However, this allows you to use the image component for all of your images, providing a consistent authoring experience and simplifying your project code.
:::
...
`getImage()` takes an options object with the [same properties as the Image component](/en/reference/modules/astro-assets/#image-properties) (except `alt`).
