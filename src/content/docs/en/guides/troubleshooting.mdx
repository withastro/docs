...
You may notice an imported component's `<style>` tag included in your HTML source even if that component doesn't appear in the final output. For example, this will occur with [conditionally rendered](/en/basics/astro-syntax/#dynamic-html) components that are not displayed. 
