...
### Using Astro with dependencies in the root of a Monorepo

When installing dependencies in a Monorepo, it is possible to also install dependencies in the root `package.json` file instead of the project `package.json`: installing dependencies this way can make some Astro packages to not work or bundle as expected.
It is recommended to just put all of Astro packages in the project `package.json`to avoid this kind of issue; if it can't be avoided you will probably need to manually put the dependencies in the `vite.ssr.noExternal` part of Astro's config:
```js
// astro.config.mjs
...
export default defineConfig({
  vite: {
    ssr: {
      noExternal: ['@astrojs/package-a', '@astrojs/package-b', ..., '@astrojs/package-N']
})
```
