...
By default, a [UI Framework component is not hydrated in the client](/en/basics/framework-components/#hydrating-interactive-components). If no `client:*` directive is provided, its HTML is rendered onto the page without JavaScript.
