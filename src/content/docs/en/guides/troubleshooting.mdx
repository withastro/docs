...
Dynamic imports will also confuse Astro's analyzer. In this case, the dynamic import causes `index.astro` to include `<script>` and `<style>` tags for both `Accordion.astro` and `Tabs.astro`.

```js title="src/components.ts"
...
---
// Astro cannot statically analyze this dynamic import:
const { Accordion } = await import('../components');
---
<Accordion />
```

### A component's `<script>` tag is not included in the bundle

In some rare cases, Astro may fail to include a `<script>` tag in the final bundle. This occurs when the component is imported but not directly re-exported. For example:
...
```astro title="src/pages/index.astro"
---
import { getAccordion } from '../components';
// Astro cannot detect that Accordion was imported here:
