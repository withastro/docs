...
- If the code is in a framework component, try to access these objects after rendering using lifecycle methods (e.g. [`useEffect()`](https://reactjs.org/docs/hooks-reference.html#useeffect) in React, [`onMounted()`](https://vuejs.org/api/composition-api-lifecycle.html#onmounted) in Vue, and [`onMount()`](https://svelte.dev/docs#run-time-svelte-onmount) in Svelte). Tell the framework component to hydrate client-side by using a [client:](https://github.com/en/reference/directives-reference/) directive, like `client:load`, to run these lifecycle methods. You can also prevent the component from rendering on the server at all by adding the [`client:only`](/en/reference/directives-reference/#clientonly) directive.

- move it to a `<script>` tag outside of the frontmatter.
  - This tells Astro to run this code on the client, where `document` and `window` are available.

If the code is in a framework component, there are two ways:

- Entirly client-side rendered
  - You can prevent the component from rendering on the server at all by adding the [`client:only`](/en/reference/directives-reference/#clientonly) directive.
- Partially client-side rendered, which requires two steps:
  - Tell the framework component to hydrate client-side by using a [`client:`](/en/reference/directives-reference/) directive.
  - Within the framework component, use a lifecycle method to specify which parts should be client-side only. e.g. [`useEffect()`](https://reactjs.org/docs/hooks-reference.html#useeffect) in React, [`onMounted()`](https://vuejs.org/api/composition-api-lifecycle.html#onmounted) in Vue, and [`onMount()`](https://svelte.dev/docs#run-time-svelte-onmount) in Svelte.
...
You may notice an imported component's `<script>` or `<style>` tags included in your HTML source even if that component doesn't appear in the final output. For example, this will occur with [conditionally rendered](/en/core-concepts/astro-components/#dynamic-html) components that are not displayed. 
