...
### Using Astro in a monorepo

When working with Astro in a monorepo setup, it is recommended to keep all the dependencies in the project `package.json` file.
However, you may also want to use Astro in the root of the monorepo (for example, to generate static routes properly). In this case, you may encounter some issues with dependencies not being installed and bundled properly.
To avoid this, you need to manually add these dependencies to the `vite.ssr.noExternal` part of Astro's config:
