...
RSS feed has a trailing slash on the end of the links by default. Use `rss` function's `trailingSlash: false` option if you configured your astro.config.mjs to `trailingSlash: "never"`.
