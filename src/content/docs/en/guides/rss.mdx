...
RSS feed includes links with a trailing slash by default. You can change that behavior by setting the `trailingSlash` option when calling `rss`.

If you have set `trailingSlash: "never"` on your `astro.config.mjs`, set `trailingSlash: false` on the call to `rss` so your feed matches your project configuration.
