...
The `items` field accepts a list of RSS feed objects, each requiring either a `title` or a `description`. Five optional values may also be included: `title` if `description` is not yet set, `description` (a short excerpt) if `title` is not yet set, `link`, `pubDate` (date of publication), `content` (the full content of your post), and a `customData` field for including any extra data, such as other frontmatter properties from your blog posts.

You can generate this array from a content collection schema.

:::caution
Note that using [glob imports](/en/guides/imports/#astroglob) (for blog posts located within `src/pages/`) to generate this array is deprecated. Please use [`pagesGlobToRssItems`](#using-glob-imports) instead.
:::
