...
By default, your [middleware](/en/guides/middleware/) is bundled into the ISR function, so it runs only when that function runs: on a cache miss or a regeneration, and not on a cache hit. If your middleware must run on every request, set [`middlewareMode`](/en/reference/adapter-reference/#middlewaremode) to [`'edge'`](#running-astro-middleware-on-vercel-edge-functions) to deploy it in front of the cache, or add the routes concerned to [`exclude`](#excluding-paths-from-caching) so that they are never cached.
