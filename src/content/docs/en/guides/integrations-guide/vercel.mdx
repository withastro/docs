...
This adapter allows Astro to deploy your [on-demand rendered routes](/en/basics/rendering/#on-demand-rendered-routes) to [Vercel](https://www.vercel.com/).

If you're using Astro as a [`static` site builder](/en/basics/rendering/#prerendered-pages), you only need this adapter if you are using additional Vercel services (e.g. [Vercel Web Analytics](https://vercel.com/docs/analytics), [Vercel Image Optimization](https://vercel.com/docs/image-optimization)). Otherwise, you do not need an adapter to deploy your `static` site.
...
Then, add the adapter and your desired [on-demand rendering mode](/en/basics/rendering/#on-demand-rendered-routes) to your `astro.config.*` file:
