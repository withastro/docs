...
### Including files on SSR

When using Server-Side Rendering (SSR), you may need to include specific files in your Netlify Functions that aren't automatically bundled during deployment.
The `includeFiles` property allows you to explicitly specify which files should be bundled with your function.
...
You can also add the `excludeFiles` property to exclude any files from the bundling process that would otherwise be included.
