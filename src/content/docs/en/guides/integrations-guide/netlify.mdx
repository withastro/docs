...
By default, all pages will be rendered on first visit and the rendered result will be reused for every subsequent visit until you redeploy. To set a revalidation time, call the [`runtime.setBuildersTtl(ttl)` local](/en/reference/api-reference/#locals) with the duration (in seconds).
...
See [CHANGELOG.md](https://github.com/withastro/adapters/tree/main/packages/netlify/CHANGELOG.md) for a history of changes to this integration.
