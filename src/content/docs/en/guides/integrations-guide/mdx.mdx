...
All [`markdown` configuration options](/en/reference/configuration-reference/#markdown-options) can be configured separately in the MDX integration, including the [Markdown processor](#processor). Options will default to those in your Markdown config, but you can modify them using the [`extendMarkdownConfig`](#extendmarkdownconfig) option.
