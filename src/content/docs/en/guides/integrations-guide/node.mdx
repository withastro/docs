...
<p>
**Type:** `'classic' | 'on-request' | 'always'` <br />
**Default:** `classic`<br />
...
If you want to also run some middleware during prerendering, you can choose `always` instead. When middleware runs, you can inspect `buildPhase` to determine whether Astro is executing it during `astro build` (`"build"`) or while handling a live request (`"request"`).
