...
[Astro Actions](astro-actions) can be integrated with [React's `useActionState()` hook](https://react.dev/reference/react/useActionState) to update client-side state when triggering actions during form submission.

To do so, pass `experimental_withState()` receiving the action you want to trigger in the `useActionState()` hook:

```jsx title="Like.tsx" ins={2,6}
...
You can also access the state stored by `useActionState()` on the server in your action `handler` by calling `experimental_getActionState()` passing in the API context, and optionally, apply a type to the result:
