...
Astro DB is a fully-managed SQL database designed for the Astro ecosystem: develop locally in Astro and deploy from your [Astro Studio dashboard](http://studio.astro.build).
...
Execute a `.ts` or `.js` file to read or write to your database. This accepts a file path as an argument, and supports usage of the `astro:db` module to write type-safe queries. Use the `--remote` flag to run against your Studio project database, or omit the flag to run against your development server. See [seed development data](#seed-development-data) for an example file.
