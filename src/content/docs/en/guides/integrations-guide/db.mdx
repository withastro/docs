...
The `text` column may optionally define a list of string literals to serve as an enum. This will cause the generated types to be restricted to the given literal values. Although the type will reflect only the given values, **no runtime validation will be performed** so removal, addition and changing values should be handled in your code.
