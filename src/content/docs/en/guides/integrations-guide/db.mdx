...
Compares your local schema against the remote database and exits non-zero if changes are required or unsafe. This is useful in CI. This is automatically run by `astro db push`.
...
Execute a raw SQL query against your database. Example selecting all rows from a `Comment` table in a remote database:
