...
Astro DB supports different driver modes for production deployments, by default it uses a node-based libSQL driver. When deploying to a serverless environment's that don't support all features of Node.js, such as Cloudflare Workers, you can switch to the web libSQL driver allowing for Astro DB to continue functioning in these environments.

To configure the driver mode, set the `mode` property in your `astro.config.mjs` file:
...
:::note
When deploying using the `web` mode, be aware of the limitations and differences compared to the `node` mode. You will only be able to make web-based connections over `libsql`, `http`, or `https`.
:::
