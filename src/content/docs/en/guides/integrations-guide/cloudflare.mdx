...
* **`custom`:** Always uses the image service configured in [Image Options](/en/reference/configuration-reference/#image-options)
...
:::note
With  **`imageService: 'custom'`** guardlines are disabled for the configured image service. This integration will not check if the configured image service is functional in cloudflare environment.
:::
