...
<p>
**Type:** `string`<br />
**Default:** `wrangler.toml`, `wrangler.json`, `wrangler.jsonc`
</p>

Define the Wrangler configuration file. If no value is set, it tracks `wrangler.toml`, `wrangler.json`, and `wrangler.jsonc` in the project root.
...
Set the [environment](https://developers.cloudflare.com/workers/wrangler/environments/) to use. You must select the environment defined in the Wrangler configuration file, otherwise an error occurs.
...
Set whether to save binding data locally to the file system and the path to save it.
