...
This adapter allows Astro to deploy your [`hybrid` or `server` rendered site](/en/core-concepts/rendering-modes/#on-demand-rendered) to [Cloudflare](https://www.cloudflare.com/).

If you're using Astro as a [static site builder](/en/core-concepts/rendering-modes/#pre-rendered), you don't need an adapter.
...
Add the Cloudflare adapter to enable SSR in your Astro project with the `astro add` command. This will install `@astrojs/cloudflare` and make the appropriate changes to your `astro.config.*` file in one step.
...
Then, add the adapter and your desired [on-demand rendering mode](/en/core-concepts/rendering-modes/#on-demand-rendered) to your `astro.config.*` file:
...
**Type:** `{ mode: 'off' } | { mode: 'local'; type: 'pages'; persistTo?: string; bindings?: Record<string, CF_BINDING> } | { mode: 'local'; type: 'workers'; persistTo?: string; };` ([CF\_BINDING](https://github.com/cloudflare/workers-sdk/blob/eaa16db25103d26ef31499634a31b86caccdf7aa/packages/wrangler/src/api/startDevWorker/types.ts#L190-L219))<br />
