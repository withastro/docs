...
:::note
With  **`imageService: 'custom'`** guardlines are disabled for the configured image service. This integration will not check if the configured image service works in Cloudflare's `workerd` runtime.
:::
