...
Determines wether and how the Cloudflare Platform/Runtime is added to `astro dev`. It contains proxies (to local `workerd` bindings) and emulations of Cloudflare specific values, allowing the emulation of the platform/runtime in the Node.js dev process. Read more about the [Cloudflare Runtime](#cloudflare-runtime).

:::note
Proxies provided by this are a best effort emulation of the real production. Although they are designed tobe as close as possbile to the real thing, there might be a slight differences and inconsistencies between the two.
:::

The `enabled` property defines if you want to enable the Cloudflare Platform/Runtime in `astro dev`.
...
The Cloudflare runtime uses bindings found in the `wrangler` and `.dev.vars` configuration file. For example, if you have an [environment variable](https://developers.cloudflare.com/workers/configuration/environment-variables/#add-environment-variables-via-wrangler) configuration set up in `wrangler.toml`:
