...
Sets the name of the KV binding used for session storage. By default, the KV namespace is automatically provisioned when you deploy, and is named `SESSION`. You can change this name by setting the binding manually in your `cf` config. See [Sessions](#sessions) for more information.
...
export default defineConfig({
  worker: {
    ...
    compatibilityDate: '2026-09-23',
    env: {
      MY_SESSION_BINDING: bindings.kv(),
    },
  },
});
```
...
Sets the name of the Images binding used when [`imageService`](#imageservice) is set to `cloudflare-binding`. By default, the binding is automatically provisioned with the name `IMAGES` when you deploy. You can change it by setting the binding manually in your `cf` config:
...
Environment variables and bindings are defined in your `cloudflare.config.ts` configuration file.



Define [environment variables](https://developers.cloudflare.com/workers/configuration/environment-variables/#add-environment-variables-via-wrangler) that do not store sensitive information in `cloudflare.config.ts`:
...
export default defineConfig({
  worker: {
    ...
    compatibilityDate: '2026-09-23',
    env: {
      MY_VARIABLE: bindings.text('test'),
    },
  },
});
```

[Secrets](https://developers.cloudflare.com/workers/configuration/secrets/) are a special type of environment variable that allow you to attach encrypted text values to your Worker. They need to be defined differently to ensure they are not visible within `cf` or Cloudflare dashboard after you set them.

To define `secrets`, add them through the [`cf` CLI](https://developers.cloudflare.com/workers/wrangler/) rather than in your `cf` config file:
...
See the [list of all supported bindings](https://developers.cloudflare.com/workers/wrangler/api/#supported-bindings) in the `cf` documentation.
...
`cf` provides a [`types`](https://developers.cloudflare.com/workers/wrangler/commands/#types) command to generate TypeScript types for your bindings. This allows you to type your environment without the need for manual type definitions.

Run `cf types` every time you change your configuration files (e.g. `cloudflare.config.ts`, `.dev.vars`).

:::note
The following example shows a script configuration to run `cf types` automatically before other commands:

```json title="package.json"
{
  "scripts": {
    "dev": "cf types && astro dev",
    "start": "cf types && astro dev",
    "build": "cf types && astro check && astro build",
    "preview": "cf types && astro preview",
    "astro": "astro"
  }
}
```
...
Astro automatically configures [Workers KV](https://developers.cloudflare.com/kv/) for session storage when using the Cloudflare adapter. `cf` can [automatically provision](https://developers.cloudflare.com/workers/wrangler/configuration/#automatic-provisioning) the KV namespace when you deploy, so no manual setup is required. Alternatively, you can define the KV binding manually in your `cloudflare.config.ts` file and set a custom binding name using the [`sessionKVBindingName`](#sessionkvbindingname) adapter option.
...
To enable Node.js compatibility, add the `nodejs_compat` flag to your `cf` configuration:
...
For Node.js APIs not yet supported in the Workers runtime, `cf` can inject polyfills (requires `nodejs_compat` and a compatibility date of 2024-09-23 or later).
...
By default, errors occurring while running your application in `cf` are minified. For better debugging, add `vite.build.minify = false` to your `astro.config.mjs`:
...
## Upgrading to v14 and Astro 7.4.0

The new version of the adapter drops the use of `wrangler` in favor of the new `cf` CLI.

The v14 of the adapter requires Astro v7.4
...
<PackageManagerTabs>
  <Fragment slot="npm">
    ...
    npm install cf
    npm install astro@v7.4.0.beta.0
    npm install @astojs/cloudflare@v14.0.0-beta.0
    ```
  </Fragment>
  <Fragment slot="pnpm">
    ```shell
    pnpm uninstall wrangler
    pnpm add cf
    pnpm add astro@v7.4.0.beta.0
    pnpm add @astojs/cloudflare@v14.0.0-beta.0
    ```
  </Fragment>
  <Fragment slot="yarn">
    ```shell
    yarn uninstall wrangler
    yarn add cf
    yarn add astro@v7.4.0.beta.0
    yarn add @astojs/cloudflare@v14.0.0-beta.0
    ```
  </Fragment>
</PackageManagerTabs>

Migrate the Wrangler configuration to the new `cloudflare.config.ts`

```ts ins="cf/config" ins="bindings" ins="defineConfig"
import { bindings, defineConfig } from 'cf/config';
