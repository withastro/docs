...
This option allows you to add custom patterns, e.g. `/fonts/*` to the automated generated `_routes.json` file. This can be useful if you need to add patterns which cannot be automatically generated. More information about the pattern can be found in [Cloudflare's routing docs](https://developers.cloudflare.com/pages/functions/routing/#functions-invocation-routes).
...
Configure additional routes in the `routes.extend.include` array. These routes are not automatically deduplicated and will be appended to the existing routes as is.
...
Configure routes to be excluded in the `routes.extend.include` array. These routes are not automatically deduplicated and will be appended to the existing routes as is.
...
export default defineConfig({
  adapter: cloudflare({
    routes: {
      extend: {
        include: [{ pattern: '/static' }], // e.g. if you want to route a prerended page to the SSR function (could be useful for middlewares) 
        exclude: [{ pattern: '/pagefind/*' }], // e.g. if you want to use starlight's pagefind, which is generated staticly during build
      }
    },
  }),
});
