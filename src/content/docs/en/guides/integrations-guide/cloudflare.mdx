...
- **`{ build: 'compile', runtime: 'cloudflare-binding'}`:** Uses `'compile'` at build time for prerendered routes, but uses `cloudflare-binding` as the runtime service used for on-demand rendered pages instead of the default noop `passthrough` service.
...
The default value of `imageService` has changed from `'compile'` to `'cloudflare-binding'`. The `cloudflare-binding` service uses the [Cloudflare Images binding](https://developers.cloudflare.com/images/transform-images/bindings/) to transform images at runtime, and the binding is automatically provisioned when you deploy.

To revert to the previous behavior, set `imageService: 'compile'` explicitly in your adapter config.
