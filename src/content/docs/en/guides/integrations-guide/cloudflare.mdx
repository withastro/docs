...
<Meta default="`'compile'`">
`'passthrough' | 'cloudflare' | 'compile' | 'custom'`
</Meta>
...
<Meta default="`{ configPath: 'wrangler.toml' }`">
`{ configPath?: string }`
</Meta>
...
<Meta default="`{ experimentalJsonConfig?: false }`">
