...
The default image node can be overriden to use a custom Astro component. This example uses a custom `MarkdocImage.astro` component to render images:
...
```astro title="src/components/Image.astro"
---
...
Note that additional attributes cannot be set using the native syntax for images (`![alt](src)`). Instead, you can use a `image` tag to set additional attributes.

Astro will automatically resolve the `src` attribute of any tags named `image`, so you can create Astro component with a `src` prop and use it as a Markdoc node:
...
---
<figure>
    <Image {src} {alt} {width} {height} {customProp} />
    {caption && <figcaption>{caption}</figcaption>}
</figure>

```

and then, in Markdoc files:
