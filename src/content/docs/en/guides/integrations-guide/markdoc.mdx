...
### Custom image component in Markdoc

Markdoc does not currently allow setting additional attributes using the native syntax for images (`![alt](src)`), however a `image` tag can be created to do so.
...
---
<Image {src} {alt} {width} {height} class="my-super-image" />
```
