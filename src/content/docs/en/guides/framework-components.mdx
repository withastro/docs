...
This is because functions can't be _serialized_ (transferred from the server to the client) by Astro. Objects with circular references are also not serializable.

For hydrated components, prop values that are passed in must be serializable. The following types are supported: 
