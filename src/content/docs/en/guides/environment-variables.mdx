...
You can also attach a mode to the filename, like `.env.production`, `.env.development`, or `.env.staging`, which makes the environment variables only take effect in that mode. The mode can be changed by passing the [`--mode` flag](/en/reference/cli-reference/#--mode-string) to the `astro dev` or `astro build` commands (defaults to `"development"` and `"production"` respectively).
