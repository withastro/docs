...
Astro supports 2 main ways for handling environment variables: the `astro:env` module and Vite's built-in support.
...
TODO: where to talk about integrations & adapters?
...
- client/public (accessible on client+server, only use not sensitive data)
- client/secret (impossible)
- server/public (accessible on server, can remain in build output)
- server/secret (accessible on server, no trace anywhere, validated at runtime unless validateSecrets is used)

### Usage

- imports
- getSecret (raw value, proxy for process.env etc BUT it's framework agnostic since provided by adapter)
...
- can only be used within vite context
- cloudflare
