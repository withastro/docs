...
Instead of using `process.env`, you use `import.meta.env`, which uses the `import.meta` feature added in ES2020.
...
Because Astro statically replaces <code>{'import.meta.env'}</code>, you cannot access it with dynamic keys like <code>{'import.meta.env[key]'}</code>.
:::
