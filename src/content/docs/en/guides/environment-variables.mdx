...
<PackageManagerTabs>
 ...
 <Fragment slot="pnpm">
  ...
  <Fragment slot="yarn">
    ...
    ```shell
    # Run the dev server connected to a "staging" API
    bun run astro dev -- --mode staging

    # Build a site that connects to a "production" API with additional debug information
    bun run astro build -- --devOutput

    # Build a site that connects to a "testing" API
    bun run astro build -- --mode testing
    ```
    </Fragment>
</PackageManagerTabs>
...
<PackageManagerTabs>
 ...
 <Fragment slot="bun">
    ```shell
    PUBLIC_POKEAPI=https://pokeapi.co/api/v2 bun run dev
    ```
 </Fragment>
 <Fragment slot="npm">
    ```shell
...
<ReadMore>For a complete list of validation fields, see the [`envField` API reference](/en/reference/modules/astro-config/#envfield).</ReadMore>
