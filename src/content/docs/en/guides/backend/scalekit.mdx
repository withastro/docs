...
[Scalekit](https://scalekit.com/) is an authentication platform that provides social login, enterprise SSO, magic links, and more. It manages the full OAuth 2.0 / OIDC flow so you get back tokens and user profile without building any login UI.
...
To connect to Scalekit, install `@scalekit-sdk/node` and the `@astrojs/node` server adapter in your project.
...
<PackageManagerTabs>
  ...
  <Fragment slot="pnpm">
  ```shell
  pnpm add @scalekit-sdk/node @astrojs/node
  ```
  </Fragment>
  <Fragment slot="yarn">
  ```shell
  yarn add @scalekit-sdk/node @astrojs/node
  ```
  </Fragment>
</PackageManagerTabs>

Then, configure the Node.js adapter and enable server-side rendering in your Astro config:

```ts title="astro.config.mjs" ins={2,3,6,7}
// @ts-check
...
Scalekit handles authentication through a standard OAuth 2.0 / OIDC redirect flow. Your app sends users to Scalekit, they sign in, and Scalekit redirects them back to your callback URL with an authorization code you exchange for tokens.

### Prerequisites

- An Astro project [initialized with Scalekit](#initializing-scalekit-in-astro).
- A redirect URI registered in your Scalekit dashboard under **Settings > Redirects** that matches `SCALEKIT_REDIRECT_URI` in your `.env` file.
...
To add sign-in and sign-out links anywhere in your project, link to the auth endpoints directly:

```astro title="src/components/AuthNav.astro"
---
const user = Astro.locals.user;
---
...
After creating all files, your project should have the following structure:
...
- [Astro blog tutorial with Scalekit authentication](https://github.com/scalekit-inc/developer-docs)
