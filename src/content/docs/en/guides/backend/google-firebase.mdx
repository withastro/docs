...
- An Astro project initialized with Firebase as described in the [Initializing Firebase in Astro](/en/guides/firebase/#initializing-firebase-in-astro) section.
...
- A Firebase project with a Firestore database. You can follow the [Firebase documentation](https://firebase.google.com/docs/firestore/quickstart) to create a new project and set up a Firestore database. 

Our Firestore collection will be called **friends** and will contain documents with the following fields:
...
Feel free to follow this example or adapt it to your own Firestore collection.

### Creating the server endpoints

We will create two endpoints to interact with the Firestore database:
...
Create two new files in a new directory `src/pages/api/friends/`: `index.ts` and `[id].ts`.
...
Now that we have Firestore endpoints set up, we can create the pages that will use them. 
