...
If you don't have an existing database and schema with Turso, create one and connect to the shell using the Turso CLI:
...
Make sure to populate the `posts` table with some content. You'll need it to make the most out of the examples below.
...
Inside `src/pages/index.astro`, import `turso` and execute a SQL query to fetch all `posts` from the table:
...
There are cases where you'll need to pass values to your SQL query for things like a `slug`, `id`, or pagination. Thankfully, the `execute` method also accepts an object that safely passes your variables to the SQL statement.

Create a file `src/pages/posts/[slug].astro`, with the following:
...
That's it! Turso is now set up and configured to read content from the database with Astro.
