...
Astro's `<Image />` component works in `.astro` and `.mdx` files only. See a [full list of component attributes](/en/guides/images/#properties) available for this component, and note that several will differ from Gatsby's attributes.

To continue using [images in Markdown (`.md`) files](/en/guides/images/#images-in-markdown-files) using standard Markdown syntax (`![]()`), you may need to update the link. Using the HTML `<img>` tag directy is not supported in `.md` files, and must be converted to Markdown syntax.
