...
Convert any Next `<Image />` components to [Astro's own image component](/en/guides/images/#image-) in `.astro` or `.mdx` files, or to a [standard HTML `<img>` / JSX `<img />`](/en/guides/images/#images-in-ui-framework-components) tag as appropriate in your React components. 
