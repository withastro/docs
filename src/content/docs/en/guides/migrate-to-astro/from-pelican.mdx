...
- Pelican is written in Python and supports plugins to extend Markdown with custom Python code (Markdown extensions, or MDX), and custom plugins to generate any html from any file format. Astro also supports MDX, that uses [Makrdown remark plugin system](https://github.com/remarkjs/remark) written in JavaScript, but is more opinionated and provides its own Astro file format that allows to write content, html templates, client-side and server-side code and styling all in the same file.
...
Pelican has many plugins that extend its functionality. Explore the [official list of integrations](https://docs.astro.build/en/guides/integrations-guide/), find more in [Astro Integrations Directory](https://astro.build/integrations/), and learn [Astro Integration API](https://docs.astro.build/en/reference/integrations-reference/) to build your own integration.
