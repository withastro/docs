...
Convert any [Nuxt `<nuxt-img/>` or `<nuxt-picture/>` components](https://image.nuxtjs.org/components/nuxt-img) to [Astro's own image component](/en/guides/images/#image--astroassets), or to a standard HTML `<img>` or `<picture>` tag. 

See a [full list of component attributes](/en/guides/images/#properties) available for Astro's `<Image />` component, and note that several will differ from Nuxt's attributes. 
