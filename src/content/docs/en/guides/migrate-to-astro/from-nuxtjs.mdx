...
In Astro, you similarly have two choices: you can [use SSR](https://docs.astro.build/en/guides/server-side-rendering/), or you can export a `getStaticPaths` function to tell the framework which routes to generate for a dynamic page.

Let's convert a `generate` function in Nuxt to a `getStaticPaths` function in Astro.
