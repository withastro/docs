...
:::note
Before deploying your Astro site with SSR (server-side rendering) enabled, make sure you have installed the [appropriate adapter](/en/guides/server-side-rendering/) and configured one of Astro's [on-demand rendering modes: `server` or `hybrid`](/en/basics/rendering-modes/#on-demand-rendered).
:::
...
1. **Can I deploy my static site by uploading HTML files via FTP?**

    Yes. Build your static site locally using the CLI. This will create a `dist/` folder with the necessary files for your your website that you can upload to a host. ([building your site locally](#building-your-site-locally))

2. **Do I have to rebuild my site every time it changes?**

    Your site needs to be rebuilt after a change like adding a new blog post. However, web hosts that support continuous deployment will monitor your site for changes and automatically build and deploy your site for you. This means the new version of your website will be published for you, after your provider has rebuilt the site. ([quick deploy options](#quick-deploy-options))

### Community question and answers

Find people asking and answering questions about deploying your Astro project:

import CommunityLinks from '~/components/CommunityLinks.astro';

<CommunityLinks
  query="deploy"
  stackOverflow="Search for questions about deployment on Stack Overflow"
  reddit="Search for questions about deployment on Reddit"
