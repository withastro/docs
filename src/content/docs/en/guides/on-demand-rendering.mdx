...
The `'server'` output mode does not bring any additional functionality and is only for convenience. It only switches the default page and endpoint behaviour so you can avoid the need to opt out of prerendering manually on the majority of your routes.
:::
