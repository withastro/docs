...
For a **highly dynamic app**, after adding an adapter, [set your build output configuration to `output: 'server'`](/en/reference/configuration-reference/#output) to **server-render all your pages by default**. This is the equivalent of opting out of prerendering on every page.
...
```astro title="src/pages/product/[id].astro" {10}
---
export const prerender = false; // Not needed in 'server' mode
