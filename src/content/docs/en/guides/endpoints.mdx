...
Endpoints support the same [dynamic routing](/en/guides/routing/#dynamic-routes) features that pages do. Name your file with a bracketed parameter name and export a [`getStaticPaths()` function](/en/reference/routing-reference/#getstaticpaths), that will be called before your endpoint and generate the paths for you. Then you can access the same generated parameters using the `params` property, but now in your endpoint too:
...
This will generate four JSON endpoints at build time: `/api/0.json`, `/api/1.json`, `/api/2.json` and `/api/3.json`. Dynamic routing with endpoints works the same as it does with pages, but `getStaticPaths()` is only supported in `static` mode, since it is called at build time.
