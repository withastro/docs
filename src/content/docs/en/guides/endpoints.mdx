...
1. **Why does pinging my API route give me the same output every time? Why does pinging my API route give me the same output every time? Why does my GET function ignore my dynamic paramaters?**

    API endpoints that can listen for requests and execute code on the client must be configured for on-demand rendering. An Astro project is static by default.
    Enable `server` mode in your Astro project, or enable `hybrid` mode and opt out of prerendering with `export const prerender = false`. ([server endpoints API routes](#server-endpoints-api-routes))

2. **Which HTTP methods are supported for static sites?**

    Static endpoints export a GET function. Other HTTPS menthods are only supported in server-rendered endpoints. ([static file endpoints](#static-file-endpoints))

### Community question and answers

Find people asking and answering questions about API endpoints in Astro:

<CommunityLinks
  query="API endpoints"
  stackOverflow="Search for questions about API endpoints on Stack Overflow"
  reddit="Search for questions about API endpoints on Reddit"
