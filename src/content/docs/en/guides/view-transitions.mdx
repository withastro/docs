...
1. **How can I remove flickering in transitions?**

    Check for the browser's current theme in `localStorage` earlier in the navigation process to prevent flashing in dark mode. This can be done by creating a function that will run in response to the `astro:after-swap`event. This event occurs immediately after the new page has replaced the old page, before the DOM elements are painted to the screen. ([`astrobefore-swap`](#astrobefore-swap))

2. **Why is my JavaScript not working on page changes after I enabled view transitions?**

    Bundled module scripts, which are the default scripts in Astro, are only ever executed once. After initial execution, they will be ignored, even if the script exists on the new page after a transition. To ensure that a script runs every time a page is loaded during client-side navigation, it should be executed by a lifecycle event. ([script behavior with view transitions](#script-behavior-with-view-transitions))

3. **How can I keep a video playing when I navigate to another page?**

    You can keep a video playing when navigating to another page by using the transition:persist directive in Astro. This directive allows you to persist components and HTML elements across page navigations instead of replacing them. ([maintaining state](#maintaining-state))


### Community question and answers

Find people asking and answering questions about view transitions in Astro:

<CommunityLinks
  query="view transitions"
  stackOverflow="Search for questions about view transitions on Stack Overflow"
  reddit="Search for questions about view transitions on Reddit"
  discord="Visit our Discord <code>#support</code> channel for posts tagged with <code>view transitions</code>"
/>
