...
You may be concerned about how all these animations would affect users that have [`prefer-reduced-motion`](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-reduced-motion) enabled in their browser. Don't worry! Currently, Astro includes a CSS media query that disables *all* view transition animations whenever this setting is enabled by the user. Instead, the browser will simply swap the DOM elements without an animation.

If you would like to control your animations more granularly, you will need to use the API by itself and make sure that the `<ViewTransitions />` component is not on present anywhere on the page. While this may be a bit of an over-correction, we want to ensure that we accommodate these users as they may have this setting enabled due to health concerns. And, we're open to suggestions if you have a better idea for covering these accessibility concerns. As always, pull requests are welcome!
