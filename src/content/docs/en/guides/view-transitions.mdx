...
The `swapFunction` object of the `astro:transitions/client` module defines a `swap` implementation with five utility functions to describe specific actions related to page elements, attributes, and scripts during the swap transition. These can be used directly to define a custom swap implementation.

The following example uses these six functions to Astro's built-in `swap` implementation:
...
<script>
```
Custom swap implementations can start with this structure and add or replace individual steps with custom logic as needed.
