...
An event listener to/that.... FILL THIS IN MATTHEW

When using the `<ViewTransitions />` component, Astro triggers the `astro:load` event on the `document` when a new page loads. This happens both when the initial page loads and for any subsequent navigation. 

Use this event to run code on every page navigation, or only once ever:
