...
You can also trigger client-side navigation via events not normally listened to by the `<ViewTransitions />` router using `navigate`. This function from the `astro:transitions/client` module is usable in scripts, and framework components added with a [client directive](/en/reference/directives-reference/#client-directives).
...
```
The following example implements the same using a React component:
