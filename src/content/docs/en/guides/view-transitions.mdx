...
:::note
Some browsers, notably Firefox, Firefox for Android, and older versions of Safari and Safari on iOS don't natively support View Transitions. Astro [provides a fallback](#fallback-control) for these browsers. However, the transitions will not automatically apply by simply adding `<ClientRouter />` as they would in browsers supporting View Transitions. You must explicitly set the `transition:name` or `transition:animate` directives on the elements where you want the transitions to occur. If you want consistency between browsers, be explicit.
:::
...
The `<ClientRouter />` router works best in browsers that support View Transitions (i.e. Chromium browsers), but also includes default fallback support for other browsers. Even if the browser does not support the View Transitions API, Astro's client router can still provide in-browser navigation using one of the fallback options. Depending on browser support, you may need to explicitly set the `name` or `animate` [`transition` directives](#transition-directives) on the elements you wish to navigate for a comparable experience across all browsers.
