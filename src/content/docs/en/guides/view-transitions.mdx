...
The `<ViewTransition />` router fires a number events on the `document` during navigation. These events provide hooks into the lifecycle of navigation, allowing you to do things like showing indicators that a new page is loading, override default behavior, and restore state as navigation is completing.
...
```
Compared to the spinner example above, this example looks simpler: If all of the listener's code can be executed synchronously, there is no need to hook into the loader's callback.  
### `astro:before-swap`
