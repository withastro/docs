...
There are some cases where you might want to not navigate via CSR. A transition will only occur when both the old and new page has the `<ViewTransitions />` component. There may be cases where you want a nice animation coming from one page, such as going from an article listing page `/articles` to an article detail page `/articles/emperor-penguins`, but you don't want the an animation when coming from somewhere else on the site.
...
There may be occasions when you need to trigger a navigation imperatively, such as via an event that occurs within JavaScript. The `astro:transitions/client` module provides a method for doing this. In any client script or client component you can use the module like so:
