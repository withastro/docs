...
For example, if implementing a `?redirect` query parameter, it would be bad if `?redirect=http://malicious-site.com` or even `?redirect=javascript:alert('Evil code')` could be used to navigate away from your site or execute arbitrary code.

To implement this safely, you could ensure only a set of known paths can be redirected to:
