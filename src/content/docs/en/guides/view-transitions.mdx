...
The following example shows all the necessary properties to define a custom transition animation.

In this instance, the custom animation `bump` is defined inside a `<style is:global>` tag in your root layout file:
...
Remember to define the animation behavior in the frontmatter of all the components using the animation:
...
```
When customizing animations, take some time to experiment with different variations. You can use different objects for forward and backward, and your keyframe animations for old and new can be different as well.
