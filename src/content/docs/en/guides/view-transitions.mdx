...
#### Building blocks for custom swap functions

<p><Since v="4.15.0" /></p>

Astro offers utility functions that can be used to define a custom swap implementation.
The `swapFunction` object of the `astro:transitions/client` module provides several reusable building blocks. The following example demonstrates how to combine these blocks to re-create Astro's built-in swap implementation.
