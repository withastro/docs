...
Keep in mind that this event is fired before the new pages scripts have started listening. As such, in the above example, any modifications to the dark mode
are managed by the current page, not the next page. This event could be used to pass page overrides to the new page to provide a more SPA experience (ex scroll to X element, tweak back button logic, etc.).
