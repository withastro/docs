...
In the example below, the `count` for the `<Counter />` component will not be reset when navigating back and forth across pages that contain the `transition:persist` attribute.
