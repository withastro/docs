...
:::tip
By default your page will reload after the `POST` request is sent, if you are using Javascript to prevent this with `event.preventDefault()` you will also want to opt out of router transitions for this form. 
:::
...
:::tip
Because module scripts that add event listeners for `DOMContentLoaded` only execute once, you will also need to add listeners for a [Lifecycle event](#lifecycle-events).
:::

See the [Update Scripts Tutorial](/en/tutorials/add-view-transitions/#update-scripts) for more information.
