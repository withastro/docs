...
When using the `<ViewTransitions />` component page navigations do not behave exactly the same as they do when navigating directly to a page (such as typing the URL into the address bar).

### Scripts

When navigating between pages with the `<ViewTransitions />` component scripts are run in sequential order to match browser behavior. If you have code that sets up global state, this state will need to take into account that the script might execute more than once. You can usually do this by checking for the global state and conditionally executing your code.
...
Additionally, Astro triggers the `astro:load` event on the `document` when a new page loads. This happens both when the initial page loads and for any subsequent navigation. You can use this event to run code on every page navigation, or only once ever.
