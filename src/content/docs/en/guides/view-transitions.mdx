...
This event is useful to restore any state on the DOM that needs to transfer over to the new page. If you are implementing dark mode support, this event can be used to restore state onto the new page like so:
...
<script>
  ...
  // MPA nav
	toggle();
  // ViewTransitions nav
	document.addEventListener('astro:beforeload', toggle);
</script>
<style is:global>
	html[data-dark] body {
		background: black;
