...
```
```js title="form.jsx" "navigate(href)"
import { navigate } from 'astro:transitions/client';
...
:::note
Using `navigate()` in a framework component requires the component to be added to the page with a [client directive](/en/reference/directives-reference/#client-directives). If `navigate()` is called during the server side render of your component, it has no effect.
:::
