...
- `options` - An optional object with the following properties:
  ...
  - `formData`: A FormData object for `POST` and `GET` requests.
...
With this option the router will trigger in-page transitions from `<form>` elements, supporting both `GET` and `POST` requests. You can opt-out of router transitions on a form using the `data-astro-reload` attribute:
