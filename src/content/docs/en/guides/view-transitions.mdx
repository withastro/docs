...
You can override this behavior with `transition:persist-props="true"`. Adding this directive will keep an island's existing props (not re-render with new values) in addition to maintaining its existing state.
