...
- `options` - An object with the following properties:
    - `history`: `'push'` | `'replace'` | `'auto'`
    	...
    	- `'replace'` it will use `history.replaceState` which will update the URL without adding a new entry into navigation.
    	- `'auto'` option will also do a `history.pushState` in most cases, but if the URL is not one that can be transitioned to, it will not throw and the current URL will remain with no changes to the browser history.
