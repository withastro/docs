...
There are times when you will want or need a full-page browser referesh. For example when a link takes a visitor to a `.pdf` document, you will need the browser to load that new page from the server. Even with view transitions enabled in your Astro project, you will be able to specify how the browser should navigate on a per-link basis, even opting out of client-side routing entirely.
