...
In Astro, you use the standard `lowercase` format for all HTML attributes instead of the `camelCase` used in [JSX](https://react.dev/learn/writing-markup-with-jsx#3-camelcase-salls-most-of-the-things). This even works for `class`, which is not supported by React.

```jsx del={1} ins={2} title="html-example.astro"
<div className="box" contentEditable />
<div class="box" contenteditable />
```

Also, in Astro, you use standard `kebab-case` format for SVG attributes instead of the `camelCase` used in JSX.

```jsx del={1} ins={2} title="svg-example.astro"
<circle strokeWidth="2" strokeLinejoin="round" />
<circle stroke-width="2" stroke-linejoin="round" />
```
