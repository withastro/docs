...
:::note
When SSR is enabled, prerendered dynamic routes will take precedence over non-prerendered dynamic routes (server dynamic routes).
For example, if you have a dynamic route `/posts/[xPrenderedRoute].astro` and a static route `/posts/[serverRoute].astro`, the static route will be used to build `/posts/create` in the prerendered build, but the dynamic route will be used in the server build.
:::
