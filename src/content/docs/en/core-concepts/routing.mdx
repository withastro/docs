...
Sometimes you will need to redirect your readers to a new page, either permanently because your site structure has changed, or in response to an action such as logging in to an authrenticated route.
...
You can specify a mapping of redirects in your Astro config. For most redirects, this is a mapping of old route to the new route:
...
These redirects follow the same rules as file-based routes. Dynamic routes are allowed as long as both the new and old route contain the same parameters, for ex:
...
You can also specify the status code by providing an object as the value:
...
When running `astro build` Astro will write out HTML files with the [meta refresh](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#examples) tag by default. Supported adapters will instead write out the host's configuration file with the redirects.
