...
A page must produce a full HTML document. If not explicitly included, Astro will add the necessary `<doctype>` declaration and `<head>` content to any `.astro` component located within `src/pages/` by default. You can opt-out of this behavior on a per-component basis by marking it as a [partial](#partials) page.
...
Like `.astro` components located outside of this folder, these files do not automatically include the `<doctype>` declaration, nor any `<head>` content such as scoped styles and scripts.
