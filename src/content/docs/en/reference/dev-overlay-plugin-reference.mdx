...
This event is fired when the user minimized or maximized the dev overlay.

```ts title="src/my-plugin.js" {3-6}
  export default {
    // ...
    init(canvas, eventTarget) {
      eventTarget.addEventListener('overlay-toggled', (event) => {
        if (event.detail.state === false) {
          console.log("The dev overlay is now minimized!")
        }
      })
    }
  }
