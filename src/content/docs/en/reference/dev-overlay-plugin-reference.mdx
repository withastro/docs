...
Shows an icon. An icon from [the icon list](#icons) can be specified using the `icon` attribute, or the SVG markup of the icon can be passed as a slot.

```html
<astro-dev-overlay-icon icon="astro:logo"></astro-dev-overlay-icon>
```
