...
interface AstroIntegration {
  ...
  hooks: {
    'astro:config:setup'?: (options: {
      ...
      injectDts: (dts: { name: string; content: string }) => void;
      codegenDir: URL;
...
**Type:** `(dts: { name: string; content: string }) => void;`

A callback function that injects a `.d.ts` file inside the user's `.astro` directory. The file will be referenced for you in the user's project, no need to create a manual reference in `src/env.d.ts`.

This is useful for generating types based on user content, virtual modules etc.
...
:::note
`astro.d.ts` and `types.d.ts` are reserved names.
:::
...
The absolute path to the `.astro` directory.

```js
const dataPath = new URL('./data.json', codegenDir);
```

:::note
This directory is always created before the `astro:config:setup` hook runs so you don't have to!
:::
