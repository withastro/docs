...
The `defineIntegration` helper allows defining an integration in a type-safe way and optionally validate options.
...
The helper accepts an optional `optionsSchema` argument that is a zod schema.
...
In setup, you'll want to add any logic that is shared between any hook, for example saving the `config` from `astro:config:done` to be used in later hooks. It needs to return Astro hooks.
  ...
  setup({ name }) {
    return {
      hooks: {
          ...
          "astro:build:done": ({ logger }) => {
              const buildLogger = logger.fork(`${name}/build`);
              // do something
