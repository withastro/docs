...
interface AstroIntegration {
  ...
  hooks: {
    ...
    'astro:config:done'?: (options: { config: AstroConfig }, bag: { logger }) => void | Promise<void>;
    'astro:server:setup'?: (options: { server: vite.ViteDevServer }, bag: { logger }) => void | Promise<void>;
    'astro:server:start'?: (options: { address: AddressInfo }, bag: { logger }) => void | Promise<void>;
    'astro:server:done'?: (bag: { logger }) => void | Promise<void>;
    'astro:build:start'?: (bag: { logger }) => void | Promise<void>;
    'astro:build:setup'?: (options: {
      vite: ViteConfigWithSSR;
      pages: Map<string, PageBuildData>;
      target: 'client' | 'server';
    }, bag: { logger }) => void | Promise<void>;
    'astro:build:generated'?: (options: { dir: URL }, bag: { logger }) => void | Promise<void>;
    'astro:build:ssr'?: (options: { manifest: SerializedSSRManifestm, entryPoints: Map<RouteData, URL> }, bag: { logger }) => void | Promise<void>;
    'astro:build:done'?: (options: { dir: URL; routes: RouteData[] }, bag: { logger }) => void | Promise<void>;
  };
}
```
...
Each hook, other than passing some options, passes a second parameter called `bag`. This is an object that contains useful APIs that 
integrations can use internally. 
...
The example above will log something similar to the following:

```shell
[astro-format] Integration ready.
```

If you need to log some messages that with a different label, you can a special method called `.fork`:
