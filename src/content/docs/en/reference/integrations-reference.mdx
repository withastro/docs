...
When publishing the above mentioned example package `@fancy/dashboard` to npm, it's required to export the `dashboard.astro` file in the `package.json` export section as follows: `"exports": {"./dashboard.astro": "./dashboard.astro"}`. Otherwise, the file will be looked up in the project folder, which will cause a `no such file or directory` exception.
