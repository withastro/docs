...
Tells you whether the project is being built for static output (HTML files) or server output (on-demand rendered pages). This is the final computed value that accounts for all routes and server islands, so adapters can use it to decide whether to build SSR functionality.
