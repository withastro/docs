...
- `astro` is an environment used during the development, and it's used as "secondary" SSR environment when the Vite `ssr` environment [isn't a runnable dev environment](https://vite.dev/guide/api-environment-frameworks#runnabledevenvironment). 

If your integrations rely on [`vite` environments](https://vite.dev/guide/api-environment) and the Astro build, you might want to optimise based on these environments provided. One of the main usage of Vite environments, is the ability to run a configure plugins conditionally.
