...
The following example checks the [`i18n.strategy`](#i18nstrategy) configuration stored in the `manifest`:
...
Defines a list of serialized route information. Each route contains the same properties as [`SSRManifest.routes`](#routes), with `routeData` converted to a JSON-serializable format.
