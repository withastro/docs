...
Determines whether this application uses some on-demand rendered routes.
...
Specifies the latest available version to notify the user in the dev toolbar. This will be `undefined` when one of the following conditions applies:
- the check fails or has not completed yet
...
Defines the serialized [debug information](/en/reference/cli-reference/#astro-info) passed to the developer toolbar for display.
