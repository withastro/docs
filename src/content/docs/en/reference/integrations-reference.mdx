...
When publishing your package (`@fancy/dashboard`, in this case) to npm, you must export `dashboard.astro` in your `package.json`. Otherwise, the file will be looked up in the project folder, which will cause a `no such file or directory` exception.
