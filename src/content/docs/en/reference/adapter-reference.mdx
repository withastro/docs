...
For example, if CSP enabled, the `<meta http-equiv="content-security-policy">` element **isn't** added to the static page, instead its `content` is available in the `experimentalRouteToHeaders` map. 
