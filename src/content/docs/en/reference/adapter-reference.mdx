...
This module is used in conjunction with [`astro/app/entrypoint`](#astroappentrypoint) to convert a NodeJS `IncomingMessage` into a web-standard `Request` and stream a web-standard `Response` into a NodeJS `ServerResponse`.
