...
- With `"auto"` (recommended), you are responsible for providing a valid module as an entrypoint using either [`serverEntrypoint`](/en/reference/adapter-reference/#serverentrypoint) or, if you need further customization at the Vite level, [`vite.build.rollupOptions.input`](https://rollupjs.org/configuration-options/#input).
