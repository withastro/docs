...
The method accepts a mandatory `request` arguments, and two other optionals arguments: `routeData` and `locals`.

[//]: # (TODO) document `routeData`

`locals` is an optional object to pass to the adapter. This object will be passed to any [middleware function](/en/guides/middleware/).
