...
- With `"explicit"` (deprecated), Astro requires providing the exports required by the host in the server entrypoint using a `createExports()` function before passing them to `setAdapter()` as an [`exports`](#exports) list. This is default for backward compatibility.
...
:::caution[Deprecated]
This property is deprecated and will be removed in a future major version. Learn how to [update your adapter with  `entrypointResolution: "auto"`](/en/guides/upgrade-to/v6/#deprecated-entrypointresolution-explicit-adapter-api).
:::
