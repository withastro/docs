...
`entrypointResolution: "explicit"` is deprecated. `entrypointResolution: "auto"` will become the default behavior in a future major version.

See [how to migrate](/en/guides/upgrade-to/v6/#deprecated-entrypointresolution-explicit-adapter-api).
:::

Specifies the method Astro will use to resolve the server entrypoint:

- With `"explicit"` (deprecated), Astro requires providing the exports required by the host in the server entrypoint using a [`createExports()`](#createexports) function before passing them to `setAdapter()` as an [`exports`](#exports) list. This is default for backward compatibility.
- With `"auto"`, you are responsible to provide a valid module as entrypoint using either [`serverEntrypoint`](/en/reference/adapter-reference/#serverentrypoint) or [`vite.build.rollupOptions.input`](https://rollupjs.org/configuration-options/#input).
...
<ReadMore>Learn more about how to [build a server entrypoint](#building-a-server-entrypoint) or a [legacy server entrypoint](#building-a-legacy-server-entrypoint), and their differences.</ReadMore>
...
This property is only available with [`entrypointResolution: "explicit"`](#entrypointresolution).
...
This module is used for rendering pages in development or that have been prebuilt through `astro build`. Astro uses the standard [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request) and [`Response`](https://developer.mozilla.org/en-US/docs/Web/API/Response) objects. Hosts that have a different API for request/response should convert to these types in their adapter.
...
Returns an [`App` instance](#astroapp).

## Building a legacy server entrypoint
...
Here is how you can build a server entrypoint when using [`entrypointResolution: "explicit"`](#entrypointresolution). You will need to create a file that executes during server-side requests to enable on-demand rendering with your particular host. Astro's adapter API attempts to work with any type of host and gives a flexible way to conform to the host APIs.
...
An exported function that takes an SSR manifest as its first argument and an object containing your adapter [`args`](#args) as its second argument. This is only available with [`entrypointResolution: "explicit"`](#entrypointresolution) and should provide the exports required by your host.
...
```js title="my-adapter.mjs" ins={10}
export default function createIntegration() {
  return {
    name: '@example/my-adapter',
    hooks: {
...
An exported function that takes an SSR manifest as its first argument and an object containing your adapter [`args`](#args) as its second argument. This is only available with [`entrypointResolution: "explicit"`](#entrypointresolution).
