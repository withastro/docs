...
In `always` mode, middleware runs during the build as well as on-request of dynamic and prerendered pages. Users can access `buildPhase` from the request context to configure middleware execution depending on the phase (`build` or `request`).
...
**Type:** `"classic" | "edge" | "always" | "on-request"`<br />
<Since v="6.0.0" />
</p>
