...
To support [`astro:env` secrets](/en/guides/environment-variables/#getsecret), you need to update the `envGetSecret` adapter feature. Additionally, the `astro/env/setup` module allows you to provide an implementation for `getSecret()`. In your server entrypoint, call `setGetEnv()` as soon as possible:
...
If you support secrets, calling `getSecret()` before `setGetEnv()` is called will throw a runtime error. This is important to keep in mind if your environment variables are tied to the request:
