...
Defines an array of named exports to use in conjunction with the [`createExports()` function](/en/reference/adapter-reference/#exports-1) of your server entrypoint.
...
Defines the path of a module in the adapter's package that is responsible of starting up the built server when `astro preview` is run.
...
The `App` constructor accepts a mandatory SSR manifest and a second optional argument, defaulting to `true`, to enable or disable streaming.
...
* Use `"deprecated"` when your adapter deprecates a feature support before removing it completely in a future version.
* Use `"experimental"` when your adapter adds support for a feature and issues or breaking changes are expected.
