...
The `app.render()` method accepts a mandatory `request` argument, and an optional argument for [`clientAddress`](/en/reference/api-reference/#astroclientaddress), [`locals`](/en/reference/api-reference/#astrolocals), and [`routeData`](/en/reference/integrations-reference/#routedata-type-reference).
