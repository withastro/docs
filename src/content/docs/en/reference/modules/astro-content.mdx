...
> **Note:** In Astro v5, `reference()` no longer validates entry identifiers at build time. Validation happens at runtime when using `getEntry()` or `getEntries()`, which may return `undefined` if the referenced entry is invalid.
>
> Despite this change, `reference()` remains valuable for its cleaner syntax and improved TypeScript support compared to manually specifying collection references.
