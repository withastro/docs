...
An error returned when the loader failed to load the collection. This can be a custom error extending the `Error` class or a built-in error.
...
An error returned when the loader failed to load the entry. This can be a custom error extending the `Error` class or a built-in error.
...
An object providing a hint for how to cache this entry.
