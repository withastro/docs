...
Defines the format expected by an action. If your action accepts form inputs, use `accept: "form"` with the `z.object()` validator to automatically parse `FormData` to a typed object.

All Zod validators are supported for form data fields (e.g. `z.coerce.date()`). Astro additionally provides special handling under the hood for your convenience for validating the following types of field inputs:
...
export const GET: APIRoute = ({ params, request }) => {
  request.url.searchParams.delete(ACTION_QUERY_PARAMS.actionName);
  request.url.searchParams.delete(ACTION_QUERY_PARAMS.actionPayload);
...
The following example handles error when incrementing the like count fails:
...
A string representation of the action that can be used to construct form action URLs.

The following example uses `queryString` to construct the URL of a confirmation form:

```astro title="src/pages/form.astro" "queryString"
---
import { actions } from 'astro:actions';
const confirmationUrl = new URL('/confirmation', Astro.url);
confirmationUrl.search = actions.myAction.queryString;
---
