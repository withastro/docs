...
A utility type that extracts the input's Zod schema type from [an action handler](#defineaction). This can be useful to reference an [action's validator](#input-validator) input type in your own type definitions.
