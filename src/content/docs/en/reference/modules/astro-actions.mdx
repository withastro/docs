...
When your action accepts form inputs, the `z.object()` validator will automatically parse `FormData` to a typed object. All Zod validators are supported to validate your inputs. However, the output type is not guaranteed to match your `input` validator except when you use arrays or booleans. Any falsy values will be internally converted to `null`.

For example, if an input of type `text` is sent with an empty value, the result will be `null` instead of an empty string (`""`).
