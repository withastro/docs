...
:::caution
Except when using arrays or booleans, any falsy values will be internally converted to `null`. For example, if an input of type `text` is sent with an empty value, the result will be `null` instead of an empty string (`""`).
:::
