...
Defines a human-readable version of an [HTTP status code](#action_error_codes).
...
**Type:** <code><a href="#actions-1">Actions</a></code>
</p>
...
**Type:** <code>\<T\>(handler: () => T | Promise\<T\>) => Promise\<<a href="#saferesult">SafeResult</a>\<ZodType, T\>\></code>
</p>
...
An array of standard HTTP status codes [defined by IANA](https://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml) using the human-readable versions as uppercase strings separated by an underscore (e.g. `BAD_REQUEST` or `PAYLOAD_TOO_LARGE`).

### `ACTION_QUERY_PARAMS`

<p>

**Type:** `{ actionName: "_action"; actionPayload: "_astroActionPayload"; }`
</p>

### `ActionInputError`

Creates a new error by passing an array of Zod issues to its constructor. This is useful to create a different type of error when validating inputs. You can then use it to refine the type of an error and handle it differently.
...
A subset of the Astro context object.
...
**Type:** `((input?: any) => Promise<SafeResult<never, Awaited<TOutput>>>) & { queryString?: string; orThrow: (input?: any) => Promise<Awaited<TOutput>>; }`
</p>
...
**Type:** <code>(typeof <a href="#action_error_codes">ACTION_ERROR_CODES</a>)[number]</code>
</p>
...
In order for Astro to recognize this type, you may need to restart the dev server or [sync the module](/en/reference/cli-reference/#astro-dev) (`s + enter`).
...
An object containing information about an inbound action request. It is available from [`getActionContext()`](#getactioncontext), and provides the action name, handler, and whether the action was called from an client-side RPC function (e.g. `actions.newsletter()`) or an HTML form action.
...
**Type:** `{ data: TOutput; error: undefined; } | { data: undefined; error: ActionError<TInput>; }`
</p>
...
**Type:** `{ type: "data"; contentType: "application/json+devalue"; status: 200; body: string; } | { type: "error"; contentType: "application/json"; status: number; body: string; } | { type: "empty"; status: 204; }`
</p>
