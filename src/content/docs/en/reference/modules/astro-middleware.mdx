...
A function for defining a middleware function with type safety. When you use this utility, the [`context`](#context) and [`next()`](#next) arguments are automatically typed, and you will get a Typescript error if you try to return a [value not supported](#middlewarehandler) in your middleware.
...
Represents an Astro middleware function. This is useful to get type safety when you are not able to use [`defineMiddleware()`](#definemiddleware).
...
Represents the `next()` function passed to middleware handlers. This function takes an optional URL path parameter to rewrite the current `Request` to a different location without triggering a new rendering phase.
