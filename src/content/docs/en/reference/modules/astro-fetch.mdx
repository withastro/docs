...
Reports whether the original request matched an Astro route. Unlike checking `state.routeData`, which can hold Astro's internal `/404` fallback for an unmatched request, `hasMatchedRoute()` returns `false` when no route matched:
