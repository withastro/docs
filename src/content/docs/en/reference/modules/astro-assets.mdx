...
**Type:** <code>(src: string, \{ domains, remotePatterns \}: \{ domains: string[], remotePatterns: <a href="#remoteimagepatternconfig">RemoteImagePatternConfig</a>[] \}) => boolean</code><br />
<Since v="4.0.0" />
</p>
...
Determindes whether the redirects a given remote resource might contain can be followed. The resource is identified by it's source URL and redirects are allowed when a matching remote pattern allows redirects.

```ts
import { isRemoteRedirectAllowed } from 'astro/assets/utils';
...
const remotePatterns = [
  {
    ...
    hostname: 'images.example.com',
    pathname: '/**', // Allow any path under this hostname
    followRedirects: true, // Follow any HTTP redirects the image 
  }
];

isRemoteRedirectAllowed(url.href, { domains, remotePatterns }); // Output: `true`
```
...
### `RemoteImageConfigPattern`

<p>

**Type:** `{ hostname?: string; pathname?: string; protocol?: string; port?: string; followRedirects?: boolean; }`<br />
<Since v="6.2.0" />
</p>

Extends the <a href="#remotepattern">RemotePattern</a> type with an optional `followRedirects` property. Used when fetching remote images.
