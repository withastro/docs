...
**Type:** `(pluginContext: Rollup.PluginContext, options: Rollup.EmitFile) => string`<br />

<Since v="6.0.0" />
</p>

Emits a client asset and tracks it for later movement to the client directory during SSR builds. This function is intended for integration authors who need to emit assets (such as images) from server-rendered content that should be available on the client.

Use this instead of `pluginContext.emitFile()` directly when working in a Vite plugin context and you need the emitted asset to be moved to the client output directory.
...
// Returns the asset handle (e.g., "a]b2c3d4")
```
