...
If you need the resulting image URL client-side, pass the `src` from a server-rendered call to the client. See [generating images with `getImage()`](/en/guides/images/#generating-images-with-getimage) for an example.
:::
