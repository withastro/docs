...
**Default:** `image.experimentalLayout | 'none'` <br />
<Since v="5.x.0" />
</p>

Defines a [responsive image]() and determines how the image should resize when its container changes size. Can be used to override the default configured value for image.experimentalLayout.
...
When a layout is set, images have automatically generated `srcset` and `sizes` attributes based on the image's dimensions and the layout type. Images with `constrained` and `full-width` layouts will have styles applied to ensure they resize according to their container.
...
**Default:** `image.experimentalObjectFit | 'cover'` <br />
<Since v="5.x.0" />
</p>
...
Values match those of CSS `object-fit`. Defaults to `cover`, or the value of [`image.experimentalObjectFit`](/en/reference/configuration-reference/#imageexperimentalobjectfit) if set. Can be used to override the default `object-fit` styles. 
...
**Type:** `string` <br />
**Default:** `image.experimentalObjectPosition | 'center'` <br />
<Since v="5.x.0" />
</p>
...
Values match those of CSS `object-position`. Defaults to `center`, or the value of [`image.experimentalObjectPosition`](/en/reference/configuration-reference/#imageexperimentalobjectposition) if set. Can be used to override the default `object-position` styles. 
