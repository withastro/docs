...
Describes the properties of a local image. This ensures that [`src`](#src-required) matches the shape of an imported image.
...
Describes the properties of a remote image. This ensures that when [`inferSize`](#infersize) is not provided or is set to `false`, both [`width` and `height`](#width-and-height-required-for-images-in-public) are required.
...
**Type:** <code>(options: <a href="#unresolvedimagetransform">UnresolvedImageTransform</a>, imageConfig: <a href="/en/reference/configuration-reference/#image-options">AstroConfig['image']</a> & AstroAdapterClientConfig) => Promise\<<a href="#getimageresult">GetImageResult</a>\></code>
</p>

A function similar to [`getImage()`](#getimage) from `astro:assets` with two required arguments: an `options` object with [the same properties as the Image component](#image-properties) and a second object for the [image configuration](/en/reference/configuration-reference/#image-options).
...
**Type:** `{ values: SrcSetValue[]; attribute: string; }`<br />
<Since v="3.3.0" />
</p>
...
**Type:** `ImageFit | undefined`<br />
<Since v="5.0.0" />
</p>

Controls the value for the `object-fit` CSS property.
...
Represents an image with transformation options.
...
See the [`inferSize` attribute](/en/reference/modules/astro-assets/#infersize) available on `<Image />`.
...
Describes an object of data collected during image import. This contains the following properties:
