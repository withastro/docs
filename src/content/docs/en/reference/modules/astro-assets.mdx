...
const buffer = await experimental_getFontBuffer(fontPath);
```
...
export const GET: APIRoute = async (context) => {
  // ...
  const buffer = await experimental_getFontBuffer(fontPath, context.url);
};
```
