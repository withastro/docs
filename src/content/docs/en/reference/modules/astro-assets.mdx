...
**Type:** <code><a href="#localimageprops">LocalImageProps</a> | <a href="#remoteimageprops">RemoteImageProps</a></code>
</p>
...
This takes an options object with the [same properties as the Image component](#image-properties) (except `alt`) and returns a [`GetImageResult` object](#getimageresult).
...
Describes the [properties of a local image](#image-properties). This ensures that [`src`](#src-required) matches the shape of an imported image.
...
Describes the [properties of a remote image](#image-properties). This ensures that when [`inferSize`](#infersize) is not provided or is set to `false`, both [`width` and `height`](#width-and-height-required-for-images-in-public) are required.
