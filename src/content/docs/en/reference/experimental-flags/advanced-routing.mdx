...
Registers a context provider under the given key. The provider's `create` function is called lazily on the first `state.resolve(key)`, and its optional `finalize` callback runs during `state.finalizeAll()`:
