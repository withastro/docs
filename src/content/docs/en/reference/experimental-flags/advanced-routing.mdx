...
These handlers replace the functionality of the adapter's default entrypoint (`@astrojs/cloudflare/utils/handler`). When using them, point `main` in your `wrangler.jsonc` to your own entrypoint file instead of the default handler. Do not use both at the same time.
