...
The `fontData` object allows you to retrieve lower-level font family data programmatically, for example, in an [API Route](/en/guides/endpoints/#server-endpoints-api-routes) or to generate your own meta tags.
...
An object where each key is a [`cssVariable`](#cssvariable-1) and the value is an array describing the associated fonts. Each entry contains the `src`, `weight`, and `style` properties.

The following example uses `fontData` to get the font buffer from the URL when using [satori](https://github.com/vercel/satori) to generate OpenGraph images:

```tsx title="src/pages/og.png.tsx" "fontData[\"--font-roboto\"]" "data[0].src[0].url"
