...
If not specified or set to `false`, no preload link will be emitted. If set to `true`, all possible links will be included.
...
In this example, only font files that have a `400` weight, or a `normal` style in the `latin` subset will be preloaded.
