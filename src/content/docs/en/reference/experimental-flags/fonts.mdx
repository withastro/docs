...
1. `experimental.fonts` accepts an array of font objects. For each font, you must specify a `provider`, the family `name`, and define a `cssVariable` to refer to your font.
    ...
    - `provider`: You can choose from the list of [built-in remote providers](#available-remote-font-providers), build your own [custom font provider](#build-your-own-font-provider), or use the [local provider](#local-font-properties) to register local font files.
...
Any unifont provider can be used to make a [custom Astro font provider](#build-your-own-font-provider).
