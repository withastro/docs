...
:::caution
We recommend not to store fonts in [`/public`](/en/reference/configuration-reference/#publicdir) as this will result in duplicated files in your build output. Instead, store them in [`/src`](/en/reference/configuration-reference/#srcdir) for example.
:::
