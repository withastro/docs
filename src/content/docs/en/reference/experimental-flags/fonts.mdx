...
1. Register a provider for your font. You can choose from the list of [built-in remote providers](#available-remote-font-providers), build your own [custom font provider](#font-provider), or use the [local provider](#local-font-families) to register local font files.
...
More configuration options, such as defining fallback font families, are available and some will depend on your provider. See the full [configuration reference](#reference) to learn more.
...
See the full [`<Font>` component reference](#font--component) for more information.
...
Remote font families accept an [Astro font provider](#font-provider). Under the hood, options are handled by [unifont](https://github.com/unjs/unifont/).
