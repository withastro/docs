...
The `fontData` object is intended for retrieving lower-level font family data programmatically, for example, in an [API Route](/en/guides/endpoints/#server-endpoints-api-routes) or to generate your own meta tags.

### `fontData`
<p>

**Type:** `Record<CssVariable, FontData>`<br />
<Since v="5.16.10" />
</p>

Contains an array of `FontData` objects by [`cssVariable`](#cssvariable-1), which contains `src`, `weight` and `style`.
