...
### Unifont compatibility

[unifont](https://github.com/unjs/unifont) types are not directly compatible with Astro's `FontProvider` type.

However, you can easily author an Astro font provider using a unifont provider under the hood. For example for unifont's Google provider:

```ts
import type { FontProvider } from "astro";
import { type GoogleOptions, type InitializedProvider, providers } from 'unifont';
