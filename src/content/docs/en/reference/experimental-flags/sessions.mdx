...
You can also pass any available options to the unstorage driver in a separate `session.options` object.
...
An optional default time-to-live for session values, in seconds. This determines how long the session data remains valid before it is automatically expired. This can be overridden by passing a `ttl` option to [`session.set`](#sessionset). 
