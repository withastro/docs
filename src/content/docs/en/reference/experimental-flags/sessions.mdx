...
Sessions require a storage driver to store the session data. The Node and Netlify adapters automatically configure a default driver for you, but other adapters currently require you to specify a driver manually. You can use [any supported driver from unstorage](https://unstorage.unjs.io/drivers/).

### Configuring a session driver

If you are using an adapter that doesn't have a default driver, or if you want to choose a different driver, you can configure it using the `session` configuration option. Choose the [unstorage `driver` name](https://unstorage.unjs.io/drivers/) that corresponds to the storage feature provided by your hosting platform, such as the [Cloudflare KV driver](https://unstorage.unjs.io/drivers/cloudflare) or the [Deno KV driver](https://unstorage.unjs.io/drivers/deno). You can also use a cross-platform driver such as [Upstash](https://unstorage.unjs.io/drivers/upstash) or [Redis](https://unstorage.unjs.io/drivers/redis).
...
<ReadMore>
  See [the unstorage driver docs](https://unstorage.unjs.io/drivers) for more information.
</ReadMore>

:::

Set the driver name in the `session.driver` property. You can find the driver name in the unstorage documentation for the driver you want to use.
...
You can also pass any available options to the unstorage driver in a separate `session.options` object. See the driver documentation for available options. For example, to set the base key for sessions in Upstash:
...
An optional default time-to-live for session values, in seconds. 

By default, session values persist until they are deleted or the session is destroyed. If you would prefer the values to expire after a certain period, you can set a `ttl` option. You can set this global default `session.ttl` option, or you can pass a `ttl` option to [`session.set()`](#sessionset). A value passed to `session.set()` will override the global default for that entry.
...
:::note
Setting a value for `ttl` does not delete the value from storage unless it is accessed after the time has passed. In that case it will be treated as if it does not exist and the value will be deleted. 

Individual drivers may also support a `ttl` option that will automatically delete sessions after the specified time. See the driver documentation for more information.
:::
