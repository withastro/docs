...
Sessions rely on a [configurable session `driver`](#enabling-experimental-sessions) provided by your [server adapter](/en/guides/on-demand-rendering/#server-adapters) to store data on the `session` object. A [session cookie](#cookies) generates and stores an identifying session ID.

Included [helper functions](#sessions-api) allow you to interact with the stored user state (e.g. add items to a shopping cart) and the session ID (e.g. delete the session ID cookie when logging out).
...
The following sections show the recommended drivers for each adapter. (These will be automatically configured for you in the stable release of this feature.) However, you can instead specify [any supported driver Unstorage](https://unstorage.unjs.io/drivers/). You can also use a cross-platform driver such as [Upstash](https://unstorage.unjs.io/drivers/upstash) or [Redis](https://unstorage.unjs.io/drivers/redis).
...
Choose the [unstorage `driver` name](https://github.com/unjs/unstorage/blob/main/src/index.ts#L6-L34) that corresponds to the storage feature provided by your hosting platform, such as the [Netlify Blobs driver (`netlifyBlobs`)](https://unstorage.unjs.io/drivers/netlify), the [Cloudflare KV driver (`cloudflareKVBinding`)](https://unstorage.unjs.io/drivers/cloudflare) or the [Deno KV driver (`denoKv`)](https://unstorage.unjs.io/drivers/deno).

:::note
Some drivers may need extra packages to be installed. For example, the `netlifyBlobs` driver requires the `@netlify/blobs` package, and Upstash requires the `@upstash/redis` package. Some drivers may also require environment variables or credentials to be set.
...
You can also pass any available options to the Unstorage driver in a separate `session.objects` object. The following example configures the [Netlify Blobs](https://unstorage.unjs.io/drivers/netlify) driver while also providing a `name` and specifying a `consistency` level:
