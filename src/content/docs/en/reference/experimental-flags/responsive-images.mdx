...
The concept of [responsive images](https://developer.mozilla.org/en-US/docs/Web/HTML/Guides/Responsive_images) means images that work well on different devices. This particularly refers to images that resize to fit their container, and that can be served in different sizes depending on the device's screen size and resolution. There are a number of additional properties that can be set to control how the image is displayed, but these can be complicated to handle manually. Poor handling of these properties can lead to images that are too large for the device, or that are not displayed correctly, and are one of the most common causes of poor core web vitals scores.
...
:::note
Enabling responsive images will generate additional image sizes for all affected images. For prerendered pages this happens during the build so may increase the build time of your project, especially if you have a large number of images. For pages rendered on-demand the images are generated as-needed, so this has no impact on build times but may increase the number of transformations performed. Depending on your image service this may incur additional costs. 
:::
...
- `layout`: The layout type for the image. Can be `constrained`, `fixed`, `full-width`, or `none`. Defaults to the value of [`image.experimentalLayout`](/en/reference/configuration-reference/#imageexperimentallayout).
