...
The server submodule allows you to access non-exhaustive set of your configuration values from `astro.config.mjs`. This includes `astro:config/client` values such as `trailingSlash` and `i18n`, but also more sensitive information about your file system configuration that is not safe to expose to the client such as `srcDir`, `cacheDir`, `outDir`.
