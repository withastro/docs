...
You can also insert the same resource, multiple times, and Astro will remove the duplicates, and merge the new ones.

In this example, the directives `img-src https://global.cdn.example.org` and `default-src https://global.cdn.example.org`
are bound to be applied to all pages.

Inside `pages/index.astro`, using the [Astro.csp.insertDirective](#cspinsertdirective), we add `img-src https://vendor.cdn.example.org` and `default-src https://global.cdn.example.org https://vendor.cdn.example.org`. 
...
After the build, the resources of the directives `image-src` and `default-src` for the `index.html` page are merged and deduplicated (no duplicates). 
