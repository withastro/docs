...
You can customize the `<meta>` element per page via runtime APIs available from the `Astro.csp` global inside `.astro` components, or the `APIContext.csp` type in endpoints and middleware.
