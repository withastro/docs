...
Setting `multipass` to true will run the optimization engine at least twice for each SVG until no further optimizations are performed. This can result in smaller file sizes at the expense of longer build times and may cause unwanted side-effects depending on how your plugins are configured.
