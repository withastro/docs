...
The `floatPrecision` parameter will be passed to all the many plugins that use it to control the number of decimal places to preserve. It can be overridden by passing a value directly to a plugin.
...
Control the precision of numeric values in path data (see [other configuration options](#other-configuration-options) above):
