...
Processes the SVG contents. The optional `path` argument is the file path of the SVG being optimized, which optimizers can use to derive per-file values. For example, SVGO's `prefixIds` plugin uses it to generate a unique ID prefix for each file.
