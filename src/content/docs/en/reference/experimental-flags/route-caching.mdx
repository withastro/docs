...
Astro's first-party adapters for Cloudflare, Netlify, and Vercel each provide a CDN cache provider that maps cache directives to the platform's native cache headers and invalidation API. Use the provider that matches your deployment target.

Each provider automatically tags cached responses with the request path so that `cache.invalidate({ path })` works on platforms that only support tag-based purges.
...
<Since pkg="@astrojs/netlify" v="7.1.0" />
</p>
...
<Since pkg="@astrojs/vercel" v="10.1.0" />
</p>
