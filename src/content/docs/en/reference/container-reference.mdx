...
A list of client renderers required by the component. Use this if your `.astro` component renders any [UI framework components](/en/guides/framework-components/) using an official Astro integration (e.g. React, Vue, etc.). For each framework rendered, you must use function `getContainerRenderer` that can be imported from the renderer library.

When using `vite` (`vitest`, Astro integrations, etc.), the renderers should be loaded by using the function `loadRenderers`, from the virtual module `astro:container`. Outside `vite`, you'll have to load the renderers manually.
...
:::note
The function `getContainerRenderer` is only available for renderers inside the `@astrojs/` npm org. Renderers outside this org will provide a similar/same function.
:::
