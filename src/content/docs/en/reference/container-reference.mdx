...
It also allows you to [manually load rendering scripts](#adding-a-server-renderer-manually) for creating containers in pages rendered on demand or other "shell" environments outside of `vite` (e.g. inside a PHP or Elixir application).
...
#### Adding a server renderer manually
...
#### Adding a client renderer manually

A client renderer is usually needed to hydrate client-side components, and it's used when using the `client:*` directives. When rendering these Astro components using a Container instance, you **must** provide the client renderer:

The following example adds server and client renderer for React components:
...
:::caution
The `addClientRenderer` function will throw an error if no **server** renderer was stored in the container instance.  
:::
