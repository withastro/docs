...
If you want to use the container API in other ways, such as on-demand pages or other "shells" e.g. inside a PHP application, or a Elixir application, you should rely on a manual approach and **not** use the `astro:container` virtual module.

In the following example, we manually import the server renderers for React and Vue components, and MDX for `.mdx` pages. 
