...
An option to pass [`properties`](en/basics/astro-components/#props) for astro components.
...
> Note: The type-hints are not automatically recognised, so you would need to share the
> interface outside of `.astro` files in order to be available for type-hints:

```js name="Card.test.js"
import Card from "../src/components/Card.astro";
import { CardProps } from "../src/components/types.ts";

const result = await container.renderToString(Card, { 
  props: { name: "Hello, world!"} satisfies CardProps,
});
```
