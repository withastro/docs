...
By default, the Container API renders components as they were [page partials](/en/basics/astro-pages/#page-partials). This is usually the behavior you want when rendering components.boolean

However, if you're rendering full-fledged Astro pages, you can opt-out of this behavior by setting `partial` to `false`:
...
console.log(result) // it will contain `<!DOCTYPE html>` at the beginning of the HTML
```
