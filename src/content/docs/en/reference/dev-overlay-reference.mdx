...
This event is fired when the user clicks on the plugin icon in the UI. The new [`state`](#state-boolean) of the plugin is included in the event in the `detail` property.
...
Indicates whether or not the the plugin is enabled. When `true`, the plugin icon will be highlighted using a red dot. Conversely, when `false`, the highlight will be removed.
...
`windowTitle` is a string that will be shown at the top of the overlay window. `windowIcon` can either be a string of a SVG file or an icon from [the icon list](#icons).
...
Shows a card with an [`icon`](#icons). Optionally, if a `link` is passed, the card will be clickable and will open the link in a new tab.
