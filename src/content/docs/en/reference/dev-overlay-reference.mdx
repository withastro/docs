...
The Astro Dev Overlay Plugin API allows you to create plugins that can be used to extend the Astro Dev Overlay with new functionalities and integrations with third-party services.
...
A plugin is a file that default exports an object with the following properties:
...
This event is fired when the user clicks on the plugin icon in the UI. The new state of the plugin is included in the event in the `detail` property.
...
`state` can be either `true` or `false`. When `true`, the plugin icon will be highlighted using a red dot. Conversely, when `false`, the highlight will be removed.
...
In addition to being able to send and receive arbritary messages, the dev overlay also sends the following messages, where `PLUGIN_ID` is the ID of a plugin:
...
The dev overlay includes a set of webcomponents that can be used to build plugins with a consistent look and feel.

### `astro-dev-overlay-window`

Shows a window with optionally a title and an icon. `windowIcon` can either be a string of a SVG file or an icon from [the icon list](/en/reference/dev-overlay-plugin-api/#icons).

The slot of the component will be used as the content of the window.

```html
<astro-dev-overlay-window windowTitle="MyWindows" windowIcon="astro:logo">
  <p>My content</p>
</astro-dev-overlay-window>
```
...
Shows a card with a icon. Optionally, if `link` is passed, the card will be clickable and will open the link in a new tab.
...
Sections are defined using the `sections` property. This property is an array of objects with the following shadpw:
...
```ts
{
	...
	clickAction?: () => void; // Action to perform when clicking on the section
...
Alternatively to those icons, you can also pass a string containing the SVG markup of the icon you want to use.
