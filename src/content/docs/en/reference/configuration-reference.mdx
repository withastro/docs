...
Whether Astro should follow HTTP redirects when fetching remote images. When `false`, remote image requests that return a redirect response are treated as failures. When `true`, Astro follows redirects before reading or transforming the image. This applies to remote image fetching in `Image`, `Picture`, `getImage()`, and `inferRemoteSize()`.
