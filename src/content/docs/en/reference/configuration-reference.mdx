...
Describes the environment variables to use in your project, where each key corresponds to the variable name and the value to the data type and validations [defined with `envField`](/en/reference/modules/astro-config/#envfield).

Four data types are supported: string, number, enumeration, and boolean.Each type requires a `context` (client or server), an `access` level (public or secret), and additional validations, such as a `default` value and an indication of whether the variable is `optional` or not (defaults to `false`).
