...
Configures the Fonts API, which allows you to use fonts from your filesystem and various font providers through a unified, fully customizable, and type-safe API.

All properties of your fonts must be configured in the Astro config. Set these to customize the data loaded from your [font provider](#fontprovider), for example, to only download certain font [`weights`](#fontweights) or [`styles`](#fontstyles).
...
[`provider`](#fontprovider), [`name`](#fontname), and [`cssVariable`](#fontcssvariable) are required.

TODO: granular font configuration heading
...
See our guide for more information on the [Fonts API](/en/guides/fonts/#fonts-api).
