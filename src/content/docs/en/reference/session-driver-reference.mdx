...
A session driver config is a function that accepts an optional [config](#config) and returns a `SessionDriverConfig` object, containing the aforementioned config and the runtime [entrypoint](#entrypoint).
...
A session driver implementation is a module that returns a default function. It accepts the driver [config](#config), and returns a `SessionDriver`. It is responsible for [storing](#setitem), [retrieving](#getitem) and [deleting][#removeitem] data when using session methods such as `context.session.set()`.
