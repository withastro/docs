...
## Toolbar app integration setup
 ...
 Integrations can add apps to the dev toolbar in [the `astro:config:setup` hook](/en/reference/integrations-reference/#astroconfigsetup). The `addDevToolbarApp()` method takes an object with the following required properties to define the toolbar app: `id`, `name`, `icon`, and `entrypoint`.
...
The icon of the app. This will be used to display the app in the toolbar. This can either be an icon from [the icon list](#icons), or a string containing the SVG markup of the icon.
...
A Dev Toolbar App is a `.js` or `.ts` file that default exports an object with the `init` property (required) and an optional `beforeTogglingOff()` function.
...
In your integration's `astro:server:setup` hook, use the `toolbar` object to send and receive messages to and from your apps.
