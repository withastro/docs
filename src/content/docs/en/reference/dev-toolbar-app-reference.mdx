...
`window-title` is a string that will be shown at the top of the toolbar window. `window-icon` can either be a string of a SVG file or an icon from [the icon list](#icons).

The slot of the component will be used as the content of the window.

```html
<astro-dev-toolbar-window window-title="MyWindows" window-icon="astro:logo">
  <p>My content</p>
</astro-dev-toolbar-window>
```
...
```html
<astro-dev-toolbar-card icon="<svg>...</svg>" link="https://docs.astro.build">Read more in the Astro Docs!</astro-dev-toolbar-card>
```
