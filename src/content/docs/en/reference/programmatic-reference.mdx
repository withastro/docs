...
If you need more control when running Astro, the `"astro"` package exports APIs to programmatically run the CLI commands. There are also two [`astro:config`](/en/reference/modules/astro-config/) helpers that can programmatically verify and merge configurations.
