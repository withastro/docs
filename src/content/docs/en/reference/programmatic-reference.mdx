...
`mergeConfig()` accepts an Astro config object and a partial config (any portion of a config, everything is optional), and returns a valid Astro config combining both values such that:
