...
- General options are overriden.
- Integrations and remark plugins are appended to the existing list.
- Vite options are merged using [Vite's own `mergeConfig` function](https://vite.dev/guide/api-javascript#mergeconfig) with default `isRoot` flag.
- Lazy options passed as functions are wrapped into new functions that recursively merge the return values from both configurations with these same rules.
