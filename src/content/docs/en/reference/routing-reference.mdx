...
Every [supported page file](/en/basics/astro-pages/#supported-page-files) located within the special `src/pages/` directory creates a single page route. When the file name contains a [parameter](#params), a route can create multiple pages dynamically, otherwise it creates a single page.

By default, all Astro page routes and endpoints are generated and prerendered at build time. [On-demand server renderdering](/en/guides/on-demand-rendering/) can be set for individual routes, or as the default.
...
A value exported from each individual page route to determine whether or not the page is prerendered.

By default, all page routes and endpoints will implicitly export a value of `true` and will be statically generated at build time. You can opt out of prerendering on one or more routes, and you can have both static and on-demand rendered routes in the same project.

### Per-page override

You can override this value to enable [on demand rendering](/en/guides/on-demand-rendering/) for an individual page or route by exporting the value `false` on any page file that can export values (e.g. `.astro`, `.mdx`, `.js`, etc.)

```astro title="src/pages/rendered-on-demand.astro"
---
...
You can override this value on all pages by default by configuring [`output: 'server'`](/en/reference/configuration-reference/#output). With this output mode, all routes and endpoints will implicitly export a value of `false` and all pages will be generated on the server upon request.

You can then override this value to enable prerendering for an individual page or route by exporting the value `true`:

```astro title="src/pages/static-about-page.astro"
---
...
A function to generate mulitple, prerendered page routes from a single `.astro` page component with a [parameter](#params) in its file name. Use this for routes that will be created at build time, also known as static site building.
...
The keys in `params` must match the parameters defined in your component filepath. The value for each `params` object must match the parameters used in the page name. `params` are encoded into the URL, so only strings are supported as values.
