...
A [`logger`](/en/reference/logger-reference/#astroruntimelogger) object is passed as a fourth argument. Use it instead of `console` to write messages to the destination configured in your [`logger` config](/en/reference/configuration-reference/#logger-options) and to respect your configured log level:

```ts ins="logger" ins=/logger\.warn\(.*\);/
