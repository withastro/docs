...
<p>
  **Type:** `(src: ImageMetadata | string): src is ImageMetadata`<br />
  <Since v="4.0.0" />
</p>
...
<p>
  **Type:** `(src: ImageMetadata | string): src is string`<br />
  <Since v="4.0.0" />
</p>
...
// Example usage of resolveSrc with a local image
const localImage = {
  src: '/images/photo.jpg',
  width: 800,
  height: 600,
...
const remoteImage = 'https://example.com/images/photo.jpg';

async function exampleResolveSrc() {
  // Resolve a local image
  const resolvedLocal = await resolveSrc(localImage);
  console.log(`Resolved local image source:`, resolvedLocal);
  
  // Resolve a remote image URL
  const resolvedRemote = await resolveSrc(remoteImage);
  console.log(`Resolved remote image source:`, resolvedRemote);
  }

await exampleResolveSrc();
```
...
async function processImage() {
  ...
  // DEPRECATED!!
  const watchMode = false; // Set to true in development mode
  const experimentalSvgEnabled = true; // Experimental SVG support
  
  try {
    const result = await emitESMImage(imageId, watchMode, experimentalSvgEnabled);
    if (result) {
...
Extracts the original image query parameters (width, height, format) from the given `URLSearchParams` object and returns them as an object. If any of the required parameters are missing or invalid, the function returns undefined.
