...
Services definitions take the shape of an exported default object with various required methods ("hooks"). 

Local services provide a `transform()` method to perform transformations on your image and  `getURL()` and `parseURL()` methods to use an endpoint for dev mode and SSR. External services provide a `getURL()` that points to the `src` of the output `<img>` tag. Both types of services can provide `getHTMLAttributes()` to determine the other attributes of the output `<img>`.

### External Services

An external service points to a remote URL to be used as the `src` attribute of the final `<img>` tag. This remote endpoint should download, transform, and return the image.
