...
Local services provide a `transform()` method to perform transformations on your image and  `getURL()` and `parseURL()` methods to use an endpoint for dev mode and SSR. External services provide a `getURL()` that points to the `src` of the output `<img>` tag. Both types of services can provide `getHTMLAttributes()` to determine the attributes of the output `<img>`.
