...
When writing a CDN provider, Astro provides [shared utilities](#cache-provider-utilities) for building cache-control headers and path-based invalidation tags.
...
Generates a cache tag for a given path (`astro-path:{path}`). Used to support [`invalidate({ path })`](/en/reference/api-reference/#cacheinvalidate) on platforms that only offer tag-based purging.
...
Indicates the most recent modification date. When multiple calls to [`cache.set()`](/en/reference/api-reference/#cacheset) provide `lastModified`, the call providing the most recent date takes precedence.
...
The `context` also includes a [`logger`](/en/reference/logger-reference/#astroruntimelogger) object. Use it instead of `console` to write messages to the destination configured in your [`logger` config](/en/reference/configuration-reference/#logger-options) and to respect your configured log level:

```ts title="my-provider/runtime.ts" ins="logger" ins={7}
import type { CacheProviderFactory } from 'astro';
