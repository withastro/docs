...
Indicates the most recent modification date. When multiple calls to [`cache.set()`](/en/reference/api-reference/#cacheset) provide `lastModified`, the most recent date takes precedence, even when it is provided by an earlier call.
