...
These directives control how server components are rendered.

### `server:defer`

The `server:defer` directive transforms the component as a Server island, causing it to be rendered on demand, outside the scope of the rest of the page rendering.

<ReadMore>See the [Server islands](/en/guides/server-islands/) page for more details on Server islands.</ReadMore>
