...
Since Astro 2.5.0, integrations can also add custom client directives through the [experimental `addClientDirective` API](/en/reference/integrations-reference/#addclientdirective-option). You can use custom `client:*` directives to change how and when components should be hydrated.
