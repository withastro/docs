...
Astro provides two built-in loaders to help you fetch your collections without the need to create your own loader. Both offer options to suit a wide range of use cases.
...
The `glob()` loader creates entries from directories of files from anywhere on the filesystem. The supported file types are Markdown, MDX, Markdoc, and JSON files. Use this loader when you have one file per entry by providing an options object with a required `pattern` property and, optionally, a `base` and/or `generateId` property.
...
Accepts a pattern of entry files to match using glob patterns supported by [micromatch](https://github.com/micromatch/micromatch#matching-features). The pattern must be relative to the base directory.
...
Defines the base directory to resolve the glob pattern from. It must be relative to the root directory, or an absolute file URL.
...
Returns a unique ID per collection for an entry. By default it generates a kebab-case slug from the entry path using [`github-slugger`](https://github.com/Flet/github-slugger).
...
The `file()` loader creates entries from a single file. The file must contain an array of objects that contain a unique `id` field, or an object with string keys. It supports JSON or YAML by default and you can provide a custom `parser` for data files it cannot parse automatically.
...
Sets the path to the file to load, relative to the content directory.
...
Provides additional options for the file loader. It is useful if you need a custom `parser` to process your file (e.g. `.toml` or `.csv`).
...
Allows you to specify a custom parser to create a collection from the contents of a file.
