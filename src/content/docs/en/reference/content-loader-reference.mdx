...
**Type:** `TError | LiveCollectionError`
</p>

The error returned if the loader failed to load the collection. This can be a custom error extending the `Error` class or a built-in error.

#### `cacheHint`

See [`CacheHint`](#cachehint-1).
...
The error returned if the loader failed to load the entry. This can be a custom error extending the `Error` class or a built-in error.

#### `cacheHint`

See [`CacheHint`](#cachehint).
