...
Astro loaders are objects or functions that allow you to load data into [content collections](/en/guides/content-collections/), which can then be used in pages and components. The [built-in `glob()` and `file()` loaders](#built-in-loaders) are used at build time to load content from the file system. You can also create your own [build-time loaders](/en/guides/content-collections/#custom-build-time-loaders) or [live loaders](/en/guides/content-collections/#creating-a-live-loader) to load content from other sources.

Each collection needs to define a loader as its data source. For build-time collections, you can define a loader inline in your project's `src/content.config.ts` file or share an object loader between multiple collections. For live collections, you'll need to create your own live loader and import it in your `src/live.config.ts` file. In both cases, you can even [publish your loader to NPM as a package](/en/reference/publish-to-npm/) to share with others and be included in our integrations library.
...
<ReadMore>Learn more about [using built-time loaders](/en/guides/content-collections/#defining-build-time-content-collections) with guided explanations and example usage.</ReadMore>
