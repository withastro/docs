...
Renders a Markdown string to HTML, returning a `RenderedContent` object. This allows the entry to be rendered in a page using the [`render()` function and `<Content />` component](/en/guides/content-collections/#rendering-body-content). To enable this, assign the rendered content to the [`rendered`](#rendered) field of the [`DataEntry`](#dataentry) object.

```ts title=loader.ts {21-22}
import type { Loader } from "astro/loaders";
import { loadFeed } from "./feed.js";
export function feedLoader({ url }): Loader {
  const feedUrl = new URL(url);
