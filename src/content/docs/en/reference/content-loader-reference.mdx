...
You can then use these hints in your pages. If you have [configured a cache provider](/en/guides/caching/#configure-caching), pass cache hints directly to [`Astro.cache.set()`](/en/reference/api-reference/#cacheset):
...
:::note
Cache hints do not automatically cause the response to be cached by Astro. They provide values you can pass to [route caching](/en/guides/caching/) or use to implement your own caching strategy.
:::
...
```ts title="Extract from the file() loader" {15} "logger"
import type { Loader } from "astro/loaders";
...
```ts title="Extract from the file() loader" {9} "config"
import type { Loader } from "astro/loaders";
import { fileURLToPath } from "node:url";
import { syncData } from "./helpers";
...
```ts title="Extract from the file() loader" {13-18}
import type { Loader } from "astro/loaders";
import { fileURLToPath } from "node:url";
import { syncData } from "./helpers";
...
Describes the raw frontmatter, parsed from the file. This may include [programmatically injected data from Markdown processor plugins](/en/guides/markdown-content/#modifying-frontmatter-programmatically).
