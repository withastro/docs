...
In static builds, this will be the `params` returned by `getStaticPaths()` used for prerendering [dynamic routes](/en/basics/routing/#dynamic-routes).
...
[Endpoint functions](/en/basics/endpoints/) receive a context object as the first parameter. It mirrors many of the `Astro` global properties.
...
It can also be used in static file endpoints for [dynamic routing](/en/basics/endpoints/#params-and-dynamic-routing).
...
The `getImage()` function is intended for generating images destined to be used somewhere else than directly in HTML, for example in an [API Route](/en/basics/endpoints/#server-endpoints-api-routes). It also allows you to create your own custom `<Image />` component.
