...
<Tabs>
  ...
  <TabItem label="context.props">
    ```ts title="src/pages/posts/[id].json.ts" "props:" {11-14}
...
`params` is an object containing the values of dynamic route segments matched for a route request. Its keys must match the [dynamic route parameters](/en/guides/routing/#dynamic-routes) from the the page file or endpoint route.
...
<Tabs>
  ...
  <TabItem label="context.params">
    ```ts title="src/pages/posts/[id].json.ts" "params:" {11-14}
    import type { APIContext } from 'astro';
...
When pages are rendered on demand, `params` can be any value matching the path segments in the dynamic route pattern.
...
`url` is a [URL](https://developer.mozilla.org/en-US/docs/Web/API/URL) object constructed from the current `request.url` URL string value. It is useful for interacting with individual properties of the request URL, like pathname and origin. 

`Astro.url` is equivalent to doing `new URL(Astro.request.url)`.

`url` will be `localhost` in dev mode for all on-demand rendered routes, and for prerendered routes when [`site`](/en/reference/configuration-reference/#site) is not configured.
...
<Tabs>
  ...
  <TabItem label="context.clientAddress">
    ```ts title="src/pages/ip-address.ts"
    export const prerender = false; // Not needed in 'server' mode
    import type { APIContext } from 'astro';
...
`preferredLocale` is a computed value to find the best match between your visitor's brower language preferences and the locales supported by your site.

It is computed by checking the configured locales in your `i18n.locales` array and the locales supported by the users's browser via the header `Accept-Language`. This value is `undefined` if no such match exists.
...
The type returned from getting a cookie via `Astro.cookies.get()`. It has the following shape:
