...
Astro will check if the URL you want rewrite exists, and if it doesn't, it will return a [`404` status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/404). Instead, if the URL you want to rewrite emits a runtime error, Astro will:
- show the overlay error in **development**;
- return a [`500` status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/500) in production.

You can also rewrite the `/404` page on purpose. This can be convenient in case you're building an e-commerce, a user attempts to view an item that is out of stock and won't be restock anymore:
...
Vice-versa, you can rewrite to a page that exists even from a page that doesn't exist:
...
The status code returned by the request will change based on the result of the rewrite. For example, if you call `context.rewrite("/foo/bar")`, but `/foo/bar` doesn't exist even though you have a dynamic route, Astro will return a `404` status code. 

You can take advantage of this logic by conditionally return different status code. For example, for a certain user can't access certain routes, you can call `context.rewrite("/404")` and Astro will render the content of the 404 page (built-in or custom). 
...
It accepts, optionally, a string, `URL`, or `Request` to trigger a [rewrite](/en/guides/routing/#rewrites) of the current request.  
