...
`params` is an object containing the values of dynamic route segments matched for a route request. Its keys must match the [dynamic route parameters](/en/guides/routing/#dynamic-routes) from the page file or endpoint route.
...
<Tabs>
  ...
  <TabItem label="context.locals">
    ```ts title="src/pages/hello.ts" "Astro.locals"
    import type { APIContext } from 'astro';
