...
`context.locals` is an object used to store & access arbitrary information during the lifecycle of a request.
Only middleware functions may set the values of `context.locals`.
...
API endpoints can read information from `context.locals`.
