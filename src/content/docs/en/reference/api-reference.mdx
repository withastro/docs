...
Normalize a path, such as `/foo/` to a parameter to use as a param for dynamic routes. Astro does not perform any conversion on params, so when taking a path from, for example, a database, and converting it into a param might result in unexpect results. `pathToParam` does normalization you might otherwise do yourself, removing leading and trailing slashes:

```astro
---
export function getStaticPaths({ pathToParam }) {
  return [
    { params: { slug: pathToParam('/foo') } },
    { params: { slug: pathToParam('bar/') } },
    { params: { slug: pathToParam('/baz/') } },
  ];
}
---
```
