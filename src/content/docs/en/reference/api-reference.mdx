...
The `getSecret()` helper function allows retrieving the raw value of an environment variable by its key. It has 2 usecases:
...
If you need to retrieve environment variables programmatically, we recommend you use `getSecret()` instead of `process.env` (or equivalent) because its implementation is provided by your adapter: It means you won't need to update all your calls if you switch adapters. It defaults to `process.env` in dev and build.
