...
`Astro.locals` is an API available via [Astro middleware](/en/guildes/middleware). It's an object that can contain any arbitrary information.

It's possible to type the information inside `Astro.locals` using the `env.d.ts` file:
...
Doing so, you will get autocompletion inside `.astro` files and middleware code.

This is complementary API of [`context.locals`](#context-locals)
...
This is complementary API of [`Astro.locals`](#astro-locals)
