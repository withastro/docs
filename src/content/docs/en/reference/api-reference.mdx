...
- You don't want to load each file immediately. `import.meta.glob()` can return functions that import the file content, rather than returning the content itself. (Regardless of using imported files on the page, all styles and scripts used in that files will still be bundled and added to the page, as this is decided by static analysis, not at runtime)
