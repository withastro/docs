...
Allows you to rewrite a request at runtime. The method accepts a string, a `URL` or a `Request`.

Use a string when you want to provide a plain path:
...
Use a `URL` type to have finer control over the path to rewrite. In the following example, we use `"../"` to easily render the parent path: 

```astro title="src/pages/blog/index.astro"
---
return Astro.redirect(new URL("../", Astro.url))
---
```

Use a `Request` type to have _total_ control over the path to rewrite. This is useful in case you want to provide the necessary headers to the route you're about to rewrite:

```astro title="src/pages/blog/index.astro"
---
return Astro.redirect(new Request(new URL("../", Astro.url), {
  headers: {
