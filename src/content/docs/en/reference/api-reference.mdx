...
The current route pattern being rendered. The value will look like a file path, without the [`srcDir`](/en/reference/configuration-reference/#srcdir) and the `pages` folder. The path will start with a leading slash, and it won't have the file extension.

For example, if the current route is `src/pages/en/blog/[slug].astro`, then `Astro.routePattern` will be `/en/blog/[slug]`.

You can use this property to understand which route is rendering your component and make decisions based on it. For example, you can use it to conditionally render certain information, or collect some metrics about which routes are slower.
