...
### `Astro.redirect()`
In [server-side rendering](/en/guides/server-side-rendering/) mode, `Astro.redirect()` allows you to redirect to another page. 
...
:::tip
This means collections **cannot** store a mix of content and data formats. If you find the need, Astro recommends splitting these entries into separate collections with a shared [schema](#schema).
:::
...
`getEntry()` is a function that retrieves a single collection entry by collection name and either the entry `id` (for JSON or YAML collections) or entry `slug` (for Markdown or MDX collections).
...
// Get `src/content/captains/captain-kirk.yaml`
const captainProfile = await getEntry('captains', 'captain-kirk');
---
```

`getEntry()` can also be used to get referenced entries to access the `data`, `body`, or `render()` properties:
...
const captainProfile = await getEntry(enterprise.data.captain);
---
```

See the `Content Collection` guide for examples of [querying](/en/guides/content-collections/#querying-collections) and [references](/en/guides/content-collections/#resolving-references).
...
`getEntries()` is a function that retrieves multiple collection entries with the same collection name. This is useful for resolving an array of [references](en/guides/content-collections/#defining-collection-references) to their associated `data`, `body`, and `render()` properties.
...
:::note
Astro recommends using the new `getEntry()` function. This accepts the same arguments as `getEntryBySlug()`, and supports querying by `id` for JSON or YAML collections.
:::
...
Query functions including [`getCollection()`](#getcollection) and [`getEntry()`](#getentry) each return entries with the `CollectionEntry` type. This type is available as a utility from `astro:content`:
...
**Example Type for "content" collections:** `'entry-1.md' | 'entry-2.md' | ...`  
**Example Type for "data" collections:** `'author-1' | 'author-2' | ...`
...
The name of the entry's collection.
...
The following properties are only available for collections [defined as `type: 'content'`](#type):
