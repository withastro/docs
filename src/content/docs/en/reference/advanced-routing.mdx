...
`src/app.ts` is a custom request pipeline entrypoint, giving you full control over how Astro handles incoming requests.
...
Create a `src/app.ts` (or `.js`, `.mjs`, `.mts`) file that default-exports an object with a `fetch` method. The `fetch` method receives a standard [Request](https://developer.mozilla.org/en-US/docs/Web/API/Request) and must return a [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response).

If no `src/app.ts` file exists, Astro uses its built-in pipeline, which runs all features automatically.
...
By default, Astro looks for `src/app.ts` as the advanced routing entrypoint. Set the `fetchFile` option to use a different file:
