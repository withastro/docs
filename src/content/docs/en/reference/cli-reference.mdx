...
Starts a local server to serve the contents of your static `dist/` directory.

This command is useful for previewing your [build](#astro-build) locally, before deploying it. It is not designed to be run in production. For help with production hosting, check out our guide on [Deploying an Astro Website](/en/guides/deploy/).
