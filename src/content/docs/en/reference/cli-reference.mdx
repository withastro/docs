...
For example, use the `astro check` command with your package manager to run diagnostics:
...
Use the following `astro --help` command in your terminal to display a list of all available commands:
...
You can also use scripts in `package.json` for shorter versions of these commands. Scripts also allow you to use the same commands that you may be familiar with from other projects, such as `npm run build`.

The following scripts for the most common commands (`astro dev`, `astro build`, and `astro preview`) are added for you automatically when you create a project using [the `create astro` wizard](/en/install/auto/#1-run-the-setup-wizard).
...
You will often use these `astro` commands, or scripts that run them, without any flags. Use flags when you want to customize the command's behavior. For example, you may wish to start the development server on a different port, or build your site including draft pages. 
