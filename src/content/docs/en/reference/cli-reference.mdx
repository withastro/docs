...
Use the following `astro` command in your terminal to display a list of all available commands:
...
The following list will display in your terminal:
...
For example, to display your project's version of Astro, run the following command for your package manager:

<PackageManagerTabs>
  <Fragment slot="npm">
  ```shell
  # show your project's version of Astro
  npx astro --version
  ```
  </Fragment>
  <Fragment slot="pnpm">
...
You can also use scripts in `package.json` for a shorter version of these commands.

The following scripts for the most common commands (`astro dev`, `astro build`, and `astro preview`) are added for you automatically when you create a project using [the `create astro` wizard](/en/install/auto/#1-run-the-setup-wizard). When you follow the instructions to [install Astro manually](/en/install/manual/#2-install-astro), you are instructed to add these scripts yourself.
...
You will often use these common commands without any flags. But, for example, you may wish to start the development server on a different port, or build your site including draft pages. You can combine these scripts (which run `astro` commands) with an appropriate flag for more options:
