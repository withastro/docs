...
Starts the preview server as a detached background process and enables [JSON logging](#--json). This flag is provided automatically when an AI agent is detected. You can also use it manually:
...
<ReadMore>See [Background mode for AI coding agents](/en/guides/build-with-ai/#background-mode) for more about automatic agent detection.</ReadMore>
...
Stops a running background preview server. Sends `SIGTERM` and waits up to 5 seconds for the server to exit gracefully. If the process is still running after that, it escalates to `SIGKILL`.

#### `astro preview status`

Checks whether a background preview server is running and displays its URL, PID, and uptime.

#### `astro preview logs`

Displays logs from a background preview server. Only works when the server was started with `astro preview --background`, since foreground servers write logs directly to the terminal.
