...
Currently, `devToolbar` is the only available preference. The full list of [default preferences](https://github.com/withastro/astro/blob/main/packages/astro/src/preferences/defaults.ts) can be found in the Astro repository.

In the future, we plan to add more preferences to personalize your Astro experience.
