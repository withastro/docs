...
Astro DB is an official integration that adds a type-safe SQL schema and a libSQL
client to your project. See the [Astro DB guide](https://docs.astro.build/en/guides/astro-db/)
for setup and usage.

### Shared flags

These flags apply to multiple `astro db` commands:

#### `--remote`

Connect to a remote libSQL database instead of the local database file. Applies to: `execute`, `push`, `shell`, `verify`. Requires the `ASTRO_DB_REMOTE_URL` environmental variables to be set, and either `ASTRO_DB_APP_TOKEN` to be set in the environment a value passed with the `--db-app-token` command-line argument.

#### `--db-app-token <token>`

Provide the remote database app token directly instead of `ASTRO_DB_APP_TOKEN`. Applies to: `execute`, `push`, `verify`. Useful in CI/CD environments where you want to supply the token at runtime.

### `astro db push`

Pushes your local Astro DB schema to your remote libSQL database. Use this after updating `db/config.ts` to apply schema changes.

#### `--force-reset`

Resets the remote database schema from scratch. Destructive and can delete production data.

#### `--dry-run`

Prints the generated SQL statements without applying them.

### `astro db execute <file-path>`

Executes a JavaScript or TypeScript file with access to `astro:db`. Commonly used for one-off seeding and data migrations.

### `astro db shell --query <sql-string>`

Runs a SQL query against your local database (default) or remote database and prints the result.
...
Compares your local schema against the remote database and exits non-zero if changes are required or unsafe. This is useful in CI.

#### `--json`

Prints a machine-readable JSON result from `verify`.
