...
<Meta default="`undefined`">`string | false`</Meta>
...
<Meta default='`"development"` when running `astro dev`, `"production"` when running `astro build`'>
...
<Meta default='`"info"`'>`"debug" | "info" | "warn" | "error" | "silent"`</Meta>
...
<Meta>`(inlineConfig: AstroInlineConfig) => Promise<DevServer>`</Meta>
...
<Meta>`(inlineConfig: AstroInlineConfig) => Promise<void>`</Meta>
...
<Meta>`(inlineConfig: AstroInlineConfig) => Promise<PreviewServer>`</Meta>
