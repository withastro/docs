...
In the Build a Blog introductory tutorial, you learned about Astro's [built-in file-based routing](/en/core-concepts/routing/#static-routes): any `.astro`, `.md`, or `.mdx` file anywhere within the `src/pages/` folder automatically became a new page on your site.
...
1. Adding view transitions to my astro site...
    ...
    <MultipleChoice>
      ...
      <Option isCorrect>
        Changes the default type of page navigation on page that contains the `<ViewTransitions />` router in its `<head>`
      </Option>
      <Option>
