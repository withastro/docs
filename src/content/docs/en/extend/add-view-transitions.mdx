...
5. Update the script controlling the theme toggle.
    ...
    The `<script>` that controls the light/dark theme toggle is located in the `<ThemeIcon />` component. For the theme toggle to continue to function on every page, add the same event listener for the `astro:page-load` event to trigger your existing function:

    ```astro title="src/components/ThemeIcon.astro" ins={8,38}
    ---
    ---
