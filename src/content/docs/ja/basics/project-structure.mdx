...
`astro.config.js`、`astro.config.mjs`、`astro.config.ts`など、複数のJavaScript設定ファイルの形式をAstroはサポートしています。基本的に`.mjs`を使用することをおすすめしますが、TypeScriptで設定ファイルを書きたい場合は`.ts`を使用してください。
