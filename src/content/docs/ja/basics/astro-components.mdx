...
クライアントサイドでパーソナライズされた動的なコンテンツをレンダリングする必要があるコンポーネントの場合、[サーバーディレクティブ](/ja/reference/directives-reference/#server-directives)を追加することでサーバーレンダリングを遅延させることができます。これらの「サーバーアイランド」は、コンテンツが利用可能になった時点でレンダリングされ、ページ全体の読み込みを遅らせることはありません。
...
<ReadMore>[`Astro.slots` ユーティリティ関数](/en/reference/astro-syntax/#astroslots)を読んで、スロットコンテンツにアクセスしてレンダリングする方法をさらに学びましょう。</ReadMore>
