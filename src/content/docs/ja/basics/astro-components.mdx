...
### コンポーネントスクリプト \{#the-component-script\}
...
## コンポーネントベースの設計 \{#component-based-design\}
...
### スロットのフォールバックコンテンツ \{#fallback-content-for-slots\}
...
:::note
HTMLコンポーネント内の[`<slot />`要素](/ja/basics/astro-components/#slots)はAstroコンポーネントと同様に機能します。[HTML Web ComponentのSlot](https://developer.mozilla.org/ja/docs/Web/HTML/Element/slot)要素を使うためには,  `<slot>`要素に`is:inline`を追加します。
:::
