...
:::caution
ページ部分は、[htmx](https://htmx.org/) や [Unpoly](https://unpoly.com/) などのフロントエンド ライブラリと組み合わせて使用​​することを目的としています。低レベルのフロントエンド JavaScript の作成に慣れている場合は、これらを使用することもできます。 このため、これらは高度な機能です。

さらに、コンポーネントにスコープ指定されたスタイルまたはスクリプトが含まれている場合は、これらの要素が HTML 出力から削除されるため、パーシャルを使用しないでください。スコープ付きスタイルが必要な場合は、部分的ではない通常のページと、コンテンツを head にマージする方法を知っているフロントエンド ライブラリを使用することをお勧めします。
:::
...
ただし、これらは特別な `src/pages/` ディレクトリに配置されているため、生成された HTML はそのファイル パスに対応する URL で利用できます。これにより、レンダリング ライブラリ (htmx、Stimulus、jQuery など) がクライアント上でアクセスし、ブラウザの更新やページ ナビゲーションを行わずに HTML のセクションをページ上に動的に読み込むことができます。

パーシャルをレンダリング ライブラリと組み合わせると、Astro で動的コンテンツを構築するための[アイランド](/ja/concepts/islands/)や[`<script>` タグ](/ja/guides/client-side-scripts/)の代替手段が提供されます。
...
- import.meta.env を使用した環境変数 (`import.meta.env.USE_PARTIALS`など) 

### ライブラリと併用する

パーシャルは、[htmx](https://htmx.org/)などのライブラリを仕様してページのセクションを動的に更新するために仕様されます。

次の例は、`hx-post`部分の URL に設定された属性を示しています。パーシャルページのコンテンツは、このページ上のターゲットの HTML 要素を更新するために使用されます。
