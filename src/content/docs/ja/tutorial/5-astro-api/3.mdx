...
## `/pages/folder/index.astro`ルーティングパターンを使う \{#use-the-pagesfolderindexastro-routing-pattern\}
...
## やってみよう - タグインデックスページを作る \{#try-it-yourself---make-a-tag-index-page\}
...
## タグの配列を作成する \{#create-an-array-of-tags\}
...
## タグのリストを作成する \{#create-your-list-of-tags\}
...
## タグリストにスタイルを追加する \{#add-styles-to-your-tag-list\}
...
## このページをナビゲーションに追加する \{#add-this-page-to-your-navigation\}
...
## チャレンジ: ブログ記事レイアウトにタグを含める \{#challenge-include-tags-in-your-blog-post-layout\}

これで、ブログの各記事にタグのリストを表示し、タグページにリンクするために必要なすべてのコードを書きましたが、これを再利用して既存のコードで使用してみましょう！

以下の手順に沿って作業を進め、出来上がった自分のコードを[最終的なコードサンプル](#code-check-in-markdownpostlayout)と比較して確認してください。
...
### MarkdownPostLayoutのコードの確認 \{#code-check-in-markdownpostlayout\}
...
- [Astroにおける静的ルーティング](/ja/guides/routing/#静的ルーティング)
