...
以上でAstroの[組み込みのファイルベースルーティング](/ja/guides/routing/#静的ルーティング)を使ったブログができましたが、これを[コンテンツコレクション](/ja/guides/content-collections/)を使うように更新していきましょう。ブログ記事のような、似たようなコンテンツのグループを管理するうえで、コンテンツコレクションは非常に強力です。
