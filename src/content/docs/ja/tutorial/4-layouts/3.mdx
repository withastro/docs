...
2. `meta`タグはすでに`BaseLayout`に含まれているため、`src/layouts/MarkdownPostLayout.astro`から削除できます。
...
3. `http://localhost:4321/posts/post-1`をブラウザーのプレビューで確認します。コンテンツは現在、以下によってレンダリングされています。
...
5. `http://localhost:4321/posts/post-1`をブラウザーのプレビューで再度確認し、上で削除した行が表示されなくなり、タイトルが1回だけ表示されていることを確認します。コンテンツが重複しないよう、必要に応じて他の調整をおこなってください。
