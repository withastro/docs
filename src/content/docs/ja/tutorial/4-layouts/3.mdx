...
TODO: 最初の`.md`ブログ記事を書いたところで、ウェブサイトの他のページと同じ見た目にしましょう！
...
TODO: ページ全体のレイアウトを定義するための`BaseLayout.astro`がすでにあります。
...
TODO: `MarkdownPostLayout.astro`は、`title`や`date`など、ブログ記事に共通するプロパティに対応するテンプレートを提供しますが、ブログ記事ページはサイトの他のページとは異なる見た目になってしまっています。これは、**レイアウトを入れ子にする**ことで解決できます。
...
2. `localhost:3000/posts/post-1`をブラウザのプレビューで確認します。コンテンツは現在、以下によってレンダリングされています。
...
5. `localhost:3000/posts/post-1`をブラウザのプレビューで再度確認し、上で削除した行が表示されなくなり、タイトルが1回だけ表示されていることを確認します。コンテンツが重複しないよう、必要に応じて他の調整をおこなってください。
