...
2. ブラウザーのプレビューを複数のサイズで開き、このページのナビゲーションメニューがスクリーンサイズに対してレスポンシブであり、またユーザーの入力に反応することを確認します。
</Steps>
...
<Steps>
1. （新しく`/scripts/`フォルダーを作成した上で）`src/scripts/menu.js`を作成し、JavaScriptをそこに移動します。
...
3. ブラウザーのプレビューを再度小さいサイズで開き、メニューがナビゲーションリンクを開閉することを確認します。
...
**`<script>`タグ内のJavaScriptはブラウザーに送信され**、ユーザーの操作に応じてページの更新や入力の切り替えなどを実行します。
:::
...
2. Astroでは任意でJavaScriptをブラウザーに送信できますが、その目的は何ですか？
    <MultipleChoice>
      <Option>
...
3. クライアントサイドのJavaScriptがユーザーのブラウザーに送信されるのは、それがどこに書かれた・インポートされたときですか？
    <MultipleChoice>
      <Option isCorrect>
        `<script>`タグ内。
      </Option>
