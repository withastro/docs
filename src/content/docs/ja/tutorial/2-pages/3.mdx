...
3. `/about`ページのライブプレビューを更新します。
    ...
    ページ内のテキストは変化せず、一方でブラウザーのタブに表示されるページタイトルが「Astro」から「私について」に変わっているはずです。
...
3. `.astro`ファイルのどちらのセクションでも、JavaScriptのすべてのモダンな**論理演算子**、**式**、**関数**を使用できます。ただし、波括弧はHTMLテンプレートボディーでのみ必要です。
:::
...
以下の各Astroテンプレート式について、ブラウザーに送信されるHTML（もしあれば）を予測できますか？正しく予測できたかどうか、クリックして確認してください！
...
- [Astroにおける動的な式](/ja/reference/astro-syntax/#jsxライクな式)
