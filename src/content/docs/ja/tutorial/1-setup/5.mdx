...
<PreCheck>
  - GitHubのリポジトリーを新しいNetlifyアプリとして追加する
  - ウェブ上にAstroサイトをデプロイする
</PreCheck>

ここでは、GitHubリポジトリーをNetlifyに接続します。Netlifyは、あなたがコードに変更をコミットするたびに、そのプロジェクトを使用してサイトをビルドし、ウェブ上にデプロイします。
...
2. <kbd>Add new project</kbd> > <kbd>Import an existing project</kbd>をクリックします。
    ...
    Gitプロバイダーに接続するように求められます。GitHubを選択し、画面の指示に従ってGitHubアカウントを認証します。次に、提供されたリストからAstroプロジェクトのGitHubリポジトリーを選択します。
...
サイト設定に表示されているURLをクリックするか、URLをブラウザーのウィンドウに入力して、新しいウェブサイトを表示します。
...
既存ウェブサイトのホームページを更新したいとします。どのような手順を踏めばよいでしょうか？
  ...
  <MultipleChoice>
    ...
    <Option>
      Netlifyアプリの設定を変更し、astro.newで新しいAstroプロジェクトを開始する。
    </Option>
    <Option isCorrect>
