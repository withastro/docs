...
Astroをシステム上で実行するには、バージョン`v18.17.1`または`v20.3.0`以降の[**Node.js**](https://nodejs.org/ja/)をインストールする必要があります（`v19`はサポート対象外です）。
...
コマンドが`v18.17.1`または`v20.3.0`よりも高いバージョン番号（`v19`は除く）を返していれば、問題ありません！
