...
Astroをシステム上で実行するには、互換性のあるバージョンの[**Node.js**](https://nodejs.org/ja/)をインストールする必要があります。Astroは**偶数**バージョンのNode.jsをサポートしています。現在サポートされている最小のバージョンは`v22.12.0`です。`v23`などの奇数バージョンはサポートされません。
