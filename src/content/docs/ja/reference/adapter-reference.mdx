...
### サーバーエントリポイント \{#server-entrypoint\}
...
##### `RenderOptions`について \{#renderoptions\}
...
###### `addCookieHeader`オプション \{#addcookieheader\}
...
###### `clientAddress`オプション \{#clientaddress\}
...
##### `app.match(request)`メソッド \{#appmatchrequest\}
...
## `astro add`コマンドでのインストールを可能にする \{#allow-installation-via-astro-add\}
...
#### サーバーレス環境での動作について \{#serverless-environments\}
...
### `edgeMiddleware`機能について \{#edgemiddleware\}
