...
`set:html={Promise<Response>}` を使うと、[Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) オブジェクトの内容を要素に挿入できます。
...
`set:html` はどんなタグでも使えますし、必ずしもHTMLを含める必要はありません。例えば、ページに [JSON-LD](https://json-ld.org/) スキーマを追加したい場合、`<script>` タグ内で [`JSON.stringify()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) と組み合わせて使うこともできます。
...
UIフレームワークコンポーネントは、特に指定がない限り、クライアントサイドでは静的なHTMLとして扱われます。つまり、`client:*`というディレクティブを使わないと、JavaScriptを含まない単なるHTMLとしてページに表示されるだけです。

クライアントディレクティブを使えるのは、`.astro`ファイルに直接インポートされたUIフレームワークコンポーネントに限ります。[動的タグ](/ja/basics/astro-syntax/#動的タグ)や、[`components`プロップを通じて渡されるカスタムコンポーネント](/ja/guides/markdown-content/#インポートしたmdxを使ったカスタムコンポーネント)では、これらのディレクティブは使用できません。
