...
[ビルトインドライバー](#ビルトインドライバー)はUnstorageと同じドライバーを提供します。より高度なカスタマイズが必要な場合は、任意の[Unstorageドライバー](https://unstorage.unjs.io/drivers)を基にして、[独自のドライバーを構築](#セッションドライバーの実装)できます。

`unstorage`パッケージをインストールして、ドライバーの指定子を[エントリポイント](#entrypoint)として渡します。
...
また、Unstorageドライバーをインポートして、自身の実装の中でラップすることもできます。これは、ドライバーに追加の処理を加えたり、[実行時に設定を上書き](/ja/guides/sessions/#overriding-the-configuration-at-runtime)したりする場合に便利です。

以下の例では、Redisドライバーの`ttl`のデフォルト値を7日間に設定しています。
...
export default function (config: RedisOptions): SessionDriver {
  const driver = redisDriver({
    ...config,
    ttl: config.ttl ?? 60 * 60 * 24 * 7, // デフォルト値を7日間に設定
  });
