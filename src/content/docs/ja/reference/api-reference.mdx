...
このコンポーネントは、ビルド時にコードブロックのシンタックスハイライトを提供します（クライアント側のJavaScriptは含まれません）。コンポーネントは内部的にはShikiによって動作し、すべての人気のある[テーマ](https://shiki.style/themes)や[言語](https://shiki.style/languages)をサポートします。さらに、カスタムテーマ、言語、および[トランスフォーマー](#トランスフォーマー)をそれぞれの属性に渡すことで追加できます。
