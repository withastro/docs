...
Astroサイト向けのセキュリティ機能をまとめたオプションです。

これらの機能は、`server`モードでオンデマンドレンダリング（SSR）するページ、または`static`モードでも事前レンダリングをオプトアウトしたページでのみ適用されます。

デフォルトでは、オンデマンドレンダリングされるページで各リクエストのURLと`origin`ヘッダーが一致するかを自動的に検証します。`checkOrigin`を`false`に設定すると、この動作を無効化できます。
...
すべてのモダンブラウザが送信する`origin`ヘッダーが各`Request`のURLと一致するかどうかを検証し、クロスサイトリクエストフォージェリ（CSRF）を防ぎます。

`origin`チェックはオンデマンドレンダリングするページでのみ行われ、かつ以下の`content-type`ヘッダーを持つ`POST`、`PATCH`、`DELETE`、`PUT`リクエストが対象です。
...
SSR時に受け入れるホスト名のパターンを定義します。設定すると、Astroは`X-Forwarded-Host`ヘッダーをこれらのパターンに照らして検証し、一致しないヘッダーは無視して元のホスト名を使用します。これにより、細工された`X-Forwarded-Host`によって`Astro.url`を書き換えるホストヘッダーインジェクション攻撃を防げます。

各パターンでは`protocol`、`hostname`、`port`を指定できます（必要なものだけで可）。ワイルドカードにも対応しています。
...
設定しない場合、`X-Forwarded-Host`ヘッダーは信頼されません。

### `security.csp`

<p>

**データ型:** `boolean | object`<br />
**デフォルト値:** `false`<br />
<Since v="6.0.0" />
</p>

[Content Security Policy (CSP)](https://developer.mozilla.org/ja/docs/Web/HTTP/CSP)を有効にして、ページが読み込めるリソースを制御し、[クロスサイトスクリプティング（XSS）](https://developer.mozilla.org/ja/docs/Glossary/Cross-site_scripting)などの攻撃面を減らします。Astroが処理・バンドルするスクリプトとスタイルには自動的にハッシュを付与し、追加のコンテンツタイプも制限できます。

この機能にはいくつかの制約があります。
- 外部スクリプトや外部スタイルは既定では許可されません。必要に応じて自分でハッシュを追加してください（[#hashes](#hashes)を参照）。
- [`<ClientRouter />`](/ja/guides/view-transitions/)を用いたAstroのビュー遷移はサポートされません。ブラウザネイティブのView Transition APIへの移行を検討してください。
- Shikiはインラインスタイルを使用するため、現在はサポートされません。

:::note
Vite開発サーバーの仕組みにより、`dev`モードではCSPを確認できません。`astro build`と`astro preview`で検証してください。
:::

有効化すると、各ページの`<head>`内に`http-equiv="content-security-policy"`を持つ`<meta>`要素が追加され、ページで使用されたスクリプトとスタイルに応じて`script-src`と`style-src`ディレクティブ（[#securitycspdirectives](#securitycspdirectives)）が自動生成されます。
...
追加オプションを指定すると、この`<meta>`要素を細かくカスタマイズできます。

##### `security.csp.algorithm`

<p>

**データ型:** `'SHA-256' | 'SHA-384' | 'SHA-512'`<br />
**デフォルト値:** `'SHA-256'`<br />
<Since v="6.0.0" />
</p>

Astroが生成するスタイル／スクリプトのハッシュで使用するハッシュ関数を選びます。
...
**データ型:** `CspDirective[]`<br />
**デフォルト値:** `[]`<br />
<Since v="6.0.0" />
</p>

`script-src`と`style-src`以外の[CSPディレクティブ](https://content-security-policy.com/#directive)を追加します。指定したディレクティブはすべてのページのメタタグに追記されます。
...
`style-src`の既定リソースを[`resources`](#resources)で上書きしたり、追加のハッシュを指定したりします。
...
`script-src`の既定リソースやハッシュを上書きします。
...
`script-src`と`style-src`の許可リソースを指定します。`'self'`は自動では含まれないため、必要ならリストに含めてください。
...
<meta
  http-equiv="content-security-policy"
  content="
    script-src https://cdn.example.com 'sha256-generatedByAstro';
    style-src 'self' https://styles.cdn.example.com 'sha256-generatedByAstro';
  "
>
```

複数の場所（設定とAPI呼び出しなど）からリソースを追加した場合でも、Astroが結合・重複排除します。
...
追加したいハッシュを指定します。`sha256-`、`sha384-`、`sha512-`のいずれかで始まる必要があります。
...
[`strict-dynamic`キーワード](https://developer.mozilla.org/ja/docs/Web/HTTP/Headers/Content-Security-Policy/script-src#strict-dynamic_%E3%82%AD%E3%83%BC%E3%83%AF%E3%83%BC%E3%83%89)を有効にし、動的に挿入されるスクリプトを許可します。
