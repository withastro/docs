...
- `prefix-other-locales`（デフォルト）: デフォルト以外の言語でのみ言語プリフィックスが表示されます。`defaultLocale`では言語プリフィックスは表示されます。URLの形式は、デフォルト以外のすべての言語では`example.com/[lang]/content/`になりますが、デフォルトのロケールでは`example.com/content/`になります。
