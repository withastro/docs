...
高速なページのトランジションを提供するために、サイト上のリンクに対してプリフェッチを有効にする。（`<ViewTransitions />`を使用しているページではデフォルトで有効化されます。この動作からオプトアウトするには`prefetch: false`を設定してください。）
...
開発モードで開発オーバーレイを有効にします。このオーバーレイを使うと、ページのアイランドを調査したり、パフォーマンスやアクセシビリティの便利な監査をしたりできるようになります。
...
実験的なi18nルーティングを設定し、カスタマイズオプションを指定できるようにします。
...
ウェブサイトがサポートするすべてのロケールのリストです（例: `['en', 'es', 'pt_BR']`）。このリストは`defaultLocale`も含む必要があります。これは必須のフィールドです。

特定の言語のフォーマットや構文が強制されることはありませんが、フォルダ構造はリスト内のロケールに完全に一致しなければいけません。
...
存在しないページへのナビゲーションしたときのフォールバック戦略です（例: 翻訳ページがまだ作成されていない場合）。

このオブジェクトを使用して、サポートしている各言語に対するフォールバック`locale`ルートを宣言してください。フォールバックが指定されなかった場合、利用できないページは404を返します。

#### 例

以下の例では、コンテンツのフォールバック戦略を、`/pt/`で利用できないページから対応する`es`バージョンにリダイレクトするように、また、`/fr/`で利用できないページから対応する`en`バージョンにリダイレクトするように設定しています。`/es/`で利用できないページは、404を返します。
...
<p>
  ...
  **データ型:** `'prefix-always' | 'prefix-other-locales'`<br />
  **デフォルト:** 'prefix-other-locales'<br />
  <Since v="3.5.0" />
</p>

サイトのURLを決定するためのルーティング戦略を制御します。

- `prefix-other-locales`（デフォルト）: デフォルト以外の言語でのみ言語プリフィックスが表示されます。`defaultLocale`では言語プリフィックスは表示されません。URLの形式は、デフォルト以外のすべての言語では`example.com/[lang]/content/`になりますが、デフォルトのロケールでは`example.com/content/`になります。
- `prefix-always`: すべてのURLに言語プリフィックスが表示されます。URLの形式は、デフォルト言語も含めて、すべてのルーティングで`example.com/[lang]/content/`になります。

注意: Astroは、デフォルト言語に対しても、すべてのコンテンツが`/[lang]/`フォルダに存在することを要求します。
