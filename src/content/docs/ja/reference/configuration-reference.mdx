...
セッションストレージに使用するUnstorageドライバーです。[Node](/ja/guides/integrations-guide/node/#sessions)、[Cloudflare](/ja/guides/integrations-guide/cloudflare/#セッション)、[Netlify](/ja/guides/integrations-guide/netlify/#セッション)アダプターは自動的にデフォルトドライバーを設定しますが、必要であれば独自のドライバーを自分で指定できます。
