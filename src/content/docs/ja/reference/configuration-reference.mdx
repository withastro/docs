...
このオプションはプロジェクトルート以外のディレクトリで`astro`のCLIコマンドを実行するときにのみ指定する必要があります。Astroは設定ファイルを見つける前にプロジェクトルートを知る必要があるため、通常このオプションは [Astroの設定ファイル](/ja/guides/configuring-astro/#supported-config-file-types)ではなくCLI経由で指定されます。
