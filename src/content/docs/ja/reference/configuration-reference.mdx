...
セッションストレージに使用するUnstorageドライバーです。[Node](/ja/guides/integrations-guide/node/#セッション)、[Cloudflare](/ja/guides/integrations-guide/cloudflare/#セッション)、[Netlify](/ja/guides/integrations-guide/netlify/#sessions)アダプターは自動的にデフォルトドライバーを設定しますが、必要であれば独自のドライバーを自分で指定できます。
