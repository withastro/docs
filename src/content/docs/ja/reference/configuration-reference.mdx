...
セッションストレージに使用するUnstorageドライバーです。[Node](/ja/guides/integrations-guide/node/#sessions)、[Cloudflare](/ja/guides/integrations-guide/cloudflare/#セッション)、[Netlify](/ja/guides/integrations-guide/netlify/#sessions)アダプターは自動的にデフォルトドライバーを設定しますが、希望する場合や、提供しないアダプターを使用している場合は、独自のドライバーを指定できます。
...
[`i18n.fallback`](#i18nfallback)が設定され、存在しないページルートに対して404ページの表示を回避する場合、このオプションはフォールバックページに[リダイレクト](/ja/guides/routing/#リダイレクト)するか、フォールバックページのコンテンツをその場で[リライト](/ja/guides/routing/#rewrites)するかを制御します。
