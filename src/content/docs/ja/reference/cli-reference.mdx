...
<PackageManagerTabs>
  ...
  ```
  （`--port`の前の`--`は、`npm`がフラグを`astro`コマンドに渡すために必要です。）
  </Fragment>
  <Fragment slot="pnpm">
  ```shell
  # `package.json`の`start`スクリプトを使用して、ポート8080で開発サーバーを起動する
...
Astroの開発サーバーを起動します。これはアセットをバンドルしないローカルのHTTPサーバーです。ホットモジュールリプレースメント（Hot Module Replacement 、HMR）を使用して、エディターで変更を保存した際にブラウザを更新します。
