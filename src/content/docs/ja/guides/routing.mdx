...
オンデマンドレンダリング用のルートでは、スプレッド構文（`[...param]`）によるレストパラメータはファイル名につき **1 つ** だけ指定できます。たとえば `src/pages/[locale]/[...slug].astro` や `src/pages/[...locale]/[slug].astro` は許可されますが、`src/pages/[...locale]/[...slug].astro` は許可されません。
