...
すべてのユーザーが同じように画像を見ているわけではないため、画像を使用する際にはアクセシビリティが特に重要です。画像に対して[説明的な代替テキスト](https://www.w3.org/WAI/tutorials/images/)を提供するために、`alt`属性を使用してください。
...
（[UIフレームワークコンポーネント内の画像](#uiフレームワークコンポーネント内の画像)を含む）既存の`<img>`タグの`src`属性を更新する必要があります。また、インポートされた画像から利用可能になった他の属性も更新する必要があるかもしれません。
...
これにより、画像を`public/`ディレクトリからプロジェクトの`src/`に移動し、処理を加えて最適化できるようになりました。`public/`内の既存の画像とリモート画像は引き続き有効ですが、Astroのビルドプロセスでは最適化されません。
