...
`src` フォルダーに `turso.ts` ファイルを作成し、`TURSO_DATABASE_URL` と `TURSO_AUTH_TOKEN` を渡して `createClient` を呼び出します。
...
次の例では、`slug` が `Astro.params` から取得した値と一致する `posts` テーブルから単一のエントリーを取得し、投稿のタイトルを表示します。
