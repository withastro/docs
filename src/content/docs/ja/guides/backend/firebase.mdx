...
Firebase環境変数のIntelliSenseを有効にしたい場合は、`src/` ディレクトリーの `env.d.ts` ファイルを編集または作成し、型を設定します。
...
次に、`src/` ディレクトリーに `firebase` という名前のフォルダーを作成し、このフォルダーに `client.ts` と `server.ts` という2つの新しいファイルを追加します。
...
新しいディレクトリー `src/pages/api/auth/` に認証に関連する3つのエンドポイント `signin.ts`、`signout.ts`、`register.ts` を作成します。
...
認証用のサーバーエンドポイントを作成後、プロジェクトディレクトリーには次の新しいファイルが含まれるようになりました。
...
新しいディレクトリー `src/pages/api/friends/` に `index.ts` と `[id].ts` という2つの新しいファイルを作成します。これらは、次の方法でFirestoreデータベースと対話する2つのサーバーエンドポイントを作成します。
...
Firestore用のサーバーエンドポイントを作成後、プロジェクトディレクトリーには次の新しいファイルが含まれるようになりました。
