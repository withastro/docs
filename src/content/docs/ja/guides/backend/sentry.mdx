...
[AstroとSentryのパートナーシップ](https://astro.build/blog/sentry-official-monitoring-partner/)と、Astro開発環境に豊富なデバッグオーバーレイをもたらすSentryのSpotlight開発ツールバーアプリについて、ブログで詳しく説明しています。Spotlightは、ローカル開発中にエラー、トレース、重要なコンテキストをブラウザーに直接表示します。
