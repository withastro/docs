...
環境変数のIntelliSenseを有効にしたい場合は、`src/` ディレクトリーの `env.d.ts` を編集または作成し、次の内容を追加します。
...
次に、`src/` ディレクトリーに `lib` という名前のフォルダーを作成します。ここにSupabaseクライアントを追加します。
...
プロジェクトの `src/pages/api/auth` ディレクトリーにこれらのエンドポイントを作成します。`static` レンダリングモードを使用している場合は、これらのエンドポイントをオンデマンドでレンダリングするために、各ファイルの先頭に `export const prerender = false` を指定する必要があります。プロジェクトには次の新しいファイルが含まれるようになります。
...
`src/pages` ディレクトリーにこれらのページを作成します。プロジェクトには次の新しいファイルが含まれるようになります。
