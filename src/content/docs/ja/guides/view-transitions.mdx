...
新しいページのコンテンツが読み込まれ、ドキュメントとしてパースされた後、ビュー遷移フェーズの直前に発生するイベントです。
...
旧ページが新ページに置き換わった直後に発生するイベントです。`document`でこのイベントをリッスンすることで、新しいページのDOM要素が描画され、スクリプトが実行される前にアクションを実行できます。
...
新しいページがユーザーに表示され、ブロッキングスタイルとスクリプトがロードされたあとの、ページナビゲーションの最後に発生するイベントです。`document`上でこのイベントをリッスンできます。
