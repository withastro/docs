...
MDXインテグレーションをインストールすると、[Astroコンポーネント](/ja/basics/astro-components/)と[UIフレームワークコンポーネント](/ja/guides/framework-components/#フレームワークコンポーネントの使用)の両方を、他のAstroコンポーネントと同じようにMDX（`.mdx`）ファイルにインポートして使用できます。
