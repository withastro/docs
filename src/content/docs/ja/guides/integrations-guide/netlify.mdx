...
たとえば、[実験的なコンテンツセキュリティポリシー](/ja/reference/configuration-reference/#securitycsp)が有効な場合、`experimentalStaticHeaders`を使用して、`<meta>`要素を作成する代わりにCSP `headers`をNetlify設定に追加できます。
