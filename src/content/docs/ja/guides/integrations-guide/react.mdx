...
この **[Astro インテグレーション][astro-integration]** は[React](https://react.dev/)コンポーネントのサーバーサイドレンダリング(SSR)とクライアントサイドのハイドレーションを可能にします。

## インストール

 Astroには標準で、公式にサポートされているインテグレーションのセットアップを自動で完了させる事が出来る`astro add`コマンドがあります。最初の方法を使用したくないユーザーは代わりにインテグレーションを[手動でインストール](#手動でのインストール) する事も出来ます。

`@astrojs/react`をインストールするには下のコマンドを実行して下さい。
