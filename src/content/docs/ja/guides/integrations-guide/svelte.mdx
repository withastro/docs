...
`@astrojs/svelte`をインストールするには、プロジェクトディレクトリーから次のコマンドを実行し、プロンプトに従ってください。
...
そして、プロジェクトのルートディレクトリーに`svelte.config.js`という新しいファイルを作成し、次のコードを追加します。
...
このインテグレーションは`@sveltejs/vite-plugin-svelte`によって提供されています。Svelteコンパイラーをカスタマイズするには、インテグレーションにオプションを指定できます。詳細については、[`@sveltejs/vite-plugin-svelte`のドキュメント](https://github.com/sveltejs/vite-plugin-svelte/blob/HEAD/docs/config.md)を参照してください。
