...
Partytownは、リソースを大量に消費するスクリプトを[Web Worker](https://developer.mozilla.org/ja/docs/Web/API/Web_Workers_API)に再配置し、[メインスレッド](https://developer.mozilla.org/ja/docs/Glossary/Main_thread)から解放するのに役立つ遅延読み込みライブラリーです。

分析や広告などのためにサードパーティーのスクリプトを使用している場合、Partytownはサイトの速度を低下させないようにするための優れた方法です。
...
サードパーティーのスクリプトは通常、`window`オブジェクトに変数を追加して、サイト全体でそれらと通信できるようにします。しかし、スクリプトがWeb Workerで読み込まれると、そのグローバルな`window`オブジェクトにアクセスできなくなります。
