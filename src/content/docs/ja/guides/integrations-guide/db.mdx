...
`.ts`または`.js`ファイルを実行し、データベース上で読み書きを行います。このコマンドはファイルパスを引数として受け入れ、型安全なクエリを書くための`astro:db`モジュールの使用もサポートしています。Studioのプロジェクトのデータベースに対して実行したい場合は`--remote`フラグを、開発用サーバーに対して実行したい場合は`--remote`フラグを省いて実行してください。サンプルファイルを見るには[開発データのシード](/ja/guides/astro-db/#データベースのシード)を確認してください。
