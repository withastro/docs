...
この[**Astroインテグレーション**][astro-integration]は、[Markdoc](https://markdoc.dev/)を使用してコンポーネント、ページ、およびコンテンツコレクションのエントリーを作成できるようにします。
...
Markdocファイルは、コンテンツコレクション内でのみ使用できます。`.mdoc`拡張子を使用して、任意のコンテンツコレクションにエントリーを追加します。
...
コンテンツに[変数][markdoc-variables]を渡す必要がある場合があります。これは、A/BテストなどのSSRパラメーターを渡す場合に便利です。
...
フロントマターにアクセスするには、コンテンツをレンダリングする場所でエントリーの`data`プロパティをを渡すことができます。
...
:::tip
パーシャルファイルまたはディレクトリーにはアンダースコア`_`プレフィックスを使用します。これにより、パーシャルがコンテンツコレクションクエリから除外されます。
:::

この例は、ブログコレクションエントリー内で使用するフッターのMarkdocパーシャルを示しています。
...
`{% partial /%}`タグを使用して、ブログ投稿エントリーの下部にフッターをレンダリングします。相対パスまたはインポートエイリアスを使用して、ファイルへのパスを指定して`file`属性を適用します。
...
これで、任意のMarkdocコンテンツエントリーからこの関数を呼び出すことができます。
