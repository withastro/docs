...
たとえば、[CSP設定](/ja/reference/configuration-reference/#securitycsp)を有効にしている場合、`experimentalStaticHeaders`を使用して、`<meta>`要素を作成する代わりにCSPヘッダーを`Response`オブジェクトに追加できます。
