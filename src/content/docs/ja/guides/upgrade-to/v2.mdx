...
この場所は[新しい`build.assets`設定オプション](/ja/reference/configuration-reference/#buildassets)で制御することができます。
...
Astro v2.0では、MDXインテグレーション設定において、[利用可能なすべてのMarkdown設定オプション](/ja/reference/configuration-reference/#markdown-options)（`drafts`を除く）を個別に設定できるようになりました。
...
Astro v2.oでは、`legacy.astroFlavoredMarkdown` オプションは完全に削除されました。`.md`ファイル内のコンポーネントをインポートして使うことはできなくなります。

#### どうすればいいの？

このレガシー フラグを削除します。 Astro では使用できなくなりました。
...
v0.24では、Astroは、Astro は、ブラウザーで参照する可能性があるアセットへの解決済み URL を取得するための `Astro.resolve()` を非推奨にしました。
...
[`Astro.glob()`](/ja/guides/imports/#astroglob)を使ってローカルのMarkdownファイルを取得したり、[コンテンツコレクション](/ja/guides/content-collections/)に変換したりできます。
...
- SSR使用時に[個別ページ静的HTMLにプリレンダリング](/ja/guides/server-side-rendering/#hybrid-rendering)をし、高速化とキャッシュ性能の向上を図る。
