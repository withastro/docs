...
Astro v4.0は、[破壊的な可能性のある変更](#breaking-changes)や、[以前に非推奨となっていた機能の削除](#previously-deprecated-features-now-removed)を含んでいます。
...
## Astro v4.0の実験的なフラグの削除 \{#astro-v40-experimental-flags-removed\}
...
### アップグレード: Vite 5.0 \{#upgraded-vite-50\}
...
### アップグレード: unified、remarkとrehypeの依存関係 \{#upgraded-unified-remark-and-rehype-dependencies\}
...
### リネーム: `entrypoint` (インテグレーションAPI) \{#renamed-entrypoint-integrations-api\}
...
### 変更: インテグレーションAPIの`app.render`のシグネチャ \{#changed-apprender-signature-in-integrations-api\}
...
+ app.render(request, { routeData, locals })
```
### 変更: アダプターはサポートする機能を指定する必要があります \{#changed-adapters-must-now-specify-supported-features\}
...
### 削除: Shiki言語の`path`プロパティ \{#removed-shiki-language-path-property\}
...
### 非推奨: ビュートランジションの`submit`イベントの`handleForms` \{#deprecated-handleforms-for-view-transitions-submit-events\}
...
## 以前に非推奨となっていた機能の削除 \{#previously-deprecated-features-now-removed\}
...
### 削除: エンドポイントでのプレーンオブジェクトの返却 \{#removed-returning-simple-objects-from-endpoints\}
...
### 削除: `build.split`と`build.excludeMiddleware` \{#removed-buildsplit-and-buildexcludemiddleware\}
...
### 削除: `markdown.drafts` \{#removed-markdowndrafts\}

Astro v3.0では、下書きの記事のビルドを制御するために`markdown.drafts`を使用することは非推奨となりました。

Astro v4.0では、このオプションは完全に削除されます。

#### どうすればいいですか？ \{#what-should-i-do-10\}

非推奨の`markdown.drafts`を使用している場合、これはもう存在しないため、削除する必要があります。

プロジェクト内の一部のページを下書きとしてマークし続けるには、[コンテンツコレクションに移行](/ja/guides/content-collections/#migrating-from-file-based-routing)し、代わりに`draft: true`フロントマタープロパティで[ページを手動でフィルタリング](/ja/guides/content-collections/#filtering-collection-queries)します。

### 削除: `getHeaders()` \{#removed-getheaders\}

Astro v3.0では、`getHeaders()` Markdownのエクスポートは非推奨となり、`getHeadings()`に置き換えられました。

Astro v4.0では、このオプションは完全に削除されます。

#### どうすればいいですか？ \{#what-should-i-do-11\}
...
#### どうすればいいですか？ \{#what-should-i-do-13\}
...
### 削除: `base`プレフィックスがない場合の301リダイレクト \{#removed-301-redirects-when-missing-a-base-prefix\}
...
#### どうすればいいですか？ \{#what-should-i-do-14\}
...
### 削除: `astro/client-image`の自動変換 \{#removed-astroclient-image-auto-conversion\}
...
#### どうすればいいですか？ \{#what-should-i-do-15\}
