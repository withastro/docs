...
Astro v5.0 では、いくつかの[破壊的な変更](#breaking-changes)や、機能の削除、非推奨への変更などが行われています。
...
<SourcePR number="11741" title="未使用のJSXコードをクリーンアップ"/>
...
Astro 4.x では、コンテンツコレクションは、[Astro v2.0 で最初に導入されたコンテンツコレクション API](https://astro.build/blog/introducing-content-collections/) を使用して定義、クエリ、およびレンダリングされました。すべてのコレクションエントリは、確保済みの `src/content/` フォルダー内のローカルファイルでした。さらに、Astro の [個々のページのビルドを除外するためのファイル名規則](/ja/guides/routing/#ページの除外) は、コンテンツコレクション API に組み込まれていました。
...
既存のコレクションは、できるだけ早く [新しいコンテンツレイヤー API に変換](/ja/guides/upgrade-to/v5/#既存のコレクションの更新) し、新しいコレクションはコンテンツレイヤー API を使用して作成することをお勧めします。
...
<ReadMore>Learn more about [Astro's official integrations](/ja/guides/integrations-guide/).</ReadMore>
...
<ReadMore>Learn more about [on-demand rendering in Astro](/ja/guides/on-demand-rendering/).</ReadMore>
...
もしあなたのアダプターがAstro内蔵のSharp画像最適化モジュールをサポートしていない場合は、[no-op画像サービス(EN)](/en/guides/images/#configure-no-op-passthrough-service)を設定することで、`<Image />`と`<Picture />`コンポーネントを使用することができます。
...
<ReadMore>[デフォルト画像サービスの設定](/ja/guides/images/#デフォルトの画像サービス)についてもっと読む。</ReadMore>
...
- `contentLayer` (See guidance for [upgrading existing content collections](#legacy-v20-コンテンツコレクション-api) to the new, preferred Content Layer API.)
...
<ReadMore>[ルーティングの優先順位](/ja/guides/routing/#ルーティングの優先順位)についてはこちらをご覧ください。</ReadMore>
...
<ReadMore>[Astroで`script`タグを使用する](/ja/guides/client-side-scripts/#astroでscriptを使用する)についてもっと読む。</ReadMore>
...
<ReadMore>Read more about [view transitions and client-side routing in Astro](/ja/guides/view-transitions/).</ReadMore>
...
<ReadMore>[AstroにおけるTypeScriptの設定](/ja/guides/typescript/#準備)の詳細はこちら。</ReadMore>
...
To address the "confirm form resubmission?" dialog on refresh, or to preserve action results across sessions, you can now [customize action result handling from middleware](/ja/guides/actions/#advanced-persist-action-results-with-a-session).

We recommend using a session storage provider [as described in our Netlify Blob example](/ja/guides/actions/#advanced-persist-action-results-with-a-session). However, if you prefer the cookie forwarding behavior from 4.X and accept the 4 KB size limit, you can implement the pattern as shown in this sample snippet:
...
<ReadMore>コンパイルされたMarkdownを返すための[`compiledContent()`関数(EN)](/en/guides/markdown-content/#importing-markdown)についてもっと読む。</ReadMore>
...
<ReadMore>Read more about [the `astro:content` API](/ja/reference/modules/astro-content/).</ReadMore>
...
<ReadMore>Read more about [syntax highlighting in Astro](/ja/guides/syntax-highlighting/).</ReadMore>
...
<ReadMore>Read more about [Markdown layouts](/ja/basics/layouts/#markdownのレイアウト).</ReadMore>
...
<ReadMore>[AstroでMarkdownプラグインを使う(EN)](/en/guides/markdown-content/#markdown-plugins)についてもっと読む。</ReadMore>
...
<ReadMore>Read more about [setting an endpoint to use for image optimization](/ja/reference/configuration-reference/#imageendpoint).</ReadMore>
...
<ReadMore>[Astroの `build` 設定オプション](/ja/reference/configuration-reference/#ビルドのオプション)についてもっと読む。</ReadMore>
...
<ReadMore>Read more about [Vite configuration settings in Astro](/ja/reference/configuration-reference/#vite).</ReadMore>
...
<ReadMore>Read more about [pagination in Astro](/ja/guides/routing/#ページネーション).</ReadMore>
...
<ReadMore>Read more about [using HTML attributes in Astro](/ja/reference/astro-syntax/#動的属性).</ReadMore>
...
<ReadMore>See more about [storing data in `context.locals`](/ja/guides/middleware/#contextlocalsにデータを保存する).</ReadMore>
...
<ReadMore>Read more about [creating dynamic routes with `params`](/ja/guides/routing/#静的ssgモード).</ReadMore>
...
<ReadMore>See the [API reference for `IntegrationRouteData`](/ja/reference/integrations-reference/#integrationroutedata-type-reference).</ReadMore>
...
<ReadMore>See the [Adapter API reference for `renderOptions`](/ja/reference/adapter-reference/#サーバーエントリポイント).</ReadMore>
...
<ReadMore>[アダプターでサポートされるAstro機能の指定](/ja/reference/adapter-reference/#astroの機能について)については、こちらをお読みください。</ReadMore>
...
<ReadMore>Read more about [developing a dev toolbar app for Astro using the Dev Toolbar API](/ja/reference/dev-toolbar-app-reference/).</ReadMore>
...
Astro v5.0に関する良い資料をご存知ですか？ [このページを編集](https://github.com/withastro/docs/edit/main/src/content/docs/ja/guides/upgrade-to/v5.mdx)し、以下にリンクを追加してください！

## 既知の問題

GitHub上の[Astroのissue](https://github.com/withastro/astro/issues/)をチェックして、報告された問題を確認するか、自分でissueを提出してください。
