...
Astro v3.0では、[デフォルトのポート](/ja/reference/cli-reference/#--port-number)が`4321`に変更されました。🚀
...
Astro v2.xでは、[`compressHTML`](/ja/reference/configuration-reference/#compresshtml)が明示的に`true`に設定されている場合にのみ、Astroは出力されたHTMLを圧縮しました。デフォルト値は`false`でした。
...
Astro v2.xでは、[`scopedStyleStrategy`](/ja/reference/configuration-reference/#scopedstylestrategy)のデフォルト値は`"where"`でした。
...
Astro v2.xでは、プロジェクトのすべてのスタイルシートはデフォルトでリンクタグとして送信されていました。[`build.inlineStylesheets`](/ja/reference/configuration-reference/#buildinlinestylesheets)の設定を使用して、`"always"`で常に`<style>`タグにインライン化するか、あるいは`"auto"`で一定サイズ以下のスタイルシートのみをインライン化するかを選択できましたが、デフォルトの設定は`"never"`でした。
...
Astro CLIを[プログラムで実行する](/ja/reference/cli-reference/#advanced-apis-experimental)には、新しい実験的なJavaScript APIを使用します。
