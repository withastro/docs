...
ページのロード時間は、サイトの使いやすさとサイト全体の楽しさに大きな影響を与えます。Astroの**オプトインのプリフェッチ**を利用すると、訪問者がサイトを操作する際に、マルチページアプリケーション（MPA）でほぼ瞬時のページナビゲーションという恩恵が得られます。
...
プリフェッチはサイト内のリンク対してのみ機能し、外部リンクに対しては機能しないことに注意してください。
...
`data-astro-prefetch`属性を追加した場合のデフォルトのプリフェッチ戦略は`hover`です。デフォルトを変更するには、`astro.config.mjs`ファイルの[`prefetch.defaultStrategy`](/en/reference/configuration-reference/#prefetchdefaultstrategy)で設定できます。
...
`data-astro-prefetch`属性のないリンクも含めて、すべてのリンクをプリフェッチしたい場合は、次のように、[`prefetch.prefetchAll`](/en/reference/configuration-reference/#prefetchprefetchall)を`true`に設定できます。
...
1. `@astrojs/prefetch`インテグレーションを削除し、`astro.config.mjs`内の`prefetch`設定を有効化する。
...
2. `@astrojs/prefetch`の設定オプションから変換する。
    ...
    - 非推奨になったインテグレーションは、ホバーまたはフォーカルしたときにプリフェッチするべきリンクを指定するために、`selector`設定オプションを使用していました。`intentSelector`設定オプションを使用していました。
