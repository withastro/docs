...
3. `defineAction()` ユーティリティを使用して `getGreeting` アクションを定義します。`input` プロパティは [Zodスキーマ(EN)](/ja/reference/modules/astro-zod/#common-data-type-validators) を使用して入力パラメーターをバリデーションするために使用され、`handler()` 関数にはサーバーで実行されるバックエンドロジックを含めます。
