...
Astroアクションを使用すると、バックエンド関数を型安全に定義して呼び出すことができます。アクションは、データの取得、JSONのパース、入力値のバリデーションを自動的に行います。これにより、[APIエンドポイント](/ja/guides/endpoints/)を使用する場合と比較して、ボイラープレート（定型コード）を大幅に削減できます。

クライアントとサーバー間のシームレスな通信のために、APIエンドポイントの代わりにアクションを使用してください。主なメリットは以下の通りです：

- [Zodバリデーション](/ja/reference/modules/astro-zod/)を使用して、JSONやフォームデータの入力を自動的に検証します。
...
3. `defineAction()` ユーティリティを使用して `getGreeting` アクションを定義します。`input` プロパティは [Zodスキーマ(EN)](/ja/reference/modules/astro-zod/#common-data-type-validators) を使用して入力パラメータを検証するために使用され、`handler()` 関数にはサーバーで実行されるバックエンドロジックを含めます。
...
`data` プロパティを使用する前に、`error` が存在するかどうかを確認するのが最善です。これによりエラーを事前に処理でき、`undefined` のチェックなしで `data` が定義されていることが保証されます。
...
データベースのエントリが見つからない場合の「Not Found」や、ユーザーがログインしていない場合の「Unauthorized」など、アクションの `handler()` からエラーをスローするには、提供されている `ActionError` を使用できます。これには、単に `undefined` を返すよりも2つの大きな利点があります。
...
クライアントからアクションを呼び出す際、`react-router` のようなクライアントサイドライブラリと統合したり、Astroの [`Maps()` 関数](/ja/guides/view-transitions/#trigger-navigation) を使用して、アクションが成功したときに新しいページにリダイレクトしたりできます。
...
アクションはデフォルトでJSONデータを受け取ります。HTMLフォームからフォームデータを受け取るには、`defineAction()`の呼び出しで`accept:'form'`を設定します：
...
アクションが[フォームデータを受け取るように設定されている](/ja/reference/modules/astro-actions/#accept-property)場合、任意のZodバリデータを使用してフィールドを検証できます（例：日付入力のための`z.coerce.date()`）。`.refine()`、`.transform()`、`.pipe()`などの拡張関数も`z.object()`バリデータ上でサポートされています。

さらに、Astroは利便性のために、以下のフィールド入力タイプに対して内部で特別な処理を提供しています：

- `number` タイプの入力は `z.number()` を使用して検証できます。
- `checkbox` タイプの入力は `z.coerce.boolean()` を使用して検証できます。
- `file` タイプの入力は `z.instanceof(File)` を使用して検証できます。
- 同じ `name` を持つ複数の入力は `z.array(/* バリデータ */)` を使用して検証できます。
- その他のすべての入力は `z.string()` を使用して検証できます。

空の入力でフォームが送信された場合、出力タイプが `input` バリデータと一致しないことがあります。空の値は、配列やブーリアンを検証する場合を除き、`null` に変換されます。たとえば、`text` タイプの入力が空の値で送信された場合、結果は空文字列 (`""`) ではなく `null` になります。

異なるバリデータのユニオン（結合）を適用するには、`z.discriminatedUnion()` ラッパーを使用して、特定のフォームフィールドに基づいて型を絞り込みます。この例では、ユーザーの「作成」または「更新」のいずれかのフォーム送信を受け入れ、`type` という名前のフォームフィールドを使用して、どのオブジェクトに対して検証するかを決定します：
...
アクションは、送信されたフォームデータをオブジェクトにパースします。その際、各入力の `name` 属性の値をオブジェクトのキーとして使用します。たとえば、`<input name="search">` を含むフォームは、 `{ search: 'ユーザーの入力内容' }` のようなオブジェクトにパースされます。アクションの `input` スキーマはこのオブジェクトを検証するために使用されます。

パースされたオブジェクトではなく、アクションハンドラーで生の `FormData` オブジェクトを受け取るには、アクション定義で `input` プロパティを省略します。

以下の例は、ユーザーのメールアドレスを受け取り、「利用規約」への同意チェックボックスを必須とする、検証済みのニュースレター登録フォームを示しています。
...
2. 送信されたフォームを処理するための `newsletter` アクションを定義します。 `email` フィールドを `z.string().email()` バリデータで検証し、 `terms` チェックボックスを `z.boolean()` で検証します：
...
3. HTMLフォームに `<script>` を追加して、ユーザー入力を送信します。この例では、フォームのデフォルトの送信動作をオーバーライドして `actions.newsletter()` を呼び出し、 `Maps()` 関数を使用して `/confirmation` にリダイレクトします：
...
`required`、`type="email"`、`pattern` などの [ネイティブHTMLフォームバリデーション属性](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Forms/Form_validation#using_built-in_form_validation) を使用して、送信前にフォーム入力を検証できます。バックエンドでのより複雑な `input` 検証には、提供されている [`isInputError()`](/ja/reference/modules/astro-actions/#isinputerror) ユーティリティ関数を使用できます。
...
Zodによる適切なスキーマ検証のために、 `<form>` 要素に追加の属性が必要になる場合があります。たとえば、ファイルのアップロードを含めるには、 `enctype="multipart/form-data"` を追加して、ファイルが `z.instanceof(File)` で正しく認識される形式で送信されるようにします：
...
まず、 [ミドルウェアファイルを作成](/ja/guides/middleware/) し、 `astro:actions` から [ `getActionContext()` ユーティリティ](/ja/reference/modules/astro-actions/#getactioncontext)をインポートします。この関数は、受信したアクションリクエストに関する情報（アクションハンドラーや、アクションがHTMLフォームから呼び出されたかどうかなど）を含む `action` オブジェクトを返します。また、 `getActionContext()` は、 `Astro.getActionResult()` によって返される値をプログラムで設定するための `setActionResult()` および `serializeActionResult()` 関数も返します：
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // HTML フォームアクションからアクションが呼び出された場合、
  // アクションハンドラーを呼び出し、遷移先ページへリダイレクトします
  if (action?.calledFrom === "form") {
...
アクションリクエストを認可するには、アクションハンドラー内に認証チェックを追加します。セッション管理やユーザー情報の処理には、 [認証ライブラリ](/ja/guides/authentication/) を使用するのが一般的です。
...
Astroは、アクションごとの権限レベルやレート制限を尊重するために、アクションハンドラー内でユーザーセッションを認可することを推奨しています。ただし、ミドルウェアからすべてのアクション（または一部のアクション）へのリクエストを制限することも可能です。
