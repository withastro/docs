...
## コンテンツコレクションとは？ \{#what-are-content-collections\}
...
### ".astro"ディレクトリ \{#the-astro-directory\}
...
### 複数のコレクションによる整理 \{#organizing-with-multiple-collections\}
...
### サブディレクトリを使った整理 \{#organizing-with-subdirectories\}
...
## コレクションの定義 \{#defining-collections\}
...
### TypeScriptのセットアップ \{#setting-up-typescript\}
...
### コレクションスキーマの定義 \{#defining-a-collection-schema\}
...
### 複数のコレクションの定義 \{#defining-multiple-collections\}
...
### サードパーティのコレクションスキーマの使用 \{#using-third-party-collection-schemas\}
...
### Zodによるデータ型の定義 \{#defining-datatypes-with-zod\}
...
### コレクション参照の定義 \{#defining-collection-references\}
...
### カスタムスラグの定義 \{#defining-custom-slugs\}
...
## コレクションのクエリ \{#querying-collections\}
...
### 参照データへのアクセス \{#accessing-referenced-data\}
...
### コレクションクエリのフィルタリング \{#filtering-collection-queries\}
...
### Astroテンプレートでのコンテンツの使用 \{#using-content-in-astro-templates\}
...
コンテンツをHTMLにレンダリングする方法については、下記の[コンテンツをHTMLにレンダリングする](#rendering-content-to-html)を参照してください。
...
### コンテンツをプロパティとして渡す \{#passing-content-as-props\}
...
### コンテンツをHTMLにレンダリングする \{#rendering-content-to-html\}
...
## コンテンツからルーティングを生成 \{#generating-routes-from-content\}
...
### 静的出力のビルド（デフォルト） \{#building-for-static-output-default\}
...
`getStaticPaths()`内で`getCollection()`を呼び出し、[コンテンツまたはデータををクエリします](/ja/guides/content-collections/#querying-collections)。それから、各コンテンツエントリーの`slug`プロパティ(コンテンツコレクション)または`id`プロパティ(データコレクション)を使用して、新しいURLパスを作成します。
...
### サーバー出力のビルド（SSR） \{#building-for-server-output-ssr\}
...
## ファイルベースのルーティングからの移行 \{#migrating-from-file-based-routing\}

`src/pages/`内のサブフォルダーでMarkdownまたはMDXファイルを使用するブログなどの既存のAstroプロジェクトがある場合は、関連するコンテンツまたはデータファイルをコンテンツコレクションに移行することを検討してください。

[ブログ作成チュートリアルの完成プロジェクト](https://github.com/withastro/blog-tutorial-demo)のコードベースを使用する[ステップバイステップのチュートリアル](/ja/tutorials/add-content-collections/)で、基本的なブログの例を`src/pages/posts/`から`src/content/posts`に変換する方法を確認してください。

## JSONスキーマの生成を有効にする \{#enabling-json-schema-generation\}
...
## ビルドキャッシュを有効にする \{#enabling-build-caching\}
...
## Remarkでフロントマターを修正 \{#modifying-frontmatter-with-remark\}
...
## フロントマターでの日付の扱い \{#working-with-dates-in-the-frontmatter\}
