...
**デフォルトではサイト全体が事前レンダリング**され、静的HTMLがブラウザーに送信されます。たとえば、クッキーをチェックしてパーソナライズしたコンテンツを表示するページなど、サーバーレンダリングが必要なルートだけ事前レンダリングを無効化できます。
...
`Astro.request`は標準の[Request](https://developer.mozilla.org/en-US/docs/Web/API/Request)オブジェクトで、`url`、`headers`、`method`、ボディーなどを取得できます。静的生成されないページで追加情報を取得する際に利用します。

#### `Astro.request.headers`

リクエストヘッダーは`Astro.request.headers`で取得します。これはブラウザーの[`Request.headers`](https://developer.mozilla.org/en-US/docs/Web/API/Request/headers)と同じく[Headers](https://developer.mozilla.org/en-US/docs/Web/API/Headers)オブジェクトです。
...
HTTPメソッドは`Astro.request.method`で取得できます。ブラウザーの[`Request.method`](https://developer.mozilla.org/en-US/docs/Web/API/Request/method)と同様で、文字列で返されます。
