...
:::tip
まずはデフォルトの`'static'`モードで始め、**ほとんどまたは全ページ**をオンデマンドレンダリングしたいと確信できるまで切り替えないことをおすすめします。これにより、静的コンテンツをレンダリングするだけのためにサーバー関数を呼び出す必要がなく、パフォーマンスを最大限に引き出せます。

`'server'`モードは機能追加ではなく、レンダリング方式のデフォルトを切り替えるだけです。
:::
...
HTMLストリーミングでは、ドキュメントをチャンクに分割し順番に送信・描画します。Astroはオンデマンドレンダリング時にコンポーネントごとにストリーミングを行い、描画された部分から即座にユーザーに表示します。これにより可能な限り速くHTMLを表示できますが、大きいドキュメントではネットワーク状況によりダウンロードが遅くなる場合や、データフェッチ待ちで描画がブロックされる場合があります。
