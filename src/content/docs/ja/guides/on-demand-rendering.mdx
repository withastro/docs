...
[Astroが保守する公式アダプター](/ja/guides/integrations/)は、以下の`astro add`コマンドで追加できます。このコマンドはアダプターのインストールと`astro.config.mjs`への設定変更を一度に行います。
...
[アダプターを手動で追加](/ja/guides/integrations/)し、`astro.config.mjs`を自分で更新することもできます(例:`@astrojs/netlify`)。
