...
[Astroが保守する公式アダプター](/ja/guides/integrations-guide/#official-integrations)は、以下の`astro add`コマンドで追加できます。このコマンドはアダプターのインストールと`astro.config.mjs`への設定変更を一度に行います。
...
[アダプターを手動で追加](/ja/guides/integrations-guide/#installing-an-npm-package)し、`astro.config.mjs`を自分で更新することもできます(例:`@astrojs/netlify`)。
...
<ReadMore>設定リファレンスの[`output`設定](/ja/reference/configuration-reference/#output)を参照してください。</ReadMore>
...
[`Astro.cookies`と`AstroCookie`型](/ja/reference/api-reference/#cookies)についてはAPIリファレンスを参照してください。

### `Response`

[`Astro.response`](/ja/reference/api-reference/#response)は標準の[`ResponseInit`](https://developer.mozilla.org/en-US/docs/Web/API/Response/Response#options)オブジェクトで、ステータスやヘッダーを設定できます。
...
オンデマンドレンダリングを使用するページでは、[Response](https://developer.mozilla.org/en-US/docs/Web/API/Response)オブジェクトを直接返すこともできます。手動で返すか、[`Astro.redirect`](/ja/reference/api-reference/#redirect)を使用します。
...
[`Astro.request`](/ja/reference/api-reference/#request)について詳しくはAPIリファレンスを参照してください。
...
詳しくは[Endpointsガイド](/ja/guides/endpoints/#server-endpoints-api-routes)を参照してください。