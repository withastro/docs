...
:::note
静的レンダリングまたはサーバーレンダリングモードを使用する任意のデプロイメントプラットフォームで、リモート接続は一般的に可能ですが、現在いくつかの制限があります。CloudflareやDenoのようなNode.js以外のランタイムでは、libSQLを使用する場合にサーバーレンダリングルートでのデータベース接続がサポートされていません。これらのプラットフォームへの対応は、今後の実装計画に含まれています。
:::
