...
Astro DBでは、データはSQLテーブルを使用して保存されます。テーブルはデータを行と列に構造化し、列は各行の値の型を強制します。Astro DBは、次のカラム型をサポートしています。
...
Astroはテーブルを通じて、コンテンツに対する自動的なTypeScriptの型付けを実現します。テーブルを定義すると、Astroは自動的にTypeScriptのインターフェースを生成し適用します。これにより、データをクエリする際に、プロパティの自動補完や型チェックなどの、完全なTypeScriptのサポートが得られます。

個々のデータベーステーブルを設定するには、`astro:db`から`defineTable()`と`column`ユーティリティをインポートして使用します。

以下の例では、必須のテキストカラム`author`と`body`をもつ`Comment`テーブルを設定し、`defineDb()`をエクスポートしてプロジェクトで利用できるようにしています。
...
フォームからのリクエストを処理してリモートにホストされたデータベースにデータを挿入する場合のように、ユーザー入力を受け付けるためには、Astroプロジェクトを[オンデマンドレンダリング](/ja/basics/rendering-modes/#オンデマンドレンダリング)に設定し、デプロイ環境向けの[SSRアダプター](/ja/guides/server-side-rendering/)を追加してください。
...
Astro DBは、Astro Studioプラットフォームに[接続](/ja/recipes/studio/)して、ホストされたデータベースをプロジェクトに迅速に追加できます。Astro Studioのウェブポータルから、新しくホストされたデータベースを表示、管理、デプロイすることができます。

Astro Studioで新しいプロジェクトを作成する方法は2つあります。
1. `astro link`を使用した[Astro Studio CLI](/en/reference/cli-reference/#astro-studio-cli)
2. [Astro StudioのウェブUI](https://studio.astro.build)

Astro StudioのウェブUIで新しいデータベースを作成するには、ヘッダーにある「プロジェクトの作成」ボタンをクリックし、表示される指示に従います。Astro Studioが必要な手順を案内してくれるため、このプロセスの最後には、自分のプロジェクト向けに設定されたホスト済みのデータベースが手に入ります。

<StudioHeading>
### テーブルスキーマのプッシュ
</StudioHeading>

プロジェクトが成長するにつれて、テーブルスキーマは変化します。ローカルで設定の変更を安全にテストした上で、デプロイ時にAstro
