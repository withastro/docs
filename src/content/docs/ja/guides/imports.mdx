...
<img src={imgReference} alt="画像の説明" />;
```
...
Astroのユーザーには、可能な限りNode.jsのビルトイン（`fs`、`path`など）を避けることをおすすめします。Astroは、[アダプター](/ja/guides/server-side-rendering/#アダプターの追加)を使用して複数のランタイムと互換性があります。これには、`fs`などのNodeビルトインモジュールをサポートしない[Deno](/ja/guides/integrations-guide/deno/)や[Cloudflare Workers](/ja/guides/integrations-guide/cloudflare/)が含まれます。
