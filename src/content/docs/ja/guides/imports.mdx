...
Astroには、[TypeScript](https://www.typescriptlang.org/)のサポートが組み込まれています。Astroプロジェクトで`.ts`や `.tsx`ファイルを直接インポートしたり、[Astroのコンポーネントスクリプト](/ja/basics/astro-components/#the-component-script)や [巻き上げられたscriptタグ](/ja/guides/client-side-scripts/)の中で直接TypeScriptコードを書けます。
