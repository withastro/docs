...
Astroのユーザーには、可能な限りNode.jsのビルトイン（`fs`、`path`など）を避けるようをおすすめします。Astroは、アダプタを使用して複数のランタイムと互換性があります。これには、`fs`などのNodeビルトインモジュールをサポートしない[Deno](https://deno.land/)や[Cloudflare Worker](https://workers.cloudflare.com/)が含まれます。
