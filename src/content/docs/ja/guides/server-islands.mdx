...
ただし、[ブラウザにはURL長の上限（2048バイト）が存在します](https://chromium.googlesource.com/chromium/src/+/master/docs/security/url_display_guidelines/url_display_guidelines.md#url-length)。これは実用上の理由と、DoS問題を引き起こさないようにするためです。クエリ文字列によってURLがこの制限を超えると、Astroは代わりにprops全体を本文に含めた`POST`リクエストを送信します。
...
Astroはサーバーアイランドに渡されるpropsを暗号化するために[暗号技術](https://developer.mozilla.org/en-US/docs/Glossary/Cryptography)を使用し、機密データが誤って露出するのを防いでいます。この暗号化はビルドごとに生成される新しいランダムな鍵に依存しており、その鍵はサーバーバンドルに埋め込まれます。
