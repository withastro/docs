...
- Astroで直接利用できる[UIフレームワークコンポーネント](/ja/basics/framework-components/)。
...
- [クライアントサイドの`<script>`タグ](/ja/guides/client-side-scripts/)や[UIフレームワークコンポーネント](/ja/basics/framework-components/)を使用した、クライアントサイドでのインタラクティビティの提供。

- アプリケーション全体にまたがるフックやラッパーではなく、Nano Storesやローカルストレージによる[共有状態](/ja/basics/sharing-state/)の管理。
