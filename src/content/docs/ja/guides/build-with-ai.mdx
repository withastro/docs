...
Windows環境、またはAIコーディングエージェントが存在しない場合は、サーバーはフォアグラウンドで起動し、ターミナルにログを出力します。なお、[`--background`フラグ](/ja/reference/cli-reference/#--background)を明示的に渡すことで、バックグラウンド実行を有効にすることも可能です。

サーバーが起動すると、ロックファイル（`.astro/dev.json`または`.astro/preview.json`）が作成され、サーバーのURL、ポート番号、およびPIDが記録されます。これにより、同一プロジェクトで複数のサーバーが重複して起動するのを防ぎます。
