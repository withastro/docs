...
Codex CLIはコマンドラインのAIコーディングツールで、Astro Docs MCPサーバーを使ってドキュメントを参照しながらAstroコードを生成できます。
...
[Raycast](https://www.raycast.com/)はMCPサーバーに接続してAI機能を強化できます。Astro Docs MCPサーバーを追加すると、質問応答時に最新ドキュメントへアクセスできます。
...
[Opencode AI](https://opencode.ai/)は、オープンソースのターミナルベースAIコーディングツールです。Astro Docs MCPサーバーを利用してドキュメントを参照しながらAstroコードを生成できます。
...
[GitHub Copilot](https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent)は、GitHub Actionsを基盤とするコーディングエージェントとして利用できます。Astro Docs MCPサーバーを有効にすることで、回答の生成やコーディングタスクの実行時に最新のAstroドキュメントを参照できるようになります。

Astro Docs MCPサーバーでドキュメントを参照できるようにするには、リポジトリのCopilot Coding Agent設定（`https://github.com/<your-org>/<your-repo>/settings/copilot/coding_agent`）に以下を追加します:
```json title="MCPの設定"
{
  "mcpServers": {
    "astro-docs": {
...
macOSとLinuxでAIコーディングエージェントが検出されると、`astro dev`とv7.2.0以降の`astro preview`は自動的にサーバーをデタッチされたバックグラウンドプロセスとして起動します。これにより、開発サーバーがエージェントのターミナルをブロックするのを防ぎ、サーバーの実行中もエージェントが作業を継続できるようになります。

Windowsでは、AIコーディングエージェントが検出された場合もサーバーはフォアグラウンドで起動し、ターミナルにログを出力します。AIコーディングエージェントを使用していない場合も同様です。いずれの場合も、[`--background`フラグ](/ja/reference/cli-reference/#--background)を指定すれば、明示的にバックグラウンドモードを有効にできます。

サーバーの起動時には、ロックファイル（`.astro/dev.json`または`.astro/preview.json`）にサーバーのURL、ポート、PIDが記録されます。これにより、同じプロジェクトでサーバーが重複して起動するのを防ぎます。

自動バックグラウンドモードを無効にするには、コマンドを実行する前に`ASTRO_DEV_BACKGROUND`か`ASTRO_PREVIEW_BACKGROUND`環境変数を設定します。
...
<ReadMore>[`astro dev`](/ja/reference/cli-reference/#astro-dev)と[`astro preview`](/ja/reference/cli-reference/#astro-preview)のフラグやサブコマンドの一覧については、CLIリファレンスを参照してください。</ReadMore>
