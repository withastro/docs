...
macOSとLinuxでAIコーディングエージェントが検出されると、`astro dev`とv7.2.0以降の`astro preview`は自動的にサーバーをデタッチされたバックグラウンドプロセスとして起動します。これにより、サーバーがエージェントのターミナルをブロックせず、実行中もエージェントが作業を続けられます。
