...
[Opencode AI](https://opencode.ai/)は、オープンソースのターミナルベースAIコーディングツールです。Astro Docs MCPサーバーを利用してドキュメントにアクセスしながらAstroコードを生成できます。
...
[GitHub Copilot](https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent)は、GitHub Actionsによって動作するコーディングエージェントとして利用できます。Astro Docs MCPサーバーを有効にすると、回答やコーディングタスクの実行時に最新のAstroドキュメントへアクセスできるようになります。

Astro Docs MCPサーバーでドキュメントにアクセスできるようにするには、リポジトリのCopilotコーディングエージェント設定（`https://github.com/<your-org>/<your-repo>/settings/copilot/coding_agent`）に以下を追加します:
```json title="MCP Configuration"
{
  "mcpServers": {
    "astro-docs": {
...
詳しい情報は[MCPサーバーでGitHub Copilotコーディングエージェントを拡張する](https://docs.github.com/en/copilot/how-tos/use-copilot-agents/coding-agent/extend-coding-agent-with-mcp)を参照してください。
