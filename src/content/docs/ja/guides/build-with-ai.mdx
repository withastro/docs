...
AIコーディングエージェントが検出されると、`astro dev`とv7.2.0以降の`astro preview`は自動的にサーバーをデタッチされたバックグラウンドプロセスとして起動します。これにより、開発サーバーがエージェントのターミナルをブロックするのを防ぎ、サーバーの実行中もエージェントが作業を継続できるようになります。
