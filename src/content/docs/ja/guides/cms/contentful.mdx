...
Astroのデフォルト静的モードを利用している場合、[動的ルーティング](/ja/basics/routing/#動的ルーティング)と`getStaticPaths()`関数を使えます。この関数はビルド時に呼ばれて、ページとなるパスのリストを生成します。
...
もしエントリーが見つからない場合は、 [`Astro.redirect`](/ja/basics/routing/#動的ルーティング)を使ってユーザーを404ページにリダイレクトできます。
