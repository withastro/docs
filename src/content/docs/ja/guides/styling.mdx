...
## Astroでのスタイリング \{#styling-in-astro\}
...
### `class:list`によるクラスの結合 \{#combining-classes-with-classlist\}
...
### 子コンポーネントへの`class`の受け渡し \{#passing-a-class-to-a-child-component\}
...
デフォルト設定でスコープ付きスタイルを使用する場合は、`data-astro-cid-*`属性も渡す必要があります。これは、Propsの残り(`...rest`)をコンポーネントに渡すことで行うことができます。`scopedStyleStrategy`を`class`または`where`に変更した場合は、`...rest`Props は必要ありません。
...
### ローカルスタイルシートのインポート \{#import-a-local-stylesheet\}

:::caution[npmパッケージを使用する場合]
npmパッケージからインポートする場合、`astro.config`を更新する必要があるかもしれません。以下の[スタイルシートをnpmパッケージからインポートする](#import-a-stylesheet-from-an-npm-package)をご覧ください。
:::

ESMのインポート構文を使用して、Astroコンポーネントのfrontmatterでスタイルシートをインポートできます。CSSのインポートは、[Astroコンポーネント内の他のESMのインポート](/ja/basics/astro-components/#the-component-script)のように動作し、**コンポーネントからの相対パス**を、ほかのimportと同様にコンポーネントスクリプトの**先頭**に記述しなければなりません。
...
### npmパッケージからスタイルシートをインポートする  \{#import-a-stylesheet-from-an-npm-package\}
...
### "link"タグで静的スタイルシートを読み込む \{#load-a-static-stylesheet-via-link-tags\}
...
この方法では`public/`ディレクトリを使用するため、Astro が提供する通常のCSS処理、バンドル、最適化はスキップされます。これらの変換が必要な場合は、上記の[ローカルスタイルシートのインポート](#import-a-local-stylesheet)の方法を使用してください。

## カスケード順序 \{#cascading-order\}
...
### スコープされたスタイル \{#scoped-styles-1\}

[スコープされたスタイル](#scoped-styles)を使用しても、CSSの詳細度が高まるわけではありませんが、常に登場順が最後になります。したがって、同じ比重の他のスタイルよりも優先されます。たとえば、スコープされたスタイルと競合するスタイルシートをインポートした場合、スコープされたスタイルの値が適用されます。
...
[linkタグ](#load-a-static-stylesheet-via-link-tags)で読み込まれたスタイルシートは、Astroファイル内の他のどのスタイルよりも先に、順番に評価されます。したがって、これらのスタイルは、インポートされたスタイルシートやスコープされたスタイルよりも優先順位が低くなります。
...
### フレームワークコンポーネントにおいて \{#in-framework-components\}
...
## Markdownのスタイル \{#markdown-styling\}

Astroのスタイル方法は、[Markdownレイアウトコンポーネント](/ja/basics/layouts/#markdownのレイアウト)で利用可能ですが、それぞれの方法には異なったスタイリングへの影響があります。

ページコンテンツを包むレイアウトに[インポートしたスタイルシート](/ja/guides/styling/#external-styles)を追加することで、Markdownコンテンツにグローバルスタイルを適用できます。
また、レイアウトコンポーネントに、[`<style is:global>`タグ](/ja/guides/styling/#global-styles)がついたMarkdownでスタイルを追加することも可能です。追加されたスタイルは、[アストロのカスケード順序
](/ja/guides/styling/#cascading-order)に沿っていることを確認し、スタイルが意図した通りに適用されているか慎重にチェックしてください。

加えて、[Tailwind](/ja/guides/integrations-guide/tailwind/)を含む[CSSのインテグレーション](/ja/guides/styling/#css-integrations)の追加もできます。
Tailwindを使用している場合、Markdownのスタイリングには[typographyプラグイン](https://tailwindcss.com/docs/typography-plugin)が便利です。

## 本番環境 \{#production\}

### バンドルの調整 \{#bundle-control\}
