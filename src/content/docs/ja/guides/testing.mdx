...
2. `cypress/e2e`フォルダに`index.spec.js`ファイルを作成します。以下のテストをファイルで使用し、ページのタイトルとヘッダーが正しいことを確認します。
    ...
    :::tip[baseUrlをセットする]
    [`"baseUrl": "http://localhost:3000"`](https://docs.cypress.io/guides/end-to-end-testing/testing-your-app#Step-3-Configure-Cypress)を`cypress.config.ts`の設定ファイルに定義して`cy.visit("/")`を使用すると、`page.goto("http://localhost:3000/")`よりも楽にURLを扱うことができます。
    :::
...
1. `playwright.config.ts`に`webServer`オブジェクトを追加し、コマンドの値を`yarn preview`に設定します。
...
1. `yarn build`を実行した上で、`yarn test:e2e`によりPlaywrightのテストを実行します。
