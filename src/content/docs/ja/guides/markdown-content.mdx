...
また、レイアウトコンポーネントで[Markdownをスタイリング](/ja/guides/styling/#markdown-styling)することもできます。
...
MDXインテグレーションをインストールすると、[Astroコンポーネント](/ja/basics/astro-components/#component-props)と[UIフレームワークコンポーネント](/ja/guides/framework-components/#フレームワークコンポーネントを利用)の両方をMDX（`.mdx`）ファイルにインポートして、他のAstroコンポーネントと同じように使用できるようになります。
...
:::note
[コンテンツコレクション](/ja/guides/content-collections/)を使用している場合は、[「Remarkでフロントマターを修正」](/ja/guides/content-collections/#modifying-frontmatter-with-remark)を参照してください。
:::
