...
📚 Astroの[ファイルベースルーティング](/ja/basics/routing/)や、[動的ルーティング](/ja/basics/routing/#動的ルーティング)を作成するオプションについてもっと読む。
...
MDXインテグレーションをインストールすると、[Astroコンポーネント](/ja/basics/astro-components/#コンポーネントのprops)と[UIフレームワークコンポーネント](/ja/basics/framework-components/#フレームワークコンポーネントを利用)の両方をMDX（`.mdx`）ファイルにインポートして、他のAstroコンポーネントと同じように使用できるようになります。
...
Markdown/MDXファイルを`src/pages/`ディレクトリに置いてページルートを作成する代わりに、ページを[動的に生成できます](/ja/basics/routing/#動的ルーティング)。
