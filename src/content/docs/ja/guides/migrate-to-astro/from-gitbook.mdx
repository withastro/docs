...
- GitBookサイトはオンラインダッシュボードで編集します。Astroでは[コードエディタ](/ja/editor-setup/)と開発環境でサイトを管理します。ローカルで開発することも、StackBlitzやCodeSandboxのようなクラウドエディタ/開発環境を選ぶこともできます。
...
同期完了後、AstroリポジトリにGitBookのコンテンツがコピーされます。今後GitBookとの同期が行われないようにするために、Git Syncを無効にしてください。
