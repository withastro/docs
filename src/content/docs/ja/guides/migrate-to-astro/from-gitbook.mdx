...
- どちらも何らかの[ファイルベースルーティング](/ja/guides/routing/)を採用しています。Astroのディレクトリー構造で既存コンテンツを管理しても違和感は少ないでしょう。
...
GitBookドキュメントサイトをAstroへ移行するには、まず[公式Starlightドキュメントテーマ](https://starlight.astro.build)スターターを使用するか、[テーマショーケース](https://astro.build/themes?search=&categories%5B%5D=docs)でコミュニティー製ドキュメントテーマを探してください。
...
Astroプロジェクトを準備したら、GitBookの[Git Sync機能](https://docs.gitbook.com/product-tour/git-sync)でコンテンツをGitHub/GitLabリポジトリーへ同期し、新しいAstroリポジトリーに取り込みます。

テンプレートのコンテンツコレクションに直接同期する場合は、`src/content/docs/en`または`src/content/docs`を同期先ディレクトリーに指定してください。

同期完了後、AstroリポジトリーにGitBookのコンテンツがコピーされます。今後の衝突を防ぐため、GitSyncは無効化してください。

:::caution
GitSyncを有効にするときは、必ず優先度を**"GitBook→GitHub"**に設定してください。これによりGitBookのコンテンツがGitHubリポジトリーへ同期され、誤って上書きするリスクを防げます。
:::
