...
:::note[クライアントディレクティブ]
アプリにインタラクティビティを持たせるには、[クライアントディレクティブ](/ja/reference/directives-reference/#client-directives)の指定が必要です。AstroはReactアプリをまず静的HTMLとして描画し、`client:～`が付いたコンポーネントだけをブラウザで実行します。
...
[既存アプリをAstroに追加](#cra-プロジェクトを-astro-に組み込む)したら、次はアプリ自体をAstroへ変換したくなるでしょう。
...
Reactアプリを変換する際には、どのReactコンポーネントを[Astroコンポーネントとして書き直すか](#jsx-ファイルから-astro-ファイルへの変換する)を決めます。制約は一つだけです。AstroコンポーネントはReactコンポーネントをインポートできますが、Reactコンポーネントは他のReactコンポーネントのみをインポートする必要があります。
