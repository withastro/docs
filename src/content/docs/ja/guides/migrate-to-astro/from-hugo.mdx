...
HugoブログをAstroへ移行するには、まず[公式ブログテーマスターター](https://astro.build/themes/blog/)を使うか、[テーマショーケース](https://astro.build/themes/)でコミュニティー製テーマを探してください。
...
ポートフォリオサイトやドキュメントサイトなど別のタイプのサイトを移行したい場合は、[astro.new](https://astro.new)で他の公式スターターテンプレートを参照してください。GitHubリポジトリーへのリンクや、IDX・StackBlitz・CodeSandbox・Gitpodで開発環境をすぐに立ち上げるためのリンクも用意されています。
