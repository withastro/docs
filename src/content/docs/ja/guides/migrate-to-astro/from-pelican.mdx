...
既存のMarkdownファイルを[Markdownページとして作成](/ja/guides/markdown-content/)し、Pelicanの`content/`ディレクトリーからAstroの`src/pages/`へコピーすることで、[ファイルベースルーティング](/ja/guides/routing/)を活用できます。
...
ポートフォリオサイトやブログサイトへの移行を検討している場合は、[astro.new](https://astro.new)で他のスターターテンプレートを確認できます。GitHubリポジトリーのリンクに加え、IDX・StackBlitz・CodeSandbox・Gitpodで即時起動できるリンクも用意されています。
