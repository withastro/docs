...
1. プロジェクトの設定ファイル`astro.config.mjs`を以下のように変更します。
    ...
    このコマンドで、Denoを使って本番のAstroサイトをローカルでプレビューできます。
...
もしプロジェクトがGitHubに保存されている場合は、[Deno Deploy website](https://dash.deno.com/)が、あなたのAstroサイトをデプロイするためのGitHub Actionsの設定方法を案内します。
...
1. [Deno Deploy CLI](https://deno.com/deploy/docs/deployctl)をインストールします。
...
1.（オプション）ビルドとデプロイを1つのコマンドに簡素化するには、`package.json`に`deploy-deno`スクリプトを追加します。
