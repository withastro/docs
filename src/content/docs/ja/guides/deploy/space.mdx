...
このガイドには、Spaceでサイトを構築するためのステップバイステップの手順が含まれています。[`@astrojs/node`アダプタ](/ja/guides/integrations-guide/node/)を使用した、静的なAstroサイトとサーバーサイドでレンダリングされたAstroサイトの両方がサポートされています。
...
Space CLIによって生成されたプロジェクトのルートにある`Spacefile`ファイルに以下の変更を加えてください。
	...
	3. Astroが生成した`dist`ディレクトリを指定します。
