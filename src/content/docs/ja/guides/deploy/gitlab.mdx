---
title: Astro サイトを GitLab ページにデプロイする
...
2. `astro.config.mjs` に `outDir:public` を設定します。この設定は、静的ビルドの出力をGitLab Pagesがファイルを公開するために必要なフォルダである `public` というフォルダに置くように Astro に指示します。
...
3. プロジェクトのルートに `.gitlab-ci.yml` というファイルを以下の内容で作成します。これにより、コンテンツを変更するたびにサイトをビルドしてデプロイすることができます。
   ...
   pages:
     ...
     only:
       # 以下のブランチにプッシュがあった場合のみ、新規ビルドとデプロイをトリガーする。
       - main
   ```
