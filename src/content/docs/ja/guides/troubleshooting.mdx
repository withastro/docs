...
まず、[`.astro`コンポーネントスクリプト](/ja/basics/astro-components/#the-component-script)または[`.mdx`ファイル](/ja/guides/markdown-content/#mdxでコンポーネントを使用)で**コンポーネントがインポート**しているかどうか確認します。
