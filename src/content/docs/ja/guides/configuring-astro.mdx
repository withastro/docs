...
## Astroの設定ファイル \{#the-astro-config-file\}
...
## サポートされている設定ファイルの種類 \{#supported-config-file-types\}
...
## 設定ファイルの指定 \{#config-file-resolving\}
...
## 相対ファイルの参照 \{#referencing-relative-files\}
...
## 出力するファイル名のカスタマイズ \{#customising-output-filenames\}
