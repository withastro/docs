...
Astroはファイル拡張子に基づいてコンポーネントを認識しレンダリングします。同じファイル拡張子を使用するフレームワーク（ReactとPreactなど）を区別するには、[複数のJSXフレームワークをレンダリングするための追加設定](/ja/guides/integrations-guide/react/#combining-multiple-jsx-frameworks)が必要です。
