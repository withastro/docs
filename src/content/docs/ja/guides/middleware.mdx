...
```
`next()`関数には、[Astro.rewrite()関数](/ja/reference/api-reference/#astrorewrite)と同じペイロードを渡せます。リライト先のパスは、文字列、URL、または`Request`として与えます。[sequence()](#ミドルウェアを連結する)により複数のミドルウェア関数を連結している場合、`next()`にパスを渡すと`Request`はその場で書き換えられ、ミドルウェアは再実行されません。チェーン内の次のミドルウェア関数は、更新された`context`をもつ新しい`Request`を受け取ります。
```js title="src/middleware.js"
