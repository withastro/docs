...
以下の例では、ミドルウェアを使用して「極秘情報」という文字列を「消去済み」という語に置き換えることで、変更されたHTMLをページにレンダリングできるようにします。
...
`src/middleware.js`からエクスポートされる必須の関数で、各ページやAPIルートのレンダリングの前に呼び出されます。2つのオプション引数、[context](#context)と[next()](#next)を受け取ります。`onRequest()`は、直接、または`next()`を呼び出すことで、`Response`を返す必要があります。
