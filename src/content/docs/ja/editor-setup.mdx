...
[Zed](https://zed.dev/)は速度と大規模なプロジェクトに最適化された、高性能なマルチプレイヤー・コードエディタです。Zedの[Astro 拡張機能](https://zed.dev/extensions/astro)には、`.astro`ファイルの構文ハイライト、コード補完、フォーマット、診断、定義へ移動などの機能が含まれています。
...
- Neovim [LSP](https://github.com/neovim/nvim-lspconfig/blob/master/doc/configs.md#astro)と[TreeSitter](https://github.com/virchau13/tree-sitter-astro) Plugins <span style="margin: 0.25em;"><Badge class="neutral-badge" text="Community" /></span> - Neovim向けにAstro用シンタックスハイライトやtreesitterパース、自動補完を提供します。
...
[dprint](https://dprint.dev/) は、JavaScript、TypeScript、CSSなど、多くの言語に対応したカスタマイズ性の高いコードフォーマッタです。[markup_fmt プラグイン](https://github.com/g-plane/markup_fmt)を使用することで、`.astro`ファイルのサポートを追加できます。
