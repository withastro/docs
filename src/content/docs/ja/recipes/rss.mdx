...
<ReadMore>完全な設定リファレンスは、[`@astrojs/rss` README](https://github.com/withastro/astro/tree/main/packages/astro-rss)を参照してください。</ReadMore>

## `items`の生成

これは、 `getCollection()` を使用してコンテンツコレクションのエントリから生成するか、 `pagesGlobToRssItems()` を使用してページファイルから生成することができます。
...
export async function GET(context) {
  ...
  return rss({
    ...
    items: blog.map((post) => ({
      ...
      // 投稿 `id` から RSS リンクを計算する
      // この例では、すべての投稿が `/blog/[id]` ルートとしてレンダリングされると仮定している。
      link: `/blog/${post.id}/`,
    })),
  });
}
