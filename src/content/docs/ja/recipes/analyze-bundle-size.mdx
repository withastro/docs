...
4. プロジェクトの`stats.html`ファイルを見つけます
    静的なサイトの場合、このファイルは`dist/`ディレクトリーのルートにあり、バンドルに含まれる内容を確認できます。
    
    Astroプロジェクトがオンデマンドレンダリングを使用している場合、2つの`stats.html`ファイルが生成されます。1つはクライアント用、もう1つはサーバー用で、それぞれ`dist/client`と`dist/server/`ディレクトリーのルートに配置されます。
