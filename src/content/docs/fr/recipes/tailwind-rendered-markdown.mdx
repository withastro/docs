...
1. Créez un composant `<Prose /> ` pour fournir une enveloppe `<div>` avec un `<slot />` pour votre Markdown rendu. Ajoutez la classe de style `prose` ainsi que les [modificateurs d'éléments Tailwind](https://tailwindcss.com/docs/typography-plugin#element-modifiers) dans l'élément parent.
    ...
    ```
    :::tip
    Le plugin `@tailwindcss/typographie` utilise des [**modificateurs d'éléments**](https://tailwindcss.com/docs/typography-plugin#element-modifiers) pour styliser les composants enfants d'un conteneur avec la classe `prose`. 
