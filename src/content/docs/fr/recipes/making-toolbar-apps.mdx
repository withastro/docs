...
## Créer une application de motivation pour la barre d'outils de développement \{#build-a-motivational-dev-toolbar-app\}
...
### Créer une intégration Astro \{#creating-the-astro-integration\}
...
2. Dans `my-integration.ts`, ajoutez le code suivant pour fournir à la fois le nom de votre intégration et [la fonction `addDevToolbarApp()`](/fr/reference/dev-toolbar-app-reference/#toolbar-app-integration-setup) requise pour ajouter votre application de barre d'outils avec le hook `astro:config:setup` :
...
### Créer l'application \{#creating-the-app\}
...
## Construire des applications avec un framework UI \{#building-apps-with-a-ui-framework\}
...
:::note 
Cependant que vous décidiez de créer votre application de barre d'outils en utilisant JavaScript ou un framework UI, vous aurez tout de même besoin de [créer l'intégration](#creating-the-astro-integration) qui ajoute l'application à la barre d'outils de développement.
:::

### Sans étape de construction \{#without-a-build-step\}
...
### Avec une étape de construction \{#with-a-build-step\}
