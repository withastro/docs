...
Les images et les conteneurs Docker peuvent être déployés sur de nombreuses plateformes différentes, comme AWS, Azure et [Google Cloud](/fr/guides/deploy/google-cloud/#cloud-run-ssr-and-static). Cette méthode ne couvre pas le déploiement de votre site sur une plateforme spécifique, mais vous montre comment configurer Docker pour votre projet.
...
## Création d'un Dockerfile \{#creating-a-dockerfile\}
...
### Ajouter un fichier .dockerignore \{#adding-a-dockerignore\}
...
### Construction en plusieurs étapes (à l'aide de SSR) \{#multi-stage-build-using-ssr\}
