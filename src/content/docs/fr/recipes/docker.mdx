...
Les images et les conteneurs Docker peuvent être déployés sur de nombreuses plateformes différentes, comme AWS, Azure et [Google Cloud](/fr/guides/deploy/google-cloud/#cloud-run-ssr-et-statique). Cette recette ne couvre pas le déploiement de votre site sur une plateforme spécifique, mais vous montre comment configurer Docker pour votre projet.
