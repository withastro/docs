...
## Pourquoi Nano Stores ? \{#why-nano-stores\}
...
- L'écriture d'un store à partir d'un fichier `.astro` ou [composant non hydraté](/fr/guides/framework-components/#hydrating-interactive-components) n'affectera *pas* la valeur reçue par les [composants côté client](/fr/reference/directives-reference/#client-directives).
...
## Installer Nano Stores \{#installing-nano-stores\}
...
## Exemple d'utilisation - Menu déroulant pour un panier de e-commerce \{#usage-example---ecommerce-cart-flyout\}
...
### Utilisation des "atoms" \{#using-atoms\}
...
### Utilisation des "maps" \{#using-maps\}
