...
Si vous préférez que la langue par défaut ne soit pas visible dans l'URL contrairement aux autres langues, vous trouverez [des instructions pour masquer la langue par défaut](/fr/recipes/i18n/#masquer-la-langue-par-défaut-dans-l-url) ci-dessous.
...
<ReadMore>En savoir plus sur [Collections de contenus](/fr/guides/content-collections/).</ReadMore>

3. Utilisez [dynamic routes](/fr/guides/routing/#routes-dynamiques) pour récupérer et rendre le contenu en fonction d'un paramètre `lang` et d'un paramètre `slug`.
...
<ReadMore>En savoir plus sur [dynamic routing](/fr/guides/routing/#routes-dynamiques).</ReadMore>
...
4. La fonction d'aide peut être utilisée pour obtenir un itinéraire traduit. Par exemple, si aucun itinéraire traduit n'est défini, l'utilisateur sera redirigé vers la page d'accueil :
...
- [Style de droite à gauche 101](https://rtlstyling.com/)
...
- [astro-i18n](https://github.com/alexandre-fernandez/astro-i18n) — Une bibliothèque d'internationalisation en premier lieu TypeScript pour Astro.
- [astro-i18n-aut](https://github.com/jlarmstrongiv/astro-i18n-aut) — Une intégration Astro pour i18n qui supporte le `defaultLocale` sans génération de page. L'intégration est indépendante de l'adaptateur et du cadre d'interface utilisateur.
