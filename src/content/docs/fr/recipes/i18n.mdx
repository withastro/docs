...
<ReadMore>Consultez la [section ressources](#resources) pour trouver des liens externes liés à de sujets tels que le style de droite-à-gauche (RTL) et le choix des balises de langue.</ReadMore>

## Méthode \{#recipe\}

### Créer des pages pour chaque langue \{#set-up-pages-for-each-language\}
...
2. Configurez `src/pages/index.astro` pour rediriger vers votre langue par défaut.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        Si vous utilisez un adaptateur SSR, vous pouvez utiliser [`Astro.redirect`](/fr/guides/routing/#dynamic-routes) pour rediriger vers la langue par défaut sur le serveur.
...
### Utiliser des collections pour le contenu traduit \{#use-collections-for-translated-content\}
...
3. Utilisez les [routes dynammiques](/fr/guides/routing/#dynamic-routes) pour récupérer et générer le contenu en fonction d'un paramètre `lang` et d'un paramètre `slug`.
    ...
    <ReadMore>En savoir plus sur les [routes dynamiques](/fr/guides/routing/#dynamic-routes).</ReadMore>
...
### Traduire les chaînes de l'UI \{#translate-ui-strings\}
...
### Permettre aux utilisateurs de passer d'une langue à l'autre \{#let-users-switch-between-languages\}
...
### Masquer la langue par défaut dans l'URL \{#hide-default-language-in-the-url\}
...
### Traduire les routes \{#translate-routes\}
...
## Bibliothèques communautaires \{#community-libraries\}
