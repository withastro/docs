...
## Code du tutoriel Construire un blog \{#build-a-blog-tutorial-code\}

Dans le [Tutoriel d'introduction à la création d'un blog](/fr/tutorial/0-introduction/), vous avez découvert le [routage intégré basé sur les fichiers](/fr/guides/routing/#static-routes) d'Astro : tout fichier `.astro`, `.md`, ou `.mdx` situé dans le dossier `src/pages/` devient automatiquement une page de votre site.
...
## Pages vs Collections \{#pages-vs-collections\}
...
## Extension du tutoriel sur le blog avec des collections de contenus \{#extending-the-blog-tutorial-with-content-collections\}
...
### Créer une collection pour vos articles de blog \{#create-a-collection-for-your-blog-posts\}
...
### Générer des pages à partir d'une collection \{#generate-pages-from-a-collection\}
...
8. Ajoutez le code suivant pour [interroger votre collection](/fr/guides/content-collections/#querying-collections) pour que le slug de chaque article de blog et le contenu de la page soient disponibles pour chaque page qu'il génèrera :
...
### Remplacer `Astro.glob()` avec `getCollection()` \{#replace-astroglob-with-getcollection\}
...
13. Le tutoriel du projet de blog génère aussi dynamiquement une page pour chaque tag en utilisant `src/pages/tags/[tag].astro` et affiche une liste de tag dans `src/pages/tags/index.astro`. 
        ...
        <Box icon="puzzle-piece">
          ### Essayez vous-même - Mettez à jour la requête dans la page Tag Index \{#try-it-yourself---update-the-query-in-the-tag-index-page\}

           Importez et utilisez `getCollection` pour récupérer les tags utilisés dans les articles du blog sur `src/pages/tags/index.astro`, en suivant les [mêmes étapes que ci-dessus](#replace-astroglob-with-getcollection).
...
### Mettez à jour les valeurs du frontmatter pour qu'elles correspondent à votre schéma. \{#update-any-frontmatter-values-to-match-your-schema\}
...
### Mettre à jour la fonction RSS \{#update-rss-function\}
