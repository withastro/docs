...
<PreCheck>
  ...
  - Ajouter des listeners d'événements pendant le processus de navigation pour déclencher des `<script>` lorsque c'est nécessaire
...
Pour naviguer entre ces pages, vous avez utilisé l'élément HTML standard `<a>`. Par exemple, pour créer un lien vers votre page "À propos", vous avez ajouté `<a href="/about/">About</a>` à l'en-tête de votre page. Lorsqu'un visiteur de votre site clique sur ce lien, le navigateur se rafraîchit et charge une nouvelle page avec un contenu entièrement nouveau.
...
3. La vue transite par le routeur...
    <MultipleChoice>
      <Option>
...
Les étapes ci-dessous vous montrent comment enrichir le produit final du didacticiel Construire un blog en ajoutant un routage côté client pour améliorer les transitions entre les pages.
...
6. Vérifier la présence d'un thème précédent pour éviter le clignotement en mode sombre.
    ...
    Pour vérifier et, si nécessaire, définir le mode sombre plus tôt dans le processus de navigation, créez une fonction qui s'exécutera en réponse à l'événement `astro:after-swap`. La fonction suivante pour vérifier le `localStorage` du navigateur pour le thème sombre sera exécutée **immédiatement après que la nouvelle page ait remplacé l'ancienne**, avant que les éléments DOM ne soient peints à l'écran.
...
9. Empêcher le routage côté client et demander au navigateur de se recharger lorsqu'il navigue vers votre page "About".
    ...
    Désormais, lorsque vous cliquez sur le lien de navigation vers votre page "About", aucune animation ne se produira. Les liens et le titre de la page ne glisseront pas et le contenu de la page ne s'affichera pas en fondu lorsque vous naviguerez vers votre page À propos **en utilisant ce lien**.

10. Ajoutez un lien vers votre page "About" à partir de votre nom d'auteur dans votre mise en page Markdown pour les articles de blog.
    
    `data-astro-reload` ne déclenche un rafraîchissement complet du navigateur que lorsque l'on accède à une nouvelle page **à partir du lien auquel il a été ajouté**. Il ne contrôle pas tous les cas où l'on navigue vers votre page "About".

  Dans votre composant `<MarkdownPostLayout />`, ajoutez un lien vers votre page "About" sur votre nom d'auteur :
    ...
    Si vous visitez un article de blog dans l'aperçu de votre navigateur et que vous cliquez sur le nom de l'auteur pour accéder à la page "About", à quoi ressemble la navigation dans la page ?
    
    <p>
      Lorsqu'un visiteur clique sur un lien vers la page "About" à partir d'un article de blog, le titre de la page et les liens de navigation de l'en-tête <Spoiler>glissent à travers l'écran</Spoiler> parce que <Spoiler>l'attribut `data-astro-reload` n'est pas défini sur ces liens.</Spoiler>.
    </p>
