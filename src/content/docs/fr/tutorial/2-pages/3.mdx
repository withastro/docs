...
<PreCheck>
  - Définissez le titre de votre page dans le frontmatter, et utilisez-le dans votre HTML
  - Affichez conditionnellement des éléments HTML
...
1. Ajoutez le code JavaScript suivant à votre script du frontmatter, entre les **délimiteurs de code** :
...
2. Ensuite, ajoutez le code suivant à votre modèle HTML, en dessous de votre contenu existant :
    ...
    ```
:::note[À retenir]
1. Rédiger un modèle Astro ressemble beaucoup à **rédiger du HTML**, mais vous pouvez inclure des expressions

 JavaScript à l'intérieur.
2. Le script du frontmatter Astro ne contient que du JavaScript.
3. Vous pouvez utiliser tous les **opérateurs logiques JavaScript**, les **expressions** et les **fonctions** modernes dans l'une ou l'autre section de votre fichier `.astro`. Mais, les accolades sont nécessaires (uniquement) dans le corps du modèle HTML.
:::
...
2. En plus du HTML, la syntaxe d'Astro vous permet d'inclure :
    ...
    <MultipleChoice>
        <Option isCorrect>Des opérateurs logiques JavaScript, des expressions et des fonctions</Option>
        <Option>YAML</Option>
...
- [Expressions dynamiques dans Astro](/fr/core-concepts/astro-syntax/#expressions-de-type-jsx)
