...
2. Si vous avez personnalisé vos billets de blog, remplacez les valeurs individuelles des balises (par exemple, "astro", "successes", "community", etc.) par les balises utilisées dans vos propres billets.
...
.glob()` pour recevoir des informations sur tous vos fichiers `.md` à transmettre à chaque route de page.
...
Vous avez maintenant un tableau `uniqueTags` avec les éléments `"astro"`, `"successes"`, `"community"`, `"blogging"`, `"setbacks"`, `"learning in public"`.
...
Accédez à `http://localhost:4321/tags/community` et vous devriez voir une liste uniquement de vos billets de blog avec la balise `community`. De même, `http://localhost:4321/tags/learning%20in%20public` devrait afficher une liste des billets de blog marqués `learning in public`.
...
- [Documentation de l'API `getStaticPaths()`](/en/reference/api-reference/#getstaticpaths)
