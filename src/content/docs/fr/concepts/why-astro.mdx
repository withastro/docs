...
- **[UI-agnostic](/fr/core-concepts/framework-components/)** : Prend en charge React, Preact, Svelte, Vue, Solid, Lit, HTMX, les composants web, et bien plus encore.
- **[Server-first](/fr/core-concepts/rendering-modes/)** : Déplace le rendu coûteux sur les appareils de vos visiteurs.
- **[Zéro JS, par défaut](/fr/core-concepts/astro-components/)** : Moins de JavaScript côté client pour ralentir votre site.
...
- **[Personnalisable](/fr/guides/guide-intégrations/)** : Tailwind, MDX, et des centaines d'intégrations à choisir.
...
L'approche "server-first" d'Astro vous permet d'opter pour un rendu côté client uniquement si, et exactement si, c'est nécessaire. Vous pouvez choisir d'ajouter des composants d'interface utilisateur qui s'exécutent sur le client. Vous pouvez tirer parti du routeur de transitions de vues d'Astro pour un contrôle plus fin des transitions et des animations de pages sélectionnées. Le rendu d'Astro au niveau du serveur, soit pré-rendu, soit à la demande, fournit des valeurs par défaut performantes que vous pouvez améliorer et étendre.
