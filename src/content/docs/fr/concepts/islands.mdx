...
description: >-
  Les îles Astro (alias Component Islands) sont un modèle d'architecture web dont
...
Dans Astro, vous pouvez utiliser n'importe quel framework d'interface utilisateur pris en charge (React, Svelte, Vue, etc.) pour rendre les îles dans le navigateur. Vous pouvez combiner différents frameworks sur la même page, ou simplement choisir votre préféré.
...
Mais parfois, le JavaScript côté client est nécessaire pour créer une interface utilisateur interactive. Au lieu de forcer votre page entière à devenir une application JavaScript à page unique (SPA), Astro vous demande de créer une île.
