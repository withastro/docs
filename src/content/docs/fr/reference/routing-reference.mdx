...
export async function getStaticPaths({ routePattern }: GetStaticPathsOptions) {
  ...
  // Appelez votre fonction utilitaire personnalisée avec `routePattern` pour générer les chemins statiques
...
- `options` - Objet facultatif avec les propriétés suivantes :
  ...
  - `format` - **Depuis la v7.1.0.** Une fonction qui permet de manipuler les URL calculées avant leur rendu.
