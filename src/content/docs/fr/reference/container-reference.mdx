...
Pour chaque intégration officielle d'Astro, importez et utilisez la fonction utilitaire `getContainerRenderer()` depuis le point d'entrée `container-renderer` qui lui est dédié afin d'exposer ses scripts de rendu client et serveur. Ces points d'entrée sont disponibles pour `@astrojs/react`, `@astrojs/preact`, `@astrojs/solid-js`, `@astrojs/svelte`, `@astrojs/vue` et `@astrojs/mdx`.
...
```astro title="src/components/Card.astro"
---
// Pour la prise en charge de TypeScript
interface Props {
  name: string;
};
...
const result = await container.renderToString(Card, {
  request: new Request("https://example.com/blog", {
    headers: {
      "x-some-secret-header": "valeur-test",
    },
  }),
});
```
...
{isAuthenticated ? <span>Vous êtes connecté</span> : <span>Vous êtes déconnecté</span>}
```
...
test("L'utilisateur est connecté", async () => {
  const result = await container.renderToString(Card, {
    locals: {
      checkAuth() {
        return true;
  ...
  // affirmer que le résultat contient « Vous êtes connecté »
});

test("L'utilisateur est déconnecté", async () => {
  const result = await container.renderToString(Card, {
    locals: {
      checkAuth() {
        return false;
  ...
  // affirmer que le résultat contient « Vous êtes déconnecté »
});
```
