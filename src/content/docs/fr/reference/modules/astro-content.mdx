...
```astro title="src/pages/[posts].astro" {10-11}
---
...
// si une entrée référencée est invalide, le tableau contiendra `undefined`.
...
const { entry: liveCollectionsPost } = await getLiveEntry(
  "blog",
  Astro.params.id,
);
...
const enterpriseRelatedPosts = await getEntries(
  enterprisePost.data.relatedPosts,
);
---
```
...
- `remarkPluginFrontmatter ` - L'objet frontmatter modifié après que tous les [modules d'extension pour processeur Markdown ont été appliqués](/fr/guides/markdown-content/#modification-programmatique-du-frontmatter). Définit sur le type `any`.
