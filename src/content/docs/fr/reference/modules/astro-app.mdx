...
Utilisez les mots clés appropriés lorsque vous [publiez votre adaptateur sur NPM](/fr/reference/publish-to-npm/#catégories) afin que votre adaptateur soit répertorié dans notre [catalogue des intégrations](https://astro.build/integrations/).
:::
...
Toutefois, si vous devez désactiver le streaming HTML (par exemple, si votre hébergeur prend uniquement en charge la mise en cache HTML non segmenté au niveau du CDN), vous pouvez désactiver le comportement par défaut en transmettant `streaming: false` à `createApp()` :
