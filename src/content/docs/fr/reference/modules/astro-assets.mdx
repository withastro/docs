...
Les propriétés prédéfinies correspondent à celles acceptées par le [composant `<Image />`](#image-), à l'exception de `alt` et `sizes`. Les propriétés suivantes utilisent des types différents.
