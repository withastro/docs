...
Si vous ne souhaitez pas utiliser l'un des [journaliseurs intégrés](#journaliseurs-intégrés), vous pouvez créer le vôtre.

Un journaliseur personnalisé se compose de deux parties :

- La [configuration du journaliseur](#la-configuration-du-journaliseur), qui permet à Astro de savoir quelle implémentation utiliser et quelle configuration transmettre
- L'[implémentation du journaliseur](#limplémentation-du-journaliseur), qui gère la logique de journalisation
...
La [configuration du journaliseur (`logger`)](/fr/reference/configuration-reference/#options-du-journaliseur) est un objet contenant un [point d'entrée (`entrypoint`)](/fr/reference/configuration-reference/#loggerentrypoint) obligatoire et une [`config`](/fr/reference/configuration-reference/#loggerconfig) facultative.
...
### L'implémentation du journaliseur

L'implémentation du journaliseur gère la logique de journalisation. Vous l'implémentez dans votre module de journaliseur en exportant une fonction par défaut qui prend la [configuration du journaliseur](#la-configuration-du-journaliseur) en paramètre et renvoie un [objet `AstroLoggerDestination`](#astrologgerdestination) avec une fonction `write()` requise.
...
```ts title="@org/journaliseur-personnalise/index.js"
import { matchesLevel } from "astro/logger";

/**
 * Exemple minimal d'une implémentation de journaliseur personnalisé.
 *
 * @param {Object} [options] - Les options du journaliseur.
 * @param {import("astro").AstroLoggerLevel} [options.level] - Le niveau minimum des journaux. Par défaut "info".
 * @returns {import("astro").AstroLoggerDestination} La destination du journaliseur personnalisé.
 */
