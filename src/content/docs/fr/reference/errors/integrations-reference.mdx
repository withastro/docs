...
- **Renderers:** [`lit`](/fr/guides/integrations-guide/lit/), [`svelte`](/fr/guides/integrations-guide/svelte/), [`react`](/fr/guides/integrations-guide/react/), [`preact`](/fr/guides/integrations-guide/preact/), [`vue`](/fr/guides/integrations-guide/vue/), [`solid`](/fr/guides/integrations-guide/solid-js/)
fr- **c:** [`sitemap`](/fr/guides/integrations-guide/sitemap/)
...
**Quand :** Lors de l'initialisation, avant que le [Vite](https://vitejs.dev/config/) ou le [Astro config](/fr/reference/configuration-reference/) n'aient été résolus.

**Pourquoi :** Pour étendre la configuration du projet. Cela comprend la mise à jour de la [configuration Astro](/fr/reference/configuration-reference/), l'application des [plugins Vite](https://vitejs.dev/guide/api-plugin.html), l'ajout de composants affichés et l'injection de scripts dans la page.
...
Une fonction de callback pour mettre à jour la [config Astro](/fr/reference/configuration-reference/) fournie par l'utilisateur. Toute configuration que vous fournissez **sera fusionnée avec la configuration de l'utilisateur + d'autres mises à jour de la configuration d'intégration**, vous êtes donc libre d'ignorer les clés !
...
- `"page-ssr"`: Importé en tant que module séparé dans le frontmatter de chaque composant de page Astro. Parce que cette étape importe votre script, le global `Astro` n'est pas disponible et votre script ne sera exécuté qu'une seule fois lorsque le `import` est évalué pour la première fois.
...
**Quand :** After a production build (SSG or SSR) has completed.

**Pourquoi :** To access generated routes and assets for extension (ex. copy content into the generated `/assets` directory). If you plan to transform generated assets, we recommend exploring the [Vite Plugin API](https://vitejs.dev/guide/api-plugin.html) and [configuring via `astro:config:setup`](#updateconfig-option) instead.
...
- `component` - cle chemin du fichier d'entrée par rapport à la racine du projet
...
c[La commande `astro add`](/fr/reference/cli-reference/#astro-add) permet aux utilisateurs d'ajouter facilement des intégrations et des adaptateurs à leur projet. Si vous voulez que _votre_ intégration soit installable avec cet outil, **ajoutez `astro-integration` au champ `keywords` dans votre `package.json`** :
