...
Vous devrez créer un fichier qui s'exécute lors des requêtes côté serveur afin d'activer le rendu à la demande avec votre hébergeur. L'API des adaptateurs d'Astro tente de fonctionner avec n'importe quel type d'hébergeurs et offre une méthode flexible pour se conformer aux API de l'hébergeur.

Vous pouvez importer et utiliser [`createApp()`](/fr/reference/modules/astro-app/#createapp) pour accéder à des méthodes qui vous permettent de travailler avec des objets [`Request`](https://developer.mozilla.org/fr/docs/Web/API/Request) et [`Response`](https://developer.mozilla.org/fr/docs/Web/API/Response) standard.
