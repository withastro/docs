...
Définit les URL résolues sur lesquelles le serveur de prévisualisation écoute, regroupées en `local` (adresses de bouclage) et `network` (adresses LAN et publiques). Astro les utilise pour enregistrer l'adresse du serveur dans le fichier de verrouillage [lorsque le serveur de prévisualisation s'exécute en arrière-plan](/fr/reference/cli-reference/#--background).
...
Indique l'[option d'ouverture configurée](/fr/reference/configuration-reference/#serveropen) pour le serveur de prévisualisation.
