...
Par exemple, certains hébergeurs serverless s'attendent à ce que vous exportiez une fonction `handler()`. Avec l'API des adaptateurs, vous y parvenez en implémentant `createExports()` dans votre point d'entrée de serveur :
