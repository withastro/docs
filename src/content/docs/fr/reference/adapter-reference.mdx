---
title: Adaptateur l'API de Astro
i18nReady: true
---
...
Un adaptateur est un type particulier d'[intégration](/fr/reference/integrations-reference/) qui fournit un point d'entrée pour l'affichage côté serveur. Un adaptateur a deux fonctions :
...
L'objet passé dans `setAdapter` est défini comme suit :
...
export interface AstroAdapterFeatures {
	...
	/**
	 * SSR uniquement. Chaque itinéraire devient sa propre fonction/fichier.
	 */
...
export type AstroFeatureMap = {
  ...
  /**
   * L'adaptateur est capable de servir des pages statiques ou affichées par le serveur.
   */
...
Fournissez une valeur pour [`routeData`](/fr/reference/integrations-reference/#routedata-type-reference) si vous connaissez déjà la route à afficher. Vous éviterez ainsi l'appel interne à [`app.match`](#appmatchrequest) pour déterminer la route à afficher.
...
- exécuter une validation spécifique ;
...
La configuration suivante indique à Astro que cet adaptateur dispose d'un support expérimental pour les actifs, mais qu'il n'est pas compatible avec les services intégrés Sharp et Squoosh :
...
et une erreur si le service utilisé pour les actifs n'est pas compatible avec l'adaptateur :
...
Un ensemble de fonctionnalités qui modifient la sortie des fichiers émis. Lorsqu'un adaptateur opte pour ces fonctionnalités, il obtiendra des informations supplémentaires à l'intérieur de crochets spécifiques.
...
Chaque fichier émis n'affichera qu'une seule page. Les pages seront émises dans un répertoire `dist/pages/` (ou sous un répertoire `/pages/` dans le répertoire spécifié pour [`outDir`](/fr/reference/configuration-reference/#outdir)), et les fichiers émis garderont les mêmes chemins de fichiers du répertoire `src/pages/`.
...
Chacun de ces itinéraires est soumis à un [démarrage à froid](https://azure.microsoft.com/fr-fr/blog/understanding-serverless-cold-start/) lorsque le gestionnaire s'exécute, ce qui peut entraîner un certain retard. Ce retard est influencé par différents facteurs.
  ...
  return {
    ...
    hooks: {
      ...
      'astro:build:ssr': ({ middlewareEntryPoint }) => {
          // n'oubliez pas de vérifier si cette propriété sort, elle sera `undefined` si l'adaptateur n'a pas opté pour la fonctionnalité
