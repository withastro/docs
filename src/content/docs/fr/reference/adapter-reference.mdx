...
interface AstroAdapter {
	...
	client?: {
		/**
		 * En-têtes à injecter dans les demandes de récupération internes d'Astro (actions, transitions de vue, îlots de serveur, préchargement).
		 * Peut être un objet d'en-têtes ou une fonction qui renvoie des en-têtes.
		 */
		internalFetchHeaders?: Record<string, string> | (() => Record<string, string>);
		/**
		 * Paramètres de requête à ajouter à toutes les URL de ressources (images, feuilles de style, scripts, etc.).
		 * Utile pour les adaptateurs qui doivent suivre les versions de déploiement ou d'autres métadonnées.
		 */
