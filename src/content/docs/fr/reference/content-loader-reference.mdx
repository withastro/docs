...
Ce chargeur accepte un objet avec les propriétés suivantes : `pattern`, `base` (optionnel), `generateId` (optionnel), `retainBody` (optionnel) et `deferRender` (optionnel).
...
Détermine s'il faut différer le rendu des entrées Markdown jusqu'à ce qu'elles soient affichées sur une page. Lorsque la valeur est `false` (valeur par défaut), leur rendu a lieu immédiatement lors de la synchronisation du contenu. Cette option ne s'applique pas à MDX, à Markdoc ni aux entrées de données (p. ex., JSON ou YAML).

Par défaut, le chargeur `glob()` effectue le rendu de chaque entrée Markdown lors de la synchronisation et stocke le HTML résultant dans le magasin de données. Cela permet de mettre en cache le contenu rendu et de le réutiliser lors des compilations ultérieures. Cependant, pour les grandes collections dont le rendu est beaucoup plus volumineux que la source, le stockage simultané de tout ce code HTML rendu en mémoire peut entraîner une saturation de la mémoire lors de la compilation.

Lorsque `deferRender` est définie sur `true`, le rendu est différé jusqu'à ce que chaque entrée soit effectivement affichée sur une page, en utilisant le même mécanisme de rendu à la demande que celui déjà employé par les fichiers `.mdx`. Cela permet de limiter l'utilisation de la mémoire pendant la commande `astro build`, au prix du fait que le HTML rendu n'est plus mis en cache dans le magasin de données.
...
const docs = defineCollection({
  /* Différer le rendu d'une grande collection Markdown pour éviter une saturation de la mémoire pendant la compilation. */
  loader: glob({
    pattern: "**/*.md",
    base: "./src/data/docs",
    deferRender: true,
...
```ts title="mon-chargeur.ts" {11-14}
import type { LiveLoader } from "astro/loaders";
...
export function myLoader(config: {
  apiKey: string;
}): LiveLoader<MyData, never, never, MyLoaderError> {
  return {
    name: "mon-chargeur",
...
```astro title="src/pages/boutique.astro"
---
export const prerender = false; // Pas nécessaire en mode `server`

import { getLiveEntry } from 'astro:content';
import { MyLoaderError } from "../mon-chargeur";
...
Vous pouvez ensuite utiliser ces indications dans vos pages. Si vous avez configuré un [fournisseur de cache](/fr/guides/caching/#configurer-la-mise-en-cache), transmettez directement les indications de cache à [`Astro.cache.set()`](/fr/reference/api-reference/#cacheset) :
...
:::note
Les indications de cache n'entraînent pas automatiquement la mise en cache de la réponse par Astro. Elles fournissent des valeurs que vous pouvez transmettre à la [mise en cache des routes](/fr/guides/caching/) ou utiliser pour implémenter votre propre stratégie de mise en cache.
:::

## Distribuer votre chargeur

Les chargeurs peuvent être définis dans votre site ou sous forme de paquet npm distinct. Si vous souhaitez partager votre chargeur avec la communauté, vous pouvez [le publier sur npm avec les mots-clés `withastro` et `astro-loader`](/fr/guides/integrations/#données-du-fichier-packagejson).
...
```ts title="src/chargeur-de-flux.ts" ins={27-35}
import type { Loader } from "astro/loaders";
...
```ts title="Extrait du chargeur file()" {15} "logger"
import type { Loader } from "astro/loaders";
...
```ts title="Extrait du chargeur file()" {9} "config"
import type { Loader } from "astro/loaders";
import { fileURLToPath } from "node:url";
import { syncData } from "./helpers";
...
export function myLoader(settings: { apiKey: string }) {
  return {
    name: "chargeur-cms",
    async load({ renderMarkdown, store }) {
      const entries = await loadFromCMS(settings.apiKey);
...
```ts title=loader.ts {19}
import type { Loader } from "astro/loaders";
import { readFile } from "node:fs/promises";
import { loadLocalMarkdown } from "./helpers";

export function myLoader() {
  return {
    name: "chargeur-personnalise",
    async load({ config, renderMarkdown, store }) {
      const files = await loadLocalMarkdown();
...
```ts title="Extrait du chargeur file()" {13-18}
import type { Loader } from "astro/loaders";
import { fileURLToPath } from "node:url";
import { syncData } from "./helpers";
...
export function file(fileName: string) {
  return {
    name: "file-loader",
    load: async ({ config, logger, store, watcher }) => {
      const url = new URL(fileName, config.root);
      const filePath = fileURLToPath(url);
      await syncData(filePath, store);
...
Décrit le frontmatter brut, analysé à partir du fichier. Cela peut inclure [des données injectées par programmation à partir de modules d'extension pour processeur Markdown](/fr/guides/markdown-content/#modification-programmatique-du-frontmatter).
...
```ts title="src/cms-loader.ts" collapse={1-4, 19-21} {14-16}
import type { LiveLoader } from "astro/loaders";
import { loadRemotePosts } from "./cms";
import type { Post, PostEntryFilter, PostCollectionFilter } from "./types";

export function myLoader(config: {
  apiKey: string;
}): LiveLoader<Post, PostEntryFilter, PostCollectionFilter> {
  return {
    name: "chargeur-cms",
    loadCollection: async ({ filter }) => {
      const posts = await loadRemotePosts(config.apiKey, filter);
      return {
        entries: posts,
        cacheHint: {
...
```ts title="src/store-loader.ts" collapse={1-8, 15-21} {27-29}
import type { LiveLoader } from "astro/loaders";
...
export function myLoader(): LiveLoader<
  Product,
  ProductEntryFilter,
  ProductCollectionFilter
> {
  return {
    name: "chargeur-boutique",
    loadCollection: async ({ filter }) => {
      const products = await loadStoreProducts(filter);
      return {
        entries: products,
    ...
    loadEntry: async ({ filter }) => {
      const product = await loadStoreProduct(filter);
      return {
        id: product.id,
        data: product,
        cacheHint: {
