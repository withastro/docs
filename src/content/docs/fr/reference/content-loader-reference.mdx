...
Vous pouvez ensuite utiliser ces indications dans vos pages. Si vous avez activé la [mise en cache expérimentale des routes](/fr/reference/route-caching/), transmettez directement les indications de cache à `Astro.cache.set()` :
...
:::note
Les indications de cache n'entraînent pas automatiquement la mise en cache de la réponse par Astro. Elles fournissent des valeurs que vous pouvez transmettre à la [mise en cache des routes](/fr/reference/route-caching/) ou utiliser pour implémenter votre propre stratégie de mise en cache.
:::
