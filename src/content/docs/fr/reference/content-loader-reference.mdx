...
* `base` - le répertoire de base [URL](https://developer.mozilla.org/fr/docs/Web/API/URL)
* `data` - les données analysées et non validées de l'entrée

Par défaut, il utilise [`github-slugger`](https://github.com/Flet/github-slugger) pour générer un slug avec des mots avec [kebab-cased](https://developer.mozilla.org/en-US/docs/Glossary/Kebab_case).
...
const products = defineCollection({
  /* Récupère toutes les entrées d'un fichier CSV à l'aide d'un interprète personnalisé. */
  loader: file("src/data/products.csv", {
    parser: (fileContent) => { /* votre interprète logique */ },
  }),
  schema: /* ... */
});
```
