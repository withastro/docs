...
Active `src/fetch.ts` comme point d'entrée personnalisé du pipeline de requêtes, vous donnant un contrôle total sur la manière dont Astro gère les requêtes entrantes.
...
## Création de `src/fetch.ts`

Créez un fichier `src/fetch.ts` (ou `.js`, `.mjs`, `.mts`) qui exporte par défaut un objet doté d'une méthode `fetch`. La méthode `fetch` reçoit un objet [Request](https://developer.mozilla.org/fr/docs/Web/API/Request) standard et doit renvoyer un objet [Response](https://developer.mozilla.org/fr/docs/Web/API/Response).

Si aucun fichier `src/fetch.ts` n'existe, Astro utilise son pipeline intégré, qui exécute automatiquement toutes les fonctionnalités.
...
Par défaut, Astro recherche `src/fetch.ts` comme point d'entrée du routage avancé. Définissez l'option `fetchFile` pour utiliser un fichier différent :
...
Avec cette configuration, Astro recherchera `src/handler.ts` au lieu de `src/fetch.ts`.
...
Enveloppe un rappel de rendu avec la logique du fournisseur de [cache](/fr/reference/route-caching/). Gère la mise en cache à l'exécution, les fournisseurs basés sur le CDN et le cas sans cache.
...
Astro fournit également des enveloppes compatibles avec Hono pour toutes les fonctions de gestion via `astro/hono`. Si vous préférez utiliser [Hono](https://hono.dev/) comme framework de routage, vous pouvez exporter une application Hono depuis `src/fetch.ts` :
