...
:::caution
Cette propriété est obsolète depuis la version 5.0. Consultez le [guide de migration](/fr/guides/upgrade-to/v5/#deprecated-routes-on-astrobuilddone-hook-integration-api).
:::
...
Contient les URL vers les chemins des fichiers de sortie, regroupés par al propriété `pattern` de [`IntegrationResolvedRoute`](#référence-de-type-integrationresolvedroute).
...
**Pourquoi :** Pour définir les options d'une route au moment de la construction ou de la demande, comme l'activation du [rendu du serveur à la demande](/fr/guides/on-demand-rendering/#enabling-on-demand-rendering).
