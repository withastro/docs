...
Un objet `Map` avec une liste de pages comme nom de propriété et [leurs données de compilation](#données-de-compilation-dune-page) comme valeur.
...
#### Données de compilation d'une page

Un objet décrivant comment compiler une page.
...
**Type :** [`SerializedSSRManifest`](#propriétés-sérialisées-du-manifeste-ssr)
</p>
...
##### Propriétés sérialisées du manifeste SSR

Une version sérialisée de [`SSRManifest`](#ssrmanifest), accessible via le [hook `astro:build:ssr`](#astrobuildssr). Elle contient les mêmes informations que `SSRManifest`, avec certaines propriétés ayant été converties en formats sérialisables.
...
Définit une liste d'informations de routage sérialisées. Chaque route contient les mêmes propriétés que [`SSRManifest.routes`](#routes-1), avec `routeData` converti au format sérialisable JSON.
...
Spécifie les données de la route (`RouteData`) vers laquelle rediriger lorsque [`RouteData.type`](#type) est `redirect`.
...
Définit une correspondance entre les identifiants des composants et leurs métadonnées de compilation. Chaque entrée contient des informations sur le comportement de [`propagation`](#propagation) et indique si elle contient des éléments d'en-tête.
...
Un objet décrivant la configuration de la stratégie de sécurité du contenu.
...
Décrit la [configuration de la stratégie de sécurité du contenu](/fr/reference/configuration-reference/).
...
Spécifie la [fonction de hachage configurée](/fr/reference/configuration-reference/).
...
Spécifie une liste des hachages générés pour les scripts du projet et des [hachages fournis par l'utilisateur](/fr/reference/configuration-reference/) pour les scripts externes.
...
Spécifie une liste de sources valides combinant les [ressources de script configurées](/fr/reference/configuration-reference/) et les [ressources de script injectées](/fr/reference/api-reference/).
...
Détermine si la prise en charge de l'[injection de script dynamique est activée dans la configuration](/fr/reference/configuration-reference/).
...
Spécifie une liste des hachages générés pour les styles du projet et des [hachages fournis par l'utilisateur](/fr/reference/configuration-reference/) pour les styles externes.
...
Spécifie une liste de sources valides combinant les [ressources de style configurées](/fr/reference/configuration-reference/) et les [ressources de style injectées](/fr/reference/api-reference/).
...
Spécifie la [liste configurée des sources valides](/fr/reference/configuration-reference/) pour des types de contenu spécifiques.
...
Lorsque la valeur de [`RouteData.type`](#type) est `redirect`, la valeur contiendra les données `IntegrationRouteData` de la route vers laquelle rediriger. Sinon, la valeur sera indéfinie.
