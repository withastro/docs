...
Cette fonction accepte soit un objet [`AstroPrerenderer`](#astroprerenderer) directement, soit une fonction de fabrique qui reçoit le prérendu par défaut et en renvoie un personnalisé. Ceci est utile lorsque vous souhaitez encapsuler ou étendre le comportement par défaut.
...
Les [environnements Vite](https://vite.dev/guide/api-environment) d'Astro vous permettent d'optimiser les modules d'extension Vite de votre intégration pour différents environnements. L'une des principales utilisations des environnements Vite est la possibilité d'exécuter et de configurer conditionnellement les modules d'extension Vite de votre intégration :
...
Définit une méthode facultative décrivant comment afficher une page. Celle-ci sera appelée par Astro pour chaque chemin renvoyé par [`getStaticPaths()`](/fr/reference/routing-reference/#getstaticpaths).
...
:::caution[Dépréciée]
Utilisez plutôt [`scriptDirective`](#ssrmanifestcspscriptdirective). Cette propriété ne répertorie que les hachages associés à la directive générique `script-src`. Elle sera supprimée dans une future version majeure.
:::
...
:::caution[Dépréciée]
Utilisez plutôt [`scriptDirective`](#ssrmanifestcspscriptdirective). Cette propriété ne répertorie que les sources associées à la directive générique `script-src`. Elle sera supprimée dans une future version majeure.
:::
...
:::caution[Dépréciée]
Utilisez plutôt [`scriptDirective.strictDynamic`](#ssrmanifestcspscriptdirective). Cette propriété sera supprimée dans une future version majeure.
:::
...
Décrit les sources, les hachages et la prise en charge de [`'strict-dynamic'`](/fr/reference/configuration-reference/#securitycspscriptdirectivestrictdynamic) pour la famille de directives `script-src`. Chaque source ou hachage peut cibler une directive plus spécifique via sa propriété `kind`. Utilisez `'element'` pour `script-src-elem`, `'attribute'` pour `script-src-attr`, ou `'default'` (identique à une chaîne de caractères simple) pour `script-src`.

##### `SSRManifest.csp.styleHashes`

:::caution[Dépréciée]
Utilisez plutôt [`styleDirective`](#ssrmanifestcspstyledirective). Cette propriété ne répertorie que les hachages associés à la directive générique `style-src`. Elle sera supprimée dans une future version majeure.
:::
...
:::caution[Dépréciée]
Utilisez plutôt [`styleDirective`](#ssrmanifestcspstyledirective). Cette propriété ne répertorie que les sources associées à la directive générique `style-src`. Elle sera supprimée dans une future version majeure.
:::
...
Décrit les sources et les hachages pour la famille de directives `style-src`. Chaque source ou hachage peut cibler une directive plus spécifique via sa propriété `kind`. Utilisez `'element'` pour `style-src-elem`, `'attribute'` pour `style-src-attr`, ou `'default'` (identique à une chaîne de caractères simple) pour `style-src`.
