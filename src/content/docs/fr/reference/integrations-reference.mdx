...
## Référence rapide de l'API \{#quick-api-reference\}
...
### `astro:config:setup` \{#astroconfigsetup\}
...
#### Option `isRestart` \{#isrestart-option\}
...
#### Option `updateConfig` \{#updateconfig-option\}
...
#### Option `addRenderer` \{#addrenderer-option\}
...
#### Option `addWatchFile` \{#addwatchfile-option\}
...
#### Option `addClientDirective` \{#addclientdirective-option\}
...
Ajoute une [directive client personnalisée](/fr/reference/directives-reference/#custom-client-directives) à utiliser dans les fichiers `.astro`.
...
#### Option `addDevToolbarApp` \{#adddevtoolbarapp-option\}
...
```
#### Option `addMiddleware` \{#addmiddleware-option\}
...
#### Option `injectRoute` \{#injectroute-option\}

**Type :** `({ pattern: string, entrypoint: string }) => void;`

Une fonction de rappel pour injecter des routes dans un projet Astro. Les routes injectées peuvent être des [pages `.astro`](/fr/basics/astro-pages/) ou des [gestionnaires de routes `.js` et `.ts`](/fr/guides/endpoints/#static-file-endpoints).
...
#### Option `injectScript` \{#injectscript-option\}
...
#### Option `setAdapter` \{#setadapter-option\}
...
#### Options `injectTypes` \{#injecttypes-options\}
...
### `astro:server:setup` \{#astroserversetup\}
...
### `astro:server:start` \{#astroserverstart\}
...
### `astro:server:done` \{#astroserverdone\}
...
### `astro:build:start` \{#astrobuildstart\}
...
### `astro:build:setup` \{#astrobuildsetup\}
...
### `astro:build:generated` \{#astrobuildgenerated\}
...
**Pourquoi :** Pour accéder aux routes et aux actifs générés pour l'extension (ex. copier le contenu dans le répertoire `/assets` généré). Si vous envisagez de transformer les actifs générés, nous vous recommandons d'explorer [Vite Plugin API](https://vitejs.dev/guide/api-plugin.html) et [configurer via `astro:config:setup`](#updateconfig-option) à la place.

```js
'astro:build:done'?: (options: { dir: URL; routes: RouteData[], pages: { pathname: string }[] }) => void | Promise<void>;
```

#### Option `dir` \{#dir-option\}
...
##### Référence du type `RouteData` \{#routedata-type-reference\}
...
### `astro:route:setup` \{#astroroutesetup\}
...
**Pourquoi :** Pour définir les options d'une route au moment de la construction ou de la demande, comme l'activation du [rendu du serveur à la demande](/fr/guides/server-side-rendering/#opting-in-to-pre-rendering-in-server-mode).
...
##### `route.prerender` \{#routeprerender\}
...
## Autoriser l'installation avec `astro add` \{#allow-installation-with-astro-add\}
...
## Commande d'intégration \{#integration-ordering\}
...
## Combiner les intégrations dans des préréglages \{#combine-integrations-into-presets\}
