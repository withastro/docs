...
```ts title="src/mon-appli.js" {5-10}
import { defineToolbarApp } from "astro/toolbar";
  ...
  init(canvas, app) {
    app.onToggled(({ state }) => {
      const text = document.createTextNode(
        `L'application est maintenant ${state ? "activée" : "désactivée"} !`,
      );
      canvas.appendChild(text);
    });
  },
...
export default defineToolbarApp({
  ...
  beforeTogglingOff() {
    const confirmation = window.confirm(
      "Êtes-vous sûr de vouloir désactiver cette application ?",
    );
    return confirmation;
  },
});
...
```ts title="src/mon-appli.js" {5-9} "server" "serveur"
import { defineToolbarApp } from "astro/toolbar";
