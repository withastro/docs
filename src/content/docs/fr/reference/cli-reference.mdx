...
<ReadMore>Consultez le [mode arrière-plan pour les agents de codage IA](/fr/guides/build-with-ai/#background-mode) pour en savoir plus sur la détection automatique des agents et le point de terminaison d'état de santé.</ReadMore>
...
Affiche les nouvelles entrées du journal au fur et à mesure de leur écriture, à l'instar de `tail -f`. Sans cette option, le contenu actuel du fichier journal est affiché et la commande se termine.
