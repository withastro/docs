...
La commande `astro preview` peut être combinée avec les [options communes](#common-flags) documentées ci-dessous pour contrôler davantage l'expérience de prévisualisation.
...
Vide le [cache de la couche de contenu](/fr/guides/content-collections/#définir-le-chargeur-de-collection-loader), forçant une nouvelle compilation complète.

### `--mode <string>`

<p><Since v="5.0.0" /></p>

Définit sur place la configuration de [`mode`](/fr/reference/programmatic-reference/#mode) pour votre projet.
