...
### Commandes `astro` \{#astro-commands\}
...
### Scripts `package.json` \{#packagejson-scripts\}
...
Peut être combiné avec les [options courantes](#common-flags) documentées ci-dessous.
...
<ReadMore>En savoir plus sur la [vérification des types dans Astro](/fr/guides/typescript/#type-checking).</ReadMore>
...
## Options courantes \{#common-flags\}

### `--root <chemin>` \{#--root-path\}
...
### `--host [adresse d'hôte optionnelle]` \{#--host-optional-host-address\}
...
## API avancées (expérimentales) \{#advanced-apis-experimental\}
...
Lien vers votre base de données hébergée dans Studio. Ce lien est nécessaire pour exécuter les [commandes Astro DB](/fr/guides/integrations-guide/db/#astro-db-cli-reference) pour la gestion de la base de données. Pour lier une base de données, vous serez invité à sélectionner une base de données existante ou à en créer une nouvelle.
