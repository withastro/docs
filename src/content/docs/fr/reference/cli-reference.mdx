...
La commande peut être combinée avec les [options communes](#options-communes) et les [sous-commandes communes](#sous-commandes-communes) pour contrôler davantage l'expérience de développement.
...
La commande accepte les [options communes](#options-communes) et les options supplémentaires suivantes :
...
La commande peut être combinée avec les [options communes](#options-communes) et les [sous-commandes communes](#sous-commandes-communes) pour contrôler davantage l'expérience de prévisualisation.
...
Configure la télémétrie pour l'utilisateur actuel de la CLI. La télémétrie est une donnée anonyme qui fournit à l'équipe Astro des informations sur les fonctionnalités Astro les plus souvent utilisées. Pour en savoir plus, voir la [page télémetrie d'Astro](https://astro.build/telemetry/).
...
Les commandes [`astro dev`](#astro-dev) et, depuis la version v7.2.0, [`astro preview`](#astro-preview) acceptent les sous-commandes suivantes pour gérer un [serveur en arrière-plan](#--background).

### `stop`

Arrête un serveur s'exécutant en arrière-plan. Envoie un signal `SIGTERM` et attend jusqu'à 5 secondes que le serveur s'arrête proprement. Si le processus est toujours en cours d'exécution après ce délai, il envoie un signal `SIGKILL`.

### `status`

Vérifie si un serveur en arrière-plan est en cours d'exécution et affiche son URL, son PID et sa durée de fonctionnement.

### `logs`

Affiche les journaux d'un serveur en arrière-plan. Ne fonctionne que lorsque le serveur a été démarré avec l'option [`--background`](#--background), puisque les serveurs exécutés au premier plan écrivent les journaux directement dans le terminal.
...
Démarre le serveur de développement, ou le serveur de prévisualisation depuis la v7.2.0, en tant que processus détaché en arrière-plan et active la [journalisation au format JSON](#--json).

Lorsque le serveur démarre, Astro écrit un fichier de verrouillage (`.astro/dev.json` ou `.astro/preview.json`) pour enregistrer l'URL, le port et le PID du serveur. Cela évite de lancer plusieurs instances du serveur pour le même projet.

Cette option est fournie automatiquement lorsqu'un agent IA est détecté. Vous pouvez également l'utiliser manuellement :
...
Empêche la vérification de l'existence d'un fichier de verrouillage et la nécessité d'en écrire un. Cela permet de démarrer un nouveau serveur de développement ou, depuis la v7.3.0, un serveur de prévisualisation parallèlement à un serveur déjà en cours d'exécution, au lieu de générer une erreur.

```shell
astro dev --ignore-lock --port 4322
```

Le nouveau serveur n'est pas suivi par les [sous-commandes communes](#sous-commandes-communes).

Lorsqu'elle est combinée avec [`--background`](#--background) ou [`--force`](#--force-string), une erreur est générée, car les deux dépendent du fichier de verrouillage.
