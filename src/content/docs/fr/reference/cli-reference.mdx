...
Compile votre site pour le déploiement. Par défaut, cela générera des fichiers statiques et les placera dans un répertoire `dist/`. Si le [SSR est activé](/fr/guides/on-demand-rendering/), cela générera les fichiers serveur nécessaires à votre site.
