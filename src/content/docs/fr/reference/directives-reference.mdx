...
📚 Consultez la page [CSS et Stylisation](/fr/guides/styling/#styles-globaux) pour plus de détails sur le fonctionnement des styles globaux.
...
`is:inline` indique à Astro de laisser la balise `<script>` ou `<style>` telle quelle dans la sortie HTML finale. Le contenu ne sera ni traité, optimisé ou regroupé. Cela limite certaines fonctionnalités d'Astro, comme l'importation d'un paquet npm ou l'utilisation d'un langage de compilation vers CSS comme Sass.
...
📚 Découvrez comment les [scripts côté client](/fr/guides/client-side-scripts/) fonctionnent dans les composants Astro.
