...
Définissez `session.ttl` pour ajouter une période d'expiration par défaut pour vos valeurs de session. Passer une option `ttl` à [`session.set()`](/en/reference/api-reference/#set) remplacera la valeur globale par défaut pour cette entrée individuelle.
