...
{/* TODO(armand): add back the missing link to API reference */}
Lorsque des ressources sont insérées plusieurs fois ou à partir de plusieurs sources (par exemple définies dans votre configuration `csp` et ajoutées à l'aide de l'API d'exécution de CSP), Astro fusionnera et dédupliquera toutes les ressources pour créer votre élément `<meta>`.
...
La source de vos fichiers de polices. Vous pouvez utiliser un [fournisseur intégré](/fr/reference/font-provider-reference/#built-in-providers) ou créer votre propre [fournisseur personnalisé](/fr/reference/font-provider-reference/#building-a-font-provider) :
