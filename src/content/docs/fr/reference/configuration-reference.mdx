...
L'exemple suivant définit le répertoire racine en utilisant la CLI :
...
Depuis la v7.0, Astro applique par défaut les règles d'espacement de JSX utilisées par des frameworks comme React. Cela supprime les espaces et les sauts de ligne autour des éléments, condense le texte multiligne sur une seule ligne et préserve les espaces à l'intérieur d'une même ligne (par exemple, un espace entre deux éléments en ligne). Pour conserver un espace qui serait autrement supprimé, incluez-le explicitement dans le code source à l'aide de constructions telles que `{" "}`.

Définir cette option sur `true` supprime les espaces, y compris les sauts de ligne, des composants `.astro` sans être destructif. Certains espaces peuvent être conservés si nécessaire pour maintenir le rendu visuel de votre HTML.
...
- Lorsque `'unsafe-inline'` est inclus comme ressource dans une directive, Astro ne génère pas de hachages pour cette directive. Selon la spécification CSP, les navigateurs ignorent `'unsafe-inline'` lorsqu'un hachage ou un nonce est présent dans la même directive, donc Astro supprime les hachages pour préserver le comportement de `'unsafe-inline'`. Notez que cela réduit la sécurité de cette directive au niveau de `'unsafe-inline'`.
...
- `preserve` - La valeur d'`Astro.url.pathname` correspond au fichier généré pour chaque page : les pages d'index incluent une barre oblique finale (par exemple `/foo/`), tandis que les pages autres que les pages d'index incluent `.html` (par exemple `/foo.html`).
...
Par défaut, il s'agit de `'fetch'`, ce qui signifie qu'Astro recherche `src/fetch.ts` (ou `.js` / `.mjs` / `.mts`).

Le fichier fetch permet de composer le pipeline de requêtes d'Astro avec la norme Web Fetch ou votre propre middleware Hono.
...
Le point d'entrée pour l'[implémentation du journaliseur](/fr/reference/logger-reference/#limplémentation-du-journaliseur).
Il peut s'agir d'un paquet npm, d'un chemin relatif à la racine de votre projet ou d'une `URL` pointant vers un fichier de votre projet :
...
Le point d'entrée du journaliseur doit être un fichier JavaScript. TypeScript n'est pas pris en charge. Si Astro ne parvient pas à charger l'implémentation, il se replie vers son journaliseur par défaut.
...
Depuis Astro v7.2.0, vous pouvez choisir de désactiver la prise en charge des sessions en définissant l'option sur `false`. Lorsque les sessions sont désactivées, le moteur d'exécution des sessions est exclu du paquet compilé SSR, et les adaptateurs ne configurent pas leur pilote de session par défaut. Cela est utile pour les environnements serverless et en périphérie où le temps d'analyse du paquet compilé est critique.
...
:::caution[Dépréciée]
Cette propriété est dépréciée et sera supprimée dans une future version majeure. Transmettez les modules d'extension au [processeur Markdown (`markdown.processor`)](#markdownprocessor) configuré à la place.

Apprenez-en davantage sur la [configuration d'un processeur Markdown](/fr/guides/markdown-content/#configurer-un-processeur-markdown) et l'[utilisation de modules d'extension](/fr/guides/markdown-content/#modules-dextension-pour-les-processeurs-markdown) dans le guide Markdown.
:::
...
:::caution[Dépréciée]
Cette propriété est dépréciée et sera supprimée dans une future version majeure. Transmettez `gfm` au [processeur Markdown (`markdown.processor`)](#markdownprocessor) configuré à la place.

Apprenez-en davantage sur la [configuration d'un processeur Markdown](/fr/guides/markdown-content/#configurer-un-processeur-markdown) et l'[utilisation de GitHub-flavored Markdown](/fr/guides/markdown-content/#github-flavored-markdown) dans le guide Markdown.
:::
...
:::caution[Dépréciée]
Cette propriété est dépréciée et sera supprimée dans une future version majeure. Transmettez-la au [processeur Markdown (`markdown.processor`)](#markdownprocessor) configuré à la place. Utilisez `smartypants` pour `unified()` ou `smartPunctuation` pour `satteri()`.

Apprenez-en davantage sur la [configuration d'un processeur Markdown](/fr/guides/markdown-content/#configurer-un-processeur-markdown) et l'[utilisation de la ponctuation intelligente](/fr/guides/markdown-content/#ponctuation-intelligente) dans le guide Markdown.
:::
...
S'il faut utiliser le [formatteur SmartyPants](https://daringfireball.net/projects/smartypants/) pour transformer les guillemets droits en guillemets typographiques, les tirets en tirets demi-cadratin ou cadratin, et les séries de trois points en points de suspension.

Pour désactiver cela, définissez l'option `smartypants` sur `false`.

Pour un meilleur contrôle de la typographie, vous pouvez à la place spécifier un objet de configuration contenant les [propriétés prises en charge par `retext-smartypants`](https://github.com/retextjs/retext-smartypants?tab=readme-ov-file#fields).

### markdown.remarkRehype

:::caution[Dépréciée]
Cette propriété est dépréciée et sera supprimée dans une future version majeure.

Pour configurer les notes de bas de page, transmettez plutôt `remarkRehype` au processeur `unified()` ou `gfm.footnotes` au processeur `satteri()`. Les autres options `remark-rehype` ne sont prises en charge que lors de l'utilisation de `unified()`.

Apprenez-en davantage sur la [configuration d'un processeur Markdown](/fr/guides/markdown-content/#configurer-un-processeur-markdown) et l'[utilisation des fonctionnalités intégrées](/fr/guides/markdown-content/#fonctionnalités-intégrées) dans le guide Markdown.
:::
...
Configure le [processeur Markdown](/fr/guides/markdown-content/#processeurs-markdown) utilisé pour le rendu des fichiers `.md`.

Sätteri, le pipeline Markdown natif d'Astro, est le processeur par défaut. Pour le configurer, installez `@astrojs/markdown-satteri` et transmettez des options à `satteri()` :
...
Pour garder le pipeline remark/rehype, installez `@astrojs/markdown-remark` et transmettez `unified()` :
...
Apprenez-en davantage sur les [processeurs Markdown officiels et comment en choisir un](/fr/guides/markdown-content/#choisir-un-processeur-markdown) dans le guide Markdown.
...
Utilisez `Astro.cache.set()` dans les routes et `context.cache.set()` dans les middlewares
