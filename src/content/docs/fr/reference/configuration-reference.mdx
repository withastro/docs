...
Pour récupérer toutes les ressources téléchargées sur le même domaine (par exemple `https://cdn.exemple.com/_astro/...`), définissez `assetsPrefix` sur le domaine racine sous forme de chaîne de caractères (quelle que soit votre configuration `de base`) :

```js
{
  build: {
    assetsPrefix: 'https://cdn.exemple.com'
  }
}
```
...
```js
{
  build: {
    assetsPrefix: {
      'js': 'https://js.cdn.exemple.com',
      'mjs': 'https://js.cdn.exemple.com',
      'css': 'https://css.cdn.exemple.com',
      'fallback': 'https://cdn.exemple.com'
    }
  }
}
```
...
Passez une URL complète (par exemple `"http://exemple.com"`) ou un chemin d'accès (par exemple `"/about"`) pour spécifier l'URL à ouvrir.
...
Les URL seront de la forme `exemple.com/[locale]/content/` pour toutes les langues autres que celles par défaut, mais `exemple.com/content/` pour la langue par défaut.

Lorsque le paramètre est défini sur `true`, toutes les URL afficheront un préfixe de langue.
Les URL seront de la forme `exemple.com/[locale]/content/` pour chaque route, y compris la langue par défaut.
...
Alternativement, vous pouvez définir cela dans le [paramètre `json.schemas` de votre configuration VSCode](https://code.visualstudio.com/docs/languages/json#_json-schemas-and-settings) :
...
Notez que cette implémentation initiale utilise une bibliothèque avec des [problèmes connus pour les schémas Zod avancés](https://github.com/StefanTerdell/zod-to-json-schema#known-issues), vous souhaiterez peut-être consulter ces limitations avant d'activer l'indicateur expérimental.
...
Pour un aperçu complet et pour donner votre avis sur cette API expérimentale, consultez la [RFC Rerouting](https://github.com/withastro/roadmap/blob/feat/reroute/proposals/0047-rerouting.md).
