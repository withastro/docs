...
```
## Options de niveau supérieur \{#top-level-options\}
...
Déployez sur votre serveur préféré, sans serveur ou sur un hôte périphérique avec des adaptateurs de construction. Importez l'un de nos adaptateurs propriétaires pour [Netlify](/fr/guides/deploy/netlify/#adapter-for-ssr), [Vercel](/fr/guides/deploy/vercel/#adapter-for-ssr) et plus encore pour utiliser Astro SSR.
...
## Options de construction \{#build-options\}
...
#### Effet sur Astro.url \{#effect-on-astrourl\}
...
## Options de la barre d'outils de développement \{#dev-toolbar-options\}
...
## Options de préchargement \{#prefetch-options\}
...
## Options d'images \{#image-options\}
...
## Options de Markdown \{#markdown-options\}
...
Options de configuration de Shiki. Consultez [la documentation de configuration de Markdown](/fr/guides/markdown-content/#shiki-configuration) pour plus d'informations sur son utilisation.
...
Lorsque [`i18n.fallback`](#i18nfallback) est configuré pour éviter d'afficher une page 404 pour les routes de page manquantes, cette option contrôle s'il faut [rediriger](/fr/guides/routing/#redirects) vers la page de secours ou [réécrire](/fr/guides/routing/#rewrites) le contenu de la page de secours en place.
...
## Indicateurs hérités \{#legacy-flags\}
...
## Indicateurs expérimentaux \{#experimental-flags\}
...
Donne la priorité aux redirections et aux routes injectées de manière égale aux côtés des routes du projet basées sur les fichiers, en suivant les mêmes [règles d'ordre de priorité des routes](/fr/guides/routing/#route-priority-order) pour toutes les routes.
...
Pour l'activer, définissez un [mode de sortie (`output`) permettant le rendu à la demande côté serveur](/fr/basics/rendering-modes/#on-demand-rendered) avec un adaptateur, et ajoutez le drapeau `serverIslands` à l'objet `experimental` :
...
#### Contenu de secours des Îles de serveurs \{#server-island-fallback-content\}
...
Vos collections de contenu existantes peuvent être [migrées vers l'API Content Layer](#migrating-an-existing-content-collection-to-use-the-content-layer-api) avec quelques petites modifications. Cependant, il n'est pas nécessaire de mettre à jour toutes vos collections en même temps pour ajouter une nouvelle collection alimentée par l'API Content Layer. Vous pouvez avoir des collections utilisant en même temps les API existantes et les nouvelles API définies dans `src/content/config.ts`.
...
#### Récupérer des données avec un `loader` \{#fetching-data-with-a-loader\}
...
Le chargeur (`loader`) est défini dans le schéma de la collection et renvoie un tableau d'entrées. Astro fournit deux fonctions de chargement intégrées (`glob()` et `file()`) pour récupérer votre contenu local, ainsi qu'un accès à l'API pour [construire votre propre chargeur et récupérer des données distantes](#creating-a-loader).
...
:::note
Les chargeurs n'[excluront pas automatiquement les fichiers précédés d'un `_`](/fr/guides/routing/#excluding-pages). Utilisez une expression régulière telle que `pattern: '**\/[^_]*.md'` dans votre chargeur pour ignorer ces fichiers.
:::

#### Interrogation et rendu avec l'API Content Layer \{#querying-and-rendering-with-the-content-layer-api\}

La collection peut être [interrogeée de la même manière que les collections de contenu](/fr/guides/content-collections/#querying-collections):
...
#### Créer un chargeur \{#creating-a-loader\}
...
#### Migration d'une collection de contenu existante pour utiliser l'API Content Layer \{#migrating-an-existing-content-collection-to-use-the-content-layer-api\}
