...
Astro fournit un optimiseur intégré utilisant [SVGO](https://svgo.dev/) :
...
Traite le contenu SVG pour un chemin de fichier donné. Les optimiseurs peuvent utiliser ce chemin pour calculer des valeurs spécifiques à chaque fichier. Par exemple, le module d'extension `prefixIds` de SVGO l'utilise pour générer un préfixe d'identifiant unique pour chaque fichier.
