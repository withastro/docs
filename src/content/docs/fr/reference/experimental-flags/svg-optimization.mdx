...
L'option `multipass` détermine si le moteur d'optimisation doit être exécuté plusieurs fois jusqu'à ce qu'aucune optimisation supplémentaire ne soit trouvée. L'option `floatPrecision` définit le nombre de décimales à conserver globalement, mais peut être redéfinie pour un module d'extension spécifique en spécifiant une valeur personnalisée dans sa propriété `params`.
