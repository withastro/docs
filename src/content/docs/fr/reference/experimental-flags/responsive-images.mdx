---
title: Images réactives expérimentales
sidebar:
...
Lorsque ce flag est activé, vous pouvez passer une propriété `layout` à n'importe quel composant `<Image />` ou `<Picture />` pour créer une image réactive. 
