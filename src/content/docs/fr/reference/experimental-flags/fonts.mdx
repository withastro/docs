...
En complément, le fournisseur de polices `googleicons()` accepte toutes les options disponibles dans les [options unifont Google Icons](https://github.com/unjs/unifont/#options-2):
...
Chaque fournisseur est responsable de la prise en charge de ces options, donc la disponibilité et le support des propriétés suivantes peut varier.
...
Utilisé pour récupérer et retourner les données d'une police de caractères à partir des options données.
...
Une fonction de rappel optionnelle, utilisé pour exécuter une logique d'initialisation. Son contexte contient un objet `storage`, utile pour la mise en cache.
...
Vous pouvez définir un fournisseur de polices Astro en utilisant un fournisseur unifont sous le capot :
