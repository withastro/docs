...
Si la dernière police du tableau `fallbacks` est un [nom de famille générique](https://developer.mozilla.org/fr/docs/Web/CSS/font-family#generic-name), une [solution de repli optimisée](https://developer.chrome.com/blog/font-fallbacks) utilisant les métriques de police sera générée. Pour désactiver cette optimisation, définissez `optimizedFallbacks` sur `false`.
