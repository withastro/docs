...
L'objet [`session`](#sessions-api) vous permet d'interagir avec l'état de l'utilisateur stocké (par exemple, ajouter des articles à un panier d'achat) et l'identifiant de session (par exemple, supprimer le cookie d'identifiant de session lors de la déconnexion).
...
Dans la plupart des cas, vous n'aurez besoin que d'utiliser [`Astro.session.get()`](#sessionget) et [`Astro.session.set()`](#sessionset). Pour les autres méthodes disponibles, voir la [section API](#sessions-api).
