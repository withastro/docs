...
<ReadMore>Apprenez-en davantage sur le [stockage de données dans `locals`](/fr/guides/middleware/#stocker-des-données-dans-contextlocals) tout en bénéficiant de la [sûreté du typage](/fr/guides/middleware/#types-du-middleware) en utilisant le middleware d'Astro.</ReadMore>
...
La session est créée lors de la première utilisation et le cookie de session est automatiquement défini. L'objet `session` a pour valeur `undefined` et enregistrera une erreur si vous essayez de l'utiliser dans les cas suivants : aucun stockage de session n'a été configuré, [les sessions sont désactivées](/fr/guides/sessions/#désactivation-des-sessions), ou la route actuelle est pré-rendue.
...
<Tabs>
  <TabItem label="Astro.session">
    ...
    await Astro.session?.load(sessionId ?? "id-de-repli-pour-la-session");
    const cart = await Astro.session?.get("cart");
    ---
  ...
  <TabItem label="context.session">
    ...
    export const GET: APIRoute = async ({ session, request }) => {
      ...
      await session?.load(sessionId ?? "id-de-repli-pour-la-session");
      const cart = await session?.get("cart");
      return Response.json({ cart });
    };
    ```
...
Les API d'exécution CSP d'Astro permettent la prise en charge de la [stratégie de sécurité du contenu (CSP)](https://developer.mozilla.org/fr/docs/Web/HTTP/Guides/CSP) afin de minimiser certains types de menaces en contrôlant les ressources qu'un document est autorisé à charger. Cela offre une protection supplémentaire contre les attaques de type scripts intersite, ou [« cross-site scripting (XSS) »](https://developer.mozilla.org/fr/docs/Glossary/Cross-site_scripting) en Anglais.

Vous pouvez personnaliser l'élément `<meta>` par page à partir de la variable globale `Astro` dans les composants `.astro`, ou du type `APIContext` dans les points de terminaison et middlewares.

Lorsque des ressources sont insérées plusieurs fois ou à partir de plusieurs sources (par exemple définies dans votre [config `csp`](/fr/reference/configuration-reference/#securitycsp) et ajoutées à l'aide des API d'exécution CSP suivantes, Astro fusionnera et dédupliquera toutes les ressources pour créer votre élément `<meta>`.

:::caution[Restreindre la portée avec des directives plus spécifiques]
Lorsque vous restreignez la portée d'une source ou d'un hash à une directive plus spécifique à l'aide de l'option `kind` (`'element'` ou `'attribute'`), les navigateurs utilisent cette directive plutôt que la directive générique `script-src` ou `style-src` pour cette portée, sans revenir à la directive générique.

Astro déplace automatiquement les hachages qu'il génère pour vos scripts et styles incorporés à la page vers la directive plus spécifique afin qu'ils continuent de fonctionner. En revanche, il ne déplace **pas** vos ressources génériques (`'default'`). Si vous combinez des ressources génériques avec des ressources ou des hachages plus spécifiques pour la même famille de directives, les ressources génériques ne s'appliqueront pas à la portée spécifique. Astro émet un avertissement s'il détecte une telle situation.

Pour résoudre le problème, déplacez toutes les ressources et tous les hachages vers les directives spécifiques.
:::
...
Vous pouvez également restreindre la portée d'une source à une directive plus spécifique en transmettant un objet avec une propriété `kind`. Utilisez `'element'` pour `style-src-elem`, `'attribute'` pour `style-src-attr`, ou `'default'` (identique à une chaîne de caractères simple) pour `style-src`.
...
Après la compilation, l'élément `<meta>` de cette page individuelle ajoutera votre source à la directive `style-src-attr`, qui s'applique uniquement aux attributs `style` incorporés dans le HTML :
...
Vous pouvez également restreindre la portée d'un hachage à une directive plus spécifique en transmettant un objet avec une propriété `kind`. Utilisez `'element'` pour `style-src-elem`, `'attribute'` pour `style-src-attr`, ou `'default'` (identique à un hachage simple) pour `style-src`.
...
Après la compilation, l'élément `<meta>` de cette page individuelle ajoutera votre hachage à la directive `style-src-elem`. Les hachages de style générés automatiquement par Astro y sont également déplacés, car `style-src-elem` a la priorité sur `style-src` pour les éléments `<style>` et `<link>` :
...
Vous pouvez également restreindre la portée d'une source à une directive plus spécifique en transmettant un objet avec une propriété `kind`. Utilisez `'element'` pour `script-src-elem`, `'attribute'` pour `script-src-attr`, ou `'default'` (identique à une chaîne de caractères simple) pour `script-src`.
...
Après la compilation, l'élément `<meta>` de cette page individuelle ajoutera votre source à la directive `script-src-elem`, qui régit les éléments `<script>` :
...
Vous pouvez également restreindre la portée d'un hachage à une directive plus spécifique en transmettant un objet avec une propriété `kind`. Utilisez `'element'` pour `script-src-elem`, `'attribute'` pour `script-src-attr`, ou `'default'` (identique à un hachage simple) pour `script-src`.
...
Après la compilation, l'élément `<meta>` de cette page individuelle ajoutera votre hachage à la directive `script-src-elem`. Les hachages de script générés automatiquement par Astro y sont également déplacés, car `script-src-elem` a la priorité sur `script-src` pour les éléments `<script>` :
...
<ReadMore>Consultez le [guide de mise en cache](/fr/guides/caching/) pour plus d'informations sur l'utilisation de la mise en cache dans votre projet Astro.</ReadMore>
...
<p>
  ...
  **Type :** [`AstroRuntimeLogger`](/fr/reference/logger-reference/#astroruntimelogger)
  <Since v="7.0.0" />
</p>
...
```astro title="src/pages/index.astro"
---
Astro.logger.error("Impossible de trouver l'ID de paiement.");
---
```
...
```astro title="src/pages/index.astro"
---
Astro.logger.warn("Impossible de trouver l'ID de paiement.");
---
```  
...
```astro title="src/pages/index.astro"
---
Astro.logger.info("Impossible de trouver l'ID de paiement.");
---
```
