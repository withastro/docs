...
<ReadMore>En savoir plus sur la façon dont les [Mises en page Markdown et MDX](/fr/guides/markdown-content/#frontmatter-layout) gèrent les propriétés.</ReadMore>

<ReadMore>Apprenez à ajouter des [définitions de type TypeScript pour vos props](/fr/guides/typescript/#componentprops-type).</ReadMore>

### `Astro.params` \{#astroparams\}

`Astro.params` est un objet contenant les valeurs des segments de routes dynamiques correspondant à cette requête.

Dans les versions statiques, il s'agira des `params` renvoyés par `getStaticPaths()` utilisés pour le pré-rendu [des routes dynamiques](/fr/guides/routing/#dynamic-routes).
...
#### `AstroCookieGetOptions` \{#astrocookiegetoptions\}
...
#### `AstroCookieSetOptions` \{#astrocookiesetoptions\}
...
### `Astro.clientAddress` \{#astroclientaddress\}
...
### `Astro.generator` \{#astrogenerator\}
...
### `Astro.preferredLocale` \{#astropreferredlocale\}
...
### `Astro.preferredLocaleList` \{#astropreferredlocalelist\}
...
### `Astro.currentLocale` \{#astrocurrentlocale\}
...
### `Astro.getActionResult()` \{#astrogetactionresult\}
...
### `Astro.callAction()` \{#astrocallaction\}
...
## Contexte du point de terminaison \{#endpoint-context\}
...
Dans les versions statiques, il s'agira des `params` retournés par `getStaticPaths()` utilisés pour le pré-rendement des [routes dynamiques](/fr/guides/routing/#dynamic-routes).
...
### `context.clientAddress` \{#contextclientaddress\}
...
### `context.redirect()` \{#contextredirect\}
...
### `context.rewrite()` \{#contextrewrite\}
...
### `context.getActionResult()` \{#contextgetactionresult\}
...
### `context.callAction()` \{#contextcallaction\}
...
Elle peut également être utilisée dans les points de terminaison de fichiers statiques pour le [routage dynamique](/fr/guides/endpoints/#params-and-dynamic-routing).
...
### Transfert de données avec `props` \{#data-passing-with-props\}
...
#### La propriété `page` de pagination \{#the-pagination-page-prop\}
...
##### `page.currentPage` \{#pagecurrentpage\}
...
##### `page.url.current` \{#pageurlcurrent\}
...
## Images (`astro:assets`) \{#images-astroassets\}
...
La fonction `getImage()` est prévue pour générer des images destinées à être utilisées ailleurs que directement en HTML, par exemple dans une [route d'API](/fr/guides/endpoints/#server-endpoints-api-routes). Elle vous permet également de créer votre propre composant `<Image />` personnalisé.

`getImage()` prend un objet d'options avec les [mêmes propriétés que le composant Image](#properties) (à l'exception de `alt`).
...
## Collections de contenu (`astro:content`) \{#content-collections-astrocontent\}
...
[Consultez le guide Collections de contenu](/fr/guides/content-collections/#defining-a-collection-schema) pour un exemple d'utilisation.
...
[Consultez le guide Collections de contenu](/fr/guides/content-collections/#defining-collection-references) pour un exemple d'utilisation.
...
[Consultez le guide Collections de contenu](/fr/guides/content-collections/#querying-collections) pour un exemple d'utilisation.
...
Consultez le guide Collections de contenu pour des exemples d'[interrogation des entrées de collection](/fr/guides/content-collections/#querying-collections).
...
`getEntries()` est une fonction qui récupère plusieurs entrées dans une même collection. Ceci est utile pour [renvoyer un tableau d'entrées référencées](/fr/guides/content-collections/#defining-collection-references) pour accéder à leurs propriétés `data`, `body` et `render()` associées.
...
### `getDataEntryById()` \{#getdataentrybyid\}
...
### Type des entrées de collection \{#collection-entry-type\}
...
Un slug d'URL préparé pour les documents Markdown ou MDX. La valeur par défaut est `id` sans l'extension de fichier, mais peut être remplacée en définissant [la propriété `slug`](/fr/guides/content-collections/#defining-custom-slugs) dans le frontmatter d'un fichier.
...
- `headings` - Une liste générée de titres, [reflétant l'utilitaire `getHeadings()` d'Astro](/fr/guides/markdown-content/#exported-properties) sur les importations Markdown et MDX.
- `remarkPluginFrontmatter` - L'objet frontmatter modifié après [l'application de plugins Remark or Rehype](/fr/guides/markdown-content/#modifying-frontmatter-programmatically). Définit sur le type `any`.
...
[Consultez le guide Collections de contenu](/fr/guides/content-collections/#rendering-content-to-html) pour un exemple d'utilisation.

### Autres types associés aux collections de contenu \{#other-content-collection-types\}
...
#### `ContentCollectionKey` \{#contentcollectionkey\}
...
- `image` - L'assistant de schéma `image()` qui vous permet [d'utiliser des images locales dans les collections de contenu](/fr/guides/images/#images-in-content-collections)
...
## Middleware (`astro:middleware`) \{#middleware-astromiddleware\}
...
<ReadMore>Consultez [Contextes de point de terminaison](#endpoint-context) pour plus d'informations sur l'objet de contexte.</ReadMore>
...
Depuis Astro v4.13.0, `next()` accepte un paramètre de chemin d'URL facultatif sous la forme d'une chaîne de caractères, d'une `URL` ou d'un objet `Request` pour [réécrire](/fr/guides/routing/#rewrites) la requête actuelle sans déclencher une nouvelle phase de rendu.
...
Une API de bas niveau pour créer un objet [`APIContext`](#endpoint-context) à transmettre à une fonction `onRequest()` du middleware Astro.

Cette fonction peut être utilisée par les intégrations/adaptateurs pour exécuter par programmation le middleware Astro.

### `trySerializeLocals()` \{#tryserializelocals\}
...
## Internationalisation (`astro:i18n`) \{#internationalization-astroi18n\}
...
### `getAbsoluteLocaleUrl()`  \{#getabsolutelocaleurl\}
...
### `getRelativeLocaleUrlList()`  \{#getrelativelocaleurllist\}
...
### `getAbsoluteLocaleUrlList()`  \{#getabsolutelocaleurllist\}
...
Utilisez cette fonction de la même manière que [`getAbsoluteLocaleUrl`](/fr/guides/internationalization/#custom-locale-paths) pour renvoyer une liste de chemins absolus pour tous les paramètres régionaux.

### `getPathByLocale()`  \{#getpathbylocale\}
...
Une fonction qui renvoie le chemin (`path`) associé à un ou plusieurs `codes` lorsque les [chemins de paramètres régionaux personnalisés](/fr/guides/internationalization/#custom-locale-paths) sont configurés.
...
### `requestHasLocale()` \{#requesthaslocale\}
...
## API client de Transitions de Vue (`astro:transitions/client`) \{#view-transitions-client-api-astrotransitionsclient\}
...
#### Option `formData` \{#formdata-option\}
...
#### Option `sourceElement` \{#sourceelement-option\}
...
Voir le guide sur la [Gestion de solution de secours](/fr/guides/view-transitions/#fallback-control) pour savoir comment choisir et configurer le comportement de repli.


### Événement `astro:before-preparation` \{#astrobefore-preparation-event\}
...
### Événement `astro:after-preparation` \{#astroafter-preparation-event\}
...
### Événement `astro:before-swap` \{#astrobefore-swap-event\}
...
### Événement `astro:after-swap` \{#astroafter-swap-event\}
...
### Événement `astro:page-load` \{#astropage-load-event\}
...
### Attributs des événements de Transitions de Vue \{#view-transitions-events-attributes\}
...
Il s'agit de la valeur littérale transmise à l'[option `info`](#info-option) de la [fonction `navigate()`](#navigate).
...
Apprenez-en davantage sur la façon de [créer votre propre fonction d'échange personnalisée](/fr/guides/view-transitions/#building-a-custom-swap-function) dans le guide Transitions de Vue.

Par défaut, cette implémentation appellera les fonctions suivantes dans l’ordre :

##### `deselectScripts()` \{#deselectscripts\}
...
##### `swapRootAttributes()` \{#swaprootattributes\}
...
##### `swapHeadElements()` \{#swapheadelements\}
...
##### `swapBodyElements()` \{#swapbodyelements\}
...
## Actions (`astro:actions`) \{#actions-astroactions\}
...
L'utilitaire `defineAction()` est utilisé pour définir de nouvelles actions à partir du fichier `src/actions/index.ts`. Il accepte une fonction [`handler()`](#handler-property) contenant la logique du serveur à exécuter et une propriété facultative [`input`](#input-validator) pour valider les paramètres d'entrée lors de l'exécution.
...
#### Propriété `handler()` \{#handler-property\}
...
#### Validateur de saisie (`input`) \{#input-validator\}
...
<ReadMore>Consultez le [guide des erreurs de saisie de formulaire](/fr/guides/actions/#displaying-form-input-errors) pour en savoir plus sur l'utilisation de `isInputError()`.</ReadMore>
...
Vous pouvez référencer les `Props` de ces composants à l'aide de l'utilitaire [`ComponentProp`](/fr/guides/typescript/#componentprops-type).
...
Ce composant fournit une coloration syntaxique pour les blocs de code au moment de la construction (aucun JavaScript côté client n'est inclus). Le composant est alimenté en interne par Shiki et prend en charge tous les [thèmes](https://shiki.style/themes) et [langues](https://shiki.style/languages) populaires. De plus, vous pouvez ajouter vos thèmes, langues, [transformateurs](#transformers) et [couleurs par défaut](https://shiki.style/guide/dual-themes#without-default-color) personnalisés en les transmettant respectivement aux attributs `theme`, `lang`, `transformers` et `defaultColor`.

:::note
Ce composant **n'hérite** pas des paramètres de votre [configuration Shiki](/fr/guides/markdown-content/#shiki-configuration). Vous devrez les définir à l'aide des propriétés du composant.
:::
...
Un composant générique utilisé pour restituer le contenu d'une [entrée de collection de contenu](/fr/guides/content-collections/#what-are-content-collections).
...
Découvrez comment [contrôler le routeur](/fr/guides/view-transitions/#router-control) et [ajouter des directives de transition](/fr/guides/view-transitions/#transition-directives) aux éléments et composants de la page.
