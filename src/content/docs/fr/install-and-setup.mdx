...
1. Exécutez la commande suivante dans votre terminal pour démarrer l’assistant d’installation :
    ...
    <PackageManagerTabs>
      ...
      <Fragment slot="bun">
      ```shell
      # créer un nouveau projet avec Bun
...
Vous pouvez démarrer un nouveau projet Astro et installer toutes les [intégrations officielles](/fr/guides/integrations/) ou intégrations communautaires prenant en charge la commande `astro add` en passant l'argument `--add` à la commande `create astro`.
...
<PackageManagerTabs>
  ...
  ```shell
  # créer un nouveau projet avec React et Partytown
  bun create astro@latest -- --add react --add partytown
  ```
  </Fragment>
</PackageManagerTabs>
...
<PackageManagerTabs>
  ...
  # créer un nouveau projet basé sur la branche principale d'un dépôt GitHub
  bun create astro@latest -- --template <utilisateur-github>/<depot-github>
  ```
  </Fragment>
</PackageManagerTabs>
...
2. Installez Astro
    ...
    :::note[Important]
    Astro doit être installé localement, et non globalement. Assurez-vous que vous **n'utilisez pas** `npm install -g astro` `pnpm add -g astro` `bun add -g astro` ou `yarn add global astro`.
    :::
