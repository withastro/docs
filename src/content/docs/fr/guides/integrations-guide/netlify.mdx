...
Cet adaptateur permet à Astro de déployer votre [site rendu `hybride` ou `serveur`](/fr/basics/rendering-modes/#on-demand-rendered) sur [Netlify](https://www.netlify.com/).

Si vous utilisez Astro en tant que [constructeur de site statique](/fr/basics/rendering-modes/#pre-rendered), vous n'avez pas besoin d'adaptateur.

Apprenez à déployer votre site Astro dans notre [Guide de déploiement Netlify](/fr/guides/deploy/netlify/).

## Pourquoi Astro Netlify \{#why-astro-netlify\}
...
Ensuite, ajoutez l'adaptateur et votre [mode de rendu à la demande](/fr/basics/rendering-modes/#on-demand-rendered) à votre fichier `astro.config.*` :
...
Suivez les instructions pour [construire votre site localement](/fr/guides/deploy/#building-your-site-locally). Après la construction, vous aurez un dossier `.netlify/` contenant à la fois [Netlify Functions](https://docs.netlify.com/functions/overview/) dans le dossier `.netlify/functions-internal/` et [Netlify Edge Functions](https://docs.netlify.com/edge-functions/overview/) dans le dossier `.netlify/edge-functions/`.
...
### Accès au contexte Edge depuis votre site \{#accessing-edge-context-from-your-site\}
...
### Exécution du middleware Astro sur les fonctions Netlify Edge \{#running-astro-middleware-on-netlify-edge-functions\}
...
### Support du CDN d'images Netlify \{#netlify-image-cdn-support\}
...
```
Pour plus d'informations, voir [le guide d'autorisation des images distantes](/fr/guides/images/#authorizing-remote-images). Cela n'est pas nécessaire pour les images hébergées sur le même domaine que votre site.

### Sites statiques avec l'adaptateur Netlify \{#static-sites-with-the-netlify-adapter\}
...
Les sites statiques devront installer cet adaptateur pour utiliser et configurer le [service d'image de Netlify](#netlify-image-cdn-support).
...
### Mise en cache des pages \{#caching-pages\}
