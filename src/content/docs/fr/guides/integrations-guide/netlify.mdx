...
Par défault, le middleware d'Astro est appliqué aux pages pré-rendues au moment de la compilation, et aux pages pré-rendues au moment de l'exécution.
...
Si votre projet n'utilise pas de sessions, vous pouvez définir [`session: false`](/fr/guides/sessions/#désactivation-des-sessions) dans votre configuration d'Astro. L'adaptateur ne configurera pas Netlify Blobs pour le stockage des sessions, et le moteur d'exécution des sessions sera exclu de votre regroupement de fonctions.
