...
Par exemple, lorsque la [politique de sécurité du contenu](/fr/reference/configuration-reference/#securitycsp) est activée, `experimentalStaticHeaders` peut être utilisée pour ajouter les en-têtes CSP (`headers`) à votre configuration Netlify, au lieu de créer un élément `<meta>` :
