...
Pour utiliser votre premier composant Lit dans Astro, consultez notre [documentation sur les frameworks d'interface utilisateur][astro-ui-frameworks]. Ceci explique :
...
Ce qui précède ne chargera le JavaScript de l'élément que lorsque l'utilisateur l'aura fait défiler jusqu'à ce qu'il soit visible. Comme il s'agit d'un rendu serveur, l'utilisateur ne verra pas de jank ; le chargement et l'hydratation se feront de manière transparente.
