...
Fournir vos propres options de `preprocess` **aura pour effet de surcharger la valeur par défaut de [`vitePreprocess()`](https://github.com/sveltejs/vite-plugin-svelte/blob/HEAD/docs/preprocess.md). Assurez-vous d'activer les drapeaux de préprocesseur nécessaires à votre projet.
