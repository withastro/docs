...
Si votre projet n'utilise pas de sessions, vous pouvez définir [`session: false`](/fr/guides/sessions/#désactivation-des-sessions) dans votre configuration d'Astro. L'adaptateur ne configurera pas le pilote de session du système de fichiers, et le moteur d'exécution des sessions sera exclu du paquet compilé pour le serveur.
