...
Par exemple, lorsque [la politique de sécurité du contenu](/fr/reference/configuration-reference/) est activée, `staticHeaders` peut être utilisé pour ajouter les en-têtes CSP à l'objet `Response` au lieu de créer un élément `<meta>` :
