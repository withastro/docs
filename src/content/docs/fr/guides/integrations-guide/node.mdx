...
Cet adaptateur permet à Astro de déployer votre [site rendu `hybrid` ou `server`](/fr/basics/rendering-modes/#on-demand-rendered) vers des cibles Node.

Si vous utilisez Astro comme [constructeur de site statique](/fr/basics/rendering-modes/#pre-rendered), vous n'avez pas besoin d'adaptateur.

## Pourquoi Astro Node.js \{#why-astro-nodejs\}
...
Tout d'abord, [effectuer un build](/fr/guides/deploy/#building-your-site-locally). En fonction du `mode` sélectionné (voir ci-dessus), suivez les étapes appropriées ci-dessous :
...
#### Hôte et port personnalisés \{#custom-host-and-port\}
...
#### Variables d'environnement d'exécution \{#runtime-environment-variables\}
