...
Astro DB est une base de données SQL entièrement gérée, conçue pour l'écosystème Astro : développez localement dans Astro et déployez depuis votre [tableau de bord Astro Studio](/fr/guides/astro-db/#astro-studio).
...
##### 1. Installer l'intégration à partir de npm via un gestionnaire de paquets \{#1-install-the-integration-from-npm-via-a-package-manager\}
...
##### 2. Ajouter l'intégration à `astro.config.mjs` \{#2-add-the-integration-to-astroconfigmjs\}
...
##### 3. Configurez votre base de données \{#3-configure-your-database\}
...
## Référence de la configuration du tableau \{#table-configuration-reference\}
...
## Référence CLI Astro DB \{#astro-db-cli-reference\}

Astro DB inclut un ensemble de commandes CLI pour interagir avec la base de données de votre projet hébergé et votre compte [Astro Studio](/fr/guides/astro-db/#astro-studio).
...
## Référence de l'utilitaire Astro DB \{#astro-db-utility-reference\}
