...
Par défaut, toutes les [options de configuration `markdown`](/fr/reference/configuration-reference/#options-de-markdown) sont héritées par l'intégration MDX. Les options transmises à l'intégration MDX remplaceront la configuration `markdown` héritée. Pour ignorer complètement les options `markdown` héritées, utilisez l'option [`extendMarkdownConfig`](#extendmarkdownconfig).

L'exemple suivant configure l'intégration MDX pour utiliser un thème de [coloration syntaxique](/fr/guides/syntax-highlighting/) différent, ajouter un [module d'extension](/fr/guides/markdown-content/#modules-dextension-pour-les-processeurs-markdown), et désactiver [GitHub-flavored Markdown](/fr/guides/markdown-content/#github-flavored-markdown) :

```js title="astro.config.mjs" {12-20}
import { defineConfig } from 'astro/config';
import { satteri } from '@astrojs/markdown-satteri';
import mdx from '@astrojs/mdx';
import { myMdastPlugin } from './mon-module-extension-satteri.mjs';
...
export default defineConfig({
  ...
  integrations: [
    mdx({
      // Utiliser un thème de coloration syntaxique différent.
      shikiConfig: { theme: 'dracula' },
      // Ajouter un module d'extension et désactiver GitHub-flavored Markdown.
      processor: satteri({
...
Par défaut, les fichiers `.mdx` sont traités via le même [processeur Markdown](/fr/guides/markdown-content/#processeurs-markdown) que vos fichiers `.md`. Vous pouvez configurer un processeur différent pour MDX ou utiliser le même processeur avec des options différentes.
...
#### Utilisation des modules d'extension Recma

<p><Since v="8.0.0" pkg="@astrojs/mdx" /></p>

Les modules d'extension Recma modifient directement la sortie [estree](https://github.com/estree/estree). Cela est utile pour modifier ou injecter des variables JavaScript dans vos fichiers MDX, et n'a aucun effet sur les fichiers `.md`.

Les modules d'extension Recma ne sont pris en charge que par le [processeur Unified](/fr/guides/markdown-content/#quand-utiliser-unified). Transmettez-les à `unified()` depuis `@astrojs/markdown-remark` en utilisant l'option `recmaPlugins` :
...
Vous pouvez également définir ces modules d'extension dans [`markdown.processor`](/fr/reference/configuration-reference/#markdownprocessor) à la place. Vos fichiers `.mdx` continueront de les utiliser, car MDX [étend votre configuration Markdown](#extendmarkdownconfig) par défaut.

Nous vous suggérons d'[utiliser AST Explorer](https://astexplorer.net/) pour expérimenter avec les sorties estree, et d'essayer [`estree-util-visit`](https://unifiedjs.com/explore/package/estree-util-visit/) pour rechercher dans les nœuds JavaScript.
...
:::caution[Déprécié]
Cette option est dépréciée et sera supprimée dans une future version majeure. Transmettez `recmaPlugins` au [`processor`](#processor) configuré à la place.

En savoir plus sur [l'utilisation des modules d'extension recma](#utilisation-des-modules-dextension-recma).
:::
...
Depuis Astro v7, le processeur Markdown par défaut ne prend pas en charge les modules d'extension Recma. Si votre projet en dépend, vous pouvez [utiliser le processeur `unified()`](/fr/guides/markdown-content/#configurer-un-processeur-markdown).
...
```astro title="src/pages/une-page.astro"
---
import { Content, components } from "../content.mdx";
