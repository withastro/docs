...
Par exemple, lorsque la [politique de sécurité du contenu](/fr/reference/configuration-reference/) est activée, `staticHeaders` peut être utilisée pour ajouter les en-têtes CSP (`headers`) à votre configuration Vercel, au lieu de créer un élément `<meta>` :
