...
Il se peut que vous n'ayez pas besoin d'installer ce paquet pour votre middleware. `@vercel/edge` n'est nécessaire que pour utiliser certaines fonctionnalités du middleware telles que la géolocalisation. Pour plus d'informations, voir [la documentation du middleware de Vercel](https://vercel.com/docs/concepts/functions/edge-middleware).

1. Ajoutez un fichier `middleware.js` à la racine de votre projet :
