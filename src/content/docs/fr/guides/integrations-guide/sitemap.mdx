...
La valeur définie peut-être soit un chemin relatif à votre URL configurée `site` pour une feuille de style locale, soit un lien URL absolu vers une feuille de style externe.
```js title="astro.config.mjs" ins={8}
import { defineConfig } from 'astro/config';
import sitemap from '@astrojs/sitemap';
export default defineConfig({
