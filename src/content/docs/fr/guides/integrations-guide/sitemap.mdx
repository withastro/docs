...
## Pourquoi Astro Sitemap ? \{#why-astro-sitemap\}
...
Avec Astro Sitemap, vous n'avez pas à vous soucier de créer ce fichier XML vous-même : l'intégration Astro Sitemap va explorer vos routes générées statiquement et créer le fichier sitemap, y compris les [routes dynamiques](/fr/guides/routing/#dynamic-routes) comme `[...slug]` ou `src/pages/[lang]/[version]/info.astro` généré par `getStaticPaths()`.

Cette intégration ne peut pas générer d'entrées sitemap pour les routes dynamiques en [mode SSR](/fr/basics/rendering-modes/#on-demand-rendered).
...
### Découverte du plan du site \{#sitemap-discovery\}
...
#### Lien vers le plan du site dans l'élément `<head>` \{#sitemap-link-in-head\}
...
#### Lien vers le plan du site dans le fichier `robots.txt` \{#sitemap-link-in-robotstxt\}
...
### changefreq, lastmod, et priority \{#changefreq-lastmod-and-priority\}
