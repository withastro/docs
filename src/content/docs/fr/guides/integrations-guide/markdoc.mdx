...
### Intégration dans un éditeur VS Code \{#vs-code-editor-integration\}
...
Interrogez ensuite votre collection à l'aide des [API de Collections de contenus](/fr/guides/content-collections/#querying-collections) :
...
## Passer les variables Markdoc \{#pass-markdoc-variables\}
...
### Accéder au frontmatter à partir du contenu de votre Markdoc \{#access-frontmatter-from-your-markdoc-content\}
...
## Afficher les composants \{#render-components\}
...
### Utiliser les composants Astro comme balises Markdoc \{#use-astro-components-as-markdoc-tags\}
...
Ce composant Astro peut maintenant être passé au props `render` pour n'importe quel [tag][markdoc-tags] ou [node][markdoc-nodes] dans votre configuration :
...
### Utiliser des composants Astro à partir de paquets npm et de fichiers TypeScript. \{#use-astro-components-from-npm-packages-and-typescript-files\}
...
### Partiels de Markdoc \{#markdoc-partials\}
...
### Mise en valeur de la syntaxe (highlighting) \{#syntax-highlighting\}
...
<ReadMore>Pour en savoir plus sur la configuration des feuilles de style Prism, [voir notre guide sur la coloration syntaxique](/fr/guides/markdown-content/#prism-configuration).</ReadMore>

## Nœuds / éléments Markdoc personnalisés \{#custom-markdoc-nodes--elements\}
...
* `id : string` Un `id` généré à partir du contenu textuel de la rubrique. Cela correspond au `slug` généré par la fonction [content `render()`](/fr/guides/content-collections/#rendering-content-to-html).

Par exemple, le titre `### Level 3 heading!` passera `level : 3` et `id : 'level-3-heading'` en tant qu'éléments du composant.

### Composants d'image personnalisés \{#custom-image-components\}
...
#### Remplacer le nœud d'image par défaut de Markdoc \{#override-markdocs-default-image-node\}
...
#### Créer une balise d'image Markdoc personnalisée \{#create-a-custom-markdoc-image-tag\}
...
### Configuration de Markdoc \{#advanced-markdoc-configuration\}
...
### Définir l'élément HTML racine \{#set-the-root-html-element\}
...
## Options de configuration de l'intégration \{#integration-config-options\}
