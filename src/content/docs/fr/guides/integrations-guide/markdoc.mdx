...
Vous pouvez avoir besoin de passer les [variables][markdoc-variables] à votre contenu. Ceci est utile pour passer des paramètres SSR comme les tests A/B.
...
Markdoc entoure les documents d'une balise `<article>` par défaut. Ceci peut être changé à partir du noeud Markdoc `document`. Ceci accepte un nom d'élément HTML ou `null` si vous préférez supprimer l'élément enveloppant :
