...
Un adaptateur SSR pour utiliser les fonctions de Cloudflare Pages. Ecrivez votre code en Astro/Javascript et déployez-le sur Cloudflare Pages.
...
Cloudflare Pages a 2 modes différents pour déployer les fonctions, le mode `advanced` qui récupère le `_worker.js` dans `dist`, ou un mode répertoire où les pages vont compiler le worker à partir d'un dossier functions à la racine du projet. Pour la plupart des projets, l'adaptateur par défaut `advanced` sera suffisant ; le dossier `dist` contiendra votre projet compilé.
...
Notez que cet adaptateur ne prend pas en charge l'utilisation de [Cloudflare Pages Middleware](https://developers.cloudflare.com/pages/platform/functions/middleware/). Astro intègre le [Astro middleware](/fr/guides/middleware/) dans chaque page.
...
Détermine comment `routes.json` sera généré si aucun [`_routes.json`](#_routes.json-personnalisé) personnalisé n'est fourni.
...
+       routes: {
+           strategy: 'include',
+           include: ['/users/*'], // Traité par une fonction personnalisée: functions/users/[id].js
+           exclude: ['/users/faq'], // Trité par les pages statique: pages/users/faq.astro
+       },
    }),
});
```
...
Si vous utilisez le mode d'exécution `advanced`, vous pouvez taper l'objet `runtime` comme suit :
...
// instancié à l'avance pour partager le module
