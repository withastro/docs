...
Cette option vous permet d'ajouter ou d'exclure des motifs personnalisés (par exemple `/fonts/*`) au fichier [`_routes.json`](#custom-_routesjson) généré qui détermine quels itinéraires sont générés à la demande. Cela peut être utile si vous avez besoin d'ajouter des modèles d'itinéraires qui ne peuvent pas être générés automatiquement, ou d'exclure des itinéraires pré-rendus.
