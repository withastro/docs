...
Cette fonctionnalité est activée par défaut. Si vous souhaitez la désactiver, utilisez `cloudflareModules: false`.
...
**Par défaut :** `{ environment: undefined }`
</p>

La propriété `environment` vous permet de définir quelle [configuration d'environnement](https://developers.cloudflare.com/workers/wrangler/environments/) utiliser dans votre configuration Wrangler.
