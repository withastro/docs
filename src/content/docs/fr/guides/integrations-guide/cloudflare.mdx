...
*   `off`: pas d'accès au runtime Cloudflare en utilisant `astro dev`. Vous pouvez également utiliser [Prévisualisation avec Wrangler](#preview-with-wrangler)
...
Vous pouvez accéder au runtime à partir des composants Astro via `Astro.locals` à l'intérieur de n'importe quel fichier 
 `.astro\`.
