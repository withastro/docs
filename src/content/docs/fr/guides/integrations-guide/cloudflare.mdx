...
Cette option vous permet d'ajouter ou d'exclure des motifs personnalisés (par exemple `/fonts/*`) au fichier [`_routes.json`](#_routesjson-personnalisé) généré qui détermine quels itinéraires sont générés à la demande. Cela peut être utile si vous avez besoin d'ajouter des modèles d'itinéraires qui ne peuvent pas être générés automatiquement, ou d'exclure des itinéraires pré-rendus.

Vous trouverez plus d'informations sur les modèles d'itinéraires personnalisés dans la [documentation de routing de Cloudflare](https://developers.cloudflare.com/pages/functions/routing/#functions-invocation-routes). Les routes spécifiées ne sont pas automatiquement dédupliquées et seront ajoutées telles quelles aux routes existantes.
