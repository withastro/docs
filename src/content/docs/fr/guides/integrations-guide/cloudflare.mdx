...
* `local` : utilise un runtime local alimenté par miniflare et workerd, qui supporte les Bindings de Cloudflare. Seulement si vous voulez utiliser des fonctionnalités non supportées, comme `eval`, les bindings sans support local choisissent [Preview with Wrangler](#preview-with-wrangler)
