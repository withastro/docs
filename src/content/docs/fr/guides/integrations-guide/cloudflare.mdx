...
Cet adaptateur permet à Astro de déployer votre [site rendu `hybrid` ou `server`](/fr/basics/rendering-modes/#on-demand-rendered) sur [Cloudflare](https://www.cloudflare.com/).

Si vous utilisez Astro comme [constructeur de sites statiques](/fr/basics/rendering-modes/#pre-rendered), vous n'avez pas besoin d'adaptateur.
...
## Pourquoi Astro Cloudflare \{#why-astro-cloudflare\}
...
Ensuite, ajoutez l'adaptateur et votre [mode de rendu à la demande](/fr/basics/rendering-modes/#on-demand-rendered) à votre fichier `astro.config.mjs` :
...
* **`custom`:** Utilise toujours le service d'image configuré dans [Image Options](/fr/reference/configuration-reference/#image-options). **Cette option ne vérifie pas si le service d'image configuré fonctionne dans le moteur d'exécution `workerd` de Cloudflare.
...
#### platformProxy.enabled \{#platformproxyenabled\}
<p>
...
#### platformProxy.configPath \{#platformproxyconfigpath\}
<p>
...
#### platformProxy.experimentalJsonConfig \{#platformproxyexperimentaljsonconfig\}
<p>
...
#### platformProxy.persist \{#platformproxypersist\}
<p>
...
:::note
L'option `--persist-to` de `wrangler` ajoute un sous-répertoire appelé `v3` sous le capot alors que la propriété `@astrojs/cloudflare` `persist` ne le fait pas. Par exemple, pour réutiliser le même emplacement qu'en lançant `wrangler dev --persist-to ./mon-dossier`, vous devez spécifier : `persist : "./mon-dossier/v3"`.
:::
...
```
### routes.extend \{#routesextend\}

Cette option vous permet d'ajouter ou d'exclure des motifs personnalisés (par exemple `/fonts/*`) au fichier [`_routes.json`](#custom-_routesjson) généré qui détermine quelles routes sont générées à la demande. Cela peut être utile si vous avez besoin d'ajouter des modèles de route qui ne peuvent pas être générés automatiquement, ou d'exclure des routes pré-rendues.
...
#### routes.extend.include \{#routesextendinclude\}
...
#### routes.extend.exclude \{#routesextendexclude\}
...
### `cloudflareModules` \{#cloudflaremodules\}
...
## Plate-forme Cloudflare \{#cloudflare-platform\}
...
#### `_routes.json` personnalisé \{#custom-_routesjson\}
...
## Importations de modules Cloudflare \{#cloudflare-module-imports\}
...
## Compatibilité Node.js \{#nodejs-compatibility\}
...
## Aperçu avec Wrangler \{#preview-with-wrangler\}
...
### Messages d'erreur significatifs \{#meaningful-error-messages\}
