...
Vous pouvez activer [Solid DevTools](https://github.com/thetarnav/solid-devtools) dans le développement en passant un objet avec `devtools : true` à votre configuration d'intégration `solid()` et en ajoutant `solid-devtools` aux dépendances de votre projet :
