...
Vous pouvez également placer la directive sur une [île Astro](/fr/concepts/islands/) (un composant du cadre d'interface utilisateur avec une directive [`client:`](/fr/reference/directives-reference/#directives-client)). Si ce composant existe sur la page suivante, l'île de l'ancienne page **avec son état actuel** continuera d'être affichée, au lieu d'être remplacée par l'île de la nouvelle page.

Dans l'exemple ci-dessous, le `count` ne sera pas réinitialisé lorsque l'on navigue au travers des pages qui contiennent le composant `<Counter />` avec l'attribut `transition:persist`.

```astro title="components/Header.astro" "transition:persist"
<Counter client:load transition:persist count={5} />
```

Vous pouvez également [identifier manuellement les éléments correspondants](#nommer-une-transition) si l'île/l'élément se trouve dans une composante différente entre les deux pages.
...
L'exemple suivant montre toutes les propriétés nécessaires pour définir une animation `fade` personnalisée :
...
L'exemple suivant navigue au page `/main` mais n'ajoute pas une nouvelle entrée à l'historique de navigation. Au lieu de cela, il réutilise l'entrée actuelle dans l'historique (`/confirmation`) et l'écrase.
...
## Comportement du script lors de la navigation dans la page
...
Si vous avez du code qui établit un état global, cet état devra prendre en compte le fait que le script peut s'exécuter plus d'une fois. Vérifiez l'état global dans votre balise `<script>` et exécutez votre code de manière conditionnelle lorsque cela est possible :
...
Les scripts de module, y compris ceux qui ajoutent des récepteurs d'événements pour `DOMContentLoaded`, ne sont jamais exécutés qu'une seule fois parce que le navigateur garde la trace des modules déjà chargés. Vous devez ajouter des récepteurs pour un [Événement du cycle de vie](#événements-du-cycle-de-vie) pour réexécuter les scripts pendant la navigation de la page côté client.

Voir le [Tutoriel sur l'ajout de Transitions de Vue](/fr/tutorials/add-view-transitions/#mise-à-jour-des-scripts) pour un exemple de mise à jour de scripts existants dans un projet.
...
<script>
  document.addEventListener('astro:page-load', () => {
    // Cette opération n'est exécutée qu'une seule fois.
    setupStuff();
  }, { once: true });
</script>
```
