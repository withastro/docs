...
Chaque projet de démarrage Astro inclut un fichier `tsconfig.json` dans votre projet. Le [composant script](/fr/basics/astro-components/#le-script-du-composant) d'Astro est Typescript, qui fournit l'outil d'édition d'Astro et vous permet d'ajouter optionnellement une syntaxe à votre JavaScript pour la vérification du type du code de votre propre projet.
...
Au lieu de cela, ces informations sont ajoutées à votre page `<head>` dans les balises HTML `<link>` et `<meta>`, comme si vous écriviez des pages HTML simples.
