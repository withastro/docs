...
L'intégration `@astrojs/prefetch` a été dépréciée dans la version 3.5.0 et n'est plus maintenue. Utilisez les instructions suivantes pour migrer vers le préchargement intégré d'Astro qui remplace cette intégration.
