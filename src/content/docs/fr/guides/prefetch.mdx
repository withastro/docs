...
## Activer le prefetching \{#enable-prefetching\}
...
### Stratégie de prefetch par défaut \{#default-prefetch-strategy\}
...
### Prefetch de tous les liens par défaut \{#prefetch-all-links-by-default\}
...
La stratégie de prefetch par défaut pour tous les liens peut être modifiée avec `prefetch.defaultStrategy` comme indiqué dans la section [Stratégie de prefetch par défaut](#default-prefetch-strategy).

## Prefetch programmatique \{#prefetch-programmatically\}
...
## Utilisation avec les transitions de vue \{#using-with-view-transitions\}

Lorsque vous utilisez [les transitions de vue](/fr/guides/view-transitions/) sur une page, le prefetch est également activé par défaut. Il définit une configuration par défaut de `{ prefetchAll : true }` qui active le [prefetch pour tous les liens](#prefetch-all-links-by-default) sur la page.
...
## Migration depuis `@astrojs/prefetch` \{#migrating-from-astrojsprefetch\}
