...
Vous pouvez ajouter n'importe quelle [intégration d'adaptateur officielle maintenue par Astro](/fr/guides/integrations/#intégrations-officielles) avec la commande `astro add` suivante. Cela installera l'adaptateur et apportera les modifications appropriées à votre fichier `astro.config.mjs` en une seule étape.
...
Vous pouvez également [ajouter un adaptateur manuellement en installant le paquet NPM](/fr/guides/integrations/#installation-dun-paquet-npm) (par exemple `@astrojs/netlify`) et en mettant à jour `astro.config.mjs` vous-même.
