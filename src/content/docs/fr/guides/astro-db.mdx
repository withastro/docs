...
<ReadMore>Voir la [référence des colonnes de la table](/fr/guides/integrations-guide/db/#configuration-de-la-table) pour plus de détails.</ReadMore>
...
Si vous **n'avez pas de données de production importantes**, alors vous pouvez [réinitialiser votre base de données](#pushing-breaking-schema-changes) en utilisant l'option `--force-reset`. Ce drapeau supprimera toutes les tables de la base de données et en créera de nouvelles afin qu'elle corresponde exactement à votre schéma actuel.
...
2. Ajoutez un nouveau schéma de table (correspondant exactement aux propriétés de la table existante) avec le nouveau nom :
  ...
  ```ts title="db/config.ts" ins={9-14}
    ...
    ```
3. [Poussez vers Astro Studio](#pousser-les-changements-de-schéma) avec `astro db push --remote`. Cela ajoutera la nouvelle table et marquera l'ancienne comme obsolète.
...
En outre, cette méthode ne fonctionnera pas dans les déploiements sans serveur, car le système de fichiers n'est pas persisté dans ces environnements.

Pour une solution entièrement gérée, [se connecter aux bases de données hébergées sur la plateforme Astro Studio](/fr/recettes/studio/) à la place.
:::
