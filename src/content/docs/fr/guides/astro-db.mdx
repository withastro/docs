...
Astro DB est une base de données SQL entièrement gérée, conçue exclusivement pour Astro. Développez localement ou connectez-vous à une base de données hébergée et gérée sur notre plateforme [Astro Studio](/fr/recipes/studio/).
...
Dans Astro DB, les données sont stockées dans des tables SQL. Les tables structurent vos données en lignes et en colonnes, où les colonnes imposent le type de chaque valeur de ligne. Astro DB prend en charge les types de colonnes suivants :
...
Les tableaux alimentent les typages TypeScript automatiques d'Astro pour votre contenu. Lorsque vous définissez une table, Astro génère et applique automatiquement une interface TypeScript. Le résultat est un support TypeScript complet lorsque vous interrogez vos données, y compris l'autocomplétion des propriétés et la vérification des types.
...
Vous pouvez interroger votre base de données depuis n'importe quelle [page Astro](/fr/basics/astro-pages/#astro-pages) ou [endpoint](/fr/guides/endpoints/) de votre projet en utilisant l'ORM `db` et le constructeur de requêtes fourni.
...
L'exemple suivant sélectionne toutes les lignes d'une table `Comment`. Cela renvoie le tableau complet des données de développement semées dans `db/seed.ts`, qui est alors disponible pour être utilisé dans votre modèle de page :
...
Astro DB peut [se connecter à la plateforme Astro Studio](/fr/recipes/studio/) pour ajouter rapidement une base de données hébergée à votre projet. Vous pouvez visualiser, gérer et déployer de nouvelles bases de données hébergées à partir du portail web Astro Studio.

Il y a deux façons de créer un projet dans Astro Studio :
1. via l'[Astro Studio CLI](/fr/reference/cli-reference/#astro-studio-cli) en utilisant `astro link`.
2. via l'[Astro Studio web UI](https://studio.astro.build)

Pour créer une nouvelle base de données dans l'interface web d'Astro Studio, cliquez sur le bouton "create project" dans l'en-tête et suivez les instructions présentées. Astro Studio vous guidera à travers les étapes nécessaires et, à la fin du processus, vous aurez une base de données hébergée configurée pour votre projet.
...
Vous pouvez aussi pousser les changements de schéma via le CLI en utilisant la commande `astro db push` :
...
Vous pouvez avoir besoin de transférer des données vers votre base de données Studio pour réaliser du seeding ou des migrations de données. Vous pouvez créer un fichier `.ts` avec le module `astro:db` pour écrire des requêtes sûres. Ensuite, exécutez le fichier contre votre base de données Studio en utilisant la commande `astro db execute <file-path> --remote` :

Les Comments suivants peuvent être ajoutés en utilisant la commande `astro db execute db/seed.ts --remote` :
...
Soyez prudent en utilisant `--remote` dans le développement. Cela se connectera à une base de données de production, et toutes les insertions, mises à jour ou suppressions seront persistées.
...
<ReadMore>
  ...
  Lorsque vous êtes prêt à déployer, consultez notre guide [Déployer avec une connexion Studio](/fr/recipes/studio/#deploy-with-a-studio-connection).
...
Utilisez la méthode `extendDb()` dans le hook `astro:db:setup` pour enregistrer des fichiers de configuration et de seed Astro DB supplémentaires.
...
Le fichier seed peut importer `Pets` et utiliser `asDrizzleTable()` pour insérer des lignes dans votre table avec vérification du type :
