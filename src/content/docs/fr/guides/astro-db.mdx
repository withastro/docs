...
Pour vous connecter à une base de données distante hébergée, utilisez l'indicateur `--remote`. Celui-ci permet un accès en lecture et en écriture à votre base de données distante, ce qui vous permet d'[accepter et de conserver les données utilisateur](#insérer) dans les environnements de production.
...
Vous pouvez interroger votre base de données depuis n'importe quelle [page Astro](/fr/basics/astro-pages/#pages-astro) ou [endpoint](/fr/guides/endpoints/) ou [action](/fr/guides/actions/) de votre projet en utilisant l'ORM `db` et le constructeur de requêtes fourni.
...
Consultez la [référence `delete()` de l'API Drizzle](https://orm.drizzle.team/docs/delete) pour un aperçu complet.
...
Si vous **n'avez pas de données de production importantes**, alors vous pouvez [réinitialiser votre base de données](#pousser-les-changements-majeurs-de-schéma) en utilisant l'option `--force-reset`. Cet indicateur supprimera toutes les tables de la base de données et en créera de nouvelles afin qu'elles correspondent exactement à votre schéma actuel.
...
Les fichiers de [configuration](#définir-votre-base-de-données) et de [données initiales](#alimentez-votre-base-de-données-pour-le-développement) des intégrations suivent le même format que leurs équivalents définis par l'utilisateur.
