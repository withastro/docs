...
## Principales similitudes entre Gridsome et Astro \{#key-similarities-between-gridsome-and-astro\}

Gridsome et Astro partagent certaines similitudes qui vous aideront à migrer votre projet :

- Gridsome et Astro sont tous deux des générateurs de sites statiques Javascript modernes avec des [structures de fichiers de projet similaire](/fr/basics/project-structure/#directories-and-files).
...
- Astro possède [une intégration officielle pour l'utilisation des composants Vue](/fr/guides/integrations-guide/vue/) et supporte [l'installation des paquets NPM](/fr/guides/imports/#npm-packages), dont plusieurs pour Vue. Vous serez en mesure d'écrire des composants d'interface utilisateur Vue et de conserver tout ou partie de vos composants Vue et dépendances Gridsome existants.
...
## Principales différences entre Gridsome et Astro \{#key-differences-between-gridsome-and-astro\}
...
Vous pouvez passer un argument `--template` à la commande `create astro` pour démarrer un nouveau projet Astro avec l'un de nos modèles de démarrage officiels. Vous pouvez aussi [démarrer un nouveau projet à partir de n'importe quel dépôt Astro existant sur GitHub](/fr/install-and-setup/#use-a-theme-or-starter-template).
...
Comme Astro interroge et importe vos fichiers locaux différemment de Gridsome, vous pouvez lire [comment charger des fichiers en utilisant `Astro.glob()`](/fr/guides/imports/#astroglob) pour comprendre comment travailler avec vos fichiers locaux.
...
- Article de blog : [Migration de Gridsome vers Astro](https://fyodor.io/migration-from-gridsome-to-astro/).
