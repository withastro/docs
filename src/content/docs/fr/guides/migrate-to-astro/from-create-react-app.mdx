...
<ReadMore>Voir comment [Construire une application à page unique (SPA) avec Astro](https://logsnag.com/blog/react-spa-with-astro) <Badge text="External" /> en utilisant React Router.</ReadMore>
...
- La [syntaxe des fichiers `.astro` est similaire à celle de JSX](/fr/basics/astro-syntax/#différences-entre-astro-et-jsx). L'écriture d'Astro devrait vous sembler familière.
...
Vous reproduirez un schéma similaire basé sur des composants [en utilisant les composants Astro HTML templating pour votre structure de base](/fr/basics/astro-components/) tout en important et en incluant des composants React individuels (qui peuvent eux-mêmes être des applications entières !) pour des îlots d'interactivité.
...
3. Déplacez tout JavaScript nécessaire, y compris les instructions d'importation, dans un ["barre de code" (`---`)](/fr/basics/astro-components/#le-script-du-composant). Remarque : le JavaScript pour [afficher le contenu de manière conditionnelle](/fr/basics/astro-syntax/#html-dynamique) est souvent écrit à l'intérieur du modèle HTML directement dans Astro.

4. Utilisez [`Astro.props`](/fr/reference/api-reference/#astroprops) pour accéder à tous les props supplémentaires qui ont été précédemment passés à votre fonction CRA.
...
Convertit toutes les instances de `{children}` en un Astro `<slot />`. Astro n'a pas besoin de recevoir `{children}` en tant que function prop et affichera automatiquement le contenu des enfants dans un `<slot />`.
...
Les composants React qui passent plusieurs ensembles d'enfants peuvent être migrés vers un composant Astro en utilisant des [slots nommés](/fr/basics/astro-components/#les-emplacements-slots-nommés).
...
Il se peut que vous deviez remplacer les [bibliothèques CSS-in-JS](https://github.com/withastro/astro/issues/4432) (par exemple, les composants stylisés) par d'autres options CSS disponibles dans Astro.
...
En savoir plus sur [Styling in Astro](/fr/guides/styling/).
