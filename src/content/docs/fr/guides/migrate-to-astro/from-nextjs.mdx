...
## Principales similitudes entre Next.js et Astro \{#key-similarities-between-nextjs-and-astro\}
...
- Les projets Astro peuvent également être de type SSG ou [SSR avec prérendu au niveau de la page](/fr/guides/server-side-rendering/).
- Astro utilise le routage basé sur des fichiers et [permet à des pages spécialement nommées de créer des itinéraires dynamiques](/fr/guides/routing/#dynamic-routes).
...
## Principales différences entre Next.js et Astro \{#key-differences-between-nextjs-and-astro\}
...
### Placer votre code source dans `src` \{#put-your-source-code-in-src\}
...
2. Remplacez [toute syntaxe Next ou JSX par une syntaxe Astro](#reference-convert-nextjs-syntax-to-astro) ou par des normes web HTML. Ceci comprend `<Link>`, `<Script>`, `{children}` et `className` par exemple.
...
4. Utilisez [`Astro.props`](/fr/reference/api-reference/#astroprops) pour accéder à toutes les propriétés supplémentaires précédemment transmises à votre fonction Next.
...
6. Remplacez `getStaticProps()` avec des déclarations d'importation ou avec [`Astro.glob()`](/fr/reference/api-reference/#astroglob) pour interroger vos fichiers locaux. Utilisez `fetch()` pour récupérer des données externes.

Voir [un exemple d'un fichier`.js` converti étape par étape depuis Next](#guided-example-next-data-fetching-to-astro).
...
#### Migration depuis le répertoire `pages` de Next.js \{#migrating-from-nextjs-pages-directory\}
...
#### Migration depuis le répertoire `/app` de Next.js \{#migrating-from-nextjs-app-directory\}
...
### Migration de pages et des articles \{#migrating-pages-and-posts\}
...
Vos pages Next JSX (`.js`) existantes devront être [converties des fichiers JSX en pages `.astro`](#tips-convert-jsx-files-to-astro-files). Vous ne pouvez pas réutiliser un fichier JSX de page dans Astro.
...
## Référence : Convertir la syntaxe Next.js en syntaxe Astro \{#reference-convert-nextjs-syntax-to-astro\}

### Les liens : de Next vers Astro \{#next-links-to-astro\}
...
### Les Imports : de Next vers Astro \{#next-imports-to-astro\}

Mettez à jour toutes les [importations de fichiers](/fr/guides/imports/) pour référencer exactement les chemins de fichiers relatifs. Cela peut être fait en utilisant des [alias d'importation](/fr/guides/typescript/#import-aliases) ou en écrivant un chemin relatif dans son intégralité.
...
### La propriété Children : de Next vers Astro \{#next-children-props-to-astro\}
...
### La récupération des données : de Next vers Astro \{#next-data-fetching-to-astro\}

Convertissez toutes les instances de `getStaticProps()` en `Astro.glob()` ou `getCollection()`/`getEntryBySlug()` afin d'accéder aux données d'autres fichiers dans la source de votre projet. Pour [récupérer des données distantes](/fr/guides/data-fetching/), utilisez `fetch()`.
...
En savoir plus sur les [importations de fichiers locaux avec `Astro.glob()`](/fr/guides/imports/#astroglob), l'[interrogation à l'aide de l'API Collections](/fr/guides/content-collections/#querying-collections) ou la [récupération de données distantes](/fr/guides/data-fetching/).

### Les styles : de Next vers Astro \{#next-styling-to-astro\}
...
### Le composant Next Image vers celui d'Astro \{#next-image-plugin-to-astro\}

Convertissez tous les composants Next `<Image />` en [composant d'image propre à Astro](/fr/guides/images/#image--astroassets) dans les fichiers `.astro` ou `.mdx`, ou en code [HTML/JSX standard avec la balise `<img />`](/fr/guides/images/#images-in-ui-framework-components) selon les besoins dans vos composants React.

Le composant `<Image />` d'Astro fonctionne uniquement dans les fichiers `.astro` et `.mdx`. Consultez la [liste complète de ses attributs de composants](/fr/guides/images/#properties) et notez que plusieurs différeront des attributs de Next.
...
## Exemple guidé : Récupération des données, de Next vers Astro \{#guided-example-next-data-fetching-to-astro\}
