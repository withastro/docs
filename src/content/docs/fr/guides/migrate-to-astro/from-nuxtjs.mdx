...
## Principales similitudes entre Nuxt et Astro \{#key-similarities-between-nuxt-and-astro\}
...
- Astro utilise le routage basé sur des fichiers et [permet à des pages spécialement nommées de créer des routes dynamiques](/fr/guides/routing/#dynamic-routes).
...
- Astro prend en charge [l'installation de paquets NPM](/fr/guides/imports/#npm-packages), y compris les bibliothèques Vue. Vous pourrez peut-être conserver certains ou tous vos composants et dépendances Vue existants.

## Principales différences entre Nuxt et Astro \{#key-differences-between-nuxt-and-astro\}
...
## Convertir votre projet NuxtJS \{#convert-your-nuxtjs-project\}
...
### Convertir les pages SFC de Vue en fichiers `.astro` \{#convert-vue-sfc-pages-to-astro-files\}
...
2. Remplacez toute [syntaxe Nuxt ou Vue par une syntaxe Astro](#reference-convert-nuxtjs-syntax-to-astro) ou par des normes web HTML. Ceci comprend `<NuxtLink>`, `:class`, `{{variable}}` et `v-if`, par exemple.

3. Déplacez le JavaScript présent dans `<script>` dans une « barrière de code » (`---`). Convertissez les propriétés de récupération de données de votre composant en JavaScript exécuté côté serveur - consultez [la récupération de données, de Nuxt vers Astro](#nuxt-data-fetching-to-astro). 
...
Voir [un exemple d'une application Nuxt convertie étape par étape](#guided-example-see-the-steps).

#### Comparaison : Vue vs Astro \{#compare-vue-vs-astro\}
...
### Migrer les fichiers de mise en page \{#migrating-layout-files\}
...
### Migrer les pages et les articles \{#migrating-pages-and-posts\}

Dans NuxtJS, vos [pages](/fr/basics/astro-pages/) se trouvent dans `/pages`. Dans Astro, **les contenus de vos pages doivent résider dans `src/pages` ou `src/content`**.

#### Pages Vue \{#vue-pages\}

Vos pages Nuxt Vue (`.vue`) existantes devront être [converties de fichiers Vue en pages `.astro`](#convert-vue-sfc-pages-to-astro-files). Vous ne pouvez pas utiliser un fichier de page Vue existant dans Astro.
...
##### Nommage du chemin de fichier dynamique \{#dynamic-file-path-naming\}
...
Lorsqu'ils font partie d'une [collection de contenu](/fr/guides/content-collections/), les fichiers Markdown et MDX vivront dans des dossiers à l'intérieur de `src/content/` et vous [générez ces pages dynamiquement](/fr/guides/content-collections/#generating-routes-from-content).

### Migrer les tests \{#migrating-tests\}
...
## Référence : Convertir la syntaxe NuxtJS en syntaxe Astro \{#reference-convert-nuxtjs-syntax-to-astro\}

### Les variables locales, de Nuxt à Astro \{#nuxt-local-variables-to-astro\}
...
### La transmission de propriété, de Nuxt à Astro \{#nuxt-property-passing-to-astro\}
...
### Les liens, de Nuxt à Astro \{#nuxt-links-to-astro\}
...
### Les importations, de Nuxt à Astro \{#nuxt-imports-to-astro\}
...
### La génération dynamique de pages, de Nuxt à Astro \{#nuxt-dynamic-page-generation-to-astro\}
...
- [Utiliser le mode de rendu côté serveur](/fr/guides/server-side-rendering/).
- Exportez une fonction `getStaticPaths()` dans le frontmatter d'une page Astro pour indiquer au framework les [routes statiques à générer dynamiquement](/fr/guides/routing/#dynamic-routes).

#### Convertir la fonction `generate` de Nuxt en une fonction `getStaticPaths` dans Astro. \{#convert-a-generate-function-in-nuxt-to-a-getstaticpaths-function-in-astro\}
...
### La récupération de données, de Nuxt à Astro \{#nuxt-data-fetching-to-astro\}
...
### Les styles, de Nuxt à Astro \{#nuxt-styling-to-astro\}
...
#### Les styles globaux \{#global-styling\}
...
#### La prise en charge de préprocesseurs \{#pre-processor-support\}

[Astro prend en charge les préprocesseurs CSS les plus populaires](/fr/guides/styling/#css-preprocessors) en les installant en tant que dépendance de développement. Par exemple, pour utiliser SCSS :
...
### Les plugins d'image de Nuxt vers Astro \{#nuxt-image-plugin-to-astro\}

Convertissez tous les [composants `<nuxt-img/>` ou `<nuxt-picture/>` de Nuxt](https://image.nuxtjs.org/components/nuxt-img) en [composant d'image propre à Astro](/fr/guides/images/#image--astroassets) dans les fichiers `.astro` ou `.mdx` ou en code [HTML standard avec la balise `<img>`](/fr/guides/images/#images-in-ui-framework-components) ou la balise `<picture>` selon le cas dans vos composants Vue.

Le composant `<Image />` d'Astro fonctionne uniquement dans les fichiers `.astro` et `.mdx`. Consultez la [liste complète de ses attributs de composants](/fr/guides/images/#properties) et notez que plusieurs différeront des attributs de Nuxt.
...
## Exemple guidé : Voir les étapes ! \{#guided-example-see-the-steps\}
...
- Article de blog : [De Nuxt à Astro - reconstruire avec Astro](https://dev.to/lindsaykwardell/from-nuxt-to-astro-rebuilding-with-astro-5ann) (en)
- Article de blog : [Migration d'application de Nuxt 2 vers Astro 3 – de la configuration à la production](https://stevenwoodson.com/blog/replatforming-from-nuxtjs-2-to-astro/) (en)
