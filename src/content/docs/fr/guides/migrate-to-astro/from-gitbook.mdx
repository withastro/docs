...
## Principales similitudes entre GitBook et Astro \{#key-similarities-between-gitbook-and-astro\}
...
## Principales différences entre GitBook et Astro \{#key-differences-between-gitbook-and-astro\}
...
## Passer de GitBook à Astro \{#switch-from-gitbook-to-astro\}
...
Vous pouvez passer un argument `--template` à la commande `create astro` pour démarrer un nouveau projet Astro avec l'un de nos modèles officiels. Ou vous pouvez [démarrer un nouveau projet à partir de n'importe quel dépôt Astro existant sur GitHub](/fr/install-and-setup/#use-a-theme-or-starter-template).
...
- Certaines fonctionnalités de GitBook reposent sur des composants personnalisés. Ces composants n'existent pas dans Astro et doivent être créés et ajoutés à votre projet via [l'attribut `tags` de la configuration de Markdoc](/fr/guides/integrations-guide/markdoc/#use-astro-components-as-markdoc-tags) ou supprimés de vos fichiers.
