...
Après avoir importé ou analysé des fichiers Markdown, vous pouvez écrire des modèles HTML dynamiques dans vos composants `.astro` qui incluent les données de la page d'accueil et le contenu du corps du texte.
...
Vous pouvez également [rendre le contenu HTML d'une entrée de collection Markdown](/fr/guides/collections de contenu/#rendering-content-to-html) en affichant un composant `<Content />`.
...
<p>Fin des ventes : {product.data.saleEndDate.toDateString()}</p>
<Content />
```
...
Vous pouvez personnaliser ces IDs en ajoutant un plugin rehype qui injecte les attributs `id` (par exemple `rehype-slug`). Vos IDs personnalisés, au lieu des valeurs par défaut d'Astro, seront reflétés dans la sortie HTML et les éléments retournés par `getHeadings()`.
...
Astro prend en charge l'ajout de plugins tiers [remark](https://github.com/remarkjs/remark) et [rehype](https://github.com/rehypejs/rehype) pour Markdown. Ces plugins vous permettent d'étendre votre Markdown avec de nouvelles fonctionnalités, comme [générer automatiquement une table des matières](https://github.com/remarkjs/remark-toc), [appliquer des étiquettes emoji accessibles](https://github.com/florianeckerstorfer/remark-a11y-emoji), et [styliser votre Markdown](/fr/guides/styling/#style-markdown).

Nous vous encourageons à consulter [awesome-remark](https://github.com/remarkjs/awesome-remark) et [awesome-rehype](https://github.com/rehypejs/awesome-rehype) pour les plugins les plus populaires ! Voir le README de chaque plugin pour les instructions d'installation spécifiques.
...
:::note
Si vous utilisez des [collections de contenus](/fr/guides/content-collections/), veuillez consulter [« Modifier le texte de présentation avec des remarques »](/fr/guides/content-collections/#modifier-le texte de présentation avec des remarques).:::
...
<Steps>
  1. Ajoutez une `customProperty` à la propriété `data.astro.frontmatter` de l'argument `file` de votre plugin :
...
export default defineConfig({
  markdown: {
    shikiConfig: {
      ...
      // Note : Shiki a d'innombrables langues intégrées, y compris le .astro !
...
<Steps>
  ...
  3. Chargez-la dans le `<head>` de votre page dans un [composant de mise en page](/fr/basics/layouts/) via une balise `<link>`. (Voir l'[utilisation de base du prisme](https://prismjs.com/#basic-usage).)
</Steps>
...
Utilisation de « marked » (https://github.com/markedjs/marked)
