...
<ReadMore>Voir le [Type des entrées de collection](/fr/reference/api-reference/#type-des-entrées-de-collection) complet.</ReadMore>
...
Vous pouvez également [rendre le contenu HTML d'une entrée de collection Markdown](/fr/guides/content-collections/#rendre-le-contenu-en-html) en affichant un composant `<Content />`.
...
Maintenant, chaque fichier Markdown ou MDX aura une propriété `customProperty` dans son frontmatter, ce qui la rendra disponible lors de [l'importation de votre Markdown](#importation-de-markdown) et à partir de [la propriété `Astro.props.frontmatter` dans vos mises en page](#propriété-de-la-mise-en-page-du-frontmatter).
...
<ReadMore>En savoir plus sur les [Layouts en Markdown](/fr/basics/layouts/#mises-en-page-markdown).</ReadMore>
