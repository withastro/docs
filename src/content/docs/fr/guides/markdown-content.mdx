...
<ReadMore>En savoir plus sur le [routage basé sur les fichiers d'Astro](/fr/basics/routing/) ou les options pour créer des [routes dynamiques](/fr/basics/routing/#routes-dynamiques).</ReadMore>
...
Après avoir installé l'intégration MDX, vous pouvez importer et utiliser les [composants Astro](/fr/basics/astro-components/#props-de-composant) et les [composants de framework d'interface utilisateur](/fr/basics/framework-components/#utilisation-des-composants-de-framework) dans des fichiers MDX (`.mdx`) comme vous le feriez avec n'importe quel autre composant Astro.
...
À la place de mettre vos fichiers Markdown/MDX dans le dossier `src/pages/` pour créer des routes de pages, vous pouvez [générer des pages dynamiquement](/fr/basics/routing/#routes-dynamiques).
