...
- **`url`** - S'il s'agit d'une page, l'URL de la page (e.g. `/en/guides/markdown-content`).
...
Pour accéder à votre contenu Markdown, passez le composant `<Content/>` à travers le `props` de la page Astro.
Vous pouvez alors récupérer le composant dans `Astro.props` et le rendre dans votre template.
...
Ce `title` sera accessible depuis les instructions `import` et [Astro.glob()](/en/reference/api-reference/#astroglob) :
