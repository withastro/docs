...
Avec [l'intégration @astrojs/mdx](/fr/guides/integrations-guide/mdx/) installée, Astro supporte aussi les fichiers (`.mdx`) [MDX](https://mdxjs.com/) qui apportent des fonctionnalités supplémentaires, telles que la prise en charge des expressions et des composants JavaScript dans votre contenu Markdown.
...
📚 En savoir plus à propos du [routage basé sur les fichiers](/fr/core-concepts/routing/) ou les options pour créer des [routes dynamiques](/fr/core-concepts/routing/#routes-dynamiques).

### Pages Brouillons

`draft: true` est une valeur optionnelle dans l'en-tête Markdown qui indique qu'une page ou un article Markdown ou MDX est "non publié". Par défaut, cette page sera :
- exclus du site construit (**aucune page ne sera build**)
- retourné par [`Astro.glob()`](/fr/reference/api-reference/#astroglob) (**visibles dans les listes d'article**)
...
Pour activer la compilation des pages brouillons par défaut, modifiez `astro.config.mjs` en ajoutant ` drafts: true` au `markdown` ou à l'intégration `mdk` :
...
Astro fournit des fonctionnalités Markdown supplémentaires et intégrées disponibles lors de l'utilisation de fichiers Markdown et MDX.

### L'avant-propos `layout`

Astro fourni aux pages Markdown et MDX une propriété spéciale `layout` d'avant-propos qui peut spécifier un chemin relatif (ou [alias](/fr/guides/aliases/)) à un [composant layout Astro](/fr/core-concepts/layouts/#markdownmdx-layouts).
...
--- 
Cet article est écrit en Markdown.
```

Les propriétés de l'avant-propos sont disponibles pour la mise-en-page via `Astro.props` :
...
📚 En savoir plus à propos des [Layouts Markdown](/fr/core-concepts/layouts/#markdownmdx-layouts).
...
Il ajoute aussi des fonctionnalités supplémentaires au MDX standard, incluant le support de [l'avant-propos de style Markdown dans MDX](https://mdxjs.com/guides/frontmatter/). Cela vous permet d'utiliser la plupart des fonctionnalités Markdown intégrées d'Astro, telles qu'une propriété d'[avant-propos de `layout`](#lavant-propos-layout) et un paramètre pour les [brouillons de page](#pages-brouillons).
...
2. un [`<slot />` par défaut](/fr/core-concepts/astro-components/#les-emplacements-slots) pour indiquer où le contenu Markdown de la page devrait être affiché.
...
Voir les [propriétés exportées vers un composant de mise en page Astro](/fr/core-concepts/layouts/#propriétés-de-mise-en-page-markdown) en utilisant la [mise en page spéciale d'Astro pour le frontmatter](#le-frontmatter-layout).
:::
