...
2. Dans ce fichier, exportez une fonction [`onRequest()`](/fr/reference/api-reference/#onrequest) à laquelle on peut passer un objet [`context`](#the-context-object) et une fonction `next()`. Il ne doit pas s'agir d'une exportation par défaut.
...
### Objet `context` \{#the-context-object\}
...
### Stocker des données dans `context.locals` \{#storing-data-in-contextlocals\}
...
### Exemple : Supprimer des informations sensibles \{#example-redacting-sensitive-information\}
...
### Type de middleware \{#middleware-types\}
...
## Enchaînement middleware \{#chaining-middleware\}
...
L'`APIContext` expose une méthode appelée `rewrite()` qui fonctionne de la même manière que [Astro.rewrite](/fr/guides/routing/#rewrites).

Utilisez `context.rewrite()` dans un middleware pour afficher le contenu d'une autre page sans [rediriger](/fr/guides/routing/#dynamic-redirects) votre visiteur vers une nouvelle page. Cela déclenchera une nouvelle phase d'affichage, entraînant la ré-exécution de tout middleware.
...
La fonction `next()` accepte la même charge utile que [la fonction `Astro.rewrite()`](/fr/reference/api-reference/#astrorewrite). L'emplacement du chemin de réécriture peut être fourni sous la forme d'une chaîne, d'une URL ou d'une `Request`.

Quand vous avez plusieurs fonctions middleware enchaînées via [sequence()](#chaining-middleware), soumettre un chemin à `next()` réécrira la `Request` en place et le middleware ne s'exécutera pas à nouveau. Le middleware suivant dans la chaîne recevra la nouvelle `Request` avec son `context` mis à jour :
