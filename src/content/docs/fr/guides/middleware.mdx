...
:::note
La valeur de `locals` ne peut pas être modifiée à l'exécution. Cela risquerait d'effacer toutes les informations stockées par l'utilisateur.  Astro effectue des vérifications et génère une erreur si la valuer de `locals` est remplacée.
:::
