...
2. Dans ce fichier, exportez une fonction [`onRequest()`]/fr/reference/api-reference/#onrequest)2. Dans ce fichier, exportez une fonction [`onRequest()`](/fr/reference/api-reference/#onrequest) à laquelle on peut passer un objet [`context`](#objet-context) et une fonction `next()`. Il ne doit pas s'agir d'une exportation par défaut.
...
`context.locals` est un objet contenant les données d'une `Response` qui peuvent être manipulées dans le middleware. 
