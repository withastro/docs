...
Astro maintient des adaptateurs officiels pour [NodeJs](https://nodejs.org/), [Vercel](https://vercel.com/), [Netlify](https://www.netlify.com/), et [Cloudflare](https://www.cloudflare.com/). 
...
2. [ajoutez l'adapter](/fr/reference/configuration-reference/#adapteur) à votre fichier `astro.config.mjs` d'importation et d'exportation par défaut, ainsi que votre mode de `sortie` désiré :
...
Vous pouvez alors choisir de surcharger le rendu à la demande par défaut du mode `server` sur n'importe quelle page ou route en exportant `const prerender = true` :
