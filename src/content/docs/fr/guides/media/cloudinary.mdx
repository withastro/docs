...
Vous pouvez ensuite utiliser les [fonctions de requête `getCollection()` ou `getEntry()`](/fr/guides/content-collections/#interroger-les-collections-consommées-lors-de-la-compilation) pour sélectionner une ou plusieurs images ou vidéos de votre collection.
