...
Vous pouvez ensuite utiliser les [fonctions de requête `getCollection()` ou `getEntry()`](/fr/guides/content-collections/#interroger-les-collections-créées-au-moment-de-la-compilation) pour sélectionner une ou plusieurs images ou vidéos de votre collection.
