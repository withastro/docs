...
Bien que les composants Mux Player et Mux Video ne nécessitent pas d'authentification et puissent lire n'importe quelle vidéo accessible au public en fonction de son `playbackId`, la connexion à vos données Mux hébergées via le SDK Node nécessite [un jeton d'accès à l'API de Mux](#mux-environment-api-access).
