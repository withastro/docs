...
4. Téléchargez votre dossier `dist` dedans ou téléchargez en utilisant [Cloud Build](https://cloud.google.com/build).
...
Avant de pouvoir déployer votre site Astro sur Cloud Run, vous devez créer un Dockerfile qui sera utilisé pour composer le Container. Vous trouverez plus d'informations sur [comment utiliser Docker avec Astro](/fr/recipes/docker/#creating-a-dockerfile) dans notre section des méthodes.
