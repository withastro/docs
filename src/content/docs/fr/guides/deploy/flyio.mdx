...
Votre projet Astro peut être déployé sur Fly.io en tant que site statique, ou en tant que site affiché côté serveur (SSR).
...
Si vous n'avez pas encore de fichier Docker, `fly launch` va en générer un pour vous, ainsi que préparer un fichier `fly.toml`. Pour les projets `server` ou `hybrid`, ce fichier Docker inclura la commande de démarrage appropriée et les variables d'environnement.
