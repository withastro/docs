...
:::note
Avant de déployer votre site Astro avec du [SSR (server-side rendering)](/fr/guides/on-demand-rendering/) activé, vérifiez que vous avez :

- Installé [l'adapteur approprié](/fr/guides/on-demand-rendering/) en tant que dépendance de votre projet (que ça soit manuellement, ou en utilisant la commande de l'adapteur `astro add`, ex. `npx astro add netlify`).
