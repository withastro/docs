...
[Clever Cloud](https://clevercloud.com) est une plateforme cloud européenne qui fournit des services automatisés et évolutifs.
...
Si vous exécutez une application Node.js à la demande, mettez à jour votre script `start` pour exécuter le serveur Node. Les applications sur Clever Cloud écoutent le port **8080**.
...
6. Ignorez [la connexion des **modules complémentaires** à votre application Clever](https://www.clevercloud.com/developers/doc/addons/) sauf si vous utilisez une base de données ou Keycloak.
7. Injectez des **variables d’environnement** :
    ...
    <PackageManagerTabs>
      ...
      ```shell
      CC_NODE_BUILD_TOOL="custom"
      CC_PRE_BUILD_HOOK="npm install -g bun && bun install"
      CC_PRE_BUILD_HOOK="bun run astro telemetry disable && bun run build"
      ```
      </Fragment>
    </PackageManagerTabs>
    ...
    <PackageManagerTabs>
      ...
      <Fragment slot="bun">
      ```shell
      CC_POST_BUILD_HOOK="bun run build"
      CC_PRE_BUILD_HOOK="npm install -g bun && bun install && bun run astro telemetry disable"
      CC_WEBROOT="/dist"
      ```
      </Fragment>
    </PackageManagerTabs>
...
- [Documentation Clever Cloud pour déployer une application Node.js](https://www.clevercloud.com/developers/doc/applications/nodejs/) (Anglais)
- [Documentation Clever Cloud pour le déploiement d'Astro en tant qu'application statique](https://www.clevercloud.com/developers/guides/astro/) (Anglais)
