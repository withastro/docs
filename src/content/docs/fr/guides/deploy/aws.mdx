...
Pour déployer votre projet sur AWS, vous devez utiliser la [console AWS](https://aws.amazon.com/console/). (La plupart de ces actions peuvent également être effectuées à l'aide de [AWS CLI](https://aws.amazon.com/cli/)). Ce guide vous guidera à travers les étapes pour déployer votre site sur AWS en commençant par la méthode la plus basique. Ensuite, il vous montrera comment ajouter des services supplémentaires pour améliorer la rentabilité et les performances.
