...
description: Apprenez à utiliser le routage i18n dans Astro.
i18nReady: true
---
...
## Configurer le routage i18n \{#configure-i18n-routing\}

Une langue par défaut ([`defaultLocale`](/fr/reference/configuration-reference/#i18ndefaultlocale)) et une liste de toutes les langues prises en charge ([`locales`](/fr/reference/configuration-reference/#i18nlocales)) doivent être spécifiées dans un objet de configuration `i18n`. De plus, vous pouvez configurer un comportement de routage et de repli plus spécifique pour correspondre aux URL souhaitées.
...
### Créer des dossiers localisés \{#create-localized-folders\}
...
Vous serez responsable de l'écriture de votre propre logique de routage ou [d'exécuter le middleware i18n d'Astro manuellement](#middleware-function) aux côtés du vôtre.
...
#### Fonction middleware \{#middleware-function\}

La fonction [`middleware`](#middleware-function) crée manuellement le middleware i18n d'Astro. Cela vous permet d'étendre le routage i18n d'Astro plutôt que de le remplacer complètement. 
...
Votre stratégie de repli se compose de deux parties : choisir les langues vers lesquelles se rabattre ([`i18n.fallback`](/fr/reference/configuration-reference/#i18nfallback)) et choisir d'effectuer une [redirection](/fr/guides/routing/#redirects) ou une [réécriture](/fr/guides/routing/#rewrites) pour afficher le contenu de secours ([`i18n.routing.fallbackType`](/fr/reference/configuration-reference/#i18nroutingfallbacktype) ajouté dans Astro v4.15.0).
...
## Chemins d'accès aux paramètres régionaux personnalisés \{#custom-locale-paths\}
...
## Détection de la langue du navigateur \{#browser-language-detection\}
