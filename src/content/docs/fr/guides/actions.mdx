...
- Générer des fonctions avec sûreté du typage pour appeler votre backend à partir du client et même [à partir des actions de formulaire HTML](#call-actions-from-an-html-form-action). Pas besoin d'appels manuels à `fetch()`.
...
### Écrivez votre première action \{#write-your-first-action\}
...
## Organiser les actions \{#organizing-actions\}
...
## Gestion des données renvoyées \{#handling-returned-data\}
...
### Vérification des erreurs \{#checking-for-errors\}
...
### Accéder directement aux données sans vérification d'erreur \{#accessing-data-directly-without-an-error-check\}
...
### Gestion des erreurs backend dans votre action \{#handling-backend-errors-in-your-action\}
...
#### Création d'une `ActionError` \{#creating-an-actionerror\}
...
#### Gestion d'une `ActionError` \{#handling-an-actionerror\}
...
### Gestion des redirections client \{#handling-client-redirects\}

Lorsque vous appelez des actions à partir du client, vous pouvez intégrer une bibliothèque côté client comme `react-router`, ou vous pouvez utiliser la [fonction `navigate()`](/fr/guides/view-transitions/#trigger-navigation) d'Astro pour rediriger vers une nouvelle page lorsqu'une action réussit.
...
## Accepter les données d'un formulaire à partir d'une action \{#accepting-form-data-from-an-action\}
...
### Validation des données du formulaire \{#validating-form-data\}
...
2. Définir une action `newsletter` pour gérer le formulaire soumis. Validez le champ `email` en utilisant le validateur `z.string().email()` et la case à cocher `terms` en utilisant `z.boolean()` :
    ...
    <ReadMore>Consultez la [référence de l'API `input`](/fr/reference/api-reference/#input-validator) pour tous les validateurs de formulaire disponibles.</ReadMore>
...
3. Ajoutez un `<script>` au formulaire HTML pour soumettre la saisie de l'utilisateur. Cet exemple remplace le comportement de soumission par défaut du formulaire pour appeler `actions.newsletter()` et redirige vers `/confirmation` à l'aide de la fonction `navigate()` :
    ...
    <ReadMore>Consultez [« Appeler des actions depuis une action de formulaire HTML »](#call-actions-from-an-html-form-action) pour découvrir une autre façon de soumettre des données de formulaire.</ReadMore>

</Steps>

### Affichage des erreurs de saisie du formulaire \{#displaying-form-input-errors\}
...
## Appeler des actions depuis une action de formulaire HTML \{#call-actions-from-an-html-form-action\}

:::note
Les pages doivent être rendues à la demande lors de l'appel d'actions à l'aide d'une action de formulaire. [Assurez-vous que le prérendu est désactivé sur la page](/fr/guides/server-side-rendering/#opting-out-of-pre-rendering-in-hybrid-mode) avant d'utiliser cette API.
:::
...
### Redirection en cas de réussite de l'action \{#redirect-on-action-success\}
...
#### Redirection dynamique en cas de réussite de l'action \{#dynamic-redirect-on-action-success\}
...
### Gérer les erreurs d'action du formulaire \{#handle-form-action-errors\}
...
Pour plus de personnalisation, vous pouvez [utiliser l'utilitaire `isInputError()`](#displaying-form-input-errors) pour vérifier si une erreur est provoquée par une entrée non valide.
...
#### Conserver les valeurs d'entrée en cas d'erreur \{#preserve-input-values-on-error\}

Les entrées seront effacées à chaque fois qu'un formulaire sera soumis. Pour conserver les valeurs d'entrée, vous pouvez [activer les transitions de vue](/fr/guides/view-transitions/#adding-view-transitions-to-a-page) sur la page et appliquer la directive `transition:persist` à chaque entrée :

```astro ins="transition:persist"
<input transition:persist required type="email" name="email" />
```

### Mettre à jour l'interface utilisateur avec un résultat d'action de formulaire \{#update-the-ui-with-a-form-action-result\}

Le résultat renvoyé par `Astro.getActionResult()` est à usage unique et sera réinitialisé à `undefined` à chaque actualisation de la page. Ceci est idéal pour [afficher les erreurs de saisie](#handle-form-action-errors) et afficher des notifications temporaires à l'utilisateur en cas de réussite.
...
:::caution
Les données d'action sont transmises à l'aide d'un cookie persistant. **Ce cookie n'est pas chiffré.** En général, nous recommandons de renvoyer les informations minimales requises par votre gestionnaire d'actions (`handler`) pour éviter les vulnérabilités et conserver d'autres informations sensibles dans une base de données.
