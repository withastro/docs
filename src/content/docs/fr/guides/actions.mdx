---
title: Actions
description: Apprenez à créer des fonctions de serveur avec sécurité de type que vous pouvez appeler de n'importe où.
i18nReady: true
---
...
Astro Actions vous permet de définir et d'appeler des fonctions backend avec sécurité de type. Les actions effectuent la récupération des données, l'analyse JSON et la validation des entrées pour vous. Cela peut réduire considérablement la quantité de code standard nécessaire par rapport à l'utilisation d'un [point de terminaison d'API](/fr/guides/endpoints/).

Utilisez des actions au lieu de points de terminaison d'API pour une communication transparente entre votre code client et votre code serveur et pour :
...
- Générer des fonctions de type sécurisé pour appeler votre backend à partir du client et même [à partir des actions de formulaire HTML](#appeler-des-actions-depuis-une-action-de-formulaire-html). Pas besoin d'appels manuels à `fetch()`.
...
Suivez ces étapes pour définir une action et l'appeler dans une balise `script` de votre page Astro.
...
Les actions renvoient un objet contenant soit `data` avec la valeur de retour sécurisée par type de votre fonction `handler()`, soit `error` avec des erreurs backend. Les erreurs peuvent provenir d'erreurs de validation sur la propriété `input` ou d'erreurs générées dans la fonction `handler()`.
...
Vous pouvez valider les entrées du formulaire avant la soumission à l'aide des [attributs natifs de validation de formulaire HTML](https://developer.mozilla.org/fr/docs/Learn/Forms/Form_validation#utiliser_la_validation_intégrée_au_formulaire) tels que `required`, `type="email"` et `pattern`. Pour une validation `input` plus complexe sur le backend, vous pouvez utiliser la fonction utilitaire [`isInputError()`](/fr/reference/api-reference/#isinputerror) fournie.
