...
La CLI de Codex est un outil de codage assisté par IA en ligne de commande qui peut utiliser le serveur MCP d'Astro Docs pour accéder à la documentation tout en générant du code Astro.
...
[Raycast](https://www.raycast.com/) peut se connecter aux serveurs MCP pour améliorer ses capacités d'IA. L'ajout du serveur MCP d'Astro Docs permet à Raycast d'accéder à la dernière documentation d'Astro tout en répondant aux questions.
...
[GitHub Copilot](https://docs.github.com/fr/copilot/concepts/agents/coding-agent/about-coding-agent) peut être utilisé comme agent de codage alimenté par les Actions de GitHub. L'activation du serveur MCP d'Astro Docs lui permet d'accéder à la documentation d'Astro la plus récente lors de la réponse aux questions ou de l'exécution de tâches de codage.

Vous pouvez le configurer pour utiliser le serveur MCP d'Astro Docs afin d'accéder à la documentation en ajoutant ce qui suit aux paramètres de l'agent de codage Copilot de votre dépôt, disponibles à l'adresse `https://github.com/<votre-organisation>/<votre-depot>/settings/copilot/coding_agent` :
```json title="Configuration MCP"
{
  "mcpServers": {
    "astro-docs": {
...
Lorsqu'un agent de codage IA est détecté, `astro dev` et, depuis la v7.2.0, `astro preview` démarrent automatiquement le serveur en tant que processus d'arrière-plan détaché. Cela évite que le serveur de développement ne bloque le terminal de l'agent et lui permet de poursuivre son travail pendant que le serveur est en cours d'exécution.

Un fichier de verrouillage (`.astro/dev.json` ou `.astro/preview.json`) est créé au démarrage du serveur, enregistrant l'URL, le port et le PID du serveur. Cela empêche le démarrage de serveurs en double pour un même projet.

Si vous n'utilisez pas d'agent de codage IA, le serveur démarre en tant que processus au premier plan et affiche les journaux dans le terminal.

Pour désactiver le mode arrière-plan automatique, définissez la variable d'environnement `ASTRO_DEV_BACKGROUND` ou `ASTRO_PREVIEW_BACKGROUND` avant d'exécuter la commande :
...
<ReadMore>Consultez la référence de la CLI pour obtenir la liste complète des options et des sous-commandes pour [`astro dev`](/fr/reference/cli-reference/#astro-dev) et [`astro preview`](/fr/reference/cli-reference/#astro-preview).</ReadMore>
