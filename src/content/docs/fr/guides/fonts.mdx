...
## Utilisation d'un fichier d'une police locale \{#using-a-local-font-file\}
...
## Utilisation de Fontsource \{#using-fontsource\}
...
## Rajoutez des polices avec TailwindCSS \{#register-fonts-in-tailwind\}

Si vous utilisez l'intégration [Tailwind](/fr/guides/integrations-guide/tailwind/), vous pouvez utiliser l'une ou l'autre des méthodes précédentes de cette page pour installer votre police, avec quelques modifications. Vous pouvez soit ajouter une déclaration [`@font-face` pour une police locale](#using-a-local-font-file) ou utiliser la stratégie [Fontsource's `import`](#using-fontsource) pour installer votre police.
