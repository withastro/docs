...
## `fetch()` dans Astro \{#fetch-in-astro\}
...
## `fetch()` dans les Composants de Framework \{#fetch-in-framework-components\}
...
### Requêtes GraphQL \{#graphql-queries\}
...
## Récupérer à partir d'un Headless CMS \{#fetch-from-a-headless-cms\}

Les composants Astro peuvent récupérer des données de votre CMS préféré et les restituer sous forme de contenu de page. Grâce aux [routes dynamiques](/fr/guides/routing/#dynamic-routes), les composants peuvent même générer des pages basées sur le contenu de votre CMS.
