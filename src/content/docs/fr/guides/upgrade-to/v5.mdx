...
- `directRenderScript` (Voir ci-dessous les modifications apportées au [comportement `<script>` par défaut](#script-tags-are-rendered-directly-as-declared).)
- `globalRoutePriority` (Voir ci-dessous pour les changements par rapport à [l'ordre de priorité des routes par défaut](#route-priority-order-for-injected-routes-and-redirects).)
- `contentLayer` (Voir les conseils pour [mettre à jour les collections de contenu existantes](#legacy-v20-content-collections-api) vers la nouvelle API de couche de contenu préférée).
...
<ReadMore>En savoir plus sur [options de configuration de la sécurité](/fr/reference/configuration-reference/#securite)</ReadMore>
...
<ReadMore>En savoir plus sur les [règles d'ordre de priorité des routes](/fr/guides/routage/#route-priority-order).</ReadMore>
...
<ReadMore>En savoir plus sur [l'utilisation des balises `script' dans Astro](/fr/guides/client-side-scripts/#using-script-in-astro).</ReadMore>
...
<ReadMore>En savoir plus sur [TypeScript configuration in Astro](/fr/guides/typescript/#setup).</ReadMore>
...
<ReadMore>En savoir plus sur la fonction [`compiledContent()`](/fr/guides/markdown-content/#importing-markdown) pour renvoyer du Markdown compilé.</ReadMore>
...
<ReadMore>En savoir plus sur [les layouts Markdown](/fr/basics/layouts/#markdown-layouts).</ReadMore>
...
<ReadMore>En savoir plus sur [l'utilisation des plugins Markdown dans Astro](/fr/guides/markdown-content/#markdown-plugins).</ReadMore>
...
<ReadMore>En savoir plus sur [les options de configuration `build` dans Astro](/fr/reference/configuration-reference/#build-options).</ReadMore>
...
<ReadMore>Plus d'informations sur [l'utilisation des attributs HTML dans Astro](/fr/reference/astro-syntax/#dynamic-attributes).</ReadMore>
...
```
<ReadMore>En savoir plus sur [stocker des données dans `context.locals`](/fr/guides/middleware/#storing-data-in-contextlocals).</ReadMore>
...
<ReadMore>En savoir plus sur [créer des routes dynamiques avec `params`](/fr/guides/routing/#static-ssg-mode).</ReadMore>
...
<ReadMore>Voir la [Référence API pour `IntegrationRouteData`](/fr/reference/integrations-reference/#integrationroutedata-type-reference).</ReadMore>
...
<ReadMore>Plus d'informations sur [spécifier les fonctionnalités Astro prises en charge dans un adaptateur](/fr/reference/adapter-reference/#astro-features).</ReadMore>
