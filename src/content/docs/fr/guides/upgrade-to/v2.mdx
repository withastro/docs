...
## Mettre à jour Astro \{#upgrade-astro\}
...
## Changements majeurs d'Astro v2.0 \{#astro-v20-breaking-changes\}
...
### Supprimé : prise en charge de Node 14 \{#removed-support-for-node-14\}
...
### Réservé : `src/content/` \{#reserved-srccontent\}
...
### Modifié : la barre oblique finale d'`Astro.site` \{#changed-astrosite-trailing-slash\}
...
### Modifié : un dossier `_astro/` dédié aux ressources de construction \{#changed-_astro-folder-for-build-assets\}
...
### Modifié : configuration de l'extension Markdown \{#changed-markdown-plugin-configuration\}

#### Supprimé : `extendDefaultPlugins` \{#removed-extenddefaultplugins\}
...
#### Ajouté : `gfm` et `smartypants` \{#added-gfm-and-smartypants\}
...
### Modifié : configuration du plugin MDX \{#changed-mdx-plugin-configuration\}

#### Remplacé : `extendPlugins` a été remplacé par `extendMarkdownConfig` \{#replaced-extendplugins-changed-to-extendmarkdownconfig\}
...
#### Ajouté : Plus d'options de configuration MDX pour correspondre à Markdown \{#added-more-mdx-config-options-to-match-markdown\}

Astro v2.0 vous permet désormais de définir individuellement [chaque option de configuration Markdown disponible](/fr/reference/configuration-reference/#markdown-options) (à l'exception de `drafts`) séparément dans la configuration de votre intégration MDX.
...
### Modifié : accès du plugin au frontmatter \{#changed-plugin-access-to-frontmatter\}
...
### Modifié : Configuration RSS \{#changed-rss-configuration\}
...
### Modifié : prise en charge de Svelte IDE \{#changed-svelte-ide-support\}
...
### Supprimé : `legacy.astroFlavoredMarkdown` \{#removed-legacyastroflavoredmarkdown\}
...
### Supprimé : `Astro.resolve()` \{#removed-astroresolve\}
...
### Supprimé : `Astro.fetchContent()` \{#removed-astrofetchcontent\}
...
### Supprimé : `Astro.canonicalURL` \{#removed-astrocanonicalurl\}
...
### Mise à jour : Vite 4 \{#updated-vite-4\}

Astro v2.0 passe de Vite 3 à [Vite 4](https://vitejs.dev/), sorti en Décembre 2022.
...
Reportez-vous au [Guide de migration Vite](https://vitejs.dev/guide/migration.html) officiel si vous rencontrez des problèmes.

## Indicateurs expérimentaux supprimés dans Astro v2.0 \{#astro-v20-experimental-flags-removed\}
...
- [Pré-rendu de pages individuelles au format HTML statique](/fr/guides/server-side-rendering/) lors de l'utilisation de SSR pour améliorer la vitesse et la mise en cache.
