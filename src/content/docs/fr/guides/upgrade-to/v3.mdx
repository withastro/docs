...
Sinon, supprimez toutes les références d'importation du composant `<Markdown />` d'Astro et du composant lui-même dans vos fichiers `.astro`. Vous devrez réécrire votre contenu directement au format HTML ou [importer Markdown](/fr/guides/markdown-content/#importer-du-contenu-markdown) à partir d'un fichier `.md`.
...
Pour [exécuter l'Astro CLI par programmation](/fr/reference/cli-reference/#api-avancées-expérimentales), utilisez les nouvelles API JavaScript expérimentales :
...
3. Migrez tous les composants `<Image />` existants.
    ...
    Supprimez tous les attributs de composant qui ne sont pas des [propriétés d'éléments d'image actuellement prises en charge](/fr/guides/images/#propriétés).
