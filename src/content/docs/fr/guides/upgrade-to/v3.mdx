...
## Mettre à niveau Astro \{#upgrade-astro\}
...
## Indicateurs expérimentaux supprimés dans Astro v3.0 \{#astro-v30-experimental-flags-removed\}
...
- Les Transitions de Vue pour des transitions de page animées et des îles persistants. Consultez [les modifications cassantes liées à l'API des Transitions de Vue et les conseils de mise à niveau](#upgrade-view-transitions-to-v3) si vous utilisiez cet indicateur expérimental.
- Une nouvelle API de services d'images `astro:assets` pour utiliser des images dans Astro, comprenant un nouveau composant `<Image />` et une fonction `getImage()`. Veuillez lire les [conseils détaillés de mise à niveau pour vos images](#upgrade-images-to-v3) **que vous utilisiez ou non cet indicateur expérimental** pour voir comment cela pourrait affecter votre projet.
...
## Changements cassants dans Astro v3.0 \{#astro-v30-breaking-changes\}
...
### Supprimé : prise en charge de Node 16 \{#removed-support-for-node-16\}
...
### Supprimé : prise en charge de TypeScript 4 \{#removed-support-for-typescript-4\}
...
#### Que devrais-je faire ? \{#what-should-i-do-1\}
...
### Supprimé : `@astrojs/image` \{#removed-astrojsimage\}
...
#### Que devrais-je faire ? \{#what-should-i-do-2\}
...
Vous trouverez [des instructions complètes et étape par étape pour supprimer l'ancienne intégration d'image](#removed-astrojsimage) dans notre guide Images.

La migration vers `astro:assets` apportera également de nouvelles options et fonctionnalités d'image que vous souhaiterez peut-être désormais utiliser. Veuillez consulter l'intégralité des [Conseils de mise à niveau d'image v3.0](#upgrade-images-to-v3) pour plus de détails !
...
### Supprimé : composant `<Markdown />` \{#removed-markdown--component\}
...
#### Que devrais-je faire ? \{#what-should-i-do-3\}
...
Sinon, supprimez toutes les références d'importation du composant `<Markdown />` d'Astro et du composant lui-même dans vos fichiers `.astro`. Vous devrez réécrire votre contenu directement au format HTML ou [importer Markdown](/fr/guides/markdown-content/#importing-markdown) à partir d'un fichier `.md`.

### Supprimé : API obsolètes de la v1.x \{#removed-deprecated-1x-apis\}
...
#### Que devrais-je faire ? \{#what-should-i-do-4\}
...
### Supprimé : Shims partiels pour les API Web dans le code serveur \{#removed-partial-shims-for-web-apis-in-server-code\}
...
#### Que devrais-je faire ? \{#what-should-i-do-5\}
...
### Supprimé : `image` de `astro:content` dans le schéma des collections de contenu \{#removed-image-from-astrocontent-in-content-collections-schema\}
...
#### Que devrais-je faire ? \{#what-should-i-do-6\}

Si vous utilisez l'assistant obsolète `image()` de `astro:content`, supprimez-le car il n'existe plus. Validez les images via [l'assistant `image` de `schema`](#update-content-collections-schemas) à la place :
...
### Supprimé : noms des thèmes Shiki avant la version 0.14 \{#removed-pre-014-shiki-theme-names\}
...
#### Que devrais-je faire ? \{#what-should-i-do-7\}
...
### Supprimé : fonctionnalités de `class:list` \{#removed-classlist-features\}
...
#### Que devrais-je faire ? \{#what-should-i-do-8\}
...
### Supprimé : passer `class:list` en tant que propriété \{#removed-passing-classlist-as-a-prop\}

Dans Astro v2.x, [les valeurs de `class:list`](/fr/reference/directives-reference/#classlist) étaient envoyées aux composants via [`Astro.props['class:list']`](/fr/reference/api-reference/#astroprops).
...
#### Que devrais-je faire ? \{#what-should-i-do-9\}
...
### Supprimé : transformation en kebab-case pour les variables CSS en camelCase \{#removed-kebab-case-transform-for-camelcase-css-variables\}

Dans Astro v2.x, les [variables CSS](/fr/guides/styling/#css-variables) écrites en camelCase et passées à l'attribut `style` étaient rendues à la fois en camelCase (tel qu'écrit) et en kebab-case (conservé pour une rétrocompatibilité).
...
#### Que devrais-je faire ? \{#what-should-i-do-10\}
...
### Supprimé : aplatissement automatique de la valeur de retour de `getStaticPaths()` \{#removed-automatic-flattening-of-getstaticpathss-return-value\}

Dans Astro v2.x, la valeur de retour de [`getStaticPaths()`](/fr/reference/api-reference/#getstaticpaths) était automatiquement aplatie pour vous permettre de renvoyer un tableau de tableaux sans erreurs.

Astro v3.0 supprime l'aplatissement automatique du résultat de `getStaticPaths()`.

#### Que devrais-je faire ? \{#what-should-i-do-11\}
...
Un [message d'erreur indiquant que la valeur de retour de `getStaticPath()` doit être un tableau d'objets](/fr/reference/errors/invalid-get-static-paths-entry/#what-went-wrong) sera fourni si vous devez mettre à jour votre code.

### Déplacé : `astro check` nécessite désormais un paquet externe \{#moved-astro-check-now-requires-an-external-package\}
...
#### Que devrais-je faire ? \{#what-should-i-do-12\}
...
### Obsolète : `build.excludeMiddleware` et `build.split` \{#deprecated-buildexcludemiddleware-and-buildsplit\}
...
Astro v3.0 remplace ces options de configuration de construction par de nouvelles [options de configuration de l'adaptateur SSR](/fr/guides/integrations-guide/#official-integrations) pour effectuer les mêmes tâches : `edgeMiddleware` et `functionPerRoute`.

#### Que devrais-je faire ? \{#what-should-i-do-13\}
...
### Obsolète : `markdown.drafts` \{#deprecated-markdowndrafts\}
...
#### Que devrais-je faire ? \{#what-should-i-do-14\}

Pour continuer à marquer certaines pages de votre projet comme brouillons, [migrer vers les collections de contenu](/fr/guides/content-collections/#migrating-from-file-based-routing) et [filtrer manuellement les pages](/fr/guides/content-collections/#filtering-collection-queries) avec la propriété du frontmatter `draft: true` à la place.

### Obsolète : renvoyer un objet simple dans les points de terminaison \{#deprecated-returning-simple-object-in-endpoints\}
...
#### Que devrais-je faire ? \{#what-should-i-do-15\}
...
### Valeur par défaut modifiée : `verbatimModuleSyntax` dans les préréglages de tsconfig.json \{#changed-default-verbatimmodulesyntax-in-tsconfigjson-presets\}
...
#### Que devrais-je faire ? \{#what-should-i-do-16\}
...
### Valeur par défaut modifiée : port `3000` \{#changed-default-port-3000\}
...
#### Que devrais-je faire ? \{#what-should-i-do-17\}
...
### Valeur par défaut modifiée : le `trailingSlash` d'import.meta.env.BASE_URL \{#changed-default-importmetaenvbase_url-trailingslash\}
...
#### Que devrais-je faire ? \{#what-should-i-do-18\}
...
### Valeur par défaut modifiée : `compressHTML` \{#changed-default-compresshtml\}
...
#### Que devrais-je faire ? \{#what-should-i-do-19\}
...
### Valeur par défaut modifiée : `scopedStyleStrategy` \{#changed-default-scopedstylestrategy\}
...
#### Que devrais-je faire ? \{#what-should-i-do-20\}

Pour conserver la [portée de style](/fr/guides/styling/#scoped-styles) actuelle de votre projetmettez à jour le fichier de configuration avec la valeur par défaut précédente :
...
### Valeur par défaut modifiée : `inlineStyleSheets` \{#changed-default-inlinestylesheets\}
...
#### Que devrais-je faire ? \{#what-should-i-do-21\}
...
### Valeur par défaut modifiée : service d'images \{#changed-default-image-service\}

Dans Astro v2.x, Squoosh était le [service de traitement d'image par défaut](/fr/guides/images/#default-image-service).
...
#### Que devrais-je faire ? \{#what-should-i-do-22\}
...
### Modifié : cas des méthodes de requête HTTP \{#changed-http-request-methods-case\}

Dans Astro v2.x, les [méthodes de requête HTTP](/fr/guides/endpoints/#http-methods) étaient écrites en utilisant des noms de fonctions en minuscules : `get`, `post`, `put`, `all` et `del`.

Astro v3.0 utilise des noms de fonctions en majuscules, y compris `DELETE` au lieu de `del`.

#### Que devrais-je faire ? \{#what-should-i-do-23\}
...
### Modifié : configuration de plusieurs frameworks JSX \{#changed-multiple-jsx-framework-configuration\}

Dans Astro v2.x, vous pouvez utiliser [plusieurs intégrations de framework JSX](/fr/guides/integrations-guide/#official-integrations) (React, Solid, Preact) dans le même projet sans avoir besoin d'identifier quels fichiers appartenaient à quel framework.
...
#### Que devrais-je faire ? \{#what-should-i-do-24\}
...
### Modifié : `Astro.cookies.get(key)` peut renvoyer `undefined` \{#changed-astrocookiesgetkey-can-return-undefined\}

Dans Astro v2.x, [`Astro.cookies.get(key)`](/fr/reference/api-reference/#astrocookies) retournerait toujours un [objet `AstroCookie`](/fr/reference/api-reference/#astrocookie) même si le cookie n'existait pas. Pour vérifier son existence, vous deviez utiliser `Astro.cookies.has(key)`.

Astro v3.0 renvoie `undefined` pour `Astro.cookies.get(key)` si le cookie n'existe pas.

#### Que devrais-je faire ? \{#what-should-i-do-25\}
...
### Modifié : exécuter le CLI d'Astro par programmation \{#changed-running-the-astro-cli-programmatically\}
...
#### Que devrais-je faire ? \{#what-should-i-do-26\}

Pour [exécuter l'Astro CLI par programmation](/fr/reference/cli-reference/#advanced-apis-experimental), utilisez les nouvelles API JavaScript expérimentales :
...
### Modifié : chemins d'exportation des points d'entrée de l'API interne d'Astro \{#changed-internal-astro-api-entry-point-export-paths\}
...
#### Que devrais-je faire ? \{#what-should-i-do-27\}
...
## Mises à niveau des fonctionnalités \{#feature-upgrades\}

### Mettre à niveau les images vers la v3 \{#upgrade-images-to-v3\}
...
#### Mise à niveau depuis `experimental.assets` \{#upgrade-from-experimentalassets\}
...
##### Supprimer l'indicateur `experimental.assets` \{#remove-experimentalassets-flag\}
...
##### Supprimer l'alias d'importation `~/assets` \{#remove-the-assets-import-alias\}

Cet alias d'importation n'est plus inclus par défaut avec `astro:assets`. Si vous utilisiez cet alias avec des ressources expérimentales, vous devez les convertir en chemins de fichiers relatifs ou [créer vos propres alias d'importation](/fr/guides/imports/#aliases).
...
##### Ajout d'une prise en charge simple des ressources pour Cloudflare, Deno, Vercel Edge et Netlify Edge \{#add-simple-asset-support-for-cloudflare-deno-vercel-edge-and-netlify-edge\}
...
Vous devez mettre à jour l'attribut `src` de toutes les balises `<img>` existantes (y compris les [images dans les composants du framework UI](/fr/guides/images/#images-in-ui-framework-components)) et vous pouvez également mettre à jour d'autres attributs qui sont désormais disponibles à partir de l'image importée.
...
#### Mettre à jour vos fichiers Markdown, MDX et Markdoc \{#update-your-markdown-mdx-and-markdoc-files\}
...
#### Supprimer `@astrojs/image` \{#remove-astrojsimage\}
...
1. Supprimez l'intégration `@astrojs/image`.
    ...
    Vous devez [supprimer l'intégration](/fr/guides/integrations-guide/#removing-an-integration) en la désinstallant puis en la supprimant de votre fichier `astro.config.mjs`.
...
3. Migrez tous les composants `<Image />` existants.
    ...
    Supprimez tous les attributs de composant qui ne sont pas des [propriétés d'éléments d'image actuellement prises en charge](/fr/guides/images/#properties).
...
#### Mettre à jour les schémas des collections de contenu \{#update-content-collections-schemas\}
...
Le nouvel assistant `image` pour les collections de contenu vous permet de valider les métadonnées de l'image à l'aide de Zod. En savoir plus sur [comment utiliser les images dans les collections de contenu](/fr/guides/images/#images-in-content-collections)

#### Navigation dans les importations d'images dans Astro v3.0 \{#navigating-image-imports-in-astro-v30\}
...
### Mettre à niveau les transitions de vue vers la v3 \{#upgrade-view-transitions-to-v3\}
...
#### Mise à niveau depuis `experimental.viewTransitions` \{#upgrade-from-experimentalviewtransitions\}
...
##### Supprimer l'indicateur `experimental.viewTransitions` \{#remove-experimentalviewtransitions-flag\}
...
##### Mettre à jour la source d'importation \{#update-import-source\}
...
#### Mettre à jour les directives `transition:animate` \{#update-transitionanimate-directives\}
...
##### Mettre à jour les noms des événements \{#update-event-names\}
