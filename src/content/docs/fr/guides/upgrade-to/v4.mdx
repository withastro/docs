...
Vous pouvez également [mettre à niveau manuellement vos intégrations Astro](/fr/guides/integrations-guide/#manual-upgrading) si nécessaire, et vous devrez peut-être également mettre à niveau d'autres dépendances de votre projet.
...
Astro v4.0 inclut [des modifications potentiellement cassantes](#breaking-changes), ainsi que la [suppression de certaines fonctionnalités précédemment obsolètes](#previously-deprecated-features-now-removed).
...
## Indicateurs expérimentaux supprimés dans Astro v4.0 \{#astro-v40-experimental-flags-removed\}
...
### Mise à niveau : Vite 5.0 \{#upgraded-vite-50\}
...
Si vous utilisez des plugins, une configuration ou des API spécifiques à Vite, consultez le [Guide de migration de Vite](https://vitejs.dev/guide/migration) pour connaître leurs modifications majeures et mettez à niveau votre projet si nécessaire. Il n’y a aucun changement majeur dans Astro lui-même.

### Mise à niveau : dépendances unified, remark et rehype \{#upgraded-unified-remark-and-rehype-dependencies\}
...
### Renommé : `entrypoint` (API d'intégrations) \{#renamed-entrypoint-integrations-api\}
...
### Modifié : signature d'`app.render` dans l'API d'intégration \{#changed-apprender-signature-in-integrations-api\}
...
+ app.render(request, { routeData, locals })
```
### Modifié : les adaptateurs doivent désormais spécifier les fonctionnalités prises en charge \{#changed-adapters-must-now-specify-supported-features\}
...
### Supprimé : Propriété `path` du langage Shiki \{#removed-shiki-language-path-property\}
...
### Obsolète : `handleForms` pour les événements `submit` de Transitions de Vue \{#deprecated-handleforms-for-view-transitions-submit-events\}
...
## Fonctionnalités précédemment obsolètes désormais supprimées \{#previously-deprecated-features-now-removed\}
...
### Supprimé : renvoyer des objets simples à partir des points de terminaison \{#removed-returning-simple-objects-from-endpoints\}
...
### Supprimé : `build.split` et `build.excludeMiddleware` \{#removed-buildsplit-and-buildexcludemiddleware\}

Dans Astro v3.0, les options de configuration de build `build.split` et `build.excludeMiddleware` ont été définies comme obsolètes et remplacées par des [options de configuration de l'adaptateur](/fr/reference/adapter-reference/#adapter-features) pour effectuer les mêmes tâches.
...
Veuillez consulter le guide de migration vers la v3 pour [mettre à jour ces propriétés de middleware obsolètes](/fr/guides/upgrade-to/v3/#deprecated-buildexcludemiddleware-and-buildsplit) avec les configurations d'adaptateur.

### Supprimé : `Astro.request.params` \{#removed-astrorequestparams\}
...
Mettre à jour toutes les occurrences vers [`Astro.params`](/fr/reference/api-reference/#astroparams), qui est le remplacement pris en charge.
...
### Supprimé : `markdown.drafts` \{#removed-markdowndrafts\}
...
### Supprimé : utilisation de `rss` dans `getStaticPaths()` \{#removed-using-rss-in-getstaticpaths\}
...
Si vous utilisez la méthode non prise en charge pour générer des flux RSS, vous devez maintenant utiliser l'[intégration `@astrojs/rss`](/fr/guides/rss/) pour une configuration RSS complète.

### Supprimé : noms de méthodes HTTP en minuscules \{#removed-lowercase-http-method-names\}
...
Veuillez consulter le guide de migration v3 [pour obtenir des conseils sur l'utilisation des méthodes de requête HTTP en majuscules](/fr/guides/upgrade-to/v3/#changed-http-request-methods-case).

### Supprimé : Redirections 301 en cas d'absence d'un préfixe `base` \{#removed-301-redirects-when-missing-a-base-prefix\}
...
### Supprimé : Conversion automatique d'`astro/client-image` \{#removed-astroclient-image-auto-conversion\}
