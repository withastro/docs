...
Consultez notre nouvelle [documentation sur la coloration syntaxique](/fr/guides/markdown-content/#syntax-highlighting) pour obtenir plus de détails. **Si vous préférez conserver Prism comme colorateur de syntaxe,** [définissez l'option `syntaxHighlight` sur ``prism'`](/fr/guides/markdown-content/#prism-configuration) dans la configuration de markdown de votre projet.
