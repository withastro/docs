...
Astro v1.0 est passé de Vite 2 à [Vite 3](https://vitejs.dev/). Nous avons géré la majeure partie de la mise à niveau pour vous dans Astro ; cependant, certains comportements subtils de Vite peuvent encore changer entre les versions. Reportez-vous au [Guide de migration Vite](https://vitejs.dev/guide/migration.html#general-changes) officiel si vous rencontrez des problèmes.

### Obsolète : `Astro.canonicalURL`

Vous pouvez maintenant utiliser le nouvel assistant [`Astro.url`](/fr/reference/api-reference/#astrourl) pour construire votre propre URL canonique à partir de l'URL de la page/de la demande actuelle. 
...
Les flux RSS doivent maintenant être générés à l'aide du paquet `@astrojs/rss`, comme décrit dans notre [Guide RSS](/fr/guides/rss/).
...
Consultez notre nouvelle [documentation sur la coloration syntaxique](/fr/guides/markdown-content/#coloration-syntaxique) pour obtenir plus de détails. **Si vous préférez conserver Prism comme colorateur de syntaxe,** [définissez l'option `syntaxHighlight` sur ``prism'`](/fr/guides/markdown-content/#configuration-de-prism) dans la configuration de markdown de votre projet.
...
Pour en savoir plus sur les plugins Vite, veuillez consulter leur [guide des plugins](https://vitejs.dev/guide/using-plugins.html).
...
Dans Astro v0.21+, des alias d'importation peuvent être ajoutés à partir de `tsconfig.json` ou `jsconfig.json`.
