...
### Modifié : valeur par défaut de `i18n.routing.redirectToDefaultLocale`
...
Dans Astro v6.0, la valeur par défaut de `i18n.routing.redirectToDefaultLocale` est désormais `false`. De plus, elle ne peut être utilisée que si `i18n.routing.prefixDefaultLocale` est défini sur `true`.
...
### Modifié : Les balises `<script>` et `<style>` sont générées dans l'ordre où elles sont définies.
...
### Modifiée : Manière dont les styles d'images adaptatives sont émis

<SourcePR number="15407" title="support responsive images"/>

Dans Astro 5.x, les images étaient calculées au moment de l'exécution et les styles `fit` et `pos` des images réactives étaient injectés dans un attribut `style`. Cela ne permettait pas la compatibilité avec la stratégie de sécurité du contenu (CSP) d'Astro pour de nombreuses raisons.
...
### Modifié : Les points de terminaison avec une extension de fichier ne sont pas accessibles avec une barre oblique finale
...
### Modifiées : Les valeurs de `import.meta.env` sont toujours incorporées
...
Si vous utilisiez auparavant cette fonctionnalité expérimentale, vous devez [supprimer cette option expérimentale de votre configuration](#experimental-flags) car elle n'existe plus.
...
### Modifié : Recadrage par défaut dans le service d'images par défaut
...
### Modifié : Ne jamais agrandir les images dans le service d’images par défaut
...
### Modifiée : Rastérisation SVG

<SourcePR number="15180" title="add support for SVG rasterization" />

Dans Astro v5.x, le service d'images Sharp par défaut d'Astro était incapable de convertir les fichiers SVG en fichiers raster (par exemple PNG, WebP). Par conséquent, le composant `<Image />` ignorait toute valeur définie pour `format` lors de l'optimisation et de la transformation des fichiers SVG.
...
Si vous comptiez auparavant sur le fait que le service d'images ignorait automatiquement la conversion des SVG, vous devez désormais vérifier le format de vos images au préalable afin d'éviter la conversion des SVG en images raster :
...
### Modifié : La fonction `getImage()` émet une erreur lorsqu'elle est appelée côté client.
...
Astro 6.0 génère une erreur d'exécution lorsque `getImage()` est appelé côté client.
...
<ReadMore>Voir [générer des images avec `getImage()`](/fr/guides/images/#générer-des-images-avec-getimage) pour un exemple complet.</ReadMore>

### Modifié : Génération d'identifiants de titres Markdown
...
Dans Astro 5.5, l'option `experimental.headingIdCompat` a été introduite pour vous permettre de rendre les ID générés par Astro pour les titres Markdown compatibles avec les plateformes courantes comme GitHub et npm, en utilisant le package populaire [`github-slugger`](https://github.com/Flet/github-slugger).
...
### Modifié : `getStaticPaths()` ne peut pas renvoyer de paramètres de type nombre
...
### Modifié : Les composants Astro ne peuvent pas être rendus dans les environnements clients de Vitest (API des conteneurs)
...
### Modifié : Nom de la configuration Rollup pour les chemins de fichier de sortie (configuration de Vite)
...
### Modifié : Hooks des intégrations et modèles d'accès à HMR (API des intégrations)
...
<ReadMore>Apprenez-en davantage sur l’[API Environment de Vite](https://vite.dev/guide/api-environment) et les [hooks d’intégration](/fr/reference/integrations-reference/#astrobuildsetup) d’Astro.</ReadMore>

### Modifié : Structure de l'interface `SSRManifest` (API des adaptateurs)
...
```ts del={2} ins={3}
// Pour récupérer le même format (par exemple, `file:///path/to/src`), effectuez la modification suivante :
...
### Modifié : Les types des schémas sont déduits au lieu d'être générés (API des chargeurs de contenu).
