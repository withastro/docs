...
Depuis Astro v6, les chemins de route peuvent être partiellement décodés lors de la normalisation des routes. Cela signifie que `Astro.url.pathname` peut être différent de `new URL(Astro.request.url).pathname` pour les segments de route encodés en pourcentage.

Cela affecte également les paramètres de route dérivés des chemins normalisés.

#### Que devrais-je faire ?

Si vous avez besoin d'accéder au chemin encodé d'origine, utilisez `new URL(Astro.request.url).pathname` au lieu de `Astro.url.pathname`.
