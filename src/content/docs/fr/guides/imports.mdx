...
Nous encourageons les utilisateurs d'Astro à éviter les builtins Node.js (`fs`, `path`, etc.) dans la mesure du possible. Astro est compatible avec de nombreux moteurs d'exécution utilisant des [adaptateurs](/fr/guides/server-side-rendering/). Cela inclut [Deno](https://github.com/denoland/deno-astro-adapter) et [Cloudflare Workers](/en/guides/integrations-guide/cloudflare/) qui ne supportent pas les modules intégrés de Node tels que `fs`.antes.
