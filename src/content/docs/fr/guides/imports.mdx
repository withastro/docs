...
### Les fichiers dans `public/` \{#files-in-public\}
...
## Déclarations d'importation \{#import-statements\}
...
Astro comprend un support intégré pour [TypeScript] (https://www.typescriptlang.org/). Vous pouvez importer des fichiers `.ts` et `.tsx` directement dans votre projet Astro, et même écrire du code TypeScript directement dans votre [script de composant Astro](/fr/basics/astro-components/#the-component-script) et n'importe quelle [balise de scripts](/fr/guides/client-side-scripts/).
...
## Autres ressources \{#other-assets\}
...
### Patterns globaux \{#glob-patterns\}
...
#### Patterns globaux dans Astro \{#glob-patterns-in-astro\}
...
## Extension de la prise en charge des types de fichiers \{#extending-file-type-support\}
