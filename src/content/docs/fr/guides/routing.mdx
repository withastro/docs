...
Par défaut, Astro gère chaque requête avec un pipeline intégré qui exécute les gestionnaires dans un ordre fixe : normalisation des barres obliques finales, redirections, sessions, actions, middleware utilisateur, rendu des pages, internationalisation (i18n) et mise en cache. Ce pipeline est conçu pour couvrir les cas d'utilisation les plus courants en matière de routage et de gestion des requêtes, mais il peut ne pas convenir aux besoins de tous les projets.

Le routage avancé d'Astro vous permet de remplacer ce pipeline par le vôtre. Vous pouvez choisir les fonctionnalités intégrées à utiliser et où les utiliser. Vous pouvez également ajouter votre propre logique personnalisée n'importe où dans le pipeline. Cela vous donne un contrôle total sur la manière dont Astro gère les requêtes entrantes.
...
Astro prend en charge plusieurs formats de fichiers pour son point d'entrée de routage avancé : `.ts`, `.js`, `.mjs` et `.mts`. Nous recommandons d'utiliser `.js` dans la plupart des cas ou `.ts` si vous avez besoin de la prise en charge de TypeScript.

### Changer le nom du fichier du point d'entrée

Par défaut, Astro recherche `src/fetch.ts` comme point d'entrée du routage avancé. Vous pouvez le modifier en définissant l'option [`fetchFile`](/fr/reference/configuration-reference/#fetchfile) dans votre fichier de configuration d'Astro.

L'exemple suivant indique à Astro de rechercher `src/handler.ts` au lieu de `src/fetch.ts` :
...
Le principal avantage du routage avancé est la possibilité d'insérer une logique personnalisée dans le pipeline de requêtes. Vous pouvez exécuter du code avant qu'Astro ne traite la requête, entre les étapes du pipeline ou après la production de la réponse.
...
Utilisez [`astro()`](/fr/reference/modules/astro-fetch/#astro) lorsque vous souhaitez conserver le comportement de routage intégré d'Astro, mais avez besoin d'une logique personnalisée autour. Cette approche préserve l'ordre du pipeline par défaut et vous permet d'ajouter un pré-traitement et un post-traitement en un seul endroit. Pour de nombreux cas d'utilisation, tels que la journalisation des requêtes et les en-têtes personnalisés, `astro()` est tout ce dont vous avez besoin.

L'exemple suivant affiche dans la console chaque requête entrante avant d'exécuter le pipeline d'Astro, et ajoute un en-tête personnalisé à la réponse une fois qu'Astro a terminé l'exécution :
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // Pré-traitement personnalisé, s'exécute avant tout gestionnaire d'Astro
    console.log(`${request.method} ${new URL(request.url).pathname}`);

    const response = await astro(state);

    // Post-traitement personnalisé, s'exécute après le rendu d'Astro
...
:::caution
Il n'est pas garanti que le chemin de la requête entrante soit identique à la route qu'Astro fait correspondre en interne, donc une vérification telle que `url.pathname.startsWith('/dashboard')` peut être contournée.

N'utilisez pas le pré-traitement pour autoriser les requêtes en comparant le chemin de la requête à une chaîne de caractères. Pour protéger les routes, utilisez un routeur qui gère la correspondance des routes, tel que [Hono](#utilisation-avec-hono).
:::
...
Astro fournit également des enveloppes compatibles avec Hono pour toutes les fonctions de gestionnaire via [`astro/hono`](/fr/reference/modules/astro-hono/). Si vous préférez utiliser [Hono](https://hono.dev/) comme framework de routage, vous pouvez exporter une application Hono depuis `src/fetch.ts` :
...
Vous pouvez également protéger des routes en enregistrant une vérification d'autorisation sur les routes Hono que vous souhaitez protéger :
...
// Protéger toutes les routes sous /tableau-de-bord.
app.use('/tableau-de-bord', requireAuth);
app.use('/tableau-de-bord/*', requireAuth);
