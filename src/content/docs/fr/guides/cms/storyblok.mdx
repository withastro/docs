...
Pour tester la configuration, dans Storyblok, créez une nouvelle histoire avec le type de contenu `blogPost` nommée `test-post`.
...
Pour rendre votre contenu, passez la propriété `content` de l'histoire au `StoryblokComponent` en tant que prop `blok`. Ce composant rendra les Bloks définis dans la propriété `content`. Dans ce cas, il rendra le composant `BlogPost`.
...
2. **Un projet Astro intégré à Storyblok** - Voir [integrating with Astro](#Intégration-avec-Astro) pour les instructions sur la façon de mettre en place l'intégration.
...
Pour ajouter un nouveau contenu, accédez à la section Contenu en cliquant sur l'onglet **Contenu**. En utilisant la bibliothèque Blok que vous avez créée à l'étape précédente, créez les histoires suivantes :
...
Si vous utilisez la génération de site statique par défaut d'Astro, vous utiliserez [dynamic routes](/fr/core-concepts/routing/#routes-dynamiques) et la fonction `getStaticPaths` pour générer les pages de votre projet.
...
Ce fichier génère une page pour chaque histoire, avec le titre et le contenu récupérés de l'API Storyblok.
...
Si l'histoire n'est pas trouvée, la demande sera redirigée vers la page 404.
...
- Storyblok fournit une [Astro Intégration](https://www.storyblok.com/mp/announcing-storyblok-astro) pour ajouter Storyblok à votre projet.
