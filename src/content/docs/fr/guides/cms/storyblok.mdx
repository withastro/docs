...
### Mise en place des identifiants \{#setting-up-credentials\}
...
### Configuration de Storyblok \{#configuring-storyblok\}
...
### Connexion des Bloks aux composants Astro \{#connecting-bloks-to-astro-components\}
...
## Créer un blog avec Astro et Storyblok \{#making-a-blog-with-astro-and-storyblok\}
...
2. **Un projet Astro intégré à Storyblok** - Voir [les intégrations avec Astro](#integrating-with-astro) pour les instructions sur la façon de mettre en place l'intégration.

### Création d'une librairie blok \{#creating-a-blok-library\}
...
### Connecter les Bloks aux composants \{#connecting-bloks-to-components\}
...
### Générer des pages \{#generating-pages\}
...
Si vous utilisez la génération de site statique par défaut d'Astro, vous utiliserez les [routes dynamiques](/fr/guides/routing/#dynamic-routes) et la fonction `getStaticPaths` pour générer les pages de votre projet.
...
#### Reconstruction sur les changements de Storyblok \{#rebuild-on-storyblok-changes\}
...
##### Ajouter un webhook à Storyblok \{#adding-a-webhook-to-storyblok\}
