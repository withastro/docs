...
Si vous souhaitez obtenir [TypeScript IntelliSense pour ces variables d'environnement](/fr/guides/environment-variables/#intellisense-for-typescript), vous pouvez créer un nouveau fichier `env.d.ts` dans le répertoire `src/` et configurer `ImportMetaEnv` comme ceci :
...
:::note
En savoir plus sur [l'obtention de variables d'environnement dans Astro](/fr/guides/environment-variables/#getting-environment-variables).
:::
...
## Créer un blog avec Astro et Kontent.ai \{#making-a-blog-with-astro-and-kontentai\}
...
### Configuration du modèle de contenu \{#setting-up-content-model\}
...
### Générer un modèle de contenu en TypeScript \{#generating-content-model-in-typescript\}
...
Dans cette section, vous allez utiliser le [Mode Static (SSG)](/fr/guides/routing/#static-ssg-mode) avec Astro.
...
#### Reconstruction sur les changements de Kontent.ai \{#rebuild-on-kontentai-changes\}
...
##### Ajouter un webhook à Kontent.ai \{#adding-a-webhook-to-kontentai\}
