...
Depuis le [backoffice Umbraco](https://docs.umbraco.com/umbraco-cms/fundamentals/backoffice), créez un type de document pour un simple article de blog appelé 'Article'.
...
3. Dans la section "Content" du backoffice Umbraco, [créez et publiez votre premier article de blog](https://docs.umbraco.com/umbraco-cms/fundamentals/data/defining-content). Répétez le processus autant de fois que vous le souhaitez.
	...
	<h2>Articles de blog</h2>
	{
        articles.items.map((article: any) => (
            <div>
                ...
                <p>{article.properties.articleDate}</p>
                <a href={article.route.path}>Lire plus</a>
            </div>
        ))
    }
</Layout>
