...
Si vous avez [opté pour le mode SSR](/fr/guides/on-demand-rendering/) avec `output : server` ou `output : hybrid`, [générez vos routes dynamiques](/fr/guides/routing/#mode-serveur-ssr) en utilisant le code suivant :
