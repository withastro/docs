...
### Ajouter l'URL de Strapi dans `.env` \{#adding-the-strapi-url-in-env\}
...
### Création de l'API wrapper \{#creating-the-api-wrapper\}
...
### Facultatif : Création de l'interface Article \{#optional-creating-the-article-interface\}
...
### Affichage d'une liste d'articles \{#displaying-a-list-of-articles\}
...
### Générer des pages d'articles \{#generating-article-pages\}

Créez le fichier `src/pages/blog/[slug].astro` pour [générer dynamiquement une page](/fr/guides/routing/#dynamic-routes) pour chaque article.
...
#### Génération du site statique \{#static-site-generation\}
...
Si vous avez [opté pour le mode SSR](/fr/guides/server-side-rendering/) avec `output : server` ou `output : hybrid`, [générez vos routes dynamiques](/fr/guides/routing/#server-ssr-mode) en utilisant le code suivant :
...
#### Reconstruction en cas de modifications \{#rebuild-on-changes\}
...
##### Ajouter un webhook à Strapi \{#adding-a-webhook-to-strapi\}
