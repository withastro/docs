...
Créez le fichier `src/pages/blog/[slug].astro` pour [générer dynamiquement une page](/fr/basics/routing/#routes-dynamiques) pour chaque article.
...
Si vous avez [opté pour le mode SSR](/fr/guides/server-side-rendering/) avec `output : server` ou `output : hybrid`, [générez vos routes dynamiques](/fr/basics/routing/#mode-serveur-ssr) en utilisant le code suivant :
