...
### Mise en place des informations d'identification \{#setting-up-credentials\}
...
:::tip
Si vous disposez d'un espace Contentful vide, consultez [setting up a Contentful model](#setting-up-a-contentful-model) pour apprendre à créer un modèle de blog de base pour votre contenu.
:::

Vous trouverez d'autres options de recherche dans la [documentation Contentful](https://contentful.github.io/contentful.js/).

## Créer un blog avec Astro et Contentful \{#making-a-blog-with-astro-and-contentful\}

Avec la configuration ci-dessus, vous êtes maintenant en mesure de créer un blog qui utilise Contentful comme CMS.

### Conditions préalables \{#prerequisites-1\}
...
2. **Un projet Astro intégré au [SDK Contentful](https://github.com/contentful/contentful.js)** - Voir l'[intégration avec Astro](#integrating-with-astro) pour plus de détails sur la façon de configurer un projet Astro avec Contentful.

### Mise en place d'un modèle Contentful \{#setting-up-a-contentful-model\}
...
### Afficher une liste d'articles de blog \{#displaying-a-list-of-blog-posts\}
...
Si vous utilisez le mode statique par défaut d'Astro, vous utiliserez [dynamic routes](/fr/guides/routing/#dynamic-routes) et la fonction `getStaticPaths()`. Cette fonction sera appelée au moment de la construction pour générer la liste des chemins qui deviendront des pages.
...
Si vous avez [opté pour le mode SSR](/fr/guides/server-side-rendering/), vous utiliserez une route dynamique qui utilise un paramètre `slug` pour récupérer les données de Contentful.

Créez une page `[slug].astro` dans `src/pages/posts`. Utilisez [`Astro.params`](/fr/reference/api-reference/#astroparams) pour récupérer le slug de l'URL, puis passez-le à `getEntries` :
...
Si l'entrée n'est pas trouvée, vous pouvez rediriger l'utilisateur vers la page 404 en utilisant [`Astro.redirect`](/fr/guides/routing/#dynamic-redirects).
...
#### Reconstruction en cas de changements dans Contentful \{#rebuild-on-contentful-changes\}
...
##### Ajouter un webhook à Contentful \{#adding-a-webhook-to-contentful\}
