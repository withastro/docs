...
Si vous utilisez le mode statique par défaut d'Astro, vous utiliserez [dynamic routes](/fr/basics/routing/#routes-dynamiques) et la fonction `getStaticPaths()`. Cette fonction sera appelée au moment de la construction pour générer la liste des chemins qui deviendront des pages.
...
Si l'entrée n'est pas trouvée, vous pouvez rediriger l'utilisateur vers la page 404 en utilisant [`Astro.redirect`](/fr/basics/routing/#redirections-dynamiques).
