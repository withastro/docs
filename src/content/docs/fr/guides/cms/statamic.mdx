...
[Statamic](https://statamic.com/) est un système de gestion de contenu (CMS) moderne et plat. Il permet aux développeurs de créer facilement des sites web et des applications dynamiques tout en offrant aux éditeurs de contenu une interface intuitive et conviviale pour la gestion du contenu.
...
FRécupérez vos données Statamic à partir de l'URL de l'API REST de votre site. Par défaut, c'est `https://[VOTRE-SITE]/api/`. Ensuite, vous pouvez rendre les propriétés de vos données en utilisant la directive Astro `set:html={}`. 
