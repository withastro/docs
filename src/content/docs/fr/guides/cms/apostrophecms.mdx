...
## Intégration avec Astro \{#integrating-with-astro\}
...
### Mise en place de la communication entre les projets \{#setting-up-project-communication\}
...
### Configuration d'Astro \{#configuring-astro\}
...
### Connecter les widgets d'ApostropheCMS aux composants Astro \{#connecting-apostrophecms-widgets-to-astro-components\}
...
Les composants mappés situés dans ce dossier reçoivent une propriété `widget` contenant les champs du schéma de votre widget, et tous les props personnalisés, à travers `Astro.props`. Ces valeurs sont alors disponibles pour l'édition sur la page.
...
Voir [la documentation d'ApostropheCMS](https://apostrophecms.com/extensions/astro-integration) pour les conventions de nommage des widgets standard, pro et personnalisés au niveau du projet.
...
### Ajouter des types de pages \{#adding-page-types\}
...
### Création du composant [...slug.astro] et récupération des données d'Apostrophe \{#creating-the-slugastro-component-and-fetching-apostrophe-data\}
...
## Créer un blog avec Astro et ApostropheCMS \{#making-a-blog-with-astro-and-apostrophecms\}
...
2. **Un projet Astro intégré à ApostropheCMS** - Pour créer un projet à partir de zéro, voir [integrating with Astro](#integrating-with-astro) pour les instructions sur la façon de configurer l'intégration, ou utiliser le projet de démarrage [astro-frontend](https://github.com/apostrophecms/astro-frontend).

### Création d'un article de blog et d'un type de page d'article \{#creating-a-blog-piece-and-piece-page-type\}
...
### Création du schéma du blog \{#creating-the-blog-schema\}

Dans un projet ApostropheCMS, les éditeurs disposent d'un ensemble de champs de saisie pour ajouter du contenu. Voici un exemple de billet de blog simple qui ajoute trois champs de saisie, un `authorName`, un `publicationDate` et une zone de `content` où les gestionnaires de contenu peuvent ajouter plusieurs instances de widgets. Dans ce cas, nous spécifions qu'ils peuvent ajouter les widgets image et texte riche que nous avons créés pendant la [configuration de l'intégration](#connecting-apostrophecms-widgets-to-astro-components).
...
### Affichage des articles de blog \{#displaying-the-blog-posts\}
...
Après avoir récupéré les données de la page et des articles dans le props `aposData`, ce composant crée des balises en utilisant les deux champs du schéma de l'article de blog que nous avons créé, mais aussi les `piece.title` et `piece._url` qui sont ajoutés à chaque article par Apostrophe.
...
### Créer des articles de blog \{#creating-blog-posts\}
...
### Déployer votre site \{#deploying-your-site\}
