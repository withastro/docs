...
1. Un Hashnode Blog
2. Un projet Astro intégré avec le paquetage [graphql-request](https://github.com/jasonkuhrt/graphql-request) installé.
...

        {
            allPosts.map((post) => (
                <div>
                    ...
                    <a href={`/post/${post.node.slug}`}>Lire plus</a>
                </div>
            ))
        }
    </body>
...
3. Créez le modèle pour chaque page en utilisant les propriétés de chaque objet `post`. L'exemple ci-dessous montre le titre de l'article et l'heure de lecture, puis le contenu complet de l'article :
