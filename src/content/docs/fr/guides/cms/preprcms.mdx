...
- Un projet Astro nouveau ou existant configuré pour le [rendu à la demande](/fr/guides/server-side-rendering/).
...
### Configuration du point de terminaison Prepr \{#configuring-the-prepr-endpoint\}
...
#### Créez une requête GraphQL pour récupérer les articles de votre blog : \{#create-a-graphql-query-to-retrieve-your-blog-articles\}
...
#### Création de pages individuelles pour les articles de blog \{#creating-individual-blog-post-pages\}

Pour créer une page pour chaque article de blog, vous devez exécuter une nouvelle requête GraphQL sur une page [dynamic routing](/fr/guides/routing/#server-ssr-mode) `.astro`. Cette requête récupérera un article spécifique par son nom (slug) et une nouvelle page sera créée pour chaque article de blog.
