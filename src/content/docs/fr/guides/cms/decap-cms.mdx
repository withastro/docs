...
## Installation de DecapCMS \{#installing-decapcms\}
...
### Decap CMS avec Netlify Identity \{#decap-cms-with-netlify-identity\}
...
### Decap CMS avec des clients OAuth externes \{#decap-cms-with-external-oauth-clients\}

Lorsque vous déployez vers des hébergeurs autres que Netlify, vous devez créer vos propres routes OAuth.

Dans Astro, cela peut être fait avec des routes rendues à la demande dans votre projet configuré avec [la sortie `server` ou `hybrid`](/fr/guides/server-side-rendering/#enable-on-demand-server-rendering) activé.
...
## Sites de production \{#production-sites\}
