...
- Un projet Astro existant [avec le rendu côté serveur (SSR) et `output : 'hybrid'` configuré](/fr/guides/server-side-rendering/).
...
Pour afficher le contenu d'un article individuel, vous pouvez importer et utiliser le composant `<Content />` pour [rendre votre contenu en HTML](/fr/guides/collections de contenus/#rendering-content-to-html) :
