...
## Intégration de Cosmic avec Astro \{#integrating-cosmic-with-astro\}

### Installation des dépendances nécessaires \{#installing-necessary-dependencies\}
...
### Configuration des clés API \{#configuring-api-keys\}
...
## Récupérer des données de Cosmic \{#fetching-data-from-cosmic\}
...
## Créer un blog avec Astro et Cosmic \{#building-a-blog-with-astro-and-cosmic\}
...
### Objets de contenu Cosmic \{#cosmic-content-objects\}
...
En utilisant la même [méthode de récupération de données](#fetching-data-from-cosmic) que ci-dessus, importez la requête `getAllPosts()` de votre fichier script et attendez les données. Cette fonction fournit des métadonnées sur chaque "post" qui peuvent être affichées dynamiquement.
...
### Générer des articles de blog individuels avec Astro \{#generating-individual-blog-posts-with-astro\}

Pour générer une page individuelle avec un contenu complet pour chaque article de blog, créez une [page de routage dynamique](/fr/guides/routing/#dynamic-routes) dans `src/pages/blog/[slug].astro`.
...
#### Rebuild sur les mises à jour du contenu de Cosmic \{#rebuild-on-cosmic-content-updates\}
