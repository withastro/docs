...
<Steps>
1. Installez les paquets [`unstorage`](https://www.npmjs.com/package/unstorage) et [`ioredis`](https://www.npmjs.com/package/ioredis) :
    <PackageManagerTabs>
      <Fragment slot="npm">
        ```sh
...
3. Utilisez ce fichier comme point d'entrée du pilote dans votre configuration d'Astro :
...
Le moteur d'exécution des sessions est automatiquement exclu du serveur compilé lorsqu'aucun pilote de session n'est configuré. Définir `session: false` indique également à votre adaptateur de ne pas fournir son pilote par défaut, désactivant ainsi complètement la prise en charge des sessions pour votre projet :
...
Les adaptateurs qui fournissent normalement un pilote de session par défaut (par exemple, [Node](/fr/guides/integrations-guide/node/#sessions), [Cloudflare](/fr/guides/integrations-guide/cloudflare/#sessions) et [Netlify](/fr/guides/integrations-guide/netlify/#sessions)) n'en configureront pas. Cela peut être utile pour les environnements serverless et en périphérie, où un paquet compilé plus petit permet de diminuer le temps de démarrage à froid.
