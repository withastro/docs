...
// Protégez toutes les routes sous /dashboard.
app.use("/dashboard", requireAuth);
app.use("/dashboard/*", requireAuth);

// Exécutez le pipeline intégré d'Astro pour toutes les autres requêtes.
app.use(astro());
...
:::caution
Le chemin public perçu par un middleware n'est pas nécessairement identique à la route qu'Astro associe en interne : une `base` configurée, l'encodage des URL et des barres obliques en double peuvent tous entraîner des différences. Un attaquant peut exploiter cette faille pour atteindre une route protégée avec un chemin que votre vérification ne reconnaît pas.

N'autorisez pas les requêtes en comparant `context.url.pathname` à une chaîne de caractères (p. ex. `context.url.pathname === "/dashboard"` ou `context.url.pathname.startsWith("/dashboard")`). Restreignez plutôt l'accès au niveau d'un routeur qui gère la correspondance des routes pour vous.
:::
