...
## Générer des `items` \{#generating-items\}
...
### Utilisation des collections de contenu \{#using-content-collections\}
...
### Utilisation des importations globales \{#using-glob-imports\}

<p><Since v="2.1.0" pkg="@astrojs/rss" /></p>

Pour créer un flux RSS à partir des documents contenus dans `src/pages/`, utilisez l'aide `pagesGlobToRssItems()`. Elle accepte un résultat [`import.meta.glob`](https://vitejs.dev/guide/features.html#glob-import) et produit un tableau d'éléments de flux RSS valides (voir [plus d'informations sur l'écriture de motifs glob](/fr/guides/imports/#glob-patterns) pour spécifier les pages à inclure).
...
### Inclure le contenu intégral de l'article \{#including-full-post-content\}
...
## Suppression des barres obliques \{#removing-trailing-slashes\}
...
## Ajouter une feuille de style \{#adding-a-stylesheet\}
...
## Activation de la découverte automatique des flux RSS \{#enabling-rss-feed-auto-discovery\}
