...
Vous pouvez ajouter la prise en charge de Tailwind 4 à votre projet avec une commande CLI ou installer manuellement les anciennes dépendances pour [prendre en charge Tailwind 3](#prise-en-charge-héritée-de-tailwind-3).

Dans Astro `>=5.2.0`, utilisez la commande `astro add tailwind` de votre gestionnaire de paquets pour installer le plugin Vite officiel pour Tailwind. Cela fournit la prise en charge de Tailwind `>=4.0.0`. Pour ajouter la prise en charge de Tailwind 4 aux versions antérieures d'Astro, [suivez les instructions dans la documentation de Tailwind][tailwind] pour ajouter ce plugin Vite manuellement.
