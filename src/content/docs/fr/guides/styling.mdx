...
##  Styliser avec Astro \{#styling-in-astro\}
...
### Combiner des classes avec `class:list` \{#combining-classes-with-classlist\}
...
### Transmettre une `classe` à un composant enfant \{#passing-a-class-to-a-child-component\}
...
### Importer une feuille de style locale \{#import-a-local-stylesheet\}

:::caution[Utilisation d'un paquet npm ?]
Vous pouvez avoir besoin de mettre à jour votre `astro.config` lorsque vous importez des paquets npm. Voir la section ["importer des feuilles de style depuis un paquet npm"](#import-a-stylesheet-from-an-npm-package) ci-dessous.
:::

Vous pouvez importer des feuilles de style dans le frontmatter de vos composants Astro en utilisant la syntaxe d'importation ESM. Les importations CSS fonctionnent comme [toute autre importation ESM dans un composant Astro](/fr/basics/astro-components/#the-component-script), qui doit être référencé comme **relatif au composant** et doit être écrit en **haut** du script de votre composant, avec toutes les autres importations.
...
### Importer une feuille de style à partir d'un paquet npm \{#import-a-stylesheet-from-an-npm-package\}
...
### Charger une feuille de style statique via les balises "link \{#load-a-static-stylesheet-via-link-tags\}
...
Comme cette approche utilise le répertoire `public/`, elle ne tient pas compte du traitement CSS normal, du regroupement et des optimisations fournis par Astro. Si vous avez besoin de ces transformations, utilisez la méthode [Import a Stylesheet](#import-a-local-stylesheet) ci-dessus.

## Ordre en cascade \{#cascading-order\}
...
### Styles encadrés \{#scoped-styles-1\}

L'utilisation de [scoped styles](#scoped-styles) n'augmente pas la _spécificité_ de votre CSS, mais ils viendront toujours en dernier dans l'_ordre d'apparition_. Ils auront donc la priorité sur les autres styles de même spécificité. Par exemple, si vous importez une feuille de style qui entre en conflit avec un style scopé, c'est la valeur du style scopé qui s'appliquera :
...
### Link Tags \{#link-tags\}
Les feuilles de style chargées via [les balises de lien](#load-a-static-stylesheet-via-link-tags) sont évaluées dans l'ordre, avant tout autre style dans un fichier Astro. Par conséquent, ces styles ont une priorité inférieure à celle des feuilles de style importées et des styles délimités :
...
## Intégrations CSS \{#css-integrations\}
...
### Dans les composants du framework \{#in-framework-components\}
...
## Frameworks et librairies \{#frameworks-and-libraries\}
...
## Style Markdown \{#markdown-styling\}
...
Vous pouvez appliquer des styles globaux à votre contenu Markdown en ajoutant des [feuilles de style importées](#external-styles) à la mise en page qui englobe le contenu de votre page. Il est également possible d'appliquer un style à votre document Markdown avec des balises [`<style is:global>`](#global-styles) dans le composant de mise en page.  Notez que tous les styles ajoutés sont soumis à [l'ordre de cascade d'Astro](#cascading-order), et vous devriez vérifier votre page rendue avec soin pour vous assurer que vos styles sont appliqués comme prévu.

Vous pouvez également ajouter des [intégrations CSS](#css-integrations), notamment [Tailwind](/fr/guides/integrations-guide/tailwind/). Si vous utilisez Tailwind, le [plugin de typographie](https://tailwindcss.com/docs/typography-plugin) peut être utile pour styliser Markdown.

## Production \{#production\}

### Contrôle des paquets \{#bundle-control\}
...
### Importations CSS `?raw` \{#raw-css-imports\}
...
### Importations CSS `?url` \{#url-css-imports\}
