...
## Que sont les collections de contenu ? \{#what-are-content-collections\}
...
Une fois que vous avez une collection, vous pouvez commencer à [interroger votre contenu](#querying-collections) en utilisant les API de contenu intégrées d'Astro.

### Le répertoire ".astro" \{#the-astro-directory\}
...
### Organisation avec plusieurs collections  \{#organizing-with-multiple-collections\}
...
### Organisation avec des sous-répertoires \{#organizing-with-subdirectories\}
...
## Définition des collections \{#defining-collections\}
...
### Configuration de TypeScript \{#setting-up-typescript\}
...
### Définition d'un schéma de collection \{#defining-a-collection-schema\}
...
### Définir plusieurs collections \{#defining-multiple-collections\}
...
```ts
// src/content/config.ts
// 1. Importe les propriétés et schémas
import { defineCollection } from 'astro:content';
...
### Utilisation de schémas de collection tiers \{#using-third-party-collection-schemas\}
...
### Définition des types de données avec Zod \{#defining-datatypes-with-zod\}
...
### Définition des références de collection \{#defining-collection-references\}
...
### Définition d'un slug personnalisé \{#defining-custom-slugs\}
...
## Interroger les collections \{#querying-collections\}
...
### Accès aux données référencées \{#accessing-referenced-data\}

Toutes les [références définies dans votre schéma](#defining-collection-references) doivent être interrogées séparément après avoir interrogé votre entrée de collection. Vous pouvez utiliser à nouveau la fonction `getEntry()`, ou `getEntries()`, pour récupérer l'entrée référencée dans l'objet `data` retourné. 
...
### Filtrer les requêtes sur les collections \{#filtering-collection-queries\}
...
### Utilisation du contenu dans les modèles Astro \{#using-content-in-astro-templates\}
...
Pour plus d'informations sur le rendu de votre contenu en HTML, voir [Rendre le contenu en HTML](#rendering-content-to-html) ci-dessous.
...
### Transmettre du contenu en tant que propriété \{#passing-content-as-props\}
...
### Rendre le contenu en HTML \{#rendering-content-to-html\}
...
### Construire pour une sortie statique (par défaut) \{#building-for-static-output-default\}
...
:::note
Si vos slugs personnalisés contiennent le caractère `/` pour produire des URLs avec plusieurs segments de chemin, vous devez utiliser un [paramètre rest (`[...slug]`)](/fr/guides/routing/#rest-parameters) dans le nom de fichier `.astro` pour cette page de routage dynamique.
:::

### Construction pour la sortie serveur (SSR) \{#building-for-server-output-ssr\}
...
## Migration basée sur le routage des fichiers \{#migrating-from-file-based-routing\}
...
## Activation de la génération de schémas JSON \{#enabling-json-schema-generation\}
...
## Activation de la mise en cache de la construction \{#enabling-build-caching\}
...
## Modifier le Frontmatter avec Remark \{#modifying-frontmatter-with-remark\}
...
Astro supporte les plugins remark ou rehype qui [modifient votre frontmatter directement](/fr/guides/markdown-content/#modifying-frontmatter-programmatically). Vous pouvez accéder à ce frontmatter modifié à l'intérieur d'un contenu en utilisant la propriété `remarkPluginFrontmatter` renvoyée par `render()` :
...
## Travailler avec des dates dans le frontmatter \{#working-with-dates-in-the-frontmatter\}
