...
:::note
Si vos slugs personnalisés contiennent le caractère `/` pour produire des URLs avec plusieurs segments de chemin, vous devez utiliser un [paramètre rest (`[...slug]`)](/fr/basics/routing/#paramètres-rest) dans le nom de fichier `.astro` pour cette page de routage dynamique.
:::
