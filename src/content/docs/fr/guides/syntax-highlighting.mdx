...
Une valeur `lang` peut inclure la prise en charge de la coloration syntaxique de certains langages supplémentaires par défaut (par exemple, `lang="svelte"` assurera également la coloration syntaxique pour `ts`).
