...
Vous pouvez cependant utiliser [le pattern Astro `<slot />`](/fr/basics/astro-components/#les-emplacements-slots) pour passer du contenu statique généré par des composants Astro en tant qu'enfants à vos composants framework **à l'intérieur d'un composant `.astro`**.
