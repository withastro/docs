...
De plus, vous pouvez utiliser les [emplacement nommés (slots)](/fr/basics/astro-components/#les-slots-nommés) pour grouper des enfants spécifiques ensemble.
...
Vous pouvez cependant utiliser [le modèle Astro `<slot />`](/fr/basics/astro-components/#les-slots) pour passer du contenu statique généré par des composants Astro en tant qu'enfants à vos composants framework **à l'intérieur d'un composant `.astro`**.
