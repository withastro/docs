...
:::caution
Les fichiers `.env` ne sont pas chargés dans les [fichiers de configuration](/fr/guides/configuring-astro/#environment-variables). 
:::
...
Lorsque l'on utilise SSR, les variables d'environnement peuvent être accédées à l'exécution en fonction de l'adaptateur SSR utilisé. Avec la plupart des adaptateurs, vous pouvez accéder aux variables d'environnement avec `process.env`, pour l'adaptateur Deno, vous devez utiliser `Deno.env.get()`. Cloudflare a sa propre façon de gérer les [variables d'environnement](/fr/guides/integrations-guide/cloudflare/#variables-denvironnement).
...
Bien que vous puissiez définir plus de variables env personnalisées dans les fichiers `.env.[mode]`, vous pouvez vouloir obtenir l'autocomplétion TypeScript pour les variables env définies par l'utilisateur qui sont préfixées par `PUBLIC_`.
