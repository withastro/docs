...
## Initialisation de Supabase dans Astro \{#initializing-supabase-in-astro\}
...
### Ajouter les informations d'identification de Supabase \{#adding-supabase-credentials\}
...
## Ajouter l'authentification avec Supabase \{#adding-authentication-with-supabase\}
...
- Un projet Astro [initialisé avec Supabase](#initializing-supabase-in-astro).
...
### Créer des points d'accès au serveur d'authentification \{#creating-auth-server-endpoints\}
...
Créez ces points de terminaison dans le répertoire `src/pages/api/auth` de votre projet. Si vous utilisez le mode de rendu `hybrid`, vous devez spécifier `export const prerender = false` en haut de chaque fichier pour restituer ces points de terminaison à la demande. Votre projet devrait maintenant inclure ces nouveaux fichiers :
...
### Création des pages d'authentification \{#creating-auth-pages\}
...
### Ajouter l'authentification OAuth \{#adding-oauth-authentication\}
