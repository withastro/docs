...
L'authentification Firebase dans Astro nécessite les trois [endpoints du serveur Astro](/fr/core-concepts/endpoints/) suivants :
...
La page d'exemple ci-dessous contient un formulaire qui enverra une requête « POST » au endpoint « /api/auth/signin » avec le token d'identification généré par l'application client Firebase.
...
La page d'exemple ci-dessous affiche le nom de l'utilisateur et un bouton pour se déconnecter. Cliquer sur le bouton enverra une requête « GET » au endpoint « /api/auth/signout ».

Le endpoint supprimera le cookie de session de l'utilisateur et redirigera l'utilisateur vers la page « /signin ».
...
- Un projet Astro initialisé avec Firebase comme décrit dans la section [Initialisation de Firebase dans Astro](#initializing-firebase-in-astro).
...
### Création des endpoints serveur

Créez deux nouveaux fichiers dans un nouveau répertoire `src/pages/api/friends/` : `index.ts` et `[id].ts`. Ceux-ci créeront deux endpoints de serveur pour interagir avec la base de données Firestore des manières suivantes :
...
:::note
Il s'agit d'une implémentation de base du endpoint « friends ». Vous pouvez ajouter plus de logique à ce endpoint en fonction de vos besoins.
:::
...
:::note
Il s'agit d'une implémentation de endpoint de terminaison `friends/:id`. Vous pouvez ajouter plus de logique à ce endpoint en fonction de vos besoins.
:::

Après avoir créé les endpoints du serveur pour Firestore, votre répertoire de projet doit maintenant inclure ces nouveaux fichiers :
...
Créez les pages qui utiliseront les endpoints Firestore:
...
L'exemple `src/pages/add.astro` ci-dessous inclut un formulaire qui enverra une requête `POST` au endpoint `/api/friends`. Ce endpoint créera un nouvel ami en utilisant les données du formulaire, puis redirigera l'utilisateur vers la page `/dashboard`.
...
`src/pages/edit/[id].astro` contiendra un formulaire pour modifier les données d'un ami et un bouton pour supprimer un ami. Lors de la soumission, cette page enverra une requête `POST` au endpoint `/api/friends/:id` pour mettre à jour les données d'un ami.

Si l'utilisateur clique sur le bouton Supprimer, cette page enverra une requête « DELETE » au endpoint « /api/friends/:id » pour supprimer un ami.
