...
## Script côté client \{#client-side-scripts\}
...
## Utilisation du `<script>` dans Astro. \{#using-script-in-astro\}
...
### Regroupement de script \{#script-processing\}
...
### Refus du traitement \{#opting-out-of-processing\}
...
<ReadMore>Voir notre page [directives reference](/fr/reference/directives-reference/#script--style-directives) pour plus d'informations sur les directives disponibles sur les balises `<script>`.</ReadMore>


### Inclure des fichiers JavaScript dans la page \{#include-javascript-files-on-your-page\}
...
### Importer des scripts locaux \{#import-local-scripts\}

**Quand l'utiliser :** Si votre script se trouve dans `src/`.

Astro construira, optimisera et ajoutera ces scripts à la page pour vous, en suivant ses [règles de regroupement des scripts](#script-processing)
...
#### Charger des scripts externes \{#load-external-scripts\}
...
## Modèles communs de script \{#common-script-patterns\}

### Gérer le `onclick` et d'autres évènements \{#handle-onclick-and-other-events\}
...
### Composants Web avec des éléments personnalisés \{#web-components-with-custom-elements\}
