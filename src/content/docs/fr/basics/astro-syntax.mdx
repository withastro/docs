...
- **La directive [define:vars](/fr/reference/directives-reference/#definevars) n'est pas supportée.** Si vous ne pouvez pas envelopper les éléments enfants avec un élément supplémentaire (par exemple `<div>`), alors vous pouvez ajouter manuellement un `style={``--myVar:${value}``}` à votre élément.
