...
## Mode du serveur `output` \{#server-output-modes\}
...
### Rendu à la demande \{#on-demand-rendered\}

Avec [un adaptateur SSR](/fr/guides/server-side-rendering/), les deux autres modes de sortie d'Astro peuvent être configurés pour afficher **à la demande une partie ou la totalité de vos pages, routes ou points d'extrémité d'API** :
...
<ReadMore>En savoir plus sur [les fonctionnalités disponibles avec le rendu à la demande et un adaptateur](/fr/guides/server-side-rendering/#on-demand-rendering-features), telles que les cookies, les objets Response et Request, le streaming HTML, et plus encore.</ReadMore>.

Envisagez d'activer le [mode "serveur" ou "hybride"](/fr/guides/server-side-rendering/#on-demand-rendering-features) dans votre projet Astro si vous avez besoin de ce qui suit :
...
- **Contenu changeant fréquemment** : Générez des pages individuelles sans avoir à reconstruire votre site de manière statique. Ceci est utile lorsque le contenu d'une page est souvent mis à jour, par exemple en affichant des données provenant d'une API appelée dynamiquement avec `fetch()`.

Les modes de sortie `server` et `hybrid` vous permettent d'inclure des [îles Astro](/fr/concepts/islands/) pour l'interactivité (ou même des applications entières rendues côté client !) dans le [Framework UI](/fr/guides/framework-components/) de votre choix. Avec les [middlewares](/fr/guides/middleware/) et l'[API de transition de vues](/fr/guides/view-transitions/) d'Astro pour les animations et la préservation de l'état à travers les itinéraires de navigation, même les applications hautement interactives sont possibles.

:::tip
Le rendu serveur à la demande dans Astro, en particulier lorsqu'il est combiné avec [view transitions](/fr/guides/view-transitions/), offre une véritable expérience d'application sans la surcharge JavaScript d'une application avec une page unique côté client.
:::
