...
Lorsque votre composant Astro nécessite une interactivité côté client, vous pouvez ajouter des [balises HTML standard `<script>`](/fr/guides/client-side-scripts/) ou des [composants UI Framework](/fr/guides/framework-components/#hydrating-interactive-components) en tant qu'« îlots client ».

Pour les composants qui doivent rendre un contenu personnalisé ou dynamique, vous pouvez différer leur rendu sur le serveur en ajoutant une [directive serveur](/fr/reference/directives-reference/#server-directives). Ces « îlots de serveur » rendront leur contenu lorsqu'il sera disponible, sans retarder le chargement complet de la page.
