...
Nano Stores _peut_ être importé, écrit et lu dans les composants côté serveur, **mais nous ne le recommandons pas !** Ceci est dû à quelques restrictions :
- L'écriture d'un store à partir d'un fichier `.astro` ou [composant non hydraté](/fr/core-concepts/framework-components/#hydratation-des-composants-interactifs) n'affectera *pas* la valeur reçue par les [composants côté client](/fr/reference/directives-reference/#directives-client).
...
Si vous voulez éviter les bibliothèques tierces, les [Svelte Stores](https://svelte.dev/tutorial/writable-stores) constituent à eux seuls un excellent outil de communication inter-îles. Néanmoins, vous pouvez préférerer Nano Stores si :
