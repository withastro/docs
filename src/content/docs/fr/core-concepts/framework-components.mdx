...
Vous pouvez cependant utiliser [le pattern Astro `<slot />`](/fr/core-concepts/astro-components/#slots) pour passer du contenu statique généré par des composants Astro en tant qu'enfants à vos composants framework **à l'intérieur d'un composant `.astro`**.
