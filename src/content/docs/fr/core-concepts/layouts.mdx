...
description: Une introduction aux composants Layouts (mises en page), un type de composant Astro qui est partagé entre les pages pour des modèles communs.
i18nReady: true
---

Les **Layouts** (ou "mises en page" en français) sont des [composants Astro](/fr/core-concepts/astro-components/) utilisés pour fournir une structure d'interface utilisateur réutilisable, comme un modèle de page.

Par convention, nous utilisons le terme "layout" pour les composants Astro qui fournissent des éléments d'interface utilisateur commune à toutes les pages, tels que les en-têtes, les barres de navigation et les pieds de page. Typiquement, un composant de mise en page Astro, fournit [aux pages Astro, Markdown ou MDX](/fr/core-concepts/astro-pages/) les éléments suivants :
- une **page shell** (`balises <html>`, `<head>` et `<body>`)
- un [**`<slot />`**](/fr/core-concepts/astro-components/#les-emplacements-slots) pour spécifier où le contenu d'une page doit être injecté.

Mais, les composants de mise en page n'ont rien de spécial ! Ils peuvent [accepter des propriétés](/fr/core-concepts/astro-components/#props-de-composant) et [importer et utiliser d'autres composants](/fr/core-concepts/astro-components/#structure-du-composant) comme n'importe quel autre composant Astro. Ils peuvent inclure [des composants de frameworks d'interface utilisateur](/fr/core-concepts/framework-components/) et [des scripts côté client](/fr/guides/client-side-scripts/). Ils ne sont même pas obligés de fournir une page shell complète, et peuvent plutôt être utilisés comme des modèles d'interface utilisateur partielle.

Les composants de mise en page sont généralement placés dans le dossier `src/layouts` de votre projet pour des raisons d'organisation, mais ce n'est pas une obligation; vous pouvez choisir de les placer n'importe où dans votre projet. Vous pouvez même placer les composants de mise en page à côté de vos pages en [préfixant les noms de mise en page par `_`](/fr/core-concepts/routing/#excluding-pages).
...
Astro fournit une propriété spéciale `layout` frontmatter pour spécifier le composant `.astro` devant être utiliser comme mise en page.
...
date: "09 Aug 2022"
---
Toutes les propriétés du frontmatter sont disponibles comme propriétés pour un composant de mise en page Astro.
...
<html>
  ...
  <body>
    ...
    <!-- 2. Le rendu HTML sera passé dans le slot par défaut. -->
    <slot />
...
Un exemple de billet de blog en Markdown peut transmettre l'objet `Astro.props` suivant, à sa mise en page :
...
*   Les informations sur les titres (c'est-à-dire les éléments `h1 -> h6`) sont disponibles via le tableau `headings`, plutôt que via la fonction `getHeadings()`.

*   `file` et `url` sont *aussi* disponibles en tant que propriétés `frontmatter` imbriquées (`frontmatter.url` et `frontmatter.file`).

*   Les valeurs définies en dehors de frontmatter (par exemple les déclarations `export` en MDX) ne sont pas disponibles. Pensez plutôt à [importer une mise en page](#importation-manuelle-de-mises-en-page-mdx).
:::
...
Ensuite, vos valeurs sont disponibles via `Astro.props` dans votre mise en page, et votre contenu MDX sera injecté dans la page où votre composant `<slot />` est écrit : 
...
Une mise en page Astro unique peut être écrite pour recevoir l'objet `frontmatter` des fichiers `.md` et `.mdx`, ainsi que toutes les propriétés nommées, passées depuis les fichiers `.astro`.

Dans l'exemple ci-dessous, la mise en page affichera le titre de la page soit à partir d'une propriété YAML `title` du frontmatter, soit à partir d'un composant Astro passant un attribut `title` :
...
Les composants de mise en page ne doivent pas nécessairement contenir une page entière de HTML. Vous pouvez décomposer vos mises en page en composants plus petits et combiner les composants de mise en page pour créer des modèles de page encore plus flexibles. Ce modèle est utile lorsque vous souhaitez partager du code entre plusieurs mises en page.

Par exemple, un composant de mise en page `BlogPostLayout.astro` peut styliser le titre, la date et l'auteur d'un article. Ensuite, un `BaseLayout.astro` à l'échelle du site pourrait gérer le reste de votre modèle de page, comme la navigation, les pieds de page, les balises méta SEO, les styles globaux et les polices. Vous pouvez également passer les propriétés reçues de votre billet à une autre mise en page, comme n'importe quel autre composant imbriqué.
