...
使用 [Vite 的客户端 - 服务器通信方法](https://vite.dev/guide/api-plugin.html#client-server-communication)，Dev Toolbar Apps 和服务器可以相互通信。为了便于发送和接收自定义消息，提供了辅助方法，可在你的工具栏应用（客户端）和你的集成（服务器端）中使用。
