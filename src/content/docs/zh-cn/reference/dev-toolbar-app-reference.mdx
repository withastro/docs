...
## 开发者工具栏应用集成设置 \{#toolbar-app-integration-setup\}
...
用于在工具栏中显示应用的图标。可以是 [图标列表](#icons) 中的图标，或包含图标 SVG 标记的字符串。
...
## 开发者工具栏应用的结构 \{#structure-of-a-dev-toolbar-app\}
...
一个标准的 [`EventTarget`](https://developer.mozilla.org/zh-CN/docs/Web/API/EventTarget) ，附带一些额外的 [客户端事件](#client-side-events)，可用于从开发者工具栏发送和接收事件。
...
一个用于[与服务器通信](#client-server-communication)的对象。
...
使用 [Vite 的客户端 - 服务器通信方法](https://vitejs.dev/guide/api-plugin.html#client-server-communication)，Dev Toolbar Apps 和服务器可以相互通信。为了便于发送和接收自定义消息，提供了辅助方法，可在你的工具栏应用（客户端）和你的集成（服务器端）中使用。
...
在一个集成中，例如[添加你的工具栏应用的集成](#toolbar-app-integration-setup)，使用 [`astro:server:setup` 钩子](/zh-cn/reference/integrations-reference/#astroserversetup) 来访问 `toolbar` 对象，以便向你的应用发送和接收消息。
...
显示图标。可以使用 `icon` 属性指定[图标列表](#icons)中的图标，或者将图标的 SVG 标记作为插槽传递。
