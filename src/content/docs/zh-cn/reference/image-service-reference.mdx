...
Astro [assets]（/en/guides/assets/） 旨在使任何图像优化服务都能轻松地在 Astro 上构建服务。
...
- **本地服务**在构建静态站点时直接处理图像转换，或者在运行时同时处理开发模式和 SSR 中的图像转换。这些通常是围绕 Sharp、ImageMagick 或 Squoosh 等库的包装。在开发模式和 SSR 中，本地服务使用 API 端点进行转换。
...
在开发模式和SSR模式下， Astro 并不提前知道哪些图像需要优化。 Astro 使用 GET 端点（默认情况下为 `/_image` ）在运行时处理图像。 `<Image />` 和 `getImage()` 将它们的选项传递给 `getURL()`，后者将返回端点 URL。 然后，端点调用 `parseURL()` 并将结果属性传递给 `transform()`。
