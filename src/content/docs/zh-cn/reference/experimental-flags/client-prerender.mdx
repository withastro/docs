...
在启用该功能之前，你可能需要考虑一下 [在客户端上进行预渲染的风险](https://developer.mozilla.org/zh-CN/docs/Web/API/Speculation_Rules_API#不安全的预获取)。
...
继续在网站上的任意 `<a />` 链接上使用 `data-astro-prefetch` 属性来选用预获取。不是将 `<link>` 标签附加到页面的 head 部分或使用 JavaScript 来获取页面，而是使用 `<script>` 标签附加相应的推测规则。
