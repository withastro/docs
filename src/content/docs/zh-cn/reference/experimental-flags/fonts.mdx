...
如果你不想局限于使用任一的 [内置提供商](#可用的远程字体提供商)（例如，你想使用[第三方 unifont 提供商](#支持第三方-unifont-provider)，或为[私有注册表构建某些内容](#支持私有注册表)），那么你便可以创建自己的字体提供商。

实现自定义字体提供商的推荐方式是导出一个函数，该函数接收[配置](#config)作为参数，并返回一个 [`FontProvider` 对象](#字体提供商对象)。

### 字体提供商对象

实验性字体 API 允许你以统一的方式访问字体。每个字体族需通过 Astro 字体提供商来获取对应的字型（font face）。
