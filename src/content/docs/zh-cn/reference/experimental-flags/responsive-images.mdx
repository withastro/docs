...
通过设置 [`image.experimentalLayout`](/en/reference/configuration-reference/#imageexperimentallayout) 指定默认布局 (`responsive`， `fixed`， 或 `full-width`) 以在整个项目中启用响应式图像。

如果未配置此值，仍然可以为任何 `<Image />` 或 `<Picture />` 组件传递 `layout` 属性以创建响应式图像。但是，Markdown 图像将不会是响应式的。

此外，可以配置 [`image.experimentalObjectFit`](/en/reference/configuration-reference/#imageexperimentalobjectfit) 和 [`image.experimentalObjectPosition`](/en/reference/configuration-reference/#imageexperimentalobjectposition) ，这将默认应用于所有处理后的图像。
