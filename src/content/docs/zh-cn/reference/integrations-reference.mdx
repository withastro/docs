...
**类型**：[`IntegrationResolvedRoute[]`](#integrationresolvedroute-type-reference)
...
**类型：**[`IntegrationRouteData[]`](#integrationroutedata-type-reference)
...
**原因：** 在构建或请求时为路由设置选项，例如启用[按需服务器渲染](/zh-cn/guides/on-demand-rendering/#enabling-on-demand-rendering)。
...
`prerender` 属性用于配置[按需服务器渲染](/zh-cn/guides/on-demand-rendering/#enabling-on-demand-rendering)的路由。如果路由文件包含一个显式的 `export const prerender` 值，该值将被用作默认值，而不是 `undefined`。
