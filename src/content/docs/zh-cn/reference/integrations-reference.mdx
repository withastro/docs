...
**原因：** 在构建或请求时为路由设置选项，例如[启用按需渲染](/zh-cn/guides/on-demand-rendering/#启用按需渲染)。
...
`prerender` 属性用于配置[启用按需渲染](/zh-cn/guides/on-demand-rendering/#启用按需渲染)的路由。如果路由文件包含一个显式的 `export const prerender` 值，该值将被用作默认值，而不是 `undefined`。
