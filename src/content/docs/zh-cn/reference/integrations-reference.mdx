...
#### `isRestart` 选项 \{#isrestart-option\}

**类型**：`boolean`

开发服务器启动时为`false`，触发重载时为`true`。用于检测此函数何时被多次调用。

#### `updateConfig` 选项 \{#updateconfig-option\}
...
#### `addRenderer` 选项 \{#addrenderer-option\}
...
#### `addWatchFile` 选项 \{#addwatchfile-option\}
...
#### `addClientDirective` 选项 \{#addclientdirective-option\}
...
添加一个 [自定义客户端指令](/zh-cn/reference/directives-reference/#custom-client-directives)，用于在 `.astro` 文件中使用。
...
#### `addDevToolbarApp` 选项 \{#adddevtoolbarapp-option\}
...
#### `addMiddleware` 选项 \{#addmiddleware-option\}
...
#### `injectRoute` 选项 \{#injectroute-option\}

**类型**：`({ pattern: string, entrypoint: string }) => void;`

用于向 Astro 项目注入路由的回调函数。注入的路由可以是 [`.astro`页面](/zh-cn/basics/astro-pages/) 或 [`.js`和`.ts`路由处理程序](/zh-cn/guides/endpoints/#static-file-endpoints)。
...
#### `injectScript` 选项 \{#injectscript-option\}
...
#### `routes` 选项 \{#routes-option\}

**类型：**[`RouteData[]`](#routedata-type-reference)
...
##### `RouteData` 类型参考 \{#routedata-type-reference\}
...
**原因：** 在构建或请求时为路由设置选项，例如启用[按需服务器渲染](/zh-cn/guides/server-side-rendering/#opting-in-to-pre-rendering-in-server-mode)。

```js
'astro:route:setup'?: (options: { route: RouteOptions }) => void | Promise<void>;
```

#### `route` 选项 \{#route-option\}
...
`prerender` 属性用于配置[按需服务器渲染](/zh-cn/guides/server-side-rendering/#opting-in-to-pre-rendering-in-server-mode)的路由。如果路由文件包含一个显式的 `export const prerender` 值，该值将被用作默认值，而不是 `undefined`。
...
如果在运行所有钩子后的最终值是 `undefined`，路由将根据 [`output` 选项](/zh-cn/reference/configuration-reference/#output) 回退到预渲染的默认值：`hybrid` 模式下预渲染，`server` 模式下按需渲染。
...
## 使用 `astro add` 安装 \{#allow-installation-with-astro-add\}
...
## 合并预置集成 \{#combine-integrations-into-presets\}
