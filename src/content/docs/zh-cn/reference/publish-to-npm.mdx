...
:::tip
关键词也会在 [集成库](https://astro.build/integrations/) 中使用！[查看关于所有我们使用的关键词](#integrations-library)
:::
...
#### 示例：使用命名导入 \{#example-using-named-imports\}
...
#### 示例：使用命名空间导入 \{#example-using-namespace-imports\}
...
#### 示例：使用独立导入 \{#example-using-individual-imports\}
...
## 发布组件 \{#publishing-your-component\}
...
### `package.json` 数据 \{#packagejson-data\}
