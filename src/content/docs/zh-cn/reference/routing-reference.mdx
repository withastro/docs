...
每个位于特殊 `src/pages/` 目录中的 [支持的页面文件](/zh-cn/basics/astro-pages/#支持的页面文件) 都会创建一条路由。当文件名包含 [paramas](#paramas) 时，路由可以动态创建多个页面，否则创建单个页面。
...
`getStaticPaths()` 函数必须返回一个对象数组，以确定 Astro 将预渲染哪些 URL 路径。每个对象必须包含一个 `params` 对象，以指定路由路径。该对象可以选择包含一个 `props` 对象，其中包含每个页面模板要 [传入到的数据](#data-passing-with-props)。
...
`getStaticPaths()` 也可以在静态文件端点中用于 [动态路由](/en/guides/endpoints/#params-和动态路由)。
...
获取上一页链接（如果在首页，将是 `undefined`）。如果为 [`base`](/zh-cn/reference/configuration-reference/#base) 设置了值，则 URL 将以该值开头。
...
获取下一页链接（如果在首页，将是 `undefined`）。如果为 [`base`](/zh-cn/reference/configuration-reference/#base) 设置了值，则 URL 将以该值开头。
...
获取第一页链接（如果在首页，将是 `undefined`）。如果为 [`base`](/zh-cn/reference/configuration-reference/#base) 设置了值，则 URL 将以该值开头。
...
获取最后一页链接（如果在首页，将是 `undefined`）。如果为 [`base`](/zh-cn/reference/configuration-reference/#base) 设置了值，则 URL 将以该值开头。
...
获取最后一页链接（如果在首页，将是 `undefined`）。如果为 [`base`](/zh-cn/reference/configuration-reference/#base) 设置了值，那么需要将 base 路径拼接到链接前面。
