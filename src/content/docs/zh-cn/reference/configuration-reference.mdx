...
默认情况下，`import.meta.env.BASE_URL` 的值包含一个尾部斜杠。如果你将 [`trailingSlash`](/zh-cn/reference/configuration-reference/#trailingslash) 选项设置为 `'never'`，则在静态资源的导入和 URL 中需要手动添加它。
...
使用构建适配器将其部署到你最喜爱的服务器、无服务器或边缘主机。导入我们的第一方适配器 [Netlify](/zh-cn/guides/deploy/netlify/#ssredge-下的适配)、[Vercel](/zh-cn/guides/deploy/vercel/#ssr-适配器)，以及更多的适配器来使用Astro SSR。
...
控制样式是否以单独的 CSS 文件发送到浏览器还是内联到 `<style>` 标签中。可以从以下选项中进行选择：

-  `'always'` - 所有样式都内联到 `<style>` 标签中。
-  `'auto'` - 只有小于 `ViteConfig.build.assetsInlineLimit`（默认为 4kb）的样式表才会被内联。否则，样式将以外部样式表的形式发送。
-  `'never'` - 所有样式都以外部样式表的形式发送。
...
当 `split` 设置为 `true` 时，Astro 将为每个页面生成一个文件。每个生成的文件将只渲染一个页面。
...
**类型：**`Object`<br />
**默认值：**`{entrypoint: 'astro/assets/services/squoosh', config?: {}}`<br />
<Since v="2.1.0" />
</p>
...
启用对 `<ViewTransitions />` 组件的实验性支持。启用此功能后，你可以使用该组件在每个页面上选择性地加入 [视图过渡动画](/zh-cn/guides/view-transitions/)，并通过 `transition:animate` 指令启用动画效果。
