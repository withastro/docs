...
启用域名支持，用于[实验性的 `domains` 路由策略](/zh-cn/guides/internationalization/#domains-实验性功能)，它允许你配置一个或多个支持的语言的 URL 模式，以使用自定义域名（或子域名）。
...
查看 [国际化指南](/zh-cn/guides/internationalization/#domains-实验性功能) 以获取更多细节，包括这个实验性功能的限制。
