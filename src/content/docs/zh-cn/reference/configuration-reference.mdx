...
在兼容的编辑器中，启用此功能将为你的内容集合条目提供 Intellisense 智能提示功能（例如代码完成、快速提示）。
...
`glob()` loader 从文件系统上的任何地方的 Markdown、MDX、Markdoc 或 JSON 文件目录中创建条目。它接受一个匹配条目文件的 `pattern`，以及你的文件所在的 `base` 文件路径。当每个条目有一个文件时使用。
...
const blog = defineCollection({
  // 默认情况下，ID 是从相对于 `base` 的文件路径生成的 slug
...
3. **将引用从 `slug` 改成 `id`**。Content layer 集合不需要 `slug` 字段。相反，所有集合都使用 `id` 字段。
    ```astro ins={7} del={6}
    // src/pages/index.astro
    ---
