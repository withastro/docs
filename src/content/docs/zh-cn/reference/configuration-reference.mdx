...
`import.meta.env.BASE_URL` 的值会遵从你的 `trailingSlash` 配置，如果你明确地包含一个尾部斜杠，或者设置了 `trailingSlash: "always"`，它将包含一个尾部斜杠。如果设置了 `trailingSlash: "never"`，`BASE_URL` 将不包含尾部斜杠，即使 `base` 中包含。
