...
由于你的组件不会与页面的其他部分一起渲染，因此可能需要添加通用内容（例如：加载信息）以临时替代它。这些后备内容将在页面上群岛加载之前的首次渲染时显示。

使用 `slot="fallback:` 属性将占位符内容添加为 Astro 组件的子组件。当群岛内容可用时，后备内容就会被替换。

下面的例子展示了将一个通用头像作为后备内容后，利用视图过渡动画转换为个性化头像的情况：
...
如需了解完整概述，或欲对该实验性 API 提供反馈建议，请参阅 [服务器群岛 RFC](https://github.com/withastro/roadmap/pull/963)。
