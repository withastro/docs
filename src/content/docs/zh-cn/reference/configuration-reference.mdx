...
Actions 可以帮助你编写可从任何地方调用的类型安全的后端函数。启用服务端渲染 [使用 `output` 属性](/zh-cn/basics/rendering-modes/#按需渲染) 并在 `experimental` 对象中添加 `actions` 标志：
...
在 `src/actions/index.ts` 中声明所有的操作。这个文件是全局操作处理器。

使用 `astro:actions` 模块中的 `defineAction()` 工具定义一个操作。这些操作接受 `handler` 属性来定义你的服务端请求处理器。如果你的操作接受参数，使用 `input` 属性通过 Zod 验证参数。

此示例定义了两个操作：`like` 和 `comment`。`like` 操作接受一个包含 `postId` 字符串的 JSON 对象，而 `comment` 操作接受包含 `postId`、`author` 和 `body` 字符串的 [FormData](https://developer.mozilla.org/zh-CN/docs/Web/API/XMLHttpRequest_API/Using_FormData_Objects)。每个 `handler` 更新你的数据库并返回一个类型安全的响应。
...
然后，使用来自 `astro:actions` 的 `actions` 对象从你的客户端组件调用一个操作。当使用 JSON 时，你可以传递一个类型安全的对象，或者当在你的操作定义中使用 `accept: 'form'` 时，传递一个 [FormData](https://developer.mozilla.org/zh-CN/docs/Web/API/XMLHttpRequest_API/Using_FormData_Objects) 对象：
...
由 `astro:i18n` 辅助函数 [`getAbsoluteLocaleUrl()`](/zh-cn/reference/api-reference/#getabsolutelocaleurl) 和 [`getAbsoluteLocaleUrlList()`](/zh-cn/reference/api-reference/#getabsolutelocaleurllist) 构建的页面路由和返回的 URL 将采用在 `i18n.domains` 中设置的选项。
