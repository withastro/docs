...
- `'ignore'`- 匹配链接，不管是否存在末尾的 `/`。
- `'always'` - 只匹配包含末尾斜杠的链接（例如：`/foo/`）。
- `'never'` - 不匹配包含末尾斜杠的链接（例如：`/foo`）。

如果你的生产主机对末尾斜杠的工作或不工作有严格的处理方式，请使用该配置选项。

如果你希望自己更严格一些，那么你也可以设置这个选项，这样在开发过程中，无论是否有末尾斜杠的 URL 都不会工作。
