...
使用构建适配器将其部署到你最喜爱的服务器、无服务器或边缘主机。导入我们的第一方适配器 [Netlify](/zh-cn/guides/deploy/netlify/#adapter-for-ssr)、[Vercel](/zh-cn/guides/deploy/vercel/#adapter-for-ssr)，以及更多的适配器来使用 Astro SSR。
...
#### 对 Astro.url 的影响 \{#effect-on-astrourl\}
...
Shiki 配置选项。使用方法见 [markdown 配置文档](/zh-cn/guides/markdown-content/#shiki-configuration)。
...
当 [`i18n.fallback`](#i18nfallback) 配置为避免给缺失的页面路由显示 404 页面时，此选项用于控制是将用户[重定向](/zh-cn/guides/routing/#redirects)到回退页面，还是将回退页面的内容[重写](/zh-cn/guides/routing/#rewrites)到原始请求的 URL 上。
...
将重定向和注入路由与基于文件的项目路由进行同等优先级处理，所有路由都遵循相同的[路由优先级顺序规则](/zh-cn/guides/routing/#route-priority-order)。
...
### experimental.rewriting \{#experimentalenv\}
...
### experimental.env \{#experimentalenvschema\}
...
环境变量有三种类型，由 `context`（client 或 server）和 `access`（secret 或 public）的组合设置在你的 [`env.schema`](#experimentalenvvalidatesecrets) 中确定：
...
#### experimental.env.schema \{#experimentalenvvalidatesecrets\}
...
#### experimental.env.validateSecrets \{#experimentalserverislands\}
...
### experimental.serverIslands \{#server-island-fallback-content\}
...
要启用该功能，请使用适配器配置为 [在服务器上按需渲染的 `output` 模式](/zh-cn/basics/rendering-modes/#on-demand-rendered)，并在 `experimental` 对象中添加 `serverIslands` 标志：
...
#### 服务器群岛的回退内容 \{#experimentalcontentintellisense\}
...
### experimental.contentIntellisense \{#experimentalcontentlayer\}
...
### experimental.contentLayer \{#fetching-data-with-a-loader\}
...
只需要一点小改动，你就可以将现有的内容集合 [迁移到 Content Layer API](#learn-more)。但是，不需要一次性更新所有集合来添加一个由 Content Layer API 提供支持的新集合。你可以在 `src/content/config.ts` 中同时定义使用现有的和新 API 的集合。
...
#### 使用 `loader` 请求数据 \{#querying-and-rendering-with-the-content-layer-api\}

Content Layer API 允许你从 `src/content/` 文件夹之外（无论是在你的项目中本地存储还是远程存储）获取内容，并使用 `loader` 属性来检索数据。

`loader` 在集合的 schema 中定义，并返回一个条目数组。Astro 提供了两个内置的 loader 函数（`glob()` 和 `file()`）用于获取你的本地内容，以及让你[构建自己的 loader 并获取远程数据](#migrating-an-existing-content-collection-to-use-the-content-layer-api) 的 API。
...
:::note
Loader 不会自动[排除以 `_` 开头的文件](/zh-cn/guides/routing/#excluding-pages)。在你的 loader 中使用正则表达式，例如 `pattern: '**\/[^_]*.md'` 来忽略这些文件。
:::

#### 使用 Content Layer API 查询和渲染 \{#creating-a-loader\}

可以使用 [与内容集合相同的方式查询集合](/zh-cn/guides/content-collections/#querying-collections)：
...
#### 创建 loader \{#migrating-an-existing-content-collection-to-use-the-content-layer-api\}
...
#### 迁移现有内容集合以使用 Content Layer API \{#learn-more\}
