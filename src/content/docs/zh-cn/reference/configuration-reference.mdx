...
你需要将 `./dist/_astro/` 中的文件上传远程地址对应的 `/_astro/`目录下。

如果要重命名`_astro`路径的话，在`build.assets`中配置新目录。

所有静态资源使用同一个域名（例如：`https://cdn.example.com/_astro/...`），将`assetsPrefix`设置成域名地址（会忽略`base`配置）。
...
你可以将`assetsPrefix`设置成对象，为不同静态资源类型指定不同的域名。但是`fallback`属性是必填的，用于其他未指定的静态资源类型。
