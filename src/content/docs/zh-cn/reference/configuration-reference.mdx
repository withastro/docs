...
预渲染页面上的末尾斜杠是由托管平台处理的，他们可能会忽视你选择的配置。有关更多信息，请参阅托管平台的文档。此时，你不能使用 Astro [重定向](/zh-cn/reference/configuration-reference/#redirects) 来处理这种情况。
...
为了避免开发环境中末尾斜杠行为的不一致性，你可以根据构建格式将 [`trailingSlash` 选项](/zh-cn/reference/configuration-reference/#trailingslash) 限制为 `'always'` 或 `'never'`：
...
使用 [`prefetch.defaultStrategy`](/zh-cn/reference/configuration-reference/#prefetchdefaultstrategy) 和 [`prefetch.prefetchAll`](/zh-cn/reference/configuration-reference/#prefetchprefetchall) 选项进一步自定义默认的预获取行为。
...
该选项需要一个由域名字符串组成的数组，但不允许使用通配符。或者你也可以使用 [`image.remotePatterns`](/zh-cn/reference/configuration-reference/#imageremotepatterns) 来定义一组允许的源 URL 模式。
...
当 [`i18n.fallback`](/zh-cn/reference/configuration-reference/#i18nfallback) 配置为避免给缺失的页面路由显示 404 页面时，此选项用于控制是将用户[重定向](/zh-cn/guides/routing/#重定向)到回退页面，还是将回退页面的内容[重写](/zh-cn/guides/routing/#重写)到原始请求的 URL 上。
