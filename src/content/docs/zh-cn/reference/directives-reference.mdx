...
**该值不会被 Astro 自动转义**！请���保你信任该值，或者在传递给模板前对其进行手动转义。忘记这样做可能会使你受到[跨网站脚本（XSS）攻击](https://owasp.org/www-community/attacks/xss/)。
...
- 不会被打包到外部文件中。这意味着控制外部文���加载的[属性（如 `defer`）](https://javascript.info/script-async-defer)将不起作用。
