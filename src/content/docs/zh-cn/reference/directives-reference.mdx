...
客户端指令只能用于直接引入到 `.astro` 组件中的 UI 框架组件。 使用[动态标签](/zh-cn/core-concepts/astro-syntax/#dynamic-tags)和[通过 `components` prop 传递的自定义组件](/zh-cn/guides/markdown-content/#custom-components-with-imported-mdx)时，不支持 Hydration 指令。
...
`define:vars={...}` 可以将服务器端的变量从组件 frontmatter 传递给客户端的 `<script>` 或 `<style>` 标签。支持任何 **JSON 可序列化的** front matter 变量，包括通过 `Astro.props` 传递给组件的 `props`。值可以使用 [`JSON.stringify()`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) 进行序列化。
...
:::caution
在 `<script>` 或 `<style>` 标签上使用 `define:vars` 相当于使用 [`is:inline` 指令](#isinline)，这意味着脚本或样式不会被打包，将直接内联到 HTML 中。
