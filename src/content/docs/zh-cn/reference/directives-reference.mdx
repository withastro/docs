...
客户端指令只能用于直接引入到 `.astro` 组件中的 UI 框架组件。 使用[动态标签](/zh-cn/core-concepts/astro-components/#动态标签)和[通过 `components` prop 传递的自定义组件](/zh-cn/guides/markdown-content/#custom-components-with-imported-mdx)时，不支持 Hydration 指令。
