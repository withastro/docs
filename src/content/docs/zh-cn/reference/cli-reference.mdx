...
当你按照[手动安装 Astro](/zh-cn/install-and-setup/#manual-setup) 的说明进行操作时，你会被告知自己添加这些脚本。你还可以手动为任何你经常使用的命令添加更多脚本到这个列表中。
...
为所有 Astro 模块生成 TypeScript 类型。这会为类型推断设置一个[`src/env.d.ts`文件](/zh-cn/guides/typescript/#setup)，并为依赖于生成类型的特性定义模块：

- `astro:content` 模块用于 [内容集合 API](/zh-cn/guides/content-collections/)。
- `astro:db` 模块用于 [Astro DB](/zh-cn/guides/astro-db/)。
- `astro:env` 模块用于 [实验性的 Astro Env](/zh-cn/reference/configuration-reference/#experimentalenvschema)。
- `astro:actions` 模块用于 [Astro Actions](/zh-cn/guides/actions/)。

## `astro add`

将集成添加到你的配置中。在[集成指南](/zh-cn/guides/integrations-guide/#automatic-integration-setup)中阅读更多信息。
...
连接到你的 Studio 托管数据库。这对于运行用于数据库管理的 [Astro DB 命令](/zh-cn/guides/integrations-guide/db/#astro-db-cli-reference)是必需的。要连接一个数据库，你会被提示选择一个已存在的数据库或创建一个新的。
