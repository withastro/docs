...
Astro 设计得易于部署到任何云提供商的 SSR（服务端渲染）服务，该功能由**适配器**[集成](/zh-cn/reference/integrations-reference/)提供。请参阅 [SSR 指南](/zh-cn/guides/server-side-rendering/#添加一个适配器) 了解如何使用现有的适配器。
...
该模块用于渲染通过 `astro build` 预构建的页面。Astro 使用标准的[Request](https://developer.mozilla.org/zh-CN/docs/Web/API/Request) 和 [Response](https://developer.mozilla.org/zh-CN/docs/Web/API/Response) 对象。有着不同的请求/响应 API 的主机应该在其适配器中转换为这些类型。
...
这个方法适合渲染匹配请求的 Astro 页面，并在 Promise 中返回 [Response](https://developer.mozilla.org/zh-CN/docs/Web/API/Response) 对象。这也适用于不渲染页面的 API 路由。
