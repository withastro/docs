...
如果你已经知道要渲染的路由，请为 [`routeData`](/zh-cn/reference/integrations-reference/#routedata-type-reference) 提供一个值。这样做将绕过内部调用 [`app.match`](#appmatchrequest) 来确定要渲染的路由。
...
## 使用 `astro add` 安装适配器 \{#allow-installation-via-astro-add\}
...
#### 无服务器（Serverless）环境 \{#serverless-environments\}
