...
当设置 `functionPerRoute: false` 时，只有一个单一的处理程序（handler）负责渲染所有路由。当此处理程序（handler）首次触发时，你将受到冷启动的影响。然后，所有其他路由都应该没有延迟。但是，你将失去 `functionPerRoute: true` 提供的代码拆分（code splitting）的好处。
