...
`inferSize` 能够获取来自未经授权域的 [远程图片的尺寸](/zh-cn/guides/images/#授权远程图片)，但图片本身将保持未处理状态。
...
一个图像格式的数组，用于 `<source>` 标签。条目将按照列出的顺序添加为 `<source>` 元素，并且此顺序决定显示哪种格式。为了获得最佳性能，请首先列出最现代的格式（例如 `webp` 或 `avif`）。默认情况下，它被设置为 `['webp']`。
