...
`Astro.getActionResult()` 是一个返回 [Action](/zh-cn/guides/actions/) 提交结果的函数。这个函数接受一个 Action 函数作为参数（例如 `actions.logout`），当接收到提交时返回一个 `data` 或 `error` 对象。否则，它将返回 `undefined`。
...
`Astro.callAction()` 是一个直接从你的 Astro 组件中调用 Action 处理程序的函数。这个函数接受一个 Action 函数作为第一个参数（例如 `actions.logout`），以及该 Action 接收的任何输入作为第二个参数。它返回 Action 的结果作为 Promise。
...
`context.getActionResult()` 是一个函数，用于从你的 Astro 组件中返回 [Action](/zh-cn/guides/actions/) 提交结果。这个函数接受一个 Action 函数作为参数（例如 `actions.logout`），当接收到提交时返回一个 `data` 或 `error` 对象。否则，它将返回 `undefined`。
...
`context.callAction()` 是一个直接从你的 Astro 组件中调用 Action 处理程序的函数。这个函数接受一个 Action 函数作为第一个参数（例如 `actions.logout`），以及该 Action 接收的任何输入作为第二个参数。它返回 Action 的结果作为 Promise。
