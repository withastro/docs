...
获取下一页链接（如果没有更多的页面，将是`undefined`）。
...
此模块提供了一些函数，帮助你使用项目配置的地区设置创建 URL。
...
使用此函数来检索一个地区的相对路径。如果该地区不存在，Astro 会抛出一个错误。
...
当 [`site`] 配置了值时，使用这个函数来检索一个地区的绝对路径。如果 [`site`] 没有被配置，该函数将返回一个相对 URL。如果地区不存在，Astro 会抛出一个错误。
...
像使用 [`getRelativeLocaleUrl`](#getrelativelocaleurl) 那样使用此函数，返回所有地区的相对路径列表。

### `getAbsoluteLocaleUrlList()` 

`getAbsoluteLocaleUrlList(path?: string, options?: GetLocaleOptions): string[]`

像使用 [`getAbsoluteLocaleUrl`](/zh-cn/guides/internationalization/#custom-locale-paths) 那样使用此函数，返回所有地区的绝对路径列表。

### `getPathByLocale()` 

`getPathByLocale(locale: string): string`

当配置了[自定义地区路径](/zh-cn/guides/internationalization/#custom-locale-paths)时，此函数返回与一个或多个 `codes` 关联的 `path`。
...
这是一个函数，它返回一个 `Response`，将重定向到配置的 `defaultLocale`。它接受一个可选的有效重定向状态码。
...
这是一个函数，允许你在自己的中间件中使用你的 [`i18n.fallback` 配置](/zh-cn/reference/configuration-reference/#i18nfallback)。
...
在你的路由中间件中使用此函数来返回一个 404 错误，当以下情况发生时：
- 当前路径不是根路径。例如：`/` 或 `/<base>`
- URL 中不包含地区代码
...
这是一个函数，允许你以编程方式创建 Astro i18n 中间件。
...
检查当前 URL 是否包含已配置的地区代码。在内部，此函数将使用 `APIContext#url.pathname`。
