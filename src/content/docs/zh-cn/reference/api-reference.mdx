...
[Shiki 转换器](https://shiki.tmrs.site/packages/transformers#shikijs-transformers) 能够被额外地被应用到代码上，即通过将它们作为数组传递给 `transformers` 属性。

注意，`transformers` 只能用于类，同时你必须提供自己的 CSS 规则来定位代码块的元素。
