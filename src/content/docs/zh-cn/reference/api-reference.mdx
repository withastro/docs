...
`handler()` 函数接受用户输入作为其第一个参数。如果设置了 [`input`](#input-验证器) 验证器，则在将用户输入传递给 `handler()` 之前，将对其进行验证。第二个参数是一个 `context` 对象，包含大多数 Astro 的 [标准端点上下文](#端点上下文)，不包括 `getActionResult()`, `callAction()`, 和 `redirect()`。
