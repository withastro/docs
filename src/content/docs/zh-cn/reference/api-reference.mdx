...
将 Cookie 标记为已删除。将 Cookie 标记为已删除。一旦 cookie 被删除，`Astro.cookies.has()` 将返回 `false`，`Astro.cookies.get()` 将返回一个值为 `undefined` 的 [`AstroCookie`](#astrocookie)。删除 cookie 时可用的选项有：`domain`、`path`、`httpOnly`、`sameSite` 和 `secure`。
