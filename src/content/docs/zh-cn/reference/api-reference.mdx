...
`Astro` 全局在所有 `.astro` 文件中均可用。它有以下功能：
...
`.glob()` 不能接受插值的变量或字符串，因为它们不是静态可分析的。(参见[故障排查](/zh-cn/guides/troubleshooting/#支持的值)以了解解决方法)。 这是因为 `Astro.glob()` 是 Vite 的 [`import.meta.glob()`](https://cn.vitejs.dev/guide/features.html#glob-import) 的包装器。
...
其他文件可能有各种不同的接口，但如果你确切知道无法识别的文件类型包含什么，那么 `Astro.glob()` 可以接受 TypeScript 泛型。
...
`Astro.response` 是标准的 [ResponseInit](https://developer.mozilla.org/zh-CN/docs/Web/API/Response/Response#init) 对象。它用于设置页面响应中的 `status`、`statusText` 和 `headers`
...
在[服务端渲染](/zh-cn/guides/server-side-rendering/)模式下，`Astro.redirect()` 允许你重定向到另一个页面。页面（而不是子组件）必须 `return` `Astro.redirect()` 的结果，以便重定向发生。
...
你以使用 `Astro.slots.render()` 将插槽的内容异步渲染为 HTML 字符串。
...
内容集合提供了 API 来配置和查询 `src/content/` 中的 Markdown 或 MDX 文档。有关功能和用法示例，请参阅[内容集合指南](/zh-cn/guides/content-collections/)。
...
[参阅 `内容集合` 指南](/zh-cn/guides/content-collections/#defining-a-collection-schema) 以获取示例用法。
...
[参阅 `内容集合` 指南](/zh-cn/guides/content-collections/#querying-collections) 以获取示例用法。
...
一个 URL-ready 的 slug。默认情况下，它是 `id`，但可以通过在文件的 frontmatter 中设置 [the `slug` property](/zh-cn/guides/content-collections/#defining-custom-slugs) 来覆盖。  

#### `data`

**类型：**`CollectionSchema<TCollectionName>`

一个从集合模式推断出的 frontmatter 属性对象（[参阅 `defineCollection()`](#definecollection)）。如果没有配置模式，则默认为 `any`。
...
[参阅 `内容集合` 指南](/zh-cn/guides/content-collections/#rendering-content-to-html) 以获取示例用法。
