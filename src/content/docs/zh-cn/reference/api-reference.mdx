...
|名&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;称  | 类&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;型                                                         | 描&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;述                                                         |
| :-------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| `get`      | `(key: string, options?: CookieGetOptions) => AstroCookie`                               | 获取 [`AstroCookie`](#astrocookie) 对象形式的 cookie，该对象包含`value`和用于将 cookie 转换为非字符串类型的工具方法。 |
| `has`     | `(key: string) => boolean`                                   | cookie 是否存在。如果这个 cookie 已经通过`Astro.cookies.set()`设置，这将返回 true，否则它将检查 `Astro.request` 中的 cookies。 |
| `set`     | `(key: string, value: string \| number \| boolean \| object, options?: CookieSetOptions) => void` | 将 cookie `key` 设置为给定的值。这将试图把 cookie 的值转换为一个字符串。选项提供了设置 [cookie 功能](https://www.npmjs.com/package/cookie#options-1)的方法，例如 `maxAge` 或 `httpOnly。` |
...
| 名称      | 类型                      | 描述                                                                                                    |
| :-------- | :-------------------------- | :------------------------------------------------------------------------------------------------------------- |
| `decode`   | `(value: string) => string`       | 允许自定义如何将 cookie 反序列化为值。 |
...
| 名称      | 类型                        | 描述                                                                                                    |
| :-------- | :-------------------------- | :------------------------------------------------------------------------------------------------------------- |
| `domain`   | `string`       | 指定域。如果未设置域，则大多数客<br/>户端将解释为应用于当前域。 |
| `expires` | `Date` |                           指定 cookie 过期日期。                                            |
| `httpOnly` | `boolean` | 如果为 true，则客户端将无法访问该<br/> Cookie。 |
| `maxAge` | `number` | 以秒为单位指定 cookie 有效的时间。 |
| `path` | `string` | 指定应用 cookie 的域的子路径。 |
| `sameSite` | `boolean \| 'lax' \| 'none' \| 'strict'` | 指定 [SameSite](https://datatracker.ietf.org/doc/html/draft-ietf-httpbis-rfc6265bis-09#section-5.4.7) cookie header 的值。 |
| `secure` | `boolean` | 如果为 true，则该 Cookie 仅在 https<br/> 网站上设置。|
| `encode` | `(value: string) => string` | 允许自定义 cookie 序列化方式。 |
