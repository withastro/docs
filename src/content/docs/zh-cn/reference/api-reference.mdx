...
在静态构建中，这将是 `getStaticPaths()` 返回的`params`，用于预渲染[动态路由](/zh-cn/basics/routing/#动态路由).。
...
将 Cookie 标记为已删除。将 Cookie 标记为已删除。一旦 cookie 被删除，`Astro.cookies.has()` 将返回 `false`，`Astro.cookies.get()` 将返回一个值为 `undefined` 的 [`AstroCookie`](#astrocookie)。选项允许设置要删除的 Cookie 的`domain`和`path`。
...
[端点函数](/zh-cn/basics/endpoints/)接收一个上下文对象作为第一个参数。它与许多 `Astro` 全局属性相似。
...
在静态构建中，这将是用于预渲染[动态路由](/zh-cn/basics/routing/#动态路由)的 `getStaticPaths()` 返回的 `params`。
...
`getImage()`函数旨在生成用于在 HTML 之外的其他地方所使用的图像，例如在 [API 路由](/zh-cn/basics/endpoints/#服务器端点api-路由) 中。它还允许你创建自定义的 `<Image />` 组件。
