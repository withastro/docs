...
`Astro.props` 是一个包含任何作为[组件属性](/zh-cn/basics/astro-components/#component-props)传递的值的对象。`.md` 和 `.mdx` 文件的布局组件接收作为参数的 frontmatter 值。
...
<ReadMore>了解如何为你的 props 添加 [TypeScript 类型定义](/zh-cn/guides/typescript/#component-props) 的内容。</ReadMore>

### `Astro.params` \{#astroparams\}

`Astro.params` 是一个包含与此请求匹配的动态路由段的值的对象。

在静态构建中，这将是 `getStaticPaths()` 返回的`params`，用于预渲染[动态路由](/zh-cn/guides/routing/#dynamic-routes).。
...
:::note
这是针对高级用例的！在大多数情况下，使用 [`<slot />` 元素](/zh-cn/basics/astro-components/#slots)呈现插槽内容会更简单。
:::
...
在静态构建中，这将是用于预渲染[动态路由](/zh-cn/guides/routing/#dynamic-routes)的 `getStaticPaths()` 返回的 `params`。
...
另见：[通过`props`传递数据](#data-passing-with-props)
...
### 通过 `props` 传递数据 \{#data-passing-with-props\}
...
#### `page` 分页参数 \{#the-pagination-page-prop\}
...
## 图像（`astro:assets`） \{#images-astroassets\}
...
`getImage()`函数旨在生成用于在 HTML 之外的其他地方所使用的图像，例如在 [API 路由](/zh-cn/guides/endpoints/#server-endpoints-api-routes) 中。它还允许你创建自定义的 `<Image />` 组件。

`getImage()` 函数接受一个选项对象，其中包含与 [Image 组件相同的属性](#properties)（除了 `alt`）。
...
## 内容集合（`astro:content`） \{#content-collections-astrocontent\}
...
有关示例请[参考 `内容集合`指南](/zh-cn/guides/content-collections/#defining-a-collection-schema) 
...
有关示例请[参考 `内容集合`指南](/zh-cn/guides/content-collections/#defining-collection-references).
...
有关示例请[参考 `内容集合`指南](/zh-cn/guides/content-collections/#querying-collections) 以获取示例用法。
...
有关`内容集合`的示例，请参考 [查询集合条目](/zh-cn/guides/content-collections/#querying-collections).
...
`getEntries()` 是一个从同一集合中检索多个集合条目的函数。这对于[返回引用条目的数组](/zh-cn/guides/content-collections/#defining-collection-references)访问其关联的`data`、`body`和`render()`属性非常有用。
...
可用于 Markdown 或 MDX 文档的 URL 标头。默认为不含文件扩展名的 "id"，但可以通过在文件的 frontmatter 中设置[`slug`属性](/zh-cn/guides/content-collections/#defining-custom-slugs)来覆盖。
...
- `headings` - 生成的标题列表，[与 Astro 的 `getHeadings()` 工具函数相同](/zh-cn/guides/markdown-content/#exported-properties) 在 Markdown 和 MDX 导入中。
- `remarkPluginFrontmatter ` - 在应用 [remark 或 rehype 插件](/zh-cn/guides/markdown-content/#modifying-frontmatter-programmatically)后修改后的 frontmatter 对象。设置为类型 `any`。
...
有关示例请[参考 `内容集合`指南](/zh-cn/guides/content-collections/#rendering-content-to-html) 以获取示例用法。

### 其他内容集合类型 \{#other-content-collection-types\}
...
- `image()` 模式辅助工具允许你 [在内容集合中使用本地图片](/zh-cn/guides/images/#images-in-content-collections)。
...
## 中间件（`astro:middleware`） \{#middleware-astromiddleware\}
...
<ReadMore>有关更多信息，请参阅 [端点上下文](#endpoint-context)。</ReadMore>
...
自从 Astro v4.13.0，`next()` 接受一个可选的 URL 路径参数，形式为字符串、`URL` 或 `Request`，用于[重写](/zh-cn/guides/routing/#rewrites)当前请求而不重新触发新的渲染阶段。
...
一个底层 API，用于创建一个 [`APIContext`](#endpoint-context) 以传递给 Astro 中间件的 `onRequest()` 函数。
...
## 国际化（`astro:i18n`） \{#internationalization-astroi18n\}
...
像使用 [`getAbsoluteLocaleUrl`](/zh-cn/guides/internationalization/#custom-locale-paths) 那样使用此函数，返回所有语言环境的绝对路径列表。
...
当配置了[自定义语言环境路径](/zh-cn/guides/internationalization/#custom-locale-paths)时，此函数返回与一个或多个 `codes` 关联的 `path`。
...
## 视图过渡客户端 API (`astro:transitions/client`) \{#view-transitions-client-api-astrotransitionsclient\}
...
#### `sourceElement` 选项 \{#sourceelement-option\}
...
有关如何选择和配置回退行为的详细信息，请参阅 [回退控制](/zh-cn/guides/view-transitions/#fallback-control) 指南。


### `astro:before-preparation` 事件 \{#astrobefore-preparation-event\}
...
### `astro:after-preparation` 事件 \{#astroafter-preparation-event\}
...
### `astro:before-swap` 事件 \{#astrobefore-swap-event\}
...
这是在 [`navigate()` 函数](#navigate) 的 [`info` 选项](#info-option) 中传递的文字值。
...
`defineAction()` 函数接受一个 [`handler()`](#handler-property) 函数，其中包含在调用 Action 时要运行的服务器逻辑，以及一个可选的 [`input`](#input-validator) 属性，用于在运行时验证输入参数。
...
`handler()` 函数被调用时，接受用户输入作为其第一个参数。如果设置了 [`input`](#input-validator) 验证器，那么用户输入将在传递给 `handler()` 前进行验证。第二个参数是一个 `context` 对象，包含大多数 Astro 的 [标准端点上下文](#endpoint-context)，不包括 `getActionResult()`, `callAction()`, 和 `redirect()`。

返回值使用 [devalue 库](https://github.com/Rich-Harris/devalue) 进行解析。它支持 JSON 值，以及 `Date()`, `Map()`, `Set()`, 或 `URL()` 的实例。

#### `input` 验证器 \{#input-validator\}
...
你可以使用 [`ComponentProp` 类型](/zh-cn/guides/typescript/#componentprops-type) 工具函数来引用这些组件的 `Props`。
...
该组件在构建时为代码块提供语法高亮（不包括客户端 JavaScript）。该组件由 Shiki 内部驱动，它支持所有流行的 [主题](https://shiki.style/themes)和 [语言](https://shiki.style/languages)。另外，你可以通过将自定义主题、语言、[转换器](#transformers) 和 [默认配色](https://shiki.style/guide/dual-themes#without-default-color) 分别传递给 `theme`、`lang`、`transformers` 和 `defaultColor` 属性以添加它们。

:::note
该组件 **不会** 继承你在 [Shiki 配置](/zh-cn/guides/markdown-content/#shiki-configuration) 中的设置。你必须使用组件属性来设置它们。
:::
...
请参见在 Astro 语法中使用[片段](/zh-cn/basics/astro-syntax/#fragments)的更多信息。
...
这个组件通过应用 Prism 的 CSS 类为代码块提供特定语言的语法高亮。注意，**你需要提供 Prism 的 CSS 样式表**（或用自己的），以启用语法高亮！参见 [Prism 配置部分](/zh-cn/guides/markdown-content/#prism-configuration)了解更多细节。
...
一个通用组件，用于呈现 [内容集合条目](/zh-cn/guides/content-collections/#what-are-content-collections) 的内容。
...
查看更多关于如何[控制路由器](/zh-cn/guides/view-transitions/#router-control)和向页面元素和组件添加[过渡动画指令](/zh-cn/guides/view-transitions/#transition-directives)的信息。
