...
Astro 的内容加载器 API 允许你从任意来源（本地或远程）载入数据，并与 Astro 的 content layer 交互以管理你的 [内容集合](/zh-cn/guides/content-collections/)。
...
**类型：** <code>(options: GlobOptions) => <a href="#the-loader-object">Loader</a></code><br />
<Since v="5.0.0" />
</p>
...
回调函数，返回集合中每个条目的唯一字符串。它接受具有以下属性的对象作为参数：
...
**类型：** <code>(fileName: string, options?: FileOptions) => <a href="#the-loader-object">Loader</a></code><br />
<Since v="5.0.0" />
</p>

`file()` 加载器从单个文件创建条目，该文件包含具有唯一 `id` 字段的对象数组或具有字符串键的对象。它支持 JSON 或 YAML，你可以为默认情况下无法解析的数据文件提供一个自定义 `parser`。
...
回调函数，用于从文件内容创建一个集合。当你需要处理默认情况下不支持的文件（例如 `.csv`）或使用 嵌套的 `.json` 文档](/zh-cn/guides/content-collections/#嵌套的-json-文档) 时，请使用它。
...
应用了所有默认值的完整、已解析的 Astro 配置对象。有关更多信息，请参阅 [配置参考](/en/reference/configuration-reference/)。
...
如果未设置，则模式中使用 [`image()` 助手函数](/zh-cn/guides/images/#内容集合中的图像) 的任何字段都将被视为 [public 路径](/zh-cn/guides/images/#在哪里存储图像) 并且跳过图像优化。
