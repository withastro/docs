...
每个集合都需要在 [其模式（schema）中定义一个加载器](/zh-cn/guides/content-collections/#定义集合-loader)。你可以在项目的 `src/content.config.ts` 文件中定义内联加载器，从而在多个集合之间共享一个加载器，甚至 [将你的加载器作为包发布到 NPM](/zh-cn/guides/integrations/) 以与其他人共享并包含在我们的集成库中。
