...
然后你可以在你的页面中使用这些提示。如果你启用了 [实验性路由缓存](/zh-cn/reference/route-caching/)，请直接将缓存提示传递给 Astro.cache.set()：
...
:::note
缓存提示不会自动让响应被 Astro 缓存。它们仅提供可以传递给 [路由缓存](/zh-cn/reference/route-caching/) 的值，或者用于实现你自己的缓存策略。
:::
