...
## 为单页面应用样式 \{#style-an-individual-page\}
...
## 使用你的第一个 CSS 变量 \{#use-your-first-css-variable\}
Astro 的 `<style>` 标签还可以使用 `define:vars={ {...} }` 指令引用 `frontmatter` 中的任何变量。你可以**在代码围栏中定义变量，然后在样式标签中将其用作 CSS 变量使用**。
...
## 动手试一试 - 定义 CSS 变量 \{#try-it-yourself---define-css-variables\}
...
### 相关资源 \{#resources\}
- [Astro 和 JSX 间的差异比较](/zh-cn/basics/astro-syntax/#differences-between-astro-and-jsx)
- [Astro `<style>` 标签](/zh-cn/guides/styling/#styling-in-astro)
- [Astro 里的 CSS 变量](/zh-cn/guides/styling/#css-variables)
