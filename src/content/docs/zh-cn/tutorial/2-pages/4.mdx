...
使用Astro的`<style></style>`标签，你可以为页面上的元素设置样式。你还可以通过给这些标签添加**属性**和**指令**，获得更多的样式设置方式。
...
## 使用你的第一个CSS变量
Astro的`<style>`标签还可以使用 `define:vars={{ {...} }}` 指令引用 `frontmatter` 中的任何变量。你可以**在代码栏中定义变量，然后在样式标签中将其用作CSS变量使用**。
...
### 资源
- [Astro syntax vs JSX - comparison](/cn/core-concepts/astro-components/#differences-between-astro-and-jsx)

- [Astro `<style>` tag](/cn/guides/styling/#styling-in-astro)

- [CSS variables in Astro](/cn/guides/styling/#css-variables)
