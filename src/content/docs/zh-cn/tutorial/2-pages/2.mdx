...
现在你已经使用 `.astro` 文件构建了页面，让我们使用 `.md` 文件编写一些博客文章！
...
3. 通过将 `/posts/post-1` 拼接到现有预览 URL 的末尾，并在浏览器中预览此页面。（例如 `localhost:3000/posts/post-1`）
...
2. 在 `localhost:3000/posts/post-1` 再次查看你的浏览器上的页面预览。你现在应该可以在此页面上看到文章内容。

3. 使用浏览器的开发工具检查此页面。请注意，虽然你没有键入任何 HTML 元素，但你的 Markdown 已转换为 HTML。你可以看到标题、段落和列表项等元素。

:::note
文件顶部的信息，在代码块内，称为 frontmatter。此数据（包括标签和文章图像）是 Astro 可以使用的*有关*你的文章的信息。它不会自动出现在页面上，但我们稍后会在教程中了解它的作用。
:::
...
1. 在 `src/pages/blog.astro` 中使用`<a href=""><a/>`标签链接到你的第一篇文章：
    ...
    <html lang = "zh">
      ...
      <body>
        ...
        <p>这是我的 Astro 学习笔记</p>
        <ul>
          <li><a href="/posts/post-1/">文章1</a></li>
        </ul>
      </body>
    </html>
    ```
...
2. 现在，在 `src/pages/posts/` 中再添加两个文件：`post-2.md` 和 `post-3.md`。这里有一些示例代码，你可以将其复制并粘贴到你的文件中，或者，你可以编写自己的代码！
    ...
    title: 我的第三篇博客文章
    author: Astro 学者
    description: "学习了一些 Astro，我根本停不下来！"
    image: 
        url: "https://astro.build/assets/blog/astro-showcase/astro-showcase-screenshot.jpg"
        alt: "来自 Astro 示例网站的网站缩略图。"
    pubDate: 2022-07-08
    tags: ["astro", "博客构建", "JavaScript","成功学"]
    ---
...
3. 使用`<a></a>`标签链接到新文章：
    ...
    <html lang = "zh">
      ...
      <body>
        ...
        <ul>
          <li><a href="/posts/post-1/">文章1</a></li>
          <li><a href="/posts/post-2/">文章2</a></li>
          <li><a href="/posts/post-3/">文章3</a></li>
        </ul>
      </body>
    </html>
    ```
...
- [什么是浏览器开发者工具？- MDN](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/What_are_browser_developer_tools) <Badge>外部链接</Badge>
