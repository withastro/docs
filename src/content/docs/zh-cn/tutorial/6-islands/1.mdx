...
4. 添加第二个 `<Greeting />` 组件，并加上 `client:load` 指令。
  ...
  5. 查看你的页面并比较这两个组件。第二个按钮可工作，因为 `client:load` 指令告诉 Astro 在页面加载时将其 JavaScript 发送并重新运行到客户端，使组件可交互。我们将其称为**被 Hydrate 的**组件。
...
- [在 Astro 中使用 UI 框架组件](/zh-cn/core-concepts/framework-components/#using-framework-components)
