...
## 创建 `src/components/` 文件夹 \{#create-a-new-srccomponents-folder\}

你需要在项目中创建一个新的文件夹 `src/components/` 来容纳 `.astro` 文件，它们会被用来生成 HTML 代码，但不会成为网站上的新页面。

## 创建一个导航组件 \{#create-a-navigation-component\}
...
### 导入并使用 Navigation.astro \{#import-and-use-navigationastro\}
...
## 自己试一试 - 在网站的其余页面上添加导航 \{#try-it-yourself---add-navigation-to-the-rest-of-your-site\}
