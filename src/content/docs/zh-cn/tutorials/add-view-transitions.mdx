...
## 教程的示例代码 \{#build-a-blog-tutorial-code\}
...
## 全页面导航 vs 客户端路由（SPA 模式） \{#full-page-navigation-vs-client-side-routing-spa-mode\}
...
## 在示例博客代码中增加视图过渡 \{#extending-the-blog-tutorial-with-view-transitions\}

通过下列步骤你将了解：如何通过添加客户端路由来增强页面转换的效果，用以扩展之前简陋的示例博客。

### 更新依赖 \{#upgrade-dependencies\}
<Steps>
...
### 添加 `<ViewTransitions />` 路由 \{#add-the-viewtransitions--router\}
<Steps>
...
### 更新你的脚本 \{#update-scripts\}

使用视图过渡后，部分脚本可能在页面导航后不再像在完整页面浏览器刷新时那样重新运行。在客户端路由期间，有几个[事件可以监听](/zh-cn/guides/view-transitions/#lifecycle-events)，并在发生时触发事件。你项目中的脚本现在需要连接到两个事件，以在页面导航期间的适当时间运行：[`astro:page-load`](/zh-cn/guides/view-transitions/#astropage-load)和[`astro:after-swap`](/zh-cn/guides/view-transitions/#astroafter-swap)。
...
### 自定义过渡动画 \{#customize-transition-animations\}
<Steps>
7. 将页面标题的默认淡入淡出动画改为滑动动画。
    ...
    <Box icon="puzzle-piece">
      ### 小试牛刀 - 淡入导航链接 \{#try-it-yourself---make-the-navigation-links-slide-in\}

      按照[上述相同的步骤](#customize-transition-animations)，添加一个动画指令，使包含所有网页页眉链接的 `Navigation.astro` 中的 `<div>` 在页面导航时播放滑动动画。
...
8. 为你的博客文章描述添加一个时间更长的淡入效果。
    ...
    <ReadMore>了解更多关于不同的 [过渡指令](/zh-cn/guides/view-transitions/#transition-directives) 和 [自定义动画](/zh-cn/guides/view-transitions/#customizing-animations) 的信息。</ReadMore>
</Steps>
### 为某些链接设置强制浏览器重新加载页面 \{#force-a-full-browser-reload-for-some-links\}
<Steps>
