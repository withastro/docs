...
## 构建博客教程代码 \{#build-a-blog-tutorial-code\}

在[构建博客入门教程](/zh-cn/tutorial/0-introduction/)中，你了解了 Astro [内置的基于文件的路由](/zh-cn/guides/routing/#static-routes)：`src/pages/` 文件夹中的任何 `.astro`、`.md` 或 `.mdx` 文件都会自动成为你站点上的页面。
...
## 页面与集合 \{#pages-vs-collections\}

即使使用内容集合，你仍会使用 `src/pages/` 文件夹来管理独立页面，例如“关于我”页面。但是，将博客文章移动到特殊的 `src/content/` 文件夹中将允许你使用更强大和高效的 API 来生成博客文章索引和展示单独的博客文章。

与此同时，你将在代码编辑器中获得更好的提示和自动补全，因为你将拥有一个 **[模式（schema）](/zh-cn/guides/content-collections/#defining-a-collection-schema)**，用于为每篇文章定义一个通用的结构，Astro 也将帮助你强制匹配该结构。在模式中，你可以指定什么时候 frontmatter 属性是需要的，例如描述或作者信息，以及每个属性的数据类型应该是什么，例如字符串或数组。这将更早地发现许多错误，并提供了详细的错误信息，帮助你快速定位问题。
...
## 使用内容集合扩展博客教程 \{#extending-the-blog-tutorial-with-content-collections\}

下面的步骤将向你展示如何通过为博客文章创建内容集合来扩展构建博客教程的完整项目。

### 升级依赖项 \{#upgrade-dependencies\}
...
2. 在博客教程中，使用的是不那么严格的 `base` TypeScript 设置。要使用内容集合，你必须通过以下方式之一为内容集合[设置 TypeScript](/zh-cn/guides/content-collections/#setting-up-typescript)，即使用 `strict` 或 `strictest` 设置，或者在 `tsconfig.json` 中添加两个选项。
...
### 为博客文章创建一个集合 \{#create-a-collection-for-your-blog-posts\}
...
5. 创建一个 `src/content/config.ts` 文件，用于为你的 `postsCollection` [定义一个模式](/zh-cn/guides/content-collections/#defining-a-collection-schema)。对于现有的博客教程代码，向文件中添加如下内容以定义其博客文章中使用的所有 frontmatter 属性：
...
### 从一个集合生成页面 \{#generate-pages-from-a-collection\}
...
8. 添加以下代码来[查询你的集合](/zh-cn/guides/content-collections/#querying-collections)，以便将每个博客文章的 slug 和页面内容提供给它将生成的每个页面：
...
### 用 `getCollection()` 代替 `Astro.glob()` \{#replace-astroglob-with-getcollection\}
...
13. 教程中的博客项目还会使用 `src/pages/tags/[tag].astro` 动态为每个标签生成一个页面，并在 `src/pages/tags/index.astro` 中展示标签列表。 
        ...
        <Box icon="puzzle-piece">
          ### 试一试 - 在标签索引页面中更新查询语句 \{#try-it-yourself---update-the-query-in-the-tag-index-page\}

          导入并使用 `getCollection` 来获取在 `src/pages/tags/index.astro` 中的博客文章中使用的标签，按照[上述相同的步骤](#replace-astroglob-with-getcollection)进行操作。
...
### 更新 frontmatter 值以匹配你的模式 \{#update-any-frontmatter-values-to-match-your-schema\}
