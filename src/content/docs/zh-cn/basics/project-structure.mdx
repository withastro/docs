...
- [UI 框架组件（React 等）](/zh-cn/basics/framework-components/)
...
**组件**是你在 HTML 页面中可重复使用的代码单元。它可以是 [Astro 组件](/zh-cn/basics/astro-components/) 或是像 React 或 Vue 这样的[UI 框架组件](/zh-cn/basics/framework-components/)。通常将你项目中所有组件都分组放在这个文件夹中。
