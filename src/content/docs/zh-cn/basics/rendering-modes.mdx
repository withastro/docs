...
Astro 页面、路由和 API 端点可以在[构建时预渲染](#pre-rendered)或者在请求路由时[由服务器按需渲染](#on-demand-rendered)。通过 [Astro 群岛](/zh-cn/concepts/islands/)，你也可以在必要时包含一些客户端渲染。

在 Astro 中，大部分处理都是在服务器上进行，而非浏览器中。这通常使你的网站或应用在性能羸弱的设备或在较慢的互联网连接上查看时比客户端渲染更快。同时，服务端渲染的 HTML 不仅快速、对 SEO 友好，并且还默认支持无障碍。

## 服务器 `output` 模式 \{#server-output-modes\}
...
<ReadMore>了解更多关于 [按需渲染和适配器的功能](/zh-cn/guides/server-side-rendering/#on-demand-rendering-features)，例如 cookie，Response 和 Request 对象，HTML 流式处理等等。</ReadMore>

如果你需要以下功能，[可以考虑在你的 Astro 项目中启用 `server` 或 `hybrid` 模式](/zh-cn/guides/server-side-rendering/#enable-on-demand-server-rendering)：
