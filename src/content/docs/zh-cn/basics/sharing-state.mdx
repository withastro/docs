...
像 React 或者 Vue 这样的 UI 框架可能鼓励使用 ["context"](https://react.dev/learn/passing-data-deeply-with-context) 来为其他组件提供上下文信息。但是在 Astro 或者 Markdown 中的 [部分激活组件 (partially hydrating components)](/zh-cn/basics/framework-components/#激活组件) 不能使用上下文封装。
...
- 从一个 `.astro` 文件或者 [非激活组件](/zh-cn/basics/framework-components/#激活组件) 写入状态库将不会影响 [客户端组件](/zh-cn/reference/directives-reference/#客户端指令) 值的获取。
