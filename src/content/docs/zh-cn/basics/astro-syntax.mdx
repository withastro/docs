...
Astro 组件语法是 HTML 的超集。这个语法[旨在让任何有编写 HTML 或 JSX 经验的人都感到熟悉](#differences-between-astro-and-jsx) ，并增加了对组件和 JavaScript 表达式的支持。

## 类 JSX 表达式 \{#jsx-like-expressions\}
...
请参阅下面的更多例子 [Astro 和 JSX 之间的差异](#differences-between-astro-and-jsx)。
:::
...
- **不支持 hydration 指令**。当使用[`client:*` hydration 指令时](/zh-cn/guides/framework-components/#hydrating-interactive-components)，Astro 需要知道哪些组件要捆绑到生产环境中，而动态标签模式阻止了这一点。
...
### Astro 和 JSX 间的差异 \{#differences-between-astro-and-jsx\}
