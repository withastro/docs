...
当你的 Astro 组件确实需要客户端交互性时，你可以添加 [标准 HTML `<script>` 标签](/zh-cn/guides/client-side-scripts/) 或 [UI 框架组件](/zh-cn/guides/framework-components/#hydrating-interactive-components)。

## 组件概述 \{#component-structure\}
...
### 组件模板 \{#the-component-template\}

在组件脚本下面的是组件模板。组件模板决定了你的组件的 HTML 输出。

如果你在这里写普通的 HTML，你的组件将在任何 Astro 页面上呈现它被导入和使用的 HTML。

但是，[Astro 的组件模板语法](/zh-cn/basics/astro-syntax/) 还支持 **JavaScript 表达式**、Astro [`<style>`](/zh-cn/guides/styling/#styling-in-astro) 和 [`<script>`](/zh-cn/guides/client-side-scripts/#using-script-in-astro) 标签、**导入的组件**和[**特殊的 Astro 指令**](/zh-cn/reference/directives-reference/)。组件脚本中定义的数据和值可以在组件模板中使用，以生成动态创建的 HTML。
...
## 基于组件的设计 \{#component-based-design\}
...
### 插槽回退内容 \{#fallback-content-for-slots\}
...
:::note
HTML 组件内的 [`<slot />` 元素](#slots)会像在 Astro 组件中那样工作。要使用 [HTML Web Component Slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) 元素，请在 `<slot>` 元素中添加 `is:inline`。
:::
