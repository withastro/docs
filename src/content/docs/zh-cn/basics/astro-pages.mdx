...
一个文件也可以使用[动态路由](/zh-cn/guides/routing/#dynamic-routes)来生成多个页面。这允许你即使内容不在特殊的 `/pages/` 目录中，也可以创建页面，例如在[内容集合](/zh-cn/guides/content-collections/)或[内容管理系统](/zh-cn/guides/cms/)中。
...
[Markdown 或 MDX 页面内容的集合](/zh-cn/guides/content-collections/)可以用来[动态生成页面](/zh-cn/guides/routing/#dynamic-routes)。
...
`.html` 文件扩展名的文件可以放在 `src/pages/` 中，并直接用作网站上的页面。请注意，某些关键的 Astro 功能在 [HTML 组件](/zh-cn/basics/astro-components/#html-components) 中不受支持。
...
要为[按需渲染的页面](/zh-cn/guides/server-side-rendering/#enable-on-demand-server-rendering)创建自定义的 500 错误页面，可以创建一个 `src/pages/500.astro` 文件。但这个自定义页面不适用于预渲染的页面，也不能被预渲染。
