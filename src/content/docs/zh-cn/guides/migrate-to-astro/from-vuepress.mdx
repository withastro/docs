...
带着你现有的 Markdown 内容来 [创建 Markdown 页面](/zh-cn/guides/markdown-content/)。 你仍然可以利用 [基于文件的路由](/zh-cn/guides/routing/)，只需将这些文档从 VuePress 的 `docs` 文件目录下 移动到 Astro 的 `src/pages/` 目录下，并且保证创建了与现有的 VuePress 项目对应的文件夹，就能够保留现有的URLs. 

VuePress 或你安装的其他任意主题可能已经为你处理了大部分站点的布局和元数据。你可能希望阅读关于 [在 Astro 中将 Markdown 页面封装为布局](/zh-cn/basics/layouts/#markdownmdx-布局) 以了解如何在 Astro 中自行管理模板，包括页面的 `<head>` 部分。
