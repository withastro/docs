...
## Next.js 和 Astro 的主要相似之处 \{#key-similarities-between-nextjs-and-astro\}

Next.js 和 Astro 有一些相似之处，可以帮助你迁移你的项目：

- [`.astro` 文件的语法类似于 JSX](/zh-cn/basics/astro-syntax/#differences-between-astro-and-jsx)。编写 Astro 应该会感觉很熟悉。
- Astro 项目也可以是 SSG 或[具有页面预渲染功能的 SSR](/zh-cn/guides/server-side-rendering/)。
- Astro 使用基于文件的路由，并[允许特定命名的页面创建动态路由](/zh-cn/guides/routing/#dynamic-routes)。
...
## Next.js 和 Astro 的主要不同之处 \{#key-differences-between-nextjs-and-astro\}
...
- [内容驱动](/zh-cn/concepts/why-astro/#content-driven)：Astro 旨在展示你的内容，并允许你仅在需要时选择加入交互性。已有的 Next.js 应用程序可能是为重客户端交互性而构建的，并且包含一些使用 `.astro` 组件难以复制、更具挑战性的功能，可能需要高级 Astro 技术来处理，例如仪表盘。

## 转换你的 Next.js 项目 \{#convert-your-nextjs-project\}
...
你可以在 `create astro` 命令中传递一个 `--template` 参数，用我们的官方启动器（如 `docs`、`blog`、`portfolio`）启动一个新的 Astro 项目。或者，你可以[从 GitHub 上任何现有的 Astro 仓库开始一个新项目](/zh-cn/install-and-setup/#use-a-theme-or-starter-template)。
...
:::tip
访问 https://astro.new 了解官方启动模板的完整列表，以及在 StackBlitz、CodeSandbox 或 Gitpod 中打开一个新项目的链接。
:::
...
### 将源码放在 `src` 目录内 \{#put-your-source-code-in-src\}
...
### Astro 的配置文件 \{#the-astro-config-file\}
...
2. 调整 [Next 或 JSX 语法为 Astro](#reference-convert-nextjs-syntax-to-astro) 或 HTML 网页标准。例如，这包括 `<Link>`、`<Script>`、`{children}` 和 `className`。

3. 将任何必要的 JavaScript，包括导入语句，移到["代码栅栏"（`---`）](/zh-cn/basics/astro-components/#the-component-script)。注意：[条件渲染内容](/zh-cn/basics/astro-syntax/#dynamic-html)的 JavaScript 通常直接写在 Astro 的 HTML 模板内。
...
6. 用导入语句或 [`Astro.glob()`](/zh-cn/reference/api-reference/#astroglob) 代替 `getStaticProps()` 来查询你的本地文件。使用 `fetch()` 来获取外部数据。

参考 [`Next.js` 文件逐步转换的例子](#guided-example-next-data-fetching-to-astro)。

#### 比较：JSX vs Astro \{#compare-jsx-vs-astro\}
...
每个 Astro 页面都明确要求有 `<html>`、`<head>` 和 `<body>` 标签，所以在不同的页面中重复使用一个布局文件是很常见的。Astro 使用一个 [`<slot />`](/zh-cn/basics/astro-components/#slots) 作为页面内容，不需要导入语句。注意标准的 HTML 模板，以及对 `<head>` 的直接访问：
...
#### 迁移 Next.js 的 `page` 目录 \{#migrating-from-nextjs-pages-directory\}
...
#### 迁移 Next.js 的 `/app` 目录 \{#migrating-from-nextjs-app-directory\}
...
### 迁移 Pages 和 Posts \{#migrating-pages-and-posts\}
...
现有的 Next JSX (.js) 页面将需要从 JSX 文件[转换为 .astro 页面](#tips-convert-jsx-files-to-astro-files)。你不能在 Astro 中使用现有的 JSX 页面文件。
...
## 参考：将 Next.js 语法转换为 Astro 语法 \{#reference-convert-nextjs-syntax-to-astro\}

### 链接 \{#next-links-to-astro\}
...
### 转换 Next 的 Children Props 到 Astro \{#next-children-props-to-astro\}
...
可以使用[命名插槽](/zh-cn/basics/astro-components/#named-slots)将有多个子组件的 React 组件迁移到 Astro 组件。

参考更多关于 [Astro 中具体的 `<slot />` 用法](/zh-cn/basics/astro-components/#slots)。

### 转换 Next 的数据获取到 Astro \{#next-data-fetching-to-astro\}
...
### 转换 Next 的样式到 Astro \{#next-styling-to-astro\}

你可能需要用 Astro 中其他可用的 CSS 选项来替换任何 [CSS-in-JS 库](https://github.com/withastro/astro/issues/4432)（例如 styled-components）。

如果有必要，将任何内联样式对象（`style={{ fontWeight: "bold" }}`）转换成内联 HTML 样式属性（`style="font-weight:bold;"`）。或者，使用 [Astro `<style>`标签](/zh-cn/guides/styling/#styling-in-astro)实现 scoped CSS styles。
...
### 转换 Next 的 图像插件到 Astro \{#next-image-plugin-to-astro\}
 
根据情况在你的 React 组件中将任何 Next 的 `<Image />` 组件转换为 [Astro 自己的图像集成组件](/zh-cn/guides/images/#image--astroassets)或者转化成[标准的 HTML `<img>` / JSX `<img />` 标签](/zh-cn/guides/images/#images-in-ui-framework-components)。

Astro 的 `<Image />` 组件只能在 `.astro` 和 `.mdx` 文件中工作。查看该组件可用的[完整组件属性列表](/zh-cn/guides/images/#properties)，并注意它与 Next 的属性有一些不同。
...
## 示例：数据请求 \{#guided-example-next-data-fetching-to-astro\}
...
- 视频：[Astro 如何使我的网站速度提升 100 倍](https://www.youtube.com/watch?v=cOxA3kMYtkM)。
