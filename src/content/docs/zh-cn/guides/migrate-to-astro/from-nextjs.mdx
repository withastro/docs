...
6. 用导入语句或 [`import.meta.glob()`](/zh-cn/reference/api-reference/#importmetaglob) 代替 `getStaticProps()` 来查询你的本地文件。使用 `fetch()` 来获取外部数据。
...
将 `getStaticProps()` 中任何实例转换为 `import.meta.glob()` 或 `getCollection()`/`getEntryBySlug()`，以便访问项目中其他文件的数据。为了[获取远程数据](/zh-cn/guides/data-fetching/)，使用 `fetch()`。
