...
- Astro 项目也可以是SSG或[具有页面预渲染功能的SSR](/zh-cn/guides/server-side-rendering/#混合渲染)。
...
你可能会发现从把你的 Next.js 布局和模板转换为[Astro布局组件](/zh-cn/core-concepts/layouts/)会很有帮助。
...
在安装了[ Tailwind 集成](/zh-cn/guides/integrations-guide/tailwind/)之后，就支持 Tailwind 。不需要对您现有的 Tailwind 代码进行任何修改
