...
用[ Astro 自己的图片集成组件](/zh-cn/guides/images/#astro-的图片集成)，或用标准的HTML`<img>`来转换任何Next`<Image />`组件。请看[组件属性的完整列表](/zh-cn/guides/integrations-guide/image/#使用)，可用于Astro的`<Image />`和`<Picture />`组件，注意有几个属性和Next不同。
