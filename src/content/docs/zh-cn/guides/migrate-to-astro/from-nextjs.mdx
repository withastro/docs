...
1. 将任何必要的 JavaScript，包括导入语句，移到["代码栅栏"（`---`）](/zh-cn/core-concepts/astro-components/#组件脚本)。注意：[条件渲染内容](/zh-cn/core-concepts/astro-syntax/#动态-html)的JavaScript通常直接写在Astro的HTML模板内。
...
将任何 Next 的 `<Image />` 组件转换为 [Astro 自己的图像集成组件](/zh-cn/guides/images/#image--astroassets) 或者转化成 React 组件中适当的 [标准 HTML `<img>` / JSX `<img />` 标签](/zh-cn/guides/images/#框架组件中的图像)。

Astro 的 `<Image />` 组件只能在 `.astro` 和 `.mdx` 文件中工作。查看该组件可用的[完整组件属性列表](/zh-cn/guides/images/#属性)，并注意其中几个会与 Next 的属性不同。
