...
你可以通过 `create astro` 命令传递一个 `--template` 参数来启动一个带有我们官方起始器的新 Astro 项目。或者，你可以[从 GitHub 上任何现有的 Astro 仓库开始一个新项目](/zh-cn/install/auto/#starter-templates)。
...
- GitBook 的一些特性依赖于自定义组件。这些组件在 Astro 中不存在，必须通过 [Markdoc 的配置 `tags` 属性](/zh-cn/guides/integrations-guide/markdoc/#use-astro-components-as-markdoc-tags) 创建并添加到你的项目中，或从你的文件中删除。
