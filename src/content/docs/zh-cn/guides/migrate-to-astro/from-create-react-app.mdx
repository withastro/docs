...
## CRA 和 Astro 的关键相似之处 \{#key-similarities-between-cra-and-astro\}

-  [`.astro` 文件的语法与 JSX 类似](/zh-cn/basics/astro-syntax/#differences-between-astro-and-jsx)。编写 Astro 应该会让你感到熟悉。

-  Astro 使用基于文件的路由，[并允许通过特殊的文件命名来创建动态路由](/zh-cn/guides/routing/#dynamic-routes)。
...
-  Astro 支持 [安装 NPM 包](/zh-cn/guides/imports/#npm-packages)，包括 React 库。许多现有的依赖项在 Astro 中可以正常工作。

## CRA 和 Astro 的关键差异 \{#key-differences-between-cra-and-astro\}
...
-  [内容驱动](/zh-cn/concepts/why-astro/#content-driven)：Astro 的设计目标是展示你的内容，并允许你在需要时选择性地启用交互。如果现有的 CRA 应用用于构建高级的客户端交互，那么可能需要使用高级的 Astro 技术来让 `.astro` 组件来完成更具挑战性的复制，例如仪表板。

## 将 CRA 添加到 Astro \{#add-your-cra-to-astro\}
...
### 添加集成和依赖 \{#add-integrations-and-dependencies\}
...
### 添加现有的应用文件 \{#add-your-existing-app-files\}
...
:::note[客户端指令]
你的应用程序需要使用 [客户端指令](/zh-cn/reference/directives-reference/#client-directives) 来实现交互。在你选择启用客户端 JavaScript 之前，Astro 将把你的 React 应用程序呈现为静态 HTML。

而使用 `client:load` 来确保你的应用程序立即从服务器加载，或者使用 `client:only="react"` 来跳过服务器端渲染，完全在客户端运行你的应用程序。
:::

有关详细信息和可用选项，请参阅我们的 [配置 Astro](/zh-cn/guides/configuring-astro/) 指南。

## 将 CRA 转换为 Astro \{#convert-your-cra-to-astro\}

除了 [将现有应用程序添加到 Astro](#add-your-cra-to-astro)，你可能也希望将应用程序本身直接转换为 Astro！
...
当你转换你的 React 应用程序时，你需要决定哪些 React 组件你将会 [重写为 Astro 组件](#converting-jsx-files-to-astro-files)。唯一的限制是 Astro 组件可以导入 React 组件，但 React 组件只能导入其他 React 组件：
...
### 比较：JSX 与 Astro \{#compare-jsx-vs-astro\}
...
### 将 JSX 文件转换为 `.astro` 文件 \{#converting-jsx-files-to-astro-files\}

下面是将 CRA（Create React App）的 `.js` 组件转换为 `.astro` 组件的一些建议：

1. 将现有 CRA 组件函数返回的 JSX 作为 HTML 模板的基础。

2. 将任何 [CRA 或 JSX 语法更改为 Astro](#reference-convert-cra-syntax-to-astro) 或 HTML 的标准规范。这包括 `{children}` 和 `className` 等。

3. 将任何必要的 JavaScript，包括导入语句，移动到 [「代码围栏」（`---`）](/zh-cn/basics/astro-components/#the-component-script) 中。注意：在 Astro 中，[条件性渲染内容](/zh-cn/basics/astro-syntax/#dynamic-html)的 JavaScript 通常直接写在 HTML 模板中。

4. 使用 [`Astro.props`](/zh-cn/reference/api-reference/#astroprops) 访问之前传递给 CRA 函数的任何其他属性。

5. 决定是否需要将任何导入的组件转换为 Astro。你可以将它们暂时保留为 React 组件，或永久保留为 React 组件。但是，如果它们不需要交互性，你可能最终会想将它们转换为 `.astro` 组件！

6. 将 `useEffect()` 替换为导入语句或 [`Astro.glob()`](/zh-cn/reference/api-reference/#astroglob) 来查询本地文件，使用 `fetch()` 来获取外部数据。
...
## 参考：将 CRA 语法转换为 Astro \{#reference-convert-cra-syntax-to-astro\}

### 在 Astro 中转换 CRA 导入语句 \{#cra-imports-to-astro\}

将任何 [文件导入](/zh-cn/guides/imports/) 准确地更新为相对引用的文件路径。可以通过使用 [导入别名](/zh-cn/guides/typescript/#import-aliases) 或完整写出相对路径来实现。
...
### 在 Astro 中转换 CRA 子组件属性 \{#cra-children-props-to-astro\}
...
传递多组子元素的 React 组件可以通过使用 [命名插槽](/zh-cn/basics/astro-components/#named-slots) 转换为 Astro 组件。

了解更多有关于 [特定 `<slot />` 用法](/zh-cn/basics/astro-components/#slots) 的内容。

### 在 Astro 中转换 CRA 数据获取 \{#cra-data-fetching-to-astro\}

在 Create React App 组件中获取数据与 Astro 类似，只有一些细微的差别。

你需要移除任何副作用钩子（`useEffect`）的实例，然后使用 `Astro.glob()` 或 `getCollection()`（或 `getEntryBySlug()`）从项目源代码的其他文件中获取数据。
...
了解更多有关于 [使用 `Astro.glob()`](/zh-cn/guides/imports/#astroglob)、 [使用集合 API 查询](/zh-cn/guides/content-collections/#querying-collections) 以及 [获取远程数据](/zh-cn/guides/data-fetching/) 的内容。

### 在 Astro 中转换 CRA 样式 \{#cra-styling-to-astro\}
...
如果已安装了 [Tailwind 集成](/zh-cn/guides/integrations-guide/tailwind/)，则可以支持 Tailwind，并且不需要更改现有的 Tailwind 代码！
