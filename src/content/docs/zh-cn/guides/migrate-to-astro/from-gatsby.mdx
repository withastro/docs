...
- [`.astro` 文件的语法类似于 JSX](/zh-cn/core-concepts/astro-syntax/#类-jsx-表达式)。编写 Astro 应该感觉熟悉。
...
- [本地文件数据](/zh-cn/guides/imports/)：Gatsby 使用 GraphQL 从你的项目文件中检索数据。Astro 使用 ESM imports 和顶层 await 函数（例如 [`Astro.glob()`](/zh-cn/guides/imports/#astroglob), [`getCollection()`](/zh-cn/guides/content-collections/#查询集合)）从你的项目文件中导入数据。你可以手动向你的 Astro 项目添加 GraphQL，但默认情况下不包含它。
...
3. 将所有必要的 JavaScript，包括 import 语句，都放入["代码围栏" (`---`)](/zh-cn/core-concepts/astro-components/#组件脚本)中。注意: 在 Astro 中，经常会直接在 HTML 模板内编写[条件渲染内容](/zh-cn/core-concepts/astro-syntax/#动态-html)的 JavaScript。
...
或者，你还可以使用 Astro 中的 [内容集合](/zh-cn/guides/content-collections/) 来存储和管理你的内容。作为集合的一部分时，Markdown 和 MDX 文件将位于 `src/content/` 内的文件夹中。你将需要自己获取内容并[动态生成这些页面](/zh-cn/guides/content-collections/#从内容生成路由)。
...
以下是一些你需要将其转换为 Astro 的 Gatsby 特有语法的示例。在编写 Astro 组件的指南中查看更多 [Astro 和 JSX 之间的区别](/zh-cn/core-concepts/astro-syntax/#astro-和-jsx-间的差异)。
