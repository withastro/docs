...
- 像 Gatsby 一样，Astro 项目可以是 SSG 或者[服务端渲染(SSR)与页面级预渲染](/zh-cn/guides/server-side-rendering/#混合渲染)。
...
1. 将现有的 Gatsby 组件函数的`return()`部分作为你的HTML模板。

2. 将任何[ Gatsby 或 JSX 语法转换为 Astro 语法](/zh-cn/guides/migrate-to-astro/from-gatsby/#参考-转化为-astro-语法)或HTML网络标准。例如 `<Link to="">`， `{children}`和 `className`。

3. 将所有必要的 JavaScript，包括 import 语句，都放入["代码围栏" (`---`)](/zh-cn/core-concepts/astro-components/#the-component-script)中。注意: 在 Astro 中，经常会直接在 HTML 模板内编写[条件渲染内容](/zh-cn/core-concepts/astro-syntax/#dynamic-html)的 JavaScript。

4. 使用[`Astro.props`](/zh-cn/reference/api-reference/#astroprops)来访问之前传递给你的Gatsby函数的任何其他 props。

5. 决定是否需要将导入的组件也转换为 Astro。如果已经安装了官方的 React 集成，你可以在 Astro 文件中[使用现有的React组件](/zh-cn/core-concepts/framework-components/)。但是，如果他们不需要交互，你可能希望将它们转换为`.astro`组件！

6. 删除任何 GraphQ L查询。改为使用 import 和[`Astro.glob()`](/zh-cn/reference/api-reference/#astroglob)语句来查询你的本地文件。

查看[Gatsby's Blog启动模板转换步骤示例](/zh-cn/guides/migrate-to-astro/from-gatsby/#引导示例-gatsby-layout-to-astro)
...
你现有的 Gatsby JSX（`.js`）页面需要被[从 JSX 文件转换为 `.astro` 页面](/zh-cn/guides/migrate-to-astro/from-gatsby/#提示将jsx文件转换为astro文件)。你不能在 Astro 中使用现有的 JSX 页面文件。

这些[`.astro` 页面](/zh-cn/core-concepts/astro-pages/) 必须位于 `src/pages/` 内，并且会基于它们的文件路径自动生成页面路由。

#### Markdown 和 MDX 页面

Astro 对 Markdown 有内建支持，并对 MDX 文件有可选集成。你现有的 [Markdown 和 MDX 文件](/zh-cn/guides/markdown-content/) 可以重用，但可能需要对它们的 frontmatter 进行一些调整，如添加 [Astro 的特殊 `layout` frontmatter 属性](/zh-cn/core-concepts/layouts/#markdownmdx-布局)。它们也可以被放置在 `src/pages/` 中以利用自动的基于文件的路由。
...
传递多个子集的 React 组件可以通过使用 [命名插槽](/zh-cn/core-concepts/astro-components/#命名插槽) 迁移到 Astro 组件。

在 Astro 中查看更多关于[特定 `<slot />` 使用的信息](/zh-cn/core-concepts/astro-components/#插槽).
...
将 Gatsby 的 `<StaticImage />` 和 `<GatsbyImage />` 组件与 [Astro 自己的图像集成组件](/zh-cn/guides/images/#astro-的图片集成) 或适当的标准 HTML `<img>` / JSX `<img />` 标签相互转换。
...
6. 为了测试条件头部，使用相同的模式创建第二个页面 `about.astro`：
