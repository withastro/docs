...
2. 将任何[ Gatsby 或 JSX 语法转换为 Astro 语法](#参考转换为-astro-语法)或HTML网络标准。例如 `<Link to="">`， `{children}`和 `className`。

3. 将所有必要的 JavaScript，包括 import 语句，都放入["代码围栏" (`---`)](/zh-cn/core-concepts/astro-components/#组件脚本)中。注意: 在 Astro 中，经常会直接在 HTML 模板内编写[条件渲染内容](/zh-cn/core-concepts/astro-syntax/#dynamic-html)的 JavaScript。
...
查看[Gatsby's Blog启动模板转换步骤示例](#引导示例gatsby-布局转换为-astro)
...
每个 Astro 页面明确要求 `<html>`、`<head>` 和 `<body>` 标签的存在，所以在页面之间复用布局文件是常见的做法。Astro 使用 [`<slot />`](/zh-cn/core-concepts/astro-components/#插槽) 而不是 React 的 `{children}` prop 来放置页面内容，并且无需导入语句。你的 Gatsby `layout.js` 和模板将不会包含这些。
