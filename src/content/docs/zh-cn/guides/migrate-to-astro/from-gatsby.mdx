...
Astro 对 Markdown 有内置支持，并对 MDX 文件有可选集成。你现有的 [Markdown 和 MDX 文件](/zh-cn/guides/markdown-content/) 可以重用，但可能需要对它们的 frontmatter 进行一些调整，如添加 [Astro 的特殊 `layout` frontmatter 属性](/zh-cn/core-concepts/layouts/#markdownmdx-布局)。它们也可以被放置在 `src/pages/` 中以利用自动的基于文件的路由。
