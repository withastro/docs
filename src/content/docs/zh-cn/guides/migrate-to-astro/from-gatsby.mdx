...
## Gatsby 和 Astro 之间的关键相似性 \{#key-similarities-between-gatsby-and-astro\}

Gatsby 和 Astro 共享一些相似性，这将帮助你迁移你的项目：

- [`.astro` 文件的语法类似于 JSX](/zh-cn/basics/astro-syntax/#jsx-like-expressions)。编写 Astro 应该感觉熟悉。
...
- 像 Gatsby 一样，Astro 项目可以是 SSG 或者[服务端渲染 (SSR) 与页面级预渲染](/zh-cn/guides/server-side-rendering/)。

## Gatsby 和 Astro 之间的关键差异 \{#key-differences-between-gatsby-and-astro\}
...
- [本地文件数据](/zh-cn/guides/imports/)：Gatsby 使用 GraphQL 从你的项目文件中检索数据。Astro 使用 ESM imports 和顶层 await 函数（例如 [`Astro.glob()`](/zh-cn/guides/imports/#astroglob), [`getCollection()`](/zh-cn/guides/content-collections/#querying-collections)）从你的项目文件中导入数据。你可以手动向你的 Astro 项目添加 GraphQL，但默认情况下不包含它。

## 转换你的 Gatsby 项目 \{#convert-your-gatsby-project\}

每个项目迁移都会有所不同，但是在将 Gatsby 转换为 Astro 时，你将执行一些常见操作。

### 创建一个新的 Astro 项目 \{#create-a-new-astro-project\}
使用你的包管理器的 `create astro` 命令启动 Astro 的 CLI 向导，或从 [Astro 主题展示](https://astro.build/themes)中选择一个社区主题。

你可以向 `create astro` 命令传递一个 `--template` 参数，以使用我们的官方起始器（例如 `docs`，`blog`，`portfolio`）启动一个新的 Astro 项目。或者，你可以[从 GitHub 上的任何现有 Astro 存储库开始一个新项目](/zh-cn/install-and-setup/#use-a-theme-or-starter-template)。
...
### 安装集成（可选） \{#install-integrations-optional\}
...
### 将你的代码放在 `src` 中 \{#put-your-code-in-src\}
...
### 提示：将 JSX 文件转换为 `.astro` 文件 \{#tips-convert-jsx-files-to-astro-files\}

以下是将 Gatsby `.js` 组件转换为 `.astro` 组件的一些提示：

1. 将现有的 Gatsby 组件函数的 `return()` 部分作为你的 HTML 模板。

2. 将任何 [Gatsby 或 JSX 语法转换为 Astro 语法](#reference-convert-to-astro-syntax)或 HTML web 标准。例如 `<Link to="">`，`{children}` 和 `className`。

3. 将所有必要的 JavaScript，包括 import 语句，都放入["代码围栏" (`---`)](/zh-cn/basics/astro-components/#the-component-script)中。注意：在 Astro 中，经常会直接在 HTML 模板内编写[条件渲染内容](/zh-cn/basics/astro-syntax/#dynamic-html)的 JavaScript。

4. 使用[`Astro.props`](/zh-cn/reference/api-reference/#astroprops)来访问之前传递给你的 Gatsby 函数的任何其他 props。
...
6. 删除任何 GraphQL 查询。改为使用 import 和 [`Astro.glob()`](/zh-cn/reference/api-reference/#astroglob) 语句来查询你的本地文件。

查看 [Gatsby's Blog 启动模板转换步骤示例](#guided-example-gatsby-layout-to-astro)

#### 对比：`.jsx` vs `.astro` \{#compare-jsx-vs-astro\}
...
### 迁移布局文件 \{#migrating-layout-files\}

首先，你可能会发现将你的 Gatsby 布局和模板转换为 [Astro 布局组件](/zh-cn/basics/layouts/)会很有帮助。

每个 Astro 页面明确要求 `<html>`、`<head>` 和 `<body>` 标签的存在，所以在页面之间复用布局文件是常见的做法。Astro 使用 [`<slot />`](/zh-cn/basics/astro-components/#slots) 而不是 React 的 `{children}` prop 来放置页面内容，并且无需导入语句。你的 Gatsby `layout.js` 和模板将不会包含这些。
...
### 迁移页面和文章 \{#migrating-pages-and-posts\}

在 Gatsby 中，你的[页面和文章](/zh-cn/basics/astro-pages/)可能存在于 `src/pages/` 或者在 `src` 外部的其他文件夹，如 `content`。而在 Astro 中，**所有的页面内容必须位于 `src/` 内**。

#### React 页面 \{#react-pages\}

你现有的 Gatsby JSX（`.js`）页面需要被[从 JSX 文件转换为 `.astro` 页面](/zh-cn/guides/migrate-to-astro/from-gatsby/#tips-convert-jsx-files-to-astro-files)。你不能在 Astro 中使用现有的 JSX 页面文件。

这些[`.astro` 页面](/zh-cn/basics/astro-pages/) 必须位于 `src/pages/` 内，并且会基于它们的文件路径自动生成页面路由。

#### Markdown 和 MDX 页面 \{#markdown-and-mdx-pages\}
...
### 重新利用配置文件 \{#repurpose-config-files\}
...
- `gatsby-browser.js`：考虑将这里使用的任何东西直接添加到你的[主布局](/zh-cn/guides/migrate-to-astro/from-gatsby/#migrating-layout-files)的 `<head>` 标签中。
...
## 参考：转换为 Astro 语法 \{#reference-convert-to-astro-syntax\}

以下是一些你需要将其转换为 Astro 的 Gatsby 特有语法的示例。在编写 Astro 组件的指南中查看更多 [Astro 和 JSX 之间的区别](/zh-cn/basics/astro-syntax/#differences-between-astro-and-jsx)。

### Gatsby 链接到 Astro \{#gatsby-links-to-astro\}
...
### Gatsby 导入到 Astro \{#gatsby-imports-to-astro\}
...
### Gatsby 子属性到 Astro \{#gatsby-children-props-to-astro\}
...
传递多个子集的 React 组件可以通过使用 [命名插槽](/zh-cn/basics/astro-components/#named-slots) 迁移到 Astro 组件。

在 Astro 中查看更多关于[特定 `<slot />` 使用的信息](/zh-cn/basics/astro-components/#slots).

### Gatsby 样式转换为 Astro \{#gatsby-styling-to-astro\}
...
### Gatsby 图像插件转换为 Astro \{#gatsby-image-plugin-to-astro\}

根据情况在你的 React 组件中将 Gatsby 的 `<StaticImage />` 和 `<GatsbyImage />` 组件转换为 [Astro 自己的图像集成组件](/zh-cn/guides/images/#image--astroassets) 或者转化成 [标准的 HTML `<img>` / JSX `<img />` 标签](/zh-cn/guides/images/#images-in-ui-framework-components)。
...
Astro 的 `<Image />` 组件只能在 `.astro` 和 `.mdx` 文件中工作。查看该组件可用的[完整组件属性列表](/zh-cn/guides/images/#properties)，并注意它与 Gatsby 的属性有一些不同。

要继续使用标准 Markdown 语法 (`![]()`) [在 Markdown (`.md`) 文件中使用图像](/zh-cn/guides/images/#images-in-markdown-files)，你可能需要更新链接。对于本地图像，不支持在 `.md` 文件中直接使用 HTML `<img>` 标签，必须转换为 Markdown 语法。
...
### Gatsby GraphQL 转换为 Astro \{#gatsby-graphql-to-astro\}
...
## 引导示例：Gatsby 布局转换为 Astro \{#guided-example-gatsby-layout-to-astro\}
...
2. 创建 `Layout.astro` 并添加此 `return` 值，[转换为 Astro 语法](/zh-cn/guides/migrate-to-astro/from-gatsby/#reference-convert-to-astro-syntax)。
