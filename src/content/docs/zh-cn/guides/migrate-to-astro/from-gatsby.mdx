...
- [本地文件数据](/zh-cn/guides/imports/)：Gatsby 使用 GraphQL 从你的项目文件中检索数据。Astro 使用 ESM imports 和顶层 await 函数（例如 [`import.meta.glob()`](/zh-cn/guides/imports/#importmetaglob), [`getCollection()`](/zh-cn/guides/content-collections/#查询构建时集合)）从你的项目文件中导入数据。你可以手动向你的 Astro 项目添加 GraphQL，但默认情况下不包含它。
