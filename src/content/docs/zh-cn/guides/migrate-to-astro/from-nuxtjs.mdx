...
## Nuxt 和 Astro 的关键相似之处 \{#key-similarities-between-nuxt-and-astro\}

Nuxt 和 Astro 有一些相似之处，这些相似性能够帮助你完成迁移：

- Astro 项目可以是 SSG（静态网站生成）或 [支持页面级预渲染的 SSR（服务端渲染）](/zh-cn/guides/server-side-rendering/)。
- Astro 使用基于文件的路由，并且 [允许特定命名的页面创建动态路由](/zh-cn/guides/routing/#dynamic-routes)。
- Astro 是 [基于组件的](/zh-cn/basics/astro-components/)，因此在迁移前后，你的标记结构将保持相似。
- Astro [官方支持使用 Vue 组件](/zh-cn/guides/integrations-guide/vue/)。
- Astro 支持 [安装 NPM 包](/zh-cn/guides/imports/#npm-packages)，包括 Vue 库。你能够保留现有的部分或全部 Vue 组件和依赖项。

## Nuxt 和 Astro 的关键区别 \{#key-differences-between-nuxt-and-astro\}
...
- [内容驱动](/zh-cn/concepts/why-astro/#content-driven)：Astro 旨在展示你的内容，并允许你仅在需要时选择加入交互性。Nuxt 应用更多是针对客户端的高交互性构建的。Astro 内置有能够处理内容的功能，例如页面生成，但是一些使用 `.astro` 组件难以复制、更具挑战性的功能，可能需要高级 Astro 技术来处理，例如仪表盘。

## 转换你的 NuxtJS 项目 \{#convert-your-nuxtjs-project\}

每个项目在迁移时的步骤不尽相同，但在从 Nuxt 转换到 Astro 时，你会执行一些常见的操作。

### 新建一个 Astro 项目 \{#create-a-new-astro-project\}
使用你的包管理器的 `create astro` 来启动 Astro 的 CLI 向导，或者从 [Astro 主题展示](https://astro.build/themes) 中选择一个社区主题。

你可以将 `--template` 参数传给 `create astro` 命令，使用我们的官方入门模板之一启动新的 Astro 项目 (例如 `docs`, `blog`, `portfolio`)。或者，你可以从 [GitHub 上的任何现有的 Astro 库开始一个新项目](/zh-cn/install-and-setup/#use-a-theme-or-starter-template) 。
...
:::tip
访问 https://astro.new 以获取官方启动模板的完整列表，以及能在 StackBlitz、CodeSandbox 或 Gitpod 中打开新项目的链接。
:::
...
### 将你的源码放入 `src` 中 \{#put-your-source-code-in-src\}
...
### 将 Vue SFC 页面转换为 `.astro` 文件 \{#convert-vue-sfc-pages-to-astro-files\}

这里有一些将 Nuxt 的 `.vue` 组件转换为 `.astro` 组件的建议：

1. 使用现有 NuxtJS 组件的 `<template>` 作为你的 HTML 模板的基础。

2. 将任意的 [Nuxt 或 Vue 语法更改为 Astro](#reference-convert-nuxtjs-syntax-to-astro) 或转换为 HTML Web 标准。包括`<NuxtLink>`，`:class`，`{{variable}}`和 `v-if`等。
...
#### 比较：Vue 和 Astro \{#compare-vue-vs-astro\}
...
在 NuxtJS 中，你的 [页面](/zh-cn/basics/astro-pages/) 位于 `/pages`。在 Astro 中，**页面的所有内容必须位于 `src/pages` 或 `src/content`** 中。
...
##### 动态文件路径命名 \{#dynamic-file-path-naming\}
...
## 参考：将 NuxtJS 语法转换到 Astro \{#reference-convert-nuxtjs-syntax-to-astro\}

### Nuxt 本地变量转换到 Astro \{#nuxt-local-variables-to-astro\}
...
### Nuxt 属性传递转换到 Astro \{#nuxt-property-passing-to-astro\}
...
### Nuxt 的链接转换到 Astro \{#nuxt-links-to-astro\}
...
### Nuxt 导入转换到 Astro \{#nuxt-imports-to-astro\}

如有必要，请更新任何 [文件导入](/zh-cn/guides/imports/) 确保引用的是精确的相对文件路径。这可以通过 [导入别名](/zh-cn/guides/typescript/#import-aliases) 或通过完整编写相对路径来完成。
...
### Nuxt 动态页面生成转换到 Astro \{#nuxt-dynamic-page-generation-to-astro\}
...
- 在 Astro 页面的前言中导出 `getStaticPaths()` 函数，以告诉框架 [需要动态生成的静态路由](/zh-cn/guides/routing/#dynamic-routes)。

#### 将 Nuxt 中的 `generate` 函数转换为 Astro 中的 `getStaticPaths` 函数。 \{#convert-a-generate-function-in-nuxt-to-a-getstaticpaths-function-in-astro\}
...
### Nuxt 数据获取转换到 Astro \{#nuxt-data-fetching-to-astro\}
...
### Nuxt 样式转换到 Astro \{#nuxt-styling-to-astro\}
...
#### 预处理器支持 \{#pre-processor-support\}

将它们作为开发依赖项安装，[Astro 支持最流行的 CSS 预处理器](/zh-cn/guides/styling/#css-preprocessors) 。例如，要使用SCSS：
...
### Nuxt 图片插件迁移到 Astro \{#nuxt-image-plugin-to-astro\}

根据情况在你的 Vue 组件中将任何 [Nuxt 的`<nuxt-img/>` 或者 `<nuxt-picture/>` 组件](https://image.nuxtjs.org/components/nuxt-img) 转换为 [Astro 自己的图像集成组件](/zh-cn/guides/images/#image--astroassets) 或者转化成 [标准的 HTML `<img>` 标签](/zh-cn/guides/images/#images-in-ui-framework-components) 或者 `<picture>` 标签。

Astro 的 `<Image />` 组件只能在 `.astro` 和 `.mdx` 文件中工作。查看该组件可用的[完整组件属性列表](/zh-cn/guides/images/#properties)，并注意它与 Nuxt 的属性有一些不同。
...
## 指导示例：请查看以下步骤： \{#guided-example-see-the-steps\}
