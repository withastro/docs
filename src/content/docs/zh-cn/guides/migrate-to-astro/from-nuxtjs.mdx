...
根据情况在你的 Vue 组件中将任何 [Nuxt 的`<nuxt-img/>` 或者 `<nuxt-picture/>` 组件](https://image.nuxtjs.org/components/nuxt-img) 转换为 [Astro 自己的图像集成组件](/zh-cn/guides/images/) 或者转化成 [标准的 HTML `<img>` 标签](/zh-cn/guides/images/#框架组件中的图像) 或者 `<picture>` 标签。
