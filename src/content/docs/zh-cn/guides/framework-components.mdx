...
## 官方的 UI 框架集成 \{#official-ui-framework-integrations\}
...
## 使用框架组件 \{#using-framework-components\}
...
## 激活组件 \{#hydrating-interactive-components\}

框架组件可以使用 [`client:*` 指令](/zh-cn/reference/directives-reference/#client-directives)来激活（hydrate）。这些是用于确定何时将组件的 JavaScript 发送到浏览器的组件属性。
...
### 可用激活指令 \{#available-hydration-directives\}

这里有几个适用于 UI 框架组件的激活指令：`client:load`、`client:idle`、`client:visible`、`client:media={QUERY}` 和 `client:only={FRAMEWORK}`。

<ReadMore>查看[指令参考](/zh-cn/reference/directives-reference/#client-directives)页面获取这些激活指令的详细描述以及用法。</ReadMore>

## 混合框架 \{#mixing-frameworks\}
...
## 向框架组件传递 Props \{#passing-props-to-framework-components\}
...
## 向框架组件传递子组件 \{#passing-children-to-framework-components\}
...
另外你可以使用[命名插槽](/zh-cn/basics/astro-components/#named-slots)来区分特定的子组件。
...
## 嵌套框架组件 \{#nesting-framework-components\}
...
## 我可以在我的框架组件中使用 Astro 组件吗？ \{#can-i-use-astro-components-inside-my-framework-components\}

任何 UI 框架组件都会成为该框架的一个“孤岛”。这些组件必须完全作为该框架的有效代码来编写，只使用它自己的导入和包。你不能在一个 UI 框架组件（如 `.jsx` 或 `.svelte`）中导入 `.astro` 组件。

不过你可以在`.astro`组件内使用 [Astro `<slot />` 模式](/zh-cn/basics/astro-components/#slots)将 Astro 组件生成的静态内容作为子项传递给框架组件。
...
## 我可以激活 Astro 组件吗？ \{#can-i-hydrate-astro-components\}
