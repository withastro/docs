...
[特定的属性可以通过 `Astro.props` 传递给布局组件](/zh-cn/core-concepts/layouts/#markdown-layout-props)。例如，你可以通过 `Astro.props.frontmatter` 访问 frontmatter 属性：
...
📚 阅读更多关于 [Markdown 布局](/zh-cn/core-concepts/layouts/#markdown-布局)。
...
我可以使用 URL `https://my-domain.com/page-1/#介绍` 直接导航到页面上的介绍。
...
它还为标准 MDX 添加了额外的功能，包括对 [MDX 中的 Markdown 样式 frontmatter](https://mdxjs.com/guides/frontmatter/)的支持。 这允许你使用 Astro 的大多数内置 Markdown 功能，例如 [frontmatter `layout`](#frontmatter-layout) 属性和 [草稿页面](#draft-pages) 的设置。
...
:::note[使用 Astro 布局？]
请参阅 [导出到 Astro 布局组件的属性](/zh-cn/core-concepts/layouts/#markdown-layout-props)，了解在使用 Astro 的特殊 [frontmatter 布局](#frontmatter-layout) 时导出的属性。
:::
...
- **（Markdown only）`rawContent()`** - 一个函数，返回原始 Markdown 文档作为字符串。
- **（Markdown only）`compiledContent()`** - 一个函数，返回将 Markdown 文档编译为 HTML 字符串。 请注意，这不包括 frontmatter 中配置的布局！ 只会将 Markdown 文档本身作为 HTML 返回。
- **（MDX only）** - MDX 文件还可以使用 `export` 语句导出数据。
...
相反，你可以使用[动态路由](/zh-cn/core-concepts/routing/#动态路由)动态生成页面。

要访问 Markdown 内容，请将 `<Content/>` 组件传递给 Astro 页面的 `props`。 然后，您可以从 `Astro.props` 中检索该组件并在页面模板中渲染它。
...
1. 从插件的文件参数中将 `customProperty`  附加到 `data.astro.frontmatter` 属性：
...
2. 将此插件应用于您的 `markdown` 或 `mdx` 集成配置：
...
你可以使用 [remark 插件](https://github.com/remarkjs/remark) 来为你的 frontmatter 添加阅读时间。 我们推荐两个辅助包：

- [`reading-time`](https://www.npmjs.com/package/reading-time) 来计算阅读时间
- [`mdast-util-to-string`](https://www.npmjs.com/package/mdast-util-to-string) 来从你的 markdown 中提取所有文本

```shell
npm install reading-time mdast-util-to-string
```

我们可以将这些包应用于 remark 插件，如下所示：
...
export function remarkReadingTime() {
  return function (tree, { data }) {
    ...
    // readingTime.text 将给我们阅读时间的友好字符串，
    // i.e. "3 min read"
...
...所有的 Markdown 文档都会有一个计算的 `minutesRead`。 例如，你可以使用它在 [markdown 布局](#markdown-和-mdx-页面) 中包含一个 “X min read” 横幅：
...
要避免从 MDX 扩展你的 Markdown 配置，请将 [`extendMarkdownConfig` 选项](/zh-cn/guides/integrations-guide/mdx/#extendmarkdownconfig) (enabled by default) to `false`:
...
Astro 主要设计用于可以保存在项目目录中的本地 Markdown 文件。 但是，在某些情况下，您可能需要从远程源获取 Markdown。 例如，您可能需要在构建网站时（或在使用 [SSR](/zh-cn/guides/server-side-rendering/) 时，用户向网站发出请求时）从远程 API 获取并呈现 Markdown。

**Astro 不包含对远程 Markdown 的内置支持！** 要获取远程 Markdown 并将其呈现为 HTML，您需要从 npm 安装并配置自己的 Markdown 解析器。 这将不会从您配置的 Astro 的内置 Markdown 和 MDX 设置中继承。 在将其添加到您的项目之前，请确保您了解这些限制。
