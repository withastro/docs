...
你的 Markdown 内容和 frontmatter 属性都可以通过 [本地文件导入](#导入-markdown) 来在组件中加以使用，或者当 [使用内容集合助手函数来查询和渲染数据请求](#markdown-from-content-collections-queries) 时也能使用。
...
集合使用专注于内容的优化 API 来 [查询和渲染你的 Markdown 内容](#markdown-from-content-collections-queries)，这种方式代替了文件导入。集合适用于共享相同结构的数据集，例如博客文章或产品项。当你在 schema 中定义集合时，还可以在编辑器中获得验证、类型安全和智能提示。
