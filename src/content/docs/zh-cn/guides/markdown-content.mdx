...
<ReadMore>阅读更多关于 Astro [基于文件的路由](/zh-cn/guides/routing/)或创建 [动态路由](/zh-cn/guides/routing/#dynamic-routes) 的选项。</ReadMore>

## Markdown 功能 \{#markdown-features\}

Astro 在使用 Markdown 和 MDX 文件时提供了一些额外的内置 Markdown 功能。

### Frontmatter `layout` \{#frontmatter-layout\}

Astro 为位于 `src/pages/` 中的 [Markdown 和 MDX 页面](/zh-cn/basics/astro-pages/#markdownmdx-页面) 提供了一个特殊的 frontmatter `layout` 属性，可以指定一个 Astro [布局组件](/zh-cn/basics/layouts/#markdown-布局) 的相对路径（或 [别名](/zh-cn/guides/imports/#aliases)）。
...
你也可以在布局组件中[为你的 Markdown 设置样式](/zh-cn/guides/styling/#markdown-styling)。
...
### 在 MDX 中使用导出的变量 \{#using-exported-variables-in-mdx\}

MDX 支持使用 `export` 语句将变量添加到你的 MDX 内容中。这些变量既可以在模板本身中访问，也可以在[导入文件](#importing-markdown)时以命名属性的形式访问。
...
# {title}
```
### 在 MDX 中使用 Frontmatter 变量 \{#using-frontmatter-variables-in-mdx\}

Astro MDX 集成默认支持在 MDX 中使用 frontmatter。在 Markdown 文件中添加 frontmatter 属性，这些变量可在模板中、其 [`layout` 组件](#frontmatter-layout)中以及在[导入文件](#importing-markdown)时以命名属性的形式访问。
...
### 在 MDX 中使用组件 \{#using-components-in-mdx\}

安装 MDX 集成后，你可以在 MDX (`.mdx`) 文件中导入并使用 [Astro 组件](/zh-cn/basics/astro-components/#component-props)和 [UI 框架组件](/zh-cn/guides/framework-components/#using-framework-components)。就像在任何其他 Astro 组件中使用它们一样。
...
#### 将自定义组件分配给 HTML 元素 \{#assigning-custom-components-to-html-elements\}
...
当你在 Astro 组件中导入 Markdown 和 MDX 文件时，你会得到一个包含它们 [导出属性](#exported-properties) 的对象。
...
### `Content` 组件 \{#the-content-component\}
...
#### 举例：动态页面路由 \{#example-dynamic-page-routing\}

与其将 Markdown/MDX 文件放在 `src/pages/` 目录中来创建页面路由，不如[动态路由](/zh-cn/guides/routing/#dynamic-routes)。
...
### 使用导入的 MDX 的自定义组件 \{#custom-components-with-imported-mdx\}

当渲染导入的 MDX 内容时，可以通过 `components` 属性传递 [自定义组件](#assigning-custom-components-to-html-elements) 。
...
## 配置 Markdown \{#configuring-markdown-and-mdx\}
...
你可以在 `astro.config.mjs` 中自定义 remark 解析 Markdown 的方式。请参阅完整的 [Markdown 配置选项](/zh-cn/reference/configuration-reference/#markdown-options)列表。

### Markdown 插件 \{#markdown-plugins\}

Astro 支持添加第三方 [remark](https://github.com/remarkjs/remark) 和 [rehype](https://github.com/rehypejs/rehype) 插件来解析 Markdown 和 MDX。这些插件允许你扩展你的 Markdown，以获得新的功能，例如 [自动生成目录](https://github.com/remarkjs/remark-toc)，[应用可访问的表情符号标签](https://github.com/florianeckerstorfer/remark-a11y-emoji)，以及[为你的 Markdown 添加样式](/zh-cn/guides/styling/#markdown-styling)。
...
#### 标题 ID 和插件 \{#heading-ids-and-plugins\}

Astro 将 `id` 属性注入到 Markdown 和 MDX 文件中的所有标题元素（`<h1>` 到 `<h6>`），并提供 `getHeadings()` 实用程序，用于在 [Markdown 导出的属性](#exported-properties) 中检索这些 ID。
...
### 以编程方式修改 frontmatter \{#modifying-frontmatter-programmatically\}

:::note
如果你使用的是 [内容集合](/zh-cn/guides/content-collections/)，请参阅[“使用 Remark 修改 Frontmatter”](/zh-cn/guides/content-collections/#modifying-frontmatter-with-remark)。
:::

你可以通过使用 [remark 或 rehype 插件](#markdown-plugins) 为所有 Markdown 和 MDX 文件添加 frontmatter 属性。
...
现在，每个 Markdown 或 MDX 文件都会在其 frontmatter 中有 `customProperty`，使其在 [导入你的 markdown](#importing-markdown) 时以及从 [布局中的 `Astro.props.frontmatter` 属性](#frontmatter-layout) 中可用。

<RecipeLinks slugs={["zh-cn/recipes/reading-time"]} />

### 从 MDX 扩展 Markdown 配置 \{#extending-markdown-config-from-mdx\}

Astro 的 MDX 集成默认情况下会扩展[你的项目的现有 Markdown 配置](/zh-cn/reference/configuration-reference/#markdown-options)。要覆盖单个选项，你可以在 MDX 配置中指定它们的等效项。
...
#### 添加你自己的主题 \{#adding-your-own-theme\}
...
#### 改变默认语法高亮模式 \{#change-default-syntax-highlighting-mode\}
...
## 请求远程 Markdown \{#fetching-remote-markdown\}
