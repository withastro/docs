...
Astro 为位于 `src/pages/` 中的 [Markdown 和 MDX 页面](/zh-cn/basics/astro-pages/#markdownmdx-页面) 提供了一个特殊的 frontmatter `layout` 属性，该属性可以指定一个相对路径（或 [别名](/zh-cn/guides/aliases/)）到一个 Astro [布局组件](/zh-cn/basics/layouts/#markdownmdx-布局)。
