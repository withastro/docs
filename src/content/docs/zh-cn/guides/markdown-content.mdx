...
- [内置的 `<Code />` 组件](/zh-cn/reference/components-reference/#code-) 中的内容（由 Shiki 提供支持）。
- [`<Prism />` 组件](/zh-cn/reference/components-reference/#prism-) 中的内容（由 Prism 提供支持）。
...
3. 通过 `<link>` 标签在 [布局组件](/zh-cn/basics/layouts/) 中的页面的 `<head>` 中加载此样式表。（参见 [Prism 基本用法](https://prismjs.com/#basic-usage)。）
</Steps>
