...
<ReadMore>阅读更多关于 Astro [基于文件的路由](/zh-cn/basics/routing/)或创建 [动态路由](/zh-cn/basics/routing/#动态路由) 的选项。</ReadMore>
...
安装 MDX 集成后，你可以在 MDX (`.mdx`) 文件中导入并使用 [Astro 组件](/zh-cn/basics/astro-components/#组件参数)和 [UI 框架组件](/zh-cn/basics/framework-components/#使用框架组件)。就像在任何其他 Astro 组件中使用它们一样。
...
与其将 Markdown/MDX 文件放在 `src/pages/` 目录中来创建页面路由，不如[动态路由](/zh-cn/basics/routing/#动态路由)。
