...
## 预获取配置 \{#prefetch-configuration\}

`prefetch` 配置也接受一个选项对象，以进一步自定义预获取。

### 预获取策略 \{#prefetch-strategies\}
...
### 默认预获取策略 \{#default-prefetch-strategy\}
...
### 默认预获取所有链接 \{#prefetch-all-links-by-default\}
...
所有链接的默认预获取策略可以通过 `prefetch.defaultStrategy` 更改，如 [默认预获取策略部分](#default-prefetch-strategy) 所示。

## 可编程的预获取 \{#prefetch-programmatically\}
...
## 与视图过渡动画一起使用 \{#using-with-view-transitions\}

当你在页面上使用 [视图过渡动画](/zh-cn/guides/view-transitions/) 时，预获取也将默认启用。它设置了一个默认配置 `{ prefetchAll: true }`，在页面上启用 [所有链接的预获取](#prefetch-all-links-by-default)。
...
## 从 `@astrojs/prefetch` 迁移 \{#migrating-from-astrojsprefetch\}
