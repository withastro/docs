...
### 用 `class:list` 组合类名 \{#combining-classes-with-classlist\}
...
### 传递 `class` 给子组件 \{#passing-a-class-to-a-child-component\}
...
### 导入本地样式表 \{#import-a-local-stylesheet\}

:::caution[使用 npm 包？]
想要导入 npm 包，你可能需要更新 `astro.config` 文件。参见下面的[“从 npm 包中导入样式表”部分](#import-a-stylesheet-from-an-npm-package)。
:::

你可以在 Astro 组件中使用 ESM 导入语法显式导入样式表。CSS 导入方式与 [Astro 组件中的其他 ESM 导入](/zh-cn/basics/astro-components/#the-component-script)一样，它应该**基于组件**进行引用，并且与其他导入一样必须位于组件脚本**顶层**：
...
### 从 npm 包中导入样式表 \{#import-a-stylesheet-from-an-npm-package\}
...
假设你从 `package-name` 导入名为 `normalize` 的 CSS 文件（省略文件扩展名）。为了确保我们能正确地预渲染你的页面，需要把`package-name` 添加到 [`vite.ssr.noExternal` 数组](https://vitejs.dev/config/ssr-options.html#ssr-noexternal)中。
...
:::note
这是[针对 Vite 的设置](https://cn.vitejs.dev/config/#ssr-noexternal) 与 [Astro SSR](/zh-cn/guides/server-side-rendering/) 无关。
:::
...
### 通过 `link` 标签加载静态样式表 \{#load-a-static-stylesheet-via-link-tags\}
...
因为这种方法使用 `public/` 目录，它跳过了 Astro 提供的 CSS 处理、捆绑和压缩。如果你需要这些转换，请使用上面的[导入样式表](#import-a-local-stylesheet)方法。
...
### 作用域样式 \{#scoped-styles-1\}

使用 [作用域样式](#scoped-styles) 不会增加 CSS 的 _特异性_，但它们始终在 _出现顺序_ 中最后。因此，它们将优先于具有相同特异性的其他样式。例如，如果你导入了与 scoped 样式冲突的样式表，scoped 样式的值将被应用：
...
如果你导入的样式更具 _特异性_，那么它将优先于 scoped 样式：
...
### Link 标签 \{#link-tags\}
通过 [link 标签](#load-a-static-stylesheet-via-link-tags) 加载的样式表按顺序评估，优先于 Astro 文件中的任何其他样式。因此，这些样式将具有低于导入样式表和作用域样式的优先级：
...
## CSS 集成 \{#css-integrations\}

Astro 支持在项目中添加流行的 CSS 库、工具和框架，如 [Tailwind](https://tailwindcss.com) 和其他库！

### Tailwind \{#tailwind\}

要在项目中使用 Tailwind，请使用包管理器的`astro add` 命令安装官方的 [Astro Tailwind 集成][tailwind]：
...
### 在框架组件中 \{#in-framework-components\}
...
Astro 内置了 PostCSS，作为 [Vite](https://cn.vitejs.dev/guide/features.html#postcss) 的一部分。要为你的项目配置 PostCSS，在项目根部创建 `postcss.config.cjs` 文件。然后你就可以在安装插件后使用 `require()` 导入插件（例如 `npm install autoprefixer`）。
...
## Markdown 样式 \{#markdown-styling\}

任何 Astro 样式方法都可以用于 [Markdown 布局组件](/zh-cn/basics/layouts/#markdown-布局)。但是不同的方式可能会对你的页面产生不同的样式效果。

你可以通过向包装页面内容的布局中添加 [导入的样式表](#external-styles) 来将全局样式应用于 Markdown 内容。你还可以在布局组件中使用 [`<style is:global>` 标签](#global-styles) 来为你的 Markdown 格式设置样式。请注意，添加的任何样式都会受到 [Astro 的级联顺序](#cascading-order)的影响，因此你应该仔细检查渲染后的页面，确保你的样式能够按照预期生效。

你也可以添加包括 [Tailwind](/zh-cn/guides/integrations-guide/tailwind/) 在内的 [CSS 集成](#css-integrations)。如果你使用 Tailwind，[typography 插件](https://tailwindcss.com/docs/typography-plugin) 可以用于样式化 Markdown。
...
完整的细节见 [Vite 的文档](https://cn.vitejs.dev/guide/assets.html#importing-asset-as-string)。

### 导入 `?url` CSS \{#url-css-imports\}
...
:::caution
用 `?url` 导入较小的 CSS 文件，可能会在最终构建时将 CSS 文件的 base64 编码内容作为数据链接返回值。你可以编写代码以支持经编码的数据链接（`data:text/css;base64,...`），或者将 [`vite.build.assetsInlineLimit`](https://cn.vitejs.dev/config/#build-assetsinlinelimit) 配置选项设为 `0` 以禁用这一功能。
:::
