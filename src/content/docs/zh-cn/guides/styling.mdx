...
假设你从 `package-name` 导入名为 `normalize` 的 CSS 文件（省略文件扩展名）。为了确保我们能正确地预渲染你的页面，需要把`package-name` 添加到 [`vite.ssr.noExternal` 数组](https://vite.dev/config/ssr-options.html#ssr-noexternal)中。
...
:::note
这是[针对 Vite 的设置](https://cn.vite.dev/config/#ssr-noexternal) 与 [Astro SSR](/zh-cn/guides/server-side-rendering/) 无关。
:::
...
完整的细节见 [Vite 文档](https://vite.dev/guide/assets.html#importing-asset-as-url)。
