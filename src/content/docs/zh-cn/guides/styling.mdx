...
Astro 同时支持 Tailwind 3 和 4。你可以通过 CLI 命令 [添加 Vite 插件](#add-tailwind-4) 到你的项目以添加 Tailwind 4 支持，或者手动安装旧版依赖项来添加 [Tailwind 3 支持](#旧版-Tailwind-3-支持)。

如果你要将你现有的 Astro 项目的 [Tailwind 3 升级到 4](#从-Tailwind-3-升级到-4)，你需要在添加 Tailwind 4 相关依赖的同时，删除旧版 Tailwind 3 相关依赖
