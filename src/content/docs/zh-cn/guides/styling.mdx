...
Astro 内置了 PostCSS，作为 [Vite](https://vitejs.dev/guide/features.html#postcss) 的一部分。要为你的项目配置 PostCSS，在项目根部创建 `postcss.config.js` 文件。然后你就可以在安装插件后使用 `require()` 导入插件（例如 `npm install autoprefixer`）。
...
完整的细节见[Vite的文档](https://vitejs.dev/guide/assets.html#importing-asset-as-string)。
