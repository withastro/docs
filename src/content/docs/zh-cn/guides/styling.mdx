...
你可以使用 CLI 命令向你的项目添加对 Tailwind 4 的支持，或是通过手动安装旧版依赖，以 [支持旧版 Tailwind 3](#旧版-tailwind-3-支持) 。

在 Astro `>=5.2.0` 版本中，通过在包管理工具中使用 `astro add tailwind` 命令，以安装官方的 Vite Tailwind 插件。该插件提供对 Tailwind `>=4.0.0` 的支持。如果要为更早版本的 Astro 提供对于 Tailwind 4 的支持，可 [参照 Tailwind 文档中的教程][tailwind]，以手动添加该 Vite 插件。
...
由于 Tailwind 4 开始，Astro 官方不再使用 `@astrojs/tailwind` 集成，因此当你从 tailwind 3 升级到 4 后，你将不再需要 `@astrojs/tailwind` 集成

<Steps>
1. 通过 [tailwind](#tailwind) 章节安装 Tailwind 4

2. 卸载项目原有的 `@astrojs/tailwind` 集成：
...
3. 从 `astro.config.mjs` 中删除 `@astrojs/tailwind` 集成：
...
4. 然后，根据 Tailwind 的 [升级指南](https://tailwindcss.com/docs/upgrade-guide#changes-from-v3) 对你的项目进行升级。
</Steps>
...
你也可以添加包括 [Tailwind](/zh-cn/recipes/tailwind-rendered-markdown/) 在内的 [CSS 集成](#css-集成)。如果你使用 Tailwind，[typography 插件](https://tailwindcss.com/docs/typography-plugin) 可以用于样式化 Markdown。
