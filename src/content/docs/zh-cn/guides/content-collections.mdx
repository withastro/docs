...
通过 [社区构建的加载器](https://astro.build/integrations/?search=&categories%5B%5D=loaders)，或者自己构建 [自定义的构建时集合加载器](#自定义构建时加载器) 或 [实时加载器](#创建一个实时加载器)，你可以从任何外部源（如 CMS、数据库或无头支付系统）获取远程数据，既可以在构建时获取，也可以按需实时获取。
...
:::tip[快速开始]
查看 [官方 Astro 博客启动模板](https://github.com/withastro/astro/tree/latest/examples/blog)，可以快速上手，其中包含使用 [内置 glob() 加载器](#glob-加载器) 和为本地 Markdown 或 MDX 博客文章集合 [定义 schema](#定义几集合-schema) 的示例。
:::
...
- [一个构建时 `schema`](#定义集合-schema) 用于类型安全（可选，但是强烈推荐！）
...
你可以使用内容加载器 API [构建自定义加载器](/zh-cn/reference/content-loader-reference/#building-a-loader)，从任何数据源（如CMS、数据库或 API 端点）获取远程内容。
...
与创建 Astro 集成或 Vite 插件类似，你可以将你的加载器 [作为 npm 包分发](/zh-cn/reference/integrations/)，供他人在他们的项目中使用。
...
你可以选择 [在实时加载器中包含类型安全](/zh-cn/reference/content-loader-reference/#the-liveloader-object)。因此，为实时集合[定义 Zod 模式](#在实时集合中使用-zod-模式) 是可选的。但是，如果你提供了的话，它将优先于实时加载器的类型。
...
你可以使用 Live Loader API 构建自定义 [实时加载器](/zh-cn/reference/content-loader-reference/#live-loaders)，以便从任何数据源（如CMS、数据库或 API 端点）按需获取最新的远程内容。你需要告诉你的实时加载器如何从所需的数据源获取和返回内容条目，以及为不成功的数据请求提供错误处理。
...
<ReadMore>查看使用 Live Loader API [构建实时加载器](/zh-cn/reference/content-loader-reference/#building-a-live-loader) 的基础知识</ReadMore>

### 在实时集合中使用 Zod 模式

你可以将 Zod 模式与实时集合一起使用，以在运行时验证和转换数据。这种 Zod 验证的工作方式与 [构建时集合的模式](#定义集合模式schema) 相同。

当你为实时集合定义一个模式时，在查询该集合时，它优先于 [实时加载器的类型](/zh-cn/reference/content-loader-reference/#the-liveloader-object)：
...
你还可以访问 [实时加载器返回的任何错误](/zh-cn/reference/content-loader-reference/#error-handling-in-live-loaders)，例如，当内容无法显示时，可以重写到 404 页面：
