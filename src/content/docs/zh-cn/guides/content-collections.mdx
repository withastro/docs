...
title: 内容集合
description: >-
  内容集合有助于组织你的 Markdown 文件以及通过模式使用带有类型检查的 front-matter。
i18nReady: true
---
...
在任何 Astro 项目中，**内容集合**是使用 Markdown 和 MDX 的最佳方式。内容集合是 Astro 的一个特性，可以帮助管理项目中的内容文件。集合有助于组织内容，验证 front-matter，并为所有内容提供自动 TypeScript 类型安全。


## 什么是内容集合?

一个**内容集合**是保留的 `src/content` 项目目录中的任何目录，比如 `src/content/newsletter` 和 `src/content/blog` 。内容集合只允许在 `src/content` 目录中。此目录不能用于其他任何目录。

一个**内容条目**是存储在内容集合目录中的任何内容片段。内容条目存储在 Markdown (`.md`)或 MDX (`.mdx`)文件中。你可以使用任何想用的文件名，但是我们建议使用一致的命名方案(小写，破折号而不是空格) ，以便更容易地查找和组织内容。
...
一旦有了一个集合，你就可以使用 Astro 的内置内容 API 开始[查询你的内容](#querying-collections)。
...
如果两个文件表示不同类型的内容(例如，一篇博客文章和一个作者描述文件) ，那么它们很可能属于不同的集合。这一点非常重要，因为许多特性(front-matter 验证、自动 TypeScript 类型安全)要求集合中的所有条目共享相似的 front-matter 结构。
...
:::note
`src/content/config.ts` 文件是可选的。但是，选择不定义集合将禁用它们的一些最佳特性，如fron-tmatter模式验证或自动 TypeScript 类型。
:::
...
defineCollection({
  schema: z.object({
    ...
    // 在 front matter 中，周围没有引号的日期被解释为 Date 对象
    publishDate: z.date(),
...
每个内容条目从它的[file `id`](/zh-cn/reference/api-reference/#id)生成一个 URL 友好的 `slug` 属性。slug 用于直接从集合中查询条目。在根据内容创建新页面和 URL 时，它也很有用。
...
Astro 提供了两个函数来查询一个集合并返回一个(或多个)内容条目: [`getCollection()`](/zh-cn/reference/api-reference/#getCollection)和[`getEntryBySlug()`](/zh-cn/reference/api-reference/#getEntryBySlug)。

```js
import { getCollection, getEntryBySlug } from 'astro:content';
// 获取集合中的所有条目。要求将集合的名称作为参数。
const allBlogPosts = await getCollection('blog');
// 从集合中获取一个条目。需要集合的名称和条目的 slug 作为参数。
const oneBlogPost = await getEntryBySlug('blog', 'enterprise');
```

这两个函数都返回由[`CollectionEntry`](/zh0cn/reference/api-reference/#Collection-entry-type)类型定义的内容条目。

#### 筛选集合查询

`getCollection()` 接受一个可选的 "filter" 回调，它允许你基于条目的 `id`、`slug` 或 `data`(frontmatter)属性对查询进行过滤。
...
有关将内容呈现为 HTML 的信息，请参见下面的[将内容渲染成 HTML](/zh-cn/guide/Content-Collection/#renderingcontent-to-HTML)。
...
如果这样做，你可以使用[`CollectionEntry`](/zh-cn/reference/api-reference/#Collection-entry-type)实用工具使用 TypeScript 正确地输入组件属性。此实用工具接受与集合模式名称匹配的字符串参数，并将继承该集合模式的所有属性。
...
内容集合存储在 `src/pages/` 目录之外。这意味着默认情况下不为集合项生成路由。你将需要手动创建一个新的[动态路由](/ezh-cn/core-views/routing/#Dynamic-path) ，以便从集合条目生成 HTML 页面。动态路由将映射传入的请求参数(例如: 在 `src/pages/blog/[...slug].astro` 中的 `Astro.params.slug`)获取集合中的正确条目。

生成路由的确切方法将取决于你的构建[`output`](/zh-cn/reference/configuration-reference/#output)模式: `static`(默认值)或 `server`(针对 SSR)。

### 构建为静态输出(默认)

如果你正在构建一个静态网站(Astro 的默认行为) ，你可以使用[`getStaticPath()`](/zh-cn/reference/api-reference/#getstaticpath)函数在构建过程中从一个 `src/pages/` 组件创建多个页面。

调用 `getStaticPath()` 中的[`getCollection()`](/zh-cn/reference/api-reference/#getCollection)来查询内容。然后，使用每个内容条目的 `slug` 属性创建新的 URL 路径。
...
如果你正在构建一个动态网站(使用 Astro 的 SSR 支持) ，则不需要在构建期间提前生成任何路径。相反，你的页面应该检查请求(使用 `Astro.request` 或 `Astro.params` )以按需找到 `slug` ，然后使用[`getEntryBySlug()`](/zh-cn/reference/api-reference/#getEntryBySlug)获取它。
...
2. 为内容集合[设置 TypeScript ](/zh-cn/guide/content-Collection/#setup-up-typecript)。
...
3. 创建至少一个集合(`src/content/` 的文件夹中) ，并将 Markdown 和 MDX 页面从 `src/pages/` 移动到 `src/content/` 的这些子目录中。当同一集合中的所有文件具有相似的 fronmatter 属性时，集合的工作效果最好。因此，请选择新的文件夹结构以反映类似类型的页面。
    ...
    例如，要迁移[教程中的博客文章](/zh-cn/lessons/2-pages/2/) ，请将 `src/pages/post/` 的内容移动到 `src/content/posts/`。

4. 创建一个 `src/content/config.ts` 文件，并为每种内容类型[定义一个模式](/zh-cn/guide/content-Collection/#Definition-a-Collection-schema)。对于博客，我们只有一种内容类型 `posts`:
...
5. [从集合中生成路由](/zh-cn/guide/content-Collection/#generating-path-from-content)。在集合中，Markdown 和 MDX 文件不再使用 Astro 的[基于文件的路由](/zh-cn/guide/Markdown-content/#file-based-routing)自动成为页面，因此必须自己生成页面。

    对于本教程，创建一个 `src/pages/post/[...slug].astro`。这个页面将使用[动态路由](/zh-cn/core-concepts/routing/#dynamic-routes)并为每个集合条目生成一个页面。

    这个页面还需要[查询你的集合](#query-Collection)来获取页面 slug，并使每个路由都可以使用页面内容。
...
7. 将 `Astro.glob()` 替换为[`getCollection()`](/zh-cn/reference/api-reference/#getCollection)以从 Markdown 文件中获取内容和元数据。还需要更新对返回的 post 对象的引用，因为你现在可以在 `data` 属性上找到 fronmatter 值。
    ...
    ```js title="src/pages/tags/[tag].astro" "post.data" "getCollection(\"posts\")"
    ...
    export async function getStaticPaths() {
      const allPosts = await getCollection("posts");
    ...
    :::note
    任何单独的 Markdown 或 MDX 文件导入都应该由[`getEntryBySlug()`](/zh-cn/reference/api-reference/#getEntryBySlug)替换。
    :::
...
Astro 支持 remark 或者 rehype 插件[直接修改 frontmatter](/zh-cn/guides/markdown-content/#modifying-frontmatter-programmatically)。你可以通过使用从 `render()` 返回的 `remarkPluginFrontmatter` 属性在一个内容条目中访问这个修改过的 Frontmatter:
...
---
<!-- 
  这个例子假设 `readingTime` 是由一个 remark 插件注入的。
  有关更多细节，请参见我们的 "reading time" 示例:
...
remark 和 rehype 管道只在渲染内容时运行，这就解释了为什么只有在对内容条目调用 `render()` 之后才可以使用 `remarkPluginFrontmatter`。相比之下，`getCollection()` 和 `getEntryBySlug()` 不能直接返回这些值，因为它们不会渲染内容。
