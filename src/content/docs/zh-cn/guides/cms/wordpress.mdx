...
[WordPress](https://wordpress.org/) 是一个内容管理系统，虽然它包含了自己的前端，但也可以作为无头 CMS 用于为你的 Astro 项目提供内容。

## 与 Astro 集成

WordPress 自带了内置的 [WordPress REST API](https://developer.wordpress.org/rest-api/)，用于将你的 WordPress 数据连接到 Astro。你还可以选择在你的站点上安装 [WPGraphQL](https://wordpress.org/plugins/wp-graphql/) 来使用 GraphQL。

### 前期准备

在开始前，你需要具备以下条件：

1. **一个Astro项目** - 如果你还没有Astro项目，我们的[安装指南](/zh-cn/install/auto/)可以帮助你快速上手；
2. **一个WordPress站点** - 你的站点的 REST API 是 `[YOUR_SITE]/wp-json/wp/v2/`，并在任何WordPress站点上都是默认提供的。你也可以在[本地环境上安装 WordPress](https://wordpress.org/support/article/installing-wordpress-on-your-own-computer/)。
...
页面 `src/pages/dinos/[slug].astro` 为每个恐龙 [动态生成了一个页面](/zh-cn/core-concepts/routing/#动态路由)。
...
-  [Astro 和 WordPress 作为 API](https://darko.io/posts/astrojs-and-wordpress-as-an-api/)。
