...
## 使用 WordPress 和 Astro 构建博客 \{#building-a-blog-with-wordpress-and-astro\}
...
### 显示 WordPress 文章列表 \{#displaying-a-list-of-wordpress-posts\}
...
### 使用 WordPress API 生成页面 \{#using-the-wordpress-api-to-generate-pages\}

页面 `src/pages/dinos/[slug].astro` 为每个恐龙 [动态生成了一个页面](/zh-cn/guides/routing/#dynamic-routes)。
...
### 返回嵌入资源 \{#returning-embedded-resources\}
