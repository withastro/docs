...
### 配置 Storyblok \{#configuring-storyblok\}
...
### 将 Bloks 连接到 Astro 组件 \{#connecting-bloks-to-astro-components\}
...
## 使用 Astro 和 Storyblok 创建博客 \{#making-a-blog-with-astro-and-storyblok\}
...
### 创建 Blok 库 \{#creating-a-blok-library\}
...
### 将 Bloks 连接到组件 \{#connecting-bloks-to-components\}
...
如果你正在使用 Astro 的默认静态站点生成功能，你将使用[动态路由](/zh-cn/guides/routing/#dynamic-routes)和 `getStaticPaths` 函数来生成你的项目页面。
...
#### 当 Storyblok 发生更改时重新构建 \{#rebuild-on-storyblok-changes\}
...
##### 将 Webhook 添加到 Storyblok \{#adding-a-webhook-to-storyblok\}
