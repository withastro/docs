...
[Storyblok](https://www.storyblok.com/) 是一个基于组件的无头 CMS，允许你使用可重用的组件（称为 Bloks）管理内容。

## 与 Astro 的集成

在这个部分，你将使用 [Storyblok 集成](https://github.com/storyblok/storyblok-astro) 将 Storyblok 连接到 Astro。
...
3. **Storyblok 预览令牌** - 这个令牌将用于获取你内容的草稿和已发布版本。你可以在 Storyblok 空间设置的访问令牌选项卡中找到并生成你的 API 令牌。
...
为了将 Astro 连接到你的 Storyblok 空间，使用以下命令安装官方的 [Storyblok 集成](https://github.com/storyblok/storyblok-astro)，根据你首选的包管理器执行相应命令：
...
2. `components` - 一个将 Storyblok 组件名称映射到本地组件路径的对象。这是为了在 Astro 中渲染你的 Storyblok Bloks 而必需的。
...
要将你的 Bloks 连接到 Astro，请在 `src` 目录下创建一个名为 `storyblok` 的新文件夹。该文件夹将包含与你的 Storyblok Blok 库中的 Bloks 匹配的所有 Astro 组件。

在这个例子中，你的 Storyblok Blok 库中有一个名为 `blogPost` 的 Blok 内容类型，具有以下字段：

-  `title` - 一个文本字段
-  `description` - 一个文本字段
-  `content` - 一个富文本字段

我们的目标是创建一个等效的 Astro 组件，使用这些字段来渲染其内容。为此，在 `src/storyblok` 中创建一个名为 `BlogPost.astro` 的新文件，并添加以下内容：
...
为了渲染你的内容，将 Story 的 `content` 属性作为 `blok` 属性传递给 `StoryblokComponent`。该组件将渲染在 `content` 属性中定义的 Bloks。在这种情况下，它将渲染 `BlogPost` 组件。
...
2. **集成了 Storyblok 的 Astro 项目** - 参考[与 Astro 集成](#integrating-with-astro)中的说明来设置集成。
...
3. `blog/astro-is-amazing` - 位于博客文件夹中，使用 `blogPost` 内容类型的故事。
   ...
   content: Hi there! This blog post was built with Astro.
   ```

现在你准备好了你的内容，返回到 Astro 项目中，开始构建你的博客。

### 将 Bloks 连接到组件

为了将你新创建的 Bloks 连接到 Astro 组件，创建一个名为 `storyblok` 的新文件夹，放置到你的 `src` 目录中，并添加以下文件：

`Page.astro` 是一个可嵌套的 Block 内容类型组件，将递归地渲染 `page `blok`  属性包含你从 Storyblok 中接收到的数据。它还包含在 Storyblok 的 `blogPost` 内容类型 Blok 中定义的字段。
...
在 `src/pages/` 目录下创建一个 `index.astro` 文件，用于渲染 `home` 页面：
...
对于你的所有博客文章生成页面，创建一个 `.astro` 页面来创建动态路由。根据你使用的是**静态站点生成**（默认情况）还是**服务器端渲染**，这种方法会有所不同。

#### 静态站点生成

如果你正在使用 Astro 的默认静态站点生成功能，你将使用[动态路由](/en/core-concepts/routing/#dynamic-routes)和 `getStaticPaths` 函数来生成你的项目页面。
...
如果你已经[启用了 SSR 模式](/en/guides/server-side-rendering/#enabling-ssr-in-your-project)，你将使用动态路由从 Storyblok 获取页面数据。
...
要部署你的网站，请访问我们的[部署指南](/en/guides/deploy/)，并按照你首选的托管提供商的说明进行操作。
...
-  Sandra Rodgers的文章：[为 Storyblok + Astro 使用可视化编辑器](https://dev.to/sandrarodgers/getting-the-visual-editor-to-work-for-storyblok-astro-2gja)
-  Jonas Gierer的文章：[Astro + Storyblok：SSR 预览实现即时可视化编辑](https://dev.to/jgierer12/astro-storyblok-ssr-preview-for-instant-visual-editing-3g9m)
-  Sandra Rodgers的文章：[使用 Netlify 的分支部署功能预览带有 Astro-Storyblok 的站点](https://dev.to/sandrarodgers/astro-storyblok-previews-site-with-netlifys-branch-deploys-feature-44dh)