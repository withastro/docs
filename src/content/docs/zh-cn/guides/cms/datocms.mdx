...
- **Astro 项目** - 如果你还没有 Astro 项目，请按照我们的[安装指南](/en/install/auto/)中的说明进行设置。
...
在此示例中，DatoCMS中的模块化内容块被命名为**content**。此查询还包括每个项目的唯一`_modelApiKey`，以检查模块字段中的数据，根据内容作者在 DatoCMS 编辑器中选择的内容来动态渲染。在 Astro 模板中使用 switch 语句，根据从查询中接收到的数据进行动态渲染。
...
3. 为 Webhook 提供一个名称，选择要触发构建的分支。点击**Save**并复制生成的URL。
...
3. 为 Webhook 提供一个名称，选择要触发构建的分支。点击**Add**并复制生成的URL。
...
## 起始项目
你还可以在 DatoCMS 市场上查看[Astro博客模板](https://www.datocms.com/marketplace/starters/astro-template-blog)，了解如何使用 Astro 和 DatoCMS 创建博客。