...
const res = await fetch("https://localhost:5000/api/posts") // https://localhost:4321/api/posts 默认端口
...
const res = await fetch("http://localhost:5000/api/posts") // https://localhost:4321/api/posts 默认端口
const posts = await res.json()
---
...
创建一个页面 `src/pages/post/[slug].astro` 为每一篇文章[动态生成一个页面](/zh-cn/core-concepts/routing/#动态路由)。
