...
### 设置 Content Delivery API \{#setting-up-the-content-delivery-api\}
...
## 使用 Umbraco 和 Astro 构建博客 \{#building-a-blog-with-umbraco-and-astro\}
...
### 在 Umbraco 中创建博客文章 \{#creating-blog-posts-in-umbraco\}
...
### 在 Astro 中展示博客文章列表 \{#displaying-a-list-of-blog-posts-in-astro\}
...
### 生成单个博客文章 \{#generating-individual-blog-posts\}

在 `/pages/` 目录的根部创建文件 `[...slug].astro`。此文件将用于[动态生成单个博客页面](/zh-cn/guides/routing/#dynamic-routes)。
...
## 本地开发、HTTPS 和自签名 SSL 证书 \{#local-dev-https-and-self-signed-ssl-certificates\}
