...
在本节中，你将使用 Umbraco 的原生 [内容传递 API](https://docs.umbraco.com/umbraco-cms/reference/content-delivery-api) 为你的 Astro 项目提供内容。

### 前提条件

要开始，你需要具备以下条件：

1. **一个 Astro 项目** - 如果你还没有 Astro 项目，我们的 [安装指南](https://docs.astro.build/zh-cn/install/auto/) 将帮助你快速启动和运行。
2. **一个 Umbraco (v12+）项目** - 如果你还没有 Umbraco 项目，请参阅这个 [安装指南](https://docs.umbraco.com/umbraco-cms/fundamentals/setup/install/)。

### 设置内容传递 API

要启用内容传递 API，请更新你的 Umbraco 项目的 `appsettings.json` 文件：
...
你可以根据需要配置额外的选项，如公共访问、API 密钥、允许的内容类型、会员授权等。更多信息请参见 [Umbraco 内容传递 API 文档](https://docs.umbraco.com/umbraco-cms/reference/content-delivery-api)。

### 获取数据

使用 `fetch()` 调用内容传递 API 来访问你的内容，并使其可用于你的 Astro 组件。
...
<ReadMore>了解更多关于 [在 Astro 中获取数据](https://docs.astro.build/zh-cn/guides/data-fetching/) 的信息。</ReadMore>
...
- **一个启用了内容传递 API 的 Umbraco 项目（v12+）** - 遵循这个 [安装指南](https://docs.umbraco.com/umbraco-cms/fundamentals/setup/install/) 来创建一个新项目。
...
要创建博客文章列表，首先使用 `fetch` 调用内容传递 API 的 `content` 端点，并按 'article' 的 contentType 进行筛选。文章对象将包括在 CMS 中设置的属性和内容。然后，你可以遍历这些文章，并显示每个标题及其文章的链接。
...
在 `/pages/` 目录的根部创建文件 `[...slug].astro`。此文件将用于[动态生成单个博客页面](https://docs.astro.build/zh-cn/guides/routing/#动态路由)。
...
要部署你的网站，请访问我们的[部署指南](https://docs.astro.build/zh-cn/guides/deploy/)并按照你选择的托管提供商的指示进行操作。
...
## 官方文档
- [内容传递 API - Umbraco 文档](https://docs.umbraco.com/umbraco-cms/reference/content-delivery-api)

## 社区资源

- [使用内容传递 API 构建性能卓越的 Astro 网站 - Louis Richardson](https://24days.in/umbraco-cms/2023/sustainable-performant/astronomically-performant/)
- [从 Umbraco 的内容传递 API 生成 TypeScript OpenAPI 客户端 - Rick Butterfield](https://rickbutterfield.dev/blog/typescript-openapi-umbraco-content-delivery/)
