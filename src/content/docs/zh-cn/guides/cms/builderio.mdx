...
## 使用 Astro 和 Builder 创建博客 \{#making-a-blog-with-astro-and-builder\}

### 创建博客文章的模型 \{#creating-a-model-for-a-blog-post\}
...
#### 设置新路由作为预览 URL \{#setting-the-new-route-as-the-preview-url\}
...
#### 测试预览 URL 设置 \{#testing-the-preview-url-setup\}
...
### 显示单个博客文章 \{#displaying-a-single-blog-post\}

创建页面 `src/pages/posts/[slug].astro`，该页面将为每篇文章[动态生成一个页面](/zh-cn/guides/routing/#dynamic-routes)。
...
#### 在 Builder 更改时重建 \{#rebuild-on-builder-changes\}
...
##### 将 webhook 添加到 Builder \{#adding-a-webhook-to-builder\}
