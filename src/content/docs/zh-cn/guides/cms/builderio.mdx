...
* **一个 Builder 账户和空间** - 如果你还没有账户，可以[免费注册](https://www.builder.io/)并创建一个新的空间。如果你已经有了一个 Builder 的空间，直接使用即可，但请确保代码中的模型名称（`blogpost`）和自定义数据字段与你的设置相匹配。
...
如果你希望你的环境变量具有 IntelliSense 功能，你可以在 `src/` 目录下创建一个 `env.d.ts` 文件，并按照以下方式配置 `ImportMetaEnv`：
...
创建页面 `src/pages/posts/[slug].astro`，该页面将为每篇文章[动态生成一个页面](/zh-cn/core-concepts/routing/#动态路由)。
