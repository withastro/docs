...
### 在 `.env` 文件中添加 Strapi URL \{#adding-the-strapi-url-in-env\}
...
### 创建 API 包装器 \{#creating-the-api-wrapper\}
...
### 可选：创建 Article 接口 \{#optional-creating-the-article-interface\}
...
### 展示文章列表 \{#displaying-a-list-of-articles\}
...
### 生成文章页面 \{#generating-article-pages\}

创建文件 `src/pages/blog/[slug].astro` 来[为每篇文章动态生成页面](/zh-cn/guides/routing/#dynamic-routes)。
...
如果你选择了 [SSR 模式](/zh-cn/guides/server-side-rendering/) 并设置了 `output: server` 或 `output: hybrid`，那么可以使用以下代码来生成你的[动态路由](/zh-cn/guides/routing/#server-ssr-mode)。
...
#### 当内容发生变化时重新构建 \{#rebuild-on-changes\}
...
##### 在 Strapi 中添加 Webhook \{#adding-a-webhook-to-strapi\}
