...
创建文件 `src/pages/blog/[slug].astro` 来[为每篇文章动态生成页面](/zh-cn/basics/routing/#动态路由)。
...
如果你选择了 [SSR 模式](/zh-cn/guides/server-side-rendering/) 并设置了 `output: server` 或 `output: hybrid`，那么可以使用以下代码来生成你的[动态路由](/zh-cn/basics/routing/#服务器ssr模式)。
