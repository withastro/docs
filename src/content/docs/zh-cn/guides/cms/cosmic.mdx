...
## 为 Astro 集成 Cosmic \{#integrating-cosmic-with-astro\}

### 安装必要的依赖 \{#installing-necessary-dependencies\}
...
## 从 Cosmic 获取数据 \{#fetching-data-from-cosmic\}
...
## 使用 Astro 和 Cosmic 构建博客 \{#building-a-blog-with-astro-and-cosmic\}

你可以使用 Cosmic 管理你的 Astro 博客的内容。同时，也可以创建 webhooks，以在你更新或添加新内容时自动重新部署你的网站。

### Cosmic 内容对象 \{#cosmic-content-objects\}
...
### 在 Astro 中显示博客文章列表 \{#displaying-a-list-of-blog-posts-in-astro\}

使用与上面相同的[数据获取方法](#fetching-data-from-cosmic)，从你的脚本文件中导入 `getAllPosts()` 查询并等待数据响应。此函数提供了关于每个 `post` 的元数据，并可以动态显示。
...
### 使用 Astro 生成单个博客文章 \{#generating-individual-blog-posts-with-astro\}

为了给每个博客文章生成一个含有完整内容的单独页面，需要创建一个位于 `src/pages/blog/[slug].astro` 的 [动态路由页面](/zh-cn/guides/routing/#dynamic-routes)。
...
#### 在 Cosmic 内容更新时重建 \{#rebuild-on-cosmic-content-updates\}
