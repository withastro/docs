...
选择 Astro 模板后，你就可以开始[部署你的 Keystatic + Astro 项目](#deploying-keystatic--astro)了！

:::

## 前期准备 \{#prerequisites\}

- 一个已经[配置了服务端渲染 (SSR) 和 `output: 'hybrid'` 或 `output: 'server'`](/zh-cn/guides/server-side-rendering/)的现有 Astro 项目。
...
## 添加 Astro 集成 \{#adding-the-astro-integration\}
...
## 创建 Keystatic 配置文件 \{#creating-a-keystatic-config-file\}
...
:::note[已经在使用内容集合了吗？]
如果你的 Astro 项目中已经使用了[内容集合](/zh-cn/guides/content-collections/#defining-collections)，请确保更新上述 schema，使其与你现有 schema 中定义的集合完全一致。
:::

现在，Keystatic 已经可以根据你的 schema 配置来管理内容了。

## 在本地运行 Keystatic \{#running-keystatic-locally\}
...
## 渲染 Keystatic 内容 \{#rendering-keystatic-content\}

你可以使用 Astro 的内容集合 API 来[查询和显示你的帖子和集合](/zh-cn/guides/content-collections/#querying-collections)，就如同在其他 Astro 项目中一样。

### 显示集合列表 \{#displaying-a-collection-list\}
...
### 显示单个条目 \{#displaying-a-single-entry\}

要展示单个帖子的内容，你可以引入并使用 `<Content />` 组件，这样可以方便地[将内容渲染成 HTML](/zh-cn/guides/content-collections/#rendering-content-to-html)。
...
## 部署 Keystatic + Astro \{#deploying-keystatic--astro\}
