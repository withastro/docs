...
1. **一个 Astro 项目** - 如果你还没有一个 Astro 项目，我们的 [安装指南](/zh-cn/install-and-setup/) 能够帮助你快速启动并运行。
...
1. 通过 Manage administrative 菜单导航到扩展页面 `admin/modules`
...
你可以在 [Drupal JSON：API文档](https://www.drupal.org/docs/core-modules-and-themes/core-modules/jsonapi-module/jsonapi) 中找到更多查询选项。
...
2. **至少有一个条目的 Drupal 站点** - 对于本教程，我们建议从标准安装的新 Drupal 站点开始。
    ...
    在 Drupal 站点的 **Content** 部分中，通过单击 **Add** 按钮创建一个新条目。然后，选择文章并填写字段：
    ...
    单击 **Save** 创建你的第一篇文章。请随意添加任意数量的文章。
