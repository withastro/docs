...
如果你想要获得[这些环境变量的 TypeScript IntelliSense](/zh-cn/guides/environment-variables/#intellisense-for-typescript)，你可以在 `src/` 目录下创建一个新的 `env.d.ts` 文件，并像下面这样配置 `ImportMetaEnv`：
...
:::note
请阅读更多有关 [在 Astro 中获取环境变量](/zh-cn/guides/environment-variables/#getting-environment-variables) 的内容。
:::
...
## 使用 Astro 和 Kontent.ai 创建博客 \{#making-a-blog-with-astro-and-kontentai\}

通过上述设置，你现在可以创建一个使用 Kontent.ai 作为内容源的博客。

### 前期准备 \{#prerequisites-1\}
...
### 设置内容模型 \{#setting-up-content-model\}
...
### 在 TypeScript 中生成内容模型 \{#generating-content-model-in-typescript\}
...
### 展示博客文章列表 \{#displaying-a-list-of-blog-posts\}
...
### 生成单个博客文章页面 \{#generating-individual-blog-posts\}

教程的最后一步是生成详细的博客文章页面。

#### 静态站点生成 \{#static-site-generation\}

在本节中，你将使用 Astro 的 [静态 (SSG) 模式](/zh-cn/guides/routing/#static-ssg-mode)。
...
### 发布网站 \{#publishing-your-site\}

要部署你的网站，请访问 [部署指南](/zh-cn/guides/deploy/) 并按照你偏好的托管提供商的说明进行操作。

#### 在 Kontent.ai 更改时重新构建 \{#rebuild-on-kontentai-changes\}
...
##### 添加 Webhook 到 Kontent.ai \{#adding-a-webhook-to-kontentai\}
