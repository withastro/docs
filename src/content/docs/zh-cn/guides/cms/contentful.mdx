...
如果你使用的是 Astro 的默认静态模式，你将使用 [动态路由](/zh-cn/basics/routing/#动态路由) 和 `getStaticPaths()` 函数。此函数将在构建时调用，以生成成为页面的路径列表。
...
如果找不到条目，你可以使用 [`Astro.redirect`](/zh-cn/basics/routing/#动态重定向) 将用户重定向到 404 页面。
