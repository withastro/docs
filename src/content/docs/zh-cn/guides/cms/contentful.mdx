...
### 从 Contentful 获取数据 \{#fetching-data\}
...
:::tip
如果你有一个空的 Contentful 空间，请查看 [设置 Contentful 模型](#setting-up-a-contentful-model) 以了解如何为你的内容创建基本的博客模型。
:::

你可以在 [Contentful 文档](https://contentful.github.io/contentful.js/) 中找到更多的查询选项。

## 使用 Astro 和 Contentful 制作博客 \{#making-a-blog-with-astro-and-contentful\}
...
2. **集成了 [Contentful SDK](https://github.com/contentful/contentful.js)** 的 Astro 项目 - 有关如何在 Astro 项目中设置 Contentful 的详细信息，请参阅 [与 Astro 集成](#integrating-with-astro)。

### 设置 Contentful 模型 \{#setting-up-a-contentful-model\}
...
### 显示博客文章列表 \{#displaying-a-list-of-blog-posts\}
...
#### 生成单独的博客文章页面 \{#generating-individual-blog-posts\}

使用与上述相同的方法从 Contentful 获取数据，但这次在将为每篇博客文章创建一个唯一的页面路由。

#### 静态站点生成 \{#static-site-generation\}

如果你使用的是 Astro 的默认静态模式，你将使用 [动态路由](/zh-cn/guides/routing/#dynamic-routes) 和 `getStaticPaths()` 函数。此函数将在构建时调用，以生成成为页面的路径列表。
...
如果找不到条目，你可以使用 [`Astro.redirect`](/zh-cn/guides/routing/#dynamic-redirects) 将用户重定向到 404 页面。
...
### 发布你的网站 \{#publishing-your-site\}

要部署你的网站，请访问我们的[部署指南](/zh-cn/guides/deploy/)，并按照你首选的托管提供商的说明操作。

#### 在 Contentful 更改后重新构建 \{#rebuild-on-contentful-changes\}
...
##### 将 Webhook 添加到 Contentful \{#adding-a-webhook-to-contentful\}
