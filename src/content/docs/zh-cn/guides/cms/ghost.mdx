...
## 使用 Astro 和 Ghost 创建博客 \{#making-a-blog-with-astro-and-ghost\}

通过上面的设置，你现在可以创建一个使用 Ghost 作为 CMS 的博客。

### 前置准备 \{#prerequisites-1\}

1. 一个 Ghost 博客 
2. 一个与 [Ghost content API](https://www.npmjs.com/package/@tryghost/content-api) 集成的 Astro 项目 - 请参见 [与 Astro 集成](/zh-cn/guides/cms/ghost/#integrating-with-astro) 了解如何将 Astro 项目与 Ghost 集成的更多详细信息。
...
### 显示文章列表 \{#displaying-a-list-of-posts\}
...
此页面 `src/pages/post/[slug].astro` 将为每一篇文章[动态生成一个页面](/zh-cn/guides/routing/#dynamic-routes)。
