...
## 安装 DecapCMS \{#installing-decapcms\}
...
### Decap CMS 与 Netlify 身份验证 \{#decap-cms-with-netlify-identity\}
...
### Decap CMS 与外部 OAuth 客户端 \{#decap-cms-with-external-oauth-clients\}

当部署到 Netlify 以外的托管服务提供商时，你必须创建自己的 OAuth 路由。

在 Astro 中，这可以通过在你的项目中配置了 [`server` 或 `hybrid` 输出](/zh-cn/guides/server-side-rendering/#enable-on-demand-server-rendering) 的按需渲染路由来完成。
