...
为了帮助你调试 Astro 组件，Astro 提供了内置的 [`<Debug />`](/zh-cn/reference/api-reference/#debug-) 组件，你可以用它把任意值直接渲染到你的 HTML 模板中。这对于在浏览器中进行快速调试是很有用的，而不必在终端和浏览器之间来回切换。
...
- 如果代码位于框架组件中，请尝试使用生命周期方法（例如 React 中的 [`useEffect()`](https://react.dev/reference/react/useEffect)、Vue 中的 [`onMounted()`](https://vuejs.org/api/composition-api-lifecycle.html#onmounted) 和 Svelte 中的 [`onMount()`](https://svelte.dev/docs#run-time-svelte-onmount)）在渲染后访问这些对象。通过使用 [client:](/zh-cn/reference/directives-reference/#client-directives) 指令（例如 `client:load`）来告诉框架组件在客户端进行水合，从而运行这些生命周期方法。你还可以通过添加 [`client:only`](/zh-cn/reference/directives-reference/#clientonly) 指令来防止组件在服务器上进行渲染。
...
首先，检查你是否在 [`.astro` 组件脚本](/zh-cn/basics/astro-components/#the-component-script)或 [`.mdx` 文件](/zh-cn/guides/markdown-content/#using-components-in-mdx) 中**导入了该组件**。

然后检查你的导入语句。

- 你的导入链接到了错误的地方吗？（检查你的导入路径）

- 你的导入与导入的组件有名称相同吗？（检查你的组件名称和它是否[遵循 `.astro` 语法](/zh-cn/basics/astro-syntax/#differences-between-astro-and-jsx)）
...
默认情况下，[UI 框架组件在客户端不会激活](/zh-cn/guides/framework-components/#hydrating-interactive-components) 。如果没有提供 `client:*` 指令，它的 HTML 将直接渲染到页面上，而无需 JavaScript。
...
你可能会注意到，即使组件不出现在最终输出中，导入的组件的 `<script>` 或 `<style>` 标签也会包含在 HTML 源中。例如，这样的情况会发生在未展示的[条件渲染](/zh-cn/basics/astro-syntax/#dynamic-html)组件中。
