...
为了帮助你调试 Astro 组件，Astro 提供了内置的 [`<Debug />`](/zh-cn/guides/troubleshooting/#astro-debug--component) 组件，你可以用它把任意值直接渲染到你的 HTML 模板中。这对于在浏览器中进行快速调试是很有用的，而不必在终端和浏览器之间来回切换。
...
这不是 Astro 的 bug。这是由于 [Vite 的 `import.meta.glob()` 函数](https://cn.vite.dev/guide/features.html#glob-import)的限制，它只支持静态字符串字面量。
