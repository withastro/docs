...
如果你在启动 Astro 时看到类似于 `"Cannot find package 'react'"` 的警告，这就意味着你需要将这个包安装到你的项目中。并非所有的包管理器都会自动为你安装对等依赖。如果你使用的是 Node 16以上的版本和 npm，那么你大可不必担心这类问题。

例如，React 是 `@astrojs/react` 集成的对等依赖。这意味着你应该将官方的 `react` 和 `react-dom` 包与你的集成一起安装。然后，该集成将自动从这些包中提取。
...
你可能会注意到，即使组件不出现在最终输出中，导入的组件的 `<script>` 或 `<style>` 标签也会包含在 HTML 源中。例如，这样的情况会发生在未展示的[条件渲染](/zh-cn/core-concepts/astro-components/#dynamic-html)组件中。
