...
在下面的示例中， 剩余参数（`[...slug]`）和 `getStaticPaths()` 的 [`props`](/zh-cn/reference/routing-reference/#data-passing-with-props) 为不同深度的 slug 生成页面。
...
<ReadMore>了解更多关于 [page 分页参数](/zh-cn/reference/routing-reference/#the-pagination-page-prop) 的内容。</ReadMore>
