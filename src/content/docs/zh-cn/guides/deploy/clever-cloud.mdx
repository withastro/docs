...
你可以在 Clever Cloud 上部署完全静态和按需渲染的 Astro 项目。无论你的 `output` 模式如何（预渲染或[按需](/zh-cn/guides/on-demand-rendering/)），你都可以选择，是部署为使用构建后挂钩运行的 **静态应用程序**，或者是部署为使用 `package.json` 需要手动配置的 **Node.js** 应用程序。

### 脚本

如果你正在使用按需的 Node.js 应用，请更新你的 `start` 脚本以运行 Node 服务器。Clever Cloud 上的应用程序监听端口 **8080**。
