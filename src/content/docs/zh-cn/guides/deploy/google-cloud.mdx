...
### Cloud Storage（仅静态网站） \{#cloud-storage-static-only\}
...
#### 创建 Dockerfile 并构建容器 \{#create-dockerfile--build-the-container\}

在将 Astro 网站部署到 Cloud Run 之前，你需要创建一个 Dockerfile，用于构建容器。在我们的操作指南部分可以找到有关如何[用 Docker 来构建你的 Astro 网站](/zh-cn/recipes/docker/#creating-a-dockerfile)更多信息。
