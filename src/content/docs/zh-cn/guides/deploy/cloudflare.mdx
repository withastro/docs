...
- 这意味着你在服务器端环境中使用的某个包或导入项与 [Cloudflare Workers 运行时 APIs](https://developers.cloudflare.com/workers/runtime-apis/nodejs/) 不兼容。
...
- 如果你导入的包中包含了 Node.js 运行时 API，请检查该包的作者是否支持 `node:*` 的导入语法。如果不支持，你可能需要找到一个替代的包或使用不同的适配器。
