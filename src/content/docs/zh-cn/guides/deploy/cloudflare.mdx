...
你可以通过 [Cloudflare Workers](https://developers.cloudflare.com/workers/static-assets/) 和 [Cloudflare Pages](https://pages.cloudflare.com/) 来部署全栈应用，包括前端的静态资源和后端的 API，以及 SSR 站点。
...
2. 如果你的站点使用了按需渲染，安装 `@astrojs/cloudflare` 适配器。
    ...
    <ReadMore>阅读更多有关 [Astro 中的按需渲染（又称 SSR）](/zh-cn/guides/on-demand-rendering/)的内容。</ReadMore>
...
<ReadMore>阅读更多有关使用 [Cloudflare 运行时 API](/en/guides/integrations-guide/cloudflare/) 的内容，例如绑定。</ReadMore>
