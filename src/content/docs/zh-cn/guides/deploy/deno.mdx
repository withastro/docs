...
要在 Astro 项目中启用 SSR 并在 Deno Deploy 上进行部署，请执行以下 `astro add` 命令添加 [Deno 适配器](/zh-cn/guides/integrations-guide/deno/)。这将一步安装适配器并对你的 `astro.config.mjs` 文件进行适当更改。
...
1. 使用你喜欢的包管理器将 [`@astrojs/deno` 适配器](https://github.com/withastro/astro/tree/main/packages/integrations/deno) 安装到你的项目依赖中。如果你不确定是否使用 npm 的话，可以在终端中运行以下命令：
