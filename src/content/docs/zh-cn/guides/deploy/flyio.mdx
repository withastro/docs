...
## 生成你的 Astro Dockerfile \{#generating-your-astro-dockerfile\}

如果你还没有 Dockerfile，`fly launch` 将为你生成一个，并准备一个 `fly.toml` 文件。对于 `server` 或 `hybrid` 项目，这个 Dockerfile 将包含适当的启动命令和环境变量。
