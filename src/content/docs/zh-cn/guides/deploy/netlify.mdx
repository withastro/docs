...
通过以下的 `astro add` 命令，添加 [Netlify 适配器](/en/guides/integrations-guide/netlify/)，在你的 Astro 项目中启用 SSR。这一步将会安装适配器并且对你的 `astro.config.mjs` 文件进行适当的修改。
...
1. 使用你喜欢的包管理器将 [@astrojs/netlify 适配器](https://github.com/withastro/astro/tree/main/packages/integrations/netlify) 安装到你的项目依赖中。如果你正在使用 npm 或者不确定使用何种包管理器，可以在终端运行以下命令：
...
1. 在 [Netlify dashboard](https://app.netlify.com/) 页面上, 点击 <kbd>添加站点</kbd>

2. 选择 <kbd>导入已存在的项目</kbd>

    当你从你的 Git 提供商中导入 Astro 仓库时，Netlify 应该会自动检测并预填充正确的配置设置。

3. 确保已输入以下设置，然后按下 <kbd>部署</kbd> 按钮:
...
4. 确认你的构建命令 (`astro build`)
    ...
    CLI 将自动检测构建设置（`astro build`）和部署目录（`dist`），并将提供一个自动生成且有这些对应设置的 [`netlify.toml` 文件](#netlifytoml-file)。
...
- [如何部署 Astro 站点](https://www.netlify.com/blog/how-to-deploy-astro/) — Netlify Blog
- [使用表单、无服务器功能和重定向功能部署 Astro 网站](https://www.netlify.com/blog/deploy-an-astro-site-with-forms-serverless-functions-and-redirects/) — Netlify Blog
- [部署演示视频](https://youtu.be/GrSLYq6ZTes) — Netlify YouTube channel
