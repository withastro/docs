...
使用 [`@astrojs/db` 集成](/zh-cn/guides/integrations-guide/db/)（`v0.8.0` 或更高版本）来将 Astro DB 添加到新的或现有的 Astro 项目中（需要 `astro@4.5` 或更高版本）。Astro 包含一个内置的 `astro add` 命令能帮你自动化设置此过程。
...
要配置一个独立的数据库表，应从 `astro:db` 导入并使用 `defineTable()` 和 `column` 工具。

这个例子配置了一个 `Comment` 表，它有 `author` 和 `body` 的必需文本列，然后通过 `defineDb()` 导出使其在你的项目中可用。
...
<ReadMore>查看 [表配置参考](/en/guides/integrations-guide/db/#表配置参考) 以获取完整的表选项参考。</ReadMore>

### 表引用

表之间的关系是数据库设计中的常见模式。例如，一个 `Blog` 表可能与 `Comment`、`Author` 和 `Category` 等其他几个表密切相关。

你可以使用**引用列**定义来这些关系表并将它们保存到你的数据库模式中。要建立关系，你需要：
...
你可以从项目中的任何 [Astro 页面](/zh-cn/basics/astro-pages/#astro页面) 或 [端点](/zh-cn/guides/endpoints/) 使用提供的 `db` ORM 和查询构建器来查询你的数据库。
...
以下例子查询了 `Comment` 表中包含 "Astro DB" 短语的所有行。使用 [the `like()` 操作符](https://orm.drizzle.team/docs/operators#like) 检查 `body` 中是否存在短语：
...
1. 通过 [Astro Studio CLI](/zh-cn/reference/cli-reference/#astro-studio-cli) 使用 `astro link`
...
以下 Comments 可以使用命令 `astro db execute db/seed.ts --remote` 进行填充：
...
当你准备好部署时，参见我们的 [使用 Studio 连接进行部署指南](/en/recipes/studio/#使用-Studio-连接进行部署)。
...
`defineDbIntegration()` 辅助函数为 `astro:db:setup` 钩子提供 TypeScript 支持和自动不全。
