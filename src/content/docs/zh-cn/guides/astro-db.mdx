...
<ReadMore>查看 [表配置参考](/zh-cn/guides/integrations-guide/db/#table-configuration-reference) 以获取完整的表选项参考。</ReadMore>
...
<ReadMore>查看[表的列参考](/zh-cn/guides/integrations-guide/db/#table-configuration-reference)以获取更多细节。</ReadMore>
...
要接受用户输入，如处理表单请求并将数据插入到你的远程托管数据库，需要为你的 Astro 项目配置 [按需渲染](/zh-cn/basics/rendering-modes/#on-demand-rendered) 并为你的部署环境 [添加一个 SSR 适配器](/zh-cn/guides/server-side-rendering/#official-adapters)。
...
2. **使用 Astro Studio CLI** 从任何本地 Astro 项目创建。你可以运行以下命令开始:
    ...
    <ReadMore>查看 [Astro DB CLI 参考](/zh-cn/guides/integrations-guide/db/#astro-db-cli-reference) 获取所有可用命令。</ReadMore>
...
查看 [CLI 参考](/zh-cn/guides/integrations-guide/db/#astro-db-cli-reference) 获取完整的命令列表。
