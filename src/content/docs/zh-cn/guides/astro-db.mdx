...
如果你必须以一种与 Astro Studio 托管的现有数据不兼容的方式更改你的表模式，你将需要重置你的生产数据库。
...
在将你的架构推送到 Astro Studio 之后，可以重命名一个表格。

如果你**没有任何重要的生产数据**，那么你可以使用 `--force-reset` 标志[重置你的数据库](#pushing-breaking-schema-changes)。这个标志将会删除数据库中的所有表格，并创建新的表格，以便它完全匹配你当前的架构。

为了在保留你的生产数据的同时重命名一个表格，你必须执行一系列非破坏性更改，以安全地将你的本地架构推送到 Astro Studio。

以下示例将一个表格从 `Comment` 重命名为 `Feedback`：
...
2. 添加一个新的表格模式（完全匹配现有表格的属性）并使用新名称：
    ...
    ```
3. 使用 `astro db push --remote` [推送到 Astro Studio](#pushing-table-schemas)。这将添加新表并将旧表标记为已弃用。
4. 更新你的本地项目代码以使用新表而不是旧表。你可能还需要将数据迁移到新表。
5. 当你确信旧表格不再被你的项目使用时，可以从你的 `config.ts` 中移除该架构：
