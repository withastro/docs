...
在开发过程中，Astro 将使用你的 DB 配置并根据你的架构生成本地类型。这些内容将在每次启动开发服务器时从你都 seed 文件重新生成，并允许你使用类型安全和自动补全来查询和处理你的数据样例。
...
:::note
虽然远程连接在任何使用静态或服务器渲染模式的部署平台上都是可能的，但目前存在一些限制。非 Node 运行时（如 Cloudflare 和 Deno）不支持在 libSQL 上使用服务器渲染路由时使用 DB。这些平台的支持计划在未来的实现中。
:::
...
:::caution
__这将销毁你的数据库__。仅在您不需要生产数据时执行此命令。
:::
...
查看 [CLI 参考](/zh-cn/guides/integrations-guide/db/#stro-db-cli-参考) 以获取完整的命令列表。
