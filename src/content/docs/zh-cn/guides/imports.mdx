...
渲染 JSX/TSX 文件需要适当的 [UI 框架](/zh-cn/guides/framework-components/)（[React](/zh-cn/guides/integrations-guide/react/)、[Preact](/en/guides/integrations-guide/preact/) 或 [Solid](/zh-cn/guides/integrations-guide/solid-js/)）。在适当的情况下使用 `.jsx`/`.tsx` 扩展名，因为 Astro 不支持 `.js`/`.ts` 文件中的 JSX。
