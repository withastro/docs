...
我们鼓励 Astro 用户尽可能避免使用 Node.js 内置模块（`fs`、`path` 等）。Astro 兼容多个运行时使用 [适配器](/zh-cn/guides/server-side-rendering/#添加一个适配器)。这包括 [Deno](/zh-cn/guides/integrations-guide/deno/) 和 [Cloudflare Workers](/zh-cn/guides/integrations-guide/cloudflare/)，它们不支持 Node 内置模块，例如 `fs`。
