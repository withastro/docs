...
Astro 内置对 [TypeScript](https://www.typescriptlang.org/) 的支持。你可以在 Astro 项目中直接导入 `.ts` 和 `.tsx` 文件，甚至可以直接在 [Astro 组件脚本](/zh-cn/basics/astro-components/#the-component-script)和任意 [hoisted script 标签](/zh-cn/guides/client-side-scripts/) 中编写 TypeScript 代码。
...
`Astro.glob()` 只接受一个参数：一个相对的 [glob 模式](/zh-cn/guides/imports/#glob-patterns) 匹配你想要导入的本地文件。它是异步的，并返回每个匹配文件的导出数组。
...
使用 `Astro.glob` 导入的 Astro 组件的类型为 [`AstroInstance`](/zh-cn/reference/api-reference/#astro-files)。你可以使用其 `default` 属性渲染每个组件实例：
...
#### Astro 中的 Glob 模式 \{#glob-patterns-in-astro\}
...
## 扩展文件类型支持 \{#extending-file-type-support\}
