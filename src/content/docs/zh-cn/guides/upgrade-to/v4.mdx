...
需要查阅 v3 的文档？请访问这个[旧版本文档站 (不被维护的 v3.6 快照)](https://web.archive.org/web/20231203051122/https://docs.astro.build/en/getting-started/)。
...
如果需要的话，你也可以[手动升级 Astro 集成](/zh-cn/guides/integrations/#手动更新)，你可能还需要升级项目中的其他依赖。
...
如果你需要参考 v3.x 项目的文档，你可以浏览这个[(不再维护的) v4.0 之前的文档快照](https://web.archive.org/web/20231203051122/https://docs.astro.build/en/getting-started/)。
...
在 Astro v3.0 中，`build.split` 和 `build.excludeMiddleware` 构建配置项被弃用，它们被实现相同功能的[适配器配置选项](/zh-cn/reference/adapter-reference/#适配器特性)替代。
