...
- [Rollup 输出文件名配置路径](#changed-rollup-output-file-name-config-path-vite-config)
- [集成钩子和 HMR 访问模式](#changed-integration-hooks-and-hmr-access-patterns-integration-api)
- [`SSRManifest` 结构](#changed-ssrmanifest-interface-structure-adapter-api)
- [使用 `RouteData` 生成路由](#removed-routedatagenerate-adapter-api)
- [包含百分号编码的百分号路由（例如 `%25`）](#removed-percent-encoding-in-routes)
- [`astro:ssr-manifest` 虚拟模块](#removed-astrossr-manifest-virtual-module-integration-api)
- [`astro/app/node` 中的 `NodeApp`](#deprecated-nodeapp-from-astroappnode-adapter-api)
- [`astro/app/node` 中的 `loadManifest()` 和 `loadApp()`](#deprecated-loadmanifest-and-loadapp-from-astroappnode-adapter-api)
- [`createExports()` 和 `start()`](#deprecated-createexports-and-start-adapter-api)
...
你可以通过 [从 `astro/zod` 导入 Zod](#deprecated-astroschema-and-z-from-astrocontent) 来确保使用与 Astro 内部相同的 Zod 版本。
...
Astro v6.0 移除了这种自动的旧版内容集合支持，以及 [`legacy.collections` 标志](#removed-legacy-content-collections)。所有内容集合现在必须使用 [Astro v5.0 中引入的 Content Layer API](https://astro.build/blog/content-layer-deep-dive/)，它为所有内容集合提供支持。**不提供向后兼容性支持。**

#### 我应该做什么？

如果你在更新到 v6 后遇到内容集合错误，请[检查项目中是否有任何需要更新到 Content Layer API 的已移除旧版功能](#如果你有以下情况)。
