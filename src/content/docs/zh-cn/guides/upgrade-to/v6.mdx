...
<ReadMore>请参阅 [Astro v5 升级指南](/zh-cn/guides/upgrade-to/v5/#旧版v20-内容集合-api) 获取将旧版集合升级到新 Content Layer API 的详细说明。</ReadMore>
...
<ReadMore>了解更多关于 [可用的 session driver](/zh-cn/reference/session-driver-reference/#building-a-session-driver)。</ReadMore>
...
<ReadMore> 请参阅 [Astro v5 升级指南](/zh-cn/guides/upgrade-to/v5/#旧版v20-内容集合-api) 了解之前 Astro v5 中旧版集合向后兼容性的指导以及将旧版集合升级到新 Content Layer API 的完整分步说明。</ReadMore>
...
<ReadMore>了解更多关于 [表单的视图过渡动画](/zh-cn/guides/view-transitions/#transitions-with-forms)。</ReadMore>
...
<ReadMore>了解更多关于 [重写](/zh-cn/guides/routing/#重写)。</ReadMore>
...
<ReadMore>了解更多关于内容加载器 API 参考中的 [`createSchema()`](/en/reference/content-loader-reference/#loadercreateschema)。</ReadMore>
...
<ReadMore>了解更多关于 [Astro 配置文件](/zh-cn/guides/configuring-astro/#astro-配置文件)。</ReadMore>
...
- `preserveScriptOrder`（请参阅下面关于 [默认 `<script>` 和 `<style>` 行为](#变更script-和-style-标签按定义顺序渲染) 的破坏性变更。）
- `staticImportMetaEnv`（请参阅下面关于 [`import.meta.env`](#变更importmetaenv-值始终内联) 的破坏性变更。）
- `headingIdCompat`（请参阅下面关于 [Markdown 标题 ID 生成](#变更markdown-标题-id-生成) 的破坏性变更。）
...
<ReadMore>请参阅 [使用 `getImage()` 生成图片](/zh-cn/guides/images/#使用-getimage-生成图像) 获取完整示例。</ReadMore>
...
<ReadMore>了解更多关于 [标题 ID](/zh-cn/guides/markdown-content/#标题-id)。</ReadMore>
...
<ReadMore>了解更多关于 [定义加载器 schema 类型](/zh-cn/reference/content-loader-reference/#loader-对象)。</ReadMore>
