...
## Astro v2.0 破坏性更改 \{#astro-v20-breaking-changes\}
...
### 移除：对 Node 14 的支持 \{#removed-support-for-node-14\}
...
### 保留：`src/content/` \{#reserved-srccontent\}
...
### 变更：Astro.site 末尾斜线 \{#changed-astrosite-trailing-slash\}
...
### 变更：用于构建资源的 `_astro/` 文件夹 \{#changed-_astro-folder-for-build-assets\}
...
### 变更：Markdown 插件配置 \{#changed-markdown-plugin-configuration\}

#### 移除：`extendDefaultPlugins` \{#removed-extenddefaultplugins\}
...
#### 新增： `gfm` 和 `smartypants` \{#added-gfm-and-smartypants\}
...
### 变更：MDX 插件配置 \{#changed-mdx-plugin-configuration\}

#### 替换：`extendPlugins` 更改为 `extendMarkdownConfig` \{#replaced-extendplugins-changed-to-extendmarkdownconfig\}
...
#### 新增：更多的 MDX 配置选项以匹配 Markdown \{#added-more-mdx-config-options-to-match-markdown\}

Astro v2.0 现在允许你在 MDX 集成配置中分别设置 [每一个可用的 Markdown 配置选项](/zh-cn/reference/configuration-reference/#markdown-options)（除了 `drafts`）。
...
### 变更：插件对 frontmatter 的访问权限 \{#changed-plugin-access-to-frontmatter\}
...
### 变更：RSS 配置 \{#changed-rss-configuration\}
...
### 变更：Svelte IDE 支持 \{#changed-svelte-ide-support\}
...
### 移除：`legacy.astroFlavoredMarkdown` \{#removed-legacyastroflavoredmarkdown\}
...
### 移除：`Astro.resolve()` \{#removed-astroresolve\}
...
### 移除：`Astro.fetchContent()` \{#removed-astrofetchcontent\}
...
### 移除：`Astro.canonicalURL` \{#removed-astrocanonicalurl\}
...
## Astro v2.0 实验性标志已移除 \{#astro-v20-experimental-flags-removed\}
