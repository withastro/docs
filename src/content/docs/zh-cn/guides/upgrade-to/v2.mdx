...
2. 查看您的 [部署环境](/en/guides/deploy/) 的文档，以确认它们是否支持 Node 16。
...
Astro v2.0 现在包含了用于将Markdown和MDX文件整理到 [内容集合](/en/guides/content-collections/) 中的Collections API。此API将`src/content/`设定为一个特殊的文件夹。

#### 我应该怎么做？

重命名现有的`src/content/`文件夹以避免冲突。 如果该文件夹存在，现在只能用于 [内容集合](/en/guides/content-collections/).
...
- **[`mdx.extendMarkdownConfig`](/en/guides/integrations-guide/mdx/#extendmarkdownconfig)** 继承全部或不继承你的 Markdown 配置
