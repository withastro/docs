...
需要将旧项目升级到 v1 吗？请参阅我们的 [旧版本迁移指南](/en/guides/upgrade-to/v1/).
...
2. 查看您的 [部署环境的](/en/guides/deploy/) 文档，以确认它们是否支持 Node 16。
...
Astro v2.0 现在包含了用于将Markdown和MDX文件整理到 [内容集](/en/guides/content-collections/) 中的Collections API。此API将`src/content/`设定为一个特殊的文件夹。

#### 我应该做什么？

重命名现有的src/content/文件夹以避免冲突。 如果该文件夹存在，现在只能用于 [内容收集](/en/guides/content-collections/).
...
#### 我应该做什么？
 ...
 在`astro.config.mjs`中，为`site`设置的URL添加末尾斜线。
...
您可以通过 [新的 `build.assets` 配置项](/en/reference/configuration-reference/#buildassets) 来控制这个位置。
...
Astro v2.0完全移除了这个配置选项，因为它已经变为默认设置。

在您的Markdown配置中应用remark和rehype插件  **不再禁用Astro的默认插件**. 无论是否配置了自定义 `remarkPlugins` 或 `rehypePlugins`，现在都将应用GitHub风格的Markdown和Smartypants。


##### 我应该做什么？

在您的配置中移除 `extendDefaultPlugins`。 这已经成为Astro v2.0的默认行为，您可以删除这一行，无需任何替换。
...
- **[`mdx.extendMarkdownConfig`](/en/guides/integrations-guide/mdx/#extendmarkdownconfig)** 用于继承你的全部或无 Markdown 配置
...
Astro v2.0 现在允许您在 MDX 集成配置中分别设置 [每一个可用的 Markdown 配置选项](/en/reference/configuration-reference/#markdown-options)（除了 `drafts`）。
...
如果你正在使用 [`@astrojs/svelte` 集成](/en/guides/integrations-guide/svelte/)，Astro v2.0 需要在你的项目中有一个 `svelte.config.js` 文件。这是为了提供 IDE 的自动补全功能。
...
如果你在 v1.x 版本中使用了这个特性，我们推荐 [使用 MDX 集成](/en/guides/integrations-guide/mdx/)，它允许你将组件和 JSX 表达式与 Markdown 语法结合起来。
...
使用 [`Astro.glob()`](/en/guides/imports/#astroglob) 方法获取 Markdown 文件，或者转换为 [内容集合](/en/guides/content-collections/) 功能。
...
- [内容集合](/en/guides/content-collections/)，用于以类型安全的方式管理你的 Markdown 和 MDX 文件。
- 在使用 SSR 时，将个别页面[预渲染为静态 HTML](/en/guides/server-side-rendering/#configuring-individual-routes)，以提高速度和缓存能力。详见 配置个别路由。
- 重新设计的错误消息覆盖层。
