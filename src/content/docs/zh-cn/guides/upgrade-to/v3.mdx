...
## Astro v3.0 实验性标志已移除 \{#astro-v30-experimental-flags-removed\}
...
- 视图过渡动画用于动画页面过渡和持久化组件。如果你使用此实验性标志，请查看 [视图过渡动画 API 重大更改及升级建议](#upgrade-view-transitions-to-v3)。
- 新的图像服务 API `astro:assets` 用于在 Astro 中使用图像，包括新的 `<Image />` 组件和 `getImage()` 函数。**无论你是否使用此实验性标志**，请阅读详细的 [图像升级建议](#upgrade-images-to-v3) ，以了解这可能如何影响你的项目。

在 [3.0 博客文章](https://astro.build/blog/astro-3/) 阅读更多关于这两个令人兴奋的功能和更多其他内容！

## Astro v3.0 破坏性更改 \{#astro-v30-breaking-changes\}
...
### 移除：对 Node 16 的支持 \{#removed-support-for-node-16\}
...
### 移除：对 TypeScript 4 的支持 \{#removed-support-for-typescript-4\}
...
### 移除：`@astrojs/image` \{#removed-astrojsimage\}
...
#### 我应该怎么做？ \{#what-should-i-do-2\}

移除 `@astrojs/image` 集成。你不仅需要卸载集成，还需要更新或删除任何导入语句和现有的 `<Image />` 和 `<Picture />` 组件。你可能还需要配置一个首选的默认图像处理服务。

你可以在我们的图像指南中找到 [完整的、逐步的删除旧图像集成的说明](#remove-astrojsimage)。

迁移 `astro:assets` 还将带来一些你现在可能希望使用的新的图像选项和功能。请参阅完整的 [v3.0 图像升级建议](#upgrade-images-to-v3) 了解完整的详情！
...
### 移除：`<Markdown />` 组件 \{#removed-markdown--component\}
...
否则，请删除所有对导入 Astro 的 `<Markdown />` 组件及其本身的引用。你需要直接将你的内容重写为 HTML，或者从 `.md` 文件中[导入 Markdown](/zh-cn/guides/markdown-content/#importing-markdown)。

### 移除：已废弃的 1.x API \{#removed-deprecated-1x-apis\}
...
### 移除：服务器代码中 Web API 的不完整垫片（shim） \{#removed-partial-shims-for-web-apis-in-server-code\}
...
#### 我应该怎么做？ \{#what-should-i-do-5\}

如果你在服务器渲染的组件中使用 Web API，你需要把使用了的地方改成根据运行平台选择性调用或使用 [`client:only` 客户端指令](/zh-cn/reference/directives-reference/#clientonly).

### 移除：内容集合 schema 中来自 `astro:content` 的 `image` \{#removed-image-from-astrocontent-in-content-collections-schema\}

在 Astro v2.x 中，内容集合 API 废弃了来自 `astro:content` 的 `image` 导出，用于在你的内容集合 schema 中使用。

Astro v3.0 完全删除了这个导出。

#### 我应该怎么做？ \{#what-should-i-do-6\}

如果你正在使用 `astro:content` 中废弃的 `image()`，请删除它，因为它已经不存在了。请通过 [来自 `schema` 的 `image` 帮助程序](#update-content-collections-schemas) 来验证图像。
...
### 移除：pre-0.14 Shiki 主题名称 \{#removed-pre-014-shiki-theme-names\}

在 Astro v2.x 中，一些 Shiki 主题名称已被重命名，但原始名称仍保留了向后兼容性。

Astro v3.0 删除了原始名称，而采用了重命名的主题名称。

#### 我应该怎么做？ \{#what-should-i-do-7\}
...
### 移除：`class:list` 功能 \{#removed-classlist-features\}
...
### 移除：将 `class:list` 作为 prop 传递 \{#removed-passing-classlist-as-a-prop\}
...
### 移除：对于 camelCase CSS 变量的 kebab-case 转换 \{#removed-kebab-case-transform-for-camelcase-css-variables\}

在 Astro v2.x 中，传递给 `style` 属性的 camelCase [CSS 变量](/zh-cn/guides/styling/#css-variables) 会被渲染为 camelCase（如写入的那样）和 kebab-case（保留向后兼容性）。
...
### 移除：自动展平 `getStaticPaths()` 的返回值 \{#removed-automatic-flattening-of-getstaticpathss-return-value\}

在 Astro v2.x 中，`getStaticPaths()` 的返回值会自动展平，以允许返回一个二维数组而不会出错。

Astro v3.0 移除了对 `getStaticPaths()` 返回值的自动展平。

#### 我应该怎么做？ \{#what-should-i-do-11\}
...
### 移动：`astro check` 现在需要一个外部包 \{#moved-astro-check-now-requires-an-external-package\}
...
#### 我应该怎么做？ \{#what-should-i-do-12\}

在升级 Astro v3.0 后运行 `astro check` 命令，并按照提示安装所需的依赖项，或者手动将 `@astrojs/check` 和 `typescript` 安装到你的项目中。

### 废弃：`build.excludeMiddleware` 和 `build.split` \{#deprecated-buildexcludemiddleware-and-buildsplit\}

在 Astro v2.x 中，`build.excludeMiddleware` 和 `build.split` 用于在 SSR 模式下使用适配器时更改特定文件的产出方式。

Astro v3.0 用新的 [SSR 适配器配置选项](/zh-cn/guides/integrations-guide/#official-integrations) 替换了这些构建配置选项，以执行相同的任务：`edgeMiddleware` 和 `functionPerRoute`。

#### 我应该怎么做？ \{#what-should-i-do-13\}
...
### 废弃：`markdown.drafts` \{#deprecated-markdowndrafts\}
...
#### 我应该怎么做？ \{#what-should-i-do-14\}

为了继续将你项目中的某些页面标记为草稿，请 [迁移到内容集合](/zh-cn/guides/content-collections/#migrating-from-file-based-routing)，并使用 `draft: true` frontmatter 属性 [手动过滤页面](/zh-cn/guides/content-collections/#filtering-collection-queries)。

### 废弃：在端点（endpoints）中返回简单对象 \{#deprecated-returning-simple-object-in-endpoints\}

在 Astro v2.x 中，端点（endpoints）可以返回一个简单的对象，它会被转换为一个 JSON 响应。

Astro v3.0 废弃了这种行为，而是直接返回一个 `Response` 对象。

#### 我应该怎么做？ \{#what-should-i-do-15\}
...
### 默认值改变：tsconfig.json 预设里的 `verbatimModuleSyntax` \{#changed-default-verbatimmodulesyntax-in-tsconfigjson-presets\}
...
### 默认值改变：`compressHTML` \{#changed-default-compresshtml\}
...
### 默认值改变：`scopedStyleStrategy` \{#changed-default-scopedstylestrategy\}
...
#### 我应该怎么做？ \{#what-should-i-do-20\}

为了保留你项目的当前 [样式作用域](/zh-cn/guides/styling/#scoped-styles)，请修改配置文件为以前的默认值：
...
### 默认值改变：`inlineStyleSheets` \{#changed-default-inlinestylesheets\}
...
### 默认值改变：图像服务 \{#changed-default-image-service\}

在 Astro v2.x 中，Squoosh 是[默认的图像处理服务](/zh-cn/guides/images/#default-image-service)。

Astro v3.0 将 Sharp 作为默认的图像处理服务，并提供了一个配置选项来使用 Squoosh。

#### 我应该怎么做？ \{#what-should-i-do-22\}
...
### 修改：HTTP 请求方法案例 \{#changed-http-request-methods-case\}

在 Astro v2.x 中，[HTTP 请求方法](/zh-cn/guides/endpoints/#http-methods) 使用小写函数名称编写：`get`、`post`、`put`、`all` 和 `del`。

Astro v3.0 使用大写函数名称，使用 `DELETE` 而不是 `del`。

#### 我应该怎么做？ \{#what-should-i-do-23\}
...
### 修改：多个 JSX 框架配置 \{#changed-multiple-jsx-framework-configuration\}
...
### 修改：`Astro.cookies.get(key)` 可以返回 `undefined` \{#changed-astrocookiesgetkey-can-return-undefined\}
...
### 修改：以编程方式运行 Astro CLI \{#changed-running-the-astro-cli-programmatically\}
...
### 修改：内部 Astro API 入口文件导出路径 \{#changed-internal-astro-api-entry-point-export-paths\}
...
#### 从 `experimental.assets` 升级 \{#upgrade-from-experimentalassets\}

如果你之前启用了 `astro:assets` 的实验性标志，你需要更新你的项目到 Astro v3.0，它现在默认包含了 assets 功能。

##### 移除 `experimental.assets` 标志 \{#remove-experimentalassets-flag\}
...
##### 移除`~/assets` 导入别名 \{#remove-the-assets-import-alias\}

这个导入别名不再默认包含在 `astro:assets` 中。如果你之前使用这个别名，你必须将它们转换为相对文件路径，或者 [创建你自己的导入别名](/zh-cn/guides/imports/#aliases)。
...
##### 为 Cloudflare, Deno, Vercel Edge 和 Netlify Edge 添加简单的资源支持 \{#add-simple-asset-support-for-cloudflare-deno-vercel-edge-and-netlify-edge\}
...
#### 决定在哪里存储你的图像 \{#decide-where-to-store-your-images\}

请参阅 [图像指南](/zh-cn/guides/images/#where-to-store-images) 来帮助你决定在哪里存储你的图像。你可能希望利用 `astro:assets` 带来的灵活性，来存储你的图像的新选项。例如，现在可以使用标准的 Markdown `![alt](src)` 语法，在 Markdown、MDX 和 Markdoc 中引用项目 `src/` 中的相对图像。

#### 更新现有的 `<img>` 标签 \{#update-existing-img-tags\}
...
你必须更新任何现有 `<img>` 标签的 `src` 属性（包括任何 [UI 框架组件中的图像](/zh-cn/guides/images/#images-in-ui-framework-components)），你也可以更新从导入的图像中获得的其他属性。
...
#### 更新你的 Markdown, MDX, 和 Markdoc 文件 \{#update-your-markdown-mdx-and-markdoc-files\}
...
#### 移除 `@astrojs/image` \{#remove-astrojsimage\}
...
1. 移除 `@astrojs/image` 集成。
    ...
    你必须通过卸载并从你的 `astro.config.mjs` 文件中删除它，来 [移除集成](/zh-cn/guides/integrations-guide/#removing-an-integration)。
...
3. 迁移任何现有的 `<Image />` 组件。
    ...
    移除任何当前不[支持的图像属性](/zh-cn/guides/images/#properties)。
...
#### 更新内容集合结构 \{#update-content-collections-schemas\}

你现在可以在 frontmatter 中声明一个与内容集合条目相关联的图像，例如博客文章的封面图像，使用相对于当前文件夹的路径：

内容集合中新的 `image` 助手可让你使用 Zod 验证图像元数据。了解更多关于 [如何在内容集合中使用图像](/zh-cn/guides/images/#images-in-content-collections) 的内容。

#### 在 Astro v3.0 中导航图像导入 \{#navigating-image-imports-in-astro-v30\}
...
### 升级视图过渡动画到 v3 \{#upgrade-view-transitions-to-v3\}
...
#### 从 `experimental.viewTransitions` 升级 \{#upgrade-from-experimentalviewtransitions\}

如果你之前启用了视图过渡动画的实验性标志，那么你需要更新你的项目到 Astro v3.0，因为现在 Astro 默认支持视图过渡动画。

##### 移除 `experimental.viewTransitions` 标志 \{#remove-experimentalviewtransitions-flag\}
...
##### 更新 `transition:animate` 指令 \{#update-transitionanimate-directives\}
