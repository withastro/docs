...
Astro 内置了对 [TypeScript](https://www.typescriptlang.org/) 的支持。你可以在 Astro 项目中导入 `.ts` 和 `.tsx` 文件，甚至可以直接在 [Astro 组件](/zh-cn/basics/astro-components/#the-component-script) 中编写 TypeScript 代码。如果你愿意，你甚至可以使用 [`astro.config.ts`](/zh-cn/guides/configuring-astro/#the-astro-config-file)。

使用 TypeScript，你可以通过约束代码中对象和组件的类型来防止运行时的错误。例如，如果你使用了 TypeScript 来[定义组件参数](#component-props)；而如果你在使用时不小心传入了一个与之不匹配的参数，编辑器将会报错。
...
Astro 的开发服务器不会进行任何类型检查，但你可以编辑 [`package.json` 中的 `scripts` 属性](#type-checking)来通过命令行工具检查代码中的类型错误。
...
### TypeScript 模板 \{#typescript-templates\}
...
### TypeScript 编辑器插件 \{#typescript-editor-plugin\}
...
Astro 支持你在 `tsconfig.json` 文件里的 `paths` 配置所定义的 [导入别名](/zh-cn/guides/imports/#aliases)。
...
## 扩展 `window` 和 `globalThis` \{#extending-window-and-globalthis\}
...
### `ComponentProps` 类型 \{#componentprops-type\}
...
下面的示例实现了一个完全类型化的多态组件，可以渲染为任何 HTML 元素。使用 [`HTMLTag`](#built-in-html-attributes) 类型来确保 `as` 属性是一个有效的 HTML 元素。
...
### 推断 `getStaticPaths()` 的类型 \{#infer-getstaticpaths-types\}
...
### 同时使用多个 JSX 框架所带来的类型错误 \{#errors-typing-multiple-jsx-frameworks-at-the-same-time\}
