...
:::caution
`.env` 文件不会加载到[配置文件](/en/guides/configuring-astro/#environment-variables)中。
:::
...
- `import.meta.env.ASSETS_PREFIX`（`string`）：如果设置了 [`build.assetsPrefix` 配置项](/zh-cn/reference/configuration-reference/#buildassetsprefix)，则指定 Astro 生成的资源链接的前缀。可以用于创建不由Astro处理的资源链接。
...
```
更多关于`.env` 文件的信息，[请参阅 Vite 文档](https://vitejs.dev/guide/env-and-mode.html#env-files)。
...
在 Vite 中，你可以使用 `import.meta.env`，而不是使用 `process.env`，它使用了 ES2020 中添加的 `import.meta` 功能。
