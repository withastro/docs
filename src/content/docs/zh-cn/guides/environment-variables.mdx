...
:::tip[要找 astro&#58;env\?]
了解更多关于类型安全环境变量的 [实验性 `astro:env` API](/zh-cn/reference/configuration-reference/#experimentalenvvalidatesecrets) 的信息！
:::

Astro 使用 Vite 内置支持的环境变量，这些变量在构建时会被静态替换，你可以[使用 Vite 的任一方法](https://cn.vitejs.dev/guide/env-and-mode.html)来处理它们。
...
:::caution
`.env` 文件不会加载到[配置文件](/zh-cn/guides/configuring-astro/#environment-variables)中。
:::
...
## 获取环境变量 \{#getting-environment-variables\}
