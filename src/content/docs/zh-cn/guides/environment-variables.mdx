...
- `import.meta.env.BASE_URL`（`string`）：为站点提供服务的基础url。它由 [`base` 配置项](/zh-cn/reference/configuration-reference/#base) 决定。
...
<PackageManagerTabs>
 <Fragment slot="yarn">
    ```shell
    POKEAPI=https://pokeapi.co/api/v2 yarn dev
    ```
 </Fragment>
 <Fragment slot="npm">
    ```shell
 ...
 <Fragment slot="pnpm">
    ```shell
    POKEAPI=https://pokeapi.co/api/v2 pnpm dev
    ```
 </Fragment>
</PackageManagerTabs>
