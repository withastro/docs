...
不同于 [`cookies`](/en/guides/on-demand-rendering/#cookies)，因为会话存储在服务端，所以对于存储更大体积的数据或是安全问题，你都无需担忧。会话存储适用于用户信息、购物车内容以及表单状态，且无需客户端 JavaScript：
...
:::note
边缘中间件（edge middleware）目前暂不支持会话功能。
:::
