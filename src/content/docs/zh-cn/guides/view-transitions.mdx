...
-  `astro-data-history="auto | push | replace"`：一个 `<a>` 标签属性，用于 [控制浏览器的历史记录](#replace-entries-in-the-browser-history)。
-  `navigate(href, options)`：任何客户端脚本或客户端组件可用的方法，用于[触发导航](#trigger-navigation)。

### 阻止客户端导航

由于关联的两个页面须使用 `<ViewTransitions />` 路由以防止重新加载全页面，因此在某些情况下你无法通过 CSR 进行导航。你可能也不希望在每次导航更改时都使用客户端路由，而是更希望在特定路由上进行传统的页面导航。

你可以通过将 `data-astro-reload` 属性添加到任何 `<a>` 标签中，按链接的方式选择退出客户端路由。此属性将覆盖任何现有的 `<ViewTransitions />` 组件，而是在导航期间触发浏览器刷新。

以下示例显示了仅从主页导航到文章时如何阻止客户端路由。这样，当从文章列表页面导航到同一页面时，仍然可以对共享元素（例如英雄图像）进行动画处理：
...
`<ViewTransitions />` 组件包含一个用于在客户端路由间进行页面导航的路由声明器。你无需配置或操作即可启用此功能。

辅助技术通过在导航后声明新页面标题来告知访客页面已被更改。在使用传统的全页面浏览器刷新服务端路由时，则会默认在新页面加载后执行此操作。而在客户端路由中，则由 `<ViewTransitions />` 组件执行此操作。

为在客户端路由中添加路由声明，该组件会向新页面添加一个带有 `aria-live` 属性设置为 `assertive` 的元素。这告诉 AT（assistive technology，辅助技术）立即声明。该组件还按照优先顺序检查以下内容来确定声明的文本：

-  如果存在 `<title>` 标签，则使用该标签。
-  使用第一个找到的 `<h1>` 标签。
-  使用页面的 `pathname`。

我们强烈建议你在每个页面中始终包含 `<title>` 标签以更好地支持无障碍。
