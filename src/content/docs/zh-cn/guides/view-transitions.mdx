...
只有关联的两个页面都使用了 `<ViewTransitions />` 路由器才能通过 CSR (client-side routing，客户端路由) 进行导航，防止重新加载全页面。你可能也不希望在每次导航更改时都使用客户端路由，而是更希望在特定路由上进行传统的页面导航。

你可以通过将 `data-astro-reload` 属性添加到任何 `<a>` 标签中，为单个链接选择不使用客户端路由器。此属性将覆盖任何现有的 `<ViewTransitions />` 组件，而是在导航期间触发浏览器刷新。
