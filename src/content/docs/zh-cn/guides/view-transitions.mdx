...
Astro 提供了一个 `<ViewTransitions />` 路由组件，可以添加到单个页面的 `<head>` 中，以控制页面在导航到另一个页面时的过渡效果。它提供了一个轻量级的客户端路由器，[拦截导航](#client-side-navigation-process) 并允许你自定义页面之间的过渡效果。

将此组件添加到一个可复用的 `.astro` 组件中，例如公共头部或布局组件，以便 [在整个站点上实现动画页面过渡（SPA 模式）](#full-site-view-transitions-spa-mode)。

Astro 的视图过渡动画支持由新的 [View Transitions](https://developer.chrome.com/docs/web-platform/view-transitions/) 浏览器 API 提供，还包括：

- 一些 [内置的动画选项](#built-in-animation-directives)，例如 `fade`、`slide` 和 `none`。
- 支持前进和后退导航动画；
- 可完全 [自定义过渡动画](#customizing-animations) 的能力，可以构建自己的动画效果；
- 用于 [阻止非页面链接的客户端导航](#preventing-client-side-navigation) 的选项。
- 对于尚未支持视图过渡动画 API 的浏览器，可以 [控制回退行为](#fallback-control)。
...
## 向页面添加视图过渡动画 \{#adding-view-transitions-to-a-page\}
...
## 整站视图过渡动画（SPA 模式） \{#full-site-view-transitions-spa-mode\}
...
对于更精细的控制，可以在单个元素上使用 [过渡动画指令](#transition-directives) 或 [覆盖默认的客户端导航](#preventing-client-side-navigation)。

## 过渡动画指令 \{#transition-directives\}
...
- `transition:name`：允许你覆盖 Astro 的默认元素匹配方式，为一对 DOM 元素 [指定过渡动画名称](#naming-a-transition)。
- `transition:animate`：在用新元素替换旧元素时，允许你通过指定动画类型来覆盖 Astro 的默认动画效果。可以使用 Astro 的 [内置动画指令](#built-in-animation-directives) 或 [创建自定义动画](#customizing-animations)。
- `transition:persist`: 允许你覆盖 Astro 的默认行为。不同于在导航到另一个页面时用将替换旧元素为新元素的方式，该行为则是在导航到另一个页面时 [保留组件和 HTML 元素的状态](#maintaining-state)。

### 命名过渡动画 \{#naming-a-transition\}
...
你也可以将指令放在一个 [Astro 群岛](/zh-cn/concepts/islands/) 上（即带有 [`client:` 指令](/zh-cn/reference/directives-reference/#client-directives) 的 UI 框架组件）。如果该组件存在于下一个页面上，旧页面上的岛屿与其**当前状态**将继续显示，而不是用新页上的岛屿替换它。
...
如果岛屿（或元素）在两个页面之间位于不同的组件中，你也可以[手动标识相应的元素](#naming-a-transition)。
...
### 内置动画指令 \{#built-in-animation-directives\}
...
-  `data-astro-reload`：一个 `<a>` 标签属性，用于[进行强制全页面导航](#preventing-client-side-navigation)。
-  `data-astro-history="auto | push | replace"`：一个 `<a>` 标签属性，用于[控制浏览器的历史记录](#replace-entries-in-the-browser-history)。
-  `navigate(href, options)`：任何客户端脚本或客户端组件可用的方法，用于[触发导航](#preventing-client-side-navigation)。

### 阻止客户端导航 \{#preventing-client-side-navigation\}
...
### 触发导航 \{#trigger-navigation\}

你还可以使用 `navigate` 方法在通常不被 `<ViewTransitions />` 路由器监听的事件里触发客户端导航。该函数来自于 `astro:transitions/client` 模块，可以在任何客户端脚本和通过[客户端指令](/zh-cn/reference/directives-reference/#client-directives)激活了的客户端组件中使用。
...
### 替换浏览器历史条目 \{#replace-entries-in-the-browser-history\}
...
`data-astro-history` 属性可以设置为与 [`navigate()` 函数的 `history` 选项](#trigger-navigation) 相同的三个值：
...
### 表单的视图过渡动画 \{#transitions-with-forms\}
...
## 客户端导航流程 \{#client-side-navigation-process\}
...
## 在视图过渡中的脚本行为 \{#script-behavior-with-view-transitions\}
...
### 脚本的重新执行 \{#script-re-execution\}

[Astro 默认的打包模块脚本](/zh-cn/guides/client-side-scripts/#script-processing)在初始执行之后将只会被执行一次。即使在页面过渡后，该脚本存在于新页面上，它也会被忽略。

与打包的模块脚本不同，[内联脚本](/zh-cn/guides/client-side-scripts/#opting-out-of-processing) 在用户访问网站期间，如果它们存在于被多次访问的页面上，就有可能被重新执行。当访客导航到一个没有该脚本的页面，然后再回到一个有该脚本的页面时，内联脚本也可能会重新执行。
...
为了确保脚本在客户端导航期间每次加载页面时都能运行，它应该由一个[生命周期事件](#lifecycle-events)执行。例如，`DOMContentLoaded` 的事件监听器可以被 [`astro:page-load`](/zh-cn/guides/view-transitions/#astropage-load) 生命周期事件替换。
...
有关在项目中更新现有脚本的示例，请参阅 [添加视图过渡动画教程](/zh-cn/tutorials/add-view-transitions/#update-scripts)。
...
#### 构建自定义交换函数 \{#building-a-custom-swap-function\}
