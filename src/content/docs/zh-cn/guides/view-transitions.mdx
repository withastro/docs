...
-  支持前置和后置导航动画；
-  可完全自定义过渡动画的能力，可以构建自己的动画效果；
-  对于尚未支持视图过渡动画 API 的浏览器，可以控制回退行为。
...
-  `slide`：旧内容从左侧滑出，新内容从右侧滑入的动画效果；在后置导航时，动画效果则相反。
...
您还可以通过定义前置和后置行为以及新旧页面来为 `transition:animate` 定义自己的动画，具体类型如下所示：
...
您可以通过在 `<ViewTransitions />` 组件上添加一个名为 `fallback` 的属性，并将其设置为 `swap` 或 `none`，来覆盖 Astro 的默认回退支持：

-  `animate`（默认，推荐使用）- Astro 将在更新页面内容之前使用自定义属性模拟视图过渡动画。
-  `swap` - Astro 不会尝试进行页面动画效果。相反，旧页面将立即被新页面替换。
-  `none` - Astro 将不进行任何页面过渡动画。在不支持的浏览器中，您将获得全量的页面导航。
...
如果您有设置全局状态的代码，那么该状态需要考虑到该脚本可能会执行多次的情况。在 `<script>` 标签中检查全局状态，并在可能的情况下有条件地执行您的代码：
...
模块脚本只会执行一次，因为浏览器会跟踪已经加载的模块。所以对于这些脚本，您无需担心重新执行的问题。

### `astro:load`

这是一个在页面导航时触发的事件，您可以在 `document` 上监听此事件。

`<ViewTransitions />` 组件在初始页面导航（MPA）和后续的前后置导航中都会触发此事件。

您可以利用此事件在每次页面导航时运行代码，或者仅在全局执行一次：
...
这是一个在新页面替换旧页面后立即触发的事件。您可以在 `document` 上监听此事件。

此事件非常适用于恢复需要在 DOM 中传递到新页面的任何状态。

例如，如果您正在实现暗模式支持，可以使用此事件在页面加载时恢复状态：
...
Astro 的 `<ViewTransitions />` 组件包含一个 CSS 媒体查询，当检测到 `prefer-reduced-motion` 设置时，禁用所有视图过渡动画，包括回退动画。相反，浏览器将以无动画的方式直接交换 DOM 元素。
