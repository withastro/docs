...
Astro 的视图过渡动画和客户端路由支持由 [View Transitions 浏览器 API](https://developer.chrome.com/docs/web-platform/view-transitions/) 提供支持，并且还包括：
...
## 浏览器原生视图过渡动画与 Astro 的 `<ClientRouter />` 之间的差异

[浏览器原生的跨文档视图过渡动画](https://developer.mozilla.org/zh-CN/docs/Web/API/View_Transition_API/Using#基本_mpa_视图过渡) 可以在 Astro 中用于为多页面应用（MPA）中文档之间的导航添加动画，常用于提供类似单页面应用的客户端路由体验。它们不会改变多页面应用的核心功能，也不会影响任何现有脚本或在页面加载时添加额外的 JavaScript。它们只是添加动画效果。

对于 View Transition API 尚未完全支持的增强型客户端路由和视图过渡功能，Astro 提供了一个内置的轻量级组件，用于启用客户端路由并将你的多页应用转换为具有平滑导航动画的[单页面应用](#启用视图过渡动画spa-模式)。
...
然而，随着浏览器 API 和 web 标准的不断发展，使用 Astro 的 `<ClientRouter />` 来实现这些附加功能 [将越来越变得不必要](https://astro.build/blog/future-of-astro-zero-js-view-transitions/)。我们建议你关注浏览器 API 的当前状态，以便你可以根据你使用的特定功能[决定是否仍然需要 Astro 的客户端路由](https://events-3bg.pages.dev/jotter/astro-view-transitions/)。
...
<ReadMore>如果将用户输入与 `navigate()` API 一起使用，请考虑启用 Astro 的 [实验性内容安全策略功能](/zh-cn/reference/configuration-reference/)，来帮助防范跨站脚本攻击（XSS）的风险。</ReadMore>
...
Astro 的 `<ClientRouter />` 组件包含一个 CSS 媒体查询，当检测到 [`prefers-reduced-motion`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/@media/prefers-reduced-motion) 设置时，禁用所有视图过渡动画，包括回退动画。相反，浏览器将不使用动画，简单直接的切换 DOM 元素。
