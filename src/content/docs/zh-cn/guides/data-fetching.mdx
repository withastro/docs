...
此外，你可以使用 `new URL("/api", Astro.url)` 构建一个 URL，指向你的项目中按需在服务器上渲染的页面和端点。
...
## 在框架组件中使用 `fetch()` \{#fetch-in-framework-components\}
...
## 从无头 CMS 中获取 \{#fetch-from-a-headless-cms\}

Astro 组件可以从你最喜欢的 CMS 中获取数据，然后将其呈现为页面内容。使用[动态路由](/zh-cn/guides/routing/#dynamic-routes)，组件甚至可以根据你的 CMS 内容生成页面。
