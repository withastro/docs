...
这是传递给 API 路由的相同的 [`context`](/zh-cn/reference/api-reference/#endpoint-context)  对象。
...
此 `locals` 对象在请求处理过程中被传递，并作为 [`APIContext`](/zh-cn/reference/api-reference/#endpoint-context) 和 `AstroGlobal` 的属性可用。这允许在中间件，API路由，和 `.astro` 页面之间共享数据。这对于在渲染步骤中存储请求特定的数据（例如用户数据）很有用。
...
一个用于创建 [`APIContext`](/zh-cn/reference/api-reference/#endpoint-context) 的底层 API，Astro 中间件可以使用它。
