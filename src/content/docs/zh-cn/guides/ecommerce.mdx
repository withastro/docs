...
一些支付处理服务（例如 [Lemon Squeezy](#lemon-squeezy)、[Paddle](#paddle)）添加了支付表单，允许你的客户在你的网站上进行购买。这些可以是托管的覆盖层或嵌入在你网站的某个页面中。它们可能提供一些基本的自定义或网站品牌化，并且可以作为脚本、按钮或外部链接添加到你的 Astro 项目中。
...
1. 将以下 `<script>` 标签添加到页面的 `head` 或 `body` 中：
    ...
    ```
2. 在页面上创建一个锚点标签，链接到你的产品 URL。点击时包含类 `lemonsqueezy-button` 以打开结账覆盖层。
...
Lemon.js 还提供了额外的行为，例如[以编程方式打开覆盖层](https://docs.lemonsqueezy.com/help/lemonjs/opening-overlays)和[处理覆盖层事件](https://docs.lemonsqueezy.com/help/lemonjs/opening-overlays)。

<ReadMore> 阅读 [Lemon Squeezy 开发者入门指南](https://docs.lemonsqueezy.com/guides/developer-guide)了解更多信息。</ReadMore>

### Paddle

[Paddle](https://www.paddle.com/) 是一种数字产品和服务的计费解决方案。它通过一个覆盖层或内联结账处理支付、税务和订阅管理。
...
在 Paddle 批准你的默认支付链接域名（你自己的网站）之后，你可以将页面上的任何元素通过使用 HTML 数据属性转变为触发结账覆盖层的触发器。
...
2. 通过添加 `paddle_button` 类，将页面上的任何元素变成 Paddle 结账按钮：
    ...
    ```
3. 添加一个 `data-items` 属性来指定你的产品的 Paddle `priceID` 和 `quantity`。你也可以选择性地传递额外的[支持的 HTML 数据属性](https://developer.paddle.com/paddlejs/html-data-attributes)来预填充数据、处理结账成功，或者设置你的按钮和结账覆盖层的样式：
...
与其通过 HTML 数据属性传递数据，不如使用 JavaScript 向结账覆盖层发送数据，以便传递多个属性并实现更大的自定义化。你还可以使用内联结账创建升级工作流程。
...
[Snipcart](https://snipcart.com/) 是一个强大的，以开发者为首的 HTML/JavaScript 购物车平台。
...
4. 在任何 HTML 元素上添加 `class="snipcart-add-item"` 属性，例如 `<button>`，点击时可以将商品添加到购物车。同时包括其他数据元素，用于 [Snipcart 常见商品属性](https://docs.snipcart.com/v3/setup/products) 如价格和描述，以及任何可选字段。
...
6. 添加一个带有 `snipcart-checkout` 类的 Snipcart 结账按钮，以打开购物车并允许访客通过结账模态框完成购买。
