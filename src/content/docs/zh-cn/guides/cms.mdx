...
<ReadMore> 探索 [超过 100 个无头内容管理系统的列表](https://jamstack.org/headless-cms/) <Badge variant="accent">外部</Badge>，在这里你可以按类型（例如，基于 Git、API 驱动）和许可证（开源或闭源）进行筛选。</ReadMore>

## 我可以在没有 CMS 的情况下使用 Astro 吗？

当然！Astro 提供了 [创作内容](/zh-cn/guides/content/) 的内置方法，包括对 Markdown 页面的支持。
