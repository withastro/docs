...
Astro 通过使用一个 [中间件](/zh-cn/guides/middleware/) 来实现其路由逻辑。这个中间件函数被放置在 [第一个位置](/zh-cn/guides/middleware/#chaining-middleware)，它会在最终执行自己的逻辑之前，等待来自任何额外中间件和每个页面路由的每一个 `Response`。
...
## 配置 i18n 路由 \{#configure-i18n-routing\}
...
### 创建本地化文件夹 \{#create-localized-folders\}
...
配置了 i18n 路由后，你现在可以使用诸如 [`astro:i18n` 模块](/zh-cn/reference/api-reference/#internationalization-astroi18n) 中提供的 [`getRelativeLocaleUrl()`](/zh-cn/reference/api-reference/#getrelativelocaleurl) 辅助函数来计算站点内页面的链接。这些生成的链接将始终提供正确的、本地化的路由，并且可以帮助你正确地使用或检查站点上的 URL。
...
- 没有区域前缀的 URL（例如 `example.com/about/`）将返回 404（未找到）状态码，除非你指定了一个[回退策略](#fallback)。
...
你将负责编写自己的路由逻辑，或者[手动执行 Astro 的 i18n 中间件](#middleware-function)与你自己的逻辑一起。
...
#### 中间件函数 \{#middleware-function\}

[`middleware`](#middleware-function) 函数手动创建 Astro 的 i18n 中间件。这允许你扩展 Astro 的 i18n 路由，而不是完全替换它。
...
你的回退策略包含两个部分：选择哪些语言应该回退到哪些其他语言（[`i18n.fallback`](/zh-cn/reference/configuration-reference/#i18nfallback)）以及选择是执行[重定向](/zh-cn/guides/routing/#redirects)还是[重写](/zh-cn/guides/routing/#rewrites)来显示回退内容（[`i18n.routing.fallbackType`](/zh-cn/reference/configuration-reference/#i18nroutingfallbacktype) 在 Astro v4.15.0 中新增）。
...
当使用来自 [`astro:i18n` 虚拟模块](/zh-cn/reference/api-reference/#internationalization-astroi18n) 的函数根据你的配置计算有效的 URL 路径时（例如 `getRelativeLocaleUrl()`），[请使用 `path` 作为 `locale` 的值](/zh-cn/reference/api-reference/#getlocalebypath)。
