...
Astro 的国际化（i18n）功能允许你将项目适配给国际受众。这个路由 API 帮助你生成、使用并验证你的多语言站点产生的 URLs。

Astro 的 i18n 路由允许你带来你的多语言内容，并支持配置默认语言、计算相对页面 URLs 以及接受访客浏览器提供的首选语言。你还可以基于每种语言指定备用语言，以便你的访客始终可以被引导至你站点上现有的内容。
...
你也可以选择[除了或替代 Astro 的 i18n 中间件之外，添加你自己的 i18n 逻辑](#manual)，这样你可以在仍然能访问 `astro:i18n` 辅助函数的同时，对你的路由有更多的控制权。


## 配置 i18n 路由

在 `i18n` 配置对象中必须指定一个默认语言（[`defaultLocale`](/zh-cn/reference/configuration-reference/#i18ndefaultlocale)）和所有支持的语言列表（[`locales`](/zh-cn/reference/configuration-reference/#i18nlocales)）。此外，你还可以配置更具体的路由和回退行为，以匹配你期望的 URLs。
...
配置了 i18n 路由后，你现在可以使用诸如 [`astro:i18n` 模块](zh-cn/reference/api-reference/#internationalization-astroi18n) 中提供的 [`getRelativeLocaleURL()`](/zh-cn/reference/api-reference/#getrelativelocaleurl) 辅助函数来计算站点内页面的链接。这些生成的链接将始终提供正确的、本地化的路由，并且可以帮助你正确地使用或检查站点上的 URLs。
...
此路由选项定义了你的默认语言的 URLs 是否应该使用语言前缀（例如 `/en/about/`）。
...
- `"prefixDefaultLocale: false"`（默认）：你的默认语言的 URLs **不会**有 `/[locale]/` 前缀。所有其他语言都会有。

- `"prefixDefaultLocale: true"`：所有 URLs，包括你的默认语言，都会有 `/[locale]/` 前缀。
...
这是**默认**值。当你的默认语言的 URLs **不会**有 `/[locale]/` 前缀，并且你的默认语言的文件位于 `src/pages/` 的根目录时，设置这个选项：
...
- 没有区域前缀的 URLs（例如 `example.com/about/`）将返回 404（未找到）状态码，除非你指定了一个[回退策略](#fallback)。
...
当这个选项被启用时，Astro 将**禁用**其 i18n 中间件，以便你可以实现自己的自定义逻辑。不能与 `routing: "manual"` 配置其他的 `routing` 选项（例如 `prefixDefaultLocale`）。
...
你可以将 `middleware` 与你自己的中间件结合使用 [路由选项](#routing)，通过使用 [`sequence`](/zh-cn/reference/api-reference/#sequence) 工具来确定顺序运行 `middleware`：
...
除了将你的网站支持的 `locales` 定义为字符串（例如 "en", "pt-br"）之外，Astro 还允许你将任意数量的[浏览器识别的语言 `codes`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Accept-Language#syntax)映射到自定义的 URL `path` 上。虽然语言环境可以是任何格式的字符串，只要它们与你的项目文件夹结构相对应，`codes` 必须遵循浏览器接受的语法。

向 `locales` 数组传递一个对象，并使用 `path` 键来定义自定义的 URL 前缀，使用 `codes` 来指示映射到此 URL 的语言。在这种情况下，你的 `/[locale]/` 文件夹名称必须与 `path` 的值完全匹配，而你的 URLs 将使用 `path` 的值来生成。
...
当使用来自 [`astro:i18n` 虚拟模块](/zh-cn/reference/api-reference/#internationalization-astroi18n) 的函数根据你的配置计算有效的 URL 路径时（例如 `getRelativeLocaleUrl()`），[使用 `path` 作为 `locale` 的值](/zh-cn/reference/api-reference/#getlocalebypath)。
...
这些属性结合了浏览器的 `Accept-Language` 头部信息和你的 `locales`（字符串或 `codes`），以自动尊重访问者的首选语言。
...
这个路由选项允许你为使用 [`@astrojs/node`](/en/guides/integrations-guide/node/) 或 [`@astrojs/vercel`](/zh-cn/guides/integrations-guide/vercel/) 适配器并配置了 `site` 的服务端渲染项目，按语言自定义你的域名。

要在你的项目中启用这个功能，如果你还没有这样做的话，请根据你的偏好[配置 i18n 路由](#configure-i18n-routing)。然后，将 `experimental.i18nDomains` 标志设置为 `true` 并添加 `i18n.domains`，将你支持的任何 `locales` 映射到自定义 URL 上：
