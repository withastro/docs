...
Astro的国际化(i18n)特性允许你根据国际用户的需要对你的项目进行改造。
...
Astro的i18n路由允许你添加多语言内容，并支持配置默认语言、计算相应语言页面的url以及接受访客浏览器提供的首选语言。你还可以在每种语言的基础上指定备选语言，以便你的访客始终可以被定向到你网站上的现有内容。

这个路由 API 帮助您生成、使用和验证你的多语言网站生成的 URL。这个 API 还在不断开发与完善中，因此请定期查看最新的文档并更新API，以获取最新的功能。


### 配置i18n路由

1. 通过向你的 Astro 配置添加一个 i18n 对象来启用选项，其中包括[默认语言(`defaultLocale`)和支持的所有语言的列表(`locales`)](#defaultlocale-与-locales)：
...
2. 基于你倾向的`默认语言`的URL路径，选择和配置[`routing`](#routing)：
...
3. 按照语言组织你的内容文件夹，将本地化内容放在相应语言的文件夹中。你的文件夹名称必须与 locales 中的语言项完全匹配，你的文件夹组织结构必须与在[`routing`](#routing)中设置的 URL 路径相匹配。

    只有当你配置 `prefixDefaultLocale: true` 时，Astro才会为你的 `defaultLocale` 生成一个默认语言的文件夹。
...
4. 配置了 i18n 路由之后，你现在可以使用 [`astro:i18n`](#astroi18n虚拟化模块) 模块提供的 [`getRelativeLocaleURL()`](#getrelativelocaleurl) 辅助函数来计算站点内页面的链接。它将始终提供正确的本地化路由，并可以帮助你正确地使用或检查站点上的 URL。你也可以自己动手编写对应的URL。
...
这是该参数的 **默认** 值。当你的默认语言的 URL **不** 需要 `/[locale]/` 前缀，并且你的默认语言文件存在于 `src/pages/` 的根目录时，可以设置此选项。
...
要在项目中启用这个功能，如果你还没有[配置i18n路由](#配置i18n路由)，请先根据你的偏好进行相应配置。然后将 `experimental.i18nDomains` 标志设置为 `true`，并添加 `i18n.domains` 来映射你支持的任何 `locales` 到自定义的 URL：
...
使用这个函数来获取locale参数对应的相对路径。如果locale参数对应的语言环境不存在，Astro会抛出一个错误。
...
当 [`site`]存在对应的取值时，使用这个函数来获取一个语言环境的绝对路径。如果 [`site`] 没有被配置，该函数返回一个相对URL。如果locale参数对应的语言环境不存在，Astro会抛出一个错误。
...
像[`getRelativeLocaleUrl`](#getrelativelocaleurl)一样去使用这个函数，返回所有语言环境的相对路径列表。
...
像[`getAbsoluteLocaleUrl`](#getabsolutelocaleurl)一样去使用这个函数，返回所有语言环境的绝对路径列表。

### `getPathByLocale(locale: string): string`

一个当配置了[自定义语言路径](#自定义语言路径)时，返回与一个或多个 `codes` 关联的 `path`的函数。

### 根据自定义路径生成URL

当一个 `locale` 是一个自定义路径时，使用 `path` 作为 `locale` 的值。

例如在下面的 `locales` 配置中，在 i18n 辅助函数中使用 `french` 作为参数：
...
---
```
请注意，对于相同的配置，其他 `astro:i18n` 函数将返回以下结果：
...
一个返回与语言环境 `path` 相关联的 `code` 的函数。

### 获取自定义路径的语言环境

给定一个配置了多个 `codes` 的自定义路径，该函数将返回**第一个**code配置的代码：
