...
这将在构建时生成三个 JSON 端点：`/api/0.json`、`/api/1.json` 和 `/api/2.json`。带端点的动态路由与页面的工作方式相同，但由于端点是一个函数而不是组件，因此 [props](/zh-cn/reference/routing-reference/#通过-props-传递数据) 是不被支持的。
