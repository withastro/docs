...
```js "import.meta.url" title="astro.config.mjs"
import { defineConfig } from 'astro/config'
...
```js ins={8-10} title="astro.config.mjs"
import { defineConfig } from 'astro/config'
...
你可以在配置文件中使用 `process.env` 访问其他环境变量，如 [CLI 设置的环境变量](/zh-cn/guides/environment-variables/#使用cli)。
