...
## 环境变量
Astro 在加载您的其他文件之前评估配置文件。因此，你不能使用 `import.meta.env` 访问在 `.env` 文件中设置的环境变量。
你可以使用配置文件中的 `process.env` 来访问其他环境变量，例如[通过CLI设置](/zh-cn/guides/environment-variables/#使用cli)。
你也可以使用`Vite`的[`loadEnv`](https://main.vitejs.dev/config/#using-environment-variables-in-config)函数手动加载 `.env` 文件。
