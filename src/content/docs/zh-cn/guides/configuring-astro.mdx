...
:::note
Vite 特定的 `import.meta` 属性，如 `import.meta.env` 或 `import.meta.glob`，无法在你的配置文件中访问。我们推荐使用 [dotenv](https://github.com/motdotla/dotenv) 或 [fast-glob](https://github.com/mrmlnc/fast-glob) 作为替代方案。
:::
...
你可以在配置文件中使用 `process.env` 访问其他环境变量，如 [CLI 设置的环境变量](/zh-cn/guides/environment-variables/#using-the-cli)。
