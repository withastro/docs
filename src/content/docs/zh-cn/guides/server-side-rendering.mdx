...
在 API 参考中查看有关 [`Astro.cookies` 和 `AstroCookie` 类型](/en/reference/api-reference/#astrocookies)的更多详细信息。
...
服务器端点，也称为 **API 路由**，是从 `src/pages/` 文件夹中的 `.js`或 `.ts` 文件导出的特殊函数。该函数接收一个[端点上下文对象](/en/reference/api-reference/#endpoint-context)并返回一个 [Response](https://developer.mozilla.org/zh-CN/docs/Web/API/Response)。 这是 SSR 的一个强大功能，API 路由能够在服务器上安全地执行代码。 要了解更多信息，请参阅我们的[端点指南](/zh-cn/core-concepts/endpoints/#服务器端点api-路由)。
