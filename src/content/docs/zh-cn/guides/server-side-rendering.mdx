...
为了决定在你的项目中使用哪一个，清选择一个 `output` 选项，这个选项代表了你的**大部分**页面和路由将如何被渲染：
...
你可以在我们的 [集成目录](https://astro.build/integrations/?search=&categories%5B%5D=adapters) 中找到官方和社区的适配器。然后选择与你的 [部署环境](/en/guides/deploy/) 相对应的适配器。
