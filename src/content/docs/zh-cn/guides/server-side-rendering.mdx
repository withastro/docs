...
请求的标头可在 `Astro.request.headers` 上找到。这就像浏览器的 [`Request.headers`](https://developer.mozilla.org/zh-CN/docs/Web/API/Request/headers)。它是一个 [Headers](https://developer.mozilla.org/zh-CN/docs/Web/API/Headers) 对象，一个类似 Map 的对象，您可以在其中检索标头，例如 cookie。
