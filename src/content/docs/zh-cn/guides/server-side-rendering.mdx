...
- **频繁更改的内容**：启用 SSR 可让您生成单个页面，而无需静态重建站点。当页面内容频繁更新时，这很有用。
...
然后，您可以在任何页面或路由中使用 export 语句选择退出默认渲染行为：
...
`server` 和 `hybrid` 模式都允许服务器渲染的页面和端点，并将根据其默认模式渲染。但是，这两种模式都允许您将单个页面标记为 opt-out 此默认行为。
...
在此过程中，浏览器以增量方式使用 HTML：解析、呈现到 DOM 中和绘制。无论您是否有意流式传输 HTML，都会发生这种情况。网络状况可能会导致大型文档下载缓慢，等待数据获取可能会阻止页面呈现。
...
将 `await` 调用移动到较小的组件中，您可以利用 Astro 的流式传输。使用以下组件执行数据获取，Astro 可以先渲染一些 HTML，例如标题，然后在数据准备就绪时呈现段落。
...
您还可以直接在模板中包含 promises。它不会阻止整个组件，而是并行解析 promise，并且只阻止它后面的标记。
...
默认情况下，Astro 将所有页面构建到一个名为 `entry.mjs` 的单个文件中。您可以在构建选项中配置 [`split`](/zh-cn/reference/configuration-reference/#buildsplit)，改为为每个页面发出一个文件：
