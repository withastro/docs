...
## 启用按需服务器渲染 \{#enable-on-demand-server-rendering\}
...
因为服务器需要按需生成至少一些页面，所以这两种模式都需要你[添加一个适配器](#add-an-adapter)来执行服务器功能。

### 添加一个适配器 \{#add-an-adapter\}
...
#### 使用 `astro add` 安装 \{#astro-add-install\}
...
### 配置 `sever` 或 `hybrid` 模式 \{#configure-server-or-hybrid\}
...
### 在 `server` 模式中选择加入预渲染 \{#opting-in-to-pre-rendering-in-server-mode\}
...
### 在 `hybrid` 模式中选择退出预渲染 \{#opting-out-of-pre-rendering-in-hybrid-mode\}
...
## 按需渲染功能特性 \{#on-demand-rendering-features\}

### HTML 流式处理 \{#html-streaming\}
...
### `Astro.request.headers` \{#return-a-response-object\}
...
### 服务器端点 \{#astrorequestheaders\}

服务器端点，也称为 **API 路由**，是从 `src/pages/` 文件夹中的 `.js`或 `.ts` 文件导出的特殊函数。按需服务器端渲染的一个强大特性是，API 路由能够在服务器上安全地执行代码。


该函数接收一个[端点上下文对象](/zh-cn/reference/api-reference/#endpoint-context)并返回一个 [Response](https://developer.mozilla.org/zh-CN/docs/Web/API/Response)。

要了解更多信息，请参阅我们的[端点指南](/zh-cn/guides/endpoints/#server-endpoints-api-routes)。
