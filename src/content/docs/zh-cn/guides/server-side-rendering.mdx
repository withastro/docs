...
[`Astro.response`](/zh-cn/reference/api-reference/#astroresponse) 是一个标准 [`ResponseInit`](https://developer.mozilla.org/zh-CN/docs/Web/API/Response/Response#options) 对象。它可用于设置响应状态和标头。
...
你可以手动或使用 [`Astro.redirect`](/zh-cn/reference/api-reference/#astroredirect) 来通过按需渲染，直接从任何页面返回 [Response](https://developer.mozilla.org/zh-CN/docs/Web/API/Response) 对象`](/zh-cn/reference/api-reference/#astroredirect)。
