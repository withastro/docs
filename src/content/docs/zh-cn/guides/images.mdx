...
在你的 `.md` 文件中使用标准的 Markdown `![alt](src)` 语法。这种语法可以与 Astro 的 [图像服务 API](/zh-cn/reference/image-service-reference/) 配合使用，来优化存储在 `src/` 目录下的本地图像。远程图像和存储在 `public/` 文件夹中的图像不会被优化。
