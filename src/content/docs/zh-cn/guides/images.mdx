...
src 属性是必需的，其格式取决于图像的存储位置以及您是否启用了对资产的实验性支持：
...
如果你无法将图片保留在 `public/` 中，我们建议启用 [资产的实验性支持](/zh-cn/guides/assets/), 或使用 `.mdx` 文件格式，该格式允许你将导入的组件与类似 Markdown 的语法结合起来。使用 [MDX integration](/zh-cn/guides/integrations-guide/mdx/) 向 Astro 添加对 MDX 的支持。
```md
<!-- src/pages/post-1.md -->
...
存储在 `src/`中的图像可以由组件（`.astro`，`.mdx`和其他UI框架）使用，但不能在Markdown文档中使用。

如果您必须使用 Markdown 文档，我们建议您将图像保存在 ['public/']（#public） 中或 [远程](#using-images-from-a-cms-or-cdn)存储他们。
...
:::note[将在 v3.0 中弃用]
Astro v3将不在主动支持 [`@astrojs/image`](https://github.com/withastro/astro/tree/main/packages/integrations/image)集成

我们建议您尽早删除它，并使用将内置在 `astro@3.0` 中的实验性 `astro:assets` 模块。
...
Image CDN 可与 Astro 配合使用。使用图片的完整网址作为 `<img>` 标签或 Markdown 中的“src”属性。
...
此属性对于映像集成的`<Image />`和`<图片/>`组件是必需的，如果未提供替代文本，这些组件将引发错误。

如果图像只是装饰性的（即无助于理解页面）请设置`alt=""`，以便屏幕阅读器知道忽略图像。
