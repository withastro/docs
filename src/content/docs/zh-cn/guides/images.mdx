...
Astro 可以使用 API 远程获取数据，也可以显示来自其完整 URL 路径的图像。
...
远程和 `public/` 图像不需要导入，而是需要 `src` 的 URL（站点上的完整路径或相对路径）。

导入并使用 Astro 原生的 [`<Image />`](#使用-image-组件显示优化后的图像) 和 [`<Picture />`](#使用-picture-组件创建响应式图像) 组件来优化图像。Astro 语法也支持 [直接编写 HTML `<img>` 标签](#display-unprocessed-images-with-the-html-img-tag)，这将跳过图像处理。
...
`<Image />` 组件接受 [一些组件属性](/zh-cn/reference/modules/astro-assets/#image-属性) 以及 HTML `<img>` 标签接受的任何属性。
...
[Astro 模板语法](/zh-cn/basics/astro-syntax/) 还支持直接编写 `<img>` 标签，并完全控制其最终输出。这些图像将不会被处理和优化。它接受所有 HTML `<img>` 标签属性，唯一必需的属性是 `src`。
...
本地图像不支持 `<img>` 标签，也不支持 `<Image />` 和 `<Picture />` 组件。

如果你需要对图像属性做更多的控制，我们建议使用 [Astro's MDX 集成](/zh-cn/guides/integrations-guide/mdx/) 来添加对 `.mdx` 文件格式的支持。MDX 允许向 Markdown 添加组件，并且还提供其他 [MDX 中可用的图像选项](#mdx-文件中的图像)。

## MDX 文件中的图像

你可以通过导入组件和图像在你的 `.mdx` 文件中使用 Astro 的 `<Image />` 和 `<Picture />`组件。使用它们就像 [在 `.astro` 文件中使用](#astro-文件中的图像) 一样。未处理的图像还支持使用 JSX `<img />` 标签，并且 [使用与 HTML `<img>` 标记相同的图像导入](#使用-html-img-标签显示未处理的图像)。
