...
但是，你可以将 `<Image />` 生成的静态内容作为子组件传递给 `.astro` 文件中的框架组件，或者使用 [命名的 `<slot/>`](/zh-cn/basics/framework-components/#我可以在我的框架组件中使用-astro-组件吗)。
...
`getImage()` 函数用于生成图像，这些图像将被用于除了直接在 HTML 中使用之外的其他地方，例如在 [API 路由](/zh-cn/basics/endpoints/#服务器端点api-路由) 中。它还允许你创建自己的自定义 `<Image />` 组件。
