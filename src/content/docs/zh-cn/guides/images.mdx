...
使用内置的 `<Picture />` Astro 组件生成一个带有多种格式和尺寸图像的 `<picture>` 标签。这允许你指定首选的文件格式来显示，同时提供一个备用格式。就像 [`<Image />` 组件](#image--1)一样，对于预渲染页面来说，图像将在构建时处理。当你的页面按需渲染时，处理将在查看页面时动态发生。
...
通过将[响应式图像属性](/zh-cn/reference/modules/astro-assets/#responsive-image-properties)应用于 `<Image />` 或 `<Picture />` 组件，Astro 将自动为你的图像生成所需的 `srcset` 和 `sizes` 值，并应用必要的[样式以确保它们正确调整大小](#响应式图像样式)。
...
Tailwind 4 与 Astro 的默认响应式样式兼容。但是，Tailwind 使用[级联层](https://developer.mozilla.org/zh-CN/docs/Web/CSS/@layer)，这意味着其规则的特异性总是低于不使用层的规则，包括 Astro 的响应式样式。因此，Astro 的样式将优先于 Tailwind 的样式。要使用 Tailwind 规则而不是 Astro 的默认样式，请不要启用 [Astro 的默认响应式样式](/en/reference/configuration-reference/#imageresponsivestyles)。
