...
在处理外部来源时，为了提供额外的保护，只有在配置中指定的 [授权图像源](#authorizing-remote-images) 才会被处理。但是，任何远程图像都可以显示。

Astro 可以使用 API 远程获取数据，也可以显示来自其完整 URL 路径的图像。请参阅我们的 [CMS 指南](/zh-cn/guides/cms/)，了解集成常见服务的示例。

## `.astro` 文件中的图像 \{#images-in-astro-files\}
...
使用内置的 `<Image />` Astro 组件来显示本地图像和 [配置的远程图像](#authorizing-remote-images) 的优化版本。
...
- **`src/` 中的本地图像** - 你必须**同时导入图像**，使用相对文件路径或配置并使用 [导入别名](/zh-cn/guides/imports/#aliases)。然后使用导入名称作为 `src` 值：
...
##### width 和 height（对于 `public/` 中的图像是必须的） \{#width-and-height-required-for-images-in-public\}
...
`inferSize` 能够获取来自未经授权域的[远程图片的尺寸](#authorizing-remote-images)，但图片本身将保持未处理状态。
...
#### `src/` 中的本地图像 \{#local-images-in-src\}

本地图像必须从现有的 `.astro` 文件的相对路径**导入**，或者配置并使用 [导入别名](/zh-cn/guides/imports/#aliases)。然后，你可以访问图像的 `src` 和其他属性，以在 `<img>` 标签中使用。
...
### 选择 `<Image />` 还是 `<img>` \{#choosing-image--vs-img\}
...
### 授权远程图像 \{#authorizing-remote-images\}
...
## 使用 CMS 或 CDN 中的图像 \{#using-images-from-a-cms-or-cdn\}

图像 CDN 与所有 Astro 图像选项兼容。在 `<Image />` 组件、`<img>` 标签或 Markdown 表示法中，使用图像的完整 URL 作为 `src` 属性。对于远程图像的图像优化，还需要 [配置授权域或 URL 模式](#authorizing-remote-images)。
...
## Markdown 文件中的图像 \{#images-in-markdown-files\}
...
## MDX 文件中的图像 \{#images-in-mdx-files\}

你可以通过导入组件和图像在你的 `.mdx` 文件中使用 Astro 的 `<Image />` 组件 和 JSX `<img />` 标签。使用它们就像 [在 `.astro` 文件中使用](#images-in-astro-files) 一样。

此外，还支持 [标准的 Markdown `![alt](src)` 语法](#images-in-markdown-files)，无需导入。
...
## 内容集合中的图像 \{#images-in-content-collections\}

内容集合中的图像将会按照你所使用的文件类型，以与 [Markdown](#images-in-markdown-files) 和 [MDX](#images-in-mdx-files) 中相同的方式处理。
...
## 框架组件中的图像 \{#images-in-ui-framework-components\}

在 UI 框架组件中添加图像时，请使用框架自己的图像语法来渲染图像（例如，在 JSX 中使用 `<img />`，在 Svelte 中使用 `<img>`）。

本地图像必须**先被导入**，才能访问它们的 [图像属性](#local-images-in-src)，例如 `src`。
...
#### 传递 Image 组件 \{#passing-the-image-component\}

`<Image />` 组件，就像任何其他 Astro 组件一样，**对 UI 框架组件不可用**。

但是，你可以将 `<Image />` 生成的静态内容作为子组件传递给 `.astro` 文件中的框架组件，或者使用 [命名的 `<slot/>`](/zh-cn/guides/framework-components/#can-i-use-astro-components-inside-my-framework-components)。
...
## 使用 `getImage()` 生成图像 \{#generating-images-with-getimage\}

:::caution
`getImage()` 依赖于仅在服务端可用的 API，当在客户端使用时会导致构建失败。
:::

`getImage()` 函数用于生成图像，这些图像将被用于除了直接在 HTML 中使用之外的其他地方，例如在 [API 路由](/zh-cn/guides/endpoints/#server-endpoints-api-routes) 中。它还允许你创建自己的自定义 `<Image />` 组件。

<RecipeLinks slugs={["zh-cn/recipes/build-custom-img-component" ]}/>

`getImage()` 函数接受一个选项对象，其中包含与 [Image 组件](#properties) 相同的属性（除了 `alt`）。
...
### 配置 no-op 透传服务 \{#configure-no-op-passthrough-service\}
