...
通过将[响应式图像属性](/en/reference/modules/astro-assets/#responsive-image-properties)应用于 `<Image />` 或 `<Picture />` 组件，Astro 将自动为你的图像生成所需的 `srcset` 和 `sizes` 值，并应用必要的[样式以确保它们正确调整大小](#响应式图像样式)。
