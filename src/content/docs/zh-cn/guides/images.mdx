...
Astro 为你提供了多种使用图像的方法，无论它们是在项目内部本地存储的，还是从外部 URL 链接的，或者在 CMS 或 CDN 中管理的！
...
当使用本地图像的原始宽高比时，`width` 和 `height` 可以从源文件中自动推断，因此是可选的。
...
`quality` 是一个可选属性，可以是：
- 一个预设值（`low`、`mid`、`high`、`max`），可以在不同格式之间自动归一化。
...
`<Image />` 组件会优化图像，并根据原始宽高比推断宽度和高度（对于本地图像），以避免 CLS。但是，它只适用于某些格式，并且不提供 `<picture>` 元素，也不支持 `srcset`。

当你不能使用 `<Image />` 组件时，使用 HTML `<img>` 元素，例如：
  - 不支持的图像格式
  - 当你不想让 Astro 优化你的图像时
...
你可以使用 [`image.domains`](/zh-cn/reference/configuration-reference/#imagedomains) 和 [`image.remotePatterns`](/zh-cn/reference/configuration-reference/#imageremotepatterns) 配置图像优化的授权图像源 URL 域和模式列表。这个配置是一个额外的安全层，用于在显示来自外部源的图像时保护你的站点。
...
在 `.md` 文件中使用标准的 Markdown `![alt](src)` 语法。这个语法可以与 Astro 的 Image Services API 一起使用，来优化你的本地图像和授权的远程图像。
...
如果你需要更多的控制图像属性，我们建议使用 `.mdx` 文件格式，它允许你在 Markdown 语法之外，还包括 Astro 的 `<Image />` 组件或 JSX `<img />` 标签。使用 [MDX 集成](/zh-cn/guides/integrations-guide/mdx/) 为 Astro 添加对 MDX 的支持。
...
现在可以标准的 Markdown `![alt](src)` 语法，在 Markdown、MDX 和 Markdoc 中引用项目 `src/` 中的相对图像。
...
3. 移除任何现有的 `<Picture />` 组件
    ...
    相反，你可以使用 HTML 图像属性 `srcset` 和 `sizes` 或 `<picture>` 标签进行艺术指导或创建响应式图像。
