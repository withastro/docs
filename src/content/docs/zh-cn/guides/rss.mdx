...
:::tip
确保已在项目的 `astro.config` 中[配置了 `site`](/zh-cn/reference/configuration-reference/#site)。这将用于生成指向您的 RSS 文章的链接。
:::

2. 在 `src/pages/` 中创建一个文件，其中包含您选择的名称和扩展名 `.xml.js`，以用作 Feed 的输出 URL。一些常见的 RSS 源 URL 名称是 `feed.xml` 或 `rss.xml`。
...
您可以从内容集合架构生成此数组，也可以使用 [glob imports](/zh-cn/guides/imports/#astroglob) 为位于 `src/pages/` 中的博客文章生成此数组。
...
## 后续步骤
在浏览器中的 `your-domain.com/rss.xml` 访问您的提要并确认您可以看到每个帖子的数据后，您现在可以[在您的网站上推广您的提要](https://medium.com/samsung-internet-dev/add-rss-feeds-to-your-website-to-keep-your-core-readers-engaged-3179dca9c91e#：~：text=com/~deno%2Drss-，Advertising%20your%20RSS%20feed，-Now%20you%20have)。将标准 RSS 图标添加到您的网站可以让您的读者知道他们可以在自己的提要阅读器中订阅您的帖子。
