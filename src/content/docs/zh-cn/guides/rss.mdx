...
[`@astrojs/rss` 包](https://github.com/withastro/astro/tree/main/packages/astro-rss) 提供了使用 [API 端点](/zh-cn/guides/endpoints/#静态文件端点) 生成 RSS 摘要的帮助程序。这使得静态构建和使用 [SSR 适配器](/zh-cn/guides/server-side-rendering/) 时的按需生成都成为可能。
...
要启用此功能，请在您的网站的 `head` 元素中添加带有以下属性的 `<link>` 标签：
