...
## 使用 `useActionState()` 集成 Actions （实验性）

`@astrojs/react` 集成提供了两个实验性函数：`experimental_withState()` 和 `experimental_getActionState()` 来与 [Astro Actions][astro-actions] 一起使用。
...
你可以将 `experimental_withState()` 和你想要触发的 action 传递给 React 的 `useActionState()` hook 作为表单 action 函数。下面的示例将 `like` action 传递给增加计数器，并将初始状态设为 `0` 个点赞。
...
`experimental_withState()` 函数会将 action 的类型与 React 的期望匹配，并保留用于渐进增强的元数据，使其即使在用户设备禁用 JavaScript 时也能正常工作。
...
你可以在 action `handler` 中通过 `experimental_getActionState()` 访问 `useActionState()` 存储的状态。它接受 [Astro API 上下文](/zh-cn/reference/api-reference/#上下文对象)，并可选地对结果应用类型。
