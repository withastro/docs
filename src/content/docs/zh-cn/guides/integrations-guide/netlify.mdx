...
Netlify 的偏差保护确保用户在部署过程中访问你的站点时，仍能从同一部署版本获取内容。Netlify 适配器会通过将当前部署 ID 注入内部请求，自动为 Astro 的功能（如 actions、server islands、视图过渡和预取请求）配置偏差保护。这可防止在活动部署期间客户端与服务器之间出现版本不匹配。

虽然 Astro 会自动为其内置功能添加偏差保护标头，但如果你向自己的网站发起自定义的 fetch 请求，你可以使用 `DEPLOY_ID` 环境变量手动加入该标头：
...
例如，当启用 [实验性的内容安全策略](/zh-cn/reference/configuration-reference/) 时，可以使用 `staticHeaders` 将 CSP `headers` 添加到你的 Netlify 配置中，而不是创建 `<meta>` 元素：
