...
学习如何在我们的 [Netlify 部署指南](/en/guides/deploy/netlify/) 上部署你的 Astro 网站.
...
[Netlify](https://www.netlify.com/) 是一个部署平台，允许您通过直接连接 GitHub 仓库来托管您的网站。这个适配器增强了 Astro 的构建流程，为通过 Netlify 部署项目做好准备。
...
Netlify 有两个无服务器平台，[Netlify Functions](https://docs.netlify.com/functions/overview/) 和 [Netlify Edge Functions](https://docs.netlify.com/edge-functions/overview/)。使用 Edge Functions，您的代码分布在更接近用户的地方，降低延迟。
...
为了将这个上下文中的值暴露给你的站点，在你项目的 [srcDir](/zh-cn/reference/configuration-reference/#srcdir) 中创建一个 `netlify-edge-middleware.ts`（或 `.js`）文件。这个文件必须导出一个函数，它返回要添加到 [Astro 的 `locals` 对象](/en/guides/middleware/#locals)中的数据，该对象在中间件和 Astro 路由中可用。
...
[Netlify 按需构建器](https://docs.netlify.com/configure-builds/on-demand-builders/) 是用于在 Netlify 的 Edge CDN 上构建和缓存页面内容的无服务器函数。你可以使用 `builders` 选项启用这些函数：
...
* [Browse Astro Netlify projects on GitHub](https://github.com/search?q=%22%40astrojs%2Fnetlify%22+filename%3Apackage.json\&type=Code) 提供更多示例!
...
这个项目由 Astro 核心团队维护。欢迎提交问题或 PR！

## 更新日志

参加 [CHANGELOG.md](https://github.com/withastro/astro/tree/main/packages/integrations/netlify/CHANGELOG.md), 了解本集成的历史变更。
