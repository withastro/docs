...
### VS Code 编辑器集成 \{#vs-code-editor-integration\}
...
然后，使用 [内容集合 API](/zh-cn/guides/content-collections/#querying-collections) 查询你的集合：
...
## 传递 Markdoc 变量 \{#pass-markdoc-variables\}
...
### 从你的 Markdoc 内容中访问 frontmatter \{#access-frontmatter-from-your-markdoc-content\}
...
### 将 Astro 组件作为 Markdoc 标签 \{#use-astro-components-as-markdoc-tags\}
...
### 使用客户端 UI 组件 \{#use-client-side-ui-components\}

标签和节点仅限于 `.astro` 文件。为了在 Markdoc 中嵌入客户端 UI 组件，[使用一个包装 `.astro` 组件来渲染一个框架组件](/zh-cn/guides/framework-components/#nesting-framework-components)，并配合你所需的 `client:` 指令。
...
```
### 从 npm 包和 TypeScript 文件中使用 Astro 组件 \{#use-astro-components-from-npm-packages-and-typescript-files\}
...
<ReadMore>要了解如何配置 Prism 样式表，[请看我们的语法高亮指南](/zh-cn/guides/markdown-content/#prism-configuration)。</ReadMore>

## 自定义 Markdoc 节点 / 元素 \{#custom-markdoc-nodes--elements\}
...
### 自定义标题 \{#custom-headings\}

`@astrojs/markdoc` 自动为你的标题添加锚点链接，并通过内容集合 API 生成一个 `headings` 列表。为了进一步自定义标题的渲染方式，你可以应用一个 Astro 组件作为 Markdoc 节点。
...
* `id: string` 从标题的文本内容生成的 `id`。这对应于由 [内容 `render()` 函数](/zh-cn/guides/content-collections/#rendering-content-to-html) 生成的 `slug`。

例如，标题 `### Level 3 heading!` 将传递 `level: 3` 和 `id: 'level-3-heading'` 作为组件的 props。

### 自定义图片组件 \{#custom-image-components\}

Astro 的 `<Image />` 组件不能直接在 Markdoc 中使用。但是你可以配置一个 Astro 组件来覆盖每次使用原生 `![]()` 图片语法时的默认图片节点，或者作为一个自定义的 Markdoc 标签让你可以指定额外的图片属性。

#### 覆盖 Markdoc 的默认图片节点 \{#override-markdocs-default-image-node\}
...
#### 创建一个自定义的 Markdoc 图片标签 \{#create-a-custom-markdoc-image-tag\}
...
## Markdoc 高级配置 \{#advanced-markdoc-configuration\}
...
### 设置根 HTML 元素 \{#set-the-root-html-element\}
...
## 集成配置选项 \{#integration-config-options\}
