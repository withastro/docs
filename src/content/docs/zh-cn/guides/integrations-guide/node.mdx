...
此适配器允许 Astro 将你的 [`hybrid` 或 `server` 渲染的网站](/zh-cn/basics/rendering-modes/#on-demand-rendered)部署到 Node 目标。

如果你将 Astro 用作[静态站点构建器](/zh-cn/basics/rendering-modes/#pre-rendered)，则不需要适配器。

## 为什么选择 Astro Node.js \{#why-astro-nodejs\}
...
首先，[执行构建](/zh-cn/guides/deploy/#building-your-site-locally)。根据选择的模式（见上文），执行以下相应的步骤：
