...
此适配器允许 Astro 将你的 [`hybrid` 或 `server` 渲染的网站](/zh-cn/core-concepts/rendering-modes/#按需渲染)部署到 Node 目标。

如果你将 Astro 用作[静态站点构建器](/zh-cn/core-concepts/rendering-modes/#预渲染)，则不需要适配器。
