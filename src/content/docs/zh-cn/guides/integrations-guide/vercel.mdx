...
此适配器允许 Astro 将你的[`hybrid` 或 `server` 渲染站点](/zh-cn/basics/rendering-modes/#on-demand-rendered) 部署到[Vercel](https://www.vercel.com/)。

如果你使用 Astro 作为 [`静态` 站点生成器](/zh-cn/basics/rendering-modes/#pre-rendered)，只有当你使用额外的 Vercel 服务时（例如 [Vercel 网络分析](https://vercel.com/docs/analytics)，[Vercel 图像优化](https://vercel.com/docs/image-optimization)），你才需要这个适配器。否则，你不需要适配器来部署你的 `静态` 站点。

在我们的 [Vercel 部署指南](/zh-cn/guides/deploy/vercel/) 中学习和部署你的 Astro 站点。

## 为什么选择 Astro Vercel \{#why-astro-vercel\}

[Vercel](https://www.vercel.com/) 是一个部署平台，它允许你通过直接连接到 GitHub 仓库来托管你的站点。这个适配器增强了 Astro 的构建过程，为通过 Vercel 进行部署做好了准备。

## 安装 \{#installation\}

Astro 包含了一个 `astro add` 命令，用于自动设置官方集成。如果你愿意，也可以[手动安装集成](#manual-install)。
...
然后，将适配器和你想要的[按需渲染模式](/zh-cn/basics/rendering-modes/#on-demand-rendered)添加到你的 `astro.config.*` 文件中：
...
`domains` 和 `remotePatterns` 属性将自动使用 [Astro 对应的 `image` 设置](/zh-cn/reference/configuration-reference/#image-options) 来填充。
...
#### ISR 缓存失效 \{#isr-cache-invalidation\}

默认情况下，ISR 函数会在你的部署期间进行缓存。你可以通过设置过期时间，或者完全排除特定路由不进行缓存，来进一步控制缓存。

##### 基于时间的失效 \{#time-based-invalidation\}
...
##### 从缓存中排除路径 \{#excluding-paths-from-caching\}
...
### 函数打包配置 \{#function-bundling-configuration\}
...
### 在 Vercel Edge 函数上运行 Astro 中间件 \{#running-astro-middleware-on-vercel-edge-functions\}
...
### Node.js 版本支持 \{#nodejs-version-support\}
