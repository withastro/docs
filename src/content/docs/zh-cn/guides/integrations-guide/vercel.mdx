...
但如果你希望使用 [服务端渲染](/en/guides/server-side-rendering/)， Astro 就需要一个与你的部署运行时相匹配的适配器。
...
你可以通过设置 `analytics: true` 来启用 [Vercel Analytics](https://vercel.com/analytics) (包括 Web Vitals 和 Audiences) 。这将在所有页面中注入Vercel的跟踪脚本。
...
**适用于：** Edge, Serverless, Static<br/>
**添加于:** `@astrojs/vercel@3.3.0`

[Vercel 的图片优化API](https://vercel.com/docs/concepts/image-optimization)选项。 查看 [Vercel 的图片优化配置文档](https://vercel.com/docs/build-output-api/v3/configuration#images)以获取支持的参数的完整列表。
...
使用此属性来强制将文件与你的函数捆绑在一起。当你发现缺失文件时，这很有帮助。
...
:::note
当为 Edge 构建时，所有依赖项都会被捆绑在一个文件中以节省空间。不会捆绑额外的文件。因此，如果你需要一些文件在函数内部，你必须在 includeFiles 中指定它们。
:::
...
可选的，你可以在 [`src文件夹`](/zh-cn/reference/configuration-reference/#srcdir) 中创建一个被适配器识别的文件，名为 `vercel-edge-middleware.(js|ts)` ，以创建 [`Astro.locals`](/en/reference/api-reference/#astrolocals) 。
...
*   `request` 的类型为 [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request);
...
*   Vercel Edge中间件将始终是**第一个**接收 `Request` 的函数，并且是接收 `Response` 的最后一个函数。 这是一种遵循 [Vercel 设定的边界](https://vercel.com/docs/concepts/functions/edge-middleware)的结构性约束。
...
**一些已知的复杂软件包（例如： [puppeteer](https://github.com/puppeteer/puppeteer))不支持捆绑，因此在此适配器下可能无法正常工作。** 默认情况下，Vercel不会包含项目的 `./node_modules` 文件夹中安装的文件和包。为了解决这个问题， `@astrojs/vercel` 适配器会使用 `esbuild` 自动捆绑你的最终构建输出。
