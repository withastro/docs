...
通过 Astro Sitemap，你无需担心自己创建这个 XML 文件：Astro Sitemap 集成将遍历你静态生成的路由并创建网站地图文件，包括 `getStaticPaths()` 生成的 [动态路由](/zh-cn/basics/routing/#动态路由)，如 `[...slug]` 或 `src/pages/[lang]/[version]/info.astro`。
