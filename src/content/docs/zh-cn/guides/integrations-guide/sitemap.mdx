...
## 为什么使用 Astro Sitemap \{#why-astro-sitemap\}
...
通过 Astro Sitemap，你无需担心自己创建这个 XML 文件：Astro Sitemap 集成将遍历你静态生成的路由并创建网站地图文件，包括 `getStaticPaths()` 生成的 [动态路由](/zh-cn/guides/routing/#dynamic-routes)，如 `[...slug]` 或 `src/pages/[lang]/[version]/info.astro`。

这个集成无法为 [SSR 模式](/zh-cn/basics/rendering-modes/#on-demand-rendered) 的动态路由生成网站地图条目。

## 安装 \{#installation\}

Astro 包含了一个 `astro add` 命令，用于自动设置官方集成。如果你愿意，也可以改为[手动安装集成](#manual-install)。
...
### 站点地图的抓取 \{#sitemap-discovery\}

你可以通过网站的 `<head>` 和 `robots.txt` 文件中的链接让抓取工具更轻松地找到你的站点地图。

#### `<head>` 中的站点地图链接 \{#sitemap-link-in-head\}
...
#### `robots.txt` 中的站点地图链接 \{#sitemap-link-in-robotstxt\}
...
### changefreq、lastmod 以及 priority \{#changefreq-lastmod-and-priority\}
