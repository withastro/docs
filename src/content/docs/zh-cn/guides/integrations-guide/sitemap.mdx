...
通过 Astro Sitemap，你无需担心自己创建这个 XML 文件：Astro Sitemap 集成将遍历你静态生成的路由并创建网站地图文件，包括 `getStaticPaths()` 生成的 [动态路由](/en/core-concepts/routing/#dynamic-routes)，如 `[...slug]` 或 `src/pages/[lang]/[version]/info.astro`。

这个集成无法为 [SSR 模式](/en/guides/server-side-rendering/) 的动态路由生成网站地图条目。
...
现在，通过 `astro build` 命令[构建你的生产网站](/en/reference/cli-reference/#astro-build)。你会在 `dist/` 目录下找到两个文件：`sitemap-index.xml` 和 `sitemap-0.xml`！
...
[了解更多关于语言属性的信息](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/lang)。
...
*   或，在 GitHub 上 [浏览使用 Astro Sitemap 的项目](https://github.com/search?q=%22@astrojs/sitemap%22+filename:package.json\&type=Code)。

## 故障排除

如果需要帮助，请查看 [Discord](https://astro.build/chat) 上的 `#support` 频道。我们友好的支持小组成员将在那里提供帮助！

您还可以查看我们的 [Astro 集成文档][astro-integration] 以了解更多关于集成的信息。

## 贡献

此软件包由 Astro 核心团队维护。欢迎提交问题或 PR！
