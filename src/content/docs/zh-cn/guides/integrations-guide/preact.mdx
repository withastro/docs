...
description: 学习如何在你的Astro项目使用 @astrojs/preact框架集成来扩展组件支持。
...
如果遇到任何问题，[请随时在GitHub留言](https://github.com/withastro/astro/issues) 并尝试以下的手动安装步骤。
...
要在 Astro 中使用你的第一个 Preact 组件，请前往我们的 [UI framework documentation][astro-ui-frameworks]. 你将会发现：

*   📦 框架组件是如何加载的，
*   💧 客户端水合选项以及，
*   🤝 将不同框架混合和嵌套在一起的机会

还可以查阅我们的 [Astro Integration Documentation][astro-integration] 以获取更多关于集成的信息。
...
当启用了 `compat` 选项后，Preact 集成将会在你的项目中渲染 React 组件和 Preact 组件，同时还允许你在 Preact 组件中导入 React 组件。在 Preact 网站的“从 React 切换到 Preact”页面中可以阅读更多信息 [“Switching to Preact (from React)”](https://preactjs.com/guide/v10/switching-to-preact) 。
...
*   在 [Astro Preact example](https://github.com/withastro/astro/tree/latest/examples/framework-preact) 中展示了如何在 Astro 项目中使用交互式的 Preact 组件。
*   在 [Astro Nanostores example](https://github.com/withastro/astro/tree/latest/examples/with-nanostores) 中展示了如何在 Astro 项目中在不同组件之间共享状态，甚至可以跨不同框架共享状态。

## 问题排查

如果需要帮助，请访问 [Discord](https://astro.build/chat) 上的 `#support` 频道。我们友好的支持团队成员将在那里帮助你！

你也可以查阅Astro集成文档 [Astro Integration Documentation][astro-integration] 获得更多集成信息。
