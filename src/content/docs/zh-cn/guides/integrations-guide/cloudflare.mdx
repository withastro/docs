...
1. 使用合适的包管理器将 `@astrojs/cloudflare` 适配器添加到项目的依赖项中。
...
2. 将适配器添加到 `astro.config.mjs` 文件中：
...
4. 在你的 `public/` 文件夹中创建一个 `.assetsignore` 文件，并向其添加以下内容：
...
Cloudflare 运行时允许你访问在 `wrangler.toml`/`wrangler.jsonc` 配置文件中定义的环境变量和 Cloudflare 资源绑定。
...
例如，你可以在 `wrangler.jsonc` 中对 [环境变量](https://developers.cloudflare.com/workers/configuration/environment-variables/#add-environment-variables-via-wrangler) 进行如下定义：
