...
此适配器允许 Astro 将你的[`hybrid` 或者 `server` 渲染站点](/zh-cn/basics/rendering-modes/#on-demand-rendered)部署到[Cloudflare](https://www.cloudflare.com/).

如果你使用 Astro 作为[静态站点构建器](/zh-cn/basics/rendering-modes/#pre-rendered)，则不需要适配器。

在我们的 [Cloudflare Pages部署指南](/zh-cn/guides/deploy/cloudflare/) 中了解如何部署Astro站点。

## 为什么选择 Astro Cloudflare \{#why-astro-cloudflare\}
...
然后，将适配器和所需的[按需渲染模式](/zh-cn/basics/rendering-modes/#on-demand-rendered)添加到 `astro.config.mjs` 文件中：
...
* **`passthrough`：** 使用现有的 [`noop`](/zh-cn/guides/images/#configure-no-op-passthrough-service) 服务。
* **`compile`：** 使用 Astro 的默认服务（sharp），但仅在构建时对预渲染的路由有效。在服务端渲染（SSR）中，对按需渲染的页面，所有 `astro:assets` 功能都将被禁用。
* **`custom`：** 总是使用在 [Image 选项](/zh-cn/reference/configuration-reference/#image-options) 中配置的图像服务。**此选项不会检查配置的图像服务是否在 Cloudflare 的 `workerd` 运行时中工作。**
...
决定是否以及如何将 Cloudflare 运行时添加到 `astro dev` 中。它包含对本地 `workerd` 绑定和 Cloudflare 特定值的代理和模拟，允许在 Node.js 开发过程中模拟运行时。阅读更多关于 [Cloudflare 运行时](#cloudflare-runtime) 的信息。
...
此选项允许你添加或排除自定义模式（例如 `/fonts/*`）到生成的 [`_routes.json`](#custom-_routesjson) 文件中，该文件决定了哪些路由是按需生成的。如果你需要添加无法自动生成的路由模式，或排除预渲染的路由，这会很有用。
...
启用 [`.wasm`、`.bin` 和 `.txt` 模块的导入](#cloudflare-module-imports)。

此功能默认启用。如果你想禁用它，请设置 `cloudflareModules: false`。

## Cloudflare 运行时 \{#cloudflare-runtime\}
...
## Cloudflare 平台 \{#cloudflare-platform\}
...
#### 自定义 `_routes.json` \{#custom-_routesjson\}
...
## Cloudflare 模块导入 \{#cloudflare-module-imports\}
...
## Node.js 兼容性 \{#nodejs-compatibility\}
...
## 使用 Wrangler 预览 \{#preview-with-wrangler\}
...
### 有意义的错误消息 \{#meaningful-error-messages\}
