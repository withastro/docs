...
此适配器允许 Astro 将您的[`hybrid` or `server` 渲染站点](/zh-cn/core concepts/rendering modes/#按需渲染)部署到[Cloudflare](https://www.cloudflare.com/).

如果您使用 Astro 作为[静态站点构建器](/zh-cn/core-concepts/rendering-modes/#预渲染)，则不需要适配器。
...
Astro 包含了一个 `asto add` 命令，用于自动设置官方集成。如果您愿意，可以改为[手动安装集成](#手动安装)。

在您的 Astro 项目中添加 Cloudflare 适配器，以在您的 Astro 项目中使用 `asto add` 命令启用SSR。这将安装`@Astrjs/Cloudflare` 并一步到位地对您的 `aster.fig.*` 文件进行相应的更改。
...
然后，将适配器和所需的[按需渲染模式](/zh-cn/core concepts/rendering modes/#按需渲染)添加到 `astro.config.*` 文件中：
...
查看 Cloudflare 的存储库以获取有关 [CF\_BINDING](https://github.com/cloudflare/workers-sdk/blob/eaa16db25103d26ef31499634a31b86caccdf7aa/packages/wrangler/src/api/startDevWorker/types.ts#L190-L219) 的更多类型信息。
