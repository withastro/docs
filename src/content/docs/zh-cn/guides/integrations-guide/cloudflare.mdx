...
如果你想使用自动生成的 `_routes.json`，但又想要包含额外的路由（例如，当你有自定义函数在 `functions` 文件夹中时），那么你可以使用 `routes.include` 选项将额外的路由添加到 `include` 数组中。
...
以下是导入一个 Wasm 模块的示例，该模块通过将请求的数字参数相加来返回响应：
...
虽然这个例子很简单，但是 Wasm 用来加速在不涉及大量 I/O 的情况下的计算密集型操作，比如嵌入图像处理库。
