...
* **`passthrough`：** 使用现有的 [`noop`](/en/guides/images/#configure-no-op-passthrough-service) 服务。
...
`persist` 属性定义了绑定数据是否以及在哪里持久化。`true` 默认使用 Wrangler 相同的位置，因此两者之间可以共享数据。如果是 `false`，则不会向文件系统持久化或从文件系统读取任何数据。
...
在 `routes.extend.exclude` 数组中配置要从按需渲染中排除的路由。这些路由将会被预渲染并静态提供，而不会调用服务端渲染（SSR）函数。此外，你还可以使用这个选项直接提供任何静态资源（例如：图片、字体、css、js、html、txt、json 等）文件，无需通过 SSR 函数路由请求。
