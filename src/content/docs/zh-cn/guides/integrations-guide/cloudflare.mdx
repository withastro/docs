...
你可以在适配器配置中 [指定要遵循的其他路由模式](#routesextend)，或者创建您自己的自定义 `_routes.json` 文件以完全覆盖自动生成。

#### 自定义 `_routes.json`

创建自定义的 `public/_routes.json` 将覆盖这种自动生成的文件。参阅 [Cloudflare 关于创建自定义 `routes.json` 的文档](https://developers.cloudflare.com/pages/platform/functions/routing/#create-a-_routesjson-file) 了解更多细节。
