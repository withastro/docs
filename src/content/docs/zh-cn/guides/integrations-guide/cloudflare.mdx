...
[Secrets](https://developers.cloudflare.com/workers/configuration/secrets/) 是特殊类型的环境变量。它能让你为 Worker 附加加密的文本值。它们需要以不同的方式定义，以确保在设置完成后，不会在 Wrangler 或 Cloudflare 仪表板中明文可见。
...
如果要为本地开发设置机密，请在 Astro 项目的根目录中添加 `.dev.vars` 文件：
