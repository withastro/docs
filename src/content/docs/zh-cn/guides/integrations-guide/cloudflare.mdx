...
此适配器允许 Astro 将你的[`hybrid` 或者 `server` 渲染站点](/zh-cn/guides/on-demand-rendering/)部署到[Cloudflare](https://www.cloudflare.com/).

如果你使用 Astro 作为[静态站点构建器](/zh-cn/guides/on-demand-rendering/#add-an-adapter)，则不需要适配器。
