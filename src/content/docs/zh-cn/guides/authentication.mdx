...
认证和授权是两种安全过程，帮助你控制谁可以访问你的网站或应用的不同部分。认证是验证访客身份的过程，而授权是允许访客访问受保护的路由和资源的过程。
...
<ReadMore>查看我们的专门指南，了解如何使用 Supabase 添加认证或使用 Firebase 添加认证。</ReadMore>
...
Auth.js 是一个与框架无关的验证解决方案。社区框架适配器 [`auth-astro`](https://www.npmjs.com/package/auth-astro) 可用于 Astro。
