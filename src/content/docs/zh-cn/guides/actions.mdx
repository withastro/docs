...
Astro Actions 允许你定义和调用具有类型安全性的后端函数。Actions 为你执行数据获取、JSON 解析和输入验证。与使用 [API 端点](/zh-cn/guides/endpoints/) 相比，这可以大大减少所需的样板代码量。
...
Actions 作为函数从 `astro:actions` 模块中导入。导入 `actions` 并在 [UI 框架组件](/zh-cn/guides/framework-components/)、[表单 POST 请求](#call-actions-from-an-html-form-action) 等客户端中使用，或在 Astro 组件中的 `<script>` 标签中调用它们。
...
- 你可以设置一个状态码，例如 `404 - 未找到` 或 `401 - 未经授权`。这可以通过让你查看每个请求的状态码来提高开发和生产中的错误调试。

- 在你的应用程序代码中，所有错误都会传递到 action 结果上的 `error` 对象。这避免了对数据进行 `undefined` 检查的需要，并允许你根据出现了什么问题向用户显示有针对性的反馈。

### 创建一个 `ActionError`

为了抛出一个错误，从 `astro:actions` 模块中导入 `ActionError()` 类。传递一个人类可读的状态 `code`（例如 `"NOT_FOUND"` 或 `"BAD_REQUEST"`），以及一个可选的 `message` 以提供有关错误的更多信息。

这个例子在 `likePost` action 中抛出一个错误，当用户未登录时，通过检查一个假设的 “user-session” cookie 进行身份验证：
...
下面的示例显示了一个验证过的 newsletter 注册表单，接受用户的电子邮件并要求“服务条款”复选框的同意。
...
3. 添加一个 `<script>` 到 HTML 表单中以提交用户输入。这个例子覆盖了表单的默认提交行为，调用 `actions.newsletter()`，并使用 `navigate()` 函数重定向到 `/confirmation`：
    ...
    <ReadMore>请参阅 [“从 HTML 表单操作调用 action”](#从-html-表单操作调用-action) 了解提交表单数据的另一种方法</ReadMore>
...
为了检索输入错误，使用 `isInputError()` 工具函数检查错误是否由无效输入引起。输入错误包含一个 `fields` 对象，其中包含每个未能验证的输入名称的消息。你可以使用这些消息提示用户纠正他们的提交。
...
:::note
页面必须在使用表单操作调用 actions 时进行按需渲染。在使用此 API 之前，请[确保页面上已禁用预渲染](/zh-cn/guides/server-side-rendering/#在-hybrid-模式中选择退出预渲染)。
:::

你可以在任何 `<form>` 元素上使用标准属性启用零 JS 表单提交。无需客户端 JavaScript 的表单提交可能对于 JavaScript 加载失败时的回退，或者如果你更喜欢完全从服务器处理表单时很有用。

调用 [Astro.getActionResult()](/zh-cn/reference/api-reference/#astrogetactionresult) 在服务器上返回表单提交的结果（`data` 或 `error`），可以用于动态重定向、处理表单错误、更新 UI 等。

调用一个 action 时，Astro 会自动处理表单提交，并将结果返回给你的页面。这使得你可以在表单提交后更新 UI，重定向到一个新页面，或者处理任何错误。
...
为了在一个 action 成功时导航到一个不同的页面，你可以在 `action` 属性中添加一个路径。
...
:::note
Astro 会使用一个单次使用的 cookie 持久化 action `data` 和 `error`。这意味着 `getActionResult()` 仅在第一次请求时返回结果，并在重新访问页面时返回 `undefined`。
:::
...
由 `Astro.getActionResult()` 返回的结果是一次性的，每当页面刷新时都会重置为 `undefined`。这对于[处理表单操作错误](#处理表单操作错误)和在成功时向用户显示临时通知非常理想。

:::tip
如果你需要在页面刷新时显示结果，请考虑将结果存储在数据库中或[在 cookie 中](/zh-cn/reference/api-reference/#astrocookies)。
:::

向 `Astro.getActionResult()` 传递一个 action，并使用返回的 `data` 属性来渲染你想要显示的任何临时 UI。这个例子使用 `addToCart` action 返回的 `productName` 属性来显示一个成功消息：
...
:::caution
Action 数据使用一个持久化 cookie 传递。**这个 cookie 没有加密**。通常，我们建议从你的 action `handler` 返回所需的最小信息，以避免漏洞，并将其他敏感信息持久化到数据库中。
