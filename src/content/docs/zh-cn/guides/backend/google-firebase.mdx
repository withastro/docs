...
- Firebase 凭据：你需要两组凭据将 Astro 连接到 Firebase：
...
要将 Firebase 凭据添加到 Astro，请在项目的根目录下创建一个名为 `.env` 的文件，并添加以下变量：
...
如果你希望为 Firebase 环境变量启用 IntelliSense 功能，可以在 `src/` 目录中编辑或创建 `env.d.ts` 文件，并配置你的类型定义：
...
:::tip
了解更多在 Astro 有关于 [环境变量](/en/guides/environment-variables/) 以及 `.env` 文件的内容。
:::
...
要将 Astro 连接到 Firebas 需要使用以下单行命令，可以根据你喜欢的包管理工具安装以下包：
...
在 `client.ts`文件中，请添加以下代码来使用你的网页应用凭据和 `firebase` 包来初始化客户端中的 Firebase：
...
:::note
请记得使用你自己的网页应用凭据替换 `firebaseConfig` 对象。
:::

在 `server.ts` 文件中，请添加以下代码来使用你的项目凭据和 `firebase-admin` 包来初始化服务器端的 Firebase：
...
:::note
请记得使用你自己的项目凭据替换 `serviceAccount` 对象。
:::
...
`register.ts` 文件包含使用 Firebase 注册用户的代码：
...
-   `src/pages/edit/[id].ts` – 包含一个表单用于编辑一个朋友和一个删除朋友的按钮。
-   `src/pages/friend/[id].ts` – 包含一个朋友的详细信息。
