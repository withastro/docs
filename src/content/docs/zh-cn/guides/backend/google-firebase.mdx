...
## 在 Astro 中初始化 Firebase \{#initializing-firebase-in-astro\}
...
### 添加 Firebase 凭据 \{#adding-firebase-credentials\}
...
## 使用 Firebase 添加身份验证 \{#adding-authentication-with-firebase\}
...
### 创建身份验证的服务器端点 \{#creating-auth-server-endpoints\}
...
### 添加 OAuth 提供商 \{#adding-oauth-providers\}
...
## 连接到 Firebase 数据库 \{#connecting-to-firestore-database\}

### 前期准备 \{#prerequisites-2\}

- 一个如 [在 Astro 中初始化 Firebase](#initializing-firebase-in-astro) 部分所述初始化了 Firebase 的 Astro 项目。
...
### 创建服务器端点 \{#creating-the-server-endpoints\}
...
#### 编辑或删除一条记录 \{#edit-or-delete-a-record\}
...
#### 展示单独的记录 \{#display-an-individual-record\}
...
#### 通过编辑按钮来展示多条记录 \{#display-a-list-of-records-with-an-edit-button\}
