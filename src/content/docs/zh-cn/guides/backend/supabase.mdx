...
## 在 Astro 中初始化 Supabase \{#initializing-supabase-in-astro\}
...
### 添加 Supabase 凭据 \{#adding-supabase-credentials\}
...
## 使用 Supabase 添加身份验证 \{#adding-authentication-with-supabase\}

Supabase 提供了开箱即用的身份验证功能。它支持电子邮件（或密码身份）验证以及如GitHub, Google等多个提供商的 OAuth 身份验证。

### 前期准备 \{#prerequisites-1\}

- [一个已经初始化 Supabase 的 Astro 项目](#initializing-supabase-in-astro).
...
### 添加 OAuth 提供商 \{#adding-oauth-authentication\}
