...
3. （可选）为你的项目安装其他 Prettier 插件，并将它们添加至配置文件中。这些附加的插件可能需要以特定的方式排列。例如，如果你正在使用 Tailwind，那么 `prettier-plugin-tailwindcss` 就必须放置在 [Prettier 插件数组的最后一位](https://github.com/tailwindlabs/prettier-plugin-tailwindcss#compatibility-with-other-prettier-plugins)。
