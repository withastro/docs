...
如果你希望默认语言不像其他语言那样在 URL 中可见，后续会有 [关于隐藏默认语言的介绍](/zh-cn/recipes/i18n/#hide-default-language-in-the-url)。

<ReadMore> 请参阅 [参考资料部分](#resources) 以获取与之相关的外部链接，例如从右到左（RTL）样式和选择语言标签。</ReadMore>

## 操作步骤 \{#recipe\}

### 为页面设置多语言 \{#set-up-pages-for-each-language\}
...
2. 设置 `src/pages/index.astro` 重定向到你的默认语言。
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        如果你正好使用 SSR 适配器，那么你也可以使用 [`Astro.redirect`](/zh-cn/guides/routing/#dynamic-redirects) 来重定向到默认语言。
...
### 为翻译内容创建内容集合 \{#use-collections-for-translated-content\}
...
3. 使用 [动态路由](/zh-cn/guides/routing/#dynamic-routes) 来获取并基于 `lang` 和 `slug` 参数的内容渲染内容。
    ...
    <ReadMore>你可以在这获取有关更多 [动态路由](/zh-cn/guides/routing/#dynamic-routes) 的内容。</ReadMore>
...
### 允许用户切换不同语言 \{#let-users-switch-between-languages\}
...
### 在 URL 中隐藏默认语言 \{#hide-default-language-in-the-url\}
