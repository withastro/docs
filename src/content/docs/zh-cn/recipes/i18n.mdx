---
title: 添加 i18n 功能
description: 通过动态路由和文档集来让你的 Astro 站点支持国际化。
type: recipe
i18nReady: true
---
...
2. 设置 `src/pages/index.astro` 重定向到你的默认语言。
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        如果你正好使用 SSR 适配器，那么你也可以使用 [`Astro.redirect`](/en/core-concepts/routing/#dynamic-redirects) 来重定向到默认语言。
...
2. 创建一个 `src/content/config.ts` 文件并且导出对应的文档集。
    ...
    ```
    📚 你可以在这获取有关更多 [文档集](/en/guides/content-collections/) 的内容。

3. 使用 [动态路由](/en/core-concepts/routing/#dynamic-routes) 来获取并基于 `lang` 和 `slug` 参数的内容渲染内容。
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        在 [SSR 模式](/en/guides/server-side-rendering/) 下，你可以直接获取到请求的内容：
    ...
    📚 你可以在这获取有关更多 [动态路由](/en/core-concepts/routing/#dynamic-routes) 的内容。


    :::tip[日期格式化]
    在上述例子中使用了内置的 [`toLocaleString()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/toLocaleString) 日期格式化方法来创建为人类可读的日期字符串，以确保日期和时间格式与用户所选择的语言相匹配。
    :::

### 翻译 UI 标签

创建条目字典来翻译你网站上用户界面的元素标签，这样可以让访问者在他们的语言环境下更好地体验你的网站。
...
4. 每个页面必须在 `<html>` 元素中包含一个 `lang` 属性以匹配页面的语言。在这个例子中，[可重用布局](/en/core-concepts/layouts/) 会从当前路由中提取语言：
...
2. 将 `<LanguagePicker />` 组件到你的网站中，以便它能在每个页面上显示。如下例子则在基础布局中将它添加到了网站的页脚部分：
