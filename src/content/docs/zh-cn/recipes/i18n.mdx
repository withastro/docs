...
2. 设置 `src/pages/index.astro` 重定向到你的默认语言。
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        如果你正好使用 SSR 适配器，那么你也可以使用 [`Astro.redirect`](/zh-cn/basics/routing/#动态重定向) 来重定向到默认语言。
...
3. 使用 [动态路由](/zh-cn/basics/routing/#动态路由) 来获取并基于 `lang` 和 `slug` 参数的内容渲染内容。
    ...
    <ReadMore>你可以在这获取有关更多 [动态路由](/zh-cn/basics/routing/#动态路由) 的内容。</ReadMore>
