...
2. 设置 `src/pages/index.astro` 重定向到你的默认语言。
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        如果你正好使用 SSR 适配器，那么你也可以使用 [`Astro.redirect`](/zh-cn/core-concepts/routing/#dynamic-redirects) 来重定向到默认语言。
...
2. 创建一个 `src/content/config.ts` 文件并且导出对应的文档集。
    ...
    ```
    📚 你可以在这获取有关更多 [文档集](/zh-cn/guides/content-collections/) 的内容。

3. 使用 [动态路由](/zh-cn/core-concepts/routing/#dynamic-routes) 来获取并基于 `lang` 和 `slug` 参数的内容渲染内容。
    ...
    📚 你可以在这获取有关更多 [动态路由](/zh-cn/core-concepts/routing/#dynamic-routes) 的内容。
...
4. 每个页面必须在 `<html>` 元素中包含一个 `lang` 属性以匹配页面的语言。在这个例子中，[可重用布局](/zh-cn/core-concepts/layouts/) 会从当前路由中提取语言：
