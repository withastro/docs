...
1. [**使用 Astro Studio 网页界面**](https://studio.astro.build) 来从一个新的或现有的 GitHub 仓库创建项目。

    要开始，请点击页眉中的“创建项目”按钮并遵循指示。Astro Studio 将连接到你的 GitHub 仓库并为你的项目创建一个新的托管数据库。

2. **使用 Astro Studio 命令行界面** 来从任何本地 Astro 项目创建。你可以运行以下命令来开始：

    <PackageManagerTabs>
    <Fragment slot="npm">
    ```shell
    # 使用你的 GitHub 账户登录 Astro Studio
    npx astro login
    ...
    登录并成功链接后，你可以运行所有 Astro DB 命令来管理你的远程数据库。

    <ReadMore>查看 [Astro DB CLI 参考](/zh-cn/guides/integrations-guide/db/#astro-db-cli-参考) 以获取所有可用命令。</ReadMore>
