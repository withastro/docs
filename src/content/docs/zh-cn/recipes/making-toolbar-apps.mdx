...
## 构建一个激励性的开发者工具栏应用 \{#build-a-motivational-dev-toolbar-app\}
...
### 创建 Astro 集成 \{#creating-the-astro-integration\}
...
2. 在 `my-integration.ts` 中，添加以下代码，来提供你的集成的名称和用于通过 `astro:config:setup` 钩子添加你的开发者工具栏应用所需的 [`addDevToolbarApp()` 函数](/zh-cn/reference/dev-toolbar-app-reference/#toolbar-app-integration-setup)：
...
## 使用 UI 框架构建应用 \{#building-apps-with-a-ui-framework\}
...
:::note 
无论你选择使用 JavaScript 还是某个 UI 框架来构建你的开发工具栏应用，你仍然需要[创建集成](#creating-the-astro-integration)，将你的应用添加到开发工具栏中。
:::

### 无需构建步骤 \{#without-a-build-step\}
