...
Astro 包含了一个[开发者工具栏](/zh-cn/guides/dev-toolbar/)，你可以使用它来检查你的网站、检查可访问性和性能问题等。这个工具栏可以通过自定义应用进行扩展。
...
在本页早些时候使用 JavaScript 构建到你现有的 Astro 项目中的同一个激励性开发工具栏应用，也可以使用 UI 框架（例如 Preact）来构建。根据你选择的框架，你可能需要或不需要构建步骤。
...
在这两种情况下，你现在可以启动你的项目，并在开启应用时看到屏幕左上角显示的励志信息。
...
4. 运行 `tsc` 来构建你的工具栏应用，或者使用 `tsc --watch` 在你进行更改时自动重建你的应用。
