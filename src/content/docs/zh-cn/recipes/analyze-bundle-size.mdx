---
title: 分析打包大小
description: 了解如何使用 `rollup-plugin-visualizer` 来分析 Astro 生成的捆绑包。
i18nReady: true
type: recipe
---
...
了解 Astro 的捆绑包，对于提高网站性能很重要。对捆绑包进行可视化，可以为减少你项目中的捆绑包大小提供线索。

## 方案

[`rollup-plugin-visualizer` 库](https://github.com/btd/rollup-plugin-visualizer)允许你可视化和分析你的 Rollup 捆绑包，看看哪些模块正在占用空间。
<Steps>
...
4. 找到你项目中的 `stats.html` 文件。
    ...
    对于完全静态的站点，它将会位于你的 `dist/` 目录的根目录下，并允许你查看捆绑包中包含的内容。
...
:::note
鉴于 Astro 独特的水合方式，该版本的捆绑包并不一定代表了用户将收到的捆绑包。
