...
端点可以用来服务多种类型的数据。在本节示例中，将展示如何通过页面的组件脚本调用了一个服务器端点来显示问候语，而不需要额外的获取请求。
...
2. 在任何 Astro 页面上，从端点中导入 `get()` 方法。使用 [`Astro` 全局对象](/zh-cn/reference/api-reference/#astro-global) 来提供请求的上下文，并在使用页面的响应对象。
