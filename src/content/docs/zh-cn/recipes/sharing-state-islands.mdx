...
像 React 或者 Vue 这样的 UI 框架可能鼓励使用 ["context"](https://react.dev/learn/passing-data-deeply-with-context) 来为其他组件提供上下文信息。但是在 Astro 或者 Markdown 中的 [部分激活组件 (partially hydrating components)](/zh-cn/guides/framework-components/#hydrating-interactive-components) 不能使用上下文封装。
...
## 为什么使用 Nano Stores? \{#why-nano-stores\}
...
- 从一个 `.astro` 文件或者 [非激活组件](/zh-cn/guides/framework-components/#hydrating-interactive-components) 写入状态库将不会影响 [客户端组件](/zh-cn/reference/directives-reference/#client-directives) 值的获取。
...
## 安装 Nano Stores \{#installing-nano-stores\}
...
## 用例 - 电商购物车抽屉 \{#usage-example---ecommerce-cart-flyout\}
