...
## 使用 Bun 创建一个新的 Astro 项目 \{#create-a-new-astro-project-with-bun\}
...
## 使用 Astro 集成 \{#using-astro-integrations\}
...
## 通过 Bun 运行 Astro \{#run-astro-with-bun\}
...
### 通过 Bun 构建你的站点 \{#building-your-site-with-bun\}
...
## 通过 `@astrojs/node` 来让 Bun 添加 SSR \{#add-ssr-with-bun\}
...
对于其余的配置和开始你的第一个测试，请参阅 [Astro 测试指南](/zh-cn/guides/testing/#configuration) 中的 Cypress 测试部分。
...
要创建你的第一个 Playwright 测试，请按照 [Astro 测试指南](/zh-cn/guides/testing/#create-your-first-playwright-test) 中的其余 Playwright 测试流程。
