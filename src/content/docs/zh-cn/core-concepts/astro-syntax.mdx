...
**如果你了解 HTML，那么你已经足够了解编写第一个 Astro 组件。**

Astro 组件语法是 HTML 的超集。这个语法是[设计用来让任何有编写 HTML 或 JSX 经验的人感到熟悉](#astro-和-jsx-间的差异) ，并增加了对包含组件和 JavaScript 表达式的支持。
...
---
<div>
  <h1>Hello {name}!</h1>  <!-- 输出 <h1>Hello Astro!</h1> -->
</div>
```
...
:::caution
HTML 属性将转换为字符串，因此不可能将函数和对象传递给 HTML 元素。
例如，不能为 Astro 组件中的 HTML 元素分配事件处理程序:
...
相反，使用客户端脚本来添加事件处理程序，就像在普通的 JavaScript 中那样:
...
可以在类 JSX 函数中使用局部变量来生成动态生成的 HTML 元素:
...
还可以通过将变量设置为 HTML 标签或组件导入来使用动态标签:
...
与 JavaScript 或 JSX 不同，Astro 组件模板可以渲染多个元素，而不需要将所有元素包装在一个 `<div>` 或 `<>` 中。
...
但是，当使用表达式动态创建多个元素时，应该像在 JavaScript 或 JSX 中那样将这些元素包装在一个  **片段** 中。Astro 支持使用 `<Fragment> </Fragment>` 或简写成 `<> </>`。
...
在添加[`set:*` 指令](/zh-cn/reference/directives-reference/#set:html)时，片段对于避免包装元素也很有用，如下面的示例所示:
