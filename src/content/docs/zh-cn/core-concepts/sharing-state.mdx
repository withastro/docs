...
像 Vue 或者 React 这样的UI框架可能会鼓励使用 ["上下文" 提供者("context" provider)](https://react.dev/learn/passing-data-deeply-with-context) 来为其他组件提供上下文信息。 但是在 Astro 或者 Markdown 中的 [部分激活组件(partially hydrating components)](/zh-cn/core-concepts/framework-components/#激活组件) 不能使用上下文封装。
...
<UIFrameworkTabs>
<Fragment slot="preact">
```jsx title="src/components/CartFlyoutToggle.jsx"
...
<UIFrameworkTabs>
<Fragment slot="preact">
```jsx title="src/components/CartFlyout.jsx"
import { useStore } from '@nanostores/preact';
import { isCartOpen } from '../cartStore';
...
<UIFrameworkTabs>
<Fragment slot="preact">
```jsx title="src/components/AddToCartForm.jsx"
