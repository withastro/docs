...
要将多个 HTML 元素传递到组件的 `<slot/>` 占位符中而无需包装 `<div>`，在 [Astro 的 `<Fragment/>` 组件](/zh-cn/reference/api-reference/#fragment-)上可以使用 `slot=""` 属性：
...
:::note
在 Astro v4.2.0 中添加了动态生成 Astro 插槽名称的功能，例如在 map 函数中生成。而对于 Astro 的早期版本，则最好在框架本身中生成这些动态插槽。
:::
...
<ReadMore>阅读更多关于如何在 Astro 项目中使用 [UI 框架组件](/zh-cn/core-concepts/framework-components/) 的内容。</ReadMore>
