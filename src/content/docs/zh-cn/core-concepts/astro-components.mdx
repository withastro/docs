...
Astro 组件最重要的一点是它们**不会在客户端上渲染**。它们在构建时或使用 [服务器端渲染（SSR）](/zh-cn/guides/server-side-rendering/) 按需呈现为 HTML。你可以在组件前端包含 JavaScript 代码，所有这些代码都将从发送到用户浏览器的最终页面中删除。结果是一个更快的网站，默认情况下不用任何 JavaScript。
...
组件被设计为**可重用**和**可组合**。你可以使用其他组件中的组件来构建越来越高级的 UI。例如，`Button` 组件可用于创建 `ButtonGroup` 组件：
