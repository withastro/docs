...
Astro 使用标准 HTML [`<a>` 元素](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a) 在路由之间导航。 没有提供特定框架的 `<Link>` 组件。
...
如果你的URL路由需要更加灵活，你可以在 `.astro` 文件名中使用一个剩余参数（`[...path]`），去匹配任何深度的文件路径。
