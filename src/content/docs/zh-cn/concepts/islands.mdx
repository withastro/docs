---
title: 群岛架构
description: 了解 Astro 的群岛架构是怎样帮助网站保持快速的。
---
...
Astro 因为是第一个内置选择性激活的主流 JavaScript Web 框架而变得流行，它使用了由 Sylor-Miller 首次提出的相同的组件群岛模式。此后，我们在 Sylor-Miller 的原始工作基础上扩充并发展，而这些都推动了类似的组件群岛通过服务端来动态渲染内容。

## 什么是群岛？

在 Astro 中，群岛是 HTML 静态页面上的增强的动态 UI 组件。

[**客户端群岛**](#client-islands)是一个交互式的 JavaScript UI 组件，它在页面的其余部分中单独进行水合，而[**服务器群岛**](#server-islands)作为一个 UI 组件则是在页面的其余内容中，通过服务器端单独渲染其动态内容。
...
你可以将群岛想象成一个个交互式小组件，漂浮在静态、轻量级、服务器渲染的 HTML 海洋中。可以为个性化或动态服务器渲染元素添加服务器岛屿，例如已登录访客的个人资料图片。

<IslandsDiagram>
    <Fragment slot="headerApp">页头（可交互群岛）</Fragment>
    <Fragment slot="sidebarApp">侧边栏（静态 HTML）</Fragment>
    <Fragment slot="main">
        静态内容（比如文本、图片...）
    </Fragment>
    <Fragment slot="carouselApp">图像轮播（可交互群岛）</Fragment>
...
群岛始终独立于页面上的其他群岛运行，且一个页面上可以存在多个群岛。尽管客户端群岛在不同的组件上下文中运行，但它们仍然可以共享状态并相互通信。
...
只需使用一个 `client:*` 指令，就可以将任何静态 UI 组件转变为交互式群岛。然后 Astro 会自动构建并打包你的客户端 JavaScript，以优化性能。
...
将 [`server:defer` 指令](/zh-cn/reference/directives-reference/#服务器指令) 添加到页面上的任何 Astro 组件，来将其变成自己的服务器群岛：
...
另一个优势是提供出色的游客体验。服务器群岛经过了优化且加载速度快，通常情况下甚至在浏览器绘制页面之前即可完成。但在渲染岛屿所用的那短短的一段时间内，你可以展示自定义的回退内容，从而防止发生任何布局偏移。
...
<ReadMore>详细了解项目中的[使用服务器群岛](/zh-cn/guides/server-islands/)</ReadMore>
