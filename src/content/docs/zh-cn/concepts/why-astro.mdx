...
- **[UI 无关](/zh-cn/guides/framework-components/)**：支持 React、Preact、Svelte、Vue、Solid、Lit、HTMX、Web 组件等等。
- **[服务器优先](/zh-cn/basics/rendering-modes/)**：将沉重的渲染移出访问者的设备。
...
1. **[内容驱动](#content-driven)**：Astro 专为展示你的内容而设计。
2. **[服务器优先](#server-first)**：网站在服务器上渲染 HTML 时运行速度更快。
3. **[默认快速](#fast-by-default)**：在 Astro 中应当不可能做出缓慢的网站。
4. **[易于使用](#easy-to-use)**：你不需要是一个专家即可使用 Astro 做点什么。
5. **[以开发者为中心](#developer-focused)**：你应该拥有成功所需的资源。
