...
你可以通过向 `create astro` 命令传递 `--add` 参数，它会在开始一个新的 Astro 项目的同时，为其安装任何支持 `astro add` 命令的 [官方集成](/zh-cn/guides/integrations/) 或社区集成。
...
2. 安装 Astro
    ...
    :::note[重要]
    Astro 必须安装在本地，不能全局安装。确保你没有运行 `npm install -g astro`、`pnpm add -g astro`、`bun add -g astro` 或 `yarn add global astro`。
    :::
