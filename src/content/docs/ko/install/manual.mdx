...
이 가이드는 [CLI 도구](/ko/install/auto/)를 선호하지 않는 분들을 위한 새 Astro 프로젝트를 수동으로 설치하고 구성하는 단계를 안내합니다. if you prefer not to use [the automatic CLI tool](/en/install/auto/).
...
프로젝트 이름으로 빈 디렉터리를 생성한 다음 해당 디렉토리로 이동합니다.
...
새 디렉토리에 들어가면 프로젝트 `package.json` 파일을 생성합니다. 이렇게 하면 Astro를 포함한 프로젝트 종속성을 관리할 수 있습니다. 만약 이 파일 형식에 익숙하지 않은 경우 다음 명령을 실행하여 파일을 생성하세요.
...
프로젝트에 React, Svelte 등과 같은 [UI 프레임워크 컴포넌트](/en/core-concepts/framework-components/)들을 추가하거나 Tailwind 또는 Partytown과 같은 다른 도구를 사용하려는 경우, [여기에서 수동으로 인터그레이션을 가져와 구성할 수 있는 방법](/en/guides/integrations-guide/) 을 알 수 있습니다.

📚 자세한 내용은 Astro의 [API 구성 참조](/en/reference/configuration-reference/)를 참조하세요.
...
📚 자세한 내용은 Astro의 [타입스크립트 설정 가이드](/en/guides/typescript/#setup)를 참조하세요.
...
이 가이드를 완전히 따랐다면 [2단계: Astro 시작하기](/en/install/auto/#2-start-astro-)로 바로 이동하여 Astro를 처음 실행하는 방법을 계속 학습할 수 있습니다.