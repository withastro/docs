...
페이지의 `<head>`에 임포트되어 추가되어야 합니다. 이는 전역 사용을 위해 공통 사이트 레이아웃에서 사용되는 `Head.astro`와 같은 컴포넌트에서 주로 수행되지만, 필요한 경우 개별 페이지에 추가할 수도 있습니다.
...
Astro 구성에 등록된 [`cssVariable`](#cssvariable-1):
...
[프리로드 링크](https://web.dev/learn/performance/optimize-web-fonts#preload)를 출력할지 여부:
...
`preload` 지시문을 사용하면 브라우저가 페이지 로드 중에 가능한 모든 글꼴 링크를 즉시 다운로드하기 시작합니다.

#### 세밀한 프리로드 (Granular preloads)
...
어떤 글꼴 파일을 프리로드할지 선택적으로 제어하려면 프리로드할 글꼴 `weight`, `style`, 또는 `subset`의 조합을 설명하는 객체 배열을 제공할 수 있습니다.

다음 예시는 `latin` 서브셋에서 `400` 두께 또는 `normal` 스타일인 글꼴 파일만 프리로드합니다.
...
[`fontData`](#fontdata) 객체를 사용하면 예를 들어 [API 라우트](/ko/guides/endpoints/#server-endpoints-api-routes)에서 또는 나만의 메타 태그를 생성하기 위해 하위 수준의 글꼴 패밀리 데이터를 프로그래밍 방식으로 가져올 수 있습니다.

그런 다음 [`getFontBuffer()`](#getfontbuffer) 함수를 가져온 URL에서 사용하여 글꼴 버퍼를 얻을 수 있습니다. 예를 들어, [API 라우트](/ko/guides/endpoints/#server-endpoints-api-routes)에서 [satori](https://github.com/vercel/satori)를 사용해 OpenGraph 이미지를 생성할 때 적절한 [형식(formats)](#formats) 구성과 결합하여 사용할 수 있습니다.
...
**타입:** `(url: string) => Promise<Buffer>`<br />
<Since v="5.16.13" />
</p>

[`fontData`](#fontdata)에서 얻은 소스 URL로부터 글꼴 파일의 버퍼를 반환합니다.

:::caution
`getFontBuffer()`는 서버에서만 사용할 수 있습니다. 클라이언트에서 이 함수를 호출하면 에러가 발생합니다.
:::

## 글꼴 구성 참조

글꼴의 모든 속성은 Astro 구성에서 설정해야 합니다. [글꼴 제공자](#사용-가능한-글꼴-제공자)로부터 로드되는 데이터를 사용자 정의하려면 이를 설정하세요. 예를 들어 특정 글꼴 두께나 스타일만 다운로드하도록 할 수 있습니다. 더 자세한 제어를 위해 [세부 글꼴 구성](#세부-글꼴-구성-granular-font-configuration)을 사용할 수 있습니다.
...
의도한 글꼴의 모양과 일치하는 [일반 패밀리 이름(generic family name)](https://developer.mozilla.org/en-US/docs/Web/CSS/font-family#generic-name)을 적어도 하나는 지정하세요. 그러면 Astro는 글꼴 메트릭을 사용하여 [최적화된 폴백](https://developer.chrome.com/blog/font-fallbacks) 생성을 시도합니다. 이 최적화를 비활성화하려면 `optimizedFallbacks`를 false로 설정하세요.
...
[글꼴 두께](https://developer.mozilla.org/en-US/docs/Web/CSS/font-weight)의 배열입니다. 구성에 값을 지정하지 않으면 불필요한 다운로드를 방지하기 위해 기본적으로 두께 `400`만 포함됩니다. 다른 글꼴 두께에 접근하려면 이 속성을 포함해야 합니다.
...
글꼴이 다운로드되어 사용할 준비가 된 시점에 따라 [글꼴이 표시되는 방식](https://developer.mozilla.org/en-US/docs/Web/CSS/@font-face/font-display)을 정의합니다.
...
특정 [유니코드 캐릭터 범위](https://developer.mozilla.org/en-US/docs/Web/CSS/@font-face/unicode-range)에 따라 글꼴을 다운로드하고 사용해야 하는 시점을 결정합니다. 페이지의 문자가 구성된 범위와 일치하면 브라우저가 글꼴을 다운로드하고 모든 문자를 페이지에서 사용할 수 있게 됩니다. 단일 글꼴에 대해 프리로드할 문자의 서브셋을 구성하려면 대신 [subsets](#서브셋-subsets) 속성을 확인하세요.
...
[글꼴 연신율(font stretch)](https://developer.mozilla.org/en-US/docs/Web/CSS/@font-face/font-stretch):
...
[타이포그래피 글꼴 기능](https://developer.mozilla.org/en-US/docs/Web/CSS/@font-face/font-feature-settings) (예: 합자(ligatures), 작은 대문자(small caps), 또는 스워시(swashes))을 제어합니다.
...
글꼴 [가변 설정(variation settings)](https://developer.mozilla.org/en-US/docs/Web/CSS/@font-face/font-variation-settings):

```js
variationSettings: "'xhgt' 0.7"
```

## 세부 글꼴 구성 (Granular font configuration)
...
글꼴 패밀리는 두께(weights)와 스타일(styles)과 같은 속성의 조합(예: `weights: [500, 600]` 및 `styles: ["normal", "bold"]`)으로 정의되지만, 이러한 조합 중 일부만 다운로드하고 싶을 수 있습니다.

어떤 글꼴 파일을 다운로드할지 더 세밀하게 제어하려면 동일한 글꼴(즉, 동일한 `cssVariable`, `name`, `provider` 속성 사용)를 서로 다른 조합으로 여러 번 지정할 수 있습니다. Astro는 결과를 병합하여 필요한 파일만 다운로드합니다. 예를 들어 일반 `500`과 `600`을 다운로드하면서 이탤릭체는 `500`만 다운로드하는 것이 가능합니다.
...
[내장 제공자](#사용-가능한-글꼴-제공자) 중 하나를 사용하고 싶지 않은 경우(예: [제3자 unifont 제공자](#제3자-unifont-제공자-지원하기)를 사용하거나 [프라이빗 레지스트리용 구축](#프라이빗-레지스트리-지원하기)을 원하는 경우) 직접 만들 수 있습니다.

사용자 정의 글꼴 제공자를 구현하는 권장 방법은 [`FontProvider` 객체](#fontprovider-객체)를 반환하고 [구성(configuration)](#config)을 매개변수로 받는 함수를 내보내는 것입니다.

### FontProvider 객체

실험적인 글꼴 API를 사용하면 통합된 방식으로 글꼴에 접근할 수 있습니다. 각 패밀리는 글꼴 페이스(font faces)를 가져오기 위해 Astro 글꼴 제공자를 사용해야 합니다.
...
다음 예시는 프라이빗 레지스트리를 위한 글꼴 제공자를 정의합니다.

<Tabs syncKey="font-provider">

<TabItem label="단순형 (Simple)">
...
<TabItem label="제공자 옵션 (Provider options)">
...
<TabItem label="패밀리 옵션 (Family options)">
