...
세션은 `session` 객체에 데이터를 저장하기 위해 [구성 가능한 세션 `driver`](#실험적-세션-활성화)에 의존합니다. [세션 쿠키](#cookies)는 식별 세션 ID를 저장합니다.

[`session` 객체](#sessions-api)를 사용하면 저장된 사용자 상태(예: 장바구니에 항목 추가) 및 세션 ID(예: 로그아웃 시 세션 ID 쿠키 삭제)와 상호 작용할 수 있습니다.
...
```js title="astro.config.mjs" ins={7-11}
  {
    ...
    experimental: {
      session: {
        // unstorage 드라이버의 이름은 camelCase 형식으로 지정합니다.
...
대부분의 경우 [`Astro.session.get()`](#sessionget)과 [`Astro.session.set()`](#sessionset)만 사용하면 됩니다. 다른 사용 가능한 메서드는 [API 섹션](#sessions-api)을 참조하세요.
