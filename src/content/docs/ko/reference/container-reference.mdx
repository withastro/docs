...
각 공식 Astro 통합에서는 전용 `container-renderer` 진입점의 `getContainerRenderer()` 도우미 함수를 사용하여 클라이언트 및 서버 렌더링 스크립트를 노출할 수 있습니다. 이러한 진입점은 `@astrojs/react`, `@astrojs/preact`, `@astrojs/solid-js`, `@astrojs/svelte`, `@astrojs/vue`, `@astrojs/mdx`에서 제공됩니다.
...
{isAuthenticated ? <span>로그인 상태입니다.</span> : <span>로그아웃 상태입니다.</span>}
```
...
test("사용자가 로그인 상태인 경우", async () => {
  ...
  // 결과에 "로그인 상태입니다."가 포함되어 있는지 확인
});

test("사용자가 로그아웃 상태인 경우", async () => {
  const result = await container.renderToString(Card, {
    locals: {
      checkAuth() {
        return false;
...
const response = await container.renderToResponse(Endpoint, {
  ...
  request: new Request("https://example.com", {
    method: "POST", // 테스트를 위해 POST 메서드 지정
  }),
});
const json = await response.json();
```
...
console.log(result); // HTML 시작 부분에 `<!DOCTYPE html>`이 포함됩니다.
```
