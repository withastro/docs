...
두 서비스 유형 모두 `options`는 사용자가 `<Image />` 컴포넌트의 속성 또는 `getImage()`에 대한 옵션으로 전달한 속성입니다. 이 훅은 이미지 구성과 Astro v7.3.0부터 로거도 받습니다.
...
이 훅은 `getURL()`에 의해 생성된 URL을 `transform` (요청 시 렌더링 및 개발 모드에서)에 사용되는 다른 속성을 가진 객체로 다시 구문 분석합니다. 이 훅은 세 가지 매개변수, 즉 구문 분석할 URL, 이미지 구성, Astro v7.3.0부터 로거를 받습니다. 빌드 중에는 사용되지 않습니다.
...
이 훅은 이미지를 변환하고 반환하며 빌드 중에 호출되어 최종 자산 파일을 생성합니다. 이 훅은 네 가지 매개변수, 즉 입력 이미지, 옵션 객체, 이미지 구성, Astro v7.3.0부터 로거를 받습니다.
...
이 훅은 사용자가 전달한 매개변수 (`options`)를 기반으로 이미지를 HTML로 렌더링하는 데 사용되는 모든 추가 속성을 반환합니다.

이 훅은 이미지 구성과 Astro v7.3.0부터 로거도 받습니다.
...
이 훅은 지정된 이미지의 여러 변형을 생성합니다. 예를 들어 `<img>` 또는 `<picture>`의 `source`에 `srcset` 속성을 생성할 수 있습니다. 이 훅은 세 가지 매개변수, 즉 옵션 객체, 이미지 구성, Astro v7.3.0부터 로거를 받습니다.
...
이 훅을 사용하면 사용자가 전달한 옵션을 검증하고 강화할 수 있습니다. 이는 기본 옵션을 설정하거나 사용자에게 매개변수가 필요함을 알리는 데 유용합니다. 이 훅은 이미지 구성과 Astro v7.3.0부터 잘못된 옵션에 대해 사용자에게 경고하는 데 사용할 수 있는 로거도 받습니다.
...
이 훅은 세 가지 매개변수, 즉 이미지 URL, 이미지 구성, Astro v7.3.0부터 로거를 받습니다.
