...
이 훅은 `getURL()`이 생성한 URL을 `transform`에 사용할 여러 속성을 담은 객체로 다시 파싱합니다. 이 훅은 구문 분석할 URL, 이미지 구성, Astro v7.3.0부터는 `logger`까지 세 가지 매개변수를 받습니다. 이 훅은 요청 시 렌더링 및 개발 모드에서만 사용되며, 빌드 중에는 사용되지 않습니다.
...
이 훅은 지정된 이미지의 여러 변형을 생성합니다. 예를 들어 `<img>`나 `<picture>`의 `source`에 대한 `srcset` 특성을 생성하기 위한 것입니다. 이 훅은 옵션 객체, 이미지 구성, Astro v7.3.0부터는 `logger`까지 세 가지 매개변수를 받습니다.
