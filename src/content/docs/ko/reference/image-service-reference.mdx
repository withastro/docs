...
로컬 서비스의 경우 이 훅은 이미지를 생성하는 엔드포인트의 URL을 반환합니다 (SSR 및 개발 모드에서). 빌드 중에는 사용되지 않습니다. `getURL()`이 가리키는 로컬 엔드포인트는 `parseURL()`과 `transform()`을 모두 호출할 수 있습니다.
...
이 훅은 `getURL()`에 의해 생성된 URL을 `transform` (SSR 및 개발 모드에서)에 사용되는 다른 속성을 가진 객체로 다시 구문 분석합니다. 빌드 중에는 사용되지 않습니다.
