...
Astro를 실행할 때 더 많은 제어가 필요한 경우, `"astro"` 패키지는 CLI 명령어를 프로그래밍 방식으로 실행할 수 있는 API를 제공합니다. 또한, 프로그래밍 방식으로 구성을 [검증](#validateconfig)하고 [병합](#mergeconfig)할 수 있는 두 가지 `astro:config` 헬퍼도 있습니다.
...
**타입:** <code>(inlineConfig: <a href="#astroinlineconfig">AstroInlineConfig</a>) => Promise\<<a href="#devserver">DevServer</a>\></code>
</p>
...
**타입:** <code>(inlineConfig: <a href="#astroinlineconfig">AstroInlineConfig</a>, options?: <a href="#buildoptions">BuildOptions</a>) => Promise\<void\></code>
</p>
...
**타입:** <code>(inlineConfig: <a href="#astroinlineconfig">AstroInlineConfig</a>) => Promise\<<a href="#previewserver">PreviewServer</a>\></code>
</p>
...
**타입:** <code>(inlineConfig: <a href="#astroinlineconfig">AstroInlineConfig</a>) => Promise\<void\></code>
</p>
...
다음 유틸리티는 [`astro/config`에서 가져와](/ko/reference/modules/astro-config/) CLI 명령에 전달하기 전에 구성을 조작하거나 유효성을 검사하는 데 사용할 수 있습니다.

### `mergeConfig()`

<p>

**타입:** <code>(defaults: <a href="/ko/reference/configuration-reference/">AstroConfig</a>, overrides: DeepPartial\<AstroConfig\>) => AstroConfig</code><br />
<Since v="5.4.0" />
</p>
...
**타입:** <code>(userConfig: any, root: string, cmd: string) => Promise\<<a href="/ko/reference/configuration-reference/">AstroConfig</a>\></code><br />
<Since v="5.4.0" />
</p>

`astro.config.mjs`에서 내보내고 Astro에 의해 가져온 것처럼 객체의 유효성을 검사합니다. 다음 인수를 받습니다:
- 유효성을 검사할 구성.
- [프로젝트의 루트 디렉터리](/ko/reference/configuration-reference/#root).
- [실행 중인 Astro 명령어](/ko/reference/cli-reference/#astro-명령) (예: `build`, `dev`, `sync`)
...
`AstroInlineConfig` 타입은 모든 [명령어 API](#명령)에서 사용됩니다. 이는 사용자 [Astro 구성](/ko/reference/configuration-reference/) 타입을 확장합니다:
...
Astro 구성 파일에 대한 사용자 지정 경로입니다.

이 값이 undefined(기본값)이거나 설정되지 않은 경우, Astro는 `root`를 기준으로 `astro.config.(js,mjs,ts,mts)` 파일을 검색하고 발견되면 구성 파일을 로드합니다.
...
Astro가 기록하는 메시지를 필터링하는 로깅 수준입니다.
