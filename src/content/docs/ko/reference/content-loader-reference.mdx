...
각 컬렉션에는 [스키마에 정의된 로더](/ko/guides/content-collections/#컬렉션-로더-정의)가 필요합니다. 프로젝트의 `src/content.config.ts` 파일에서 로더를 인라인으로 정의하거나, 여러 컬렉션 간에 하나의 로더를 공유하거나, 심지어 [로더를 NPM 패키지로 게시](/ko/reference/publish-to-npm/)하여 다른 사람들과 공유하고 통합 라이브러리에 포함시킬 수도 있습니다.
