...
이 명령어는 [공통 플래그](#공통-플래그) 및 [공통 하위 명령어](#공통-하위-명령어)와 함께 사용할 수 있습니다.
...
잠금 파일이 있는지 확인하지 않고 파일을 기록할 필요도 없도록 합니다. 이를 통해 새 개발 서버 또는 Astro v7.3.0부터 미리보기 서버를 이미 실행 중인 서버와 함께 시작할 수 있으며, 오류가 발생하지 않습니다.
