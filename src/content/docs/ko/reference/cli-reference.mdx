...
### `package.json` 스크립트 \{#packagejson-scripts\}
...
아래에 설명된 [공통 플래그](#common-flags)와 결합할 수 있습니다.
...
#### `--tsconfig <path-to-file>` \{#--tsconfig-path-to-file\}

수동으로 사용할 `tsconfig.json` 파일을 지정합니다. 제공되지 않은 경우 Astro는 구성을 찾으려고 시도하거나 프로젝트의 구성을 자동으로 추론합니다.

#### `--minimumFailingSeverity <error|warning|hint>` \{#--minimumfailingseverity-errorwarninghint\}
...
#### `--minimumSeverity <error|warning|hint>` \{#--minimumseverity-errorwarninghint\}
...
<ReadMore>[Astro의 타입 검사](/ko/guides/typescript/#type-checking)에 대해 자세히 알아보세요.</ReadMore>
...
모든 Astro 모듈에 대한 TypeScript 타입을 생성합니다. 이는 타입 추론을 위한 [`src/env.d.ts` 파일](/ko/guides/typescript/#setup)을 설정하고 생성된 타입에 의존하는 기능에 대한 모듈을 정의합니다.
...
구성에 통합을 추가합니다. [통합 안내서](/ko/guides/integrations-guide/#automatic-integration-setup)에서 자세히 알아보세요.
...
### `--host [선택적 호스트 주소]` \{#--host-optional-host-address\}
...
## 고급 API (실험적 기능) \{#advanced-apis-experimental\}
...
[Vite의 개발 서버](https://ko.vitejs.dev/guide/api-javascript#vitedevserver)에서 노출되는 [Chokidar 파일 감시기](https://github.com/paulmillr/chokidar#getting-started)입니다.
...
## Astro Studio CLI \{#astro-studio-cli\}
...
Studio에서 호스팅하는 데이터베이스에 연결합니다. 데이터베이스 관리를 위해 [Astro DB 명령](/ko/guides/integrations-guide/db/#astro-db-cli-reference)을 실행하는 데 필요합니다. 데이터베이스를 연결하려면 기존 데이터베이스를 선택하거나 새 데이터베이스를 생성하라는 메시지가 표시됩니다.
