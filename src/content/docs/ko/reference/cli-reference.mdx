...
개발 환경을 더욱 세부적으로 제어하기 위해 이 명령어는 [공통 플래그](#공통-플래그) 및 [공통 하위 명령어](#공통-하위-명령어)와 함께 사용할 수 있습니다.
...
이 명령어는 미리보기 환경을 더욱 세부적으로 제어하기 위해 [공통 플래그](#공통-플래그) 및 [공통 하위 명령어](#공통-하위-명령어)와 함께 사용할 수 있습니다.
...
[`astro dev`](#astro-dev) 명령과 Astro v7.2.0부터 지원되는 [`astro preview`](#astro-preview) 명령은 [백그라운드 서버](#--background)를 관리하기 위해 다음 하위 명령어를 지원합니다.

### `stop`

실행 중인 백그라운드 서버를 중지합니다. `SIGTERM` 신호를 보내 서버가 정상적으로 종료될 때까지 최대 5초 동안 기다립니다. 그 후에도 프로세스가 계속 실행 중이면 `SIGKILL` 신호를 보내 강제 종료합니다.

### `status`

백그라운드 서버가 실행 중인지 확인하고 URL, PID 및 가동 시간을 표시합니다.

### `logs`

백그라운드 서버의 로그를 표시합니다. 포그라운드 서버는 터미널에 로그를 직접 출력하므로, 이 명령은 [`--background` 플래그](#--background)로 시작된 서버에서만 작동합니다.
...
개발 서버를, Astro v7.2.0부터는 미리보기 서버도 분리된 백그라운드 프로세스로 시작하고 [JSON 로깅](#--json)을 활성화합니다.

서버가 시작되면 Astro는 서버의 URL, 포트, PID를 기록하는 잠금 파일(`.astro/dev.json` 또는 `.astro/preview.json`)을 생성합니다. 이를 통해 동일한 프로젝트에서 서버 인스턴스가 여러 개 실행되는 것을 방지합니다.

macOS 및 Linux에서 AI 에이전트가 감지되면 이 플래그가 자동으로 추가됩니다. Windows에서는 자동 백그라운드 실행이 일어나지 않습니다. AI 에이전트가 감지되더라도 서버는 포그라운드에 유지되어 에이전트가 프로세스를 직접 관리할 수 있습니다. 모든 플랫폼에서 필요에 따라 직접 사용할 수도 있습니다.
...
잠금 파일의 존재 여부를 확인하거나 해당 파일을 작성할 필요가 없도록 합니다. v7.3.0부터, 이를 통해 새로운 개발 서버나 프리뷰 서버가 이미 실행 중인 서버와 함께 시작될 수 있으며, 오류가 발생하지 않습니다.

```shell
astro dev --ignore-lock --port 4322
```

새 서버는 [공통 하위 명령어](#공통-하위-명령어)로 추적되지 않습니다.

[`--background`](#--background) 또는 [`--force`](#--force-string)와 함께 사용하면 오류가 발생합니다. 두 옵션 모두 잠금 파일에 의존하기 때문입니다.
