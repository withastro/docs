...
다음은 사용자 정의 `getLocalizedData()` 도우미 함수에 `routePattern`을 전달하여 라우트 세그먼트를 현지화하고 정적 경로 배열을 반환하는 방법을 보여주는 예시입니다. [params](https://github.com/ko/reference/routing-reference/#params) 객체는 각 라우트 세그먼트(`locale`, `files`, `slug`)에 대한 명시적 값으로 설정됩니다. 이 값들은 라우트를 생성하는 데 사용되며 `Astro.params`를 통해 페이지 템플릿에서 활용할 수도 있습니다.
