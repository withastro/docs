...
특별한 `src/pages/` 디렉터리에 위치한 모든 [지원되는 페이지 파일](/ko/basics/astro-pages/#지원되는-페이지-파일)이 라우트를 생성합니다. 파일 이름에 [매개변수](#매개변수)가 포함되어 있을 경우 라우트는 동적으로 여러 페이지를 생성할 수 있으며, 그렇지 않으면 단일 페이지를 생성합니다.
...
파일 경로에 하나 이상의 [매개변수](#매개변수)가 있는 단일 `.astro` 페이지 컴포넌트에서 여러 개의 사전 렌더링된 페이지 라우트를 생성하는 함수입니다. 이는 정적 사이트 빌딩이라고도 하는, 빌드 타임에 생성될 라우트에 사용됩니다.
...
이전 페이지의 URL을 가져옵니다(첫 페이지인 경우 `undefined`). [`base`](/ko/reference/configuration-reference/#base)에 대한 값이 설정된 경우, URL은 해당 값으로 시작합니다.
