...
이 함수는 [Image 컴포넌트와 동일한 속성](#image-)을 가진 (`alt`와 `sizes` 제외) 옵션 객체를 사용하며, [`GetImageResult` 객체](#getimageresult)를 반환합니다.
...
사전 정의된 속성은 `alt`와 `sizes`를 제외하고 [`<Image />` 컴포넌트](#image-)에서 허용하는 속성과 일치합니다. 다음 속성들은 다른 타입을 사용합니다.
