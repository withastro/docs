...
이 함수는 [Image 컴포넌트와 동일한 속성](#image-속성)을 가진 (`alt` 제외) 옵션 객체를 사용하며, [`GetImageResult` 객체](#getimageresult)를 반환합니다.
...
**타입:** [`AstroConfig["image"]`](/ko/reference/configuration-reference/#image-options)<br />
<Since v="3.0.9" />
</p>

사용자가 설정하여 모든 기본값과 병합된 [이미지 구성 옵션](/ko/reference/configuration-reference/#image-options)입니다.
...
<ReadMore>예시 사용법과 함께 [`src/`의 가져온 이미지](/ko/guides/images/#src-의-이미지)에 대해 알아보세요.</ReadMore>
...
[원격 이미지의 속성](#image-)을 설명합니다. 이는 [`inferSize`](#infersize)가 제공되지 않거나 `false`로 설정된 경우 [`width`와 `height`](#width-and-height-required-for-images-in-public)가 모두 필수임을 보장합니다.
...
**타입:** <code>(options: <a href="#unresolvedimagetransform">UnresolvedImageTransform</a>, imageConfig: <a href="/ko/reference/configuration-reference/#image-options">AstroConfig['image']</a>) => Promise\<<a href="#getimageresult">GetImageResult</a>\></code>
</p>

`astro:assets`의 [`getImage()`](#getimage)와 유사한 함수로, 두 개의 필수 인수([Image 컴포넌트와 동일한 속성](#image-)을 가진 `options` 객체와 [이미지 구성](/ko/reference/configuration-reference/#image-options)을 위한 객체)를 받습니다.
...
외부 이미지 변환 서비스가 제공해야 하는 훅을 정의합니다. [`getUrl()` 훅](/ko/reference/image-service-reference/#geturl)이 필수이며, 추가로 [세 가지 훅](/ko/reference/image-service-reference/#hooks)을 지원합니다.

<ReadMore>이미지 서비스 API 참조 문서에서 사용법 예시를 통해 [외부 서비스](/ko/reference/image-service-reference/#external-services) 구축 방법을 알아보세요.</ReadMore>
...
로컬 이미지 변환 서비스가 제공해야 하는 훅을 정의합니다. 이는 [`getUrl()`](/ko/reference/image-service-reference/#geturl), [`parseUrl()`](/ko/reference/image-service-reference/#parseurl), [`transform()`](/ko/reference/image-service-reference/#transform) 훅을 요구하며, [추가 훅](/ko/reference/image-service-reference/#hooks)을 지원합니다.

<ReadMore>이미지 서비스 API 참조 문서에서 사용법 예시를 통해 [로컬 서비스](/ko/reference/image-service-reference/#local-services) 구축 방법을 알아보세요.</ReadMore>
