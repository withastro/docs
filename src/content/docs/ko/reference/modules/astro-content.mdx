...
`reference()` 함수는 콘텐츠 구성에서 한 컬렉션에서 다른 컬렉션으로의 관계 또는 "reference"를 정의하는 데 사용됩니다. 이 함수는 컬렉션 이름을 허용하고 콘텐츠 프런트매터 또는 데이터 파일에 지정된 항목 식별자의 유효성을 검사한 다음, 컬렉션 이름과 참조 ID를 포함하는 객체로 참조를 변환합니다.
