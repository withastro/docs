...
모든 훅을 실행한 후 최종 값이 `undefined`인 경우, 경로는 [`output` 옵션](/ko/reference/configuration-reference/#output)에 따라 프리렌더의 기본값으로 돌아갑니다: `hybrid` 모드에서는 사전 렌더링되며, `server` 모드에서는 온디맨드로 렌더링됩니다.
