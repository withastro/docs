...
프로젝트에서 [대체 라우트가 포함된 i18n](/ko/guides/internationalization/#fallback)을 사용하는 경우, 요청된 로케일을 사용할 수 없을 때 이 라우트가 대체되는 라우트 목록이 값으로 설정됩니다. 대체 콘텐츠는 `i18n.routing.fallbackType`에 따라 리디렉션 또는 리라이트로 제공될 수 있습니다. 그렇지 않은 경우 값은 빈 배열이 됩니다.
