...
로더는 `refreshContextData` 속성에 접근하여 웹훅 본문을 가져올 수 있습니다. 자세한 내용은 [`refreshContextData`](/ko/reference/content-loader-reference/) 속성을 참조하세요.
...
**타입:** <code>(prerenderer: <a href="#astroprerenderer">AstroPrerenderer</a> | ((defaultPrerenderer: AstroPrerenderer) => AstroPrerenderer)) => void</code><br />
<Since v="6.0.0" />
</p>

빌드에 사용할 사용자 정의 사전 렌더러를 설정하는 콜백 함수입니다. 이를 통해 어댑터가 고유한 사전 렌더링 로직을 제공할 수 있습니다.

이 함수는 [`AstroPrerenderer` 객체](#astroprerenderer)를 직접 인자로 받거나, 기본 사전 렌더러를 인자로 받아 사용자 정의 사전 렌더러를 반환하는 팩토리 함수를 인자로 받을 수 있습니다. 이는 기본 동작을 래핑하거나 확장하고 싶을 때 유용합니다.
...
Astro는 Vite가 기본적으로 제공하는 환경인 `ssr` 및 `client`를 상속합니다.

또한 Astro가 생성하는 두 가지 다른 환경이 있습니다:
...
Astro의 [Vite 환경](https://ko.vite.dev/guide/api-environment)을 사용하면 다양한 환경에 맞춰 통합의 Vite 플러그인을 최적화할 수 있습니다. Vite 환경의 주요 용도 중 하나는 통합의 Vite 플러그인을 조건부로 실행하고 구성하는 기능입니다:
...
페이지를 렌더링하는 방법을 설명하는 선택적 메서드를 정의합니다. 이 메서드는 [`getStaticPaths()`](/ko/reference/routing-reference/#getstaticpaths)에서 반환된 각 경로에 대해 Astro에 의해 호출됩니다.
...
예를 들어, `/blog/[...id].astro`와 같은 라우트에서 `generate()` 함수는 다음을 반환할 수 있습니다:
...
**타입:** `(Component: any, props: Record<string, any>, children: Record<string, string>, metadata?: AstroComponentMetadata) => Promise<boolean>`
</p>

렌더러가 해당 컴포넌트를 처리해야 하는지 여부를 결정합니다. 등록된 각 렌더러 중 하나가 `true`를 반환할 때까지 순차적으로 호출됩니다.

이 함수는 컴포넌트 생성자 또는 함수, props, 슬롯 콘텐츠, 그리고 컴포넌트의 하이드레이션 지시어 및 소스에 대한 정보를 포함하는 선택적 [`metadata`](#astrocomponentmetadata) 객체를 인자로 받습니다.

#### `SSRLoadedRendererValue.renderToStaticMarkup()`

<p>

**타입:** `(Component: any, props: Record<string, any>, children: Record<string, string>, metadata?: AstroComponentMetadata) => Promise<{ html: string; attrs?: Record<string, string>; }>`
</p>

프레임워크 컴포넌트를 서버에서 정적 HTML 마크업으로 렌더링합니다.

이 함수는 컴포넌트 생성자 또는 함수, props, 슬롯 콘텐츠, 그리고 선택 사항인 [`metadata`](#astrocomponentmetadata) 객체를 인자로 받습니다. 렌더러는 `metadata`를 사용하여 컴포넌트가 클라이언트에서 하이드레이션될지 여부를 결정할 수 있으며, 이는 클라이언트 측 하이드레이션 상태를 조건부로 포함하는 데 유용할 수 있습니다.
...
렌더러의 [`check()`](#ssrloadedrenderervaluecheck) 및 [`renderToStaticMarkup()`](#ssrloadedrenderervaluerendertostaticmarkup) 함수의 네 번째 인자로 전달되는 객체입니다. 하이드레이션 지시어를 포함하여 렌더링되는 컴포넌트에 대한 정보를 포함합니다.
...
에러 메시지 및 디버깅에 사용되는 컴포넌트의 표시 이름입니다.

#### `AstroComponentMetadata.hydrate`

<p>

**타입:** `'load' | 'idle' | 'visible' | 'media' | 'only' | undefined`
</p>

컴포넌트에 사용된 [클라이언트 지시어](/ko/reference/directives-reference/#클라이언트-지시어)입니다 (있는 경우). `undefined`인 경우 컴포넌트는 클라이언트에서 하이드레이션되지 않습니다.

렌더러는 이 값을 사용하여 조건부로 클라이언트 측 하이드레이션 상태를 포함할 수 있습니다. 예를 들어, 렌더러는 하이드레이션되지 않을 컴포넌트에 대해 전송 상태 직렬화를 건너뛸 수 있습니다.

```ts
async function renderToStaticMarkup(Component, props, children, metadata) {
  const willHydrate = !!metadata?.hydrate;
  // 컴포넌트가 하이드레이션되지 않는 경우 하이드레이션 상태 직렬화 건너뛰기
...
하이드레이션 지시어에 대한 추가 인자입니다. 예를 들어, `client:media="(max-width: 768px)"`의 값은 `"(max-width: 768px)"`가 됩니다. `client:only="react"`의 경우 값은 `"react"`가 됩니다.
...
**타입:** `{ value: string; namespace?: boolean } | undefined`
</p>

컴포넌트 내보내기에 대한 정보입니다. `value` 속성은 내보내기의 이름입니다 (예: 기본 내보내기의 경우 `"default"`).
...
항상 `true`입니다. Astro가 이 컴포넌트에 대해 정적 슬롯 최적화를 지원함을 나타냅니다. [`supportsAstroStaticSlot`](#ssrloadedrenderervaluesupportsastrostaticslot)을 `true`로 설정한 렌더러는 이를 [`hydrate`](#astrocomponentmetadatahydrate)와 결합하여 슬롯을 렌더링하는 방법을 결정할 수 있습니다.
...
[콘텐츠 보안 정책 구성](/ko/reference/configuration-reference/)을 설명합니다.
...
[설정된 해시 함수](/ko/reference/configuration-reference/)를 지정합니다.
...
프로젝트 스크립트용으로 생성된 해시 목록과 외부 스크립트용으로 [사용자가 제공한 해시](/ko/reference/configuration-reference/) 목록을 지정합니다.
...
[설정된 스크립트 리소스](/ko/reference/configuration-reference/)와 [삽입된 스크립트 리소스](/ko/reference/api-reference/)를 결합한 유효한 소스 목록을 지정합니다.
...
[설정에서 동적 스크립트 삽입 지원이 활성화되었는지 여부](/ko/reference/configuration-reference/)를 결정합니다.
...
프로젝트 스타일용으로 생성된 해시 목록과 외부 스타일용으로 [사용자가 제공한 해시](/ko/reference/configuration-reference/) 목록을 지정합니다.
...
[설정된 스타일 리소스](/ko/reference/configuration-reference/)와 [삽입된 스타일 리소스](/ko/reference/api-reference/#cspinsertstyleresource)를 결합한 유효한 소스 목록을 지정합니다.
...
특정 콘텐츠 유형에 대해 [설정된 유효한 소스 목록](/ko/reference/configuration-reference/)을 지정합니다.

#### `SSRManifest.devToolbar`

<p>

**타입:** `{ enabled: boolean; latestAstroVersion: string | undefined; debugInfoOutput: string | undefined; }`<br />
<Since v="6.0.0" />
</p>
...
Astro의 최신 사용 가능한 버전을 지정합니다. 이는 개발 툴바에서 사용자에게 업데이트 사용 가능 여부를 알리는 데 사용됩니다. 다음 조건 중 하나가 적용될 때 이 값은 `undefined`가 됩니다.
