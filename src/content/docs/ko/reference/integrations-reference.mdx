...
#### `addRenderer` 옵션 \{#addrenderer-option\}
...
#### `addWatchFile` 옵션 \{#addwatchfile-option\}
...
#### `addClientDirective` 옵션 \{#addclientdirective-option\}
...
#### `addDevToolbarApp` 옵션 \{#adddevtoolbarapp-option\}
...
#### `addMiddleware` 옵션 \{#addmiddleware-option\}
...
#### `injectRoute` 옵션 \{#injectroute-option\}

**타입:** `({ pattern: string, entrypoint: string }) => void;`

Astro 프로젝트에 경로를 삽입하는 콜백 함수입니다. 주입된 경로는 [`.astro` 페이지](/ko/basics/astro-pages/) 또는 [`.js` 및 `.ts` 경로 처리기](/ko/guides/endpoints/#static-file-endpoints)일 수 있습니다.
...
#### `injectScript` 옵션 \{#injectscript-option\}
...
**사용 목적:** 확장을 위해 생성된 경로 및 자산에 액세스하기 위해 (예: 생성된 `/assets` 디렉터리에 콘텐츠 복사). 생성된 자산을 변환하려는 경우 대신 [Vite 플러그인 API](https://ko.vitejs.dev/guide/api-plugin.html)를 탐색하고 [`astro:config:setup`을 통해 구성](#updateconfig-option)하는 것이 좋습니다.
...
#### `routes` 옵션 \{#routes-option\}

**타입:** [`RouteData[]`](#routedata-type-reference)
...
##### `RouteData` 타입 참조 \{#routedata-type-reference\}
...
**사용 목적:** 빌드 시 또는 요청 시 경로에 대한 옵션을 설정하기 위해. 예: [주문형 서버 렌더링](/ko/guides/server-side-rendering/#opting-in-to-pre-rendering-in-server-mode) 활성화

```js
'astro:route:setup'?: (options: { route: RouteOptions }) => void | Promise<void>;
```

#### `route` 옵션 \{#route-option\}
...
`prerender` 속성은 경로에 대한 [주문형 서버 렌더링](/ko/guides/server-side-rendering/#opting-in-to-pre-rendering-in-server-mode)을 구성하는 데 사용됩니다. 경로 파일에 명시적인 `export const prerender` 값이 포함되어 있으면 `undefined` 대신 해당 값이 기본값으로 사용됩니다.
...
## `astro add`로 설치 허용 \{#allow-installation-with-astro-add\}
...
## 통합을 프리셋으로 결합 \{#combine-integrations-into-presets\}
