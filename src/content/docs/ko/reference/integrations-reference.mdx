...
**사용 목적:** 빌드 시 또는 요청 시 경로에 대한 옵션을 설정하기 위해. 예: [주문형 서버 렌더링](/ko/guides/on-demand-rendering/) 활성화
...
`prerender` 속성은 경로에 대한 [주문형 서버 렌더링](/ko/guides/on-demand-rendering/)을 구성하는 데 사용됩니다. 경로 파일에 명시적인 `export const prerender` 값이 포함되어 있으면 `undefined` 대신 해당 값이 기본값으로 사용됩니다.
