...
이 참조 페이지는 자체 통합을 작성하는 모든 사람을 위한 것입니다. 프로젝트에서 통합을 사용하는 방법을 알아보려면 [통합 사용](/ko/guides/integrations-guide/) 가이드를 확인하세요.
...
Astro는 통합이 Astro의 특정 수명 주기 동안 실행할 수 있도록 구현할 수 있는 훅을 제공합니다. Astro 훅은 글로벌 `Astro` 네임스페이스의 일부인 `IntegrationHooks` 인터페이스에 정의되어 있습니다.
...
**사용 목적:** 프로젝트 구성을 확장하기 위해 사용. 여기에는 [Astro 구성](/ko/reference/configuration-reference/) 업데이트, [Vite 플러그인](https://ko.vite.dev/guide/api-plugin.html) 적용, 컴포넌트 렌더러 추가, 페이지에 스크립트 삽입이 포함됩니다.
...
- `clientEntrypoint` - 컴포넌트가 사용될 때마다 클라이언트에서 실행되는 파일의 경로입니다. 이는 주로 JS를 사용하여 컴포넌트를 렌더링하거나 하이드레이션하는 데 사용됩니다.
- `serverEntrypoint` - 컴포넌트가 사용될 때마다, 서버 측 요청 또는 정적 빌드 중에 실행되는 파일의 경로입니다. 이는 해당되는 경우 하이드레이션을 위한 훅을 사용하여 컴포넌트를 정적 마크업으로 렌더링해야 합니다. [React의 `renderToString` 콜백](https://ko.react.dev/reference/react-dom/server/renderToString)이 전형적인 예시입니다.
...
지시어의 엔트리포인트는 esbuild를 통해서만 번들로 제공되며 컴포넌트 하이드레이션 속도를 늦추지 않도록 작게 유지해야 합니다.
...
- `pattern` - 경로는 브라우저에 출력되어야 합니다 (예: `/foo/bar`). `pattern`은 동적 라우트를 표시하기 위해 Astro의 파일 경로 구문을 사용할 수 있습니다 (예: `/foo/[bar]` 또는 `/foo/[...bar]`). `pattern`에는 파일 확장자가 필요하지 **않습니다**.
...
- `"before-hydration"`: 하이드레이션시키는 스크립트가 실행되기 전에 클라이언트 측에서 가져옵니다. Vite에 의해 최적화되고 해결됩니다.
...
**사용 목적:** 빌드 시 또는 요청 시 경로에 대한 옵션을 설정하기 위해. 예: [온디맨드 서버 렌더링](/ko/guides/server-side-rendering/#server-모드에서-사전-렌더링-선택) 활성화
...
`prerender` 속성은 경로에 대한 [온디맨드 서버 렌더링](/ko/guides/server-side-rendering/#server-모드에서-사전-렌더링-선택)을 구성하는 데 사용됩니다. 경로 파일에 명시적인 `export const prerender` 값이 포함되어 있으면 `undefined` 대신 해당 값이 기본값으로 사용됩니다.
...
모든 훅을 실행한 후 최종 값이 `undefined`인 경우, 경로는 [`output` 옵션](/ko/reference/configuration-reference/#output)에 따라 사전 렌더링의 기본값으로 돌아갑니다: `hybrid` 모드에서는 사전 렌더링되며, `server` 모드에서는 온디맨드로 렌더링됩니다.

### 사용자 정의 훅

[global augmentation](https://www.typescriptlang.org/docs/handbook/declaration-merging.html#global-augmentation)을 통해 `IntegrationHooks` 인터페이스를 확장하여 사용자 정의 훅을 통합에 추가할 수 있습니다.
...
향후 내장 훅에 `astro:` 접두사를 사용할 수 있습니다. 사용자 정의 훅의 이름을 지정할 때는 다른 접두사를 선택하세요.
