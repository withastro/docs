...
**타입:** `{ host?: string; port: number; urls?: { local: string[]; network: string[] }; closed(): Promise<void>; stop(): Promise<void>; }`
</p>
...
**타입:** `{ local: string[]; network: string[] } | undefined`<br />
<Since v="7.2.0" />
</p>

프리뷰 서버가 수신 대기 중인 URL을 정의합니다. URL은 `local`(루프백 주소)과 `network`(LAN 및 공용 주소)로 그룹화됩니다. Astro는 [프리뷰 서버가 백그라운드에서 실행될 때](/ko/reference/cli-reference/#--background) 서버 주소를 잠금 파일에 기록하는 데 이 URL을 사용합니다.
