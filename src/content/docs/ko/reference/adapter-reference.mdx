...
interface AstroAdapter {
  ...
  supportedAstroFeatures: AstroAdapterFeatureMap;
 	client?: {
		/**
		 * Astro의 내부 fetch 호출에 (액션, 뷰 전환, 서버 아일랜드, 프리페치) 주입할 헤더입니다.
		 * 헤더 객체 또는 헤더를 반환하는 함수가 될 수 있습니다.
		 */
		internalFetchHeaders?: Record<string, string> | (() => Record<string, string>);
		/**
		 * 모든 자산 URL (이미지, 스타일시트, 스크립트 등)에 추가할 쿼리 매개변수입니다.
		 * 배포 버전 또는 기타 메타데이터를 추적해야 하는 어댑터에 유용합니다.
		 */
