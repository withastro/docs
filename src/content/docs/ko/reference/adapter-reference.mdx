...
헤더의 값은 애플리케이션에서 활성화 및 사용되는 기능에 따라 변경될 수 있습니다. 예를 들어, [CSP가 활성화된 경우](/ko/reference/configuration-reference/), `<meta http-equiv="content-security-policy">` 요소가 정적 페이지에 추가되지 않습니다. 대신, 해당 `content`는 `experimentalRouteToHeaders` 맵에서 사용할 수 있습니다.
