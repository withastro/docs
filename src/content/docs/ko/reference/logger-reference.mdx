...
[내장 로거](#내장-로거) 중 하나를 사용하지 않으려는 경우, 직접 만들 수 있습니다.

사용자 정의 로거는 두 가지 요소로 구성됩니다.

- [로거 설정](#로거-설정): Astro에 사용할 구현체와 전달할 설정을 지정합니다.
- [로거 구현체](#로거-구현체): 실제 로깅 처리를 담당합니다.

사용자 정의 로거를 정의하면 Astro에서 발생하는 로그를 포함하여 모든 로그를 직접 제어하게 됩니다.

### 로거 설정

[`logger` 설정](/ko/reference/configuration-reference/#로거-옵션)은 필수 항목인 [`entrypoint`](/ko/reference/configuration-reference/#loggerentrypoint)와 선택 항목인 [`config`](/ko/reference/configuration-reference/#loggerconfig)를 포함하는 객체입니다.

다음 예시는 `@org/custom-logger` 패키지에서 내보내는 사용자 정의 로거를 설정하고 `level` 옵션을 전달하는 방법을 보여줍니다.
...
로거 구현체는 로깅 로직을 담당합니다. 로거 모듈에서 [로거 설정](#로거-설정)을 매개변수로 받는 기본 함수를 내보내어 구현하며, 이 함수는 필수 `write()` 함수를 포함하는 [`AstroLoggerDestination` 객체](#astrologgerdestination)를 반환해야 합니다.

다음 예시는 설정의 로그 `level`을 반영하는 최소한의 로거 구현을 보여줍니다.

```ts title="@org/custom-logger/index.js"
import { matchesLevel } from "astro/logger";

/**
 * 사용자 정의 로거 구현의 간단한 예입니다.
 *
 * @param {Object} [options] - 로거 옵션입니다.
 * @param {import("astro").AstroLoggerLevel} [options.level] - 출력할 로그의 최소 레벨입니다. 기본값은 `"info"`입니다.
 * @returns {import("astro").AstroLoggerDestination} 사용자 정의 로거의 출력 대상입니다.
 */
function orgLogger({ level } = { level: "info" }) {
  return {
    write(message) {
      // 메시지를 출력해야 하는지 판단하려면 이 유틸리티를 사용합니다.
      if (matchesLevel(message.level, level)) {
        // 로그 레벨을 고려하여 메시지를 원하는 위치에 기록합니다.
      }
    },
  };
}
