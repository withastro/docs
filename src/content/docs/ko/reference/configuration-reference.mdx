...
빌드 어댑터를 사용하여 선호하는 서버, 서버리스, 에지 호스트에 배포하세요. Astro SSR을 사용하려면 [Netlify](/ko/guides/deploy/netlify/#adapter-for-ssr), [Vercel](/ko/guides/deploy/vercel/#adapter-for-ssr) 등을 위한 자사 어댑터 중 하나를 가져오세요.
...
프로젝트 루트 디렉터리가 아닌 디렉터리에서 `astro` CLI 명령을 실행하는 경우에만 이 옵션을 제공해야 합니다. 일반적으로 이 옵션은 [Astro 구성 파일](/ko/guides/configuring-astro/#supported-config-file-types) 대신 CLI를 통해 제공됩니다. Astro가 구성 파일을 찾으려면 먼저 프로젝트 루트를 알아야 하기 때문입니다.
...
#### security.checkOrigin \{#securitycheckorigin\}
...
#### Astro.url에 미치는 영향 \{#effect-on-astrourl\}
...
### build.assetsPrefix \{#buildassetsprefix\}
...
### build.serverEntry \{#buildserverentry\}
...
### build.redirects \{#buildredirects\}
...
### build.inlineStylesheets \{#buildinlinestylesheets\}
...
### prefetch.prefetchAll \{#prefetchprefetchall\}
...
### prefetch.defaultStrategy \{#prefetchdefaultstrategy\}
...
#### image.service.config.limitInputPixels \{#imageserviceconfiglimitinputpixels\}
...
### image.remotePatterns \{#imageremotepatterns\}
...
## Markdown 옵션 \{#markdown-options\}

### markdown.shikiConfig \{#markdownshikiconfig\}
...
Shiki 구성 옵션. 사용법은 [Markdown 구성 문서](/ko/guides/markdown-content/#shiki-configuration)를 참조하세요.

### markdown.syntaxHighlight \{#markdownsyntaxhighlight\}
...
### markdown.remarkPlugins \{#markdownremarkplugins\}
...
### markdown.rehypePlugins \{#markdownrehypeplugins\}
...
### markdown.smartypants \{#markdownsmartypants\}
...
### markdown.remarkRehype \{#markdownremarkrehype\}
...
#### i18n.routing.prefixDefaultLocale \{#i18nroutingprefixdefaultlocale\}
...
#### i18n.routing.redirectToDefaultLocale \{#i18nroutingredirecttodefaultlocale\}
...
#### i18n.routing.fallbackType \{#i18nroutingfallbacktype\}
...
누락된 페이지 경로에 대해 404 페이지를 표시하지 않도록 [`i18n.fallback`](#i18nfallback)을 구성한 경우 이 옵션은 대체 페이지로 [리디렉션](/ko/guides/routing/#redirects)할 것인지 아니면 대체 페이지의 콘텐츠를 그대로 [리라이트](/ko/guides/routing/#rewrites)할 것인지 제어합니다.
...
#### i18n.routing.manual \{#i18nroutingmanual\}
...
## Experimental 플래그 \{#experimental-flags\}

Astro는 사용자가 새로운 기능에 조기에 액세스할 수 있도록 experimental 플래그를 제공합니다.
이러한 플래그는 안정성이 보장되지 않습니다.

### experimental.directRenderScript \{#experimentaldirectrenderscript\}
...
### experimental.contentCollectionCache \{#experimentalcontentcollectioncache\}
...
### experimental.clientPrerender \{#experimentalclientprerender\}
...
### experimental.globalRoutePriority \{#experimentalglobalroutepriority\}
...
모든 경로에 대해 동일한 [경로 우선 순위 규칙](/ko/guides/routing/#route-priority-order)을 따라 파일 기반 프로젝트 경로와 함께 리디렉션 및 삽입된 경로의 우선 순위를 동일하게 지정합니다.
...
### experimental.env \{#experimentalenv\}
...
전체 개요를 확인하고 이 실험적 API에 대한 피드백을 제공하려면 [Astro Env RFC](https://github.com/withastro/roadmap/blob/main/proposals/0049-astro-env.md)를 참조하세요.

#### experimental.env.schema \{#experimentalenvschema\}
...
#### experimental.env.validateSecrets \{#experimentalenvvalidatesecrets\}
...
### experimental.serverIslands \{#experimentalserverislands\}
...
활성화하려면 어댑터를 사용하여 [주문형 서버 렌더링 `output` 모드](/ko/basics/rendering-modes/#on-demand-rendered)를 구성하고 `experimental` 객체에 `serverIslands` 플래그를 추가합니다
...
#### 서버 아일랜드 대체 콘텐츠 \{#server-island-fallback-content\}
...
### experimental.contentIntellisense \{#experimentalcontentintellisense\}
...
### experimental.contentLayer \{#experimentalcontentlayer\}
...
기존 콘텐츠 컬렉션은 몇 가지 작은 변경을 통해 [Content Layer API로 마이그레이션](#migrating-an-existing-content-collection-to-use-the-content-layer-api)할 수 있습니다. 그러나 Content Layer API로 구동되는 새 컬렉션을 추가하기 위해 모든 컬렉션을 한 번에 업데이트할 필요는 없습니다. `src/content/config.ts`에 정의된 기존 API와 새 API를 동시에 사용하는 컬렉션을 가질 수 있습니다.
...
#### `loader`를 사용하여 데이터 가져오기 \{#fetching-data-with-a-loader\}

Content Layer API를 사용하면 (프로젝트에 로컬로 저장하든 원격으로 저장하든) `src/content/` 폴더 외부에서 콘텐츠를 가져올 수 있으며, `loader` 속성을 사용하여 데이터를 검색할 수 있습니다.

`loader`는 컬렉션의 스키마에 정의되어 있으며 항목의 배열을 반환합니다. Astro는 로컬 콘텐츠를 가져오기 위한 두 가지 내장 로더 함수 (`glob()` 및 `file()`)와 [자체 로더를 구성하고 원격 데이터를 가져오기 위한](#creating-a-loader) API 액세스를 제공합니다.
...
:::note
로더는 [`_`로 시작하는 파일을 자동으로 제외](/ko/guides/routing/#excluding-pages)하지 않습니다. 이러한 파일을 제외하려면 `pattern: '**\/[^_]*.md'`와 같은 정규 표현식을 사용하세요.
:::

#### Content Layer API로 쿼리 및 렌더링하기  \{#querying-and-rendering-with-the-content-layer-api\}

컬렉션은 [콘텐츠 컬렉션과 동일한 방식으로 쿼리](/ko/guides/content-collections/#querying-collections)할 수 있습니다:
...
#### Content Layer API를 사용하기 위해 기존 콘텐츠 컬렉션 마이그레이션하기 \{#migrating-an-existing-content-collection-to-use-the-content-layer-api\}
