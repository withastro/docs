...
다음 참조는 Astro에서 지원되는 모든 구성 옵션을 다룹니다. Astro 구성에 대해 자세히 알아보려면 [Astro 구성](/ko/guides/configuring-astro/) 가이드를 읽어보세요.
...
정적 라우트와 동적 라우트를 모두 리디렉션할 수 있지만 동일한 종류의 경로로만 리디렉션할 수 있습니다.
...
SSR에 대한 자세한 내용은 [서버 측 렌더링 가이드를 참조](/ko/guides/server-side-rendering/)하고, 전체 호스트 목록은 [배포 가이드](/ko/guides/deploy/)를 참조하세요.
...
이러한 기능은 `server` 모드를 사용하여 온디맨드 렌더링된 페이지 (SSR) 또는 `hybrid` 모드에서 사전 렌더링을 선택 해제한 페이지에만 존재합니다.
...
"origin" 확인은 온디맨드 렌더링된 페이지에 대해서만 실행되며, `content-type` 헤더가 `'application/x-www-form-urlencoded'`, `'multipart/form-data'`, `'text/plain'` 중 하나인 `POST`, `PATCH`, `DELETE`, `PUT` 요청에 대해서만 실행됩니다.
...
모든 경로에 대해 동일한 [경로 우선 순위 규칙](/ko/guides/routing/#라우트-우선순위)을 따라 파일 기반 프로젝트 경로와 함께 리디렉션 및 삽입된 경로의 우선 순위를 동일하게 지정합니다.

이를 통해 특정 유형의 경로에 자동으로 우선순위를 지정하지 않음으로써 프로젝트의 경로를 보다 효과적으로 제어할 수 있으며 모든 경로에 대한 라우트 우선순위 순서를 표준화합니다.
...
라우트 우선순위가 동일한 두 경로가 동일한 URL을 빌드하려고 시도하는 경로 충돌이 발생하는 경우 Astro는 충돌하는 경로를 식별하는 경고를 기록합니다.
...
활성화하려면 어댑터를 사용하여 [온디맨드 서버 렌더링 `output` 모드](/ko/basics/rendering-modes/#온디맨드-렌더링)를 구성하고 `experimental` 객체에 `serverIslands` 플래그를 추가합니다
...
Astro VS Code 확장 프로그램에서 이 기능을 사용하려면 VS Code 설정에서 `astro.content-intellisense` 옵션도 활성화해야 합니다. Astro 언어 서버를 직접 사용하는 편집기에서 이 기능을 활성화하려면 `contentIntellisense: true` 초기화 매개 변수를 전달하세요. 이 초기 기능에 대한 자세한 내용은 [콘텐츠 인텔리센스 구현 PR](https://github.com/withastro/language-tools/pull/915)을 참조하세요.
