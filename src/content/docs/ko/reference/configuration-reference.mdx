...
- **비밀 서버 변수**: 이 변수는 최종 번들의 일부가 아니며 `astro:env/server` 모듈을 통해 서버에서 액세스할 수 있습니다. `getSecret()` 도우미 함수를 사용하면 스키마에 지정되지 않은 비밀을 검색할 수 있습니다.
...
활성화하려면 어댑터를 사용하여 [주문형 서버 렌더링 `output` 모드](/ko/basics/rendering-modes/#주문형-렌더링)를 구성하고 `experimental` 객체에 `serverIslands` 플래그를 추가합니다
