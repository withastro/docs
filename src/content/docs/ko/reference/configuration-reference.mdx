...
기본적으로 Astro는 `.astro` 컴포넌트에서 줄바꿈을 포함한 HTML의 공백을 무손실 방식으로 제거합니다. HTML의 시각적 렌더링을 유지하기 위해 필요한 경우 일부 공백은 보존될 수 있습니다.

6.2.0 버전부터 이 옵션을 `"jsx"`로 설정할 수도 있습니다. 이 경우 Astro는 React와 같은 프레임워크에서 사용하는 JSX 공백 제거 규칙을 적용합니다. 앞뒤 공백은 `{" "}`와 같은 구문을 통해 소스 코드에 명시적으로 포함된 경우에만 유지되며, 그렇지 않은 경우에는 완전히 제거됩니다.

이 옵션을 false로 설정하면 HTML 압축이 비활성화되고 모든 공백이 보존됩니다.
...
`.md` 파일을 렌더링할 때 사용할 Markdown 프로세서를 구성합니다. 기본값은 `@astrojs/markdown-remark`의 `unified()`(remark/rehype 파이프라인)입니다.
