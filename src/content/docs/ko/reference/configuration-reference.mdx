...
Astro v7.2.0부터 이 옵션을 `false`로 설정하여 세션 지원을 비활성화할 수 있습니다. 세션을 비활성화하면 세션 런타임이 SSR 번들에서 제외되고, 어댑터는 기본 세션 드라이버를 구성하지 않습니다. 이는 번들 파싱 시간을 줄이는 것이 중요한 서버리스 및 엣지 런타임에서 유용합니다.
...
:::caution[지원 중단]
대신 `@astrojs/markdown-remark`의 `unified({ remarkPlugins })`에 `remarkPlugins`를 전달한 뒤, 이를 `markdown.processor`로 설정하세요. 이 옵션은 향후 메이저 버전에서 제거될 예정입니다.
:::
...
:::caution[지원 중단]
대신 `@astrojs/markdown-remark`의 `unified({ rehypePlugins })`에 `rehypePlugins`를 전달한 뒤, 이를 `markdown.processor`로 설정하세요. 이 옵션은 향후 메이저 버전에서 제거될 예정입니다.
:::
...
:::caution[지원 중단]
대신 `@astrojs/markdown-remark`의 `unified({ remarkRehype })`에 `remarkRehype`를 전달한 뒤, 이를 `markdown.processor`로 설정하세요. 이 옵션은 향후 메이저 버전에서 제거될 예정입니다.
:::
...
`.md` 파일을 렌더링하는 데 사용되는 Markdown 프로세서를 구성합니다. 기본값은 Astro의 네이티브 Markdown 파이프라인인 `@astrojs/markdown-satteri`의 `satteri()`입니다.
...
remark/rehype 파이프라인을 사용하려면 `@astrojs/markdown-remark`를 설치한 다음 `unified()`를 전달하세요:
