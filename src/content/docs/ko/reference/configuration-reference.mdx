...
빌드 어댑터를 사용하여 선호하는 서버, 서버리스, 에지 호스트에 배포하세요. Astro SSR을 사용하려면 [Netlify](/ko/guides/deploy/netlify/#adapter-for-ssr), [Vercel](/ko/guides/deploy/vercel/#ssr용-어댑터) 등을 위한 자사 어댑터 중 하나를 가져오세요.
...
**타입:** `('file' | 'directory')`<br />
**기본값:** `'directory'`
</p>

각 페이지의 출력 파일 형식을 제어합니다. 이 값은 어댑터에 의해 설정될 수 있습니다.
  - `'file'`인 경우 Astro는 각 페이지에 대해 HTML 파일 (예: "/foo.html")을 생성합니다.
  - `'directory'`인 경우 Astro는 각 페이지에 대해 중첩된 `index.html` 파일 (예: "/foo/index.html")이 있는 디렉터리를 생성합니다.
...
모든 경로에 대해 동일한 [경로 우선 순위 규칙](/ko/core-concepts/routing/#경로-우선순위)을 따라 파일 기반 프로젝트 경로와 함께 리디렉션 및 삽입된 경로의 우선 순위를 동일하게 지정합니다.
