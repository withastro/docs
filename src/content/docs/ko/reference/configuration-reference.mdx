...
Astro 통합을 시작하는 데 도움이 필요하면 [통합 가이드](/ko/guides/integrations/)를 읽어보세요.
...
다음 예시는 CLI를 사용하여 루트 디렉터리를 설정합니다.
...
Astro가 HTML의 공백을 처리하는 방식을 제어합니다. 이는 개발 모드와 최종 빌드 출력 모두에 영향을 미칩니다.

Astro 7.0 버전부터는 React와 같은 프레임워크에서 사용하는 JSX 공백 규칙이 기본적으로 적용됩니다. 이 규칙은 요소 주변의 공백과 줄바꿈을 제거하고, 여러 줄의 텍스트를 한 줄로 합치며, 한 줄 내의 공백(예: 두 인라인 요소 사이의 공백)은 유지합니다. 제거될 공백을 유지하려면 `{" "}`와 같은 구문을 사용하여 소스에 명시적으로 추가해야 합니다.

이 옵션을 `true`로 설정하면 기본 JSX 공백 규칙 대신 `.astro` 컴포넌트에서 줄바꿈을 포함한 공백을 손실 없이 제거합니다. HTML의 시각적 렌더링을 유지하기 위해 필요한 경우 일부 공백은 보존될 수 있습니다.
...
Vite에 추가 설정 옵션을 전달합니다. Astro가 필요한 일부 고급 설정을 지원하지 않을 때 유용합니다.
...
이 기능을 활성화하면 Astro의 처리 및 번들링된 스크립트와 스타일에 대한 기본 보안이 강화되며, 이러한 요소와 추가 콘텐츠 유형을 추가로 구성할 수 있습니다.

이 기능에는 몇 가지 제한 사항이 있습니다:
- 외부 스크립트 및 외부 스타일은 기본적으로 지원되지 않지만, [자신만의 해시를 제공](#securitycspscriptdirectivehashes)할 수 있습니다.
- `<ClientRouter />`를 사용하는 [Astro의 뷰 전환](/ko/guides/view-transitions/)은 지원되지 않지만, Astro의 네이티브 뷰 전환 및 내비게이션 API 개선 사항을 사용하지 않는 경우 [브라우저 네이티브 뷰 전환 API로 마이그레이션](https://events-3bg.pages.dev/jotter/astro-view-transitions/)하는 것을 고려할 수 있습니다.
- Shiki는 현재 지원되지 않습니다. Shiki 함수는 인라인 스타일을 사용하므로 Astro CSP 구현과 함께 작동할 수 없습니다. 프로젝트에 CSP와 구문 강조가 모두 필요한 경우 [`<Prism />` 사용](/ko/guides/syntax-highlighting/#prism-)을 고려하세요.
- 지시어에 리소스로 `'unsafe-inline'`이 포함되면 Astro는 해당 지시어에 해시를 생성하지 않습니다. CSP 명세에 따르면 동일한 지시어에 해시 또는 nonce가 있으면 브라우저는 `'unsafe-inline'`을 무시하므로, Astro는 `'unsafe-inline'`의 동작을 유지하기 위해 해시 생성을 억제합니다. 이 경우 해당 지시어의 보안 수준이 `'unsafe-inline'` 수준으로 낮아진다는 점에 유의하세요.

:::note
Vite 개발 서버의 특성상 이 기능은 `dev` 모드에서 작업하는 동안 지원되지 않습니다. 대신, `build` 및 `preview`를 사용하여 Astro 프로젝트에서 이 기능을 테스트할 수 있습니다.
:::

활성화되면 Astro는 각 페이지의 `<head>` 요소 내부에 `<meta>` 요소를 추가합니다.
...
Astro가 스타일과 스크립트의 해시를 생성할 때 사용할 [해시 함수](https://developer.mozilla.org/ko/docs/Glossary/Hash_function)입니다.
...
```
빌드 후, `<meta>` 요소는 Astro의 기본 지시어와 함께 여러분의 지시어를 `content` 값에 추가할 것입니다.
...
Astro v7.1부터 각 항목은 문자열 또는 객체가 될 수 있습니다. 객체를 사용하면 `kind` 필드를 통해 해시를 더 구체적인 지시어에 적용할 수 있습니다. `kind` 필드에서 사용할 수 있는 값은 다음과 같습니다.
- `"element"`: 해시를 `style-src-elem` 지시어에 추가합니다.
- `"attribute"`: 해시를 `style-src-attr` 지시어에 추가합니다.
- `"default"`: 해시를 `style-src` 지시어에 추가합니다.

`"default"` 해시는 `style-src`에 추가되며, `kind: "element"`를 사용하면 대신 `style-src-elem`에 추가됩니다. `style-src-attr`에는 추가되지 않습니다. Astro가 자동으로 생성한 해시도 동일한 규칙을 따릅니다.
...
해시를 `"element"`에 적용하면 `style-src` 대신 `style-src-elem` 지시어에 추가됩니다. Astro가 자동으로 생성한 해시도 함께 이동합니다.
...
빌드가 완료되면 `<meta>` 요소에는 해당 해시가 `style-src` 대신 `style-src-elem` 지시어에 포함됩니다.
...
**타입:** `Array<CspResourceEntry>`<br />
**기본값:** `[]`<br />
<Since v="6.0.0" />
</p>

Astro의 기본 소스를 덮어쓰기 위한 `style-src` 지시어의 유효한 소스 목록입니다. 이 목록에는 기본적으로 `'self'`가 포함되지 않으므로, 유지하려면 이 목록에 포함해야 합니다. 이 리소스는 모든 페이지에 추가됩니다.

Astro v7.1부터 각 항목은 문자열 또는 객체가 될 수 있습니다. 객체를 사용하면 `kind` 필드를 통해 소스를 더 구체적인 지시어에 적용할 수 있습니다. `kind` 필드에서 사용할 수 있는 값은 다음과 같습니다.
- `"element"`: 소스를 `style-src-elem` 지시어에 추가합니다.
- `"attribute"`: 소스를 `style-src-attr` 지시어에 추가합니다.
- `"default"`: 소스를 `style-src` 지시어에 추가합니다.

`"attribute"` 소스는 `'none'`, `'unsafe-hashes'`, `'unsafe-inline'`, `'report-sample'` 중 하나여야 하며, `'unsafe-hashes'`는 `"element"`와 함께 사용할 수 없습니다. 일반적인 사용 사례로는 `{ resource: "'unsafe-inline'", kind: "attribute" }`를 사용하여 `define:vars`나 Shiki에서 생성되는 인라인 `style` 속성을 허용하는 방법이 있습니다.

해시와 달리 `"default"` 소스는 다른 지시어로 이동하지 않고 `style-src`에만 추가됩니다. `"default"` 소스와 구체적인 소스(예: `"element"` 또는 `"attribute"`)를 함께 사용하면 Astro는 경고를 출력합니다.
...
`"default"` 소스는 더 구체적인 지시어에 복사되지 않습니다. 다음 예제에서는 `"default"` 소스와 `"element"` 소스가 서로 다른 지시어에 추가됩니다.
...
빌드가 완료되면 `<meta>` 요소는 `https://styles.cdn.example.com`을 `style-src`에만 유지합니다. 즉, 이 값은 `style-src-elem`에 추가되지 않으며 `<style>` 및 `<link>` 요소에는 적용되지 않습니다. (반면 Astro가 생성한 해시는 `style-src-elem`으로 이동합니다.)
...
리소스가 여러 번 삽입되거나 여러 소스(예: `csp` 구성에 정의된 것과 [CSP 런타임 API](/ko/reference/api-reference/#csp)를 사용하여 추가된 것)에서 제공되는 경우, Astro는 모든 리소스를 병합하고 중복을 제거하여 `<meta>` 요소를 생성합니다.
...
Astro v7.1부터 각 항목은 문자열 또는 객체가 될 수 있습니다. 객체를 사용하면 `kind` 필드를 통해 해시를 더 구체적인 지시어에 적용할 수 있습니다. `kind` 필드에서 사용할 수 있는 값은 다음과 같습니다.
- `"element"`: 해시를 `script-src-elem` 지시어에 추가합니다.
- `"attribute"`: 해시를 `script-src-attr` 지시어에 추가합니다.
- `"default"`: 해시를 `script-src` 지시어에 추가합니다.

`"default"` 해시는 `script-src`에 추가되며, `kind: "element"`를 사용하면 대신 `script-src-elem`에 추가됩니다. `script-src-attr`에는 추가되지 않습니다. Astro가 자동으로 생성한 해시도 동일한 규칙을 따릅니다.
...
해시를 `"element"`에 적용하면 `script-src-elem` 지시어에 추가됩니다. Astro가 자동으로 생성한 해시도 함께 이동합니다.
...
빌드가 완료되면 `<meta>` 요소에는 해당 해시가 `script-src` 대신 `script-src-elem` 지시어에 포함됩니다.
...
`script-src` 지시어에 대한 유효한 소스 목록으로, Astro의 기본 소스를 재정의합니다. 기본적으로 `'self'`는 포함되지 않으므로, 이를 유지하려면 이 목록에 포함해야 합니다. 이러한 리소스는 모든 페이지에 추가됩니다.

Astro v7.1부터 각 항목은 문자열 또는 객체가 될 수 있습니다. 객체를 사용하면 `kind` 필드를 통해 소스를 더 구체적인 지시어에 적용할 수 있습니다. `kind` 필드에서 사용할 수 있는 값은 다음과 같습니다.
- `"element"`: 소스를 `script-src-elem` 지시어에 추가합니다.
- `"attribute"`: 소스를 `script-src-attr` 지시어에 추가합니다.
- `"default"`: 소스를 `script-src` 지시어에 추가합니다.

`"attribute"` 소스는 `'none'`, `'unsafe-hashes'`, `'unsafe-inline'`, `'report-sample'` 중 하나여야 하며, `'unsafe-hashes'`는 `"element"`와 함께 사용할 수 없습니다.

해시와 달리 `"default"` 소스는 이동하지 않으며 `script-src`에만 유지됩니다. 그러나 브라우저가 대체하지 않으므로 `"element"` 또는 `"attribute"` 소스도 함께 적용한 경우에는 이 규칙이 적용되지 않습니다. 이러한 상황이 발생하면 Astro는 경고를 출력합니다. 필요한 경우 더 구체적인 지시어에도 해당 소스를 추가하세요.
...
빌드가 완료되면 `<meta>` 요소는 `https://cdn.example.com`을 `script-src`에만 유지합니다. 즉, 이 값은 `script-src-elem`에 추가되지 않으며 `<script>` 요소에는 적용되지 않습니다. (반면 Astro가 자동으로 생성한 해시는 `script-src-elem`으로 이동합니다.)
...
이는 `script-src`에 적용됩니다. `kind: "element"`를 사용하여 `script` 리소스나 해시를 `script-src-elem`에도 적용한 경우, 동적으로 삽입된 `<script>` 요소가 계속 동작할 수 있도록 `strict-dynamic`이 `script-src-elem`에 상속됩니다.
...
- `preserve` - `Astro.url.pathname`은 각 페이지에 생성되는 파일 경로를 그대로 따릅니다. 인덱스 페이지의 경로는 후행 슬래시로 끝나고(예: `/foo/`), 인덱스 페이지가 아닌 경우에는 `.html` 확장자가 붙습니다(예: `/foo.html`).
...
기본값은 `'fetch'`이며, Astro는 `src/fetch.ts`(또는 `.js` / `.mjs` / `.mts`)를 찾습니다.

fetch 파일을 사용하면 Astro의 요청 파이프라인을 Web Fetch 표준이나 자체 Hono 미들웨어와 함께 구성할 수 있습니다.
...
개발 및 프로덕션 환경에서 Astro가 로그를 기록하는 방식을 구성합니다.

기본적으로 Astro는 콘솔에 사람이 읽기 쉬운 형식의 로그를 출력하는 내장 로거를 사용합니다. [사용자 정의 로거 핸들러](/ko/reference/logger-reference/#사용자-정의-로거)를 제공하거나 [내장 로거 핸들러](/ko/reference/logger-reference/#내장-로거) 중 하나를 사용하여 이 동작을 사용자 정의할 수 있습니다.
...
[로거 구현](/ko/reference/logger-reference/#로거-구현체)에 대한 엔트리포인트입니다.
npm 패키지, 프로젝트 루트에 상대적인 경로 또는 프로젝트 내부의 파일을 가리키는 `URL`일 수 있습니다.
...
로거 엔트리포인트는 JavaScript 파일이어야 합니다. TypeScript는 지원되지 않습니다. Astro가 로거 구현체를 로드하지 못하면 기본 로거가 대신 사용됩니다.
...
Astro v7.2.0부터 이 옵션을 `false`로 설정하여 세션 지원을 비활성화할 수 있습니다. 세션을 비활성화하면 세션 런타임이 SSR 번들에서 제외되고, 어댑터는 기본 세션 드라이버를 구성하지 않습니다. 이는 번들 파싱 시간이 성능에 중요한 서버리스 및 엣지 런타임에서 유용합니다.
...
Astro의 내장 Sharp 이미지 서비스를 사용할 때 `sharp().jpeg()`에 전달되는 기본 인코더 옵션입니다.
...
Astro의 내장 Sharp 이미지 서비스를 사용할 때 `sharp().webp()`에 전달되는 기본 인코더 옵션입니다.
...
Astro의 내장 Sharp 이미지 서비스를 사용할 때 `sharp().avif()`에 전달되는 기본 인코더 옵션입니다.
...
Astro의 내장 Sharp 이미지 서비스를 사용할 때 `sharp().png()`에 전달되는 기본 인코더 옵션입니다.
...
:::caution[지원 중단]
이 속성은 지원 중단되었으며 향후 메이저 버전에서 제거될 예정입니다. 구성된 [`markdown.processor`](#markdownprocessor)에 대신 플러그인을 전달하세요.

Markdown 가이드에서 [Markdown 프로세서 설정하기](/ko/guides/markdown-content/#markdown-프로세서-설정하기) 및 [플러그인 사용하기](/ko/guides/markdown-content/#markdown-프로세서-플러그인)에 대해 자세히 알아보세요.
:::
...
:::caution[지원 중단]
이 속성은 지원 중단되었으며 향후 메이저 버전에서 제거될 예정입니다. 구성된 [`markdown.processor`](#markdownprocessor)에 대신 `gfm`을 전달하세요.

Markdown 가이드에서 [Markdown 프로세서 설정하기](/ko/guides/markdown-content/#markdown-프로세서-설정하기) 및 [GitHub-Flavored Markdown 사용하기](/ko/guides/markdown-content/#github-flavored-markdown)에 대해 자세히 알아보세요.
:::
...
:::caution[지원 중단]
이 속성은 지원 중단되었으며 향후 메이저 버전에서 제거될 예정입니다. 구성된 [`markdown.processor`](#markdownprocessor)에 전달하세요. `unified()`에는 `smartypants`를, `satteri()`에는 `smartPunctuation`을 사용하세요.

Markdown 가이드에서 [Markdown 프로세서 설정하기](/ko/guides/markdown-content/#markdown-프로세서-설정하기) 및 [스마트 구두점 사용하기](/ko/guides/markdown-content/#스마트-구두점)에 대해 자세히 알아보세요.
:::
...
:::caution[지원 중단]
이 속성은 지원 중단되었으며 향후 메이저 버전에서 제거될 예정입니다.

각주를 구성하려면 `remarkRehype`를 `unified()` 프로세서에 전달하거나 `gfm.footnotes`를 `satteri()` 프로세서에 전달하세요. 다른 `remark-rehype` 옵션은 `unified()`를 사용할 때만 지원됩니다.

Markdown 가이드에서 [Markdown 프로세서 설정하기](/ko/guides/markdown-content/#markdown-프로세서-설정하기) 및 [내장 기능 사용하기](/ko/guides/markdown-content/#내장-기능)에 대해 자세히 알아보세요.
:::
...
`.md` 파일을 렌더링하는 데 사용하는 [Markdown 프로세서](/ko/guides/markdown-content/#markdown-프로세서)를 구성합니다.

Astro의 네이티브 Markdown 파이프라인인 Sätteri가 기본 프로세서입니다. 구성하려면 `@astrojs/markdown-satteri`를 설치하고 `satteri()`에 옵션을 전달하세요.
...
remark/rehype 파이프라인을 계속 사용하려면 `@astrojs/markdown-remark`를 설치한 다음 `unified()`를 전달하세요.
...
공식 Markdown 프로세서와 선택 방법은 [Markdown 가이드](/ko/guides/markdown-content/#markdown-프로세서-선택하기)에서 자세히 알아보세요.
...
[Astro에서 커스텀 폰트를 사용하는 방법](/ko/guides/fonts/)에 대한 자세한 내용은 가이드를 참조하세요.
...
폰트의 의도된 모양과 일치하는 [일반 패밀리 이름](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/font-family#generic-name)을 하나 이상 지정하세요. 그러면 Astro는 폰트 메트릭을 사용하여 [최적화된 대체 폰트](https://developer.chrome.com/blog/font-fallbacks) 생성을 시도합니다. 이 최적화를 비활성화하려면 `optimizedFallbacks`를 false로 설정하세요.
...
대체 폰트를 생성할 때 Astro의 기본 최적화를 활성화할지 여부입니다. [`fallbacks`](#fontfallbacks)가 생성되는 방식을 완전히 제어하기 위해 이 기본 최적화를 비활성화할 수 있습니다.
...
요청별 캐싱을 제어하려면 라우트에서는 `Astro.cache.set()`을 사용하고, 미들웨어나 API 라우트에서는 `context.cache.set()`을 사용하세요.
