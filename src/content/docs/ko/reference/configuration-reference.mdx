...
자세한 내용은 호스팅 플랫폼의 문서를 참조하세요. 현재 이 사용 사례에서는 Astro [리디렉션](/ko/reference/configuration-reference/#redirects)을 사용할 수 없습니다.
...
개발에서 후행 슬래시 동작의 불일치를 방지하려면 빌드 형식에 따라 [`trailingSlash` 옵션](/ko/reference/configuration-reference/#trailingslash)을 `'always'` 또는 `'never'`로 제한할 수 있습니다.
...
[`prefetch.defaultStrategy`](/ko/reference/configuration-reference/#prefetchdefaultstrategy) 및 [`prefetch.prefetchAll`](/ko/reference/configuration-reference/#prefetchprefetchall) 옵션을 사용하여 기본 프리페치 동작을 추가로 사용자 정의하세요.
...
이 옵션에는 개별 도메인 이름의 배열이 문자열로 필요합니다. 와일드카드는 허용되지 않습니다. 대신 [`image.remotePatterns`](/ko/reference/configuration-reference/#imageremotepatterns)를 사용하여 허용되는 소스 URL 패턴 목록을 정의하세요.
...
누락된 페이지 경로에 대해 404 페이지를 표시하지 않도록 [`i18n.fallback`](/ko/reference/configuration-reference/#i18nfallback)을 구성한 경우 이 옵션은 대체 페이지로 [리디렉션](/ko/guides/routing/#리디렉션)할 것인지 아니면 대체 페이지의 콘텐츠를 그대로 [리라이트](/ko/guides/routing/#url-재작성-rewrites)할 것인지 제어합니다.
