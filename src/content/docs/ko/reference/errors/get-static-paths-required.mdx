...
[정적 모드](/ko/guides/routing/#정적ssg-모드)에서는 모든 경로가 빌드 시 결정되어야 합니다. 따라서 동적 라우트는 생성할 다양한 경로를 반환하는 `getStaticPaths` 함수를 `export`해야 합니다.

**더 보기:**

-  [동적 라우트](/ko/guides/routing/#동적-라우트)
