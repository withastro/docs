...
## 툴바 앱 통합 설정 \{#toolbar-app-integration-setup\}
...
개발 툴바에 앱을 표시하는 데 사용되는 아이콘입니다. 이는 [아이콘 목록](#icons)의 아이콘이거나 아이콘의 SVG 마크업을 포함하는 문자열일 수 있습니다.
...
## 개발 툴바 앱의 구조 \{#structure-of-a-dev-toolbar-app\}
...
개발 툴바에서 이벤트를 주고 받을 수 있는 몇 가지 추가 [클라이언트 측 이벤트 도우미](#client-side-events)가 포함된 표준 [`EventTarget`](https://developer.mozilla.org/ko/docs/Web/API/EventTarget)입니다.
...
[서버와 통신](#client-server-communication)하는데 사용할 수 있는 객체입니다.
...
## 클라이언트 측 이벤트 \{#client-side-events\}
...
[클라이언트-서버 통신을 위한 Vite의 방법](https://ko.vitejs.dev/guide/api-plugin.html#client-server-communication)을 사용하면 개발 툴바 앱과 서버가 서로 통신할 수 있습니다. 사용자 정의 메시지를 쉽게 주고받을 수 있도록 개발 툴바 앱 (클라이언트)과 통합 (서버) 모두에서 사용할 수 있는 도우미 메서드가 제공됩니다.
...
[툴바 앱을 추가하는 통합](#toolbar-app-integration-setup)과 같은 통합에서는 앱에서 메시지를 주고받기 위한 `toolbar` 객체에 액세스하기 위해 [`astro:server:setup` 후크](/ko/reference/integrations-reference/#astroserversetup)을 사용하세요.
...
강조 표시의 오른쪽 상단에 표시할 [아이콘](#icons)입니다.
...
아이콘을 표시합니다. [아이콘 목록](#icons)의 아이콘은 `icon` 속성을 사용하여 지정하거나 아이콘의 SVG 마크업을 슬롯으로 전달할 수 있습니다.
