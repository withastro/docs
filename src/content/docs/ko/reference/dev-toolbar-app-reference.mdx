...
통합은 [`astro:config:setup` 훅](/ko/reference/integrations-reference/#astroconfigsetup)의 개발 툴바에 앱을 추가할 수 있습니다.
...
[`astro:config:setup` 훅](/ko/reference/integrations-reference/#astroconfigsetup)에 사용할 수 있는 개발 툴바 앱을 추가하는 함수입니다. 개발 툴바 앱을 정의하려면 다음과 같은 필수 속성이 있는 객체를 사용합니다: [`id`](#id), [`name`](#name), [`icon`](#icon), [`entrypoint`](#entrypoint).

### `id`

앱의 고유 식별자입니다. 이는 훅 및 이벤트에서 앱을 고유하게 식별하는 데 사용됩니다.
...
앱에서 [`init()` 훅의 `server` 객체](#server)를 사용하여 서버와 메시지를 주고받습니다.
...
[툴바 앱을 추가하는 통합](#툴바-앱-통합-설정)과 같은 통합에서는 앱에서 메시지를 주고받기 위한 `toolbar` 객체에 액세스하기 위해 [`astro:server:setup` 훅](/ko/reference/integrations-reference/#astroserversetup)을 사용하세요.
