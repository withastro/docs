...
export default defineToolbarApp({
  init(canvas) {
    canvas.appendChild(document.createTextNode("환영합니다!"));
  },
});
```
...
export default defineToolbarApp({
  init(canvas, app, server) {
    server.send("my-message", { message: "안녕하세요!" });
...
export default defineToolbarApp({
  init(canvas, app) {
    app.onToggled((options) => {
      console.log(`앱이 ${options.state ? "활성화" : "비활성화"}되었습니다!`);
    });
  },
});
```
...
export default defineToolbarApp({
  init(canvas, app) {
    app.onToolbarPlacementUpdated((options) => {
      console.log(`툴바가 ${options.placement} 위치에 배치되었습니다!`);
    });
  },
});
```
...
export default defineToolbarApp({
  init(canvas, app, server) {
    server.send("my-app:my-message", { message: "안녕하세요!" });
  },
});
```
