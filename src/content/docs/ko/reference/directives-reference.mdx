...
이러한 지시어는 [UI 프레임워크 컴포넌트](/ko/core-concepts/framework-components/)가 페이지에서 수화되는 방식을 제어합니다.

기본적으로 UI 프레임워크 컴포넌트는 클라이언트에서 수화되지 않습니다. `client:*` 지시어가 제공되지 않으면 HTML이 JavaScript 없이 페이지에 렌더링됩니다.

클라이언트 지시어는 `.astro` 컴포넌트로 직접 가져온 UI 프레임워크 컴포넌트에서만 사용할 수 있습니다. [동적 태그](/ko/core-concepts/astro-syntax/#동적-태그)와 [`components` prop을 통해 전달된 사용자 정의 컴포넌트](/ko/guides/markdown-content/#가져온-mdx가-있는-사용자-정의-컴포넌트)를 사용할 때는 수화 지시어가 지원되지 않습니다.
...
📚 전역 스타일의 작동 방식에 대한 자세한 내용은 [스타일링 및 CSS](/ko/guides/styling/#전역-스타일) 페이지를 참조하세요.
...
:::caution
`<script>` 또는 `<style>` 태그에 `src` 이외의 속성이 사용될 때마다 `is:inline` 지시어가 사용됩니다.
:::
...
📚 Astro 컴포넌트에서 [클라이언트 측 스크립트](/ko/guides/client-side-scripts/)가 어떻게 작동하는지 알아보세요.
...
:::caution
`<script>` 태그에 `define:vars`를 사용하는 것은 [`is:inline` 지시어](#isinline)를 의미합니다. 이는 스크립트가 번들링되지 않고 HTML에 직접 인라인된다는 의미입니다.
