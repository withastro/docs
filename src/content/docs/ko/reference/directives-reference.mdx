...
템플릿 지시어는 어떤 방식으로든 요소나 컴포넌트의 동작을 제어하는 ​​데 사용됩니다. 템플릿 지시어는 여러분의 삶을 더 쉽게 만들어주는 일부 컴파일러 기능을 활성화할 수 있습니다 (예: `class` 대신 `class:list` 사용). 또는 지시어는 Astro 컴파일러에게 해당 컴포넌트에 대해 특별한 작업을 수행하도록 지시할 수 있습니다 (예: `client:load`를 사용하여 하이드레이션).
...
이러한 지시어는 [UI 프레임워크 컴포넌트](/ko/guides/framework-components/)가 페이지에서 하이드레이션되는 방식을 제어합니다.

기본적으로 UI 프레임워크 컴포넌트는 클라이언트에서 하이드레이션되지 않습니다. `client:*` 지시어가 제공되지 않으면 HTML이 JavaScript 없이 페이지에 렌더링됩니다.

클라이언트 지시어는 `.astro` 컴포넌트로 직접 가져온 UI 프레임워크 컴포넌트에서만 사용할 수 있습니다. [동적 태그](/ko/basics/astro-syntax/#동적-태그)와 [`components` prop을 통해 전달된 사용자 정의 컴포넌트](/ko/guides/integrations-guide/mdx/#가져온-mdx를-사용하는-사용자-정의-컴포넌트)를 사용할 때는 하이드레이션 지시어가 지원되지 않습니다.
...
페이지 로드 시 컴포넌트 JavaScript를 즉시 로드하고 하이드레이션합니다.
...
페이지의 초기 로드가 완료되고 `requestIdleCallback` 이벤트가 실행되면 컴포넌트 JavaScript를 로드하고 하이드레이션합니다. [`requestIdleCallback`](https://developer.mozilla.org/ko/docs/Web/API/Window/requestIdleCallback)을 지원하지 않는 브라우저를 사용하는 경우, document의 [`load`](https://developer.mozilla.org/ko/docs/Web/API/Window/load_event) 이벤트가 사용됩니다.
...
컴포넌트가 사용자의 뷰포트에 들어가면 컴포넌트 JavaScript를 로드하고 하이드레이션합니다. 이는 가시성을 추적하기 위해 내부적으로 `IntersectionObserver`를 사용합니다.
...
선택적으로 `rootMargin` 값을 기본 `IntersectionObserver`에 전달할 수 있습니다. `rootMargin`이 지정되면 컴포넌트 자체가 아닌 컴포넌트 주변 지정된 마진 (픽셀 단위)이 뷰포트에 들어갈 때 컴포넌트 JavaScript가 하이드레이션됩니다.

```astro
<HeavyImageCarousel client:visible={{rootMargin: "200px"}} />
```

`rootMargin` 값을 지정하면 레이아웃 변경 (CLS)이 줄어들고, 느린 인터넷 연결에서 컴포넌트가 하이드레이션되는 데 더 많은 시간이 허용되며, 컴포넌트가 더 빨리 대화형으로 만들어져 페이지의 안정성과 응답성이 향상됩니다.
...
`client:media={string}`은 특정 CSS 미디어 쿼리가 충족되면 JavaScript 컴포넌트를 로드하고 하이드레이션합니다.
...
`client:only={string}` HTML 서버 렌더링을 **건너뛰고** 클라이언트에서만 렌더링합니다. 페이지 로드 시 즉시 컴포넌트를 로드, 렌더링 및 하이드레이션한다는 점에서 `client:load`와 유사하게 작동합니다.
...
Astro 2.6.0부터 통합에서는 컴포넌트를 하이드레이션해야 하는 방법과 시기를 변경하기 위해 사용자 정의 `client:*` 지시어를 추가할 수도 있습니다.
