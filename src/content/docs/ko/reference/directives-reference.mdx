...
클라이언트 지시어는 `.astro` 컴포넌트로 직접 가져온 UI 프레임워크 컴포넌트에서만 사용할 수 있습니다. [동적 태그](/ko/basics/astro-syntax/#dynamic-tags)와 [`components` prop을 통해 전달된 사용자 정의 컴포넌트](/ko/guides/markdown-content/#custom-components-with-imported-mdx)를 사용할 때는 수화 지시어가 지원되지 않습니다.
...
### 사용자 정의 클라이언트 지시어 \{#custom-client-directives\}

Astro 2.6.0부터 통합에서는 컴포넌트를 수화해야 하는 방법과 시기를 변경하기 위해 사용자 정의 `client:*` 지시어를 추가할 수도 있습니다.

사용자 정의 클라이언트 지시어 생성에 대해 자세히 알아보려면 [`addClientDirective` API](/ko/reference/integrations-reference/#addclientdirective-option) 페이지를 방문하세요.

## 스크립트 & 스타일 지시어 \{#script--style-directives\}
...
<ReadMore>전역 스타일의 작동 방식에 대한 자세한 내용은 [스타일링 및 CSS](/ko/guides/styling/#global-styles) 페이지를 참조하세요.</ReadMore>
...
스크립트의 경우, 대신 [변수를 스크립트에 수동으로 전달](/ko/guides/client-side-scripts/#pass-frontmatter-variables-to-scripts)해 보세요.
:::

## 고급 지시어 \{#advanced-directives\}
