...
`.glob()`은 정적으로 분석할 수 없기 때문에 이를 보간하는 변수나 문자열을 사용할 수 없습니다. (해결 방법은 [문제 해결 안내서](/ko/guides/troubleshooting/#supported-values)를 참조하세요.) 이는 `Astro.glob()`이 Vite의 [`import.meta.glob()`의 래퍼이기 때문입니다.](https://ko.vitejs.dev/guide/features.html#glob-import).
...
`Astro.props`는 [컴포넌트 속성](/ko/basics/astro-components/#component-props)으로 전달된 모든 값을 포함하는 객체입니다. `.md` 및 `.mdx` 파일의 레이아웃 컴포넌트는 프런트매터 값을 props로 받습니다.
...
<ReadMore>[Markdown 및 MDX 레이아웃](/ko/guides/markdown-content/#frontmatter-layout)이 props를 처리하는 방법에 대해 자세히 알아보세요.</ReadMore>

<ReadMore>[props에 대한 TypeScript 타입 정의](/ko/guides/typescript/#component-props)를 추가하는 방법을 알아보세요.</ReadMore>

### `Astro.params` \{#astroparams\}

`Astro.params`는 이 요청과 일치하는 동적 경로 세그먼트의 값을 포함하는 객체입니다.

정적 빌드에서 이 객체는 [동적 경로](/ko/guides/routing/#dynamic-routes) 사전 렌더링에 사용되는 `getStaticPaths()`에서 반환하는 `params`입니다.
...
:::note
이는 고급 사용 사례를 위한 것입니다! 대부분의 경우 [`<slot />` 요소](/ko/basics/astro-components/#slots)를 사용하여 슬롯 콘텐츠를 렌더링하는 것이 더 간단합니다.
:::
...
## 엔드포인트 Context \{#endpoint-context\}
...
함께 보기: [`props`를 사용한 데이터 전달](#data-passing-with-props)
...
[동적 라우팅](/ko/guides/endpoints/#params-and-dynamic-routing)을 위한 정적 파일 엔드포인트에서도 사용할 수 있습니다.
...
### `props`를 사용한 데이터 전달 \{#data-passing-with-props\}
...
#### 페이지네이션의 `page` prop \{#the-pagination-page-prop\}
...
## 이미지 (`astro:assets`) \{#images-astroassets\}
...
`getImage()` 함수는 HTML이 아닌 다른 곳 (예: [API 경로](/ko/guides/endpoints/#server-endpoints-api-routes))에서 사용할 이미지를 생성하기 위한 것입니다. 또한 사용자 정의 `<Image />` 컴포넌트를 만들 수도 있습니다.

`getImage()`는 [Image 컴포넌트와 동일한 속성](#properties)을 가진 옵션 객체를 사용합니다 (`alt` 제외).
...
## 콘텐츠 컬렉션 (`astro:content`) \{#content-collections-astrocontent\}
...
사용 예시는 [`콘텐츠 컬렉션` 안내서를 참조하세요](/ko/guides/content-collections/#defining-a-collection-schema).
...
사용 예는 [`콘텐츠 컬렉션` 안내서를 참조하세요](/ko/guides/content-collections/#defining-collection-references).
...
사용 예시는 [`콘텐츠 컬렉션` 안내서를 참조하세요](/ko/guides/content-collections/#querying-collections).
...
[컬렉션 항목 쿼리](/ko/guides/content-collections/#querying-collections)의 예시는 `콘텐츠 컬렉션` 안내서를 참조하세요.
...
`getEntries()`는 동일한 컬렉션에서 여러 컬렉션 항목을 검색하는 함수입니다. 이는 [참조된 항목의 배열을 반환](/ko/guides/content-collections/#defining-collection-references)하여 관련 `data`, `body`, `render()` 속성에 액세스하는 데 유용합니다.
...
**타입:** `(collection: string, id: string) => Promise<CollectionEntry<collection>>`<br />
<Since v="2.5.0" />
</p>
...
### 컬렉션 항목 타입 \{#collection-entry-type\}
...
Markdown 또는 MDX 문서용 URL 지원 슬러그입니다. 파일 확장자가 없는 `id`가 기본값이지만 파일의 프런트매터에서 [`slug` 속성](/ko/guides/content-collections/#defining-custom-slugs)을 설정하여 재정의할 수 있습니다.
...
- `headings` - Markdown 및 MDX 가져오기에 대해 생성된 제목 목록, [Astro의 `getHeadings()` 유틸리티 미러링](/ko/guides/markdown-content/#exported-properties).
- `remarkPluginFrontmatter ` - [remark 또는 rehype 플러그인이 적용된 후](/ko/guides/markdown-content/#modifying-frontmatter-programmatically) 수정된 frontmatter 객체. `any` 타입으로 설정합니다.
...
사용 예시는 [`콘텐츠 컬렉션` 안내서를 참조하세요](/ko/guides/content-collections/#rendering-content-to-html).

### 기타 콘텐츠 컬렉션 타입 \{#other-content-collection-types\}
...
- `image` - [콘텐츠 컬렉션에서 로컬 이미지를 사용](/ko/guides/images/#images-in-content-collections)할 수 있게 해주는 `image()` 스키마 도우미
...
## 미들웨어 (`astro:middleware`) \{#middleware-astromiddleware\}
...
<ReadMore>컨텍스트 객체에 대해 더 자세히 알아보기 위해 [엔드포인트 컨텍스트](#endpoint-context)를 방문하세요.</ReadMore>
...
Astro v4.13.0부터 `next()`는 새 렌더링 단계를 다시 트리거하지 않고 현재 요청을 [리라이트](/ko/guides/routing/#rewrites)하기 위해 문자열 형식의 선택적 URL 경로 매개 변수인 `URL` 또는 `Request`를 허용합니다.
...
Astro 미들웨어 `onRequest()` 함수에 전달될 [`APIContext`](#endpoint-context)를 생성하는 저수준 API입니다.
...
## 국제화 (`astro:i18n`) \{#internationalization-astroi18n\}
...
모든 로케일에 대한 절대 경로 목록을 반환하려면 [`getAbsoluteLocaleUrl`](/ko/guides/internationalization/#custom-locale-paths)처럼 이 함수를 사용하세요.
...
[사용자 정의 로케일 경로](/ko/guides/internationalization/#custom-locale-paths)가 구성된 경우 하나 이상의 `codes`와 연결된 `path`를 반환하는 함수입니다.
...
## View Transitions 클라이언트 API (`astro:transitions/client`) \{#view-transitions-client-api-astrotransitionsclient\}
...
#### `sourceElement` 옵션 \{#sourceelement-option\}
...
대체 동작을 선택하고 구성하는 방법은 [대체 제어](/ko/guides/view-transitions/#fallback-control) 가이드를 참조하세요.


### `astro:before-preparation` 이벤트 \{#astrobefore-preparation-event\}
...
### `astro:after-preparation` 이벤트 \{#astroafter-preparation-event\}
...
### `astro:before-swap` 이벤트 \{#astrobefore-swap-event\}
...
### View Transitions 이벤트 속성  \{#view-transitions-events-attributes\}
...
이는 [`navigate()` 함수](#navigate)의 [`info` 옵션](#info-option)에 전달된 리터럴 값입니다.
...
[`navigate()` 함수](#navigate)를 사용할 때, 이는 호출에서 지정된 요소가 됩니다.
...
- `replace`: 현재 `NavigationHistoryEntry`가 새 페이지의 항목으로 대체됩니다.
- `traverse`: `NavigationHistoryEntry`이 생성되지 않습니다. 히스토리 내 위치가 변경됩니다.
  순회 방향은 [`direction` 속성](#direction)에 지정됩니다.
...
탐색 중인 페이지의 URL입니다. 이 속성은 수정할 수 있으며, 라이프사이클이 끝날 때의 값이 다음 페이지의 `NavigationHistoryEntry`에 사용됩니다.
...
view transitions가 활성화된 HTML 양식을 제출할 때 이 필드는 양식의 데이터로 자동 설정됩니다. [`navigate()` 함수](#navigate)를 사용하는 경우 이 값은 옵션에 지정된 것과 동일합니다.
...
이 탐색에 사용된 view transition 객체입니다. [View Transitions API](https://developer.mozilla.org/ko/docs/Web/API/View_Transitions_API)를 지원하지 않는 브라우저에서는 편의를 위해 동일한 API를 구현하지만 DOM 통합이 없는 객체입니다.
...
[나만의 사용자 지정 스왑 함수](/ko/guides/view-transitions/#building-a-custom-swap-function)를 구축하는 방법에 대한 자세한 내용은 View Transitions 가이드를 참조하세요.
...
## 액션 (`astro:actions`) \{#actions-astroactions\}
...
`defineAction()` 유틸리티는 `src/actions/index.ts` 파일에서 새 액션을 정의하는 데 사용됩니다. 이 함수는 실행할 서버 로직이 포함된 [`handler()`](#handler-property) 함수와 런타임에 입력 매개변수를 검사하는 선택적인 [`input`](#input-validator) 속성을 받습니다.
...
`handler()`는 사용자 입력을 첫 번째 인수로 받아 호출됩니다. [`input`](#input-validator) 유효성 검사기가 설정되어 있으면 사용자 입력은 핸들러로 전달되기 전에 유효성이 검사됩니다. 두 번째 인수는 `getActionResult()`, `callAction()` 및 `redirect()`를 제외한 대부분의 Astro [표준 엔드포인트 컨텍스트](#endpoint-context)를 포함하는 `context` 객체입니다.

반환 값은 [devalue 라이브러리](https://github.com/Rich-Harris/devalue)를 사용하여 구문 분석됩니다. 이 라이브러리는 `Date()`, `Map()`, `Set()` 및 `URL()`의 인스턴스와 JSON 값을 지원합니다.

#### `input` 유효성 검사기 \{#input-validator\}
...
`accept: 'form'`과 함께 사용되는 경우 `input`은 `z.object()` 유효성 검사기를 사용해야 합니다.
...
<ReadMore>[양식 입력 오류 가이드](/ko/guides/actions/#displaying-form-input-errors)에서 `isInputError()` 사용에 대한 자세한 내용을 참조하세요.</ReadMore>
...
[`ComponentProp` 타입](/ko/guides/typescript/#component-props) 유틸리티를 사용하여 이 컴포넌트의 `Props`를 참조할 수 있습니다.
...
:::note
이 컴포넌트는 [Shiki 구성](/ko/guides/markdown-content/#shiki-configuration)에서 설정을 상속하지 **않습니다**. 반드시 컴포넌트 props를 사용하여 설정해야 합니다.
:::
...
Astro 구문의 [프래그먼트 사용](/ko/basics/astro-syntax/#fragments)에 대해 자세히 알아보세요.
...
이 컴포넌트는 Prism의 CSS 클래스를 적용하여 코드 블록에 대한 언어별 구문 강조를 제공합니다. 구문 강조를 표시하려면 **Prism CSS 스타일시트를 제공** (또는 자신만의 스타일시트)해야합니다! 자세한 내용은 [Prism 구성 섹션](/ko/guides/markdown-content/#prism-configuration)을 참조하세요.
...
[콘텐츠 컬렉션 항목](/ko/guides/content-collections/#what-are-content-collections)의 콘텐츠를 렌더링하는 데 사용되는 일반 컴포넌트입니다.
...
페이지 요소 및 컴포넌트에 [라우터 제어](/ko/guides/view-transitions/#router-control) 및 [전환 지시어 추가](/ko/guides/view-transitions/#transition-directives) 방법에 대해 자세히 알아보세요.
