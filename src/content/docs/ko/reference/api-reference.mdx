...
[나만의 사용자 지정 스왑 함수](/ko/guides/view-transitions/#사용자-정의-스왑-함수-구현)를 구축하는 방법에 대한 자세한 내용은 View Transitions 가이드를 참조하세요.
...
`handler()`는 사용자 입력을 첫 번째 인수로 받아 호출됩니다. [`input`](#input-유효성-검사기) 유효성 검사기가 설정되어 있으면 사용자 입력은 핸들러로 전달되기 전에 유효성이 검사됩니다. 두 번째 인수는 `getActionResult()`, `callAction()` 및 `redirect()`를 제외한 대부분의 Astro [표준 엔드포인트 컨텍스트](#엔드포인트-context)를 포함하는 `context` 객체입니다.
