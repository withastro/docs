...
정적 빌드에서 이 객체는 [동적 경로](/ko/basics/routing/#동적-경로) 사전 렌더링에 사용되는 `getStaticPaths()`에서 반환하는 `params`입니다.
...
[엔드포인트 함수](/ko/basics/endpoints/)는 첫 번째 매개변수로 context 객체를 받습니다. 이는 `Astro` global 속성의 많은 부분을 반영합니다.
...
[동적 라우팅](/ko/basics/endpoints/#params-및-동적-라우팅)을 위한 정적 파일 엔드포인트에서도 사용할 수 있습니다.
...
`getImage()` 함수는 HTML이 아닌 다른 곳 (예: [API 경로](/ko/basics/endpoints/#서버-엔드포인트-api-라우트))에서 사용할 이미지를 생성하기 위한 것입니다. 또한 사용자 정의 `<Image />` 컴포넌트를 만들 수도 있습니다.
