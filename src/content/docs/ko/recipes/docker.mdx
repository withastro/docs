...
Docker 이미지와 컨테이너는 AWS, Azure, [Google Cloud](/ko/guides/deploy/google-cloud/#cloud-run-ssr-and-static) 등 다양한 플랫폼에 배포할 수 있습니다. 이 레시피에서는 사이트를 특정 플랫폼에 배포하는 방법을 다루지 않지만 프로젝트에 Docker를 설정하는 방법을 보여줍니다.
...
## Dockerfile 생성 \{#creating-a-dockerfile\}
...
### .dockerignore 추가 \{#adding-a-dockerignore\}
...
### Multi-stage 빌드 (SSR 사용) \{#multi-stage-build-using-ssr\}
...
3. 이제 웹 사이트가 성공적으로 빌드되어 컨테이너에 패키징되었으므로 이를 클라우드 공급자에 배포할 수 있습니다. 한 가지 예시는 [Google Cloud](/ko/guides/deploy/google-cloud/#cloud-run-ssr-and-static) 배포 가이드와 Docker 문서의 [앱 배포](https://docs.docker.com/language/nodejs/deploy/) 페이지를 참조하세요.
</Steps>