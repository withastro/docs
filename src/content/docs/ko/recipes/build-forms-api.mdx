...
description: JavaScript를 사용하여 양식 제출을 API 라우트로 보내는 방법을 알아보세요.
i18nReady: true
type: recipe
---
...
- [SSR](/ko/guides/server-side-rendering/) (`output: 'server'` 또는 `'hybrid'`)가 활성화된 프로젝트가 필요합니다.
...
4. `<FeedbackForm />` 컴포넌트를 가져와 페이지에 포함합니다. 양식 로직이 원할 때 하이드레이션되도록 하려면 `client:*` 지시어를 사용하세요.
