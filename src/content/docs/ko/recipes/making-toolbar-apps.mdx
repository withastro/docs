...
개발 툴바 앱은 [`astro:config:setup` 훅](/ko/reference/integrations-reference/#astroconfigsetup)를 사용하여 [Astro 통합](/ko/guides/integrations-guide/)으로만 추가할 수 있습니다. 기존 Astro 프로젝트의 도구 모음에 추가할 도구 모음 앱과 통합을 모두 만들어야 합니다.
...
2. `my-integration.ts`에 다음 코드를 추가하여 `astro:config:setup` 훅으로 개발 툴바 앱을 추가하는 데 필요한 통합 이름과 [`addDevToolbarApp()` 함수](/ko/reference/dev-toolbar-app-reference/#툴바-앱-통합-설정)를 모두 제공합니다.
