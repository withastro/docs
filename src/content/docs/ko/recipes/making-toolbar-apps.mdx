...
## 동기를 부여하는 개발 툴바 앱 구축 \{#build-a-motivational-dev-toolbar-app\}
...
### Astro 통합 생성 \{#creating-the-astro-integration\}
...
2. `my-integration.ts`에 다음 코드를 추가하여 `astro:config:setup` 후크로 개발 툴바 앱을 추가하는 데 필요한 통합 이름과 [`addDevToolbarApp()` 함수](/ko/reference/dev-toolbar-app-reference/#toolbar-app-integration-setup)를 모두 제공합니다.
...
## UI 프레임워크를 사용하여 앱 빌드 \{#building-apps-with-a-ui-framework\}
...
:::note
그러나 JavaScript 또는 UI 프레임워크를 사용하여 개발 툴바 앱을 빌드하기로 선택한 경우에도 개발 툴바에 앱을 추가하는 [통합 생성](#creating-the-astro-integration)이 필요합니다.
:::

### 빌드 단계 생략 \{#without-a-build-step\}
