...
2. 기본 언어로 리디렉션하려면 `src/pages/index.astro`를 설정하세요.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        SSR 어댑터를 사용하는 경우 [`Astro.redirect`](/ko/core-concepts/routing/#동적-리디렉션)를 사용하여 서버의 기본 언어로 리디렉션할 수 있습니다.
...
3. [동적 경로](/ko/core-concepts/routing/#동적-경로)를 사용하여 `lang` 및 `slug` 매개변수를 기반으로 콘텐츠를 가져와 렌더링하세요.
    ...
    <ReadMore>[동적 라우팅](/ko/core-concepts/routing/#동적-경로)에 대해 자세히 알아보세요.</ReadMore>
...
4. 각 페이지에는 페이지의 언어와 일치하는 `<html>` 요소의 `lang` 속성이 있어야 합니다. 이 예시에서 [재사용 가능한 레이아웃](/ko/core-concepts/layouts/)은 현재 경로에서 언어를 추출합니다.
