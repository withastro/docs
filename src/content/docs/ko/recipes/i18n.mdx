...
다른 언어와 달리 기본 언어가 URL에 표시되지 않도록 하려면 아래의 [기본 언어를 숨기는 방법](/ko/recipes/i18n/#hide-default-language-in-the-url)을 참조하세요.

<ReadMore>오른쪽에서 왼쪽 (RTL) 스타일 지정 및 언어 태그 선택과 같은 관련 주제에 대한 외부 링크는 [리소스 섹션](#resources)을 참조하세요.</ReadMore>

## 레시피 \{#recipe\}

### 언어별 페이지 설정 \{#set-up-pages-for-each-language\}
...
2. 기본 언어로 리디렉션하려면 `src/pages/index.astro`를 설정하세요.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        SSR 어댑터를 사용하는 경우 [`Astro.redirect`](/ko/guides/routing/#dynamic-redirects)를 사용하여 서버의 기본 언어로 리디렉션할 수 있습니다.
...
### 번역된 콘텐츠에 컬렉션 사용 \{#use-collections-for-translated-content\}
...
3. [동적 경로](/ko/guides/routing/#dynamic-routes)를 사용하여 `lang` 및 `slug` 매개변수를 기반으로 콘텐츠를 가져와 렌더링하세요.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        [SSR 모드](/ko/guides/server-side-rendering/)에서 요청된 항목을 직접 가져옵니다.
    ...
    <ReadMore>[동적 라우팅](/ko/guides/routing/#dynamic-routes)에 대해 자세히 알아보세요.</ReadMore>
...
### UI 문자열 번역 \{#translate-ui-strings\}
...
### 사용자가 언어를 전환할 수 있도록 허용 \{#let-users-switch-between-languages\}
...
### URL에서 기본 언어 숨기기 \{#hide-default-language-in-the-url\}
...
## 커뮤니티 라이브러리 \{#community-libraries\}
