...
[Bun](https://bun.sh/)은 올인원 자바스크립트 런타임 및 툴킷입니다. 더 많은 내용은 [Bun 공식 문서](https://bun.sh/docs)를 참고하세요.
...
Bun 사용에 문제가 있는 경우, [Bun 공식 리포지터리에서 이슈를 열어주세요.](https://github.com/oven-sh/bun/issues/new/choose)
:::
...
`create-astro` 명령어로 새로운 Astro 프로젝트를 생성합니다.

```bash
bunx create-astro@latest my-astro-project-using-bun
```
:::tip 
여러분은 또한 `--template` 플래그를 사용하여, [Astro로 만들어진 다른 리포지터리로부터 새로운 Astro 프로젝트를 생성할 수도 있습니다.](/ko/install/auto/#starter-templates)
```bash
...
`bunx create-astro` 명령어로 프로젝트를 시작하는 경우, CLI에서 자동으로 Bun을 사용하여 의존성을 설치하므로 이 단계를 건너뛸 수 있습니다.

그렇지 않은 경우 Bun을 사용하여 의존성을 설치해야 합니다.
...
설치한 패키지를 `tsconfig.json`의 types에 추가합니다.
...
Astro는 빌드 결과물을 `dist/` 디렉토리에 담습니다. 그런 다음, `preview` 명령어를 사용하여 사이트를 제공할 수 있습니다.
...
사이트를 위와 동일한 명령어로 다시 빌드합니다.

```bash
bunx --bun astro build
```

마지막으로 빌드된 사이트를 실행하려면 다음 명령어를 사용합니다.
...
Bun을 사용하여 Astro를 개발 중이신가요? 블로그 글이나 비디오를 이 페이지에 추가하세요!

- [Bun을 사용하여 Cloudflare Pages 사이트 빌드](https://blog.otterlord.dev/posts/hello-from-bun/) - 블로그 글
