...
[아일랜드 아키텍처 / 부분 하이드레이션](/ko/concepts/islands/)를 사용하여 Astro 웹 사이트를 구축할 때 다음 문제에 직면했을 수 있습니다: **컴포넌트 간 상태를 공유하고 싶습니다.**

React 또는 Vue와 같은 UI 프레임워크는 다른 컴포넌트가 사용하도록 ["context" providers](https://ko.react.dev/learn/passing-data-deeply-with-context)를 권장할 수 있습니다. 하지만 Astro 또는 Markdown에서 [컴포넌트를 부분적으로 하이드레이션](/ko/guides/framework-components/#인터랙티브-컴포넌트-하이드레이션)하는 경우 이러한 컨텍스트 래퍼를 사용할 수 없습니다.
...
- `.astro` 파일이나 [하이드레이션되지 않은 컴포넌트](/ko/guides/framework-components/#인터랙티브-컴포넌트-하이드레이션)에서 저장소에 쓰는 것은 [클라이언트 측 컴포넌트](/ko/reference/directives-reference/#클라이언트-지시어)에서 받은 값에 영향을 주지 _않습니다._
...
여기에서 [Nano Stores 사용 가이드](https://github.com/nanostores/nanostores#guide)로 이동하거나 아래 예시를 따라갈 수 있습니다!
