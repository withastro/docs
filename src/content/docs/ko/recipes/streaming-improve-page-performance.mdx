...
이러한 컴포넌트를 사용하는 아래 Astro 페이지는 페이지의 일부를 더 빨리 렌더링할 수 있습니다. `<head>`, `<body>` 및 `<h1>` 태그는 더 이상 데이터 가져오기로 인해 차단되지 않습니다. 그런 다음 서버는 `RandomName` 및 `RandomFact`에 대한 데이터를 병렬로 가져오고 결과 HTML을 브라우저로 스트리밍합니다.
