...
Astro 페이지, 경로 및 API 엔드포인트는 [빌드 시 사전 렌더링](#pre-rendered)되거나 경로가 요청될 때 [서버에서 요청 시 렌더링](#on-demand-rendered)될 수 있습니다. [Astro 아일랜드](/ko/concepts/islands/)를 사용하면 필요한 경우 일부 클라이언트 측 렌더링을 포함할 수도 있습니다.
...
## 서버 `output` 모드 \{#server-output-modes\}
...
다음이 필요한 경우 Astro 프로젝트에서 [`server` 또는 `hybrid` 모드를 활성화시키는 것을 고려해보세요.](/ko/guides/server-side-rendering/#enable-on-demand-server-rendering) 
