...
단일 파일은 [동적 라우팅](/ko/guides/routing/#동적-라우트)을 사용하여 여러 페이지를 생성할 수도 있습니다. 이를 통해 콘텐츠가 [콘텐츠 컬렉션](/ko/guides/content-collections/)이나 [CMS](/ko/guides/cms/)와 같이 `/pages/` 디렉터리 외부에 있는 경우에도 페이지를 생성할 수 있습니다.
...
[온디맨드 렌더링](/ko/guides/server-side-rendering/#온디맨드-서버-렌더링-활성화)된 페이지에 대해 사용자 정의 500 오류 페이지를 표시하려면 `src/pages/500.astro` 파일을 생성하세요. 이 사용자 정의 페이지는 사전 렌더링된 페이지에서는 사용할 수 없으며 사전 렌더링할 수도 없습니다.
