...
Astro 컴포넌트 구문은 HTML의 상위 집합입니다. [HTML이나 JSX를 작성해 본 경험이 있다면 이 구문이 친숙하게 느껴질 것입니다.](#differences-between-astro-and-jsx) 구문은 컴포넌트 및 JavaScript 표현식을 포함하는 것을 지원합니다.


## JSX와 유사한 표현식 \{#jsx-like-expressions\}
...
[Astro와 JSX의 차이점](#differences-between-astro-and-jsx)에 대한 자세한 예는 하단을 참고하세요.
:::
...
- **수화 지시어는 지원되지 않습니다.** [`client:*` 수화 지시어](/ko/guides/framework-components/#hydrating-interactive-components)를 사용하면 Astro는 프로덕션을 위해 번들링할 컴포넌트를 알아야 하지만 동적 태그 패턴이 이를 알 수 없게 만듭니다.
...
### Astro와 JSX의 차이점 \{#differences-between-astro-and-jsx\}
