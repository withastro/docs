...
:::tip
이 가이드는 Astro 커뮤니티에서 사용되는 몇 가지 인기 있는 규칙을 설명합니다. Astro에서 예약되어 이름을 임의로 변경할 수 없는 디렉터리는 `src/pages/` 뿐입니다. 이 두 디렉터리를 제외한 나머지 디렉터리에 대해 가장 적합한 방식으로 이름을 변경하고 재구성할 수 있습니다.
:::
