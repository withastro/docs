...
Astro는 JavaScript 구성 파일에 대해 `astro.config.js`, `astro.config.mjs`, `astro.config.ts` 등 여러 파일 형식을 지원합니다. 대부분의 경우 `.mjs`를 사용하고 구성 파일에서 TypeScript를 작성하려면 `.ts`를 사용하는 것이 좋습니다.
