...
Astro는 **파일 기반 라우팅** 전략을 활용합니다. `src/pages/` 디렉터리의 각 파일은 파일 경로에 따라 사이트의 엔드포인트가 됩니다.

Astro leverages a routing strategy called **file-based routing**. Each file in your `src/pages/` directory becomes an endpoint on your site based on its file path.

[dynamic routing](/ko/core-concepts/routing/#dynamic-routes)을 이용하여 단일 파일로 여러 페이지를 생성할 수도 있습니다. 이렇게 하면 콘텐츠가 [content collection](/ko/guides/content-collections/)이나 [CMS](/ko/guides/cms/)와 같이 특별한 `/pages/` 디렉터리 외부에 있는 경우에도 페이지를 만들 수 있습니다.
...
파일 확장자가 `.html`인 파일은 `src/pages/`에 작성하여 페이지로 바로 사용할 수 있습니다. [HTML Components](/ko/core-concepts/astro-components/#html-components)는 Astro의 주요 기능의 일부가 지원되지 않는다는 점에 유의하세요.
