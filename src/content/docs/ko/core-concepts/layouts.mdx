...
- 개별 페이지 내용이 삽입될 위치를 지정하는 [**`<slot />`**](/ko/core-concepts/astro-components/#슬롯)
 ...
 하지만 레이아웃 구성 요소에 특별한 것은 없습니다! 그들은 [props을 받고](/ko/core-concepts/astro-components/#컴포넌트-props) 다른 Astro 컴포넌트처럼 다른 컴포넌트를 [가져와서 사용](/ko/core-concepts/astro-components/#컴포넌트-구조)할 수 있습니다. [UI 프레임워크 컴포넌트](/ko/core-concepts/framework-components/) 및 [클라이언트 사이드 스크립트](/ko/guides/client-side-scripts/)를 포함할 수 있습니다. 전체 페이지 셸을 제공할 필요도 없으며 부분 UI 템플릿으로 대신해서 사용할 수도 있습니다.
...
`MarkdownLayoutProps` 또는 `MDXLayoutProps` 헬퍼를 사용하여 레이아웃의 [`Props` type](/ko/guides/typescript/#컴포넌트-props)을 설정할 수 있습니다. 
