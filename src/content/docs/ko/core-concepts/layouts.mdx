...
우리는 일반적으로 headers, navigation bars 및 footers과 같은 페이지 간에 공유되는 공통 UI 요소를 제공하는 Astro 컴포넌트에 대해 "레이아웃"이라는 용어를 사용합니다. 일반적인 Astro 레이아웃 컴포넌트는 [Astro, Markdown 또는 MDX 페이지](/ko/core-concepts/astro-pages/)에 다음 기능을 제공합니다.
- 페이지 쉘 (`<html>`, `<head>` 및 `<body>` 태그들)
- 개별 페이지 내용이 삽입될 위치를 지정하는 [**`<slot />`**](/ko/core-concepts/astro-components/#slots)

 하지만 레이아웃 구성 요소에 특별한 것은 없습니다! 그들은 [props을 받고](/ko/core-concepts/astro-components/#component-props) 다른 Astro 컴포넌트처럼 다른 컴포넌트를 [가져와서 사용](/ko/core-concepts/astro-components/#component-structure)할 수 있습니다. [UI 프레임워크 컴포넌트](/ko/core-concepts/framework-components/) 및 [클라이언트 사이드 스크립트](/ko/guides/client-side-scripts/)를 포함할 수 있습니다. 전체 페이지 셸을 제공할 필요도 없으며 부분 UI 템플릿으로 대신해서 사용할 수도 있습니다.
...
📚 [슬롯](/ko/core-concepts/astro-components/#slots)에 대해 자세히 알아봅니다.
...
2. 페이지의 마크다운/MDX 내용을 렌더링할 위치를 나타내는 기본 [`<slot />`](/ko/core-concepts/astro-components/#slots)입니다.
...
frontmatter에 존재하지 않거나 존재할 수 없는 정보를 MDX 레이아웃에 전달해야 할 수도 있습니다. 이 경우에는 다른 컴포넌트처럼 [`<Layout />` 컴포넌트](/ko/core-concepts/layouts/)를 가져와 사용하고 props를 전달할 수 있습니다.
