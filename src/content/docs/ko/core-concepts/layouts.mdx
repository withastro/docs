...
예를 들어, `BlogPostLayout.astro` 레이아웃 컴포넌트는 게시물의 제목, 날짜 및 작성자를 스타일화할 수 있습니다. 그러면 사이트 전체에서 `BaseLayout.astro`가 네비게이션과 푸터와 같은 페이지 템플릿의 나머지 부분을 처리할 수 있습니다. 또한 다른 중첩된 컴포넌트와 마찬가지로 게시물에서 받은 props를 다른 레이아웃으로 전달할 수 있습니다.
