...
Astro 컴포넌트에 대해 알아야 할 가장 중요한 점은 **클라이언트에서 렌더링되지 않는다는 것**입니다. 빌드 시점 혹은 [server-side-rendering (SSR)](/ko/guides/server-side-rendering/)을 통해 온디맨드 방식으로 HTML로 렌더링됩니다. 컴포넌트 프론트매터에 자바스크립트 코드를 포함할 수 있으며, 사용자의 브라우저로 전송되는 최종 페이지에서 모든 코드가 제거됩니다. 그 결과 기본적으로 자바스크립트 풋프린트가 전혀 추가되지 않아 사이트 속도가 빨라집니다.

Astro 컴포넌트에 클라이언트 측 상호 작용이 필요한 경우에는 [standard HTML `<script>` tags](/ko/guides/client-side-scripts/)나 [UI Framework components](/ko/core-concepts/framework-components/#hydrating-interactive-components)를 추가할 수 있습니다. 자세한 내용은 다음 링크에서 확인하세요.
...
여기에 일반 HTML을 작성하면 해당 컴포넌트는 Astro 페이지에서 불러져 사용되는 경우 작성된 해당 HTML을 렌더링합니다.
...
`Props` 타입 인터페이스를 사용하여 TypeScript로 props를 정의할 수도 있습니다. Astro는 프론트매터에서 `Props` 인터페이스를 자동으로 선택하여 타입 경고나 오류를 표시합니다. props를 이용헤 `Astro.props`에서 디스트럭처링할 때 기본값을 지정할 수도 있습니다.
...
:::tip
명명된 슬롯은 [UI framework components](/ko/core-concepts/framework-components/)에도 전달할 수 있습니다!
:::
...
Astro는 `.html` 파일을 컴포넌트로 가져와서 사용하거나 `src/pages/` 하위 디렉토리에 페이지로 배치하는 기능을 지원합니다. 프레임워크 없이 구축된 기존 사이트의 코드를 재사용하고 싶거나, 컴포넌트에 동적인 기능이 들어가지 않도록 하려면 HTML 컴포넌트를 이용하는 것이 좋습니다.
...
- [`public/`](/ko/guides/images/#public) 폴더에 있는 에셋만 참조할 수 있습니다.
