...
- **수화 지시어는 지원되지 않습니다.** [`client:*` 수화 지시어](/ko/core-concepts/framework-components/#인터랙티브-컴포넌트-하이드레이션하기)를 사용하면 Astro는 프로덕션을 위해 번들링할 컴포넌트를 알아야 하지만 동적 태그 패턴이 이를 알 수 없게 만듭니다.
