...
- **변수 이름은 대문자로 시작해야 합니다.** 예를 들어 `element`가 아니라 `Element`를 사용하세요. 그렇지 않으면 Astro는 변수 이름을 리터럴 HTML 태그로 렌더링하는 것을 시도합니다.

- **하이드레이션 지시문은 지원되지 않습니다.** [`client:*` 하이드레이션 지시문](/ko/core-concepts/framework-components/#인터랙티브-컴포넌트-하이드레이션하기)을 사용하면 Astro는 프로덕션을 위해 번들링할 컴포넌트를 알아야 합니다. 그런데 동적 태그 패턴이 이를 알 수 없게 만듭니다.
