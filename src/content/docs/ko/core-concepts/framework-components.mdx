...
프레임워크 컴포넌트는 [`client:*` directive](/ko/reference/directives-reference/#클라이언트-지시어)를 이용하여 대화형(하이드레이션)으로 만들 수 있습니다. 이는 컴포넌트의 자바스크립트가 브라우저로 전송되는 시기를 결정하는 컴포넌트 속성입니다.
...
📚 하이드레이션 지시어에 대한 자세한 설명과 사용법은 [directives reference](/ko/reference/directives-reference/#클라이언트-지시어) 페이지를 참조하세요.
