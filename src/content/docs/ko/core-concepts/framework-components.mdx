---
title: 프레임워크 컴포넌트
description: React, Svelte와 같은 다양한 프레임워크를 사용하는 방법에 대해 배웁니다.
i18nReady: true
---
...
통합 설치
섹션 제목 통합 설치하기
Astro는 React, Preact, Svelte, Vue, SolidJS, AlpineJS 및 Lit에 대한 통합 옵션과 함께 제공됩니다. 프로젝트에서 이러한 Astro 통합 중 하나 또는 여러 개를 설치 및 구성할 수 있습니다.



## 통합 설치

Astro는 React, Preact, Svelte, Vue, SolidJS, AlpineJS 및 Lit에 대한 [optional integrations](/ko/guides/integrations-guide/)과 함께 제공됩니다. 프로젝트에서 하나 또는 여러 개의 Astro 통합을 설치 하고 설정할 수 있습니다.
...
기본적으로 프레임워크 컴포넌트는 정적 HTML로 렌더링됩니다. 이는 비대화형 컴포넌트를 템플릿화하는 데 유용하며, 필요하지 않은 자바스크립트를 클라이언트에 전송하는 것을 방지합니다.
...
`client:only` 를 제외한 모든 클라이언트 지시문을 사용하면 컴포넌트는 먼저 서버에서 렌더링되어 정적 HTML을 생성합니다. 자바스크립트 컴포넌트는 지시어에 따라 브라우저로 전송됩니다. 이후 컴포넌트가 하이드레이트 되어 인터랙티브하게 됩니다.
...
:::note[Accessibility]
이 컴포넌트들을 Astro에서 사용할 경우, 프레임워크별 접근성 패턴이 동일하게 작동해야 합니다. 접근성 관련 자바스크립트가 제대로 로드되고 실행되도록 클라이언트 지시어를 반드시 설정해야 합니다.
:::
...
이는 Astro에서 함수를 직렬화(서버에서 클라이언트로 전송)할 수 없기 때문입니다.
:::
...
:::note
Astro 컴포넌트는 하이드레이션되는 프레임워크 컴포넌트를 포함하더라도 항상 정적 HTML로 렌더링됩니다. 즉, Astro 컴포넌트에서 프레임워크 컴포넌트에 React의 'render props'를 전달하면, Astro 컴포넌트는 이 패턴이 요구하는 클라이언트 런타임 동작을 제공하지 못하기 때문에 작동하지 않습니다. 대신 명명된 슬롯을 사용합니다.
:::
...
하지만 [Astro의 `<slot />` 패턴](/ko/core-concepts/astro-components/#슬롯)을 이용하여 Astro 컴포넌트가 생성한 정적 콘텐츠를 **.astro 컴포넌트 내에서** 프레임워크 컴포넌트에게 자식 요소로 전달할 수 있습니다.
