...
## Astro에서 Firebase 초기화 \{#initializing-firebase-in-astro\}

### 전제조건 \{#prerequisites\}

- [웹 앱이 구성된 Firebase 프로젝트](https://firebase.google.com/docs/web/setup).
- [서버 측 렌더링 (SSR)](/ko/guides/server-side-rendering/)이 활성화된 Astro 프로젝트
...
### Firebase 사용자 인증 정보 추가 \{#adding-firebase-credentials\}
...
## Firebase로 인증 추가 \{#adding-authentication-with-firebase\}

### 전제조건 \{#prerequisites-1\}

- Astro 프로젝트 [Firebase로 초기화](#initializing-firebase-in-astro).
- Firebase 콘솔의 *Authentication > Sign-in*에서 이메일/비밀번호 인증이 활성화된 Firebase 프로젝트.

### 인증 서버 엔드포인트 생성 \{#creating-auth-server-endpoints\}
...
### OAuth 공급자 추가 \{#adding-oauth-providers\}
...
## Firestore 데이터베이스에 연결 \{#connecting-to-firestore-database\}

### 전제조건 \{#prerequisites-2\}

- [Astro에서 Firebase 초기화](#initializing-firebase-in-astro) 섹션에 설명된 대로 Firebase로 초기화된 Astro 프로젝트
...
### 서버 엔드포인트 생성 \{#creating-the-server-endpoints\}
...
#### 레코드 편집 또는 삭제 \{#edit-or-delete-a-record\}
...
#### 개별 레코드 표시 \{#display-an-individual-record\}
...
#### 편집 버튼 및 레코드 목록 표시 \{#display-a-list-of-records-with-an-edit-button\}
