...
## Astro에서 Supabase 초기화 \{#initializing-supabase-in-astro\}
...
### Supabase 자격 증명 추가 \{#adding-supabase-credentials\}
...
## Supabase로 인증 추가 \{#adding-authentication-with-supabase\}

Supabase는 즉시 인증을 제공합니다. GitHub, Google, Kakao 및 기타 여러 제공업체를 통해 이메일/비밀번호 인증과 OAuth 인증을 지원합니다.

### 선행조건 \{#prerequisites-1\}

- [Supabase로 초기화된](#initializing-supabase-in-astro) Astro 프로젝트가 필요합니다.
...
프로젝트의 `src/pages/api/auth` 디렉터리에 이러한 엔드포인트를 생성합니다. `hybrid` 렌더링 모드를 사용하는 경우, 각 파일 상단에 `export const prerender = false`를 지정해야 요청에 따라 이러한 엔드포인트를 렌더링할 수 있습니다. 이제 프로젝트에 이러한 새 파일이 포함되어야 합니다.
...
### 인증 페이지 만들기 \{#creating-auth-pages\}
...
### OAuth 인증 추가 \{#adding-oauth-authentication\}
