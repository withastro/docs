...
## Astro에서 스타일링하기 \{#styling-in-astro\}
...
### `class:list`를 사용하여 클래스 결합 \{#combining-classes-with-classlist\}
...
### `class`를 하위 컴포넌트에 전달 \{#passing-a-class-to-a-child-component\}
...
### 로컬 스타일시트 가져오기 \{#import-a-local-stylesheet\}

:::caution[npm 패키지를 사용하시나요?]
npm 패키지에서 가져올 때 `astro.config`를 업데이트해야 할 수도 있습니다. 아래의 ["npm 패키지에서 스타일시트 가져오기" 섹션](#import-a-stylesheet-from-an-npm-package)을 참조하세요.
:::

ESM 가져오기 구문을 사용하여 Astro 컴포넌트 프런트매터에 스타일시트를 가져올 수 있습니다. CSS 가져오기는 [Astro 컴포넌트의 다른 ESM 가져오기](/ko/basics/astro-components/#the-component-script)처럼 작동합니다. 이는 **컴포넌트를 기준으로 참조**되어야 하며 다른 가져오기와 함께 컴포넌트 스크립트 **상단**에 작성되어야 합니다.
...
### npm 패키지에서 스타일시트 가져오기 \{#import-a-stylesheet-from-an-npm-package\}
...
`normalize` (파일 확장자는 생략됨)라는 `package-name`에서 CSS 파일을 가져오고 있다고 가정해 보겠습니다. 페이지를 올바르게 사전 렌더링할 수 있도록 하려면 [`vite.ssr.noExternal` 배열](https://ko.vitejs.dev/config/ssr-options.html#ssr-noexternal)에 `package-name`을 추가하세요.
...
:::note
이는 [Astro SSR](/ko/guides/server-side-rendering/)과 관련이 없거나 필요하지 않은 [Vite 전용 설정](https://ko.vitejs.dev/config/ssr-options.html#ssr-noexternal)입니다.
:::
...
### "link" 태그를 통해 정적 스타일시트 로드 \{#load-a-static-stylesheet-via-link-tags\}
...
이 접근 방식은 `public/` 디렉터리를 사용하기 때문에 Astro에서 제공하는 일반적인 CSS 처리, 번들링 및 최적화를 건너뜁니다. 이러한 변환이 필요한 경우 위 [스타일시트 가져오기](#import-a-local-stylesheet) 방법을 사용하세요.

## Cascading 순서 \{#cascading-order\}
...
### 범위가 지정된 스타일 \{#scoped-styles-1\}

[범위 스타일](#scoped-styles)을 사용해도 CSS의 _구체성_ 이 높아지지는 않지만 항상 _표시 순서_ 에서는 마지막에 표시됩니다. 따라서 동일한 특성을 지닌 다른 스타일보다 우선합니다. 예를 들어 범위가 지정된 스타일과 충돌하는 스타일시트를 가져오는 경우 범위가 지정된 스타일의 값이 적용됩니다.
...
[링크 태그](#load-a-static-stylesheet-via-link-tags)를 통해 로드된 스타일시트는 Astro 파일의 다른 스타일보다 먼저 순서대로 평가됩니다. 따라서 이러한 스타일은 가져온 스타일시트 및 범위가 지정된 스타일보다 우선순위가 낮습니다.
...
## CSS 통합 \{#css-integrations\}

Astro는 [Tailwind](https://tailwindcss.com) 등과 같은 프로젝트에 인기 있는 CSS 라이브러리, 도구, 프레임워크를 추가할 수 있도록 지원합니다!

### Tailwind \{#tailwind\}

프로젝트에서 Tailwind를 사용하려면 패키지 관리자의 `astro add` 명령을 사용하여 공식 [Astro Tailwind 통합][tailwind]을 설치하세요.
...
<ReadMore>Astro 통합 설치, 가져오기, 구성에 대한 지침은 [통합 안내서](/ko/guides/integrations-guide/)를 참조하세요.</ReadMore>

## CSS 전처리기 \{#css-preprocessors\}

Astro는 [Sass][sass], [Stylus][stylus], [Less][less]부터 [Vite][vite-preprocessors] 등의 CSS 전처리기를 지원합니다.

### Sass와 SCSS \{#sass-and-scss\}
...
### 프레임워크 컴포넌트에서 \{#in-framework-components\}
...
Astro에는 [Vite](https://ko.vitejs.dev/guide/features.html#postcss)의 일부로 PostCSS가 포함되어 있습니다. 프로젝트에 PostCSS를 구성하려면 프로젝트 루트에 `postcss.config.cjs` 파일을 생성하세요. 플러그인을 설치한 후 `require()`를 사용하여 플러그인을 가져올 수 있습니다 (예: `npm install autoprefixer`).
...
## Markdown 스타일링 \{#markdown-styling\}

모든 Astro 스타일 지정 방법은 [Markdown 레이아웃 컴포넌트](/ko/basics/layouts/#markdown-layouts)에 사용할 수 있지만, 방법에 따라 페이지에 스타일 효과가 달라집니다.

페이지 콘텐츠를 래핑하는 레이아웃에 [가져온 스타일시트](#external-styles)를 추가하여 Markdown 콘텐츠에 전역 스타일을 적용할 수 있습니다. 레이아웃 컴포넌트에서 [`<style is:global>` 태그](#global-styles)를 사용하여 Markdown의 스타일을 지정할 수도 있습니다. 추가된 모든 스타일에는 [Astro의 cascading 순서](#cascading-order)가 적용되며 렌더링된 페이지를 주의 깊게 확인하여 스타일이 의도한 대로 적용되는지 확인해야 합니다.

[Tailwind](/ko/guides/integrations-guide/tailwind/)를 포함한 [CSS 통합](#css-integrations)을 추가할 수도 있습니다. Tailwind를 사용하는 경우 [타이포그래피 플러그인](https://tailwindcss.com/docs/typography-plugin)이 Markdown 스타일을 지정하는 데 유용할 수 있습니다.

## 프로덕션 \{#production\}

### 번들 제어 \{#bundle-control\}
...
자세한 내용은 [Vite 문서](https://ko.vitejs.dev/guide/assets.html#importing-asset-as-string)를 참조하세요.

### `?url` CSS 가져오기 \{#url-css-imports\}
...
:::caution
`?url`을 사용하여 더 작은 CSS 파일을 가져오면 CSS 파일의 base64 인코딩 콘텐츠가 최종 빌드의 데이터 URL로 반환될 수 있습니다. 인코딩된 데이터 URL (`data:text/css;base64,...`)을 지원하는 코드를 작성하거나 [`vite.build.assetsInlineLimit`](https://ko.vitejs.dev/config/build-options.html#build-assetsinlinelimit) 구성 옵션을 `0`으로 설정하여 이 기능을 비활성화하세요.
:::
...
자세한 내용은 [Vite 문서](https://ko.vitejs.dev/guide/assets.html#importing-asset-as-url)를 참조하세요.

[less]: https://lesscss.org/
[sass]: https://sass-lang.com/
[stylus]: https://stylus-lang.com/
[svelte-style]: https://svelte.dev/docs#component-format-style
[tailwind]: /ko/guides/integrations-guide/tailwind/
[vite-preprocessors]: https://ko.vitejs.dev/guide/features.html#css-pre-processors
