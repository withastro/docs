...
📚 `class:list`에 대해 자세히 알아보려면 [지시어 참조](/ko/reference/directives-reference/#classlist) 페이지를 확인하세요.
...
📚 `define:vars`에 대해 자세히 알아보려면 [지시어 참조](/ko/reference/directives-reference/#definevars) 페이지를 확인하세요.
...
📚 `public/` 또는 `src/` 디렉터리에 있는 [정적 자산](/ko/guides/imports/) 사용에 대해 자세히 알아보세요.
...
ESM 가져오기 구문을 사용하여 Astro 컴포넌트 프런트매터에 스타일시트를 가져올 수 있습니다. CSS 가져오기는 [Astro 컴포넌트의 다른 ESM 가져오기](/ko/core-concepts/astro-components/#컴포넌트-스크립트)처럼 작동합니다. 이는 **컴포넌트를 기준으로 참조**되어야 하며 다른 가져오기와 함께 컴포넌트 스크립트 **상단**에 작성되어야 합니다.
...
:::tip
Astro의 일반적인 패턴은 [레이아웃 컴포넌트](/ko/core-concepts/layouts/)에 전역 CSS를 가져오는 것입니다. 우선순위가 가장 낮도록 다른 가져오기보다 먼저 레이아웃 컴포넌트를 가져와야 합니다.
:::
...
📚 Astro 통합 설치, 가져오기, 구성에 대한 지침은 [통합 안내서](/ko/guides/integrations-guide/)를 참조하세요.
...
모든 Astro 스타일 지정 방법은 [Markdown 레이아웃 컴포넌트](/ko/core-concepts/layouts/#markdownmdx-레이아웃)에 사용할 수 있지만, 방법에 따라 페이지에 스타일 효과가 달라집니다.
...
[Tailwind](/ko/guides/integrations-guide/tailwind/)를 포함한 [CSS 통합](#css-통합)을 추가할 수도 있습니다. Tailwind를 사용하는 경우 [타이포그래피 플러그인](https://tailwindcss.com/docs/typography-plugin)이 Markdown 스타일을 지정하는 데 유용할 수 있습니다.
