...
## `@astrojs/rss` 설정 \{#setting-up-astrojsrss\}

패키지 [`@astrojs/rss`](https://github.com/withastro/astro/tree/main/packages/astro-rss)는 [API endpoints](/ko/guides/endpoints/#static-file-endpoints)를 사용하여 RSS 피드를 생성하기 위한 헬퍼를 제공합니다. 이렇게 하면 [SSR adapter](/ko/guides/server-side-rendering/#add-an-adapter)를 사용할 때 정적 빌드 및 온디맨드 생성을 모두 사용할 수 있습니다.
...
### 콘텐츠 컬렉션 사용하기 \{#using-content-collections\}
...
### Glob Imports 사용하기 \{#using-glob-imports\}
...
### 게시물의 전체 콘텐츠 포함하기 \{#including-full-post-content\}
...
## 후행 슬래시 제거 \{#removing-trailing-slashes\}
...
## 스타일시트 추가하기 \{#adding-a-stylesheet\}
...
## RSS 피드 자동 검색 활성화 \{#enabling-rss-feed-auto-discovery\}
