...
- `import.meta.env.ASSETS_PREFIX`: [`build.assetsPrefix` config option](/en/reference/configuration-reference/#buildassetsprefix)이 설정된 경우 Astro에서 생성된 에셋 링크의 접두사입니다. 이 접두사는 Astro가 처리하지 않는 에셋 링크를 생성하는 데 사용할 수 있습니다.
