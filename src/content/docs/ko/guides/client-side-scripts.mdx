...
## 클라이언트 측 스크립트 \{#client-side-scripts\}
...
## Astro에서 `<script>` 사용하기 \{#using-script-in-astro\}
...
### 처리 거부 \{#opting-out-of-processing\}
...
<ReadMore>`<script>` 태그에서 사용할 수 있는 지시어에 대한 자세한 내용은 [지시어 참조](/ko/reference/directives-reference/#script--style-directives) 페이지를 확인하세요.</ReadMore>

### 페이지에 JavaScript 파일 포함 \{#include-javascript-files-on-your-page\}

스크립트를 별도의 `.js`/`.ts` 파일로 작성하거나 다른 서버의 외부 스크립트를 참조해야 할 수도 있습니다. `<script>` 태그의 `src` 속성에서 이를 참조하여 수행할 수 있습니다.

#### 로컬 스크립트 가져오기 \{#import-local-scripts\}

**사용 시기:** 스크립트가 `src/` 디렉터리에 있을 때.

Astro는 [스크립트 처리 규칙](#script-processing)에 따라 이러한 스크립트를 빌드, 최적화하여 페이지에 추가합니다.
...
#### 외부 스크립트 로드 \{#load-external-scripts\}
...
## 일반적인 스크립트 패턴 \{#common-script-patterns\}

### `onclick` 및 기타 이벤트 처리 \{#handle-onclick-and-other-events\}
...
### 사용자 정의 요소가 있는 웹 컴포넌트 \{#web-components-with-custom-elements\}
...
### 프런트매터 변수를 스크립트에 전달 \{#pass-frontmatter-variables-to-scripts\}

Astro 컴포넌트에서 `---` 펜스 사이의 [프런트매터](/ko/basics/astro-components/#the-component-script)에 있는 코드는 서버에서 실행되며 브라우저에서는 사용할 수 없습니다. 서버에서 클라이언트로 변수를 보내려면 변수를 저장한 다음 브라우저에서 JavaScript가 실행될 때 읽는 방법이 필요합니다.
