...
이 **[Astro 통합](/ko/guides/integrations-guide/)**을 통해 [Vue 3](https://vuejs.org/) 컴포넌트에 대한 서버 측 랜더링 및 클라이언트 측 하이드레이션이 가능해집니다.

## 설치

Astro에는 공식 통합 설정을 자동화하는 `astro add` 명령이 포함되어 있습니다. 원하는 경우 통합을 [수동으로 설치](#수동-설치)할 수도 있습니다.

`@astrojs/vue`를 설치하려면 프로젝트 디렉터리에서 다음을 실행하고 프롬프트를 따릅니다:
...
문제가 발생하면 [GitHub에 보고](https://github.com/withastro/astro/issues)하고 아래의 수동 설치 단계를 시도합니다.
...
Astro에서 첫 번째 Vue 컴포넌트를 사용하려면 [UI 프레임워크 문서](/ko/core-concepts/framework-components/#using-framework-components)로 이동합니다. 다음을 확인할 수 있습니다:
...
통합에 대한 자세한 내용은 [Astro 통합 문서](/ko/guides/integrations-guide/)를 확인합니다.
