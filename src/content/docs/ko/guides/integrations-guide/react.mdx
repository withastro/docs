...
:::tip[v7.0.0으로 업그레이드하시나요?]
`@astrojs/react` v7.0.0에서는 `babel` 통합 옵션이 제거되었습니다. 마이그레이션 방법은 [React 통합을 v7.0.0으로 업그레이드하기](#react-통합을-v700으로-업그레이드하기)를 참조하세요.
:::
...
기본적으로 `@astrojs/react`는 [Oxc](https://oxc.rs/)를 사용하여 JSX를 컴파일하고 Fast Refresh를 활성화합니다. 컴포넌트나 훅을 메모이제이션하지는 않습니다.

`compiler: true`로 설정하면 [실험적인 Oxc React Compiler](https://oxc.rs/docs/guide/usage/transformer/react-compiler)로 클라이언트 컴포넌트와 훅을 자동으로 메모이제이션합니다. `useMemo()`, `useCallback()`, `React.memo()`를 직접 작성하지 않아도 불필요한 리렌더링을 줄일 수 있습니다.

컴파일러를 사용하려면 [`oxc-transform-react`](https://www.npmjs.com/package/oxc-transform-react)를 설치해야 합니다:
...
컴파일러는 설치된 React 버전을 대상으로 합니다. React 17과 18에는 컴파일러 런타임 헬퍼가 포함되어 있지 않습니다. 이러한 버전 중 하나를 사용하는 프로젝트라면 [`react-compiler-runtime`](https://www.npmjs.com/package/react-compiler-runtime)도 함께 설치하세요:
...
컴파일러 구성을 더 세밀하게 제어하려면 객체를 전달할 수도 있습니다.

다음 예시는 `"use memo"` 지시어로 표시된 컴포넌트와 훅만 컴파일하도록 `compilationMode`를 구성합니다:
...
컴파일러는 통합의 [`include` 및 `exclude` 옵션](#여러-jsx-프레임워크-결합)이 적용되는 모든 곳에 적용됩니다. 서버 렌더링, 의존성, `.astro` 파일은 건너뜁니다.
...
## React 통합을 v7.0.0으로 업그레이드하기

`@astrojs/react` v7.0.0에서는 JSX 컴파일과 Fast Refresh 활성화를 위해 Babel 대신 [Oxc](https://oxc.rs/)를 사용하며, `@vitejs/plugin-react` v6으로 업그레이드됩니다.

### 제거됨: `babel` 옵션

제거된 `babel` 옵션 대신 [`vite.plugins`](/ko/reference/configuration-reference/#vite)에서 [`@rolldown/plugin-babel`](https://github.com/rolldown/plugins/tree/main/packages/babel)로 사용자 정의 Babel 변환을 구성하세요.

`@rolldown/plugin-babel`과 `@babel/core`를 설치하세요:
...
그런 다음 Babel 플러그인과 프리셋을 `vite.plugins` 아래의 `babel()` 플러그인으로 옮기세요.

다음 예시는 제거된 `babel` 옵션에서 `babel-plugin-styled-components`를 옮깁니다:
...
이전에 `babel` 콜백으로 구성했던 조건부 변환은 [`@rolldown/plugin-babel`의 `overrides` 및 프리셋 훅](https://github.com/rolldown/plugins/tree/main/packages/babel#options)을 참조하세요.

`babel()`과 [`compiler: true`](#react-compiler)를 함께 사용할 수 있습니다. Babel 구성에 `babel-plugin-react-compiler`가 포함되어 있다면 먼저 제거하세요. 동일한 컴포넌트에 React Compiler 변환이 두 번 적용되는 것을 방지할 수 있습니다.
