...
## 왜 Astro Sitemap인가? \{#why-astro-sitemap\}
...
Astro Sitemap을 사용하면 이 XML 파일을 직접 생성하는 것에 대해 걱정할 필요가 없습니다. Astro Sitemap 통합은 정적으로 생성된 경로를 크롤링하여 사이트맵 파일을 생성합니다. 여기에는 `getStaticPaths()`에 의해 생성된 `[...slug]` 또는 `src/pages/[lang]/[version]/info.astro`와 같은 [동적 경로](/ko/guides/routing/#dynamic-routes)가 포함됩니다.

이 통합은 [SSR 모드](/ko/basics/rendering-modes/#on-demand-rendered)에서 동적 경로에 대한 사이트맵 항목을 생성할 수 없습니다.
...
#### `robots.txt` 내 사이트맵 링크 \{#sitemap-link-in-robotstxt\}
...
### changefreq, lastmod, priority \{#changefreq-lastmod-and-priority\}
