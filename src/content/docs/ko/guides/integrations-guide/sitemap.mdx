...
Astro Sitemap을 사용하면 이 XML 파일을 직접 생성하는 것에 대해 걱정할 필요가 없습니다. Astro Sitemap 통합은 정적으로 생성된 경로를 크롤링하여 사이트맵 파일을 생성합니다. 여기에는 `getStaticPaths()`에 의해 생성된 `[...slug]` 또는 `src/pages/[lang]/[version]/info.astro`와 같은 [동적 경로](/ko/guides/routing/#동적-라우트)가 포함됩니다.
