...
`_routes.json` 파일을 통해 [Cloudflare 라우팅](https://developers.cloudflare.com/pages/platform/functions/routing/#functions-invocation-routes)을 사용하여 함수를 호출하는 경로와 정적 자산을 정의할 수 있습니다.

#### 사용자 정의 `_routes.json`

기본적으로 어댑터는 애플리케이션을 기반으로 `include` 및 `exclude` 규칙을 사용하여 `_routes.json` 파일을 생성합니다.
이를 통해 Cloudflare는 함수 호출 없이 파일을 제공하고 정적 리디렉션을 처리할 수 있습니다. 사용자 정의 `_routes.json`을 생성하면 이 자동 생성이 재정의됩니다. 자세한 내용은 [사용자 지정 `routes.json` 생성에 대한 Cloudflare 설명서](https://developers.cloudflare.com/pages/platform/functions/routing/#create-a-_routesjson-file)를 참조하세요.
