...
이 기능은 기본적으로 활성화되어 있습니다. 비활성화하려면 `cloudflareModules: false`로 설정하세요.
...
`configPath` 속성을 사용하면 Astro 프로젝트의 루트를 기준으로 Wrangler 구성 파일을 정의할 수 있습니다.

#### platformProxy.environment
<p>
**타입:** `{ environment?: string }`<br />
**기본값:** `{ environment?: undefined }`
</p>

`environment` 속성을 사용하면 Wrangler 구성에서 사용할 [환경 구성](https://developers.cloudflare.com/workers/wrangler/environments/)을 정의할 수 있습니다.
...
정적 자산 라우팅은 빌드 디렉터리 (예: `./dist`)의 파일 구조를 기반으로 합니다. 일치하는 항목이 없으면 SSR을 위해 Worker로 대체됩니다. [Cloudflare Workers를 사용한 정적 자산 라우팅](https://developers.cloudflare.com/workers/static-assets/routing/)에 대해 자세히 알아보세요.
