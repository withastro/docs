...
이 어댑터를 사용하면 Astro가 [`hybrid` 또는 `server` 렌더링된 사이트](/ko/basics/rendering-modes/#온디맨드-렌더링)를 [Cloudflare](https://www.cloudflare.com/)에 배포할 수 있습니다.
...
그런 다음 어댑터와 원하는 [on-demand 렌더링 모드](/ko/basics/rendering-modes/#온디맨드-렌더링)를 `astro.config.mjs` 파일에 추가하세요.
...
* **`compile`:** Astro의 기본 서비스 (sharp)를 사용하지만 빌드 시 사전 렌더링된 경로에서만 사용됩니다. 온디맨드 렌더링 페이지에 대한 SSR 중에는 모든 `astro:assets` 기능이 비활성화됩니다.
...
온디맨드 렌더링에서 제외할 경로를 `routes.extend.exclude` 배열에 구성합니다. 대신 이러한 경로는 사전 렌더링되어 정적으로 제공되며 SSR 함수를 호출하지 않습니다. 또한 이 옵션을 사용하면 SSR 함수를 통해 요청을 라우팅하지 않고도 정적 자산 (예: 이미지, 글꼴, CSS, js, html, txt, json 등) 파일을 직접 제공할 수 있습니다.
...
export default defineConfig({
  adapter: cloudflare({
    routes: {
      extend: {
        include: [{ pattern: '/static' }], // 온디맨드 렌더링을 위해 사전 렌더링된 페이지를 SSR 함수로 라우팅합니다.
