...
이 어댑터를 사용하면 Astro가 [`hybrid` 또는 `server` 렌더링된 사이트](/ko/guides/on-demand-rendering/)를 [Cloudflare](https://www.cloudflare.com/)에 배포할 수 있습니다.

Astro를 정적 사이트 빌더 로 사용하는 경우 어댑터가 필요하지 않습니다.
...
그런 다음 어댑터와 원하는 [on-demand 렌더링 모드](/ko/guides/on-demand-rendering/)를 `astro.config.mjs` 파일에 추가하세요.
