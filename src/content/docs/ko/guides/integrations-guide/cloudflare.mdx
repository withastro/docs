...
이 어댑터를 사용하면 Astro가 [`hybrid` 또는 `server` 렌더링된 사이트](/ko/core-concepts/rendering-modes/#on-demand-rendered)를 [Cloudflare](https://www.cloudflare.com/)에 배포할 수 있습니다.

Astro를 [정적 사이트 빌더](/ko/core-concepts/rendering-modes/#pre-rendered)로 사용하는 경우 어댑터가 필요하지 않습니다.
...
그런 다음 어댑터와 원하는 [on-demand 렌더링 모드](/ko/core-concepts/rendering-modes/#on-demand-rendered)를 `astro.config.mjs` 파일에 추가하세요.
...
[custom `_routes.json`](#custom-_routesjson)이 제공되지 않은 경우 `routes.json` 파일이 생성되는 방법을 결정합니다.
...
Cloudflare 빌드와 Astro 개발 서버 모두에서 이 기능을 활성화하려면 `astro.config.mjs` 파일에 `wasmModuleImports: true`를 추가하세요. [Wasm 모듈 사용](#use-wasm-modules)에 대해 자세히 알아보세요.
...
[Cloudflare Runtime](#cloudflare-runtime)에 대해 자세히 알아보세요.
...
* `off`: `astro dev`를 사용하여 런타임에 액세스할 수 없습니다. 런타임에 액세스해야 할 때 [Preview with Wrangler](#preview-with-wrangler)를 선택하여 로컬에서 프로덕션 환경을 시뮬레이션할 수 있습니다.
* `local`: Cloudflare의 바인딩을 지원하는 miniflare 및 workerd로 구동되는 로컬 런타임을 사용합니다. `eval`과 같이 지원되지 않는 기능을 사용하려는 경우에만 로컬 지원이 없는 바인딩에서 [Preview with Wrangler](#preview-with-wrangler)를 선택하세요.
