...
MDX를 사용하면 Astro에서 [Markdown 콘텐츠 내에서 변수, JSX 표현식 및 컴포넌트를 사용](/ko/guides/markdown-content/#mdx-only-features)할 수 있습니다. MDX로 작성된 기존 콘텐츠가 있는 경우 이 통합을 통해 해당 파일을 Astro 프로젝트로 가져올 수 있습니다.
...
Astro MDX 통합을 사용하면 `src/pages/` 디렉터리에 `.mdx` 파일을 추가하여 [프로젝트에 MDX 페이지를 추가](/ko/guides/markdown-content/#markdown-and-mdx-pages)할 수 있습니다. [`.mdx` 파일을 `.astro` 파일로 가져올 수도 있습니다](/ko/guides/markdown-content/#importing-markdown).

Astro의 MDX 통합은 Markdown 스타일의 프런트매터를 포함하여 표준 MDX에 기능을 추가합니다. 이를 통해 [특별한 프런트매터 `layout` 속성](/ko/guides/markdown-content/#frontmatter-layout)과 같은 Astro에 내장된 Markdown 기능 대부분을 사용할 수 있습니다.
...
MDX 최적화 프로그램이 [컴포넌트 prop을 통해 가져온 MDX 콘텐츠로 전달된 사용자 정의 컴포넌트](/ko/guides/markdown-content/#custom-components-with-imported-mdx)를 처리하지 못하도록 방지하는 `optimize`의 선택적 속성입니다.
