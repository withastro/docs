...
기본적으로 MDX 통합은 모든 [`markdown` 구성 옵션](/ko/reference/configuration-reference/#markdown-옵션)을 상속합니다. MDX 통합에 전달된 옵션은 상속된 `markdown` 구성을 재정의합니다. 상속된 `markdown` 옵션을 완전히 무시하려면 [`extendMarkdownConfig`](#extendmarkdownconfig) 옵션을 사용하세요.

다음 예제에서는 MDX 통합에 다른 [구문 강조](/ko/guides/syntax-highlighting/) 테마를 사용하고, [플러그인](/ko/guides/markdown-content/#markdown-프로세서-플러그인)을 추가하며, [GitHub-Flavored Markdown](/ko/guides/markdown-content/#github-flavored-markdown)을 비활성화하도록 구성합니다.
...
export default defineConfig({
  ...
  integrations: [
    mdx({
      ...
      // 플러그인 추가 및 GitHub-Flavored Markdown 비활성화
      processor: satteri({
...
기본적으로 `.mdx` 파일은 `.md` 파일과 동일한 [Markdown 프로세서](/ko/guides/markdown-content/#markdown-프로세서)를 통해 렌더링됩니다. MDX 파일에 다른 프로세서를 구성하거나 동일한 프로세서에 다른 옵션을 지정할 수 있습니다.

예를 들어, `.md` 파일에는 기본 [Sätteri](https://satteri.bruits.org/) 프로세서를 사용하고 `.mdx` 파일은 `@astrojs/markdown-remark`를 통해 remark 및 rehype로 렌더링하려면 다음과 같이 설정합니다.
...
recma 플러그인은 출력되는 [estree](https://github.com/estree/estree)를 직접 수정합니다. 이는 MDX 파일에서 JavaScript 변수를 수정하거나 삽입하는 데 유용하며, `.md` 파일에는 영향을 주지 않습니다.

recma 플러그인은 [Unified 프로세서](/ko/guides/markdown-content/#unified를-사용하는-경우)에서만 지원됩니다. `recmaPlugins` 옵션을 사용하여 `@astrojs/markdown-remark`의 `unified()`에 전달하세요.
...
대신 [`markdown.processor`](/ko/reference/configuration-reference/#markdownprocessor)에 이러한 플러그인을 설정할 수도 있습니다. MDX는 기본적으로 Markdown 구성을 [확장](#extendmarkdownconfig)하므로 `.mdx` 파일에서도 해당 플러그인이 실행됩니다.

estree 출력을 실험하려면 [AST Explorer를 사용](https://astexplorer.net/)하고, JavaScript 노드 전체에서 검색하려면 [`estree-util-visit`](https://unifiedjs.com/explore/package/estree-util-visit/)를 사용해 보세요.
...
:::caution[지원 중단]
이 옵션은 지원 중단되었으며 향후 메이저 버전에서 제거될 예정입니다. 구성된 [`processor`](#processor)에 대신 `recmaPlugins`를 전달하세요.

[recma 플러그인 사용하기](#recma-플러그인-사용하기)에서 자세히 알아보세요.
:::
...
Astro v7부터 기본 Markdown 프로세서는 recma 플러그인을 지원하지 않습니다. 프로젝트에서 recma 플러그인을 사용하는 경우, 대신 [`unified()` 프로세서](/ko/guides/markdown-content/#markdown-프로세서-설정하기)를 사용할 수 있습니다.
