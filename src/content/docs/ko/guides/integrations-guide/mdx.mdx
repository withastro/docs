...
예를 들어, `.md` 파일에는 기본 remark/rehype 프로세서를 유지하면서 `.mdx` 파일은 `@astrojs/markdown-satteri`를 사용하는 [Sätteri](https://satteri.bruits.org/)로 렌더링하려면 다음과 같이 설정합니다.
...
export default defineConfig({
  ...
  integrations: [
    mdx({
      // Markdown의 `syntaxHighlight`가 재정의되어,
      // `.mdx` 파일은 대신 Shiki를 사용합니다.
      syntaxHighlight: 'shiki',

      // 이 옵션으로 인해 `.mdx` 파일에 대한 `markdown.processor` 설정이 재정의됩니다.
