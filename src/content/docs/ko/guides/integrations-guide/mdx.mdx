...
기본적으로 모든 [`markdown` 구성 옵션](/ko/reference/configuration-reference/#markdown-옵션)은 MDX 통합에 상속됩니다. MDX 통합에 전달된 옵션은 상속된 `markdown` 구성을 재정의합니다. 상속된 `markdown` 옵션을 완전히 무시하려면 [`extendMarkdownConfig`](#extendmarkdownconfig) 옵션을 사용하세요.
...
기본적으로 `.mdx` 파일은 `.md` 파일과 동일한 [Markdown 프로세서](/ko/guides/markdown-content/#markdown-프로세서)를 통해 렌더링됩니다. MDX에 다른 프로세서를 구성하거나 동일한 프로세서에 다른 옵션을 사용할 수 있습니다.
