...
### 내보낸 변수를 MDX에서 사용하기 \{#using-exported-variables-in-mdx\}
...
MDX 통합을 설치한 후에는 다른 Astro 컴포넌트에서 사용하는 것과 마찬가지로 MDX (`.mdx`) 파일에서 [Astro 컴포넌트](/ko/basics/astro-components/)와 [UI 프레임워크 컴포넌트](/ko/guides/framework-components/#using-framework-components) 모두를 가져와서 사용할 수 있습니다.
...
#### 가져온 MDX를 사용하는 사용자 정의 컴포넌트 \{#custom-components-with-imported-mdx\}

가져온 MDX 콘텐츠를 렌더링할 때 [사용자 정의 컴포넌트](#assigning-custom-components-to-html-elements)는 `components` 속성을 통해 전달될 수 있습니다.
...
#### HTML 요소에 사용자 정의 컴포넌트 할당 \{#assigning-custom-components-to-html-elements\}
...
* [Markdown 구성에서 상속된 옵션](#options-inherited-from-markdown-config)
...
모든 [`markdown` 구성 옵션](/ko/reference/configuration-reference/#markdown-options)은 MDX 통합에서 별도로 구성할 수 있습니다. 여기에는 remark 및 rehype 플러그인, 구문 강조 등이 포함됩니다. 옵션은 기본적으로 Markdown 구성의 옵션으로 설정됩니다 ([이를 수정하려면 `extendMarkdownConfig` 옵션 참조](#extendmarkdownconfig)).
...
<ReadMore>전체 옵션 목록은 [Markdown 옵션 참조](/ko/reference/configuration-reference/#markdown-options)를 확인하세요.</ReadMore>
...
MDX는 기본적으로 [프로젝트의 기존 Markdown 구성](/ko/reference/configuration-reference/#markdown-options)을 확장합니다. 개별 옵션을 재정의하려면 MDX 구성에서 해당 옵션을 지정하면 됩니다.
...
MDX 최적화 프로그램이 [컴포넌트 prop을 통해 가져온 MDX 콘텐츠에 전달된 사용자 정의 컴포넌트](/ko/guides/markdown-content/#custom-components-with-imported-mdx)와 같은 특정 요소 이름을 처리하지 못하도록 방지하는 `optimize`의 선택적 속성입니다.
...
MDX 파일이 [`export const components = { ... }`를 사용하여 사용자 정의 컴포넌트를 구성](/ko/guides/markdown-content/#assigning-custom-components-to-html-elements)하는 경우, 이 옵션을 수동으로 구성할 필요가 없습니다. 최적화 프로그램은 이를 자동으로 감지합니다.
