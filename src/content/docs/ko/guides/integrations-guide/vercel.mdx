...
Astro는 이 어댑터를 사용하여 [`hybrid` 또는 `server` 방식으로 렌더링된 사이트](/ko/core-concepts/rendering-modes/#요청-시-렌더링)를 [Vercel](https://www.vercel.com/)에 배포할 수 있게 해줍니다.

Astro를 [정적 사이트 빌더](/ko/core-concepts/rendering-modes/#사전-렌더링)로 사용하는 경우에는 어댑터가 필요하지 않습니다.
...
그런 다음, `astro.config.*` 파일에 어댑터와 원하는 [요청 시 렌더링 모드](/ko/core-concepts/rendering-modes/#요청-시-렌더링)를 추가하세요.
...
<ReadMore>[Vercel에 프로젝트를 배포](/ko/guides/deploy/vercel/)하는 방법에 대해 자세히 알아보세요.</ReadMore>
...
Vercel 에지 미들웨어를 사용하여 요청을 가로채고 응답을 보내기 전에 리디렉션할 수 있습니다. Vercel 미들웨어는 에지, SSR, 정적 배포를 위해 실행할 수 있습니다. 미들웨어용으로 이 패키지를 설치할 필요가 없을 수도 있습니다. @vercel/edge는 위치정보와 같은 일부 미들웨어 기능을 사용하는 데에만 필요합니다. 자세한 정보는 [Vercel의 미들웨어 문서](https://vercel.com/docs/concepts/functions/edge-middleware)를 참조하세요.
