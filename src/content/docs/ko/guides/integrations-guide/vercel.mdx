...
기본적으로 Astro는 [미들웨어](/ko/guides/middleware/)를 ISR 함수에 번들링합니다. 따라서 미들웨어는 해당 함수가 실행될 때, 즉 캐시 누락 또는 재생성 시에만 실행되고 캐시 적중 시에는 실행되지 않습니다. 모든 요청에서 미들웨어를 실행해야 한다면 [`middlewareMode`를 `'edge'`로 설정](#vercel-edge-functions에서-astro-미들웨어-실행)하여 캐시 앞에서 배포하세요. 관련 라우트를 [`exclude`](#캐싱에서-경로-제외)에 추가하여 캐시가 적용되지 않도록 할 수도 있습니다.
...
이 기능이 활성화되면, 콘텐츠 보안 정책과 같은 Astro 기능에서 제공하는 경우 어댑터는 [Vercel의 `vercel.json` 파일에 정적 헤더](https://vercel.com/docs/project-configuration#headers)를 저장합니다.
...
`@astrojs/vercel` 어댑터는 코드 베이스의 Astro 미들웨어에서 [에지 함수](https://vercel.com/docs/functions/edge-functions)를 생성할 수 있습니다. `middlewareMode`가 `'edge'`로 설정되면 에지 함수는 [`isr`](#isr)로 캐시된 라우트를 포함하여 [주문형 렌더링 라우트](/ko/guides/on-demand-rendering/)에 대한 모든 요청에서 미들웨어 코드를 실행합니다. 정적 자산과 사전 렌더링된 페이지는 Vercel의 파일 시스템에서 제공되므로 미들웨어를 호출하지 않습니다.

기본적으로 ISR 함수에 번들링된 미들웨어는 캐시 누락 또는 재생성 시에만 실행됩니다. `middlewareMode: 'edge'`를 사용하면 미들웨어가 ISR 캐시 앞에서 실행되므로 캐시 적중 시에도 실행됩니다. 즉, 캐시된 라우트에 대한 모든 요청이 에지 함수 호출을 트리거합니다.
