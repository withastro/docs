...
`@astrojs/vercel` 어댑터는 코드 베이스의 Astro 미들웨어에서 [에지 함수](https://vercel.com/docs/functions/edge-functions)를 생성할 수 있습니다. [`middlewareMode`](/ko/reference/adapter-reference/#middlewaremode)가 `'edge'`로 설정되면 에지 함수는 정적 자산, 사전 렌더링된 페이지, 요청 시 렌더링되는 페이지를 포함한 모든 요청에 대해 미들웨어 코드를 실행합니다.
