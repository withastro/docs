...
Astro는 이 어댑터를 사용하여 [`hybrid` 또는 `server` 방식으로 렌더링된 사이트](/ko/guides/on-demand-rendering/)를 [Vercel](https://www.vercel.com/)에 배포할 수 있게 해줍니다.

Astro를 static 사이트 빌더로 사용하는 경우 추가 Vercel 서비스 (예: [Vercel Web Analytics](https://vercel.com/docs/analytics), [Vercel Image Optimization](https://vercel.com/docs/image-optimization))를 사용하는 경우에만 이 어댑터가 필요합니다. 그렇지 않으면 `static` 사이트를 배포하는 데 어댑터가 필요하지 않습니다.
