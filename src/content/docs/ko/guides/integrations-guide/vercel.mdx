...
Astro는 이 어댑터를 사용하여 [`hybrid` 또는 `server` 방식으로 렌더링된 사이트](/ko/basics/rendering-modes/#on-demand-rendered)를 [Vercel](https://www.vercel.com/)에 배포할 수 있게 해줍니다.

Astro를 [`static` 사이트 빌더](/ko/basics/rendering-modes/#pre-rendered)로 사용하는 경우 추가 Vercel 서비스 (예: [Vercel Web Analytics](https://vercel.com/docs/analytics), [Vercel Image Optimization](https://vercel.com/docs/image-optimization))를 사용하는 경우에만 이 어댑터가 필요합니다. 그렇지 않으면 `static` 사이트를 배포하는 데 어댑터가 필요하지 않습니다.

[Vercel 배포 가이드](/ko/guides/deploy/vercel/)에서 Astro 사이트를 배포하는 방법을 알아보세요.

## 왜 Astro Vercel인가? \{#why-astro-vercel\}
...
Astro는 공식 통합을 설정을 자동화하기 위한 `astro add` 명령을 포함하고 있습니다. 원한다면, [수동으로 통합 설치](#manual-install)할 수도 있습니다.
...
그런 다음, `astro.config.*` 파일에 어댑터와 원하는 [요청 시 렌더링 모드](/ko/basics/rendering-modes/#on-demand-rendered)를 추가하세요.
...
`domains` 및 `remotePatterns` 속성은 [Astro의 해당 `image` 설정](/ko/reference/configuration-reference/#image-options)을 통해 자동으로 채워집니다.
...
##### 캐싱에서 경로 제외 \{#excluding-paths-from-caching\}
...
### 함수 번들링 구성 \{#function-bundling-configuration\}
...
### Vercel Edge Functions에서 Astro 미들웨어 실행 \{#running-astro-middleware-on-vercel-edge-functions\}
...
### Node.js 버전 지원 \{#nodejs-version-support\}
