...
예를 들어, [실험적인 콘텐츠 보안 정책](/ko/reference/configuration-reference/#securitycsp)이 활성화된 경우, `<meta>` 요소를 생성하는 대신 `experimentalStaticHeaders`를 사용하여 CSP `headers`를 Netlify 구성에 추가할 수 있습니다.
