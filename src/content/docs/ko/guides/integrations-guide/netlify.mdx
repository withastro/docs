...
이 어댑터를 사용하면 Astro가 [`hybrid` 또는 `server` 렌더링된 사이트](/ko/core-concepts/rendering-modes/#on-demand-rendered)를 [Netlify](https://www.netlify.com/)에 배포할 수 있습니다.
...
그런 다음 어댑터와 원하는 [주문형 렌더링 모드](/ko/core-concepts/rendering-modes/#on-demand-rendered)를 `astro.config.*` 파일에 추가하세요.
