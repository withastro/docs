...
태그와 노드는 `.astro` 파일로 제한됩니다. Markdoc에 클라이언트 측 UI 컴포넌트를 포함하려면 원하는 `client:` 지시어로 [프레임워크 컴포넌트를 렌더링하는 래퍼 `.astro` 컴포넌트를 사용](/ko/core-concepts/framework-components/#nesting-framework-components)하세요.
...
프런트매터에 액세스하려면 콘텐츠를 렌더링하는 항목 `data` 속성을 [변수로](#pass-markdoc-variables) 전달할 수 있습니다.
