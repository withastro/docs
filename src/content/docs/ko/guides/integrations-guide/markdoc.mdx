...
### VS Code 편집기 통합 \{#vs-code-editor-integration\}
...
그런 다음 [콘텐츠 컬렉션 API](/ko/guides/content-collections/#querying-collections)를 사용하여 컬렉션을 쿼리하세요.
...
## Markdoc 변수 전달 \{#pass-markdoc-variables\}
...
### Markdoc 콘텐츠에서 프론트매터에 액세스 \{#access-frontmatter-from-your-markdoc-content\}
...
## 컴포넌트 렌더링 \{#render-components\}

`@astrojs/markdoc`은 Markdoc의 모든 기능을 사용하여 UI 컴포넌트를 콘텐츠에 연결하는 구성 옵션을 제공합니다.

### Astro 컴포넌트를 Markdoc 태그로 사용 \{#use-astro-components-as-markdoc-tags\}
...
### 클라이언트 측 UI 컴포넌트 사용 \{#use-client-side-ui-components\}

태그와 노드는 `.astro` 파일로 제한됩니다. Markdoc에 클라이언트 측 UI 컴포넌트를 포함하려면 원하는 `client:` 지시어와 함께 [프레임워크 컴포넌트를 렌더링하는 래퍼 `.astro` 컴포넌트를 사용](/ko/guides/framework-components/#nesting-framework-components)하세요.
...
### npm 패키지 및 TypeScript 파일의 Astro 컴포넌트 사용 \{#use-astro-components-from-npm-packages-and-typescript-files\}
...
## Markdoc Partials \{#markdoc-partials\}
...
<ReadMore>Prism 스타일시트 구성에 대해 알아보려면 [구문 강조 가이드를 참조](/ko/guides/markdown-content/#prism-configuration)하세요.</ReadMore>

## 사용자 정의 Markdoc 노드 / 요소 \{#custom-markdoc-nodes--elements\}
...
### 사용자 정의 제목 \{#custom-headings\}

`@astrojs/markdoc`는 제목에 앵커 링크를 자동으로 추가하고 [콘텐츠 컬렉션 API를 통해 `headings` 목록을 생성](/ko/guides/content-collections/#rendering-content-to-html)합니다. 제목을 렌더링하는 방법을 추가로 사용자 정의하려면 Astro 컴포넌트를 [Markdoc 노드로][markdoc-nodes] 적용하세요.
...
* `id: string` 제목의 텍스트 콘텐츠에서 생성된 `id`입니다. 이는 [콘텐츠 `render()` 함수](/ko/guides/content-collections/#rendering-content-to-html)에 의해 생성된 `slug`에 해당합니다.

예를 들어 `### Level 3 heading!`라는 제목은 `level: 3` 및 `id: 'level-3-heading'`을 컴포넌트 props로 전달합니다.

### 사용자 정의 이미지 컴포넌트 \{#custom-image-components\}
...
#### Markdoc의 기본 이미지 노드 재정의 \{#override-markdocs-default-image-node\}
...
#### 사용자 정의 Markdoc 이미지 태그 생성 \{#create-a-custom-markdoc-image-tag\}
...
### 루트 HTML 요소 설정 \{#advanced-markdoc-configuration\}
