...
이 어댑터를 사용하면 Astro가 [`hybrid` 또는 `server` 렌더링 사이트](/ko/basics/rendering-modes/#온디맨드-렌더링)를 Node 대상에 배포할 수 있습니다.
...
standalone 모드의 경우 서버는 페이지 및 API 라우트 외에도 파일 제공을 처리합니다.
