...
이 어댑터를 사용하면 Astro가 [`hybrid` 또는 `server` 렌더링 사이트](/ko/basics/rendering-modes/#on-demand-rendered)를 Node 대상에 배포할 수 있습니다.

Astro를 [정적 사이트 빌더](/ko/basics/rendering-modes/#pre-rendered)로 사용하는 경우에는 어댑터가 필요하지 않습니다.

## 왜 Astro Node.js인가? \{#why-astro-nodejs\}
...
먼저 [빌드를 수행](/ko/guides/deploy/#building-your-site-locally)합니다. 선택한 모드 (위 참조)에 따라 아래의 적절한 단계를 따르세요.
...
#### 사용자 정의 호스트 및 포트 \{#custom-host-and-port\}
