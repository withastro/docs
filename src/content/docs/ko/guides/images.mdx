...
`getImage()` 함수는 HTML이 아닌 다른 곳 (예: [API 라우트](/ko/guides/endpoints/#서버-엔드포인트-api-라우트))에서 사용할 이미지를 생성하기 위한 것입니다. 현재 `<Picture>` 및 `<Image>` 컴포넌트가 지원하지 않는 옵션이 필요한 경우, `getImage()` 함수를 사용하여 사용자 정의 `<Image />` 컴포넌트를 생성할 수 있습니다.
