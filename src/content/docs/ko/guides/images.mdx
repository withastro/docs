...
`<Image />` 또는 `<Picture />` 컴포넌트에 [layout 속성](/ko/reference/modules/astro-assets/#layout)을 적용하면 Astro는 이미지에 필요한 `srcset` 및 `sizes` 값을 생성하고 필요한 [스타일을 적용하여 이미지 크기가 올바르게 조정](#반응형-이미지-스타일)되도록 합니다.
...
또는 CDN에서 제공하는 자체 SDK를 사용해 Astro 프로젝트에 보다 쉽게 통합할 수도 있습니다. 예를 들면 다음과 같습니다.

- Cloudinary는 `CldImage` 컴포넌트를 사용해 이미지를 쉽게 삽입할 수 있는 [Astro SDK](https://astro.cloudinary.dev/)를 제공하며, Node.js 환경에서 `<img>` 태그와 함께 사용할 URL을 생성할 수 있는 [Node.js SDK](https://cloudinary.com/documentation/node_integration)도 제공합니다.
- ImageKit은 이미지 서비스를 등록하는 [Astro 통합](https://github.com/imagekit-developer/imagekit-astro)을 제공합니다. 이를 사용하면 Astro의 내장 `<Image />` 및 `<Picture />` 컴포넌트는 물론 Markdown의 `![]()` 및 MDX 이미지까지 모두 자동으로 ImageKit을 통해 처리됩니다. 기존 이미지 구문을 변경하지 않고도 CDN 전송, 자동 AVIF/WebP 변환, 반응형 `srcset`, 그리고 요청 시 이미지 변환 기능을 활용할 수 있습니다.
...
[어댑터](https://astro.build/integrations/?search=&categories%5B%5D=adapters)가 Astro의 내장 Sharp 이미지 최적화를 지원하지 않는 경우 (예: Cloudflare) `<Image />` 및 `<Picture />` 컴포넌트를 사용할 수 있도록 무작동 이미지 서비스를 구성할 수 있습니다. Astro는 이러한 환경에서 이미지 변환 및 처리를 수행하지 않습니다. 그러나 CLS (Cumulative Layout Shift) 없음, 강제된 `alt` 속성 및 일관된 작성 환경을 포함하여 `astro:assets` 사용의 다른 이점을 계속 누릴 수 있습니다.
