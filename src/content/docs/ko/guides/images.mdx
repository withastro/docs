...
그러나 `<Image />`에서 생성된 정적 콘텐츠를 `.astro` 파일 내 프레임워크 컴포넌트에 하위 항목으로 전달하거나 [명명된 `<slot/>`](/ko/basics/framework-components/#프레임워크-컴포넌트-중-astro-컴포넌트를-사용할-수-있나요)을 사용하여 전달할 수 있습니다.
