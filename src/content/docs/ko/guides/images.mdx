...
Astro는 API를 사용하여 원격으로 데이터를 가져오거나 전체 URL 경로를 이용하여 이미지를 표시할 수 있습니다.
 ...
 외부 소스를 다룰 때 추가적인 보호를 위해 Astro의 이미지 컴포넌트와 도우미 함수는 [구성에서 지정한 승인된 이미지 소스](#authorizing-remote-images)의 이미지만 처리 (예: 최적화, 변환)합니다. 다른 소스의 원격 이미지는 처리하지 않고 표시됩니다.
...
최적화된 이미지를 위해 Astro의 기본 [`<Image />`](#display-optimized-images-with-the-image--component) 및 [`<Picture />`](#create-responsive-images-with-the-picture--component) 컴포넌트를 가져와 사용합니다. Astro 구문에서는 이미지 처리를 생략하기 위해 [HTML `<img>` 태그](#display-unprocessed-images-with-the-html-img-tag)를 직접 작성할 수도 있습니다.
...
- `src/` 폴더에 있는 로컬 이미지
- 인증된 소스에서 가져오는 [구성된 원격 이미지](#authorizing-remote-images)
...
`<Image />` 컴포넌트는 HTML `<img>` 태그가 허용하는 모든 속성뿐만 아니라 [여러 컴포넌트 속성](/ko/reference/modules/astro-assets/#image-properties)을 허용합니다.
...
<ReadMore>[`<Picture />` 컴포넌트의 속성](/ko/reference/modules/astro-assets/#picture-properties)에 대한 자세한 내용은 `astro:assets` 참조에서 알아보세요.</ReadMore>

### HTML `<img>` 태그를 사용하여 처리되지 않은 이미지 표시

[Astro 템플릿 구문](/ko/basics/astro-syntax/)은 최종 출력에 대한 완전한 제어를 위해 `<img>` 태그를 직접 작성하는 것도 지원합니다. 이러한 이미지는 처리 및 최적화되지 않습니다. 모든 HTML `<img>` 태그 속성을 허용하며, 필수 속성은 `src`뿐입니다.
...
이미지 CDN은 [모든 Astro 이미지 옵션](#images-in-astro-files)에서 작동합니다. 이미지의 전체 URL을 `<Image />` 컴포넌트, `<img>` 태그 또는 Markdown 표기법에서 `src` 속성으로 사용하세요. 원격 이미지로 이미지를 최적화하려면 [승인된 도메인 또는 URL 패턴을 구성](#원격-이미지-승인)하세요.
...
이미지 속성에 대한 더 많은 제어가 필요한 경우, [Astro의 MDX 통합](/ko/guides/integrations-guide/mdx/)을 사용하여 `.mdx` 파일 형식에 대한 지원을 추가하는 것이 좋습니다. MDX를 사용하면 Markdown에 컴포넌트를 추가할 수 있으며, [MDX에서 사용할 수 있는 추가 이미지 옵션](#images-in-mdx-files)도 제공됩니다.

## MDX 파일의 이미지

컴포넌트와 이미지를 모두 가져와 `.mdx` 파일에서 Astro의 `<Image />` 및 `<Picture />` 컴포넌트를 사용할 수 있습니다. [`.astro` 파일에서 사용되는 것](#astro-파일의-이미지)처럼 사용하세요. JSX `<img />` 태그는 처리되지 않은 이미지에도 지원되며 [HTML `<img>` 태그와 동일한 이미지 가져오기를 사용](#display-unprocessed-images-with-the-html-img-tag)합니다.
...
그러나 `<Image />`에서 생성된 정적 콘텐츠를 `.astro` 파일의 프레임워크 컴포넌트에 자식으로 전달하거나 [명명된 `<slot/>`](/ko/guides/framework-components/#can-i-use-astro-components-inside-my-framework-components)]을 사용하여 전달할 수 있습니다:
...
`src`와 같은 이미지 속성에 액세스하려면 [로컬 이미지를 먼저 가져와야 합니다](#display-unprocessed-images-with-the-html-img-tag).
