...
<ReadMore>Astro의 [파일 기반 라우팅](/ko/guides/routing/) 또는 [동적 경로](/ko/guides/routing/#dynamic-routes) 생성 옵션에 대해 자세히 알아보세요.</ReadMore>

## Markdown 기능 \{#markdown-features\}

Astro는 Markdown 및 MDX 파일을 사용할 때 사용할 수 있는 몇 가지 추가 내장 Markdown 기능을 제공합니다.

### 프런트매터 `layout` \{#frontmatter-layout\}

Astro는 Astro [레이아웃 컴포넌트](/ko/basics/layouts/#markdown-layouts)에 대한 상대 경로 (또는 [별칭](/ko/guides/imports/#aliases))를 지정할 수 있는 특별한 프런트매터 `layout` 속성이 있는 [Markdown 및 MDX 페이지](/ko/basics/astro-pages/#markdownmdx-pages) (`src/pages/`에 위치하는)를 제공합니다.
...
그런 다음 `Astro.props`를 통해 [레이아웃 컴포넌트에 특정 속성을 사용할 수 있습니다](/ko/basics/layouts/#markdown-layout-props). 예를 들어 `Astro.props.frontmatter`를 통해 frontmatter 속성에 액세스할 수 있습니다.
...
레이아웃 컴포넌트에서 [Markdown 스타일을 지정](/ko/guides/styling/#markdown-styling)할 수도 있습니다.

<ReadMore>[Markdown 레이아웃](/ko/basics/layouts/#markdown-layouts)에 대해 자세히 알아보세요.</ReadMore>
...
### 특수 문자 이스케이프 \{#escaping-special-characters\}
...
## MDX 전용 기능 \{#mdx-only-features\}

Astro [MDX 통합](/ko/guides/integrations-guide/mdx/)을 추가하면 JSX 변수, 표현식, 컴포넌트를 사용하여 Markdown 작성이 향상됩니다.

또한 [MDX의 Markdown 스타일 프런트매터](https://mdxjs.com/guides/frontmatter/)에 대한 지원을 포함하여 표준 MDX에 추가 기능을 추가합니다. 이를 통해 [프런트매터 `layout`](#frontmatter-layout) 속성과 같은 Astro의 내장 Markdown 기능 대부분을 사용할 수 있습니다.

`.mdx` 파일은 Astro의 HTML과 같은 구문이 아닌 [MDX 구문](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)으로 작성되어야 합니다.

### MDX에서 내보낸 변수 사용 \{#using-exported-variables-in-mdx\}

MDX는 `export` 구문을 사용하여 MDX 콘텐츠에 변수를 추가할 수 있도록 지원합니다. 이러한 변수는 템플릿 자체에서 액세스할 수 있을 뿐만 아니라 [파일을 다른 곳으로 가져올 때](#importing-markdown) 명명된 속성으로 액세스할 수 있습니다.
...
### MDX에서 프런트매터 변수 사용 \{#using-frontmatter-variables-in-mdx\}

Astro MDX 통합에는 기본적으로 MDX에서 프런트매터 사용에 대한 지원이 포함되어 있습니다. Markdown 파일에서와 마찬가지로 프런트매터 속성을 추가하면 이러한 변수에 액세스하여 템플릿, [`layout` 컴포넌트](#frontmatter-layout), [파일을 다른 곳으로 가져올 때](#importing-markdown) 명명된 속성으로 사용할 수 있습니다.
...
### MDX에서 컴포넌트 사용 \{#using-components-in-mdx\}

MDX 통합을 설치한 후 다른 Astro 컴포넌트에서 사용하는 것처럼 [Astro 컴포넌트](/ko/basics/astro-components/#component-props)와 [UI 프레임워크 컴포넌트](/ko/guides/framework-components/#using-framework-components)를 MDX (`.mdx`) 파일로 가져와 사용할 수 있습니다.
...
#### HTML 요소에 맞춤 컴포넌트 할당 \{#assigning-custom-components-to-html-elements\}
...
Astro 컴포넌트에서 Markdown 및 MDX 파일을 가져오면 해당 [내보낸 속성](#exported-properties)이 포함된 객체를 얻게 됩니다.
...
### 내보낸 속성 \{#exported-properties\}

:::note[Astro 레이아웃을 사용하시나요?]
Astro의 특수한 [프런트매터 layout](#frontmatter-layout)을 사용할 때 [Astro 레이아웃 컴포넌트로 내보낸 속성](/ko/basics/layouts/#markdown-layout-props)을 확인하세요.
:::
...
### `Content` 컴포넌트 \{#the-content-component\}
...
#### 예: 동적 페이지 라우팅 \{#example-dynamic-page-routing\}

페이지 경로를 생성하기 위해 Markdown/MDX 파일을 `src/pages/` 디렉터리에 두는 대신 [페이지를 동적으로 생성](/ko/guides/routing/#dynamic-routes)할 수 있습니다.
...
### 가져온 MDX가 있는 사용자 정의 컴포넌트 \{#custom-components-with-imported-mdx\}

가져온 MDX 콘텐츠를 렌더링할 때 [사용자 정의 컴포넌트](#assigning-custom-components-to-html-elements)는 `components` prop을 통해 전달될 수 있습니다.
...
## Markdown 및 MDX 구성 \{#configuring-markdown-and-mdx\}
...
### Markdown 플러그인 \{#markdown-plugins\}

Astro는 Markdown 및 MDX에 대한 타사 [remark](https://github.com/remarkjs/remark) 및 [rehype](https://github.com/rehypejs/rehype) 플러그인 추가를 지원합니다. 이러한 플러그인을 사용하면 [목차 자동 생성](https://github.com/remarkjs/remark-toc), [접근 가능한 이모티콘 라벨 적용](https://github.com/florianeckerstorfer/remark-a11y-emoji), [Markdown 스타일 지정](/ko/guides/styling/#markdown-styling)과 같은 새로운 기능으로 Markdown을 확장할 수 있습니다.
...
#### 제목 ID 및 플러그인 \{#heading-ids-and-plugins\}

Astro는 Markdown 및 MDX 파일의 모든 제목 요소 (`<h1>` ~ `<h6>`)에 `id` 속성을 삽입하고 [Markdown 내보낸 속성](#exported-properties)에서 이러한 ID를 검색하기 위한 `getHeadings()` 유틸리티를 제공합니다.
...
#### 플러그인 맞춤설정 \{#customizing-a-plugin\}
...
### 프로그래밍 방식으로 프런트매터 수정하기 \{#modifying-frontmatter-programmatically\}

:::note
[콘텐츠 컬렉션](/ko/guides/content-collections/)을 사용하는 경우, ["프런트매터를 Remark로 수정하기"](/ko/guides/content-collections/#modifying-frontmatter-with-remark)을 참조하세요.
:::

[remark 또는 rehype 플러그인](#markdown-plugins)을 사용하여 모든 Markdown 및 MDX 파일에 프런트매터 속성을 추가할 수 있습니다.
...
이제 모든 Markdown 또는 MDX 파일의 프런트매터에 `customProperty`가 있으므로 [Markdown을 가져올 때](#importing-markdown) [레이아웃의 `Astro.props.frontmatter` 속성](#frontmatter-layout)에서 사용할 수 있습니다.

<RecipeLinks slugs={["ko/recipes/reading-time"]} />

### MDX에서 Markdown 구성 확장 \{#extending-markdown-config-from-mdx\}
...
#### 나만의 테마 추가하기 \{#adding-your-own-theme\}
...
#### 기본 구문 강조 모드 변경 \{#change-default-syntax-highlighting-mode\}
...
## 원격 마크다운 가져오기 \{#fetching-remote-markdown\}
