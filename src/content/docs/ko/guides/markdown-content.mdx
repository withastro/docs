...
[Markdown 프로세서](#markdown-프로세서-선택하기) 플러그인(예: `rehype-slug`)을 사용해 `id` 속성을 삽입함으로써 이러한 제목 ID를 사용자 지정할 수 있습니다. 사용자 지정 ID는 Astro의 기본 ID 대신 HTML 출력과 `getHeadings()`가 반환하는 항목에 반영됩니다.
...
Astro는 설정 가능한 **Markdown 프로세서**를 사용해 Markdown을 렌더링합니다. 기본적으로는 활발한 플러그인 생태계를 갖춘 [remark](https://remark.js.org/) 및 [rehype](https://github.com/rehypejs/rehype)의 [unified](https://unifiedjs.com/) 파이프라인을 사용합니다.

Astro는 [GitHub-Flavored Markdown](https://github.github.com/gfm/)과 [SmartyPants](https://daringfireball.net/projects/smartypants/)를 자동으로 적용합니다. 이를 통해 텍스트에서 클릭 가능한 링크를 생성하고, 인용 부호 및 em dash(긴 대시)의 서식을 지정하는 등의 편리한 기능을 사용할 수 있습니다.

플러그인을 사용해 Markdown 처리를 확장하거나, 추가 파서 기능을 활성화하거나, [완전히 다른 프로세서로 전환](#sätteri-프로세서로-전환하기)할 수 있습니다. 전체 [Markdown 구성 옵션](/ko/reference/configuration-reference/#markdown-옵션) 목록도 확인해 보세요.

### Markdown 프로세서 선택하기

[`markdown.processor` 옵션](/ko/reference/configuration-reference/#markdownprocessor)은 `.md` 및 `.mdx` 파일을 렌더링할 엔진을 제어합니다. Astro는 두 가지 공식 옵션을 제공합니다.

- **`unified()`** (기본값): 풍부한 플러그인 생태계를 갖춘 [remark](https://remark.js.org/) 및 [rehype](https://github.com/rehypejs/rehype) 파이프라인입니다.
- **`satteri()`**: 자체 플러그인 모델을 갖춘 [Sätteri](https://satteri.bruits.org/) 파이프라인입니다. 더 빠른 Markdown 및 MDX 컴파일러로, 별도로 설치해야 하는 `@astrojs/markdown-satteri` 패키지를 통해 제공됩니다.

### remark 및 rehype 플러그인 사용하기

기본 `unified()` 프로세서는 서드파티 [remark](https://github.com/remarkjs/remark) 및 [rehype](https://github.com/rehypejs/rehype) 플러그인을 지원합니다. 이러한 플러그인을 사용하면 [목차 자동 생성](https://github.com/remarkjs/remark-toc), [접근 가능한 이모지 레이블 적용](https://github.com/florianeckerstorfer/remark-a11y-emoji), [Markdown 스타일 적용](/ko/guides/styling/#markdown-스타일링)과 같은 새로운 기능으로 Markdown을 확장할 수 있습니다.
...
`@astrojs/markdown-remark`에서 `unified`를 가져온 뒤, `markdown.processor`를 통해 플러그인을 전달하세요. 이 예시에서는 Markdown 파일에 [`remark-toc`](https://github.com/remarkjs/remark-toc) 및 [`rehype-accessible-emojis`](https://www.npmjs.com/package/rehype-accessible-emojis)를 적용합니다.
...
remark 및 rehype 대신 [Sätteri](https://satteri.bruits.org/)를 사용하려면 `@astrojs/markdown-satteri`를 설치한 다음 `satteri`를 가져와 `markdown.processor`에 전달하세요:
...
Sätteri 프로세서는 `mdastPlugins` 및 `hastPlugins`를 통해 자체 플러그인을 사용할 수 있으며, `features`를 통해 선택적 파서 기능을 활성화하거나 비활성화할 수 있습니다. 사용 가능한 플러그인과 기능에 대해서는 [Sätteri 문서](https://satteri.bruits.org/docs/)를 참조하세요.

프로세서를 전환하면 `.md` 및 `.mdx` 파일 모두에서 remark 및 rehype 대신 Sätteri가 사용됩니다. 구성에 포함된 remark 또는 rehype 플러그인은 적용되지 않습니다. Sätteri를 `.mdx` 파일에만 사용하려면 [MDX 통합의 `processor` 옵션](/ko/guides/integrations-guide/mdx/#processor)을 설정하세요.
...
Astro의 MDX 통합은 기본적으로 [프로젝트의 기존 Markdown 구성](/ko/reference/configuration-reference/#markdown-옵션)을 확장하며, 여기에는 [Markdown 프로세서](#markdown-프로세서-선택하기)도 포함됩니다. 개별 옵션을 재정의하려면 MDX 구성에서 해당 옵션을 지정하면 됩니다.
