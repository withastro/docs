...
<ReadMore>Astro의 [파일 기반 라우팅](/ko/basics/routing/) 또는 [동적 경로](/ko/basics/routing/#동적-경로) 생성 옵션에 대해 자세히 알아보세요.</ReadMore>
...
MDX 통합을 설치한 후 다른 Astro 컴포넌트에서 사용하는 것처럼 [Astro 컴포넌트](/ko/basics/astro-components/#컴포넌트-props)와 [UI 프레임워크 컴포넌트](/ko/basics/framework-components/#프레임워크-컴포넌트-사용하기)를 MDX (`.mdx`) 파일로 가져와 사용할 수 있습니다.
...
페이지 경로를 생성하기 위해 Markdown/MDX 파일을 `src/pages/` 디렉터리에 두는 대신 [페이지를 동적으로 생성](/ko/basics/routing/#동적-경로)할 수 있습니다.
...
export default defineConfig({
  markdown: {
    shikiConfig: {
      ...
      wrap: true,
      // 맞춤 transformers 추가: https://shikiji.netlify.app/guide/transformers
      // 일반 transformers 찾기: https://shikiji.netlify.app/packages/transformers
      transformers: [],
    },
  },
});
