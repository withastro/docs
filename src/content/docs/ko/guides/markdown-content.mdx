...
Astro는 구성 가능한 **Markdown 프로세서**를 사용하여 Markdown을 렌더링합니다. 기본적으로 Astro에 포함된 네이티브 Markdown 및 MDX 파이프라인인 [Sätteri](https://satteri.bruits.org/)를 사용합니다.
...
* **`satteri()`** (기본값): 자체 플러그인 모델을 갖춘 빠른 Markdown 및 MDX 컴파일러인 네이티브 [Sätteri](https://satteri.bruits.org/) 파이프라인입니다. Astro에 포함되어 있으므로 별도의 설치가 필요하지 않습니다.
* **`unified()`**: 풍부한 플러그인 생태계를 갖춘 [remark](https://remark.js.org/) 및 [rehype](https://github.com/rehypejs/rehype) 파이프라인입니다. 별도로 설치해야 하는 `@astrojs/markdown-remark` 패키지에서 제공합니다.
...
`@astrojs/markdown-satteri`에서 `satteri`를 가져온 다음 `markdown.processor`를 통해 옵션을 전달하세요. 다음 예시는 mdast 플러그인을 추가하고 `directive` 파서 기능을 활성화하는 방법을 보여줍니다.
...
Sätteri 대신 [remark 및 rehype](https://unifiedjs.com/)를 사용하려면 `@astrojs/markdown-remark`를 설치한 후 `unified`를 가져와 `markdown.processor`에 전달하세요.
