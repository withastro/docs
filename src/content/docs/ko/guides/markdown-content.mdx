...
- 방대한 [remark](https://remark.js.org/) 또는 [rehype](https://github.com/rehypejs/rehype) 플러그인 생태계를 활용하려는 경우, 또는 [MDX 파일에서 recma 플러그인](/ko/guides/integrations-guide/mdx/#recma-플러그인-사용하기)이 필요한 경우
    ...
    <MarkdownProcessorTabs>
      ...
      export function exampleRemarkPlugin() {
        return function (tree, file) {
          file.data.astro.frontmatter.newProperty = "새 속성";
          // `title`이 필수 프런트매터 속성이라고 가정
          file.data.astro.frontmatter.computedProperty = `${file.data.astro.frontmatter.title} | 내 사이트 제목`;
        };
      }
      ```
      </Fragment>
