...
[Astro가 관리하는 공식 어댑터 통합](/ko/guides/integrations/#official-integrations)은 다음 `astro add` 명령으로 추가할 수 있습니다. 이 명령은 어댑터를 설치하고 `astro.config.mjs` 파일에 적절한 변경사항을 한 번에 적용합니다.
...
[NPM 패키지를 설치하여 어댑터를 수동으로 추가할 수도 있습니다](/ko/guides/integrations/#installing-an-npm-package)(예: `@astrojs/netlify`). 그리고 `astro.config.mjs`를 직접 업데이트합니다.
