...
Astro 통합 설치 및 구성에 대한 자세한 내용은 [통합 안내서](/ko/guides/integrations/)를 참조하세요.
...
[`client:*` 지시어를 사용하여](/ko/reference/directives-reference/#클라이언트-지시어) 대화형 프레임워크 컴포넌트에 전달되는 Props는 [직렬화](https://developer.mozilla.org/ko/docs/Glossary/Serialization)되어야 합니다. 즉, 네트워크를 통해 전송하거나 저장하기에 적합한 형식으로 변환해야 합니다. 그러나 Astro는 모든 유형의 데이터 구조를 직렬화하지는 않습니다. 따라서 하이드레이션된 컴포넌트에 props로 전달할 수 있는 항목에는 제한이 있습니다.

지원되는 prop 유형:
일반 객체, `number`, `string`, `Array`, `Map`, `Set`, `RegExp`, `Date`, `BigInt`, `URL`, `Uint8Array`, `Uint16Array`, `Uint32Array`, `Infinity`

함수와 같이 지원되지 않는 데이터 구조가 컴포넌트로 전달되면 컴포넌트가 서버에서 렌더링되는 동안 사용할 수 있으며, 상호작용은 제공하지 못합니다. 예를 들어, 하이드레이션된 컴포넌트에 함수를 전달하는 것은 지원되지 않는데, Astro는 서버에서 함수를 클라이언트에서 실행 가능한 방식으로 전달할 수 없기 때문입니다.
