...
## 공식 UI 프레임워크 통합 \{#official-ui-framework-integrations\}
...
## 프레임워크 컴포넌트 사용 \{#using-framework-components\}
...
## 인터랙티브 컴포넌트 하이드레이션 \{#hydrating-interactive-components\}

프레임워크 컴포넌트는 [`client:*` 지시어](/ko/reference/directives-reference/#client-directives)를 이용하여 대화형으로 만들 수 있습니다 (하이드레이션). 이는 컴포넌트의 자바스크립트가 브라우저로 전송되는 시기를 결정하는 컴포넌트 속성입니다.
...
### 사용 가능한 하이드레이션 지시어 \{#available-hydration-directives\}

UI 프레임워크 컴포넌트에서 사용할 수 있는 하이드레이션 지시어는 `client:load`, `client:idle`, `client:visible`, `client:media={QUERY}`, `client:only={FRAMEWORK}`입니다.

<ReadMore>하이드레이션 지시어에 대한 자세한 설명과 사용법은 [지시어 참조](/ko/reference/directives-reference/#client-directives) 페이지를 참조하세요.</ReadMore>

## 프레임워크 혼합 \{#mixing-frameworks\}
...
## 프레임워크 컴포넌트에 Props 전달 \{#passing-props-to-framework-components\}
...
## 프레임워크 컴포넌트에 자식 요소 전달 \{#passing-children-to-framework-components\}
...
또한, [명명된 슬롯](/ko/basics/astro-components/#named-slots)을 사용하여 특정 하위 요소를 그룹화할 수 있습니다.
...
## 프레임워크 컴포넌트 중첩 \{#nesting-framework-components\}
...
## 프레임워크 컴포넌트에서 Astro 컴포넌트를 사용할 수 있나요? \{#can-i-use-astro-components-inside-my-framework-components\}
...
하지만 [Astro의 `<slot />` 패턴](/ko/basics/astro-components/#slots)을 사용하여 Astro 컴포넌트가 생성한 정적 콘텐츠를 **.astro 컴포넌트에서** 프레임워크 컴포넌트에 자식 요소로 전달할 수 있습니다.
...
## Astro 컴포넌트를 하이드레이션 할 수 있나요? \{#can-i-hydrate-astro-components\}
