...
Astro는 TypeScript를 통해 컴포넌트 props 타입을 지원합니다. 활성화하려면 컴포넌트 프론트매터에 TypeScript `Props` 인터페이스를 추가하세요. `export`를 사용할 수 있지만 반드시 필요한 것은 아닙니다. [Astro VSCode 확장](/ko/editor-setup/)은 템플릿에서 컴포넌트를 사용할 때 스스로 `Props` 인터페이스를 찾아 적절한 TypeScript 지원을 제공합니다.
...
Astro에는 일반적인 props 타입 패턴을 위한 몇 가지 내장 유틸리티 타입이 함께 제공됩니다. 이는 `astro/types` 패키지에서 가져와 사용할 수 있습니다.
...
Astro에는 다양한 HTML 요소로 렌더링될 수 있는 완전한 타입 안정성을 갖춘 컴포넌트를 보다 쉽게 ​​만들 수 있도록 도와주는 도우미가 포함되어 있습니다. 이는 전달된 props에 따라 `<a>` 또는 `<button>`으로 렌더링될 수 있는 `<Link>`와 같은 컴포넌트에 유용합니다.
