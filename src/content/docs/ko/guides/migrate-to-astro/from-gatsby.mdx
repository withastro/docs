...
## Gatsby와 Astro의 주요 유사점 \{#key-similarities-between-gatsby-and-astro\}

Gatsby와 Astro는 프로젝트 마이그레이션에 도움이 되는 몇 가지 유사점을 공유합니다.

- [`.astro` 파일의 구문은 JSX와 유사합니다](/ko/basics/astro-syntax/#jsx-like-expressions). Astro를 쓰는 것은 친숙하게 느껴질 것입니다.
...
- Astro는 React 라이브러리를 포함하여 [NPM 패키지 설치](/ko/guides/imports/#npm-packages)를 지원합니다. 기존 종속성 중 상당수가 Astro에서 작동합니다.

- Gatsby와 마찬가지로 Astro 프로젝트는 SSG 또는 [페이지 수준 사전 렌더링이 포함된 SSR](/ko/guides/server-side-rendering/)일 수 있습니다.

## Gatsby와 Astro의 주요 차이점 \{#key-differences-between-gatsby-and-astro\}
...
- [로컬 파일 데이터](/ko/guides/imports/): Gatsby는 GraphQL을 사용하여 프로젝트 파일에서 데이터를 검색합니다. Astro는 프로젝트 파일에서 데이터를 가져오기 위해 ESM imports 및 최상위 await 함수를 사용합니다 (예: [`Astro.glob()`](/ko/guides/imports/#astroglob), [`getCollection()`](/ko/guides/content-collections/#querying-collections)). Astro 프로젝트에 GraphQL을 수동으로 추가할 수 있지만 기본적으로 포함되어 있지는 않습니다.

## Gatsby 프로젝트 전환 \{#convert-your-gatsby-project\}

각 프로젝트 마이그레이션은 다르게 보이지만 Gatsby에서 Astro로 변환할 때 수행하게 되는 몇 가지 일반적인 작업이 있습니다.

### 새 Astro 프로젝트 만들기 \{#create-a-new-astro-project\}
...
### 통합 설치 (선택사항) \{#install-integrations-optional\}
...
### `src`에 코드 추가 \{#put-your-code-in-src\}
...
### 팁: JSX 파일을 `.astro` 파일로 변환 \{#tips-convert-jsx-files-to-astro-files\}

다음은 Gatsby `.js` 컴포넌트를 `.astro` 컴포넌트로 변환하기 위한 몇 가지 팁입니다.

1. 기존 Gatsby 컴포넌트 함수의 `return()`만 HTML 템플릿으로 사용하세요.

2. [Gatsby 또는 JSX 구문을 Astro 구문으로](#reference-convert-to-astro-syntax) 또는 HTML 웹 표준으로 변경합니다. 예를 들어 여기에는 `<Link to="">`, `{children}`, `className`이 포함됩니다.

3. import 구문을 포함하여 필요한 JavaScript를 ["코드 펜스" (`---`)](/ko/basics/astro-components/#the-component-script)로 이동합니다. 참고: [조건부 콘텐츠 렌더링](/ko/basics/astro-syntax/#dynamic-html)을 위한 JavaScript는 Astro에서 직접 HTML 템플릿 내에 작성되는 경우가 많습니다.
...
[단계별 변환된 Gatsby 블로그 시작 템플릿의 예시](#guided-example-gatsby-layout-to-astro)를 참조하세요.

#### 비교: `.jsx`와 `.astro` \{#compare-jsx-vs-astro\}
...
### Layout 파일 마이그레이션 \{#migrating-layout-files\}

Gatsby 레이아웃과 템플릿을 [Astro 레이아웃 컴포넌트](/ko/basics/layouts/)로 변환하여 시작하는 것이 도움이 될 수 있습니다.

각 Astro 페이지에는 `<html>`, `<head>`, `<body>` 태그가 명시적으로 필요하므로 페이지 전체에서 레이아웃 파일을 재사용하는 것이 일반적입니다. Astro는 페이지 콘텐츠에 대해 React의 `{children}` prop 대신 [`<slot />`](/ko/basics/astro-components/#slots)을 사용하며 import 구문은 필요하지 않습니다. Gatsby의 `layout.js` 및 템플릿에는 이러한 항목이 포함되지 않습니다.
...
### Pages 및 Posts 마이그레이션 \{#migrating-pages-and-posts\}
...
기존 Gatsby JSX (`.js`) 페이지를 [JSX 파일에서 `.astro` 페이지로 변환](#tips-convert-jsx-files-to-astro-files)해야 합니다. Astro에서는 기존 JSX 페이지 파일을 사용할 수 없습니다.

이러한 [`.astro` 페이지](/ko/basics/astro-pages/)는 `src/pages/` 내에 있어야 하며 파일 경로에 따라 페이지 경로가 자동으로 생성됩니다.

#### Markdown 및 MDX 페이지 \{#markdown-and-mdx-pages\}

Astro에는 Markdown에 대한 기본 지원과 MDX 파일에 대한 선택적 통합이 있습니다. 기존 [Markdown 및 MDX 파일](/ko/guides/markdown-content/)을 재사용할 수 있지만 [Astro의 특수 `layout` frontmatter 속성](/ko/basics/layouts/#markdown-layouts)을 추가하는 등 프런트매터에 일부 조정이 필요할 수 있습니다. 자동 파일 기반 라우팅을 활용하기 위해 `src/pages/` 내에 배치할 수도 있습니다.

또는 Astro에서 [콘텐츠 컬렉션](/ko/guides/content-collections/)을 사용하여 콘텐츠를 저장하고 관리할 수 있습니다. 컬렉션의 일부인 경우 Markdown 및 MDX 파일은 `src/content/` 내 폴더에 저장됩니다. 콘텐츠를 직접 검색하고 [해당 페이지를 동적으로 생성](/ko/guides/content-collections/#generating-routes-from-content)합니다.
...
### 구성 파일 용도 변경 \{#repurpose-config-files\}
...
- `gatsby-browser.js`: 여기에 사용된 모든 항목을 [기본 레이아웃](#migrating-layout-files)의 `<head>` 태그에 직접 추가하는 것을 고려해 보세요.
...
## 참조: Astro 구문으로 변환 \{#reference-convert-to-astro-syntax\}

다음은 Astro로 변환해야 하는 Gatsby 관련 구문의 몇 가지 예시입니다. Astro 컴포넌트 작성 가이드에서 [Astro와 JSX의 차이점](/ko/basics/astro-syntax/#differences-between-astro-and-jsx)을 자세히 알아보세요.

### Gatsby Links 변환 \{#gatsby-links-to-astro\}
...
### Gatsby Imports 변환 \{#gatsby-imports-to-astro\}

필요한 경우 상대 파일 경로를 정확하게 참조하도록 [파일 가져오기](/ko/guides/imports/)를 업데이트하세요. 이는 [별칭 가져오기](/ko/guides/typescript/#import-aliases)를 사용하거나 상대 경로 전체를 작성하여 수행할 수 있습니다.
...
### Gatsby Children Props 변환 \{#gatsby-children-props-to-astro\}
...
여러 children 집합을 전달하는 React 컴포넌트는 [명명된 슬롯](/ko/basics/astro-components/#named-slots)을 사용하여 Astro 컴포넌트로 마이그레이션될 수 있습니다.

[Astro의 특정 `<slot />` 사용법](/ko/basics/astro-components/#slots)에 대해 자세히 알아보세요.

### Gatsby 스타일링 변환 \{#gatsby-styling-to-astro\}
...
필요한 경우 인라인 스타일 객체 (`style={{ fontWeight: "bold" }}`)를 인라인 HTML 스타일 속성(`style="font-weight:bold;"`)으로 변환합니다. 또는 범위가 지정된 CSS 스타일에는 [Astro `<style>` 태그](/ko/guides/styling/#styling-in-astro)를 사용하세요.
...
### Gatsby 이미지 플러그인 변환 \{#gatsby-image-plugin-to-astro\}

Gatsby의 `<StaticImage />` 및 `<GatsbyImage />` 컴포넌트를 [Astro의 자체 이미지 통합 컴포넌트](/ko/guides/images/#image--astroassets)로 변환하거나 React 컴포넌트에서 적절하게 [표준 HTML `<img>` / JSX `<img />`](/ko/guides/images/#images-in-ui-framework-components) 태그로 변환하세요.
...
Astro의 `<Image />` 컴포넌트는 `.astro` 및 `.mdx` 파일에서만 작동합니다. [컴포넌트 속성의 전체 목록](/ko/guides/images/#properties)을 확인하고 몇 가지 속성이 Gatsby 속성과 다르다는 점에 유의하세요.

표준 Markdown 구문 (`![]()`)을 사용하여 [Markdown (`.md`) 파일의 이미지](/ko/guides/images/#images-in-markdown-files)를 계속 사용하려면 링크를 업데이트해야 할 수도 있습니다. HTML `<img>` 태그를 직접 사용하는 것은 로컬 이미지용 `.md` 파일에서 지원되지 않으며 Markdown 구문으로 변환해야 합니다.
...
### Gatsby GraphQL 변환 \{#gatsby-graphql-to-astro\}

GraphQL 쿼리에 대한 모든 참조를 제거하고 대신 [`Astro.glob()`](/ko/guides/imports/#astroglob)을 사용하여 로컬 파일의 데이터에 액세스하세요.

또는 콘텐츠 컬렉션을 사용하는 경우 [`getEntry()` 및 `getCollection()`](/ko/guides/content-collections/#querying-collections)을 사용하여 `src/content/`에서 Markdown 및 MDX 파일을 쿼리하세요.
...
## 안내 예시: Gatsby layout 변환 \{#guided-example-gatsby-layout-to-astro\}
...
2. `Layout.astro`를 생성하고 [Astro 구문으로 변환](#reference-convert-to-astro-syntax)된 이 `return` 값을 추가합니다.
