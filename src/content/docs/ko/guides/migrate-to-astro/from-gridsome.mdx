...
- Gridsome은 Vue 기반 단일 페이지 애플리케이션 (SPA)입니다. Astro 사이트는 [`.astro` 컴포넌트](/ko/basics/astro-components/)를 사용하여 구축된 다중 페이지 앱이지만 [React, Preact, Vue.js, Svelte, SolidJS, AlpineJS, Lit를 포함하여](/ko/basics/framework-components/) 원시 HTML 템플릿도 지원합니다.
