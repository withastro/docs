...
## Pelican과 Astro의 주요 유사점 \{#key-similarities-between-pelican-and-astro\}

Pelican과 Astro는 프로젝트 마이그레이션에 도움이 되는 몇 가지 유사점을 공유합니다.

- Pelican과 Astro는 모두 블로그와 같은 [콘텐츠 중심 웹사이트](/ko/concepts/why-astro/#content-driven)에 이상적으로 적합한 정적 사이트 생성기입니다.
...
## Pelican과 Astro의 주요 차이점 \{#key-differences-between-pelican-and-astro\}
...
## Pelican에서 Astro로 전환 \{#switch-from-pelican-to-astro\}
...
Pelican은 사이트 레이아웃과 메타데이터의 대부분을 처리했을 수도 있습니다. `<head>` 페이지를 포함하여 Astro에서 템플릿 작성을 직접 관리하는 방법을 알아보려면 [Astro 레이아웃을 Markdown 페이지 래퍼로 빌드](/ko/basics/layouts/#markdown-layouts)하는 방법을 읽어보세요.
