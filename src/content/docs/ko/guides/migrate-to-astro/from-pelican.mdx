...
- Pelican은 템플릿 작성을 위해 HTML 파일과 Jinja 구문을 사용합니다. [Astro 구문](/ko/core-concepts/astro-components/)은 JSX와 유사한 HTML 상위 집합입니다. 유효한 모든 HTML은 유효한 `.astro` 구문입니다.
...
기존 Markdown 콘텐츠 파일을 가져와 [Markdown 페이지를 만듭니다](/ko/guides/markdown-content/). 이러한 문서를 Pelican의 `content/` 폴더에서 Astro의 `src/pages/`로 복사하여 [파일 기반 라우팅](/ko/core-concepts/routing/)을 활용할 수 있습니다. 파일이 어디에 있어야 하는지 알아보려면 [Astro의 프로젝트 구조](/ko/core-concepts/project-structure/)를 읽어보세요.

Pelican은 사이트 레이아웃과 메타데이터의 대부분을 처리했을 수도 있습니다. `<head>` 페이지를 포함하여 Astro에서 템플릿 작성을 직접 관리하는 방법을 알아보려면 [Astro 레이아웃을 Markdown 페이지 래퍼로 빌드](/ko/core-concepts/layouts/#markdownmdx-레이아웃)하는 방법을 읽어보세요.
