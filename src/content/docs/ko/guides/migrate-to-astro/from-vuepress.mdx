...
- VuePress는 Vue 기반 단일 페이지 애플리케이션 (SPA)입니다. Astro 사이트는 [`.astro` 컴포넌트](/ko/basics/astro-components/)를 사용하여 구축된 다중 페이지 앱이지만 [React, Preact, Vue.js, Svelte, SolidJS, AlpineJS, Lit](/ko/basics/framework-components/) 및 원시 HTML 템플릿도 지원할 수 있습니다.
...
[Markdown 페이지를 생성](/ko/guides/markdown-content/)하기 위해 기존 Markdown 콘텐츠 파일을 가져오세요. 이러한 문서를 VuePress의 `docs`에서 Astro의 `src/pages/`로 이동하여 [파일 기반 라우팅](/ko/basics/routing/)을 계속 활용할 수 있습니다. 기존 VuePress 프로젝트에 해당하는 이름으로 폴더를 생성하면 기존 URL을 유지할 수 있습니다.
