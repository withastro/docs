...
Astro의 [React integration](/ko/guides/integrations-guide/react/)은 Create React App (CRA)와 같은 전체 React 앱을 포함하여 [Astro 컴포넌트 내부에서 React 컴포넌트를 사용](/ko/core-concepts/framework-components/)하기 위한 지원을 제공합니다!
...
[기본 구조에 Astro HTML 템플릿 컴포넌트를 사용](/ko/core-concepts/astro-components/)하여 유사한 컴포넌트 기반 디자인을 복제하는 동시에 상호 작용을 위한 개별 React 컴포넌트 (전체 앱일 수도 있음)를 가져오고 포함합니다.
...
CRA를 Astro 프로젝트로 재구성하기 전에 [Astro 아일랜드](/ko/concepts/islands/) 및 [Astro 컴포넌트](/ko/core-concepts/astro-components/)에 대해 알아보는 것이 도움이 될 수 있습니다.
...
3. import 구문을 포함하여 필요한 JavaScript를 ["코드 펜스" (`---`)](/ko/core-concepts/astro-components/#the-comComponent-script)로 이동합니다. 참고: [조건부 콘텐츠 렌더링](/ko/core-concepts/astro-syntax/#동적-html)을 위한 JavaScript는 Astro에서 직접 HTML 템플릿 내에 작성되는 경우가 많습니다.
