...
## CRA와 Astro의 주요 유사점 \{#key-similarities-between-cra-and-astro\}
...
## CRA와 Astro의 주요 차이점 \{#key-differences-between-cra-and-astro\}
...
- [콘텐츠 중심](/ko/concepts/why-astro/#content-driven): Astro는 콘텐츠를 선보이고 필요할 때만 상호 작용을 선택할 수 있도록 설계되었습니다. 기존 CRA 앱은 높은 클라이언트 측 상호 작용을 위해 빌드될 수 있으며 대시보드와 같은 `.astro` 컴포넌트를 사용하여 복제하기가 더 어려운 항목을 포함하려면 고급 Astro 기술이 필요할 수 있습니다.

## Astro에 CRA 추가 \{#add-your-cra-to-astro\}
...
### 통합 및 종속성 추가 \{#add-integrations-and-dependencies\}
...
### 기존 앱 파일 추가 \{#add-your-existing-app-files\}
...
:::note[클라이언트 지시어]
앱에 상호작용을 위해서는 [클라이언트 지시어](/ko/reference/directives-reference/#client-directives)가 필요합니다. Astro는 클라이언트 측 JavaScript를 선택할 때까지 React 앱을 정적 HTML로 렌더링합니다.
...
## CRA를 Astro로 전환 \{#convert-your-cra-to-astro\}

[기존 앱을 Astro에 추가](#add-your-cra-to-astro)한 후 앱 자체를 Astro로 변환하고 싶을 것입니다!
...
React 앱을 변환할 때 [Astro 컴포넌트로 다시 작성할](#converting-jsx-files-to-astro-files) React 컴포넌트를 결정하게 됩니다. 유일한 제한 사항은 Astro 컴포넌트가 React 컴포넌트를 가져올 수 있다는 것입니다. 그러나 React 컴포넌트는 다른 React 컴포넌트만 가져와야 합니다.
...
### JSX 파일을 `.astro` 파일로 변환 \{#converting-jsx-files-to-astro-files\}

다음은 CRA `.js` 컴포넌트를 `.astro` 컴포넌트로 변환하기 위한 몇 가지 팁입니다.

1. 기존 CRA 컴포넌트 함수의 반환된 JSX를 HTML 템플릿의 기초로 사용합니다.

2. [CRA 또는 JSX 구문을 Astro로](#reference-convert-cra-syntax-to-astro) 또는 HTML 웹 표준으로 변경합니다. 예를 들어 여기에는 `{children}` 및 `className`이 포함됩니다.
...
## 참조: CRA 구문을 Astro로 변환 \{#reference-convert-cra-syntax-to-astro\}

### CRA Imports 변환 \{#cra-imports-to-astro\}
...
### CRA Children Props 변환 \{#cra-children-props-to-astro\}
...
### CRA 데이터 페칭 변환 \{#cra-data-fetching-to-astro\}
...
### CRA 스타일링 변환 \{#cra-styling-to-astro\}
