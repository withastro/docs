...
Astro로 변환하려면 밑줄이 그어진 동적 라우트 속성 (예: `_name.vue`)을 대괄호 쌍 (예: `[name].astro`)으로 묶도록 변경하세요.
...
- SSR 사용
- 가능한 모든 정적 라우트를 정의하기 위한 [`nuxt.config.js` 파일의 `generate` 함수 사용](https://nuxtjs.org/docs/configuration-glossary/configuration-generate/)

Astro에서도 마찬가지로 두 가지 선택이 있습니다.

- [SSR 사용](/ko/guides/server-side-rendering/).
- Astro 페이지의 프런트매터에 `getStaticPaths()` 함수를 내보내 프레임워크에 [동적으로 생성할 정적 라우트](/ko/guides/routing/#동적-라우트)를 알려줍니다.
...
- [`fetch` 훅](https://nuxtjs.org/docs/features/data-fetching/#the-fetch-hook)
