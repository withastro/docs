...
Nuxt 프로젝트를 Astro로 변환하는 동안 사용할 [Astro의 선택적 통합](/ko/guides/integrations/) 중 일부를 설치하는 것이 유용할 수 있습니다.
...
2. `src/pages/index.astro`를 생성합니다.
    ...
    - `:attr="val"`은 `attr={val}`로 변경됩니다. 단, `key`는 Astro의 `li` 요소에서 속성으로 사용되지 않으므로 제외됩니다.
