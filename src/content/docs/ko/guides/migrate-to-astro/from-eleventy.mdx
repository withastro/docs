...
- Eleventy는 다양한 템플릿 언어를 지원합니다. Astro는 [널리 사용되는 여러 JS 프레임워크 (예: React, Svelte, Vue, Solid)의 컴포넌트](/ko/basics/framework-components/)를 지원하지만 대부분의 페이지 템플릿에서는 [Astro 레이아웃, 페이지 및 컴포넌트](/ko/basics/astro-components/)를 사용합니다.
