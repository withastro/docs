...
- Eleventy는 다양한 템플릿 언어를 지원합니다. Astro는 [널리 사용되는 여러 JS 프레임워크 (예: React, Svelte, Vue, Solid)의 컴포넌트](/ko/core-concepts/framework-components/)를 지원하지만 대부분의 페이지 템플릿에서는 [Astro 레이아웃, 페이지 및 컴포넌트](/ko/core-concepts/astro-components/)를 사용합니다.

- Astro는 사이트 메타데이터를 포함하여 사이트 빌드 중 쿼리 및 처리에 사용할 수 있는 모든 파일에 대해 [`src/` 디렉터리](/ko/core-concepts/project-structure/#src)를 사용합니다. 이 안에는 [파일 기반 라우팅을 위한 특수 `src/pages/` 폴더](/ko/core-concepts/astro-pages/)가 있습니다.

- Astro는 빌드 중에 처리하거나 변환할 필요가 없는 [정적 자산용 `public/` 폴더](/ko/core-concepts/project-structure/#public)를 사용합니다.
...
Eleventy 프로젝트를 통해 다양한 템플릿 언어를 사용하여 사이트를 빌드할 수 있었습니다. Astro 프로젝트에서 페이지 템플릿은 대부분 UI 요소, 레이아웃 및 전체 페이지로 사용할 수 있는 Astro 컴포넌트를 사용하여 수행됩니다. 컴포넌트를 사용하여 Astro에서 템플릿을 작성하는 방법을 보려면 [Astro의 컴포넌트 구문](/ko/core-concepts/astro-components/)을 살펴보세요.
