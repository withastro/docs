...
- Astro와 Docusaurus는 모두 [파일 기반 라우팅](/ko/basics/routing/)을 사용하여 `src/pages` 디렉터리에 있는 모든 MDX 파일에 대한 페이지 경로를 자동으로 생성합니다. 기존 콘텐츠에 Astro의 파일 구조를 사용하고 새 페이지를 추가하는 것이 친숙하게 느껴질 것입니다.
...
- Docusaurus는 React 기반 단일 페이지 애플리케이션 (SPA)입니다. Astro 사이트는 [`.astro` 컴포넌트](/ko/basics/astro-components/)를 사용하여 구축된 다중 페이지 앱이지만 [React, Preact, Vue.js, Svelte, SolidJS, AlpineJS, Lit](/ko/basics/framework-components/) 및 원시 HTML 템플릿도 지원할 수 있습니다.
