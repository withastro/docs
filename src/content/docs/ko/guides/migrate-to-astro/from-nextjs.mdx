...
- [`.astro` 파일의 구문은 JSX와 유사합니다](/ko/core-concepts/astro-syntax/#astro와-jsx의-차이점). Astro를 쓰는 것은 친숙하게 느껴질 것입니다.
...
Next `<Image />` 컴포넌트를 `.astro` 또는 `.mdx` 파일의 [Astro 자체 이미지 컴포넌트](/ko/guides/images/#image--astroassets)로 변환하거나 [표준 HTML ` <img>` / JSX `<img />`](/ko/guides/images/#ui-프레임워크-컴포넌트의-이미지) 태그를 React 컴포넌트에 적절하게 추가하세요.
