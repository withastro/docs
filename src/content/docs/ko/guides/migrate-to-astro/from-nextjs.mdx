...
## Next.js와 Astro의 주요 유사점 \{#key-similarities-between-nextjs-and-astro\}

Next.js와 Astro는 프로젝트 마이그레이션에 도움이 되는 몇 가지 유사점을 공유합니다.

- [`.astro` 파일의 구문은 JSX와 유사합니다](/ko/basics/astro-syntax/#differences-between-astro-and-jsx). Astro를 쓰는 것은 친숙하게 느껴질 것입니다.
- Astro 프로젝트는 SSG 또는 [페이지 수준 사전 렌더링이 포함된 SSR](/ko/guides/server-side-rendering/)일 수도 있습니다.
- Astro는 파일 기반 라우팅을 사용하며 [특별히 명명된 페이지가 동적 경로를 생성하도록 허용합니다](/ko/guides/routing/#dynamic-routes).
...
## Next.js와 Astro의 주요 차이점 \{#key-differences-between-nextjs-and-astro\}
...
- [콘텐츠 중심](/ko/concepts/why-astro/#content-driven): Astro는 콘텐츠를 선보이고 필요할 때만 상호 작용을 선택할 수 있도록 설계되었습니다. 기존 Next.js 앱은 높은 클라이언트 측 상호 작용을 위해 빌드될 수 있으며 대시보드와 같은 `.astro` 컴포넌트를 사용하여 복제하기가 더 어려운 항목을 포함하려면 고급 Astro 기술이 필요할 수 있습니다.

## Next.js 프로젝트 변환 \{#convert-your-nextjs-project\}
...
### `src`에 소스 코드를 배치 \{#put-your-source-code-in-src\}
...
### Astro 구성 파일 \{#the-astro-config-file\}
...
2. [Next 또는 JSX 구문을 Astro로](#reference-convert-nextjs-syntax-to-astro) 또는 HTML 웹 표준으로 변경합니다. 예를 들어 여기에는 `<Link>`, `<Script>`, `{children}`, `className`이 포함됩니다.
...
[단계별로 변환된 Next `.js` 파일의 예시](#guided-example-next-data-fetching-to-astro)를 참조하세요.

#### 비교: JSX vs Astro \{#compare-jsx-vs-astro\}
...
### 레이아웃 파일 마이그레이션 \{#migrating-layout-files\}
...
각 Astro 페이지에는 `<html>`, `<head>`, `<body>` 태그가 명시적으로 필요하므로 페이지 전체에서 레이아웃 파일을 재사용하는 것이 일반적입니다. Astro는 import 구문이 필요 없이 페이지 콘텐츠에 [`<slot />`](/ko/basics/astro-components/#slots)을 사용합니다. 표준 HTML 템플릿과 `<head>`에 대한 직접 액세스를 참고하세요.
...
#### Next.js의 `pages` 디렉터리에서 마이그레이션 \{#migrating-from-nextjs-pages-directory\}
...
#### Next.js의 `/app` 디렉터리에서 마이그레이션 \{#migrating-from-nextjs-app-directory\}
...
### 페이지 및 게시물 마이그레이션 \{#migrating-pages-and-posts\}

Next.js에서 게시물은 `/pages` 또는 `/app/routeName/page.jsx`에 있습니다.

Astro에서 **모든 페이지 콘텐츠는 `src/`**, `src/pages` 또는 `src/content` 내에 있어야 합니다.

#### React 페이지 \{#react-pages\}

기존 Next JSX (`.js`) 페이지를 [JSX 파일에서 `.astro` 페이지로 변환](#tips-convert-jsx-files-to-astro-files)해야 합니다. Astro에서는 기존 JSX 페이지 파일을 사용할 수 없습니다.
...
Astro에는 Markdown에 대한 기본 지원과 MDX 파일에 대한 선택적 통합이 있습니다. 기존 [Markdown 및 MDX 파일](/ko/guides/markdown-content/)을 재사용할 수 있지만 [Astro의 특수 `layout` frontmatter 속성](/ko/basics/layouts/#markdown-layouts)을 추가하는 등 frontmatter에 대한 일부 조정이 필요할 수 있습니다. 더 이상 Markdown으로 생성된 각 경로에 대해 페이지를 수동으로 만들 필요가 없습니다. 이러한 파일은 `src/pages/` 내에 배치하여 자동 파일 기반 라우팅을 활용할 수 있습니다.

또는 Astro에서 [콘텐츠 컬렉션](/ko/guides/content-collections/)을 사용하여 콘텐츠를 저장하고 관리할 수 있습니다. 컬렉션의 일부인 경우 Markdown 및 MDX 파일은 `src/content/` 내의 폴더에 저장됩니다. 콘텐츠를 직접 검색하고 [해당 페이지를 동적으로 생성](/ko/guides/content-collections/#generating-routes-from-content)합니다.
...
## 참조: Next.js 구문을 Astro로 변환 \{#reference-convert-nextjs-syntax-to-astro\}

### Next Links 변환 \{#next-links-to-astro\}
...
### Next Imports 변환 \{#next-imports-to-astro\}

상대 파일 경로를 정확하게 참조하도록 모든 [파일 가져오기](/ko/guides/imports/)를 업데이트하세요. 이는 [별칭 가져오기](/ko/guides/typescript/#import-aliases)를 사용하거나 상대 경로 전체를 작성하여 수행할 수 있습니다.
...
### Next Children Props 변환 \{#next-children-props-to-astro\}
...
### Next 데이터 페칭 변환 \{#next-data-fetching-to-astro\}
...
[`Astro.glob()`을 사용한 로컬 파일 가져오기](/ko/guides/imports/#astroglob), [Collections API를 사용한 쿼리](/ko/guides/content-collections/#querying-collections) 또는 [원격 데이터 가져오기](/ko/guides/data-fetching/)에 대해 자세히 알아보세요.

### Next 스타일링 변환 \{#next-styling-to-astro\}
...
### Next 이미지 플러그인 변환 \{#next-image-plugin-to-astro\}

Next `<Image />` 컴포넌트를 `.astro` 또는 `.mdx` 파일의 [Astro 자체 이미지 컴포넌트](/ko/guides/images/#image--astroassets)로 변환하거나 [표준 HTML ` <img>` / JSX `<img />`](/ko/guides/images/#images-in-ui-framework-components) 태그를 React 컴포넌트에 적절하게 추가하세요.

Astro의 `<Image />` 컴포넌트는 `.astro` 및 `.mdx` 파일에서만 작동합니다. [컴포넌트 속성의 전체 목록](/ko/guides/images/#properties)을 확인하고 일부 속성이 Next 속성과 다르다는 점에 유의하세요.
...
## 안내 예시: Next 데이터 페칭 변환 \{#guided-example-next-data-fetching-to-astro\}
