...
`create astro` 명령에 `--template` 인수를 전달하여 공식 스타터 (예: `docs`, `blog`, `portfolio`) 중 하나를 사용하여 새 Astro 프로젝트를 시작할 수 있습니다. 또는 [GitHub의 기존 Astro 저장소에서 새 프로젝트를 시작](/ko/install-and-setup/#cli-마법사를-통해-설치)할 수 있습니다.
...
또는 Astro에서 [콘텐츠 컬렉션](/ko/guides/content-collections/)을 사용하여 콘텐츠를 저장하고 관리할 수 있습니다. 콘텐츠를 직접 검색하고 [해당 페이지를 동적으로 생성](/ko/guides/content-collections/#콘텐츠에서-경로-생성)합니다.
...
[`import.meta.glob()`을 사용한 로컬 파일 가져오기](https://ko.vite.dev/guide/features.html#glob-import), [Collections API를 사용한 쿼리](/ko/guides/content-collections/#컬렉션-쿼리) 또는 [원격 데이터 가져오기](/ko/guides/data-fetching/)에 대해 자세히 알아보세요.
