...
## 지원하는 구성 파일 타입 \{#supported-config-file-types\}
...
## 구성 파일 확인 \{#config-file-resolving\}
...
## 상대 파일 참조 \{#referencing-relative-files\}
...
## 출력 파일 이름 사용자 정의 \{#customising-output-filenames\}
...
대신, 구성 파일에서 `process.env`를 사용하여 다른 환경 변수를 가져올 수 있으며, 이는 [CLI에 설정하는 것](/ko/guides/environment-variables/#using-the-cli)과 동일하게 동작합니다.
