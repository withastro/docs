...
모든 Astro 스타터 프로젝트에는 `tsconfig.json` 파일이 포함되어 있습니다. Astro의 [컴포넌트 스크립트](/ko/basics/astro-components/#the-component-script)는 TypeScript이며, 이는 Astro의 편집기 도구를 제공하고 프로젝트 코드의 타입 체크를 위해 JavaScript에 선택적으로 구문을 추가할 수 있게 해줍니다.
