...
프레임워크 렌더러, CSS 도구 및 기타 패키지를 Astro에 추가하는 방법에 대한 지침은 [Astro 통합 가이드](/ko/guides/integrations/)를 참조하세요.
...
[Discord](https://astro.build/chat)에 오셔서 채팅하시고 `#support` 포럼 채널에서 문제를 설명해 주세요. [GitHub](https://github.com/withastro/community-support/discussions)에서 직접 질문할 수도 있습니다. 우리는 언제나 기꺼이 도와드리겠습니다!

현재 [Astro에서 열려 있는 이슈](https://github.com/withastro/astro/issues/)를 방문하여 알려진 문제가 있는지 확인하거나 버그 보고서를 제출하세요.

또한 [로드맵 토론](https://github.com/withastro/roadmap/discussions)을 방문하여 Astro의 알려진 제한 사항을 발견했는지 확인하고 사용 사례와 관련된 현재 제안이 있는지 확인할 수 있습니다.
