...
### `console.log()`로 디버깅 \{#debugging-with-consolelog\}
...
### 프레임워크 컴포넌트 디버깅 \{#debugging-framework-components\}
...
### Astro `<Debug />` 컴포넌트 \{#astro-debug--component\}

Astro 컴포넌트 디버깅을 돕기 위해 Astro는 모든 값을 컴포넌트 HTML 템플릿에 직접 렌더링하는 내장 [`<Debug />`](/ko/reference/api-reference/#debug-) 컴포넌트를 제공합니다. 이는 터미널과 브라우저 사이를 앞뒤로 전환할 필요 없이 브라우저에서 빠르게 디버깅하는 데 유용합니다.
...
Astro 컴포넌트에서 `<script>` 태그는 기본적으로 [JS 모듈](https://developer.mozilla.org/ko/docs/Web/JavaScript/Guide/Modules)로 호이스트되어 로드됩니다. 태그에 [`is:inline` 지시어](/ko/reference/directives-reference/#isinline) 또는 기타 속성을 포함한 경우 이 기본 동작은 제거됩니다.
...
### `document` (or `window`) is not defined \{#document-or-window-is-not-defined\}
...
## 일반적인 문제 \{#common-gotchas\}

### 내 컴포넌트가 렌더링되지 않습니다. \{#my-component-is-not-rendering\}

먼저, [`.astro` 컴포넌트 스크립트](/ko/basics/astro-components/#the-component-script) 또는 [`.mdx` 파일](/ko/guides/markdown-content/#using-components-in-mdx)에서 **컴포넌트를 가져왔**는지 확인하세요.

그런 다음 가져오기 (import) 문을 확인하세요.

- 가져오기가 잘못된 위치로 연결되어 있습니까? (가져오기 경로를 확인하세요.)

- 가져오기가 가져온 컴포넌트와 이름이 동일합니까? (컴포넌트 이름이 [`.astro` 구문을 따르는지 확인하세요](/ko/basics/astro-syntax/#differences-between-astro-and-jsx)).

- 가져오기에 확장자를 포함시켰나요? (가져온 파일에 확장자가 포함되어 있는지 확인하세요. 예: `.astro`, `.md`, `.vue`, `.svelte`. 참고: `.js(x)` 및 `.ts(x)` 파일에는 확장자가 **필요하지 않습니다**.)

### 내 컴포넌트가 대화형이 아닙니다. \{#my-component-is-not-interactive\}

컴포넌트가 렌더링 중이지만 (위 참조) 사용자 상호 작용에 응답하지 않는 경우 컴포넌트를 수화하기 위한 [`client:*` 지시어](/ko/reference/directives-reference/#client-directives)가 누락되었을 수 있습니다.

기본적으로 [UI 프레임워크 컴포넌트는 클라이언트에서 수화되지 않습니다](/ko/guides/framework-components/#hydrating-interactive-components). `client:*` 지시어가 제공되지 않으면 해당 HTML이 JavaScript 없이 페이지에 렌더링됩니다.
...
### 패키지 'X'를 찾을 수 없습니다 \{#cannot-find-package-x\}
...
### `Astro.glob()` - 일치하는 항목이 없습니다. \{#astroglob---no-matches-found\}

`Astro.glob()`을 사용하여 파일을 가져올 때, 필요한 모든 파일과 일치하는 올바른 glob 구문을 사용해야 합니다.

#### 파일 경로 \{#filepaths\}

예를 들어 `src/pages/index.astro`에서 `../components/**/*.js`를 사용하여 다음 파일을 모두 가져옵니다.
...
이것은 Astro의 버그가 아닙니다. 이는 정적 문자열 리터럴만 지원하는 [Vite의 `import.meta.glob()` 함수](https://ko.vitejs.dev/guide/features.html#glob-import)의 제한 때문입니다.
...
### Astro를 Yarn 2+ (Berry)와 함께 사용하기 \{#using-astro-with-yarn-2-berry\}
...
### 모노레포에서 Astro에 종속성 추가 \{#adding-dependencies-to-astro-in-a-monorepo\}
...
### 컴포넌트에서 `<head>` 사용하기 \{#using-head-in-a-component\}
...
### 예상치 못한 `<script>` 또는 `<style>`이 포함되었습니다. \{#an-unexpected-script-or-style-is-included\}

해당 컴포넌트가 최종 출력에 표시되지 않더라도 가져온 컴포넌트의 `<script>` 또는 `<style>` 태그가 HTML 소스에 포함되어 있음을 확인할 수 있습니다. 예를 들어, 이는 표시되지 않는 [조건부 렌더링](/ko/basics/astro-syntax/#dynamic-html) 컴포넌트에서 발생합니다.
...
## Markdown에서 특수 문자 이스케이프 \{#escaping-special-characters-in-markdown\}
...
## 최소한의 복제물 만들기 \{#creating-minimal-reproductions\}
...
### [astro.new](https://astro.new)를 통해 StackBlitz 만들기 \{#create-a-stackblitz-via-\}
