...
해당 컴포넌트가 최종 출력에 표시되지 않더라도 가져온 컴포넌트의 `<script>` 또는 `<style>` 태그가 HTML 소스에 포함되어 있음을 확인할 수 있습니다. 예를 들어, 이는 표시되지 않는 [조건부 렌더링](/ko/reference/astro-syntax/#동적-html) 컴포넌트에서 발생합니다.
