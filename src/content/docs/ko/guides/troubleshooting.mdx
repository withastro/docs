...
[프레임워크 컴포넌트](/ko/basics/framework-components/) (예: React 및 Svelte)는 고유합니다. 기본적으로 서버 측에서 렌더링됩니다. 즉 `console.log()` 디버그 출력이 터미널에 나타납니다. 그러나 브라우저에 대해 수화될 수도 있으며, 이로 인해 디버그 로그가 브라우저에도 나타날 수 있습니다.
...
먼저, [`.astro` 컴포넌트 스크립트](/ko/basics/astro-components/#컴포넌트-스크립트) 또는 [`.mdx` 파일](/ko/guides/markdown-content/#mdx에서-컴포넌트-사용)에서 **컴포넌트를 가져왔**는지 확인하세요.
...
기본적으로 [UI 프레임워크 컴포넌트는 클라이언트에서 수화되지 않습니다](/ko/basics/framework-components/#인터랙티브-컴포넌트-하이드레이션하기). `client:*` 지시어가 제공되지 않으면 해당 HTML이 JavaScript 없이 페이지에 렌더링됩니다.
