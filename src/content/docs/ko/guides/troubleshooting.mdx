...
기본적으로 [UI 프레임워크 컴포넌트는 클라이언트에서 수화되지 않습니다](/ko/core-concepts/framework-components/#인터랙티브-컴포넌트-하이드레이션하기). `client:*` 지시어가 제공되지 않으면 해당 HTML이 JavaScript 없이 페이지에 렌더링됩니다.
