...
[프레임워크 컴포넌트](/ko/core-concepts/framework-components/) (예: React 및 Svelte)는 고유합니다. 기본적으로 서버 측에서 렌더링됩니다. 즉 `console.log()` 디버그 출력이 터미널에 나타납니다. 그러나 브라우저에 대해 수화될 수도 있으며, 이로 인해 디버그 로그가 브라우저에도 나타날 수 있습니다.
...
**해결책:** 인라인 스크립트 실행을 허용하려면 `script-src: 'unsafe-inline'`을 포함하도록 CSP를 업데이트하세요.

## 일반적인 문제

### 내 컴포넌트가 렌더링되지 않습니다.

먼저, [`.astro` 컴포넌트 스크립트](/ko/core-concepts/astro-components/#컴포넌트-스크립트) 또는 [`.mdx` 파일](/ko/guides/markdown-content/#using-components-in-mdx)에서 **컴포넌트를 가져왔**는지 확인하세요.

그런 다음 가져오기 (import) 문을 확인하세요.

- 가져오기가 잘못된 위치로 연결되어 있습니까? (가져오기 경로를 확인하세요.)

- 가져오기가 가져온 컴포넌트와 이름이 동일합니까? (컴포넌트 이름이 [`.astro` 구문을 따르는지 확인하세요](/ko/core-concepts/astro-syntax/#astro와-jsx의-차이점)).
...
기본적으로 [UI 프레임워크 컴포넌트는 클라이언트에서 수화되지 않습니다](/ko/core-concepts/framework-components/#인터랙티브-컴포넌트-하이드레이션하기). `client:*` 지시문이 제공되지 않으면 해당 HTML이 JavaScript 없이 페이지에 렌더링됩니다.

:::tip
[Astro 컴포넌트](/ko/core-concepts/astro-components/)는 클라이언트 측 런타임이 없는 HTML 전용 템플릿 컴포넌트입니다. 그러나 Astro 컴포넌트 템플릿에서 `<script>` 태그를 사용하여 전역 범위에서 실행되는 브라우저에 JavaScript를 보낼 수 있습니다.
:::
...
Astro에서 `<head>` 태그를 사용하는 것은 다른 HTML 태그처럼 작동합니다. 페이지 상단으로 이동하거나 기존 `<head>`와 병합되지 않습니다. 이 때문에 일반적으로 페이지 전체에 하나의 `<head>` 태그만 포함하려고 합니다. 단일 `<head>`와 해당 콘텐츠를 [레이아웃 컴포넌트](/ko/core-concepts/layouts/)에 작성하는 것이 좋습니다.

### 예상치 못한 `<script>` 또는 `<style>`이 포함되었습니다.

해당 컴포넌트가 최종 출력에 표시되지 않더라도 가져온 컴포넌트의 `<script>` 또는 `<style>` 태그가 HTML 소스에 포함되어 있음을 확인할 수 있습니다. 예를 들어, 이는 표시되지 않는 [조건부 렌더링](/ko/core-concepts/astro-syntax/#동적-html) 컴포넌트에서 발생합니다.
