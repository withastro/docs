...
## DecapCMS 설치 \{#installing-decapcms\}
...
4. React 앱에 `admin` 경로를 추가하세요. 이 파일은 `config.yml`과 함께 `public/admin/index.html`일 수도 있고, Astro 경로를 사용하려는 경우 `src/pages/admin.astro`일 수도 있습니다.
...
### Decap CMS와 Netlify Identity \{#decap-cms-with-netlify-identity\}
...
### Decap CMS와 외부 OAuth 클라이언트 \{#decap-cms-with-external-oauth-clients\}

Netlify 이외의 호스팅 제공업체에 배포하는 경우 자체 OAuth 경로를 생성해야 합니다.

Astro에서는 [`server` 또는 `hybrid` 출력](/ko/guides/server-side-rendering/#enable-on-demand-server-rendering)이 활성화된 상태로 구성된 프로젝트의 주문형 렌더링 경로를 통해 이 작업을 수행할 수 있습니다.
