...
Storyblok과 같은 일부 헤드리스 CMS는 Astro 사이트용 콘텐츠를 가져오는 데 도움이 되는 Astro [통합](/ko/guides/integrations/)을 제공합니다. 다른 CMS들은 원격 콘텐츠를 가져오기 위해 설치하고 사용하는 라이브러리인 JavaScript SDK를 제공합니다.

<ReadMore> 유형 (예: Git 기반, API 기반) 및 라이선스 (오픈 소스 또는 비공개 소스)별로 필터링할 수 있는 [100개 이상의 헤드리스 콘텐츠 관리 시스템 목록](https://jamstack.org/headless-cms/) <Badge text="외부 링크" />을 살펴보세요.</ReadMore>

## CMS 없이 Astro를 사용할 수 있나요?

예! Astro는 [Markdown](/ko/guides/markdown-content/)을 기본적으로 지원합니다.
