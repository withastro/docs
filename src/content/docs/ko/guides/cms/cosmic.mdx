...
Cosmic을 사용하여 Astro 블로그 콘텐츠를 관리하고 웹훅을 생성하여 새 콘텐츠를 업데이트하거나 추가할 때 웹사이트를 자동으로 재배포할 수 있습니다.
...
콘텐츠 객체를 업데이트하거나 추가할 때마다 호스팅 플랫폼 (예: Vercel)에서 사이트 재배포를 트리거하도록 Cosmic에서 직접 웹훅을 설정할 수 있습니다.

"Settings" > "webhooks"에서 Vercel의 URL 엔드포인트를 추가하고 웹훅을 트리거하려는 객체 유형을 선택하세요. “Add webhook”를 클릭하여 저장하세요.
