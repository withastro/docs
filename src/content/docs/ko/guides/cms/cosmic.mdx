...
## Cosmic과 Astro의 통합 \{#integrating-cosmic-with-astro\}

### 필요한 종속성 설치 \{#installing-necessary-dependencies\}
...
## Cosmic에서 데이터 가져오기 \{#fetching-data-from-cosmic\}
...
## Astro와 Cosmic으로 블로그 만들기 \{#building-a-blog-with-astro-and-cosmic\}

Cosmic을 사용하여 Astro 블로그 콘텐츠를 관리하고 웹후크를 생성하여 새 콘텐츠를 업데이트하거나 추가할 때 웹사이트를 자동으로 재배포할 수 있습니다.

### Cosmic 콘텐츠 객체 \{#cosmic-content-objects\}
...
### Astro에서 블로그 게시물 목록 표시하기 \{#displaying-a-list-of-blog-posts-in-astro\}

위와 동일한 [데이터 가져오기 방법](#fetching-data-from-cosmic)을 사용하여 스크립트 파일에서 `getAllPosts()` 쿼리를 가져오고 데이터를 기다립니다 (await 사용). 이 함수는 동적으로 표시될 수 있는 각 `post`에 대한 메타데이터를 제공합니다.
...
### Astro로 개별 블로그 게시물 생성하기 \{#generating-individual-blog-posts-with-astro\}

각 블로그 게시물의 전체 콘텐츠가 포함된 개별 페이지를 생성하려면 `src/pages/blog/[slug].astro`에서 [동적 라우팅 페이지](/ko/guides/routing/#dynamic-routes)를 생성하세요.
...
#### Cosmic 콘텐츠 업데이트로 재빌드 \{#rebuild-on-cosmic-content-updates\}
