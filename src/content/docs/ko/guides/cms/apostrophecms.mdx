...
1. [Node.js 어댑터](/ko/guides/integrations-guide/node/)가 설치되고 `output: 'server'`가 구성된 **온디맨드 렌더링된 Astro 프로젝트** - 아직 Astro 프로젝트가 없다면 [설치 가이드](/ko/install-and-setup/)를 참조하여 즉시 설치하여 실행할 수 있습니다.
