...
이러한 [환경 변수를 위한 TypeScript IntelliSense](/ko/guides/environment-variables/#intellisense-for-typescript)를 얻으려면 `src/` 디렉터리에 새 `env.d.ts` 파일을 만들고 다음과 같이 `ImportMetaEnv`를 구성할 수 있습니다.
...
:::note
[Astro에서 환경 변수 가져오기](/ko/guides/environment-variables/#getting-environment-variables)에 대해 자세히 알아보세요.
:::
...
## Astro와 Kontent.ai로 블로그 만들기 \{#making-a-blog-with-astro-and-kontentai\}
...
### 콘텐츠 모델 설정 \{#setting-up-content-model\}
...
### TypeScript에서 콘텐츠 모델 생성 \{#generating-content-model-in-typescript\}
...
이 섹션에서는 Astro와 함께 [정적 (SSG) 모드](/ko/guides/routing/#static-ssg-mode)를 사용합니다.
...
#### Kontent.ai 변경에 따른 재빌드 \{#rebuild-on-kontentai-changes\}
...
##### Kontent.ai에 웹후크 추가 \{#adding-a-webhook-to-kontentai\}
