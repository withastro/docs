...
이제 Astro 프로젝트에서 이 환경 변수를 사용할 수 있습니다.
...
`lib/strapi.ts`에 새 파일을 만들고 다음 래퍼 함수를 ​​추가하여 Strapi API와 상호 작용합니다.
...
각 아티클에 대해 [동적으로 페이지를 생성](/ko/core-concepts/routing/#동적-경로)하려면 `src/pages/blog/[slug].astro` 파일을 만드세요.
...
`output: server` 또는 `output: hybrid`를 사용하여 [SSR 모드를 선택](/ko/guides/server-side-rendering/)한 경우, 다음 코드를 사용하여 [동적 경로를 생성](/ko/core-concepts/routing/#서버-ssr-모드)하세요.
