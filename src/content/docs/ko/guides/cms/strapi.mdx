...
### `.env`에 Strapi URL 추가 \{#adding-the-strapi-url-in-env\}
...
### API 래퍼 만들기 \{#creating-the-api-wrapper\}
...
### 선택 사항: Article 인터페이스 생성 \{#optional-creating-the-article-interface\}
...
### 아티클 목록 표시 \{#displaying-a-list-of-articles\}
...
### 아티클 페이지 생성 \{#generating-article-pages\}

각 아티클에 대해 [동적으로 페이지를 생성](/ko/guides/routing/#dynamic-routes)하려면 `src/pages/blog/[slug].astro` 파일을 만드세요.
...
`output: server` 또는 `output: hybrid`를 사용하여 [SSR 모드를 선택](/ko/guides/server-side-rendering/)한 경우, 다음 코드를 사용하여 [동적 경로를 생성](/ko/guides/routing/#server-ssr-mode)하세요.
...
##### Strapi에 웹후크 추가하기 \{#adding-a-webhook-to-strapi\}
