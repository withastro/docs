...
`output: server` 또는 `output: hybrid`를 사용하여 [SSR 모드를 선택](/ko/guides/server-side-rendering/)한 경우, 다음 코드를 사용하여 [동적 라우트를 생성](/ko/guides/routing/#서버-ssr-모드)하세요.
...
프로젝트가 Astro의 기본 정적 모드를 사용하는 경우 콘텐츠가 변경될 때 새 빌드를 트리거하도록 웹훅을 설정해야 합니다. 호스팅 제공업체로 Netlify 또는 Vercel을 사용하는 경우 웹훅 기능을 사용하여 Strapi에서 새 빌드를 트리거할 수 있습니다.
...
[Strapi 웹훅 가이드](https://strapi.io/blog/webhooks)에 따라 Strapi 관리 패널에서 웹훅을 생성하세요.
