...
## 자격 증명 설정 \{#setting-up-credentials\}
...
## Astro와 Builder로 블로그 만들기 \{#making-a-blog-with-astro-and-builder\}

### 블로그 게시물 모델 만들기 \{#creating-a-model-for-a-blog-post\}
...
### 미리보기 설정 \{#setting-up-the-preview\}
...
#### 새 경로를 미리보기 URL로 설정 \{#setting-the-new-route-as-the-preview-url\}
...
#### 미리보기 URL 설정 테스트 \{#testing-the-preview-url-setup\}
...
### 블로그 게시물 작성하기 \{#creating-a-blog-post\}
...
### 블로그 게시물 목록 표시하기 \{#displaying-a-list-of-blog-posts\}
...
### 단일 블로그 게시물 표시 \{#displaying-a-single-blog-post\}

각 게시물에 대한 [페이지를 동적으로 생성](/ko/guides/routing/#dynamic-routes)하는 `src/pages/posts/[slug].astro` 페이지를 만듭니다.
...
### 사이트 게시 \{#publishing-your-site\}

웹사이트를 배포하려면 [배포 가이드](/ko/guides/deploy/)를 방문하여 선호하는 호스팅 제공업체의 지침을 따르세요.

#### Builder 변경 사항에 따라 다시 빌드 \{#rebuild-on-builder-changes\}
...
##### Builder에 웹후크 추가 \{#adding-a-webhook-to-builder\}
