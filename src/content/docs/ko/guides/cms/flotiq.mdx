...
Astro는 모든 페이지를 미리 렌더링하거나 요청이 있을 때 온디맨드로 경로를 생성하는 기능을 모두 지원합니다. [정적 사이트 생성](#정적-사이트-생성) 또는 [온디맨드 렌더링](#온디맨드-렌더링)에 대한 지침에 따라 블로그 게시물의 페이지 경로를 빌드하세요.
...
[SSR](/ko/guides/server-side-rendering/) 모드를 사용하는 경우 `slug`를 기준으로 단일 게시물을 가져와야 합니다.
