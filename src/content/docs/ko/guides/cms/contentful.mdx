...
:::tip
Contentful space가 비어 있는 경우 [Contentful model 설정](#setting-up-a-contentful-model)을 확인하여 콘텐츠에 대한 기본 블로그 모델을 만드는 방법을 알아보세요.
:::

[Contentful 문서](https://contentful.github.io/contentful.js/)에서 더 많은 쿼리 옵션을 확인할 수 있습니다.

## Astro와 Contentful로 블로그 만들기 \{#making-a-blog-with-astro-and-contentful\}
...
2. **[Contentful SDK](https://github.com/contentful/contentful.js)와 통합된 Astro 프로젝트** - Contentful을 사용하여 Astro 프로젝트를 설정하는 방법에 대한 자세한 내용은 [Astro와 통합](#integrating-with-astro)을 참조하세요.

### Contentful model 설정 \{#setting-up-a-contentful-model\}
...
### 개별 블로그 게시물 생성 \{#generating-individual-blog-posts\}

위와 동일한 방법을 사용하여 Contentful에서 데이터를 가져오지만, 이번에는 페이지에서 각 블로그 게시물에 대한 고유한 페이지 경로를 생성합니다.

#### 정적 사이트 생성 \{#static-site-generation\}

Astro의 기본 정적 모드를 사용하는 경우 [동적 경로](/ko/guides/routing/#dynamic-routes) 및 `getStaticPaths()` 함수를 사용합니다. 이 함수는 페이지가 되는 경로 목록을 생성하기 위해 빌드 시 호출됩니다.
...
항목을 찾을 수 없으면 [`Astro.redirect`](/ko/guides/routing/#dynamic-redirects)를 사용하여 사용자를 404 페이지로 리디렉션할 수 있습니다.
...
#### Contentful 변경에 대한 재빌드 \{#rebuild-on-contentful-changes\}
...
##### Contentful에 웹후크 추가하기 \{#adding-a-webhook-to-contentful\}
