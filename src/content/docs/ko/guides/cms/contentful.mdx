...
Astro의 기본 정적 모드를 사용하는 경우 [동적 라우트](/ko/guides/routing/#동적-라우트) 및 `getStaticPaths()` 함수를 사용합니다. 이 함수는 페이지가 되는 경로 목록을 생성하기 위해 빌드 시 호출됩니다.
...
`params` 내부의 속성은 동적 라우트의 이름과 일치해야 합니다. 파일 이름이 `[slug].astro`이므로 `slug`를 사용합니다.
...
http://localhost:4321/로 이동하여 게시물 중 하나를 클릭하여 동적 라우트가 작동하는지 확인하세요!

#### 서버 측 렌더링

[SSR 모드를 선택](/ko/guides/server-side-rendering/)한 경우 `slug` 매개변수를 사용하는 동적 라우트를 사용하여 Contentful에서 데이터를 가져옵니다.
...
프로젝트가 Astro의 기본 정적 모드를 사용하는 경우 콘텐츠가 변경될 때 새 빌드를 트리거하도록 웹훅을 설정해야 합니다. Netlify 또는 Vercel을 호스팅 공급자로 사용하는 경우 웹훅 기능을 사용하여 Contentful 이벤트에서 새 빌드를 트리거할 수 있습니다.
...
3. 웹훅의 이름을 제공하고 빌드를 트리거할 브랜치를 선택합니다. **Save**를 클릭하고 생성된 URL을 복사하세요.
</Steps>
...
Contentful space **settings**에서 **Webhooks** 탭을 클릭하고 **Add Webhook** 버튼을 클릭하여 새 웹훅을 생성합니다. 웹훅의 이름을 제공하고 이전 섹션에서 복사한 웹훅 URL을 붙여넣습니다. 마지막으로 **Save**을 눌러 웹훅을 만듭니다.
