...
모든 블로그 게시물에 대한 페이지를 생성하려면 동적 경로를 생성하는 `.astro` 페이지를 생성하세요. 이 접근 방식은 **정적 사이트 생성** (기본값)을 사용하는지 **서버 측 렌더링**을 사용하는지에 따라 달라집니다.
...
[SSR 모드를 선택](/ko/guides/server-side-rendering/)한 경우 동적 경로를 사용하여 Storyblok에서 페이지 데이터를 가져옵니다.
