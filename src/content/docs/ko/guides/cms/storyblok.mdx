...
### Storyblok 구성 \{#configuring-storyblok\}
...
### Bloks를 Astro 컴포넌트에 연결하기 \{#connecting-bloks-to-astro-components\}
...
## Astro와 Storyblok으로 블로그 만들기 \{#making-a-blog-with-astro-and-storyblok\}
...
2. **Storyblok과 통합된 Astro 프로젝트** - 통합 설정 방법에 대한 지침은 [Astro와 통합](#integrating-with-astro)을 참조하세요.

### blok 라이브러리 만들기 \{#creating-a-blok-library\}
...
### Bloks를 컴포넌트에 연결하기 \{#connecting-bloks-to-components\}
...
Astro의 기본 정적 사이트 생성을 사용하는 경우 [동적 경로](/ko/guides/routing/#dynamic-routes) 및 `getStaticPaths` 함수를 사용하여 프로젝트 페이지를 생성합니다.
...
#### Storyblok 변경에 따라 다시 빌드 \{#rebuild-on-storyblok-changes\}
...
##### Storyblok에 웹후크 추가하기 \{#adding-a-webhook-to-storyblok\}
