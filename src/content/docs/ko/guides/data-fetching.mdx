...
## Astro의 `fetch()` 함수 \{#fetch-in-astro\}
...
## 프레임워크 컴포넌트의 `fetch()` 함수 \{#fetch-in-framework-components\}
...
## 헤드리스 CMS에서 데이터 가져오기 \{#fetch-from-a-headless-cms\}

Astro 컴포넌트는 즐겨 사용하는 CMS에서 데이터를 가져온 다음 이를 페이지 콘텐츠로 렌더링할 수 있습니다. [동적 경로](/ko/guides/routing/#dynamic-routes)를 사용하면 컴포넌트가 CMS 콘텐츠를 기반으로 페이지를 생성할 수도 있습니다.
