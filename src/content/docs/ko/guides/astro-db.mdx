...
양식 요청 처리 및 원격 호스팅 데이터베이스에 데이터 삽입과 같은 사용자 입력을 허용하려면 Astro 프로젝트에 [온디맨드 렌더링](/ko/basics/rendering-modes/#요청-시-렌더링)을 구성하고 배포 환경에 [SSR 어댑터를 추가](/ko/guides/server-side-rendering/#공식-어댑터)하세요.
...
추가 Astro DB 구성 및 시드 파일을 등록하려면 `astro:db:setup` 훅에서 `extendDb()` 메서드를 사용하세요.
`defineDbIntegration()` 도우미는 `astro:db:setup` 훅에 대한 TypeScript 지원 및 자동 완성 기능을 제공합니다.
