...
Astro DB의 데이터는 SQL 테이블을 사용하여 저장됩니다. 테이블은 데이터를 행과 열로 구조화하며, 여기서 열은 각 행 값의 타입을 설정합니다. Astro DB는 다음 열 타입을 지원합니다.
...
테이블은 Astro의 콘텐츠에 대한 자동 TypeScript 타이핑을 지원합니다. 테이블을 정의하면 Astro는 자동으로 TypeScript 인터페이스를 생성하고 테이블에 적용합니다. 결과적으로 속성 자동 완성 및 타입 검사를 포함하여 데이터를 쿼리할 때 완전한 TypeScript 지원이 제공됩니다.

개별 데이터베이스 테이블을 구성하려면 `astro:db`에서 `defineTable()` 및 `column` 유틸리티를 가져와 사용하세요.

이 예시에서는 `author` 및 `body`에 대한 필수 텍스트 열로 `Comment` 테이블을 구성한 다음 `defineDb()` 내보내기를 통해 프로젝트에서 사용할 수 있도록 합니다.
...
Astro DB는 [Astro Studio 플랫폼에 연결](/ko/recipes/studio/)하여 호스팅된 데이터베이스를 프로젝트에 빠르게 추가할 수 있습니다. Astro Studio 웹 포털에서 새로운 호스팅 데이터베이스를 보고, 관리하고, 배포할 수 있습니다.

Astro Studio에서 프로젝트를 생성하는 방법에는 두 가지가 있습니다.

1. `astro link`를 사용하여 [Astro Studio CLI](/ko/reference/cli-reference/#astro-studio-cli)를 통해
2. [Astro Studio 웹 UI](https://studio.astro.build)를 통해

Astro Studio 웹 UI에서 새 데이터베이스를 생성하려면 헤더에서 "create project" 버튼을 클릭하고 제시된 지침을 따르세요. Astro Studio는 필요한 단계를 안내하며, 프로세스가 끝나면 프로젝트에 맞게 호스팅된 데이터베이스가 구성됩니다.
...
배포할 준비가 되면 [Studio 연결 및 배포 가이드](/ko/recipes/studio/#studio-연결-및-배포)를 확인하세요.
