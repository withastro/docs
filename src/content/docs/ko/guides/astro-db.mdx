...
양식 요청 처리 및 원격 호스팅 데이터베이스에 데이터 삽입과 같은 사용자 입력을 허용하려면 Astro 프로젝트에 [온디맨드 렌더링](/ko/basics/rendering-modes/#온디맨드-렌더링)을 구성하고 배포 환경에 [SSR 어댑터를 추가](/ko/guides/server-side-rendering/#공식-어댑터)하세요.
