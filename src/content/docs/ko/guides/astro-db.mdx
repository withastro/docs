...
:::caution
[Astro Studio를 종료합니다](https://astro.build/blog/goodbye-astro-studio/). 따라서 2024년 10월 1일 이후에는 더 이상 데이터베이스를 만들 수 없습니다.

대신 [기존 Studio 데이터베이스를 Turso로 마이그레이션](#astro-studio에서-turso로-마이그레이션)하거나 [Astro DB를 libSQL 서버에 연결](#libsql)하는 것을 권장합니다.
:::
...
배포할 준비가 되면 [Studio 연결 및 배포 가이드](#studio-연결을-사용하여-배포)를 확인하세요.
...
데이터베이스에 [변경 사항을 배포하고 푸시하는 명령](#studio-연결을-사용하여-배포)은 libSQL을 사용할 때 Astro Studio에서 호스팅하는 데이터베이스에 연결할 때와 동일합니다. 그러나 `--remote` 옵션이 있는 `astro build` 및 `astro db push`와 같은 명령을 실행할 때는 두 환경 변수를 모두 로컬로 설정해야 합니다.
