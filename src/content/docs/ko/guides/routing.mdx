...
기본적으로 Astro는 후행 슬래시 정규화, 리디렉션, 세션, 액션, 사용자 미들웨어, 페이지 렌더링, i18n, 캐싱 순으로 핸들러를 실행하는 내장 파이프라인을 통해 모든 요청을 처리합니다. 이 파이프라인은 라우팅 및 요청 처리의 일반적인 사용 사례를 지원하도록 설계되었지만, 모든 프로젝트의 요구 사항에 적합한 것은 아닐 수 있습니다.

Astro의 고급 라우팅을 사용하면 이 파이프라인을 자신만의 파이프라인으로 대체할 수 있습니다. 어떤 내장 기능을 사용할지, 그리고 어디에서 사용할지 선택할 수 있습니다. 또한 파이프라인의 어느 위치에서든 사용자 정의 로직을 추가할 수 있습니다. 이를 통해 Astro가 들어오는 요청을 처리하는 방식을 완전히 제어할 수 있습니다.
...
Astro는 고급 라우팅 엔트리포인트에 대해 `.ts`, `.js`, `.mjs`, `.mts` 등 다양한 파일 형식을 지원합니다. 대부분의 경우 `.js`를 사용하는 것을 권장하며, TypeScript 지원이 필요한 경우 `.ts`를 사용하세요.

### 엔트리포인트 파일 이름 변경

기본적으로 Astro는 `src/fetch.ts`를 고급 라우팅 엔트리포인트로 찾습니다. Astro 구성 파일에서 [`fetchFile`](/ko/reference/configuration-reference/#fetchfile) 옵션을 설정하여 이를 변경할 수 있습니다.

다음 예시는 Astro가 `src/fetch.ts` 대신 `src/handler.ts`를 찾도록 지정합니다:
...
고급 라우팅의 가장 큰 장점은 요청 파이프라인에 사용자 지정 로직을 삽입할 수 있다는 점입니다. Astro가 요청을 처리하기 전, 파이프라인 단계 사이, 또는 응답이 생성된 후에 코드를 실행할 수 있습니다.
...
Astro는 [`astro/hono`](/ko/reference/modules/astro-hono/)를 통해 모든 핸들러 함수를 위한 Hono 호환 래퍼도 제공합니다. Hono를 라우팅 프레임워크로 사용하려면 `src/fetch.ts`에서 Hono 앱을 내보낼 수 있습니다.
