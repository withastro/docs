...
## 페이지 간 이동 \{#navigating-between-pages\}
...
#### 예: 여러 수준의 동적 페이지 \{#example-dynamic-pages-at-multiple-levels\}
...
#### SSR에 대한 `[...slug]` 예시 수정 \{#modifying-the-slug-example-for-ssr\}

SSR 페이지는 `getStaticPaths()`를 사용할 수 없기 때문에 props를 받을 수 없습니다. [이전 예시](#example-dynamic-pages-at-multiple-levels)는 객체에서 `slug` 매개변수 값을 조회하여 SSR 모드에 맞게 조정할 수 있습니다. 경로가 루트 ("/")에 있으면 슬러그 매개변수는 `undefined`가 됩니다. 객체에 값이 없으면 404 페이지로 리디렉션됩니다.
...
Astro 구성에서 [영구적으로 이동된 페이지로 사용자를 리디렉션](#configured-redirects)하는 규칙을 정의할 수 있습니다. 또는 사용자가 여러분의 사이트를 사용할 때 [사용자를 동적으로 리디렉션](#dynamic-redirects)할 수도 있습니다.

### 구성된 리디렉션 \{#configured-redirects\}
...
이러한 리디렉션은 [파일 기반 라우팅과 동일한 우선순위 규칙](#route-priority-order)을 따르며, 프로젝트에서 같은 이름의 기존 페이지 파일보다 항상 우선순위가 낮습니다. 예를 들어 프로젝트에 `src/pages/old-page.astro` 파일이 포함되어 있으면 `/old-page`는 `/new-page`로 리디렉션되지 않습니다.
...
:::tip
영구적으로 이동한 콘텐츠를 제공하거나 사용자를 새 URL을 사용하는 다른 페이지로 안내하려면 (예: 로그인 후 사용자 대시보드) [리디렉션](#redirects)을 사용하세요.
:::
...
- **page.url.first** - 세트의 첫 페이지에 대한 링크
- **page.url.last** - 세트의 마지막 페이지에 대한 링크
...
#### 완전한 API 참조 \{#complete-api-reference\}
...
<ReadMore>[페이지네이션의 `page` prop](/ko/reference/api-reference/#the-pagination-page-prop)에 대해 알아보세요.</ReadMore>

### 중첩된 페이지네이션 \{#nested-pagination\}
