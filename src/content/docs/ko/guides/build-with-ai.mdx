...
[Raycast](https://www.raycast.com/)는 MCP 서버에 연결하여 AI 기능을 향상시킬 수 있습니다. Astro Docs MCP 서버를 추가하면 Raycast가 질문에 답하면서 최신 Astro 문서에 액세스할 수 있습니다.
...
[GitHub Copilot](https://docs.github.com/ko/copilot/concepts/agents/coding-agent/about-coding-agent)은 GitHub Actions으로 구동되는 코딩 에이전트로 사용할 수 있습니다. Astro Docs MCP 서버를 활성화하면 질문에 답하거나 코딩 작업을 수행할 때 최신 Astro 문서에 액세스할 수 있습니다.
...
AI 코딩 에이전트가 macOS 및 Linux에서 감지되면 `astro dev`가 서버를 분리된 백그라운드 프로세스로 자동 시작합니다. Astro v7.2.0부터는 `astro preview`도 동일하게 동작합니다. 이를 통해 서버가 에이전트의 터미널을 점유하지 않으며, 서버가 실행되는 동안에도 에이전트가 계속 작업할 수 있습니다.

Windows에서는 AI 코딩 에이전트 없이 실행할 때와 마찬가지로 서버가 포그라운드로 시작되며 로그를 터미널에 출력합니다. [`--background` 플래그](/ko/reference/cli-reference/#--background)를 전달하면 명시적으로 선택할 수 있습니다.

서버가 시작되면 서버의 URL, 포트, PID를 기록하는 잠금 파일(`.astro/dev.json` 또는 `.astro/preview.json`)이 생성됩니다. 이를 통해 동일한 프로젝트에서 중복된 서버가 시작되는 것을 방지합니다.

자동 백그라운드 모드를 사용하지 않으려면 명령을 실행하기 전에 `ASTRO_DEV_BACKGROUND` 또는 `ASTRO_PREVIEW_BACKGROUND` 환경 변수를 다음과 같이 설정하세요.
...
<ReadMore>[CLI 참조](/ko/reference/cli-reference/)에서 [`astro dev`](/ko/reference/cli-reference/#astro-dev) 및 [`astro preview`](/ko/reference/cli-reference/#astro-preview)의 모든 플래그와 하위 명령어 목록을 확인하세요.</ReadMore>
