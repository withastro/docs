...
AI 코딩 에이전트가 감지되면 `astro dev`가 서버를 분리된 백그라운드 프로세스로 자동 시작합니다. Astro v7.2.0부터는 `astro preview`도 동일하게 동작합니다. 이를 통해 서버가 에이전트의 터미널을 점유하지 않으며, 서버가 실행되는 동안에도 에이전트가 계속 작업할 수 있습니다.

서버가 시작되면 서버의 URL, 포트, PID를 기록하는 잠금 파일(`.astro/dev.json` 또는 `.astro/preview.json`)이 생성됩니다. 이를 통해 동일한 프로젝트에서 중복된 서버가 시작되는 것을 방지합니다.

AI 코딩 에이전트를 사용하지 않는 경우 서버는 포그라운드 프로세스로 시작되며 로그를 터미널에 출력합니다.
