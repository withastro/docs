...
- 클라이언트는 물론 [HTML 양식 액션에서도](#call-actions-from-an-html-form-action) 백엔드를 호출할 수 있는 타입이 안전한 함수를 생성하세요. 수동 `fetch()` 호출이 필요 없습니다.
...
액션은 `astro:actions` 모듈에서 함수로 사용할 수 있습니다. `actions`를 가져와서 [UI 프레임워크 컴포넌트](/ko/guides/framework-components/), [양식 POST 요청](#call-actions-from-an-html-form-action) 또는 Astro 컴포넌트에서 `<script>` 태그를 사용하여 클라이언트 측에서 호출합니다.
...
## 반환된 데이터 처리 \{#handling-returned-data\}
...
### 오류 확인 없이 `data`에 직접 접근하기 \{#accessing-data-directly-without-an-error-check\}
...
### 액션에서 백엔드 오류 처리 \{#handling-backend-errors-in-your-action\}
...
#### `ActionError` 만들기  \{#creating-an-actionerror\}
...
#### `ActionError` 처리하기 \{#handling-an-actionerror\}
...
### 클라이언트 리디렉션 처리하기  \{#handling-client-redirects\}

클라이언트에서 액션을 호출할 때 `react-router`와 같은 클라이언트 측 라이브러리와 통합하거나, 액션이 성공하면 새 페이지로 리디렉션하는 Astro의 [`navigate()` 함수](/ko/guides/view-transitions/#trigger-navigation)를 사용할 수 있습니다.
...
## 액션에서 양식 데이터 수신하기  \{#accepting-form-data-from-an-action\}
...
### 양식 데이터 검증하기 \{#validating-form-data\}
...
2. 제출된 양식을 처리할 `newsletter` 액션을 정의합니다. `z.string().email()` 유효성 검사기를 사용하여 `email` 필드의 유효성을 검사하고 `z.boolean()`을 사용하여 `terms` 체크박스의 유효성을 검사합니다:
    ...
    <ReadMore>사용 가능한 모든 양식 유효성 검사기는 [`input` API 참조](/ko/reference/api-reference/#input-validator)를 확인하세요.</ReadMore>
...
3. HTML 양식에 `<script>`를 추가하여 사용자 입력을 제출합니다. 이 예시에서는 양식의 기본 제출 동작을 재정의하여 `actions.newsletter()`를 호출하고 `navigate()` 함수를 사용하여 `/confirmation`으로 리디렉션합니다:
    ...
    <ReadMore>양식 데이터를 제출하는 다른 방법은 [“HTML 양식 액션에서 액션 호출”](#call-actions-from-an-html-form-action)을 참조하세요.</ReadMore>
    
</Steps>

### 양식 입력 오류 표시 \{#displaying-form-input-errors\}
...
## HTML 양식 액션에서 액션 호출 \{#call-actions-from-an-html-form-action\}

:::note
양식 액션을 사용하여 액션을 호출할 때는 페이지가 주문형으로 렌더링되어야 합니다. 이 API를 사용하기 전에 [페이지에서 사전 렌더링이 비활성화되어 있는지 확인](/ko/guides/server-side-rendering/#opting-out-of-pre-rendering-in-hybrid-mode)하세요.
:::
...
서버에서 [Astro.getActionResult()](/ko/reference/api-reference/#astrogetactionresult)를 호출하면 양식 제출 결과 (`data` 또는 `error`)가 반환되며, 동적 리디렉션, 양식 오류 처리, UI 업데이트 등에 사용할 수 있습니다.
...
### 액션 성공 시 리디렉션 \{#redirect-on-action-success\}

클라이언트 측 JavaScript 없이 액션이 성공했을 때 다른 페이지로 이동하려면 `action` 속성에 경로를 추가하면 됩니다.

예를 들어, `action={'/confirmation' + actions.newsletter}`는 `newsletter` 액션이 성공하면 `/confirmation`으로 이동합니다:
...
#### 액션 성공 시 동적으로 리디렉션 \{#dynamic-redirect-on-action-success\}
...
### 양식 액션 오류 처리 \{#handle-form-action-errors\}
...
더 많은 사용자 지정이 필요한 경우 [`isInputError()` 유틸리티를 사용](#displaying-form-input-errors)하여 잘못된 입력으로 인해 오류가 발생했는지 확인할 수 있습니다.
...
:::note
Astro는 일회용 쿠키로 액션 `data`와 `error`를 유지합니다. 즉, `getActionResult()`는 첫 번째 _요청에만_ 결과를 반환하고 페이지를 다시 방문할 때는 `undefined`를 반환합니다.
:::

#### 오류 시 입력값 유지 \{#preserve-input-values-on-error\}

입력값은 양식이 제출될 때마다 지워집니다. 입력 값을 유지하려면 페이지에서 [view transitions을 활성화](/ko/guides/view-transitions/#adding-view-transitions-to-a-page)하고 각 입력에 `transition:persist` 지시문을 적용하면 됩니다:

```astro ins="transition:persist"
<input transition:persist required type="email" name="email" />
```

### 양식 액션 결과로 UI 업데이트 \{#update-the-ui-with-a-form-action-result\}

`Astro.getActionResult()`가 반환하는 결과는 일회용이며 페이지를 새로고침할 때마다 `undefined`로 재설정됩니다. 이는 [입력 오류 표시](#handle-form-action-errors)와 성공 시 사용자에게 임시 알림을 표시하는 데 이상적입니다.

:::tip
페이지를 새로 고칠 때마다 같은 결과를 표시해야 하는 경우, 결과를 데이터베이스 또는 [쿠키에](/ko/reference/api-reference/#astrocookies)에 저장하는 것이 좋습니다.
:::
...
:::caution
액션 데이터는 영구 쿠키를 사용하여 전달됩니다. **이 쿠키는 암호화되지 않습니다.** 일반적으로 취약점을 피하기 위해 액션 `handler`에서 필요한 최소한의 정보만 반환하고 다른 민감한 정보는 데이터베이스에 보존하는 것이 좋습니다.

예를 들어, 전체 `product` 객체를 반환하는 대신 `addToCart` 액션에서 제품 이름을 반환할 수 있습니다:
