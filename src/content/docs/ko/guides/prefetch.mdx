...
## 프리페치 구성 \{#prefetch-configuration\}

또한 `prefetch` 구성은 프리페치를 사용자 정의하기 위한 옵션 객체를 허용합니다.

### 프리페치 전략 \{#prefetch-strategies\}
...
### 기본 프리페치 전략 \{#default-prefetch-strategy\}
...
### 모든 링크에 대한 프리페치를 기본값으로 설정하기 \{#prefetch-all-links-by-default\}
...
모든 링크에 대한 기본 프리페치 전략은 [기본 프리페치 전략 섹션](#default-prefetch-strategy)에 표시된 대로 `prefetch.defaultStrategy`를 사용하여 변경할 수 있습니다.

## 프로그래밍 방식으로 프리패치 \{#prefetch-programmatically\}
...
## View Transitions와 함께 사용 \{#using-with-view-transitions\}

페이지에서 [View Transitions](/ko/guides/view-transitions/)를 사용하면 기본적으로 프리페치도 활성화됩니다. 페이지에서 [모든 링크에 대한 프리페칭](#prefetch-all-links-by-default)을 활성화하는 `{ prefetchAll: true }`의 기본 구성을 설정합니다.
...
## `@astrojs/prefetch`에서 마이그레이션 \{#migrating-from-astrojsprefetch\}
