...
## Astro v3.0 실험적 플래그 제거 \{#astro-v30-experimental-flags-removed\}
...
- 페이지 전환 애니메이션 및 지속적 아일랜드에 대한 View Transitions. 이 실험적 플래그를 사용하는 경우 [view transitions API 주요 변경 사항 및 업그레이드 조언](#upgrade-view-transitions-to-v3)을 참조하세요.
- 새로운 `<Image />` 컴포넌트 및 `getImage()` 함수를 포함하는 Astro에서 이미지를 사용하기 위한 새로운 이미지 서비스 API인 `astro:assets`. **이 실험적 플래그를 사용하고 있는지 여부에 관계없이** 자세한 [이미지 업그레이드 조언](#upgrade-images-to-v3)을 읽고 이것이 프로젝트에 어떤 영향을 미칠 수 있는지 확인하세요.

[3.0 블로그 게시물](https://astro.build/blog/astro-3/)에서 이 두 가지 흥미로운 기능에 대해 자세히 알아보세요!

## Astro v3.0 주요 변경 사항 \{#astro-v30-breaking-changes\}
...
### 제거됨: Node 16에 대한 지원 \{#removed-support-for-node-16\}
...
### 제거됨: TypeScript 4 지원 \{#removed-support-for-typescript-4\}
...
### 제거됨: `@astrojs/image` \{#removed-astrojsimage\}
...
이미지 가이드에서 [이전 이미지 통합을 제거하기 위한 전체 단계별 지침](#remove-astrojsimage)을 확인할 수 있습니다.

`astro:assets`로 마이그레이션하면 사용하고 싶은 몇 가지 새로운 이미지 옵션과 기능도 제공됩니다. 자세한 내용은 전체 [v3.0 이미지 업그레이드 조언](#upgrade-images-to-v3)을 참조하세요!
...
### 제거됨: `<Markdown />` 컴포넌트 \{#removed-markdown--component\}
...
그렇지 않으면 Astro의 `<Markdown />` 컴포넌트와 `.astro` 파일의 컴포넌트 자체 가져오기에 대한 모든 참조를 삭제하세요. 콘텐츠를 HTML로 직접 다시 작성하거나 `.md` 파일에서 [Markdown을 가져와야](/ko/guides/markdown-content/#importing-markdown) 합니다.

### 제거됨: 더 이상 사용되지 않는 1.x API \{#removed-deprecated-1x-apis\}
...
### 제거됨: 서버 코드의 Web API에 대한 부분 shims \{#removed-partial-shims-for-web-apis-in-server-code\}
...
### 제거됨: 콘텐츠 컬렉션 스키마 `astro:content`의 `image` \{#removed-image-from-astrocontent-in-content-collections-schema\}

Astro v2.x에서 콘텐츠 컬렉션 API는 콘텐츠 컬렉션 스키마에서 사용하기 위한 `astro:content`의 `image` 내보내기를 더 이상 사용하지 않습니다.

Astro v3.0은 이 내보내기를 완전히 제거합니다.

#### 어떻게 해야 하나요? \{#what-should-i-do-6\}

`astro:content`에서 더 이상 사용되지 않는 `image()`를 사용하는 경우 더 이상 존재하지 않으므로 제거하세요. 대신 [`schema`의 `image` 도우미](#update-content-collections-schemas)를 통해 이미지를 확인하세요.
...
### 제거됨: `class:list` 기능 \{#removed-classlist-features\}
...
### 제거됨: `class:list`를 prop으로 전달 \{#removed-passing-classlist-as-a-prop\}
...
### 제거됨: camelCase CSS 변수에 대한 kebab-case 변환 \{#removed-kebab-case-transform-for-camelcase-css-variables\}

Astro v2.x에서는 `style` 속성에 전달된 camelCase [CSS 변수](/ko/guides/styling/#css-variables)가 작성된 대로 camelCase 및 하위 호환성을 위한 kebab-case로 렌더링되었습니다.
...
#### 어떻게 해야 하나요? \{#what-should-i-do-10\}
...
### 제거됨: `getStaticPaths()`의 반환 값 자동 플랫 \{#removed-automatic-flattening-of-getstaticpathss-return-value\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-11\}

요소가 _객체_ 인 배열 대신 요소가 배열인 배열을 반환하는 경우 이제 `.flatMap` 및 `.flat`을 사용하여 플랫 배열을 반환해야 합니다.

코드를 업데이트해야 하는 경우 [`getStaticPaths()`의 반환 값이 객체의 배열이어야 함을 나타내는 오류 메시지](/ko/reference/errors/invalid-get-static-paths-entry/#what-went-wrong)가 제공됩니다.

### 이동됨: `astro check`에는 이제 외부 패키지가 필요합니다. \{#moved-astro-check-now-requires-an-external-package\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-12\}

Astro v3.0으로 업그레이드한 후 `astro check` 명령을 실행하고 프롬프트에 따라 필요한 종속성을 설치하거나 프로젝트에 `@astrojs/check` 및 `typescript`를 수동으로 설치하세요.

### 더 이상 사용되지 않음: `build.excludeMiddleware` 및 `build.split` \{#deprecated-buildexcludemiddleware-and-buildsplit\}

Astro v2.x에서는 `build.excludeMiddleware` 및 `build.split`을 사용하여 SSR 모드에서 어댑터를 사용할 때 특정 파일이 내보내지는 방식을 변경했습니다.

Astro v3.0은 이러한 빌드 구성 옵션을 새로운 [SSR 어댑터 구성 옵션](/ko/guides/integrations-guide/#official-integrations)으로 대체하여 동일한 작업인 `edgeMiddleware` 및 `functionPerRoute`를 수행합니다.

#### 어떻게 해야 하나요? \{#what-should-i-do-13\}
...
### 더 이상 사용되지 않음: `markdown.drafts` \{#deprecated-markdowndrafts\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-14\}

프로젝트의 일부 페이지를 계속해서 초안으로 표시하려면 [콘텐츠 컬렉션으로 마이그레이션](/ko/guides/content-collections/#migrating-from-file-based-routing)하고 대신 `draft: true` 프런트매터 속성을 사용하여 [페이지를 수동으로 필터링](/ko/guides/content-collections/#filtering-collection-queries)하세요.

### 더 이상 사용되지 않음: 엔드포인트에서 단순 객체 반환 \{#deprecated-returning-simple-object-in-endpoints\}

Astro v2.x에서 엔드포인트는 JSON 응답으로 변환되는 간단한 객체를 반환할 수 있습니다.

Astro v3.0에서는 `Response` 객체를 직접 반환하기 위해 이 동작을 더 이상 사용하지 않습니다.

#### 어떻게 해야 하나요? \{#what-should-i-do-15\}
...
### 기본값 변경: tsconfig.json 프리셋의 `verbatimModuleSyntax` \{#changed-default-verbatimmodulesyntax-in-tsconfigjson-presets\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-16\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-18\}
...
### 기본값 변경: `compressHTML` \{#changed-default-compresshtml\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-19\}
...
### 변경된 기본값: `scopedStyleStrategy` \{#changed-default-scopedstylestrategy\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-20\}

프로젝트의 현재 [스타일 범위 지정](/ko/guides/styling/#scoped-styles)을 유지하려면 구성 파일을 이전 기본값으로 업데이트하세요.
...
### 기본값 변경: `inlineStyleSheets` \{#changed-default-inlinestylesheets\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-21\}
...
### 변경된 기본값: 이미지 서비스 \{#changed-default-image-service\}

Astro v2.x에서는 Squoosh가 [기본 이미지 처리 서비스](/ko/guides/images/#default-image-service)였습니다.

Astro v3.0에서는 이제 Sharp가 기본 이미지 처리 서비스로 포함되어 있으며 대신 Squoosh를 사용하기 위한 구성 옵션이 제공됩니다.

#### 어떻게 해야 하나요? \{#what-should-i-do-22\}
...
### 변경됨: HTTP 요청 메서드 \{#changed-http-request-methods-case\}

Astro v2.x에서는 [HTTP 요청 메서드](/ko/guides/endpoints/#http-methods)가 소문자 함수 이름 (`get`, `post`, `put`, `all` 및 `del`)을 사용하여 작성되었습니다.

Astro v3.0은 `del` 대신 `DELETE`를 포함한 대문자 함수 이름을 사용합니다.

#### 어떻게 해야 하나요? \{#what-should-i-do-23\}
...
### 변경됨: 다중 JSX 프레임워크 구성 \{#changed-multiple-jsx-framework-configuration\}

Astro v2.x에서는 어떤 파일이 어떤 프레임워크를 사용하는지 식별할 필요 없이 동일한 프로젝트에서 [여러 JSX 프레임워크 통합](/ko/guides/integrations-guide/#official-integrations) (React, Solid, Preact)을 사용할 수 있습니다.
...
#### 어떻게 해야 하나요? \{#what-should-i-do-24\}
...
### 변경됨: `Astro.cookies.get(key)`는 `undefined`를 반환할 수 있습니다. \{#changed-astrocookiesgetkey-can-return-undefined\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-25\}
...
### 변경됨: Astro CLI를 프로그래밍 방식으로 실행 \{#changed-running-the-astro-cli-programmatically\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-26\}

[Astro CLI를 프로그래밍 방식으로 실행](/ko/reference/cli-reference/#advanced-apis-experimental)하려면 새로운 실험적 JavaScript API를 사용하세요.
...
### 변경됨: 인터널 Astro API 엔트리포인트 내보내기 경로 \{#changed-internal-astro-api-entry-point-export-paths\}
...
#### 어떻게 해야 하나요? \{#what-should-i-do-27\}
...
## 기능 업그레이드 \{#feature-upgrades\}

### 이미지를 v3로 업그레이드 \{#upgrade-images-to-v3\}
...
#### `experimental.assets`에서 업그레이드 \{#upgrade-from-experimentalassets\}

이전에 `astro:assets`에 대한 실험적 플래그를 활성화한 경우 이제 기본적으로 자산 기능이 포함된 Astro v3.0용으로 프로젝트를 업데이트해야 합니다.

##### `experimental.assets` 플래그 제거 \{#remove-experimentalassets-flag\}
...
##### `~/assets` 가져오기 별칭 제거 \{#remove-the-assets-import-alias\}

이 가져오기 별칭은 더 이상 `astro:assets`에 기본적으로 포함되지 않습니다. 실험적 자산과 함께 이 별칭을 사용하는 경우 이를 상대 파일 경로로 변환하거나 [자신만의 가져오기 별칭을 생성](/ko/guides/imports/#aliases)해야 합니다.
...
##### Cloudflare, Deno, Vercel Edge, Netlify Edge에 대한 간단한 자산 지원 추가 \{#add-simple-asset-support-for-cloudflare-deno-vercel-edge-and-netlify-edge\}
...
#### 이미지를 저장할 위치 결정 \{#decide-where-to-store-your-images\}

[이미지를 저장할 위치](/ko/guides/images/#where-to-store-images)를 결정하는 데 도움이 되는 이미지 안내서를 참조하세요. `astro:assets`가 제공하는 유연성을 추가하여 이미지를 저장하는 새로운 옵션을 활용하고 싶을 수도 있습니다. 예를 들어, 이제 표준 Markdown `![alt](src)` 구문을 사용하여 `src/` 프로젝트의 상대 이미지를 Markdown, MDX, Markdoc에서 참조할 수 있습니다.

#### 기존 `<img>` 태그 업데이트 \{#update-existing-img-tags\}
...
기존 `<img>` 태그 ([UI 프레임워크 컴포넌트의 이미지](/ko/guides/images/#images-in-ui-framework-components) 포함)의 `src` 속성을 업데이트해야 하며 가져온 이미지에서 현재 사용할 수 있는 다른 속성도 업데이트할 수도 있습니다.
...
#### Markdown, MDX, Markdoc 파일 업데이트 \{#update-your-markdown-mdx-and-markdoc-files\}
...
#### `@astrojs/image` 제거 \{#remove-astrojsimage\}
...
1. `@astrojs/image` 통합을 제거합니다.
    ...
    통합을 제거한 다음 `astro.config.mjs` 파일에서도 [통합을 제거](/ko/guides/integrations-guide/#removing-an-integration)해야 합니다.
...
3. 기존 `<Image />` 컴포넌트를 마이그레이션합니다.
    ...
    [현재 지원되는 이미지 자산 속성](/ko/guides/images/#properties)이 아닌 컴포넌트 속성을 제거하세요.
...
#### 콘텐츠 컬렉션 스키마 업데이트 \{#update-content-collections-schemas\}

이제 현재 폴더에 대한 상대 경로를 사용하여 블로그 게시물의 표지 이미지와 같은 콘텐츠 컬렉션 항목에 대한 관련 이미지를 프런트매터에서 선언할 수 있습니다.

콘텐츠 컬렉션을 위한 새로운 `image` 도우미를 사용하면 Zod를 사용하여 이미지 메타데이터의 유효성을 검사할 수 있습니다. [콘텐츠 컬렉션에서 이미지를 사용하는 방법](/ko/guides/images/#images-in-content-collections)에 대해 자세히 알아보세요.

#### Astro v3.0에서 이미지 가져오기 탐색 \{#navigating-image-imports-in-astro-v30\}
...
### view transitions를 v3로 업그레이드 \{#upgrade-view-transitions-to-v3\}
...
#### `experimental.viewTransitions`에서 업그레이드 \{#upgrade-from-experimentalviewtransitions\}

이전에 view transitions에 대한 실험적 플래그를 활성화한 경우 이제 기본적으로 view transitions을 허용하는 Astro v3.0용으로 프로젝트를 업데이트해야 합니다.

##### `experimental.viewTransitions` 플래그 제거 \{#remove-experimentalviewtransitions-flag\}
...
##### 가져오기 소스 업데이트 \{#update-import-source\}
...
##### `transition:animate` 지시어 업데이트 \{#update-transitionanimate-directives\}
...
##### 이벤트 이름 업데이트 \{#update-event-names\}
