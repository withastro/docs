...
Astro v3.0은 이러한 빌드 구성 옵션을 새로운 [SSR 어댑터 구성 옵션](/ko/guides/integrations/#공식-통합)으로 대체하여 동일한 작업인 `edgeMiddleware` 및 `functionPerRoute`를 수행합니다.
...
Astro v2.x에서는 어떤 파일이 어떤 프레임워크를 사용하는지 식별할 필요 없이 동일한 프로젝트에서 [여러 JSX 프레임워크 통합](/ko/guides/integrations/#공식-통합) (React, Solid, Preact)을 사용할 수 있습니다.
