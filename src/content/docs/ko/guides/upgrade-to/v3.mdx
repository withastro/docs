...
- 페이지 전환 애니메이션 및 지속적 아일랜드에 대한 View Transitions. 이 실험적 플래그를 사용하는 경우 [view transitions API 주요 변경 사항 및 업그레이드 조언](/ko/guides/view-transitions/#v2x에서-v30으로-업그레이드)을 참조하세요.
- 새로운 `<Image />` 컴포넌트 및 `getImage()` 함수를 포함하는 Astro에서 이미지를 사용하기 위한 새로운 이미지 서비스 API인 `astro:assets`. **이 실험적 플래그를 사용하고 있는지 여부에 관계없이** 자세한 [이미지 업그레이드 조언](/ko/guides/images/#v2x에서-v30으로-업그레이드)을 읽고 이것이 프로젝트에 어떤 영향을 미칠 수 있는지 확인하세요.
...
이미지 가이드에서 [이전 이미지 통합을 제거하기 위한 전체 단계별 지침](/ko/guides/images/#astrojsimage-제거)을 확인할 수 있습니다.

`astro:assets`로 마이그레이션하면 사용하고 싶은 몇 가지 새로운 이미지 옵션과 기능도 제공됩니다. 자세한 내용은 전체 [v3.0 이미지 업그레이드 조언](/ko/guides/images/#v2x에서-v30으로-업그레이드)을 참조하세요!
...
`astro:content`에서 더 이상 사용되지 않는 `image()`를 사용하는 경우 더 이상 존재하지 않으므로 제거하세요. 대신 [`schema`의 `image` 도우미](/ko/guides/images/#콘텐츠-컬렉션-스키마-업데이트)를 통해 이미지를 확인하세요.
