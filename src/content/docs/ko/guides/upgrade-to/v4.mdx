...
이 가이드는 Astro v3에서 Astro v4로 마이그레이션하는 데 도움이 됩니다.

이전 프로젝트를 v3으로 업그레이드해야 합니까? [이전 마이그레이션 가이드](/ko/guides/upgrade-to/v3/)를 참조하세요.
...
v4.0으로 업그레이드한 후 프로젝트가 예상대로 작동하지 않으면 이 가이드에서 모든 주요 변경 사항에 대한 개요와 코드베이스를 업데이트 방법에 대한 지침을 확인하세요.
...
Vite 특정 플러그인, 구성 또는 API를 사용하는 경우 [Vite 마이그레이션 가이드](https://ko.vite.dev/guide/migration.html)에서 주요 변경 사항을 확인하고 필요에 따라 프로젝트를 업그레이드하세요. Astro 자체에는 큰 변화가 없습니다.
...
[더 이상 사용되지 않는 미들웨어 속성](/ko/guides/upgrade-to/v3/#더-이상-사용되지-않음-buildexcludemiddleware-및-buildsplit)을 어댑터 구성으로 업데이트하려면 v3 마이그레이션 가이드를 참조하세요.
