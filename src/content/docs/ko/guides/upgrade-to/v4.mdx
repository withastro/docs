...
필요한 경우 [Astro 통합을 수동으로 업그레이드](/ko/guides/integrations-guide/#manual-upgrading)할 수도 있으며 프로젝트의 다른 종속성을 업그레이드해야 할 수도 있습니다.
...
Astro v4.0에서는 [잠재적 주요 변경 사항](#breaking-changes)이 포함되고, [더 이상 사용되지 않는 일부 기능이 제거](#previously-deprecated-features-now-removed)되었습니다.
...
## Astro v4.0 실험적 플래그가 제거됨 \{#astro-v40-experimental-flags-removed\}
...
### 업그레이드: unified, remark, rehype 종속성 \{#upgraded-unified-remark-and-rehype-dependencies\}
...
### 이름이 변경됨: `entrypoint` (통합 API) \{#renamed-entrypoint-integrations-api\}
...
### 변경됨: 통합 API의 `app.render` 시그니처 \{#changed-apprender-signature-in-integrations-api\}
...
+ app.render(request, { routeData, locals })
```
### 변경됨: 이제 어댑터는 지원되는 기능을 지정해야 합니다. \{#changed-adapters-must-now-specify-supported-features\}
...
### 제거됨: Shiki 언어 `path` 속성 \{#removed-shiki-language-path-property\}
...
### 더 이상 사용되지 않음: View Transitions의 `submit` 이벤트를 위한 `handleForms` \{#deprecated-handleforms-for-view-transitions-submit-events\}
...
## 더 이상 사용되지 않는 기능 제거 \{#previously-deprecated-features-now-removed\}
...
### 제거됨: 엔드포인트에서 단순 객체 반환 \{#removed-returning-simple-objects-from-endpoints\}
...
### 제거됨: `build.split` 및 `build.excludeMiddleware` \{#removed-buildsplit-and-buildexcludemiddleware\}

Astro v3.0에서는 `build.split` 및 `build.excludeMiddleware` 빌드 구성 옵션이 더 이상 사용되지 않으며, 동일한 작업을 수행하기 위한 [어댑터 구성 옵션](/ko/reference/adapter-reference/#adapter-features)으로 대체되었습니다.

Astro v4.0은 이러한 속성을 완전히 제거합니다.

#### 무엇을 해야 하나요? \{#what-should-i-do-8\}
...
### 제거됨: `markdown.drafts` \{#removed-markdowndrafts\}

Astro v3.0에서는 `markdown.drafts`를 사용하여 초안 게시물 작성을 제어하는 ​​것이 더 이상 사용되지 않습니다.

Astro v4.0에서는 이 옵션이 완전히 제거되었습니다.

#### 무엇을 해야 하나요? \{#what-should-i-do-10\}
...
#### 무엇을 해야 하나요? \{#what-should-i-do-13\}

더 이상 사용되지 않는 소문자 이름을 사용하는 경우 이제 해당 이름을 해당하는 대문자로 바꿔야 합니다.

대문자 HTTP 요청 방법을 사용하는 지침은 v3 마이그레이션 가이드를 참조하세요.

### 제거됨: `base` 접두사가 누락된 경우 301 리디렉션 \{#removed-301-redirects-when-missing-a-base-prefix\}
...
#### 무엇을 해야 하나요? \{#what-should-i-do-14\}
...
### 제거됨: `astro/client-image` 자동 변환 \{#removed-astroclient-image-auto-conversion\}
...
#### 무엇을 해야 하나요? \{#what-should-i-do-15\}
