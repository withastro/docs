...
기본 사이트 URL도 기본 로케일에 대해 접두사가 붙은 현지화된 경로로 리디렉션되도록 이 동작을 변경할 수 있습니다. `prefixDefaultLocale: true`가 설정된 경우, 추가로 `redirectToDefaultLocale: true`를 구성할 수 있습니다. 이렇게 하면 `src/pages/index.astro`에 의해 생성된 홈 URL (`/`)이 `/[defaultLocale]/`로 리디렉션됩니다.
...
[`middleware()`](/ko/reference/modules/astro-i18n/#middleware) 함수는 Astro의 i18n 미들웨어를 수동으로 생성합니다. 이를 통해 Astro의 i18n 라우팅을 완전히 교체하는 대신 확장할 수 있습니다.
...
- `/about.astro` 파일은 `https://example.com/about`의 URL을 생성합니다.
