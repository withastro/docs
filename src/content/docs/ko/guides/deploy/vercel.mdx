...
Astro 프로젝트에서 SSR을 활성화하기 위해 다음 `astro add` 명령을 사용하여 [Vercel 어댑터](/ko/guides/integrations-guide/vercel/)를 추가하세요. 그러면 이렇게 하면 어댑터가 설치되고 `astro.config.mjs` 파일에 한 번에 적절히 이루어집니다.
