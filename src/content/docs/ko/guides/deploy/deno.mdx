        ...
        jobs:
          deploy:
            ...
            steps:
              ...
              # npm을 사용하지 않나요? `npm run build`를 `pnpm run build` 또는 `yarn run build`로 변경하세요.
              - name: Build Astro
                run: npm run build
...
4. (선택 사항) 빌드와 배포를 하나의 명령어로 처리하려면, 사용하는 패키지 관리자에 맞게 조정하여 `package.json`에 `deploy-deno` 스크립트를 추가하세요.
