...
프로젝트를 성공적으로 처리하고 빌드할 수 있도록 몇 가지 구성 단계를 거쳐 GitHub Pages에 Astro 사이트를 배포할 수 있습니다.

### GitHub Pages용 Astro 프로젝트 준비

기본적으로 GitHub Pages는 Jekyll 정적 사이트 처리를 적용하므로 Astro의 `_astro/` 디렉터리와 같이 밑줄로 시작하는 파일 및 폴더를 방해할 수 있습니다.

이 Jekyll 처리를 방지하려면 Astro 프로젝트의 `public/` 디렉터리에 빈 `.nojekyll` 파일을 추가하세요. Astro는 빌드 과정에서 이 파일을 최종 `dist/` 폴더에 복사합니다. 이렇게 하면 GitHub Pages에서 Jekyll 처리가 비활성화되고 Astro 프로젝트 파일이 올바르게 제공될 수 있습니다.

다음은 이 파일이 추가된 최소한의 Astro 프로젝트를 보여주는 예시입니다.
