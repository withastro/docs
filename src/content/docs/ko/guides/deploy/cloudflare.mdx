...
[Cloudflare SSR 어댑터](/ko/guides/integrations-guide/cloudflare/)와 함께 온디맨드 렌더링을 사용하는 프로젝트를 빌드한다면 `[Error] Could not resolve "XXXX. The package "XXXX" wasn't found on the file system but is built into node.`와 같은 오류 메시지와 함께 서버는 빌드에 실패합니다.
