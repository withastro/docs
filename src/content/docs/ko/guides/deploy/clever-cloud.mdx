...
완전히 정적인 프로젝트 및 주문형으로 렌더링된 Astro 프로젝트를 모두 Clever Cloud에 배포할 수 있습니다. `output` 모드 ([사전 렌더링 또는 주문형](/ko/guides/on-demand-rendering/))에 관계없이 포스트 빌드 훅을 사용하여 실행되는 **정적 애플리케이션**으로 배포하거나 `package.json`과 함께 즉시 작동하는 **Node.js** 애플리케이션으로 배포할 수 있습니다.
