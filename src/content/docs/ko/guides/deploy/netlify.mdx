...
### 웹사이트 UI 배포 \{#website-ui-deployment\}
...
#### `netlify.toml` 파일 \{#netlifytoml-file\}
...
4. 빌드 명령 (`astro build`)을 확인하세요.
    ...
    CLI는 빌드 설정 (`astro build`)과 배포 디렉터리 (`dist`)를 자동으로 감지하고 해당 설정을 사용하여 [`netlify.toml` 파일](#netlifytoml-file)을 자동으로 생성하도록 제안합니다.
...
### Node.js 버전 설정 \{#set-a-nodejs-version\}
...
## Netlify Functions 사용하기 \{#using-netlify-functions\}
