...
컬렉션을 정의하려면 프로젝트에 `content.config.ts` 파일을 만들어야 합니다(`.js` 및 `.mjs` 확장자도 지원됨). 이 파일은 Astro가 다음 구조에 따라 콘텐츠 컬렉션을 구성하는 데 사용하는 특수 파일입니다:
...
```ts  title="content.config.ts" {5,9}
import { defineCollection, z } from 'astro:content';
...
```ts title="content.config.ts" {6-11,15-19}
import { defineCollection, z } from 'astro:content';
import { glob, file } from 'astro/loaders'; // 레거시 API에서는 사용 불가능
