...
## 정적 파일 엔드포인트 \{#static-file-endpoints\}
...
### `params` 및 동적 라우팅 \{#params-and-dynamic-routing\}

엔드포인트는 페이지와 동일한 [동적 라우팅](/ko/guides/routing/#dynamic-routes) 기능을 지원합니다. 대괄호로 묶인 매개변수 이름을 사용하여 파일 이름을 지정하고 [`getStaticPaths()` 함수](/ko/reference/api-reference/#getstaticpaths)를 내보냅니다. 그런 다음 엔드포인트 함수에 전달된 `params` 속성을 통해 매개변수를 사용할 수 있습니다.
...
## 서버 엔드포인트 (API 라우트) \{#server-endpoints-api-routes\}
...
:::note
이 예시를 시도하기 전에 [주문형 렌더링 모드를 활성화](/ko/guides/server-side-rendering/)하고, `hybrid` 모드에서 사전 렌더링을 선택 해제하세요.
:::
...
또한 내보낸 해당 함수가 없는 다른 메서드와 일치하도록 `ALL` 함수를 내보낼 수도 있습니다. 일치하는 메서드가 없는 요청이 있는 경우 사이트의 [404 페이지](/ko/basics/astro-pages/#custom-404-error-page)로 리디렉션됩니다.
