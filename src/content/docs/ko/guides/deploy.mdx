...
1. **지속적 배포**(예: [Netlify](/ko/guides/deploy/netlify/) 또는 [Vercel](/ko/guides/deploy/vercel/))을 지원하는 호스트를 선택하고 Git 저장소를 새로운 사이트/프로젝트로 가져옵니다.
...
1. "Deploy"를 클릭하면 해당 호스트의 고유 URL(예: `new-astro-site.netlify.app`)에 새 웹사이트가 생성됩니다.
...
1. CLI를 실행하고 인증, 설정 등에 대한 지침을 따릅니다.

1. 사이트를 빌드하고 호스트에 배포합니다.
    ...
    다른 호스트에서는 [로컬에서 사이트를 구축](#로컬로-사이트-구축)하고 명령줄을 사용하여 배포해야 합니다.

## 사이트를 로컬로 빌드

Netlify 및 Vercel과 같은 많은 호스트는 여러분을 위해 사이트를 빌드한 다음 해당 빌드 출력을 웹에 게시합니다. 그러나 일부 사이트에서는 로컬로 빌드한 다음 배포 명령을 실행하거나 빌드 출력을 업로드해야 합니다.
...
기본적으로 빌드 출력은 `dist/` 디렉터리에 배치됩니다. 이 위치는 [`outDir` 구성 옵션](/ko/reference/configuration-reference/#outdir)을 사용하여 변경할 수 있습니다.

## SSR용 어댑터 추가

:::note
[SSR(서버 측 렌더링)](/ko/guides/server-side-rendering/)을 활성화하여 Astro 사이트를 배포하기 전에 다음 사항을 확인하세요.

- 프로젝트 종속성에 [적절한 어댑터](/ko/guides/server-side-rendering/)를 설치합니다. (수동으로 또는 `npx astro add netlify`와 같은 어댑터의 `astro add` 명령 사용).
