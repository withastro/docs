...
```astro
---
// 1. './style.module.css' 클래스 이름을 고유하고 범위가 지정된 값으로 변환합니다.
// 2. 원래 클래스 이름을 최종적으로 범위가 지정된 값에 매핑하는 객체를 반환합니다.
import styles from './style.module.css';
---

{/* 이 예제는 Astro를 사용하지만, 모든 프레임워크에서 CSS 모듈을 사용할 수 있습니다. */}
<div className={styles.error}>오류 메시지</div>
```
...
// `./src/components/`에 있는 `.astro`로 끝나는 모든 파일을 가져옵니다.
...
<p>
  제가 가장 좋아하는 게시물을 확인해 보세요: <a href={myFeaturedPost.url}
