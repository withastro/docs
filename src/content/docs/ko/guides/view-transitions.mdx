...
Astro는 단 몇 줄의 코드로 view transitions를 지원합니다. View transitions는 브라우저의 일반적인 전체 페이지 탐색 새로고침 없이 페이지 콘텐츠를 업데이트하고 페이지 간에 매끄러운 애니메이션을 제공합니다. [View Transition API에 대한 브라우저 지원](https://developer.mozilla.org/ko/docs/Web/API/View_Transition_API#%EB%B8%8C%EB%9D%BC%EC%9A%B0%EC%A0%80_%ED%98%B8%ED%99%98%EC%84%B1)이 부족한 경우, Astro를 사용하면 모든 방문자에게 최상의 경험을 제공하기 위해 [대체 전략을 제어](#대체-제어)할 수 있습니다.
...
`<ClientRouter />` 라우터는 view transitions를 지원하는 브라우저 (예: Chromium 브라우저)에서 가장 잘 작동하지만 다른 브라우저를 위한 기본 대체 지원도 포함합니다. 브라우저가 View Transitions API를 지원하지 않더라도 Astro의 클라이언트 라우터는 대체 옵션 중 하나를 사용하여 브라우저 내 탐색을 계속 제공할 수 있습니다.

브라우저 지원에 따라, 모든 브라우저에서 유사한 경험을 제공하기 위해 애니메이션 효과를 적용할 요소에 `name` 또는 `animate` [전환 지시어](#전환-지시어)를 명시적으로 설정해야 할 수 있습니다.
