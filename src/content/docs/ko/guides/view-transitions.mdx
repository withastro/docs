...
view transitions를 사용하면 일부 스크립트는 전체 페이지 브라우저 새로 고침과 같은 페이지 탐색 후 더 이상 다시 실행되지 않을 수 있습니다. [클라이언트 측 라우팅 중에 수신 대기할 수 있는 몇 가지 이벤트](#라이프사이클-이벤트)가 있으며, 이벤트가 발생하면 이벤트를 실행합니다. 기존 스크립트를 이벤트 리스너에 래핑하여 탐색 주기의 적절한 시점에 실행되도록 할 수 있습니다.
