...
- `astro-data-reload`: [전체 페이지 탐색 강제](#preventing-client-side-navigation)를 위한 `<a>` 태그 속성
- `astro-data-history="auto | push | replace"`: [브라우저 기록 제어](#replace-entries-in-the-browser-history)를 위한 `<a>` 태그 속성
...
`DOMContentLoaded`에 대한 이벤트 리스너를 추가하는 스크립트를 포함한 모듈 스크립트는 브라우저가 이미 로드된 모듈을 추적하기 때문에 한 번만 실행됩니다. 클라이언트 측 페이지 탐색 중 스크립트를 다시 실행하려면 [수명 주기 이벤트](#lifecycle-events)에 대한 리스너를 추가해야 합니다.
