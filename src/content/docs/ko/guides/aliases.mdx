...
이 예에서 개발자는 `src/pages/about/company.astro`, `src/components/controls/Button.astro` 및 `src/assets/logo.png` 파일 간 트리 관계를 이해해야 합니다. 그리고 `company.astro` 파일을 이동하려면 이러한 가져오기도 업데이트해야 합니다.

`tsconfig.json` 또는 `jsconfig.json`에 가져오기 별칭을 추가할 수 있습니다.
...
이 변경을 통해 프로젝트의 어느 곳에서나 별칭을 사용하여 가져올 수 있습니다.
