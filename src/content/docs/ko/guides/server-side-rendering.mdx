...
## 주문형 서버 렌더링 활성화 \{#enable-on-demand-server-rendering\}
...
서버는 요청 시 최소한 일부 페이지를 생성해야 하기 때문에 이 두 모드 모두 서버 기능을 수행하려면 [어댑터 추가](#add-an-adapter)가 필요합니다.
...
#### `astro add` 설치 \{#astro-add-install\}

다음 `astro add` 명령을 사용하여 [Astro에서 유지관리하는 공식 어댑터](/ko/guides/integrations-guide/#official-integrations)를 추가할 수 있습니다. 그러면 어댑터가 설치되고 `astro.config.mjs` 파일이 한 번에 적절하게 변경됩니다.
...
### `server` 또는 `hybrid` 구성 \{#configure-server-or-hybrid\}
...
### `server` 모드에서 사전 렌더링 선택 \{#opting-in-to-pre-rendering-in-server-mode\}
...
### `hybrid` 모드에서 사전 렌더링 선택 해제 \{#opting-out-of-pre-rendering-in-hybrid-mode\}
...
## 주문형 렌더링 기능 \{#on-demand-rendering-features\}

### HTML 스트리밍 \{#html-streaming\}
...
#### `Response` 객체 반환 \{#return-a-response-object\}
...
자세한 내용은 [엔드포인트 안내서](/ko/guides/endpoints/#server-endpoints-api-routes)를 참조하세요.
