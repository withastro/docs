...
2. 원하는 `output` 모드와 함께 `astro.config.mjs` 파일의 가져오기 및 기본 내보내기에 [어댑터를 추가](/ko/reference/configuration-reference/#어댑터)합니다.
...
자세한 내용은 [엔드포인트 안내서](/ko/basics/endpoints/#서버-엔드포인트-api-라우트)를 참조하세요.
