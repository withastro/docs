...
## 달라진 점은 무엇인가요? \{#what-has-changed\}
현재 Astro의 최신 릴리스는 <Version pkgName="astro" /> 입니다.

모든 변경사항의 전체 목록은 [Astro의 변경 로그](https://github.com/withastro/astro/blob/main/packages/astro/CHANGELOG.md)에서, 새로운 [주버전 버전](#major-changes)으로 업그레이드하는 데 필요한 중요한 지침은 [업그레이드 가이드](#upgrade-guides)에서 확인할 수 있습니다.


## 최신 버전으로 업그레이드 \{#upgrade-to-the-latest-version\}
...
### 특정 버전 설치 \{#install-a-specific-version-number\}
...
이 문서는 [부버전 릴리스](#minor-changes) 및 [주버전 버전 릴리스](#major-changes)가 있을 때마다 업데이트됩니다. 새로운 기능이 추가되거나, 기존 사용법이 변경되면, 문서가 업데이트되어 Astro의 **현재 동작**을 반영합니다. 프로젝트가 업데이트되지 않은 경우, 일부 동작이 최신 문서와 일치하지 않을 수 있습니다.
...
모든 [주버전 버전 릴리스](#major-changes) 이후에는 중요한 변경 사항에 대한 정보와 프로젝트 코드 업그레이드 지침이 포함된 **업그레이드 가이드**를 확인할 수 있습니다.
...
### 이전 문서 (관리되지 않음) \{#older-docs-unmaintained\}
...
### 주버전 변경 \{#major-changes\}

주버전 릴리스에는 최소한 일부 기존 코드에 대한 획기적인 변경 사항이 포함됩니다. 이러한 중요한 변경 사항은 항상 Astro의 ["vX로 업그레이드" 가이드](#upgrade-guides)에 문서화되어 있습니다.
...
### Node.js 지원 및 업그레이드 정책 \{#nodejs-support-and-upgrade-policies\}
...
### 연장된 유지 관리 \{#extended-maintenance\}
