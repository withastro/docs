...
> "아일랜드" 아키텍처의 일반적인 아이디어는 믿을 수 없을 정도로 간단합니다. 서버에서 HTML 페이지를 렌더링하고 매우 동적인 영역 [...] 주위에 플레이스홀더나 슬롯을 삽입한 다음, 클라이언트에서 작은 자체 포함 위젯으로 "하이드레이션"할 수 있습니다. , 서버에서 렌더링된 초기 HTML을 재사용합니다.

> — Jason Miller, Preact의 창시자

이 아키텍처 패턴의 기반이 되는 기술은 **부분** 또는 **선택적 하이드레이션**라고도 합니다.

대조적으로, 대부분의 JavaScript 기반 웹 프레임워크는 전체 웹사이트를 하나의 대규모 JavaScript 애플리케이션(단일 페이지 애플리케이션 또는 SPA라고도 함)으로 하이드레이션하고 렌더링합니다. SPA는 단순함과 강력함을 제공하지만, 클라이언트 측 JavaScript 사용이 많아 페이지 로드 성능 문제가 있습니다.

SPA는 [Astro 페이지 내부에도 삽입](/ko/guides/migrate-to-astro/from-create-react-app/)되어 있습니다. 그러나 SPA는 선택적이고 전략적으로 하이드레이션하는 기본 능력이 부족하여 오늘날 대부분의 웹 프로젝트에서 과도한 선택이 되고 있습니다.

Astro는 Sylor-Miller가 처음 만든 컴포넌트 아일랜드 패턴을 사용하여 선택적 하이드레이션 기능이 내장된 최초의 주류 JavaScript 웹 프레임워크로 인기를 얻었습니다.
...
또 다른 이점은 병렬 로딩입니다. 위의 예시 그림에서 우선순위가 낮은 '이미지 캐러셀' 아일랜드는 우선순위가 높은 '헤더' 아일랜드를 차단할 필요가 없습니다. 둘은 병렬로 로드되고 별도로 하이드레이션됩니다. 즉, 페이지 아래에 있는 무거운 캐러셀을 기다리지 않아도 은 즉시 상호작용이 가능합니다.
...
페이지의 어떤 컴포넌트가 브라우저에서 실행되어야 하는지 Astro에게 명시적으로 알려주는 것은 개발자의 몫입니다. Astro는 페이지에 필요한 것만 정확하게 하이드레이션하고 사이트의 나머지 부분은 정적 HTML로 남겨둡니다.
