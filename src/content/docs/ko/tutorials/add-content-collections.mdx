...
[블로그 구축 튜토리얼 개요](/ko/tutorial/0-introduction/)에서 Astro의 [내장 파일 기반 라우팅](/ko/guides/routing/#정적-라우트)에 대해 배웠습니다. `src/pages/` 폴더의 모든 `.astro`, `.md`, `.mdx` 파일은 자동으로 사이트의 페이지가 됩니다.
...
가이드에서 [Astro의 콘텐츠 컬렉션](/ko/guides/content-collections/)에 대해 자세히 읽어보거나 아래 지침에 따라 기본 블로그를 `src/pages/posts/`에서 `src/content/posts/`로 변환해 보세요.
