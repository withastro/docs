...
[블로그 구축 튜토리얼 개요](/ko/tutorial/0-introduction/)에서 Astro의 [내장 파일 기반 라우팅](/ko/core-concepts/routing/#정적-경로)에 대해 배웠습니다. `src/pages/` 폴더의 모든 `.astro`, `.md`, `.mdx` 파일은 자동으로 사이트의 페이지가 됩니다.
...
7. 각 블로그 게시물의 슬러그와 페이지 콘텐츠를 생성하기 위해 [컬렉션을 쿼리](/ko/guides/content-collections/#컬렉션-쿼리)해야 합니다. 이를 위해서 다음 코드를 추가합니다.
