...
## 블로그 튜토리얼 코드 구축 \{#build-a-blog-tutorial-code\}

[블로그 만들기 개요 튜토리얼](/ko/tutorial/0-introduction/)에서 Astro의 [내장된 파일 기반 라우팅](/ko/guides/routing/#static-routes)에 대해 배웠습니다. `src/pages/` 폴더 내 모든 `.astro`, `.md`, `.mdx` 파일은 자동으로 사이트의 새 페이지가 됩니다.
...
## 전체 페이지 탐색 vs 클라이언트 측 라우팅 (SPA 모드) \{#full-page-navigation-vs-client-side-routing-spa-mode\}
...
### 여러분의 지식을 테스트해보세요 \{#test-your-knowledge\}
...
## View transitions로 블로그 튜토리얼 확장 \{#extending-the-blog-tutorial-with-view-transitions\}

아래 단계에서는 페이지 전환을 향상시키기 위해 클라이언트 측 라우팅을 추가하여 블로그 만들기 튜토리얼의 최종 제품을 확장하는 방법을 보여줍니다.

### 종속성 업그레이드 \{#upgrade-dependencies\}
<Steps>
...
### `<ViewTransitions />` 라우터 추가 \{#add-the-viewtransitions--router\}
<Steps>
...
6. 다크 모드에서 깜박이는 것을 방지하려면 테마를 미리 확인하세요.
    ...
    <Box icon="question-mark">
      ### 여러분의 지식을 테스트해보세요 \{#test-your-knowledge-1\}
...
### 전환 애니메이션 사용자 정의 \{#customize-transition-animations\}
<Steps>
...
7. 페이지 제목의 기본 `fade` 애니메이션을 `slide`로 변경합니다.
    ...
    <Box icon="puzzle-piece">
      ### 직접 시도하기 - 탐색 링크에 슬라이드 인 애니메이션을 추가하세요. \{#try-it-yourself---make-the-navigation-links-slide-in\}

      [위와 동일한 단계](#customize-transition-animations)에 따라, 모든 헤더 링크가 포함된 `Navigation.astro`의 `<div>`가 페이지 탐색 시 슬라이드 인 애니메이션이 작동하도록 애니메이션 지시어를 추가하세요.
...
8. 블로그 게시물 설명에 더 긴 페이드인을 추가하세요.
    ...
    <ReadMore>다양한 [전환 지시어](/ko/guides/view-transitions/#transition-directives) 및 [애니메이션 맞춤설정](/ko/guides/view-transitions/#customizing-animations)에 대해 자세히 알아보세요.</ReadMore>
</Steps>
### 일부 링크에 브라우저 새로고침 추가 \{#force-a-full-browser-reload-for-some-links\}
<Steps>
