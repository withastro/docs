...
[블로그 만들기 개요 튜토리얼](/ko/tutorial/0-introduction/)에서 Astro의 [내장된 파일 기반 라우팅](/ko/guides/routing/#정적-라우트)에 대해 배웠습니다. `src/pages/` 폴더 내 모든 `.astro`, `.md`, `.mdx` 파일은 자동으로 사이트의 새 페이지가 됩니다.
...
가이드에서 [Astro의 view transitions](/ko/guides/view-transitions/)에 대해 읽어보거나 아래 지침에 따라 기본 블로그를 `src/pages/posts/`에서 `src/content/posts/`로 변환해 보세요.
...
3. 사이트 미리보기에서 페이지 간 탐색을 시작합니다.
    ...
    **데스크톱 모드와 같은 큰 화면 크기**에서 사이트 미리보기를 확인하세요. 사이트의 페이지를 탐색할 때 새 페이지 콘텐츠가 페이드 인되면서 이전 페이지 콘텐츠도 페이드 아웃되는 것처럼 보입니다. 기본값이 만족스럽지 않은 경우, [view transitions 가이드](/ko/guides/view-transitions/)를 사용하여 사용자 정의 동작을 추가하세요.
...
아직 탐험할 것이 훨씬 더 많습니다! view transitions로 수행할 수 있는 추가 작업은 [전체 view transitions 가이드](/ko/guides/view-transitions/)를 참조하세요.
