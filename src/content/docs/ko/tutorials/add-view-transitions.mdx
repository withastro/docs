...
[블로그 만들기 개요 튜토리얼](/ko/tutorial/0-introduction/)에서 Astro의 [내장된 파일 기반 라우팅](/ko/basics/routing/#정적-경로)에 대해 배웠습니다. `src/pages/` 폴더 내 모든 `.astro`, `.md`, `.mdx` 파일은 자동으로 사이트의 새 페이지가 됩니다.
