...
`create astro` 명령에 `--add` 인수를 전달하여 `astro add` 명령을 지원하는 [공식 통합](/ko/guides/integrations/) 또는 커뮤니티 통합을 설치하면서 새 Astro 프로젝트를 시작할 수 있습니다.
...
2. Astro를 설치합니다.
    ...
    :::note[중요]    
    Astro는 전역이 아닌 로컬에 설치해야 합니다. `npm install -g astro`, `pnpm add -g astro`, `bun add -g astro` 또는 `yarn add global astro`를 실행하고 있지 *않은지* 확인하세요.
    :::
