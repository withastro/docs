...
[`create astro` CLI 명령](#cli-마법사를-통한-설치)은 처음부터 새로운 Astro 프로젝트를 시작하는 가장 빠른 방법입니다. 새로운 Astro 프로젝트 설정의 모든 단계를 안내하며 몇 가지 공식 스타터 템플릿 중에서 선택할 수 있도록 합니다.
...
<PackageManagerTabs>
  <Fragment slot="npm">
  ```shell
  # react 및 tailwind가 포함된 새 프로젝트를 생성합니다.
...
5. `astro.config.mjs`를 만듭니다.
    ...
    React, Svelte 등과 같은 [UI 프레임워크 컴포넌트](/ko/guides/framework-components/)를 포함하거나 프로젝트에서 Tailwind 또는 Partytown과 같은 다른 도구를 사용하려면 여기에서 [통합을 수동으로 가져오고 구성](/ko/guides/integrations-guide/)합니다.
