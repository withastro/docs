...
3. `postsCollection`의 [스키마를 정의](/ko/guides/content-collections/#컬렉션-스키마-정의)하기 위해 `content.config.ts` 파일을 생성하세요. 기존의 블로그 튜토리얼 코드를 위해 다음 내용을 파일에 추가하세요. 이는 블로그 게시물에 사용된 모든 프런트매터 속성을 정의하기 위한 것입니다:
