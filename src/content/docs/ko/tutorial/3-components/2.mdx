...
2. 페이지 하단에 푸터를 표시하려면 닫는 `</body>` 태그 바로 앞에있는 각 페이지의 Astro 템플릿에 새로운 `<Footer />` 컴포넌트를 추가하세요.
...
3. 브라우저 미리보기에서 각 페이지의 새 푸터 텍스트가 보이는지 확인하세요.
</Steps>
...
여러 소셜 네트워크 (예: Instagram, Twitter, LinkedIn)를 표시하도록 푸터를 사용자 정의하고 사용자 이름을 포함하여 자신의 프로필에 직접 연결하세요.
...
2. 브라우저 미리보기를 확인하면 각 페이지에 이 세 가지 플랫폼에 대한 링크가 표시되는 새 푸터가 표시됩니다.
</Steps>
...
3. 브라우저 미리보기를 다시 확인하고 각 페이지에 업데이트된 푸터가 표시되는지 확인하세요.
</Steps>
