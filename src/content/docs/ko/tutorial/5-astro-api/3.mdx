...
## `/pages/folder/index.astro` 라우팅 패턴 사용 \{#use-the-pagesfolderindexastro-routing-pattern\}
...
## 직접 시도해 보기 - 태그 색인 페이지 만들기 \{#try-it-yourself---make-a-tag-index-page\}
...
## 태그 배열 생성 \{#create-an-array-of-tags\}
...
2. 그런 다음 페이지 컴포넌트에 다음 JavaScript 줄을 추가합니다. 이는 고유 태그 목록을 반환하기 위해 `src/pages/tags/[tag].astro`에서 사용한 것과 동일합니다.
...
## 태그 목록 만들기 \{#create-your-list-of-tags\}
...
## 태그 목록에 스타일 추가 \{#add-styles-to-your-tag-list\}
...
## 이 페이지를 탐색에 추가 \{#add-this-page-to-your-navigation\}
...
## 과제: 블로그 게시물 레이아웃에 태그 포함 \{#challenge-include-tags-in-your-blog-post-layout\}

이제 각 블로그 게시물에 태그 목록을 표시하고 태그 페이지에 연결하는 데 필요한 모든 코드를 작성했습니다. 재사용할 수 있는 기존 작업이 있습니다!

아래 단계를 수행한 후 [최종 코드 샘플](#code-check-in-markdownpostlayout)과 비교하여 작업 내용을 확인하세요.
...
### 코드 체크인: MarkdownPostLayout \{#code-check-in-markdownpostlayout\}
...
- [Astro의 정적 라우팅](/ko/guides/routing/#static-routes)
