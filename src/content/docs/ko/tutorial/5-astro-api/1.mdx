...
description: |-
  튜토리얼: 첫 번째 Astro 블로그 구축 - Astro.glob()을 사용하여 프로젝트 파일의 데이터에 액세스
i18nReady: true
---
...
<PreCheck>
  - `Astro.glob()`을 사용하여 모든 게시물의 데이터에 한 번에 액세스
  - 블로그 페이지에 동적으로 생성된 게시물 목록 표시
  - 각 목록 항목에 `<BlogPost />` 컴포넌트를 사용하도록 리팩터링
</PreCheck>

## 게시물 목록 동적으로 표시 \{#dynamically-display-a-list-of-posts\}

<Steps>
1. 모든 Markdown 파일에 대한 정보를 반환하려면 `blog.astro`에 다음 코드를 추가하세요. `Astro.glob()`은 각 블로그 게시물에 대해 하나씩 객체 배열을 반환합니다.
...
2. 게시물 제목과 URL을 사용하여 전체 게시물 목록을 동적으로 생성하려면 개별 `<li>` 태그를 다음 Astro 코드로 바꾸세요.
    ...
    이제 `Astro.glob()`에서 반환된 배열을 매핑하여 블로그 게시물의 전체 목록이 동적으로 생성됩니다.
...
## 과제: BlogPost 컴포넌트 만들기 \{#challenge-create-a-blogpost-component\}
...
- [`Astro.glob()` API 문서](/ko/reference/api-reference/#astroglob)
