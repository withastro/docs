...
## 첫 번째 레이아웃 컴포넌트 만들기 \{#create-your-first-layout-component\}
...
## 페이지에서 레이아웃 사용 \{#use-your-layout-on-a-page\}
...
## 페이지별 값을 props로 전달 \{#pass-page-specific-values-as-props\}
...
## 직접 시도해 보기 - 레이아웃을 어디에서나 사용하세요 \{#try-it-yourself---use-your-layout-everywhere\}

**리팩터링**하여 다른 페이지 (`blog.astro` 및 `about.astro`)가 새 `<BaseLayout>` 컴포넌트를 사용하여 공통 페이지 요소를 렌더링하도록 하세요. (참고: BaseLayout을 사용하여 `about.astro` 페이지를 렌더링하면 이 페이지의 `<head>`에 추가된 `<style>` 태그가 손실됩니다. 사용자 정의 `<h1>` 스타일을 유지하려면 스타일 태그를 페이지 컴포넌트의 body로 이동하세요.)
...
- [Astro `<slot />`](/ko/basics/astro-components/#slots)
