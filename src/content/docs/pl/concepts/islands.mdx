...
Astro pomogło zapoczątkować i spopularyzować nowy wzorzec architektury frontendu zwaną **Architekturą Wysp.** Działa ona poprzez renderowanie większości twojej strony do szybkiego, statycznego HTML-a z mniejszymi "wyspami" JavaScriptu dodanymi, gdy na stronie jest potrzebna interaktywność lub personalizacja (np. karuzela zdjęć). Pomaga to uniknąć monolitycznych payloadów JavaScriptu, kóre spowalniają responsywność wielu innych, nowoczesnych frameworków webowych JavaScript.
