---
title: Astro Wyspy
description: Astro Wyspy (znane również jako Wyspy Komponentów) to wzorzec architektury webowej zapoczątkowany przez Astro. Termin "architektura wysp" został po raz pierwszy użyty przez Katie Sylor-Miller, architektkę frontendu w Etsy, w 2019 roku, a następnie rozwinięty przez Jasona Millera, twórcę Preact.
i18nReady: true
---

import IslandsDiagram from '~/components/IslandsDiagram.astro';

Astro zapoczątkował i spopularyzował architekturę frontendu zwaną **Wyspy.** Architektura Wysp zapewnia lepszą wydajność frontendu, pomagając uniknąć monolitycznych wzorców JavaScript i automatycznie usuwając ze strony cały niepotrzebny JavaScript. Deweloperzy mogą nadal korzystać ze swoich ulubionych komponentów UI i frameworków z Astro, jednocześnie czerpiąc te korzyści.
...
SPA mają swoje miejsce, nawet [osadzone wewnątrz strony Astro](/en/guides/migrate-to-astro/from-create-react-app/). Jednak SPA brakuje wrodzonej zdolności do selektywnej i strategicznej hydracji, co czyni je zbyt ciężkim wyborem dla większości projektów w sieci dzisiaj.

Astro zyskało popularność jako pierwszy powszechny framework webowy JavaScript z wbudowaną selektywną hydracją, wykorzystując wzorzec wysp komponentów po raz pierwszy nazwany przez Sylor-Miller.
...
<IslandsDiagram>
  ...
  <Fragment slot="main">
    Statyczna treść jak tekst, obrazy, itp.
  </Fragment>
...
Może to brzmieć restrykcyjnie, ale to zachowanie utrzymuje strony Astro szybkimi domyślnie i chroni deweloperów przed przypadkowym wysyłaniem niepotrzebnego lub niechcianego JavaScriptu, który mógłby spowolnić ich stronę internetową.
...
Co więcej, możesz powiedzieć Astro dokładnie jak i kiedy renderować każdy komponent. Jeśli ta karuzela obrazów jest naprawdę kosztowna do załadowania, możesz dołączyć specjalną [dyrektywę klienta](/en/reference/directives-reference/#client-directives), która mówi Astro, aby ładować karuzelę tylko wtedy, gdy staje się widoczna na stronie. Jeśli użytkownik nigdy jej nie zobaczy, nigdy się nie załaduje.
