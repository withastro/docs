...
Astro traktuje również pliki Markdown (`.md`) wewnątrz `src/pages/` jako strony w Twojej końcowej witrynie. Jeśli masz zainstalowaną [Integrację MDX](/pl/guides/integrations-guide/mdx/#installation), traktuje pliki MDX (`.mdx`) w ten sam sposób. Są one powszechnie używane do stron z dużą ilością tekstu, takich jak posty na blogu i dokumentacja.

[Kolekcje stron zawartości Markdown lub MDX](/pl/guides/content-collections/) w `src/content/` mogą być używane do [dynamicznego generowania stron](/pl/guides/routing/#dynamic-routes).

Układy stron są szczególnie przydatne dla [plików Markdown](#strony-markdownmdx). Pliki Markdown mogą używać specjalnej właściwości frontmatter `layout`, aby określić [komponent układu](/pl/basics/layouts/), który opakuje ich zawartość Markdown w pełny dokument strony `<html>...</html>`.
...
Aby uzyskać niestandardową stronę błędu 500 do wyświetlenia dla stron, które są [renderowane na żądanie](/pl/guides/server-side-rendering/#włącz-renderowanie-na-żądanie-po-stronie-serwera), utwórz plik `src/pages/500.astro`. Ta niestandardowa strona nie jest dostępna dla stron renderowanych wstępnie i nie może być wstępnie renderowana.
...
:::caution
Strony częściowe są przeznaczone do użycia w połączeniu z biblioteką front-endową, taką jak [htmx](https://htmx.org/) lub [Unpoly](https://unpoly.com/). Możesz ich również użyć, jeśli czujesz się komfortowo pisząc niskopoziomowy JavaScript front-endowy. Z tego powodu są one zaawansowaną funkcją.

Dodatkowo, strony częściowe nie powinny być używane, jeśli komponent zawiera style lub skrypty o ograniczonym zasięgu, ponieważ te elementy zostaną usunięte z wyjścia HTML. Jeśli potrzebujesz stylów o ograniczonym zasięgu, lepiej jest użyć zwykłych, a nie stron częściowych wraz z biblioteką frontendową, która potrafi scalić zawartość do nagłówka.
:::
