...
- **[Dyrektywa define:vars](/pl/reference/directives-reference/#definevars) nie jest wspierana.** Jeśli nie możesz zawrzeć pochodnych za pomocą dodatkowego elementu (n.p. `<div>`), to możesz ręcznie dodać ``style={`--mojaZmienna:${wartosc}`}`` do Twojego Elementu. 
