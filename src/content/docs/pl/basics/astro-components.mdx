...
Aby przekazać wiele elementów HTML do slotu komponentu bez zawijania całości w element `<div>`, użyj atrybutu `slot=""` na [wbudowanym komponencie `<Fragment />`](/pl/reference/api-reference/#fragment-):
...
:::note
[Element `<slot />`](/pl/basics/astro-components/#slots) umieszczony wewnątrz komponentu HTML będzie działał normalnie, tak jak w przypadku komponentu Astro. Jeżeli chcesz zamiast tego użyć elementu [HTML Web Component Slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot), dodaj do niego `is:inline`.
:::
