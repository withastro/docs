...
## Zaktualizuj Astro \{#upgrade-astro\}
...
Astro v4.0 zawiera [potencjalne zmiany, które mogą powodować problemy](#breaking-changes), jak również [usunięcie niektórych wcześniej zdezaktualizowanych funkcji](#previously-deprecated-features-now-removed).
...
## Usunięto Eksperymentalne Flagi w Astro v4.0 \{#astro-v40-experimental-flags-removed\}
...
### Zaktualizowano: Vite 5.0 \{#upgraded-vite-50\}
...
Jeśli używasz wtyczek, konfiguracji lub API specyficznych dla Vite, sprawdź [przewodnik migracyjny Vite](https://vitejs.dev/guide/migration) w celu sprawdzenia zmian powodujących problemy i zaktualizuj swój projekt w razie potrzeby. W Astro samym w sobie nie ma zmian powodujących problemy.

### Uaktualnione: jednolite zależności dla remark i rehype \{#upgraded-unified-remark-and-rehype-dependencies\}
...
#### Co powinienem zrobić? \{#what-should-i-do-1\}
...
## Zmiany powodujące problemy \{#breaking-changes\}
...
### Zmieniona nazwa: `entrypoint` (API integracji) \{#renamed-entrypoint-integrations-api\}
...
#### Co powinienem zrobić? \{#what-should-i-do-2\}
...
### Zmiana: Podpis `app.render` w API Integracji \{#changed-apprender-signature-in-integrations-api\}
...
#### Co powinienem zrobić? \{#what-should-i-do-3\}
...
+ app.render(request, { routeData, locals })
```
### Zmiana: adaptery muszą teraz określić obsługiwane funkcje \{#changed-adapters-must-now-specify-supported-features\}
...
#### Co powinienem zrobić? \{#what-should-i-do-4\}
...
### Usunięto: Właściwość path języka Shiki \{#removed-shiki-language-path-property\}
...
#### Co powinienem zrobić? \{#what-should-i-do-5\}
...
## Niewspierane funkcje \{#deprecated\}
...
### Niewspierane: `handleForms` dla zdarzeń `submit` w przejściach widoku \{#deprecated-handleforms-for-view-transitions-submit-events\}
...
#### Co powinienem zrobić? \{#what-should-i-do-6\}
...
## Poprzednio niewspierane, teraz usunięte funkcje \{#previously-deprecated-features-now-removed\}
...
### Usunięto: zwracanie prostych obiektów z endpointów \{#removed-returning-simple-objects-from-endpoints\}
...
#### Co powinienem zrobić? \{#what-should-i-do-7\}
...
### Usunięto: `build.split` i `build.excludeMiddleware` \{#removed-buildsplit-and-buildexcludemiddleware\}
...
#### Co powinienem zrobić? \{#what-should-i-do-8\}
...
### Usunięto: `Astro.request.params` \{#removed-astrorequestparams\}
...
#### Co powinienem zrobić? \{#what-should-i-do-9\}

Zaktualizuj wszystkie wystąpienia do [`Astro.params`](/pl/reference/api-reference/#astroparams), który jest obsługiwanym zamiennikiem.
...
### Usunięto: `markdown.drafts` \{#removed-markdowndrafts\}
...
#### Co powinienem zrobić? \{#what-should-i-do-10\}

Jeśli korzystasz z niewspieranej `markdown.drafts`, musisz ją teraz usunąć, ponieważ już nie istnieje.

Aby kontynuować oznaczanie niektórych stron w projekcie jako robocze, [przenieś się do kolekcji treści](/pl/guides/content-collections/#migrating-from-file-based-routing) i [ręcznie odfiltruj strony](/pl/guides/content-collections/#filtering-collection-queries) z właściwością frontmatter `draft: true`.

### Usunięto: `getHeaders()` \{#removed-getheaders\}

W Astro w wersji 3.0 eksport `getHeaders()` dla Markdowna został zdezaktualizowany i zastąpiony przez `getHeadings()`.

W Astro w wersji 4.0 ta opcja została całkowicie usunięta.

#### Co powiniem zrobić? \{#what-should-i-do-11\}
...
### Usunięto: użycie `rss` w `getStaticPaths()` \{#removed-using-rss-in-getstaticpaths\}
...
#### Co powinienem zrobić? \{#what-should-i-do-12\}

Jeśli korzystasz z nieobsługiwaniej metody generowania kanałów RSS, musisz teraz użyć [`integracji @astrojs/rss`](/pl/guides/rss/) dla kompletnego ustawienia RSS.

### Usunięto: małe litery w nazwach metod HTTP \{#removed-lowercase-http-method-names\}
...
#### Co powinienem zrobić? \{#what-should-i-do-13\}
...
### Usunięto: przekierowania 301 przy braku prefiksu `base` \{#removed-301-redirects-when-missing-a-base-prefix\}
...
#### Co powinienem zrobić? \{#what-should-i-do-14\}
...
### Usunięto: Automatyczna konwersja `astro/client-image` \{#removed-astroclient-image-auto-conversion\}
...
#### Co powinienem zrobić? \{#what-should-i-do-15\}
...
## Zasoby społecznościowe \{#community-resources\}
