...
{/*TODO: Aktualizacja linku*/}
<ReadMore>Zobacz pełną dokumentację API Akcji, aby uzyskać szczegółowe informacje na temat [`defineAction()`](/en/reference/modules/astro-actions/#defineaction) i jego właściwości.</ReadMore>
...
{/*TODO: Aktualizacja linku*/}
<ReadMore>[Zobacz pełną dokumentację API [`handler()`](/en/reference/modules/astro-actions/#handler-property) aby uzyskać szczegółowe informacje.</ReadMore>
...
{/*TODO: Aktualizacja linku*/}
Kiedy wywołujesz akcje z klienta, możesz zintegrować je z biblioteką po stronie klienta, taką jak `react-router`, lub użyć funkcji Astro [`navigate()`](/en/guides/view-transitions/#trigger-navigation) do przekierowania na nową stronę po udanym wykonaniu akcji.
...
2. Zdefiniuj akcję `newsletter` do obsługi przesłanego formularza. Zweryfikuj pole `email` za pomocą walidatora `z.string().email()`, a pole `terms` za pomocą `z.boolean()`:
    ...
    ```
{/*TODO: Aktualizacja linku*/}
    <ReadMore>Zobacz [dokumentację API `input`](/en/reference/modules/astro-actions/#input-validator) aby uzyskać informacje o wszystkich dostępnych walidatorach formularzy.</ReadMore>
...
## Wywoływanie akcji z akcji formularza HTML
{/*TODO: Aktualizacja linku*/}
:::note
Strony muszą być renderowane na żądanie, gdy wywołujesz akcje za pomocą akcji formularza. [Upewnij się, że prerendering jest wyłączony na stronie](/en/guides/on-demand-rendering/#enabling-on-demand-rendering) przed użyciem tego API.
:::
...
{/*TODO: Aktualizacja linku*/}
Wywołanie [Astro.getActionResult()](/en/reference/api-reference/#getactionresult) na serwerze zwraca wynik przesłania formularza (`data` lub `error`), i może być użyte do dynamicznego przekierowania, obsługi błędów formularza, aktualizacji interfejsu użytkownika i więcej.
...
Dla większej elastyczności, możesz [użyć narzędzia `isInputError()`](#wyświetlane-błędów-wejścia-formularza) do sprawdzenia, czy błąd jest spowodowany nieprawidłowym wejściem.
...
{/*TODO: Aktualizacja linku*/}
Pola formularza będą wyczyszczone w przypadku jego przesłania. Aby zachować dane wejściowe, możesz [włączyć przejścia widoku](/en/guides/view-transitions/#adding-view-transitions-to-a-page) na stronie i zastosować dyrektywę `transition:persist` do każdego pola formularza:
