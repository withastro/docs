...
2. Wybierz hosta wspierającego **ciągłe wdrażanie** (np. [Netlify](/en/guides/deploy/netlify/) lub [Vercel](/en/guides/deploy/vercel/)) i zaimportuj swoje repozytorium jako nowa strona/projekt.
...
:::note
Przed wdrożeniem swojej strony Astro z włączonym [SSR (renderowaniem po stronie serwera)](/pl/guides/server-side-rendering/), upewnij się, że:

- Zainstalowałeś [odpowiedni adapter](/pl/guides/server-side-rendering/) do zależności projektu (ręcznie lub za pomocą polecenia adaptera `astro add`, np. `npx astro add netlify`).
