...
description: Dowiedz się, jak konfigurować i dostosowywać swój projekt Astro oraz środowisko programistyczne.
i18nReady: true
---
...
Każdy projekt startowy Astro zawiera plik `tsconfig.json`. [Skrypty komponentów](/pl/basics/astro-components/#the-component-script) w Astro domyślnie używają składni TypeScriptu, co zapewnia wsparcie narzędzi w edytorze oraz umożliwia opcjonalne dodanie typów do własnego kodu JavaScript.
...
Często stosowanym wzorcem w Astro jest utworzenie [komponentu](/pl/basics/astro-components/) `<Head />` w pliku `.astro`, który można użyć we wspólnym [układzie strony (layout)](/pl/basics/layouts/), tak aby obowiązywał dla wszystkich stron.
