...
Ten przewodnik przeprowadzi Cię przez kroki niezbędne do ręcznej instalacji i konfiguracji nowego projektu Astro, jeśli nie chcesz używać [automatycznego narzędzia CLI](/[;]/install/auto/).
...
Kiedy już znajdziesz się w nowym katalogu, utwórz plik projektu `package.json`. W ten sposób będziesz zarządzał paczkami projektu, w tym Astro. Jeśli nie jesteś zaznajomiony z tym formatem pliku, uruchom następujące polecenie, aby go utworzyć.
...
Najpierw zainstaluj paczki projektu Astro wewnątrz swojego projektu.
...
Następnie zastąp dowolny element placeholder "scripts" w swoim `package.json` następującym:
...
Dla tego przewodnika, skopiuj i wklej następujący fragment kodu (włączając `---` kreski) do nowego pliku:

```astro title="src/pages/index.astro"
---
// Witamy w Astro! Wszystko pomiędzy tymi potrójnymi kreskami w kodzie 
// nigdy nie jest uruchamiane w przeglądarce.
console.log('To działa w terminalu, nie w przeglądarce!')
---
...
Należy również utworzyć katalog `public/` do przechowywania statycznych zasobów. Astro zawsze będzie dołączać te zasoby do końcowego kompilatora, więc możesz bezpiecznie odwoływać się do nich z wnętrza swoich szablonów komponentów.

W edytorze tekstu utwórz nowy plik w swoim katalogu pod adresem `public/robots.txt`. `robots.txt` jest prostym plikiem, który większość stron zawiera, aby powiedzieć botom wyszukiwarek, takim jak Google, jak mają traktować Twoją stronę.
...
Jeśli chcesz uwzględnić [komponenty frameworka UI](/pl/core-concepts/framework-components/) takie jak React, Svelte, itp. lub użyć innych narzędzi takich jak Tailwind czy Partytown w swoim projekcie, to tutaj [ręcznie zaimportujesz i skonfigurujesz integracje](/pl/guides/integrations-guide/).

📚 Przeczytaj [referencje konfiguracji API Astro](/pl/reference/configuration-reference/), aby uzyskać więcej informacji.

## 6. Dodaj wsparcie dla TypeScript

TypeScript jest konfigurowany za pomocą pliku `tsconfig.json`. Nawet jeśli nie piszesz kodu TypeScript, plik ten jest ważny, aby narzędzia takie jak Astro i VS Code wiedziały jak interpretować Twój projekt. Niektóre funkcje (jak npm package imports) nie są w pełni obsługiwane w edytorze bez pliku `tsconfig.json`. 
...
📚 Przeczytaj Astro [przewodnik konfiguracji TypeScript](/pl/guides/typescript/#setup), aby uzyskać więcej informacji.
...
Gratulacje, jesteś teraz przygotowany do korzystania z Astro!

Jeśli podążałeś za tym przewodnikiem, możesz przejść bezpośrednio do [Krok 2: Uruchom Astro](/en/install/auto/#2-start-astro-), aby kontynuować i dowiedzieć się jak uruchomić Astro po raz pierwszy.
