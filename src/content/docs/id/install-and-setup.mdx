---
title: Install dan Pengaturan Astro
description: 'Cara menginstal Astro dan mulai mengerjakan proyek baru.'
i18nReady: true
---
import { Tabs, TabItem, FileTree, CardGrid, LinkCard, Steps } from '@astrojs/starlight/components';
import ReadMore from '~/components/ReadMore.astro';

Perintah [`create astro` CLI](#install-from-the-cli-wizard) adalah cara tercepat untuk memulai proyek Astro baru dari awal. Perintah ini akan memandu Anda melalui setiap langkah penyiapan proyek Astro baru dan memungkinkan Anda memilih dari beberapa templat awal resmi yang berbeda. 

Atau, Anda dapat memulai proyek Anda [menggunakan tema atau templat awal yang ada](#use-a-theme-or-starter-template).

Untuk menginstal Astro secara manual, lihat  [panduan instalasi manual langkah demi langkah kami.](#manual-setup).

:::tip[Pratinjau daring]
Lebih suka mencoba Astro di peramban Anda? Kunjungi [astro.new](https://astro.new/) untuk menelusuri templat awal kami dan memulai proyek Astro baru tanpa perlu meninggalkan peramban Anda.
:::

## Prasyarat

- **Node.js** - `v18.17.1` atau `v20.3.0` dan atau yang lebih tinggi. ( `v19` tidak didukung.)
- **Text Editor** - Kami merekomendasikan [VS Code](https://code.visualstudio.com/) bersama [ekstensi resmi Astro](https://marketplace.visualstudio.com/items?itemName=astro-build.astro-vscode).
- **Terminal** - Astro diakses melalui antarmuka baris perintah (CLI).

## Kompatibilitas peramban

Astro dibuat dengan Vite yang secara default menargetkan peramban dengan dukungan JavaScript modern. Untuk referensi lengkap, Anda dapat melihat [daftar versi peramban yang saat ini didukung di Vite](https://vitejs.dev/guide/build.html#browser-compatibility).
...
<Steps>
1. Jalankan perintah berikut di terminal Anda untuk memulai panduan instalasi praktis kami:
    ...
    Anda dapat menjalankannya `create astro` di komputer Anda, jadi tidak perlu membuat direktori kosong baru untuk proyek Anda sebelum memulai. Jika Anda belum memiliki direktori kosong untuk proyek baru Anda, panduan dalam instalasi akan membantu secara otomatis membuatkannya.

    Jika semuanya berjalan dengan lancar, Anda akan melihat pesan sukses yang diikuti dengan langkah selanjutnya yang direkomendasikan. Dan setelah proyek Anda dibuat, Anda dapat menggunakan perintah `cd` untuk masuk ke direktori proyek baru dan memulai menggunakan Astro.

2. Jika Anda melewatkan `npm install` dalam langkah tersebut selama panduan, then be sure to install your dependencies before continuing.
