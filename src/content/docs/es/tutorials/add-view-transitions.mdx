...
En el [tutorial introductorio Crear un Blog](/es/tutorial/0-introduction/), aprendiste sobre la [navegación incorporada basada en archivos](/es/basics/routing/#rutas-estáticas) de Astro: cualquier archivo `.astro`, `.md` o `.mdx` en cualquier lugar dentro de la carpeta `src/pages/` se convertía automáticamente en una nueva página en tu sitio.
