...
En el [tutorial introductorio de Crear un Blog](/es/tutorial/0-introduction/), aprendiste acerca del [sistema de rutas basado en archivos](/es/basics/routing/#rutas-estáticas): cualquier archivo `.astro`, `.md`, o `.mdx` dentro del directorio `src/pages/` se convertirá automáticamente en una página en tu sitio.
