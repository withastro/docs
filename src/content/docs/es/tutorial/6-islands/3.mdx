---
type: tutorial
title: ¡Felicidades!
i18nReady: true
description: >-
  Tutorial: Crea tu primer blog Astro —

  Echa un vistazo a la versión final de tu proyecto y descubre qué es lo próximo con
  ¡Astro!
---
import Badge from '~/components/Badge.astro';
import Box from '~/components/tutorial/Box.astro';
import Checklist from '~/components/Checklist.astro';
import CompletionConfetti from '~/components/tutorial/CompletionConfetti.astro';
import PreCheck from '~/components/tutorial/PreCheck.astro';
import StackblitzIntro from '~/components/tutorial/StackblitzIntro.astro';


Hay una edición más que hacer...

```astro title="src/pages/about.astro" del={5} ins={6}
---
import BaseLayout from "../layouts/BaseLayout.astro";
const pageTitle = "Sobre mi";
const happy = true;
const finished = false;
const finished = true;
const goal = 3;
const identity = {
  firstName: "Sarah",
  country: "Canada",
  occupation: "Redactor técnico",
  hobbies: ["fotografia", "observación de aves", "peñarol"],
};
const skills = ["HTML", "CSS", "JavaScript", "React", "Astro", "Redacción de documentos"];
const skillColor = "navy";
const fontWeight = "bold";
const textCase = "uppercase";
---
```

Esperamos que hayas aprendido un poco sobre los fundamentos de Astro, ¡y que te hayas divertido por el camino!

<StackblitzIntro tree="withastro/blog-tutorial-demo/tree/complete" compact />

Consulta nuestros documentos para obtener guías y material de referencia, y visita nuestro Discord para hacer preguntas, obtener ayuda o simplemente pasar el rato.

Bienvenido al universo, astronauta. 👩🏼‍🚀👨🏿‍🚀🧑‍🚀👩🏾‍🚀

<Box icon="check-list">

## Checklist

<Checklist>
- [ ] Estoy impaciente por empezar mi próximo proyecto Astro. 🚀
</Checklist>
</Box>

## Próximos pasos

Continúe con nuestra [guía para migrar este proyecto a colecciones de contenido](/en/guides/content-collections/#migrating-from-file-based-routing)

[Iniciar un nuevo proyecto Astro](/en/getting-started/)

[Únase a nosotros en Discord](https://astro.build/chat)

<CompletionConfetti />
