...
4. Añade un segundo componente `<Greeting />` con la directiva `client:load`.
  ...
  5. Vuelve a visitar tu página y compara los dos componentes. El segundo botón funciona porque la directiva `client:load` le dice a Astro que envíe y vuelva a ejecutar su JavaScript en el _client_ cuando la página _loads_, haciendo que el componente sea interactivo. Esto se llama un componente **hidratado**.
