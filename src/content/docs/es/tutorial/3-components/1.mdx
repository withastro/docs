...
<PreCheck>
  - Crea una nueva carpeta para los componentes
  - Crea un componente Astro para mostrar tus enlaces de navegación
  - Sustituye el HTML existente por un nuevo componente de navegación reutilizable
</PreCheck>

## Crea una nueva carpeta `src/components/`.

Para guardar los archivos `.astro` que generarán HTML pero que no se convertirán en nuevas páginas de su sitio web, necesitarás una nueva carpeta en tu proyecto: `src/components/`. 
...
2. Copia tus enlaces para navegar entre páginas desde la parte superior de cualquier página y pégalos en tu nuevo archivo, `Navigation.astro`:
    ...
    ```
    :::tip
    Si no hay nada en el frontmatter de tu archivo `.astro`, usted no tienes que escribir las cercas de código. Siempre puedes volver a añadirlas cuando las necesites.
    :::
...
2. A continuación, sustituye los elementos de enlace HTML de navegación existentes por el nuevo componente de navegación que acaba de importar:
...
- [Visión general de los componentes Astro](/es/core-concepts/astro-components/#estructura-de-un-componente)
