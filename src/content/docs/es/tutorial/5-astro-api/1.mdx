...
description: |-
  Tutorial: Construye tu primer blog Astro -
...
Ahora que ya tienes unas cuantas entradas de blog a las que enlazar, vamos a configurar la página Blog para que cree una lista de ellas automáticamente!

<PreCheck>
  - Acceda a los datos de todos sus puestos a la vez utilizando `Astro.glob()`.
  - Mostrar una lista de entradas generada dinámicamente en la página del blog
  - Refactoriza para utilizar un componente `<BlogPost />` para cada elemento de la lista
</PreCheck>
...
2. Para generar toda la lista de entradas dinámicamente, utilizando los títulos y las URL de las entradas, sustituya las etiquetas `<li>` individuales por el siguiente código Astro:
...
4. Vuelve a visitar la página de tu blog en la vista previa de tu navegador en `localhost:3000/blog` y busca una lista actualizada con cuatro elementos, incluida tu nueva entrada de blog
...
3. Añada la plantilla utilizada para crear cada elemento de la lista de entradas de su blog.
...
4. Importe el nuevo componente a su página Blog.
...
5. Compruébelo usted mismo: vea el código del componente terminado.
...
Si su componente Astro contiene la siguiente línea de código: 
...
- [`Astro.glob()` Documentación API](/en/reference/api-reference/#astroglob)
