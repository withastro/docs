---
type: tutorial
title: Crear una página index de etiquetas
i18nReady: true
description: |-
...
<PreCheck>
  - Añadir una nueva página utilizando el patrón de enrutamiento `/my-folder/index.astro`
  - Mostrar una lista de todas sus etiquetas únicas, con enlaces a cada página de etiquetas
  - Actualice su sitio con enlaces de navegación a esta nueva página de Etiquetas
</PreCheck>

## Utilizar el patrón de enrutamiento `/folder/index.astro`
...
## Pruébelo usted mismo: cree una página de índice de etiquetas
...
3. Crea una página mínima en `src/pages/tags/index.astro` que utilice tu diseño. Esto ya lo has hecho antes.
    ...
    <details>
      ...
      2. Importe y utilice su `<BaseLayout>`.
      ...
      3. Defina un título de página y páselo a su maqueta como atributo de componente.
...
4. Compruebe de nuevo la vista previa de su navegador y debería tener una página formateada, ¡lista para añadirle contenido!

</Box>

## Crear una matriz de etiquetas

Anteriormente ha mostrado elementos en una lista desde un array usando `map()`. ¿Cómo sería definir un array con todas tus etiquetas y mostrarlas en una lista en esta página?
...
Usted podría hacer esto, pero entonces usted tendría que volver a este archivo y actualizar su matriz cada vez que utilice una nueva etiqueta en una futura entrada del blog.

Afortunadamente, ya conoces una forma de obtener los datos de todos tus archivos Markdown en una sola línea de código y, a continuación, devolver una lista de todas tus etiquetas.
...
## Cree su lista de etiquetas

En lugar de crear elementos en una lista desordenada esta vez, crea una `<p>` para cada elemento, dentro de un `<div>`. El patrón debería resultarte familiar.

1. Añada el siguiente código a su plantilla de componentes:
    ...
    ```
  En la vista previa de su navegador, compruebe que puede ver sus etiquetas en la lista.

2. Para que cada etiqueta enlace con su propia página, añada el siguiente enlace `<a>` al nombre de cada etiqueta:
...
2. Defina estas nuevas clases CSS añadiendo la siguiente etiqueta `<style>` a esta página:
...
3. Compruebe la vista previa de su navegador en `localhost:3000/tags` para verificar que tiene algunos estilos nuevos y que cada una de las etiquetas de la página tiene un enlace que funciona a su propia página de etiquetas individual.

### Registro de códigos

Este es el aspecto que debería tener su nueva página:
...
## Añada esta página a su navegación

En este momento, puedes navegar a `localhost:3000/tags` y ver esta página. Desde esta página, puede hacer clic en los enlaces a sus páginas de etiquetas individuales.

Pero aún así, debe hacer que estas páginas se puedan descubrir desde otras páginas de su sitio web.

1. En su componente `Navigation.astro`, incluya un enlace a esta nueva página de índice de etiquetas.
...
Siga los pasos que se indican a continuación y compruebe su trabajo comparándolo con la [muestra de código final](#registro-de-códigos-markdownpostlayout).
...
Para utilizar props (valores pasados) de una entrada de blog `.md` en su diseño, como etiquetas, necesita anteponer al valor una palabra determinada.
...
Para comprobar tu trabajo, o si simplemente quieres un código completo y correcto para copiar en `MarkdownPostLayout.astro`, aquí tienes el aspecto que debería tener tu componente Astro:
...
Haga coincidir cada ruta de archivo con una segunda ruta de archivo que creará una página en la misma ruta.
...
- [Enrutamiento estático en Astro](/es/core-concepts/routing/#static-routes)
