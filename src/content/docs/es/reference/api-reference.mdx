...
### `Astro.redirect()`
En el modo de [renderizado del lado del servidor](/es/guides/server-side-rendering/), `Astro.redirect()` te permite redireccionar a otra página.
...
Este ejemplo define referencias desde un autor de blog a la colección `authors` y a un array de publicaciones relacionadas en la misma colección `blog`:
...
const blog = defineCollection({
  type: 'content',
  schema: z.object({
    // Hacer referencia a un único autor de la colección `authors` mediante el campo `id`.
    author: reference('authors'),
    // Hacer referencia a un array de publicaciones relacionadas de la colección `blog` mediante el campo `slug`.
...
[Consulta la guía de `Content Collection`](/es/guides/content-collections/#defining-collection-references) para ver un ejemplo de uso.
...
// Obtener `src/content/blog/enterprise.md`
const enterprisePost = await getEntry('blog', 'enterprise');
// Obtener `src/content/captains/picard.yaml`
...
Consulta la guía de "Colecciones de Contenido" para ver ejemplos de [consulta de entradas de colecciones](/es/guides/content-collections/#querying-collections).
...
`getEntries()` es una función que recupera múltiples entradas de una misma colección. Esto es útil para [devolver un arreglo de entradas referenciadas](/es/guides/content-collections/#defining-collection-references) para acceder a sus propiedades asociadas `data`, `body` y `render()`.
...
---
```
[Consulta la guía `Content Collection`](/es/guides/content-collections/#querying-collections) para ver un ejemplo de uso.
...
Un slug listo para ser utilizado en una URL para documentos Markdown o MDX. Por defecto, se genera a partir del `id` sin la extensión de archivo, pero se puede modificar estableciendo [la propiedad `slug`](/es/guides/content-collections/#defining-custom-slugs) en el frontmatter del archivo.
