...
En builds estáticos, esto serán los `params` devueltos por `getStaticPaths()` usados para prerrenderizar [rutas dinámicas](/es/basics/routing/#rutas-dinámicas).
...
[Las funciones de Endpoint](/es/basics/endpoints/) reciben un objeto de contexto como primer parámetro. Posee muchas de las propiedades del objeto global `Astro`.
...
También puede ser usado en endpoints de archivo estáticos para [enrutamiento dinámico](/es/basics/endpoints/#params-y-enrutamiento-dinámico).
...
La función `getImage()` está diseñada para generar imágenes destinadas a ser utilizadas en algún lugar que no sea directamente en HTML, por ejemplo, en una [Ruta de API](/es/basics/endpoints/#endpoints-del-servidor-rutas-de-api). También te permite crear tu propio componente `<Image />` personalizado.
