...
La función `getImage()` está diseñada para generar imágenes destinadas a ser utilizadas en algún lugar que no sea directamente en HTML, por ejemplo, en una [Ruta de API](/es/core-concepts/endpoints/#endpoints-del-servidor-rutas-de-api). También te permite crear tu propio componente `<Image />` personalizado.
...
Un componente utilizado con las [directivas `set:*`](/en/reference/directives-reference/#sethtml) para representar contenido HTML sin elementos de envoltura adicionales:
...
Consulta más información sobre [el uso de fragmentos](/es/core-concepts/astro-syntax/#fragments) en la sintaxis de Astro.
...
Obtén más información sobre cómo [controlar el enrutador](/es/guides/view-transitions/#control-de-enrutador) y [añadir directivas de transición](/es/guides/view-transitions/#directivas-de-transición) a elementos de página y componentes.
