...
Una app de la barra de herramientas para desarrolladores es un archivo `.js` o `.ts` que exporta por defecto un objeto con las siguientes propiedades obligatorias:
...
Si se devuelve un valor falsy, la desactivación se cancelará y la aplicación permanecerá habilitada.
