...
Este componente también se puede usar para crear [imágenes responsivas](#responsive-image-properties) que pueden ajustarse según el tamaño de su contenedor o el tamaño y la resolución de la pantalla de un dispositivo.

```astro title="src/components/MyComponent.astro"
---
// import the Image component and the image
import { Image } from 'astro:assets';
import myImage from "../assets/my_image.png"; // Imagen is 1600x900
---
...
El componente `<Image />` acepta las siguientes propiedades listadas y las [propiedades de imagen responsiva](#responsive-image-properties), además de todas las propiedades que acepta la etiqueta HTML `<img>`.
...
- **Imágenes locales en `src/`**: debes **importar también la imagen** usando una ruta relativa del archivo o configurar y usar un [alias de importación](/en/guides/imports/#aliases). Luego, utiliza el nombre del import como valor de `src`:
...
El atributo `densities` no es compatible con [imágenes responsivas](#responsive-image-properties) que tengan una propiedad `layout` o una configuración `image.layout`, y será ignorado si se establece.
...
`inferSize` puede obtener las dimensiones de una [imagen remota desde un dominio que no ha sido autorizado](/en/guides/images/#authorizing-remote-images), sin embargo, la imagen en sí permanecerá sin procesar.
...
`<Picture />` acepta todas las propiedades del [componente `<Image />`](#image-properties), incluidas las [propiedades de imagen responsiva](#responsive-image-properties), además de las siguientes:
...
Define una [imagen responsiva](#responsive-image-properties) y determina cómo debe cambiar el tamaño de la imagen cuando cambia el tamaño de su contenedor. Se puede usar para sobrescribir el valor configurado por defecto para [`image.layout`](/en/reference/configuration-reference/#imagelayout).
...
Los valores coinciden con los de `object-fit` en CSS. El valor predeterminado es `cover`, o el valor de [`image.objectFit`](/en/reference/configuration-reference/#imageobjectfit) si está configurado. Se puede usar para sobrescribir los estilos `object-fit` predeterminados."
...
Los valores coinciden con los de `object-position` en CSS. El valor predeterminado es `center`, o el valor de [`image.objectPosition`](/en/reference/configuration-reference/#imageobjectposition) si está configurado. Se puede usar para sobrescribir los estilos `object-position` predeterminados."
...
La función `getImage()` está pensada para generar imágenes destinadas a ser usadas en otro lugar que no sea directamente en HTML, por ejemplo, en una [ruta de API](/en/guides/endpoints/#server-endpoints-api-routes). También te permite crear tu propio componente `<Image />`.

`getImage()` recibe un objeto de opciones con [las mismas propiedades que el componente Image](#image-properties) (excepto `alt`).
