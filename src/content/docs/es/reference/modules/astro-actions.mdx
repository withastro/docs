...
La utilidad `defineAction()` se usa para definir nuevas actions desde el archivo `src/actions/index.ts`. Acepta una función [`handler()`](#handler-property) que contiene la lógica del servidor a ejecutar, y una propiedad opcional [`input`](#input-validator) para validar los parámetros de entrada en tiempo de ejecución.
...
<ReadMore>Mira la [guía avanzada de sesiones](/es/guides/actions/#advanced-persist-action-results-with-a-session) para ver una implementación de ejemplo usando Netlify Blob.</ReadMore>
...
La utilidad `getActionPath()` acepta una action y devuelve una ruta URL para que puedas ejecutar una llamada a la action usando `fetch()` directamente. Esto te permite proporcionar detalles como encabezados personalizados cuando llamas a tu action. Luego, puedes [manejar los datos devueltos con formato personalizado](/es/guides/actions/#handling-returned-data) según sea necesario, como si hubieras llamado a la action directamente.
