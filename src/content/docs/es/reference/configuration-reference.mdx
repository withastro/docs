...
Controla si los estilos se envían al navegador en un archivo de css independiente o dentro de la etiqueta `<style>`. Elije entre las siguientes opciones:
 - `'always'` - todos los estilos se incrustan en etiquetas `<style>`.
 - `'auto'` - sólo las hojas de estilo más pequeñas que `ViteConfig.build.assetsInlineLimit` (por defecto: 4kb) se incrustan. De lo contrario, los estilos se envían en hojas de estilo externas.
...
Permite que las integraciones utilicen la [API experimental `addClientDirective`](/en/reference/integrations-reference/#addclientdirective-option) en el hook `astro:config:setup` para añadir directivas de cliente personalizadas en los archivos de Astro.
...
Para activar esta función, establece `experimental.middleware` en `true` en tu configuración de Astro:
...
Habilita el soporte experimental para SSR híbrido con pre-renderizado habilitado por defecto.
...
Luego añade `export const prerender = false` a cualquier página o endpoint que desees excluir del pre-renderizado.
