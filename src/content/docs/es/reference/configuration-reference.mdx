...
Por defecto, el valor de `import.meta.env.BASE_URL` incluye una barra diagonal al final. Si tienes la opción [`trailingSlash`](/en/reference/configuration-reference/#trailingslash) establecida en `'never'`, deberás agregarla manualmente en tus importaciones de activos estáticos y URLs.
