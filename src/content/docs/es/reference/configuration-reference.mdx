...
Para recuperar todos los activos cargados en el mismo dominio (por ejemplo, `https://cdn.example.com/_astro/...`), establezca `assetsPrefix` en el dominio raíz como cadena (independientemente de su configuración `base`):
...
También puede pasar un objeto a `assetsPrefix` para especificar un dominio diferente para cada tipo de fichero.
...
Sin embargo, esto significa que los scripts ya no se colocan en el `<head>` y que varios scripts de una página ya no se agrupan.
Si activa esta opción, compruebe que todas las etiquetas `<script>` se comportan como es debido.
...
Alternativamente, puede establecer esto en su [VSCode `json.schemas` settings](https://code.visualstudio.com/docs/languages/json#_json-schemas-and-settings):
...
Tenga en cuenta que esta implementación inicial utiliza una biblioteca con [problemas conocidos para esquemas Zod avanzados](https://github.com/StefanTerdell/zod-to-json-schema#known-issues), por lo que puede que desee consultar estas limitaciones antes de activar la bandera experimental.
