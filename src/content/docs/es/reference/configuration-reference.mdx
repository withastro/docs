...
```
Ambas rutas de página construidas y URLs devueltas por las funciones auxiliares `astro:i18n` [`getAbsoluteLocaleUrl()`](/es/reference/api-reference/#getabsolutelocaleurl) y [`getAbsoluteLocaleUrlList()`](/es/reference/api-reference/#getabsolutelocaleurllist) utilizarán las opciones establecidas en `i18n.domains`.
