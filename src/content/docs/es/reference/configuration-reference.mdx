...
Controla si los estilos se envían al navegador en un archivo de css independiente o dentro de la etiqueta `<style>`. Elige entre las siguientes opciones:
...
Permite que las integraciones utilicen la [API experimental `addClientDirective`](/es/reference/integrations-reference/#addclientdirective-option) en el hook `astro:config:setup` para añadir directivas de cliente personalizadas en los archivos de Astro.
