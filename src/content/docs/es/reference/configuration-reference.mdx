...
Establece el directorio para almacenar en caché los artefactos de construcción. Los archivos de este directorio se utilizarán en compilaciones posteriores para acelerar el tiempo de compilación.
...
Esta es una opción para minificar tu salida HTML y reducir el tamaño de tus archivos HTML. Cuando está activada, Astro elimina todos los espacios en blanco de tu HTML, incluidos los saltos de línea, de los componentes `.astro`. Esto ocurre tanto en el modo de desarrollo como en la compilación final.
...
Especifica la estrategia utilizada para delimitar los estilos dentro de los componentes de Astro. Elije entre:
...
- 'hybrid' - Construye un sitio estático con unas pocas páginas renderizadas en el lado del servidor.
...
Controla si los estilos se envían al navegador en un archivo css independiente o dentro de la etiqueta `<style>`. Elije entre las siguientes opciones:
 - `'always'` - todos los estilos se colocan dentro de las etiquetas `<style>`.
 - `'auto'` - sólo las hojas de estilo de tamaño inferior a `ViteConfig.build.assetsInlineLimit` (default: 4kb) se alinean. De lo contrario, los estilos se envían en hojas de estilo externas.
...
Permitir que las integraciones utilicen el [experimental `addClientDirective` API](/en/reference/integrations-reference/#addclientdirective-option) en el hook `astro:config:setup` para añadir directivas de cliente personalizadas en los archivos de Astro.
Para activar esta función, establece `experimental.customClientDirectives` en `true` en tu configuración de Astro:
...
Activa la compatibilidad experimental con el middleware de Astro.

Para activar esta función, establece `experimental.middleware` en `true` en tu configuración Astro:
...
Habilitar soporte experimental para SSR híbrido con pre-renderizado habilitado por defecto.

Para activar esta función, primero establece `experimental.hybridOutput` como `true` en tu configuración de Astro, y establece `output` como `hybrid`.
...
A continuación, añade `export const prerender = false` a cualquier página o punto final que desees excluir del pre-renderizado.
