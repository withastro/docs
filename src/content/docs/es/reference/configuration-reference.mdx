...
Una lista de todos los idiomas admitidos por el sitio web (por ejemplo, `['en', 'es', 'pt_BR']`). Esta lista también debe incluir el `defaultLocale`. Este es un campo obligatorio.
...
El siguiente ejemplo configura tu estrategia de reserva de contenido para redirigir las páginas no disponibles en `/pt/` a su versión `es`, y las páginas no disponibles en `/fr/` a su versión `en`. Las páginas no disponibles en `/es/` devolverán un 404.
...
Controla la estrategia de enrutamiento para determinar las URL de tu sitio.

 - `prefix-other-locales`(default): Solo los idiomas no predeterminados mostrarán un prefijo de idioma. El `defaultLocale` no mostrará un prefijo de idioma.
...
Nota: Astro requiere que todo el contenido exista dentro de una carpeta `/[lang]/`, incluso para el idioma predeterminado.
