...
Añade una [directiva de cliente personalizada](/es/reference/directives-reference/#directivas-de-cliente-personalizadas) para utilizarla en los archivos Astro. Como se trata de una característica experimental, también tienes que activar la opción de configuración [`experimental.customClientDirectives`](/es/reference/configuration-reference/#experimentalcustomclientdirectives) para utilizar esta API.
