...
#### Opción `addDevOverlayPlugin`

<Since v="3.4.0" />

**Tipo:** `(pluginEntrypoint: string) => void;`

Agrega un [plugin de superposición de desarrollo personalizado](/es/reference/dev-overlay-plugin-reference/).
...
```js title="astro-dev-overlay-plugin/integration.js"
/**
 * @type {() => import('astro').AstroIntegration}
 */
export default () => ({
...
```js title="astro-dev-overlay-plugin/plugin.js"
...
export default {
  ...
  icon: "<svg>...</svg>",
  init() {
    console.log("¡Soy un plugin de superposición de desarrollo!")
  },
};
```
