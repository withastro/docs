...
Añade una [directiva de cliente personalizada](/es/reference/directives-reference/#directivas-de-cliente-personalizadas) para utilizarla en los archivos Astro. Como se trata de una función experimental, también tienes que activar la opción de configuración [`experimental.customClientDirectives`](/en/reference/configuration-reference/#experimentalcustomclientdirectives) para utilizar esta API.

Ten en cuenta que los puntos de entrada de la directiva sólo se empaquetan a través de esbuild y deben ser pequeños para que no ralenticen la hidratación de los componentes.
