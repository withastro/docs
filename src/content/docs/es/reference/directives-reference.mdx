...
`set:html` se puede utilizar en cualquier etiqueta y no es necesario incluir HTML. Por ejemplo, puede utilizarse con [`JSON.stringify()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) en una etiqueta `<script>` para agregar un esquema [JSON-LD](https://json-ld.org/) a su página.
