...
Las aplicaciones de barra de herramientas para desarrollo sólo pueden ser agregadas por [las integraciones de Astro](/es/guides/integrations-guide/) usando [el hook `astro:config:setup`](/en/reference/integrations-reference/#astroconfigsetup). Necesitarás crear la aplicación de barra de herramientas y la integración que agregará la barra de herramientas a tu proyecto de Astro existente.
...
2. En `my-integration.ts`, agrega el siguiente código para proveer el nombre de tu integración y la [función `addDevToolbarApp()`](/en/reference/dev-toolbar-app-reference/#toolbar-app-integration-setup) que añadirá tu aplicación de barra de herramientas con el hook `astro:config:setup`:
