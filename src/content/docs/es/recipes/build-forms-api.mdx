...
Un formulario HTML hace que el navegador se actualice o navegue a una nueva página. Para enviar datos de formulario a un punto final de API en su lugar, debe interceptar el envío del formulario usando JavaScript.

Esta receta muestra como enviar datos de un formulario a un punto final de API y manejar esos datos.
...
1. Crea un componente de formulario usando tu framework UI. Cada entrada debe tener un atributo `name` que describa el valor de esa entrada.
  ...
  3. Crea una función que acepte un evento de envío, luego pásalo como un manejador de eventos `submit` a tu formulario. En la función, llama a `preventDefault` en el evento para anular el proceso de envío predeterminado del navegador.
      ...
      Luego, crea un objeto `FormData` y envíalo a tu punto final usando `fetch`.
