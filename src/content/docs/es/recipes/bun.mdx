...
Bun incluye un ejecutor de pruebas rápido, integrado y compatible con Jest a través del comando `bun test`. Si quieres usarlo lee la [documentación de `bun test`](https://bun.sh/docs/cli/test).

Sin embargo, también es posible utilizar Cypress o Playwright para un enfoque moderno de las pruebas de aplicaciones web.

### Cypress

Cypress es una herramienta de pruebas front-end y tiene la misión de "hacer agradable la experiencia de las pruebas y generar felicidad en los desarrolladores". Esto le permite escribir pruebas de extremo a extremo para tu sitio Astro.
...
Para el resto de la configuración y para iniciar tu primera prueba, sigue el resto del proceso de Cypress en la [Guía de pruebas de Astro](/es/guides/testing/#configuración).

### Playwright

Playwright es un framework de pruebas de extremo a extremo que te permite probar tu código Astro en todos los motores de renderizado modernos, incluidos Chromium, WebKit y Firefox.
...
Para crear tu primera prueba Playwright, sigue las instrucciones para el resto del proceso Playwright en la [Guía de pruebas de Astro](/es/guides/testing/#cree-su-primer-test-con-playwright).
