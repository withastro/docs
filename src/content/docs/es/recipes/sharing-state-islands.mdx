...
- Escribir en un store desde un archivo `.astro` o un [componente no-hidratado](/es/guides/framework-components/#hydrating-interactive-components) _no_ afectará el valor recibido por un [componente del lado del cliente](/es/reference/directives-reference/#directivas-del-cliente).
