...
Los [endpoints del servidor](/es/guides/endpoints/#endpoints-del-servidor-rutas-de-api) se pueden utilizar como endpoints de API REST para ejecutar funciones como autenticaciones, acceso a bases de datos y verificaciones sin exponer datos sensibles al cliente.

En esta receta, se utiliza una ruta de API para verificar Google reCAPTCHA v3 sin exponer el secreto a los clientes.

## Prerrequesitos
- Un proyecto con [SSR](/es/guides/on-demand-rendering/) habilitado, con la opción `output: 'server'`.
