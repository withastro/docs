...
:::tip
¿Estás usando componentes de framework? Mira [como compartir el estado entre Islands](/es/core-concepts/sharing-state/)!
:::
...
2. Crea una tienda. En este ejemplo, la tienda rastrea si un diálogo está abierto o no:
...
3. Importa y utiliza la tienda en una etiqueta `<script>` en los componentes que compartirán el estado.
...
Los siguientes componentes `Button` y `Dialog`  utilizan el estado compartido `isOpen` para controlar si un `<div>` en particular está oculto o mostrado:
  ...
  <script>
    ...
    // Configura la tienda a true cuando se hace clic en el botón
    function openDialog() {
      isOpen.set(true);
    }
  ...
  <script>
    ...
    // Escucha los cambios en la tienda y muestra/oculta el diálogo en consecuencia
