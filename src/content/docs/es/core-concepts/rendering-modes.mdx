...
[Considera habilitar el modo `server` o `hybrid`](/es/guides/server-side-rendering/#habilitar-bajo-demanda-renderizado-del-servidor) en tu proyecto de Astro si necesitas lo siguiente:

- **Enpoints de API**: Crea páginas específicas que funcionen como endpoints de API para tareas como el acceso a la base de datos, la autenticación y la autorización, manteniendo los datos sensibles ocultos del cliente.
