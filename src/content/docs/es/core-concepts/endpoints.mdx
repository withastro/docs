...
En el modo SSR,  algunos proveedores requieren que el encabezado `Content-Type` header to return an image. devuelva una imagen. En este caso, utiliza un objeto `Response` para especificar una propiedad `headers` Por ejemplo, para generar una imagen binaria en formato `.png`:
