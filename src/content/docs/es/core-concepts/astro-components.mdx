...
Los slots pueden ser transferidos a otros componentes. Por ejemplo, cuando se crean layouts anidados:
...
:::note
Slots con nombre pueden ser transferidos a otro componente usando los atributos `name` y `slot` en una etiqueta `<slot />`
:::
