...
Puedes excluir páginas o directorios de ser generados añadiendo el prefijo (`_`). Los archivos con el prefijo `_` no serán reconocidos por el router y no serán generados en el directorio `dist/`.

Puedes usar esto para temporalmente inhabilitar páginas y también para poner tests, utilidades y componentes en la misma carpeta como sus páginas correspondientes.
