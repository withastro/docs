...
Una plantilla de un componente de Astro puede renderizar múltiples elementos sin necesidad de envolver todo en un único `<div>` o `<>`, a diferencia de JavaScript o JSX.
...
Sin embargo, cuando se utiliza una expresión para crear dinámicamente múltiples elementos, debes envolver estos elementos dentro de un **fragmento** como lo harías en JavaScript o JSX. Astro admite el uso de `<Fragment> </Fragment>` o la abreviatura `<> </>`.
