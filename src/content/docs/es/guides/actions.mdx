...
<ReadMore>[Consulta la referencia de la API de `handler()`](/es/reference/modules/astro-actions/#handler-property) para más detalles.</ReadMore>
...
2. Define una action `newsletter` para manejar el formulario enviado. Valida el campo `email` usando el validador `z.string().email()`, y la casilla `terms` con `z.boolean()`:
    ...
    <ReadMore>Consulta la [referencia de la API de `input`](/es/reference/modules/astro-actions/#input-validator) para ver todos los validadores de formulario disponibles.</ReadMore>
