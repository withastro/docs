...
En el ejemplo siguiente, el estado interno del componente del recuento no se restablecerá cuando se navegue hacia adelante y hacia atrás por páginas que contengan el componente `<Counter />` con el atributo `transition:persist`.
...
Esto le permite controlar si los accesorios de una isla deben persistir o no tras la navegación.
...
Al añadir transiciones de vista a un proyecto Astro existente, es posible que algunos de sus scripts ya no se vuelvan a ejecutar tras la navegación por la página, como ocurría con las actualizaciones de página completa del navegador. Utilice la siguiente información para asegurarse de que sus scripts se ejecutan como se espera.
