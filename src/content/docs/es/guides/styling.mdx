...
Usando la estrategia por defecto de estilo acotado, también debes pasar el atributo `data-astro-cid-*`. Puedes hacer esto pasando el `...rest` de las props al componente. Si has cambiado `scopedStyleStrategy` a `'class'` o `'where'`, la prop `...rest` no es necesaria.
...
Debido a que el atributo `data-astro-cid-*` incluye al hijo en el alcance de su padre, es posible que los estilos se filtren del padre al hijo. Para evitar que esto tenga efectos secundarios no deseados, asegúrate de usar nombres de clase únicos en el componente hijo.
:::
