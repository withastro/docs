...
## Estilando en Astro \{#styling-in-astro\}
...
### Combinando clases con `class:list` \{#combining-classes-with-classlist\}
...
### Variables de CSS \{#css-variables\}
...
### Pasando una `class` a un componente hijo \{#passing-a-class-to-a-child-component\}
...
### Importando una hoja de estilo local \{#import-a-local-stylesheet\}

:::caution[¿Usando un paquete npm?]
Es posible que debas actualizar el archivo `astro.config` al importar CSS desde paquetes npm. Consulta la sección ["importando hojas de estilo desde un paquete npm"](#import-a-stylesheet-from-an-npm-package) a continuación.
:::

Puedes importar hojas de estilo en el script de tu componente de Astro utilizando la sintaxis de importación ESM. Las importaciones de CSS funcionan como [cualquier otra importación ESM en un componente de Astro](/es/basics/astro-components/#the-component-script), deben referenciarse usando **la ruta relativa al componente** y deben estar escritas en la **parte superior** del script de su componente, con cualquier otra importación.
...
### Importando una hoja de estilos desde un paquete npm \{#import-a-stylesheet-from-an-npm-package\}
...
### Carga una hoja de estilos a través de etiquetas de "link" \{#load-a-static-stylesheet-via-link-tags\}
...
Debido a que este método utiliza la carpeta `public/`, se salta el procesamiento, empaquetado y las optimizaciones de CSS que proporciona Astro. Si necesitas estas transformaciones, utiliza el método anterior [importando una hoja de estilo local](#import-a-local-stylesheet).

## Orden de Cascada \{#cascading-order\}
...
### Etiquetas link \{#link-tags\}
Las hojas de estilo cargadas mediante [etiquetas link](#load-a-static-stylesheet-via-link-tags) son evaluadas en orden, antes que cualquier otro estilo en un archivo de Astro. Por lo tanto, esos estilos tendrán menor importancia que las hojas de estilo importadas y los estilos locales:
...
## Integraciones CSS \{#css-integrations\}
...
<ReadMore>Consulta la [guía de integraciones](/es/guides/integrations-guide/) para obtener instrucciones sobre cómo instalar, importar y configurar estas integraciones de Astro.</ReadMore>
...
### En componentes de framework \{#in-framework-components\}
...
## Estilando Markdown \{#markdown-styling\}
...
Puedes aplicar estilos globales a tu contenido de Markdown agregando [hojas de estilo importadas](#external-styles) a la plantilla que envuelve el contenido de tu página. También es posible estilizar tu Markdown con etiquetas [`<style is:global>`](#global-styles) en el componente de diseño. Ten en cuenta que cualquier estilo agregado está sujeto al [orden de cascada de Astro](#cascading-order), por lo que debes revisar cuidadosamente tu página renderizada para asegurarte de que los estilos se apliquen como se pretende.

También puedes agregar [integraciones de CSS](#css-integrations), incluyendo [Tailwind](/es/guides/integrations-guide/tailwind/). Si usas Tailwind, el [plugin de tipografía](https://tailwindcss.com/docs/typography-plugin) puede ser útil para estilar Markdown.

## Producción \{#production\}

### Control de paquetes \{#bundle-control\}
...
### Importaciones `?raw` de CSS \{#raw-css-imports\}
...
###  Importaciones `?url` de CSS \{#url-css-imports\}
