...
Una vez que tengas una colección, puedes comenzar a [consultar tu contenido](#consultando-colecciones) utilizando las API de contenido integradas de Astro.
...
:::note
El archivo `src/content/config.ts` es opcional. Sin embargo, si eliges no definir tus colecciones, deshabilitarás algunas de sus mejores características como la validación del esquema del frontmatter o la escritura automática de TypeScript.
:::
...
Los esquemas hacen cumplir un frontmatter o datos de entrada consistentes dentro de una colección. Un esquema **garantiza** que estos datos existen en una forma predecible cuando necesitas hacer referencia o consultarlos. Si algún archivo viola su esquema de colección, Astro proporcionará un error útil para informarte. 

Los esquemas también alimentan las escrituras automáticas de TypeScript de Astro para tu contenido. Cuando defines un esquema para tu colección, Astro generará y aplicará automáticamente una interfaz de TypeScript. El resultado es un soporte completo de TypeScript cuando consultas tu colección, incluyendo el autocompletado de propiedades y la comprobación de tipos.

Para definir tu primer colleción, crea un archivo `src/content/config.ts` si no existe (las extensiones `.js` y `.mjs` también son compatibles). Este archivo debe:

1. **Importa las utilidades adecuadas** de `astro:content`.
2. **Define cada colección que deseas validar.** Esto incluye un `type` (introducido en Astro v2.5.0) que especifica si la colección contiene formatos de autoría de contenido como Markdown (`type: 'content'`) o formatos de datos como JSON o YAML (`type: 'data'`). También incluye un `schema` que define la forma de tu frontmatter o datos de entrada.
...
// 2. Define un `type` y `schema` para cada colección
const blogCollection = defineCollection({
...
Como tu proyecto crece, también eres libre de reorganizar tu base de código y mover la lógica fuera del archivo `src/content/config.ts`. Definir tus esquemas por separado puede ser útil para reutilizar esquemas en múltiples colecciones y compartir esquemas con otras partes de tu proyecto.
...
// 3. Exporta múltiples colecciones para registrarlas
export const collections = {
  'blog': blogCollection,
...
Puedes importar esquemas de colección desde cualquier lugar, incluidos los paquetes npm externos. Esto puede ser útil cuando trabajas con temas y bibliotecas que proporcionan sus propios esquemas de colección para que los uses.
...
// Exporta la colección de blog, usando un esquema externo de 'my-blog-theme'
export const collections = {
  'blog': blogCollection,
};
```
...
Astro usa [Zod](https://github.com/colinhacks/zod) para alimentar sus esquemas de contenido. Con Zod, Astro puede validar el frontmatter de cada archivo dentro de una colección *y* proporcionar tipos automáticos de TypeScript cuando consultas el contenido desde dentro de tu proyecto.
...
Las Entradas de colección pueden "referenciar" otras entradas relacionadas.
...
const blog = defineCollection({
  ...
  schema: z.object({
    ...
    // Referencia a una matriz de publicaciones relacionadas de la colección `blog` por `slug`
...
Cuando usas `type: 'content'`, cada entrada de contenido genera una propiedad `slug` compatible con URL a partir de su [file `id`](/en/reference/api-reference/#id). El slug se utiliza para consultar la entrada directamente desde tu colección. También es útil cuando creas nuevas páginas y URL a partir de tu contenido.
...
Cualquier [referencia definida en tu esquema](#defining-collection-references) debe consultarse por separado después de consultar por primera vez la entrada de tu colección. Puedes usar la función `getEntry()` de nuevo, o `getEntries()`, para recuperar la entrada referenciada del objeto `data` devuelto.
...
Una vez que hayas consultado tus entradas de colección, puedes acceder a cada entrada directamente dentro de la plantilla de tu componente Astro. Esto te permite renderizar HTML para cosas como enlaces a tu contenido (usando el `slug` de contenido) o información sobre tu contenido (usando la propiedad `data`).
...
Si haces esto, puedes usar la utilidad [`CollectionEntry`](/es/reference/api-reference/#colección-de-tipo-entrada) para escribir correctamente las props de tu componente usando TypeScript. Esta utilidad toma un argumento de string que coincide con el nombre del esquema de tu colección y heredará todas las propiedades de ese esquema de colección.
...
Las Colecciones de contenido se almacenan fuera del directorio `src/pages/`. Esto significa que no se generan rutas para los elementos de tu colección de forma predeterminada. Deberás crear manualmente una nueva [ruta dinámica](/es/core-concepts/routing/#dynamic-routes) para generar páginas HTML a partir de las entradas de tu colección. Tu ruta dinámica asignará el parámetro de solicitud entrante (por ejemplo, `Astro.params.slug` en `src/pages/blog/[...slug].astro`) para recuperar la entrada correcta dentro de una colección.
...
:::note
Si tu slugs personalizados contienen el carácter `/` para producir URL con múltiples segmentos de ruta, debes usar un [parámetro rest (`[...slug]`)](/es/core-concepts/routing/#parámetros-rest) en el nombre de archivo `.astro` para esta página de enrutamiento dinámico.
:::
...
1. [Actualiza](/es/guides/upgrade-to/v2/) a Astro v2.0 o posterior, y actualiza todas las integraciones a sus últimas versiones.
...
5. [Genera las rutas desde tus colecciones](/es/guides/content-collections/#generando-rutas-desde-el-contenido). Dentro de una colección, los archivos Markdown y MDX ya no se convierten automáticamente en páginas utilizando el [enrutamiento basado en archivos](/es/guides/markdown-content/#file-based-routing) de Astro, por lo que debes generar las páginas tú mismo.
...
Astro admite los plugins remark o rehype que [modifican tu frontmatter directamente](/es/guides/markdown-content/#modificando-el-frontmatter-programáticamente). Puedes acceder a este frontmatter modificado dentro de una entrada de contenido usando la propiedad `remarkPluginFrontmatter` devuelta de `render()`:
