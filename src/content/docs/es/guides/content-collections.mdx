...
Ambas funciones devuelven entradas de contenido tal como se definen en el tipo [`CollectionEntry`](/es/reference/api-reference/#colección-de-tipo-entrada).
...
Si haces esto, puedes usar la utilidad [`CollectionEntry`](/es/reference/api-reference/#colección-de-tipo-entrada) para tipar correctamente las props de tu componente usando TypeScript. Esta utilidad toma un argumento de tipo string que coincide con el nombre del esquema de tu colección y heredará todas las propiedades de ese esquema de colección.
