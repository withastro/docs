...
Llama `getCollection()` dentro de `getStaticPaths()` para [consultar tu contenido o tu colección de datos](/en/guides/content-collections/#consultado-colecciones). Luego, crea tus nuevas rutas URL usando la propiedad `slug` (colecciones de contenido) o la propiedad `id`(colecciones de datos) para cada entrada de contenido.
