...
Cuando usas `type: 'content'`, cada entrada de contenido genera una propiedad `slug` compatible con URL a partir de su [file `id`](/es/reference/api-reference/#id). El slug se utiliza para consultar la entrada directamente desde tu colección. También es útil cuando creas nuevas páginas y URL a partir de tu contenido.
...
Both functions return content entries as defined by the [`CollectionEntry`](/es/reference/api-reference/#colección-de-tipo-entrada) type.
Ambos
