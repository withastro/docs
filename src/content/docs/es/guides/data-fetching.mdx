...
Todos los [componentes de Astro](/es/core-concepts/astro-components/) tienen acceso a la [función global `fetch()`](https://developer.mozilla.org/es/docs/Web/API/fetch) en su script de componente para hacer peticiones HTTP a APIs usando la URL completa (e.g. https://example.com/api o `Astro.url + "/api"`).
