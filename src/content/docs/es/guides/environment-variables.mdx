...
Cuando se utiliza SSR, las variables de entorno pueden ser accedidas en tiempo de ejecución según el adaptador SSR que se esté utilizando. Con la mayoría de los adaptadores, puedes acceder a las variables de entorno con `process.env`, pero para el adaptador Deno, debes utilizar `Deno.env.get()`. Cloudflare tiene su [propia forma](/es/guides/integrations-guide/cloudflare/#variables-de-entorno) de manejar las variables de entorno.
Astro verificará el entorno del servidor en busca de variables; si no existen, buscará en archivos .env.
