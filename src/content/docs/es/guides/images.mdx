...
Utiliza la sintaxis estándar de Markdown `![alt](src)` en tus archivos `.md`. Tus imágenes locales almacenadas en `src/` y las imágenes remotas serán procesadas y optimizadas. Cuando [configures imágenes responsivas globalmente](/en/reference/configuration-reference/#imagelayout), estas imágenes también serán [responsivas](#imagenes-responsivas).
...
Cuando este comportamiento responsivo se [configura globalmente](/en/reference/configuration-reference/#imagelayout), se aplicará a todos los componentes de imagen y también a cualquier imagen local y remota que utilice [la sintaxis Markdown `![]()`](/es/guides/images/#imágenes-en-archivos-markdown).
