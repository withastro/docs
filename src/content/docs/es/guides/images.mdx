...
Sin embargo, puedes pasar el contenido estático generado por `<Image />` a un componente de framework dentro de un archivo `.astro` como hijos o utilizando un [`<slot/>` nombrado](/es/guides/framework-components/#can-i-use-astro-components-inside-my-framework-components):
...
La función `getImage()` está destinada a generar imágenes que se utilizarán en otro lugar que no sea directamente en HTML, por ejemplo, en una [Ruta API](/es/guides/endpoints/#server-endpoints-api-routes). También te permite crear tu propio componente `<Image />` personalizado.
...
1. Elimina la integración `@astrojs/image`.
    ...
    Debes [eliminar la integración](/es/guides/integrations-guide/#removing-an-integration) desinstalándola y luego eliminándola de tu archivo `astro.config.mjs`.
