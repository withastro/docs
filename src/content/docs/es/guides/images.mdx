...
Utiliza la sintaxis estándar de Markdown `![alt](src)` en tus archivos `.md`. Tus imágenes locales almacenadas en `src/` y las imágenes remotas serán procesadas y optimizadas. Cuando [configures imágenes responsivas globalmente](/es/reference/configuration-reference/#opciones-de-imagen), estas imágenes también serán [responsivas](#imagenes-responsivas).
...
Cuando este comportamiento responsivo se [configura globalmente](/es/reference/configuration-reference/#opciones-de-imagen), se aplicará a todos los componentes de imagen y también a cualquier imagen local y remota que utilice [la sintaxis Markdown `![]()`](/es/guides/images/#imágenes-en-archivos-markdown).
...
Configurar [`image.responsiveStyles: true`](/es/reference/configuration-reference/#opciones-de-imagen) aplica un pequeño número de estilos globales para garantizar que tus imágenes se redimensionen correctamente. En la mayoría de los casos, querrás habilitar esto como valor predeterminado; tus imágenes no serán responsivas sin estilos adicionales.
...
Las imágenes locales deben importarse desde la ruta relativa del archivo `.astro` existente, o puedes configurar y usar un [alias de importación](/es/guides/imports/#alias). Luego, puedes acceder a las propiedades `src` y otras de la imagen para usarlas en la etiqueta `<img>`.
