...
2. Actualizar tipos (si es necesario).
		...
		Si tenías tipos especiales configurados para `@astrojs/image` en `src/env.d.ts`, es posible que necesites cambiarlos de nuevo a los tipos predeterminados de Astro si tu actualización a la versión 3 no completó este paso por ti.
...
3. Migre cualquier componente `<Imagen />` existente.
