...
Astro proporciona componentes integrados [`<Image />`](#image-) y [`<Picture />`](#picture-), procesamiento de la [sintaxis de imágenes Markdown](#images-in-markdown-files) (`![]()`), [Componentes SVG](#svg-components), y [una función generadora de imágenes](#generating-images-with-getimage) para optimizar y/o transformar tus imágenes. Además, puedes configurar [redimensionamiento automático de imágenes responsivas](#responsive-image-behavior) de forma predeterminada, o establecer propiedades responsivas en componentes de imágenes e imágenes individuales.
...
El lenguaje de plantillas de Astro te permite renderizar imágenes optimizadas con el componente [`<Image />`](/es/reference/modules/astro-assets/#image-) y generar múltiples tamaños y formatos con el componente [`<Picture />`](/es/reference/modules/astro-assets/#picture-). Ambos componentes también aceptan [propiedades de imagen responsivas](#responsive-image-behavior) para redimensionarse según el tamaño del contenedor y responder al tamaño y la resolución de la pantalla del dispositivo.

Además, puedes importar y utilizar [archivos SVG como componentes de Astro](#svg-components) en componentes `.astro`.

Todas las etiquetas HTML nativas, incluidas `<img>` y `<svg>`, también están disponibles en componentes `.astro`. [Las imágenes renderizadas con etiquetas HTML](#display-unprocessed-images-with-the-html-img-tag) no serán procesadas (por ejemplo, optimizadas, transformadas) y se copiarán en tu carpeta de construcción tal cual.
...
Utiliza la sintaxis estándar de Markdown `![alt](src)` en tus archivos `.md`. Tus imágenes locales almacenadas en `src/` y las imágenes remotas serán procesadas y optimizadas. Cuando [configures imágenes responsivas globalmente](/es/reference/configuration-reference/#imagelayout), estas imágenes también serán [responsivas](#responsive-image-behavior).
...
Puedes utilizar los componentes `<Image />` y `<Picture />` de Astro en tus archivos `.mdx` importando tanto el componente como tu imagen. Úsalos tal y como se utilizan [en los archivos `.astro`](#images-in-astro-files). La etiqueta JSX `<img />` también es compatible con imágenes sin procesar y [utiliza la misma importación de imágenes que la etiqueta HTML `<img>`](#display-unprocessed-images-with-the-html-img-tag).

Además, se admite la [sintaxis estándar de Markdown `![alt](src)`](#images-in-markdown-files) sin necesidad de importarla.
...
[Las imágenes locales deben importarse primero](#display-unprocessed-images-with-the-html-img-tag) para acceder a sus propiedades de imagen, como `src`. Luego, se pueden renderizar como lo harías normalmente en la sintaxis de imagen de ese framework:
...
- [imágenes remotas configuradas](#authorizing-remote-images) de fuentes autorizadas
...
El componente `<Image />` acepta [varias propiedades de componente](/es/reference/modules/astro-assets/#image-properties) así como cualquier atributo aceptado por la etiqueta HTML `<img>`.
...
<ReadMore>Consulta los detalles de las [propiedades del componente `<Picture />`](/es/reference/modules/astro-assets/#picture-properties) en la referecia `astro:assets`</ReadMore>
...
Con [propiedades de imagen responsivas](/es/reference/modules/astro-assets/#responsive-image-properties) aplicadas a los componentes `<Image />` o `<Picture />`, Astro generará automáticamente los valores `srcset` y `sizes` requeridos para tus imágenes y aplicará los [estilos necesarios para garantizar que se redimensionen correctamente](#responsive-image-styles).

Cuando este comportamiento responsivo se [configura globalmente](/es/reference/configuration-reference/#imagelayout), se aplicará a todos los componentes de imagen y también a cualquier imagen local y remota que utilice [la sintaxis Markdown `![]()`](/es/guides/images/#images-in-markdown-files).
...
<ReadMore>Consulta los detalles de las [imágenes responsivas en la documentación de MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Guides/Responsive_images).</ReadMore>
...
Configurar [`image.responsiveStyles: true`](/es/reference/configuration-reference/#imageresponsivestyles) aplica un pequeño número de estilos globales para garantizar que tus imágenes se redimensionen correctamente. En la mayoría de los casos, querrás habilitar esto como valor predeterminado; tus imágenes no serán responsivas sin estilos adicionales.

Sin embargo, si prefieres manejar el estilo de las imágenes responsivas tú mismo, o necesitas [sobrescribir estos valores predeterminados al usar Tailwind 4](#responsive-images-with-tailwind-4), deja configurado el valor predeterminado `false`.
...
Referencia la importación predeterminada de cualquier archivo `.svg` local. Dado que esta importación se trata como un componente de Astro, debes usar las mismas convenciones (por ejemplo, capitalización) que al [usar etiquetas dinámicas](/es/reference/astro-syntax/#dynamic-tags).
...
Tu componente SVG, como `<Image />` o cualquier otro componente de Astro, no está disponible dentro de los componentes de UI framework, pero puede [ser pasado a un componente de framework](#images-in-ui-framework-components) dentro de un componente `.astro`.
...
Los CDNs de imágenes funcionan con [todas las opciones de imagen de Astro](#images-in-astro-files). Utiliza la URL completa de una imagen como el atributo `src` en el componente `<Image />`, una etiqueta `<img>` o en notación Markdown. Para la optimización de imágenes con imágenes remotas, también [configura tus dominios autorizados o patrones de URL](#authorizing-remote-images).
...
## Imagenes en colecciones de contenido
