...
Sin embargo, puedes pasar el contenido estático generado por `<Image />` a un componente de framework dentro de un archivo `.astro` como hijos o utilizando un [`<slot/>` nombrado](/es/basics/framework-components/#puedo-usar-componentes-de-astro-dentro-de-mis-componentes-de-framework):
...
La función `getImage()` está destinada a generar imágenes que se utilizarán en otro lugar que no sea directamente en HTML, por ejemplo, en una [Ruta API](/es/basics/endpoints/#endpoints-del-servidor-rutas-de-api). También te permite crear tu propio componente `<Image />` personalizado.
