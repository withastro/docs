...
Cuando tienes múltiples funciones de middleware encadenadas a través de [sequence()](#chaining-middleware), enviar una ruta a `next()` reescribirá la `Request` en su lugar y el middleware no se ejecutará de nuevo. La siguiente función de middleware en la cadena recibirá la nueva `Request` con su `context` actualizado:
```js title="src/middleware.js"
