...
`context.locals` es un objeto que contiene datos de una `Response` y que se puede manipular dentro del middleware.
...
Múltiples middlewares pueden ser unidos en un orden específico utilizando [`sequence()`](/en/reference/api-reference/#sequence):
