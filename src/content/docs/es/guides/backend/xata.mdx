...
Usando la generación de código desde la línea de comandos de Xata y eligiendo la opción de TypeScript, se genera una instancia del SDK para ti, con tipos adaptados a tu esquema de base de datos. Además, `@xata.io/client` se agregó a tu `package.json`.

Tus variables de entorno de Xata y la URL de la base de datos fueron automáticamente extraídas por la instancia del SDK, así que no hay más trabajo de configuración necesario.
...
Para consultar tus posts, importa y usa `getXataClient()` en un archivo `.astro`. El ejemplo de abajo consulta los primeros 50 posts de la base de datos de ejemplo de Xata.
...
```
Es importante notar que el SDK necesita ser generado cada vez que tu esquema cambie. Así que, evita hacer cambios a los archivos generados que crea la línea de comandos de Xata porque una vez que el esquema se actualice, tus cambios se sobrescribirán.
