...
{/* TODO: add link to: /en/guides/deploy/netlify/ and /en/guides/deploy/vercel/ */}
<Steps>
1. Añade tu repositorio a un proveedor de Git en línea (p. ej. en GitHub, GitLab, Bitbucket)

2. Elige un proveedor de alojamiento que soporte **despliegue continuo** (p. ej. Netlify o Vercel) e importa tu repositorio Git como un nuevo sitio o proyecto.
...
{/* TODO: add link to: es/reference/configuration-reference/#outdir */}
Por defecto, los archivos generados se colocarán en `dist/`. Esta ubicación se puede cambiar usando la opción de configuración `outDir`.
...
{/* TODO: add link to: /es/reference/configuration-reference/#integrations */}
- Añadido el adaptador a la importación y exportación por defecto de tu archivo `astro.config.mjs` cuando lo instalas manualmente. (¡El comando `astro add` se encargará de este paso por ti!)
:::
