...
Para activar SSR en su proyecto de Astro, puede incorporar el sofisticado [adaptador Deno][Deno adapter]. Al ejecutar el comando `astro add`, puede instalar este adaptador avanzado y simultáneamente implementar las modificaciones necesarias en su archivo `astro.config.mjs`.
...
1. Añadir [el adaptador `@astrojs/deno`][Deno adapter] a las dependencias de tu proyecto con la ayuda de su administrador de paquetes preferido. Si no está seguro de qué administrador de paquetes está utilizando o si está utilizando npm, ejecute el siguiente comando en la terminal:
...
[Adaptador Deno]: https://github.com/denoland/deno-astro-adapter
