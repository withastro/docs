...
- Incluye un campo `name` en tu `package.json`. (Esto puede ser cualquier cosa y no afectará su despliegue.)
- Incluye un script `build` en tu `package.json`. (Tu proyecto de Astro ya debería incluir esto).
- Instala el paquete [`@astrojs/node`](https://www.npmjs.com/package/@astrojs/node) y configura el script `start` en `node ./dist/server/entry.mjs`.
- Configura `astro.config.mjs` para usar `@astrojs/node` y para usar `host: true`.
