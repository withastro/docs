...
1. Crea un nuevo archivo en tu proyecto en `.github/workflows/deploy.yml` y pega el siguiente YAML.
    ...
    :::caution
    La [acción](https://github.com/withastro/action) oficial de Astro escanea el lockfile para detectar tu gestor de paquetes preferido (`npm`, `yarn`, `pnpm`, o `bun`). Debes commitear el archivo `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml` o `bun.lockb` a tu repositorio.
    :::
