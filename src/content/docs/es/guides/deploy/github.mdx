...
2. Crea un nuevo archivo en tu proyecto en `.github/workflows/deploy.yml`, dentro del mismo copia y pega el siguiente código YAML.
    ...
    :::caution
    La [action oficial de Astro](https://github.com/withastro/action) busca un archivo lockfile para detectar tu gestor de paquetes (`npm`, `yarn`, `pnpm`, or `bun`). Debes tener el archivo `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, o `bun.lockb`, generado automáticamente por tu gestor de paquetes, en tu repositorio.
    :::
