...
4. Crea un archivo llamado `.gitlab-ci.yml` en la raíz de tu proyecto con el siguiente contenido. Esto compilará y desplegará tu proyecto cada vez que realices cambios en el contenido:
   ...
   image: node:lts
    ...
    pages:
      script:
        # Especifica los pasos necesarios para construir tu aplicación aquí.
        - npm run build

      artifacts:
        paths:
          # La carpeta que contiene los archivos construidos que se van a publicar.
          # Debe llamarse "public".
          - public

      only:
        # Inicia una nueva compilación y despliegue solo cuando se realiza un "push" al repositorio.
