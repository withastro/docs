...
La página Astro a continuación, utlizando estos componentes, puede renderizar partes de la página más rápido. Las etiquetas `<head`>, `<body>` y `<h1>` ya no se bloquean por las solicitudes de datos. El servidor luego obtendrá los datos `RandomName` y `RandomFact` en paralelo y transmitirá el HTML resultante al navegador.
