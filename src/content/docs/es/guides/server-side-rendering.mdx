...
export async function GET() {
  ...
  return {
    body: JSON.stringify({ number, message: `Aquí tienes un numero aleatorio: ${number}` }),
  };
}
```
...
La función toma un [contexto de endpoint](/en/reference/api-reference/#endpoint-context) y devuelve un [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response).
