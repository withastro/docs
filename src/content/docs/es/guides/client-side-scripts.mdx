...
::: nota
El atributo `async` es valioso para scripts normales porque evita que bloqueen el renderizado. Sin embargo, los scripts del módulo ya tienen este comportamiento. Agregar `async` a un script del módulo hará que se ejecute antes de que la página se haya cargado por completo. Probablemente no es lo que desea.
:::
...
Astro empaquetará, optimizará y agregará estos scripts a la página por ti, siguiendo sus [reglas de procesado](#procesado-de-scripts).
