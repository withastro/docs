...
:::note
El comportamiento de empaquetado predeterminado de Astro se desactivará en algunas situaciones. En particular, al agregar `type=module` o cualquier otro atributo que no sea `src` a la etiqueta `<script>` hará que Astro lo trate como si tuviera una directiva `is:inline`. Lo mismo ocurrirá cuando el script esté escrito dentro de una expresión JSX.
:::
