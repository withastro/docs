...
Con [un adaptador instalado](/es/guides/on-demand-rendering/#adaptadores-de-servidor) para realizar la renderización diferida, agrega la [directiva `server:defer`](/es/reference/directives-reference/#server-directives) a cualquier componente de tu página para convertirlo en su propia isla:
...
Estos componentes pueden hacer [todo lo que normalmente harías en una página renderizada bajo demanda](/es/guides/on-demand-rendering/#on-demand-rendering-features) usando un adaptador, como obtener contenido y acceder a cookies:
