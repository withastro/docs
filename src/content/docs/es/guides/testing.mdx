...
2. Crea un archivo `index.spec.js` en la carpeta `cypress/e2e`. Usa el siguiente test en el archivo para verificar que el título de la página y el encabezado son correctos.
    ...
    Puedes establecer [`"baseUrl": "http://localhost:3000"`](https://docs.cypress.io/guides/end-to-end-testing/testing-your-app#Step-3-Configure-Cypress) en el archivo de configuración `cypress.config.ts` para usar `cy.visit("/")` en lugar de `cy.visit("http://localhost:3000/")` para una URL más conveniente.
    :::
