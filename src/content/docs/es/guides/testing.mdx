...
1. Escoge una página para probar. Este ejemplo probará la página de ejemplo `index.astro` de abajo.
...
2. Crea un archivo `index.spec.js` en la carpeta `cypress/e2e`. Usa el siguiente test en el archivo para verificar que el título de la página y el encabezado son correctos.
    ...
    :::tip[establecer un baseUrl]
    Puedes establecer [`"baseUrl": "http://localhost:3000"`](https://docs.cypress.io/guides/end-to-end-testing/testing-your-app#Step-3-Configure-Cypress) en el archivo de configuración `cypress.config.ts` para usar `cy.visit("/")` en lugar de `page.goto("http://localhost:3000/")` para una URL más conveniente.
    :::
...
Para más información sobre Cypress puedes encontrarla en los siguientes enlaces:
