...
<Tabs group="typescript-version">
  <TabItem label="TypeScript ≤4.9">
		Establece [`importsNotUsedAsValues`](https://www.typescriptlang.org/tsconfig#importsNotUsedAsValues) en `"error"`. TypeScript verificará tus importaciones y te indicará cuándo se debe utilizar `import type`. Esta configuración se incluye de forma predeterminada en nuestras plantillas `strict` y `strictest`.
  ...
  <TabItem label="TypeScript ≥5.0">
		Establece [`verbatimModuleSyntax`](https://www.typescriptlang.org/tsconfig#verbatimModuleSyntax) en `true`. TypeScript verificará tus importaciones y te indicará cuándo se debe utilizar `import type`.
