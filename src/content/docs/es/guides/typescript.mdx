...
Tenga en cuenta que `window` está solo disponible en el código del lado del cliente. `globalThis` está disponible tanto en el lado del servidor como en el lado del cliente, pero su valor del lado del servidor no se compartirá con el cliente.

Si tu solo quieres tipar una propiedad en el objeto `window`, proporciona una interfaz `Window` en su lugar:
