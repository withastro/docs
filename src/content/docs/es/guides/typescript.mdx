...
Astro tiene compatibilidad integrada con [TypeScript](https://www.typescriptlang.org/). Puedes importar archivos `.ts` y `.tsx` en tu proyecto de Astro, escribir código TypeScript directamente dentro del [componente de Astro](/es/basics/astro-components/#the-component-script), e incluso utilizar un archivo de configuración [`astro.config.ts`](/es/guides/configuring-astro/#the-astro-config-file)  si así lo deseas.
...
Astro es compatible con [alias de importación](/es/guides/imports/#aliases) definidos en la configuración `tsconfig.json` o `jsconfig.json` usando `paths`.
