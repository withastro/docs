...
El campo `items` acepta una lista de objetos de feed RSS, cada uno con un `link`, `title` y `pubDate` requeridos. También se pueden incluir tres valores opcionales: `description` (un breve extracto), `content` (el contenido completo de tu publicación) y un campo `customData` para incluir cualquier dato adicional, como otras propiedades de frontmatter de tus publicaciones de blog.
...
Estiliza tu RSS feed para proveer una experiencia de usuario más placentera a la hora de ver el archivo en el navegador.
