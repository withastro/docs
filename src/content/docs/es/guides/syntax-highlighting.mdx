...
- Todas las [secciones de código (\`\`\`)](#bloques-de-código-en-markdown ) utilizadas en archivos Markdown o MDX.
- El contenido dentro del [componente integrado `<Code />`](#code-) (impulsado por Shiki) en archivos `.astro`.
- El contenido dentro del [componente `<Prism />`](#prism-) (impulsado por Prism) en archivos `.astro`.

Puedes agregar [integraciones de la comunidad como Expressive Code](https://astro.build/integrations/?search=syntax+highlight) para obtener aún más opciones de marcado y anotación en tus bloques de código.
...
Puedes configurar cualquier tema [tema incorporado de Shiki](https://shiki.style/themes) para los bloques de código en Markdown en tu configuración de Astro:
