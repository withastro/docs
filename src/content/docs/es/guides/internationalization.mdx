...
Para habilitar esto en tu proyecto, [configura el enrutamiento i18n](#configuración-de-rutas-i18n) con tus preferencias si aún no lo has hecho. Luego, establece la bandera `experimental.i18nDomains` en `true` y agrega `i18n.domains` para mapear cualquiera de tus `locales` admitidos a URLs personalizadas:
