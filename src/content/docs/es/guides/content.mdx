...
Los archivos Markdown y MDX que viven en tu directorio `src/pages` generarán automáticamente páginas en tu proyecto utilizando el [enrutamiento basado en archivos](/es/basics/routing/) de Astro, creado en una URL correspondiente a la ruta del archivo de la publicación. 
...
Si estás escribiendo tu contenido en un CMS, puedes obtener tus publicaciones y usar el [enrutamiento dinámico](/es/basics/routing/#rutas-dinámicas) para usar un archivo `.astro` para generar una ruta para cada publicación. En el modo estático predeterminado de Astro, estas rutas se generan en el momento de la compilación. Si optas por el [modo SSR](/es/guides/server-side-rendering/), respondes responde a una petición en tiempo de ejecución y obtienes el contenido a bajo petición.
