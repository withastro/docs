...
Primero, comprueba que has **importado el componente** en el [script del componente `.astro`](/es/basics/astro-components/#the-component-script) o en el [frontmatter de `.md`](/es/guides/markdown-content/#usando-componentes-en-mdx).
...
Por defecto, un [componente de framework no es hidratado en el cliente](/es/guides/framework-components/#hydrating-interactive-components). Si no se provee una directiva `client:*`, su HTML es renderizado en la página sin JavaScript.
