...
Esta **[integración de Astro][astro-integration]** permite renderización en el servidor y hidratación en el cliente para tus elementos personalizados de [Lit](https://lit.dev/).
...
Sin embargo, hay una diferencia clave con los *elementos personalizados* de Lit en comparación con los *componentes convencionales*:  puedes utilizar directamente el nombre de la etiqueta del elemento.

Astro necesita saber qué etiqueta está asociada con qué script de componente. Exponemos esto mediante la exportación de una variable `tagName` desde el script del componente. Se ve así:
