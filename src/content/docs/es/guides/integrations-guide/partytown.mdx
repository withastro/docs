...
Los scripts de terceros suelen agregar variables al objeto `window` ara que puedas comunicarte con ellos en todo tu sitio. Pero cuando un script se carga en un web worker, no tiene acceso a ese objeto `window` global.
...
*   [Explora proyectos con Astro Partytown en GitHub](https://github.com/search?q=%22@astrojs/partytown%22+filename:package.json\&type=Code) para ver más ejemplos.
