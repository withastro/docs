...
Este adaptador permite a Astro desplegar tu [sitio renderizado de manera `hybrid` o `server`](/es/basics/rendering-modes/#on-demand-rendered) en [Cloudflare](https://www.cloudflare.com/).

Si estás utilizando Astro como un [constructor de sitios estáticos](/es/basics/rendering-modes/#pre-rendered), no necesitas un adaptador.
...
Luego, añade el adaptador y el [modo de renderizado](/es/basics/rendering-modes/#on-demand-rendered) deseado a tu archivo `astro.config.mjs`:
