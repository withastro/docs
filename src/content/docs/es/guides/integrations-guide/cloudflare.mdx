...
En el modo `directory`, el adaptador compilará la parte del cliente de tu aplicación de la misma manera, pero moverá el script del worker a una carpeta `functions` en la raíz del proyecto. El adaptador solo colocará un archivo `[[ruta]].js` en esa carpeta, lo que te permite agregar plugins adicionales y middleware de páginas que se pueden incluir en el control de versiones. La documentación de Cloudflare contiene más información sobre [cómo escribir funciones personalizadas](https://developers.cloudflare.com/pages/platform/functions/).
...
Puedes acceder a todos los enlaces y variables de entorno de Cloudflare desde los componentes y rutas de la API de Astro a través de la API del adaptador.
...
Por defecto, `@astrojs/cloudflare` generará un archivo `_routes.json` que lista todos los archivos de tu carpeta `dist/` y los redirige desde el archivo `_redirects` en el array `exclude`. Esto permitirá que Cloudflare sirva archivos y procese redirecciones estáticas sin la invocación de una función. La creación de un archivo `_routes.json` personalizado anulará esta optimización automática y, si no se configura manualmente, causará invocaciones a funciónes que se descontarán de los límites de peticiones de tu plan de Cloudflare.
