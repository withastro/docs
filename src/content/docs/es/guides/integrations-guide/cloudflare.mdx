...
Determina que servicio de imágenes es utilizado por el adaptador. El adaptador utilizará el modo `passthrough` de forma predeterminada cuando se configure un servicio de imágenes incompatible. De lo contrario, utilizará el servicio de imágenes configurado globalmente:
...
Configura rutas adicionales para ser generadas bajo demanda por el adaptador de Cloudflare en la matriz `routes.extend.include`.
...
### Assets
Los activos construidos por Astro tienen todos un nombre con un hash y, por lo tanto, pueden recibir encabezados de caché largos. De forma predeterminada, Astro en Cloudflare añadirá un encabezado de este tipo para estos archivos.
...
// crea una instancia con anticipación para compartir el módulo
