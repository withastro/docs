...
Este adaptador permite a Astro desplegar tu [sitio renderizado de manera `hybrid` o `server`](/es/core-concepts/rendering-modes/#renderizado-bajo-demanda) en [Cloudflare](https://www.cloudflare.com/).

Si estás utilizando Astro como un [constructor de sitios estáticos](/es/core-concepts/rendering-modes/#pre-renderizado), no necesitas un adaptador.
...
Luego, añade el adaptador y el [modo de renderizado](/es/core-concepts/rendering-modes/#renderizado-bajo-demanda) deseado a tu archivo `astro.config.mjs`:
