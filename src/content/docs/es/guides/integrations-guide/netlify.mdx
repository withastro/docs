...
Agrega el adaptador de Netlify para habilitar SSR en tu proyecto de Astro con el siguiente comando `astro add`. Esto instalará el adaptador y hará los cambios apropiados en tu archivo `astro.config.mjs` en un solo paso.
...
Construimos el directorio `dist` en la base de tu proyecto. Para cambiar esto, usa la opción `dist`:
...
Nosotros checamos los tipos mime comunes para archivos de audio, imagen y video. Para incluir tipos mime específicos que deben tratarse como datos binarios, incluye la opción `binaryMediaTypes` con una lista de tipos mime binarios.
