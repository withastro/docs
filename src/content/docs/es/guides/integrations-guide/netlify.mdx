...
If you are using images hosted on another domain, you must authorize the domain or URL patterns using the [`image.domains`](/es/reference/configuration-reference/#imagedomains) or [`image.remotePatterns`](/es/reference/configuration-reference/#imageremotepatterns) configuration options:
