...
Por defecto, todas las páginas se renderizarán en la primera visita y el resultado renderizado se reutilizará para cada visita posterior hasta que vuelvas a desplegar. Para establecer un tiempo de revalidación, llama a la [`runtime.setBuildersTtl(ttl)` local](/es/guides/middleware/#locals) con la duración (en segundos).

Como un ejemplo, para el siguiente fragmento, Netlify almacenará el HTML renderizado durante 45 segundos.
...
Es importante tener en cuenta que los constructores bajo demanda ignoaran los parámetros de consulta al comprobar las páginas en caché. Por ejemplo, si `example.com/?x=y` está en caché, se servirá para `example.com/?a=b` (parámetros de consulta diferentes) y `example.com/` (sin parámetros de consulta) también.
