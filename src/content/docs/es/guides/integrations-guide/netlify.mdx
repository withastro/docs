...
### Incluir ficheros en SSR

Cuando se utiliza SSR, es posible que necesite incluir archivos específicos en sus Funciones de Netlify que no se empaquetan automáticamente durante el despliegue.
La propiedad `includeFiles` permite forzar a que los archivos se incluyan en el empaquetado de tu función.
...
Tambien puedes usar la propiedad `excludeFiles` para excluir cualquier archivo del proceso de empaquetado que de otra manera se incluiría.
