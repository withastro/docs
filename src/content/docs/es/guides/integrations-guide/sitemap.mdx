...
Con Astro Sitemap, no tienes que preocuparte por crear este archivo XML por ti mismo: la integración Astro Sitemap rastreará tus rutas generadas estáticamente y creará el archivo sitemap, incluyendo [rutas dinámicas](/es/core-concepts/routing/#dynamic-routes) como `[...slug]` o `src/pages/[lang]/[version]/info.astro` generado por `getStaticPaths()`.

Esta integración no puede generar entradas de mapa del sitio para rutas dinámicas en [modo SSR](/es/guides/server-side-rendering/).
