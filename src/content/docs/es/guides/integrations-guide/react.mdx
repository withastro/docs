...
:::tip[¿Vas a actualizar a v7.0.0?]
`@astrojs/react` v7.0.0 elimina la opción de integración `babel`. Consulta [Actualizar la integración de React a v7.0.0](#actualizar-la-integración-de-react-a-v700) para obtener instrucciones de migración.
:::
...
Por defecto, `@astrojs/react` usa [Oxc](https://oxc.rs/) para compilar tu JSX y habilitar Fast Refresh. No memoiza tus componentes ni hooks.

Establece `compiler: true` para memoizar automáticamente los componentes y hooks del cliente con el [compilador experimental de React de Oxc](https://oxc.rs/docs/guide/usage/transformer/react-compiler). Esto puede reducir los renderizados innecesarios sin tener que escribir `useMemo()`, `useCallback()`, o `React.memo()` tú mismo.

El compilador requiere la instalación de [`oxc-transform-react`](https://www.npmjs.com/package/oxc-transform-react):
...
El compilador apunta a la versión de React que tengas instalada. React 17 y 18 no incluyen los helpers de tiempo de ejecución del compilador. Si tu proyecto usa una de estas versiones, también instala [`react-compiler-runtime`](https://www.npmjs.com/package/react-compiler-runtime):
...
El compilador se aplica en cualquier lugar donde apliquen las [opciones `include` y `exclude`](#combinar-múltiples-frameworks-jsx) de la integración. Omite el renderizado en el servidor, las dependencias y los archivos `.astro`.
...
`@astrojs/react` v7.0.0 reemplaza Babel con [Oxc](https://oxc.rs/) para compilar JSX, habilitar Fast Refresh y se actualiza a `@vitejs/plugin-react` v6.

### Eliminada: opción `babel`

Configura transformaciones personalizadas de Babel con [`@rolldown/plugin-babel`](https://github.com/rolldown/plugins/tree/main/packages/babel) en [`vite.plugins`](/es/reference/configuration-reference/#vite) en lugar de la opción `babel` eliminada.
...
Para las transformaciones condicionales configuradas previamente con un callback de `babel`, consulta los [`overrides` y hooks de presets de `@rolldown/plugin-babel`](https://github.com/rolldown/plugins/tree/main/packages/babel#options).

Puedes combinar `babel()` con [`compiler: true`](#react-compiler). Si tu configuración de Babel incluye `babel-plugin-react-compiler`, elimínalo primero. Esto evita aplicar las transformaciones del React Compiler dos veces a los mismos componentes.
