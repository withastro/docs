...
Este adaptador permite a Astro desplegar tu [sitio renderizado en modo `hybrid` o `server`](/es/basics/rendering-modes/#on-demand-rendered) a [Vercel](https://www.vercel.com/).

Si utilizas Astro como un [generador de sitios estáticos](/es/basics/rendering-modes/#pre-rendered), sólo necesitarás este adaptador si utilizas servicios adicionales de Vercel (por ejemplo, [Vercel Web Analytics](https://vercel.com/docs/analytics), [Vercel Image Optimization](https://vercel.com/docs/image-optimization)). De lo contrario, no necesita un adaptador para desplegar su sitio `static`.
...
A continuación, añade el adaptador y el [modo de renderizado bajo demanda](/es/basics/rendering-modes/#on-demand-rendered) que deseas a tu archivo `astro.config.*`:
