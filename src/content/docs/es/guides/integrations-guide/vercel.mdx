...
El adaptador `@astrojs/vercel/serverless` puede crear automáticamente el middleware de Vercel Edge a partir de un middleware de Astro en tu base de código.
...
export default function ({ request, context }) {
  // has algo con la solicitud y el contexto
  return {
...
*   El middleware de Vercel Edge sera siempre la **primera** función en recibir la `Request` y la última función en recibir la `Response`. Esta es una restricción arquitectónica que sigue los [límites establecidos por Vercel](https://vercel.com/docs/concepts/functions/edge-middleware).
