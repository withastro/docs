...
Este adaptador permite a Astro desplegar su [sitio renderizado `hybrid` o `server`](/es/core-concepts/rendering-modes/#on-demand-rendered) a [Vercel](https://www.vercel.com/).

Si utiliza Astro como [constructor de sitios estáticos](/es/core-concepts/rendering-modes/#pre-rendered), no necesita un adaptador.
...
A continuación, añada el adaptador y el [modo de renderizado bajo demanda](/es/core-concepts/rendering-modes/#on-demand-rendered) que desee a su archivo `astro.config.*`:
