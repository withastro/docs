...
Aprende como desplegar tu sitio Astro en nuestro [guía de despliegue en Vercel](/es/guides/deploy/vercel/).
...
Si deseas [usar renderizado del lado del servidor (SSR)](/es/guides/server-side-rendering/), Astro requiere un adaptador que coincida con el tiempo de ejecución de tu despliegue.
...
Agrega el adaptador Vercel para habilitar SSR en tu proyecto Astro con el siguiente comando `astro add`. Esto instalará el adaptador y realizará los cambios apropiados en tu archivo `astro.config.mjs` en un solo paso.
...
:::note
Desplegar en el Edge tiene [sus limitaciones](https://vercel.com/docs/concepts/functions/edge-functions#known-limitations). Una función de borde no puede tener más de 1 MB de tamaño y no admiten API nativas de Node.js, entre otras.
:::

Puedes cambiar a donde apuntar cambiando la importación:
...
Esa esta propiedad para excluir cualquier archivo del proceso de empaquetado que de otra manera se incluirá.
...
Puedes usar el middleware de Vercel para interceptar una solicitud y redirigirla antes de enviar una respuesta. El middleware de Vercel puede ejecutarse para despliegues de borde, SSR y estáticos. No necesitas instalar `@vercel/edge` para escribir middleware, pero si necesitas instalarlo para usar características como la geolocalización. Para obtener más información, consulta la [documentación de middleware de Vercel](https://vercel.com/docs/concepts/functions/edge-middleware).
...
**Un par de paquetes complejos conocidos (ejemplo: [puppeteer](https://github.com/puppeteer/puppeteer)) no admiten el empaquetado y, por lo tanto, no funcionarán correctamente con este adaptador.** De forma predeterminada, Vercel no incluye los archivos y paquetes instalados de npm de la carpeta `./node_modules` de tu proyecto. Para abordar esto, el adaptador `@astrojs/vercel` empaqueta automáticamente tu salida de compilación final usando `esbuild`.
...
Este paquete es mantenido por el equipo principal de Astro. ¡Estás invitado a enviar un problema o PR!

## Changelog

Ve el [CHANGELOG.md](https://github.com/withastro/astro/tree/main/packages/integrations/vercel/CHANGELOG.md) para un historial de cambios en esta integración.
