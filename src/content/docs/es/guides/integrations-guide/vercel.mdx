...
```
El middleware edge tiene acceso al [`RequestContext`](https://vercel.com/docs/functions/edge-middleware/middleware-api#requestcontext) de Vercel como `ctx.locals.vercel.edge`. Si usas TypeScript, puedes obtener las tipografías adecuadas actualizando `src/env.d.ts` para usar `EdgeLocals`:
