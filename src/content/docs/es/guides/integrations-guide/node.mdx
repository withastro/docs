...
Este adaptador permite a Astro desplegar tu [sitio renderizado en modo `hybrid` o `server`](/es/core-concepts/rendering-modes/#renderizado-bajo-demanda) a objetivos de Node.

Si estás usando Astro como un [constructor de sitios estáticos](/es/core-concepts/rendering-modes/#pre-renderizado), no necesitas un adaptador.
...
Luego, agrega el adaptador y tu [modo de renderizado bajo de manda](/es/core-concepts/rendering-modes/#renderizado-bajo-demanda) deseado a tu archivo `astro.config.*`:
