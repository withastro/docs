...
Este adaptador permite a Astro desplegar tu [sitio renderizado en modo `hybrid` o `server`](/es/basics/rendering-modes/#on-demand-rendered) a objetivos de Node.

Si estás usando Astro como un [constructor de sitios estáticos](/es/basics/rendering-modes/#pre-rendered), no necesitas un adaptador.
...
Luego, agrega el adaptador y tu [modo de renderizado bajo de manda](/es/basics/rendering-modes/#on-demand-rendered) deseado a tu archivo `astro.config.*`:
...
Primero, [realiza una construcción](/es/guides/deploy/#building-your-site-locally). Dependiendo del `mode` seleccionado (ver arriba) sigue los pasos apropiados a continuación:
