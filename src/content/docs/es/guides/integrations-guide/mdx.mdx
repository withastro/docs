...
MDX te permite [usar variables, expresiones JSX y componentes dentro del contenido Markdown](/en/guides/markdown-content/#mdx-only-features) en Astro. Si tienes contenido existente escrito en MDX, esta integración te permite traer esos archivos a tu proyecto de Astro.
...
Con la integración MDX de Astro, puedes [añadir páginas MDX a tu proyecto](/en/guides/markdown-content/#markdown-and-mdx-pages) añadiendo archivos `.mdx` dentro de tu directorio `src/pages/` directory. También puedes[importar archivos `.mdx`](/en/guides/markdown-content/#importing-markdown) en archivos `.astro`.

La integración MDX de Astro agrega características adicionales a MDX estándar, incluyendo el frontmatter estilo Markdown. Esto te permite utilizar la mayoría de las características de Markdown incorporadas en Astro, como una [propiedad especial de frontmatter `layout`](/en/guides/markdown-content/#frontmatter-layout) y una [propiedad para marcar una página como borrador](/en/guides/markdown-content/#draft-pages).

Consulta cómo funciona MDX en Astro con ejemplos en nuestra [guía de Markdown y MDX](/en/guides/markdown-content/).
...
Todas las [opciones de configuración de `markdown`](/en/reference/configuration-reference/#markdown-options) excepto `drafts`  pueden ser configuradas por separado en la integración MDX. Esto incluye plugins de remark y rehype, resaltado de sintaxis y más. Las opciones se establecerán por defecto con las de tu configuración de Markdown ([consulta la opción `extendMarkdownConfig`](#extendmarkdownconfig) para modificar esto).

:::note
No hay una configuración de MDX separada para [incluir páginas marcadas como borrador en la compilación](/en/reference/configuration-reference/#markdowndrafts). Esta configuración de Markdown será respetada tanto por archivos de Markdown como de MDX y no se puede anular específicamente para archivos de MDX.
:::
...
:::caution
MDX no admite pasar plugins de remark y rehype como una cadena. En su lugar, debes instalar, importar y aplicar la función del complemento.
:::

📚 Consulta la [referencia de opciones de Markdown](/en/reference/configuration-reference/#markdown-options) para ver una lista completa de opciones.
...
Por defecto, MDX extenderá [la configuración de Markdown existente en tu proyecto](/en/reference/configuration-reference/#markdown-options). Para anular opciones individuales, puedes especificar su equivalente en la configuración de MDX.
