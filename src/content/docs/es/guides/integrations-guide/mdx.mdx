...
Todas las [opciones de configuración de `markdown`](/es/reference/configuration-reference/#opciones-de-markdow) excepto `drafts`  pueden ser configuradas por separado en la integración MDX. Esto incluye plugins de remark y rehype, resaltado de sintaxis y más. Las opciones se establecerán por defecto con las de tu configuración de Markdown ([consulta la opción `extendMarkdownConfig`](#extendmarkdownconfig) para modificar esto).
