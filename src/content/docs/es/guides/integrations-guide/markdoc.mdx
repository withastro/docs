...
Las etiquetas y nodos están restringidos a archivos `.astro`. Para incrustar componentes de interfaz de usuario del lado del cliente en Markdoc, [usa un componente `.astro` de envoltura que renderice un componente de framework](/es/basics/framework-components/#anidando-componentes-de-framework) con tu directiva `client:` deseada.
