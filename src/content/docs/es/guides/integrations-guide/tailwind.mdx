...
Astro incluye un comando `astro add` para automatizar la configuración de integraciones oficiales. Si lo prefieres, puedes [instalar integraciones manualmente](#manual-install) en su lugar.
...
### Configurando Tailwind \{#configuring-tailwind\}
...
### Configurando la Integración \{#configuring-the-integration\}
...
Ahora puedes [importar tu propio `base.css` como una hoja de estilo local](/es/guides/styling/#import-a-local-stylesheet).
...
### Las clases no existen con las directivas `@apply` \{#class-does-not-exist-with-apply-directives\}
...
### Los modificadores basados en clases no funcionan con las directivas `@apply` \{#class-based-modifiers-do-not-work-with-apply-directives\}
