...
Si has usado las instrucciones de instalación rápida y has dicho que sí a cada instrucción, verás un archivo `tailwind.config.cjs` en el directorio raíz de tu proyecto. Usa este archivo para tus cambios de configuración de Tailwind. Puedes aprender a personalizar Tailwind usando este archivo [en la documentación de Tailwind](https://tailwindcss.com/docs/configuration).
...
La integración de Astro Tailwind maneja la comunicación entre Astro y Tailwind y tiene sus propias opciones. Cambia estas en el archivo `astro.config.mjs` (*no* el archivo de configuración de Tailwind) que es donde se encuentran los ajustes de integración de tu proyecto.

#### config.path

Si quieres usar un archivo de configuración de Tailwind diferente en lugar del predeterminado `tailwind.config.(js|cjs|mjs)`, especifica la ubicación de ese archivo usando la opción `config.path` de esta integración. Si `config.path` es relativo, se resolverá en relación con la raíz.
...
Para deshabilitar este comportamiento predeterminado, establece `config.applyBaseStyles` en `false`. Esto puede ser útil si necesitas definir tu propio archivo `base.css` (para incluir una [directiva `@layer`](https://tailwindcss.com/docs/functions-and-directives#layer), por ejemplo). Esto también puede ser útil si no deseas que `base.css` se importe en cada página de tu proyecto.
...
Ahora puedes [importar tu propio `base.css` como una hoja de estilo local](/guides/styling/#importando-una-hoja-de-estilo-local).
...
[astro-ui-frameworks]: /en/core-concepts/framework-components/#usando-componente-de-framework
...
Ve el [CHANGELOG.md](https://github.com/withastro/astro/tree/main/packages/integrations/tailwind/CHANGELOG.md) para un historial de cambios en esta integración.
