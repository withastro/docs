...
<Tabs>
  <TabItem label="HTTP transmisible">
    ```json title="Configuración de MCP" {3-6}
    {
      "mcpServers": {
        "Astro docs": {
          "type": "http",
  ...
  <TabItem label="Proxy local">
    ```json title="Configuración de MCP" {3-7}
    {
      "mcpServers": {
        "Astro docs": {
          "type": "stdio",
...
[Raycast](https://www.raycast.com/) puede conectarse a servidores MCP para mejorar sus capacidades de IA. Al añadir el servidor MCP de Astro Docs, Raycast puede acceder a la documentación más reciente de Astro mientras responde a las preguntas.
...
Puedes configurarlo para usar el servidor MCP de la documentación de Astro para acceder a la documentación agregando lo siguiente a la configuración del agente de codificación de Copilot en tu repositorio, disponible en `https://github.com/<tu-org>/<tu-repo>/settings/copilot/coding_agent`:
```json title="Configuración MCP"
{
  "mcpServers": {
    "astro-docs": {
...
Cuando se detecta un agente de programación de IA en macOS y Linux, `astro dev` y a partir de la v7.2.0, `astro preview` inician automáticamente el servidor como un proceso en segundo plano independiente. Esto evita que el servidor bloquee la terminal del agente y le permite seguir trabajando mientras el servidor está en ejecución.

En Windows, o sin un agente de programación de IA, el servidor se inicia en primer plano y muestra los registros en la terminal. Aún puedes habilitar esta opción de forma explícita pasando la [flag `--background`](/es/reference/cli-reference/#--background).

Al iniciar el servidor, se escribe un archivo de bloqueo (`.astro/dev.json` o `.astro/preview.json`) que registra la URL, el puerto y el PID del servidor. Esto evita que se inicien servidores duplicados para un mismo proyecto.

Para desactivar el modo automático en segundo plano, establece la variable de entorno `ASTRO_DEV_BACKGROUND` o `ASTRO_PREVIEW_BACKGROUND` antes de ejecutar el comando:
...
<ReadMore>Consulta la referencia de la CLI para ver la lista completa de opciones y subcomandos de [`astro dev`](/es/reference/cli-reference/#astro-dev) y [`astro preview`](/es/reference/cli-reference/#astro-preview).</ReadMore>
