...
Al iniciarse el servidor, se genera un archivo de bloqueo (`.astro/dev.json` o `.astro/preview.json`) que registra la URL, el puerto y el PID del servidor. Esto evita que se inicien servidores duplicados para un mismo proyecto.

Si no estás utilizando un agente de programación por IA, el servidor se inicia como un proceso en primer plano y muestra los registros en la terminal.

Para desactivar el modo automático en segundo plano, configura la variable de entorno `ASTRO_DEV_BACKGROUND` o `ASTRO_PREVIEW_BACKGROUND` antes de ejecutar el comando:
