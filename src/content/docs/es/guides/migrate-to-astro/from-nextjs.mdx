...
## Principales similitudes entre Next.js y Astro \{#key-similarities-between-nextjs-and-astro\}
...
- Astro cuenta con soporte para [instalar paquetes NPM](/es/guides/imports/#npm-packages), incluyendo bibliotecas de React. Muchas de tus dependencias existentes funcionarán en Astro.

## Principales diferencias entre Next.js y Astro \{#key-differences-between-nextjs-and-astro\}
...
### Instalar integraciones (opcional) \{#install-integrations-optional\}
...
### Coloca tu código fuente en `src` \{#put-your-source-code-in-src\}
...
### El archivo de configuración de Astro \{#the-astro-config-file\}
...
### Consejos: Convertir archivos JSX a archivos `.astro` \{#tips-convert-jsx-files-to-astro-files\}
...
2. Cambia cualquier [sintaxis de Next o JSX a Astro](#reference-convert-nextjs-syntax-to-astro) o a los estándares web HTML. Esto incluye `<Link>`, `<Script>`, `{children}`, y `className`, por ejemplo.

3. Mueve cualquier JavaScript necesario, incluyendo las declaraciones de importación, a un ["bloque de código" (`---`)](/es/basics/astro-components/#the-component-script). Nota: JavaScript para [renderizar contenido de forma condicional](/es/basics/astro-syntax/#html-dinamico) se escribe a menudo directamente dentro de la plantilla HTML en Astro.
...
Mira [un ejemplo de un archivo Next `.js` convertido paso a paso](#guided-example-next-data-fetching-to-astro).

#### Comparación: JSX vs Astro \{#compare-jsx-vs-astro\}
...
### Migrando archivos de diseño \{#migrating-layout-files\}
...
#### Migrando desde el directorio `pages` de Next.js \{#migrating-from-nextjs-pages-directory\}
...
#### Migrando desde el directorio `/app` de Next.js \{#migrating-from-nextjs-app-directory\}
...
### Migrando páginas y publicaciones \{#migrating-pages-and-posts\}
...
#### Páginas de React \{#react-pages\}

Tus páginas JSX existentes (`.js`) de Next deberán [convertirse de archivos JSX a páginas `.astro`](#tips-convert-jsx-files-to-astro-files). No puedes usar un archivo de página JSX existente en Astro
...
#### Páginas de Markdown y MDX \{#markdown-and-mdx-pages\}
...
### Migrando Tests \{#migrating-tests\}
...
## Referencia: Cómo convertir la sintaxis de Next.js a Astro \{#reference-convert-nextjs-syntax-to-astro\}

### Migrando los enlaces de Next a Astro \{#next-links-to-astro\}
...
### Migrando las importaciones de Next a Astro \{#next-imports-to-astro\}
...
### Migrando las propiedades Children de Next a Astro \{#next-children-props-to-astro\}
...
Los componentes de React que pasan múltiples conjuntos de hijos se pueden migrar a un un componente de Astro usando [slots con nombre](/es/basics/astro-components/#named-slots).

Ver más acerca del [uso especifico de `<slot />` en Astro](/es/basics/astro-components/#slots).

### Migrando la obtención de datos de Next a Astro \{#next-data-fetching-to-astro\}
...
### Migrando el sistema de estilos de Next a Astro \{#next-styling-to-astro\}
...
Si es necesario, convierte cualquier objeto de estilo en línea (`style={{ fontWeight: "bold" }}`) a atributos de estilo HTML en línea (`style="font-weight:bold;"`). O, usa una [etiqueta `<style>` de Astro](/es/guides/styling/#styling-in-astro) para estilos CSS con ámbito limitado.
...
### Plugin de Imagen de Next a Astro \{#next-image-plugin-to-astro\}
...
## Ejemplo guiado: Cómo migrar la obtención de datos en Next a Astro \{#guided-example-next-data-fetching-to-astro\}
