...
- La [sintaxis de los archivos `.astro` es similar a la de JSX](/es/guides/server-side-rendering/#renderizado-híbrido). Escribir en Astro debería resultarte familiar.
- Los proyectos de Astro también pueden ser SSG o [SSR con prerenderización a nivel de página](/es/guides/server-side-rendering/#renderizado-híbrido).
...
- [MPA vs SPA](/es/concepts/mpa-vs-spa/): Next.js es una aplicación de página única de React, y utiliza `index.js` como raíz de tu proyecto. Astro es un sitio multipágina, y `index.astro` es tu página de inicio.
...
- [Enfoque en el contenido](/es/concepts/why-astro/): Astro fue diseñado principalmente para sitios enfocados en el contenido con pequeñas piezas de interactividad. Una aplicación existente de Next.js puede estar construida para una alta interactividad del lado del cliente y puede incluir elementos que son difíciles de replicar usando componentes de `.astro`, como paneles de control.
...
Cada página de Astro requiere explícitamente que se incluyan las etiquetas `<html>`, `<head>`, y `<body>`, por lo que es comñun reutilizar un archivo de diseño en varias páginas. Astro utiliza [`<slot />`](/es/core-concepts/astro-components/#slots) para el contenido de la página, sin necesidad de importar ninguna declaración. Observa la estructura estándar de templating de HTML y el acceso directo al elemento `<head>`:  
...
Como Astro produce HTML sin procesar, es posible escribir tests end-to-end utilizando la salida del paso de compilación. Cualquier test end-to-end escrito anteriormente  podría funcionar sin problemas si has podido coincidir con el marcado de tu sitio de Next. Bibliotecas de testing como Jest y React Testing Library pueden ser importadas y utilizadas en Astro para testear tus componentes de React.
...
Astro no utiliza ningún componentes especial para los enlaces, aunque puedes crear tu propio componente `<Link>`. Luego puedes importar y usar este `<Link>` como lo harías con cualquier otro componente.
...
Actualiza cualquier [importación de archivos](/es/guides/imports/) para referencias rutas de archivo relativas de maneta exacta. Esto puede hacerse utilizando un [alias de importación](/es/guides/typescript/#alias-de-importación), o escribiendo la ruta relativa completa. 

Ten encuenta que `.astro` y varios otros tipos de archivos deben ser importados con su extensión de achivo completa.
...
Ver más acerca de [importaciones de archivos locales con `Astro.glob()`](/es/guides/imports/#astroglob), [consultas usando la API de colecciones](/es/guides/content-collections/#querying-collections) o [obtención de datos remotos](/es/guides/data-fetching/).
