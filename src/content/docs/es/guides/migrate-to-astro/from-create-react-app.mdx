...
<ReadMore>Consulta cómo [crear una aplicación de una sola página (SPA) con Astro](https://logsnag.com/blog/react-spa-with-astro) <Badge text="Externo" />utilizando React Router.</ReadMore>
...
## Principales similitudes entre CRA y Astro \{#key-similarities-between-cra-and-astro\}
...
- Astro tiene soporte para [instalar paquetes de NPM](/es/guides/imports/#npm-packages), incluidas bibliotecas de React. Muchas de tus dependencias existentes funcionarán en Astro.

## Principales diferencias entre CRA y Astro \{#key-differences-between-cra-and-astro\}
...
- [Orientado al contenido](/es/concepts/why-astro/#content-driven): Astro fue diseñado para resaltar tu contenido y permitirte optar por la interactividad solo cuando sea necesario. Una aplicación existente de CRA puede estar diseñada para una alta interactividad del lado del cliente y puede requerir técnicas avanzadas de Astro para incluir elementos que son más desafiantes de replicar utilizando componentes `.astro`, como paneles de control.

## Agrega tu CRA a Astro \{#add-your-cra-to-astro\}
...
### Agregar integraciones y dependencias \{#add-integrations-and-dependencies\}
...
### Agrega los archivos de tu aplicación existente \{#add-your-existing-app-files\}
...
### Renderizar tu app \{#render-your-app\}
...
## Convierte tu CRA a Astro \{#convert-your-cra-to-astro\}

Después de [agregar tu aplicación existente a Astro](#add-your-cra-to-astro), ¡probablemente querrás convertir la propia aplicación a Astro!
...
### Compara: JSX vs Astro \{#compare-jsx-vs-astro\}
...
### Convirtiendo archivos JSX a archivos `.astro` \{#converting-jsx-files-to-astro-files\}
...
2. Convierte cualquier [sintaxis CRA o JSX a Astro](#reference-convert-cra-syntax-to-astro) o a los estándares web de HTML. Esto incluye `{children}` y `className`, por ejemplo.

3. Mueve cualquier JavaScript necesario, incluyendo declaraciones de importación, dentro de una ["valla de código" (`---`)](/es/basics/astro-components/#the-component-script). Nota: El JavaScript para [renderizar contenido condicional](/es/basics/astro-syntax/#html-dinamico) a menudo se escribe directamente dentro de la plantilla HTML en Astro.
...
## Referencia: Convertir Sintaxis de CRA a Astro \{#reference-convert-cra-syntax-to-astro\}

### Importaciones de CRA a Astro \{#cra-imports-to-astro\}
...
### Props Children de CRA a Astro \{#cra-children-props-to-astro\}
...
Los componentes de React que pasan múltiples conjuntos de hijos pueden migrarse a un componente de Astro utilizando [slots con nombre](/es/basics/astro-components/#named-slots).

Para obtener más información sobre el uso específico de `<slot />` en Astro, consulta [Slots](/es/basics/astro-components/#slots).

### Data Fetching de CRA a Astro \{#cra-data-fetching-to-astro\}
...
### Estilando CRA a Astro \{#cra-styling-to-astro\}
...
Si es necesario, convierte cualquier objeto de estilo en línea (`style={{ fontWeight: "bold" }}`) en atributos de estilo HTML en línea (`style="font-weight:bold;"`). O bien, utiliza una [etiqueta `<style>` de Astro](/es/guides/styling/#styling-in-astro) para estilos CSS con alcance local.
