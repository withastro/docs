...
Convierte cualquier [componente `<nuxt-img/>` or `<nuxt-picture/>` de Nuxt](https://image.nuxt.com/usage/nuxt-img) al [componente de imagen propio de Astro](/es/guides/images/#image-) en archivos `.astro` o `.mdx`, o al tag estándar [`<img>`](/es/guides/images/#imágenes-en-componentes-ui-de-frameworks) o `<picture>` de HTML según corresponda en tus componentes de Vue.
