...
Ve [un ejemplo de una aplicación Nuxt convertida paso a paso](#ejemplo-guiado-ver-los-pasos).
...
Tus páginas Vue existentes (`.vue`) de Nuxt deberán [convertirse de archivos Vue a páginas `.astro`](#conviertir-páginas-vue-sfc-a-archivos-astro). No puedes usar un archivo de página Vue existente en Astro.
...
Astro tiene soporte incorporado para Markdown y una integración opcional para archivos MDX. Puedes reutilizar cualquier página Markdown y MDX existente, pero puede que requieran algunos ajustes en su frontmatter, como agregar [la propiedad especial `layout` del frontmatter de Astro](/es/core-concepts/layouts/#plantillas-de-markdownmdx).
...
Cuando forma parte de una [colección de contenido](/es/guides/content-collections/), los archivos Markdown y MDX vivirán en carpetas dentro de `src/content/` y [generarás esas páginas dinámicamente](/es/guides/content-collections/#generating-routes-from-content).

### Migrando Tests

Como Astro genera HTML sin procesar, es posible escribir tests de extremo a extremo utilizando la salida del paso de compilación. Cualquier prueba de extremo a extremo escrita anteriormente podría funcionar de inmediato, si has podido hacer coincidir el marcado de tu sitio Nuxt. Las bibliotecas de pruebas como Jest y Vue Testing Library se pueden importar y usar en Astro para probar tus componentes Vue.
...
- Exporta una función `getStaticPaths()` en el frontmatter de una página de Astro para indicarle al framework qué [rutas estáticas generar dinámicamente](/es/core-concepts/routing/#dynamic-routes).
...
Ve una [lista completa de atributos de componentes](/es/guides/integrations-guide/image/#uso) disponibles para los componentes `<Image />` y `<Picture />` de Astro, y ten en cuenta que varios serán diferentes a los atributos de Nuxt.
