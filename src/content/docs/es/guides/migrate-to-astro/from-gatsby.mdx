...
Accede a `astro:assets` actualmente está disponible bajo una flag experimental, y es la forma recomendada de comenzar un nuevo proyecto de Astro. Si estás migrando a Astro pre-v3.0, te recomendamos [habilitar la flag experimental para usar assets](/es/guides/images/) como solución de imagen.
:::

Convierte los componentes `<StaticImage />` y `<GatsbyImage />` de Gatsby con [`<Image />`](/es/guides/images/), o utiliza una etiqueta HTML `<img>` / JSX `<img />` estándar según corresponda.
