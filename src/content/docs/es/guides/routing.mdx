...
Dado que Astro utiliza [transmisión de HTML](/es/guides/on-demand-rendering/#transmisión-de-html) en el renderizado bajo demanda, las redirecciones deben realizarse a nivel de página, no dentro de los componentes secundarios.
...
:::tip
Para contenido que se ha trasladado de forma permanente, o para dirigir a tu usuario a una página diferente con una nueva URL (por ejemplo, un panel de control de usuario después de iniciar sesión), utiliza un [redireccionamiento](#redirecciones) en su lugar.
:::
...
Las rutas internas tienen prioridad sobre cualquier ruta definida por el usuario o por una integración, ya que son necesarias para que las funciones de Astro funcionen. Las siguientes son las rutas reservadas de Astro:

- `_astro/`: Sirve todos los recursos estáticos al cliente, incluidos los documentos CSS, los scripts del cliente empaquetados, las imágenes optimizadas y cualquier recurso de Vite.
- `_server_islands/`: Sirve los componentes dinámicos diferidos en una [isla de servidor](/es/guides/server-islands/).
