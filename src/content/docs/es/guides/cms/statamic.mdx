...
description: Agrega contenido a tu proyecto Astro usando Statamic como CMS
type: cms
stub: false
service: Statamic
i18nReady: true
...
3. **Un sitio Statamic** - Si necesitas un sitio Statamic, [esta guía](https://statamic.dev/quick-start-guide) te ayudará a empezar.
...
### Obteniendo Datos
:::caution
Si estás usando Statamic y Astro en tu máquina local, recuerda usar `127.0.0.1` en lugar de `localhost` al obtener datos de la API.

Cuando se solicita desde el servidor Astro, `localhost` no se resuelve correctamente como lo hace en el navegador y cualquier solicitud a cualquiera de las API fallará.
:::
