...
Crear una nueva carpeta `src/lib/` y agregar un nuevo archivo llamado `prepr.js`. Aquí es donde configurarás el punto final de Prepr. Agrega el siguiente código para obtener tus datos de Prepr CMS:
...
Para crear una página por cada publicación de blog, ejecutarás una nueva consulta GraphQL en una página `.astro` de [enrutamiento dinámico](/es/guides/routing/#server-ssr-mode). Esta consulta obtendrá un artículo específico por su slug y se creará una nueva página para cada publicación de blog individual.
