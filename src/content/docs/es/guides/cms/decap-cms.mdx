...
Decap añade una ruta (normalmente `/admin`) a tu proyecto que cargará una aplicación React para permitir a los usuarios autorizados administrar el contenido directamente desde el sitio web implementado. Decap enviará los cambios directamente al repositorio fuente de su proyecto Astro.
...
2. Importa el paquete a una etiqueta `<script>` en su página `<body>`
...
4. Agrega la ruta `admin` para tu aplicación React. Este archivo puede ser `public/admin/index.html` junto con `config.yml` o, si prefieres utilizar una ruta Astro, `src/pages/admin.astro`.
...
5. Para habilitar la carga de medios a una carpeta específica a través del editor Decap, agregue una ruta adecuada:
     ```yml title="/public/admin/config.yml"
...
## Uso
Navega hasta `yoursite.com/admin/` para usar el editor Decap CMS.
...
Al implementar en netlify, configura la identidad para tu proyecto a través del panel de Netlify e incluye el [Netlify Identity Widget](https://github.com/netlify/netlify-identity-widget) en la ruta `admin` de tu proyecto. Opcionalmente, incluye el widget de identidad en la página de inicio de tu sitio si planeas invitar a nuevos usuarios por correo electrónico.

### Decap CMS con clientes OAuth externos

Al implementar en proveedores de hosting distintos de Netlify, debes crear tus propias rutas OAuth.

En Astro, esto se puede hacer con rutas renderizadas bajo demanda en tu proyecto configurado con [salida `servidor` o `híbrida`](/es/guides/server-side-rendering/habilita-la-renderización-de-servidor-bajo-demanda) activado.

Consulta [Documentos OAuth de Decap](https://decapcms.org/docs/external-oauth-clients/) para obtener una lista de clientes OAuth compatibles mantenidos por la comunidad.

## Recursos de la comunidad

- Usa la integración de Astro [`astro-decap-cms`](https://github.com/delucis/astro-netlify-cms) para añadir Decap CMS a tu proyecto.

- Puedes empezar con la [Plantilla de Inicio de Astro Blog con Decap CMS](https://github.com/delucis/astro-netlify-cms-starter).
