...
Presiona **Publish** para guardar la entrada. Acabas de crear tu primer publicación de post.

¡Siéntete libre de agregar tantos posts de blog como quieras, luego cambia a tu editor de código favorito para comenzar a hackear con Astro!
...
Esta llamada de búsqueda devolverá una arreglo de tus publicaciones de blog en `entries.items`. Puedes usar `map()` para crear una nueva matriz (`posts`) que formatee tus datos devueltos.

El ejemplo de abajo devuelve las propiedades `items.fields` de nuestro modelo de contenido para crear una vista previa de la publicación de blog, y al mismo tiempo, reformatea la fecha a un formato más legible.
...
Usa el mismo método para obtener tus datos de Contentful que arriba, pero esta vez, en una página que creará una ruta de página única para cada publicación de blog.


#### Generación de sitios estáticos

Si estás usand el modo estático predeterminado de Astro, usarás [rutas dinámicas](/es/core-concepts/routing/#rutas-dinámicas) y la función `getStaticPaths()`. Esta función se llamará en tiempo de compilación para generar la lista de rutas que se convierten en páginas.
...
Finalmente, puedes usar las propiedades de la página para mostrar tu publicación de blog.
...
Finalmente, puedes usar `post` para mostrar tu publicación de blog en la sección de plantilla.
