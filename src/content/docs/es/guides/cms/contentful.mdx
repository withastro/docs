...
description: Agrega contenido a tu proyecto de Astro utilizando Contentful como CMS.
type: cms
service: Contentful
i18nReady: true
---
importar FileTree desde '~/components/tabs/PackageManagerTabs.astro'.
import PackageManagerTabs from '~/components/tabs/PackageManagerTabs.astro'


[Contentful](https://www.contentful.com/)  es un CMS sin cabeza que te permite gestionar contenido, integrarte con otros servicios y publicar en múltiples plataformas.

## Integración con Astro

En esta sección, utilizaremos el [SDK de Contentful](https://github.com/contentful/contentful.js) para conectar tu espacio de Contentful a Astro sin necesidad de JavaScript en el lado del cliente.

### Prerrequisitos

Para comenzar, necesitarás tener lo siguiente:

1. **Un proyecto Astro** -  Si aún no tienes un proyecto de Astro, nuestra [guía de instalación](/es/install/auto/)  te ayudará a comenzar rápidamente.

2. **Una cuenta de Contentful y un espacio de Contentful**. Si no tienes una cuenta, puedes [registrarte](https://www.contentful.com/sign-up/) para obtener una cuenta gratuita y crear un nuevo espacio de Contentful. También puedes utilizar un espacio existente si ya tienes uno.

3. **Credenciales de Contentful** - Puedes encontrar las siguientes credenciales en tu panel de control de Contentful **Settings > API keys**. Si no tienes ninguna clave de API, puedes crear una seleccionando **Add API key**.
...
Ahora deberías poder utilizar estas variables de entorno en tu proyecto.
...
Este fragmento de código crea un nuevo cliente de Contentful, pasando las credenciales desde el archivo `.env`.

:::precaución
Mientras estés en modo de desarrollo, tu contenido se obtendrá desde la API de vista previa de Contentful. Esto significa que podrás ver contenido no publicado desde la aplicación web de Contentful.

En el momento de la construcción, tu contenido se obtendrá desde la API de entrega de Contentful. Esto significa que solo estará disponible el contenido publicado en el momento de la construcción.
:::

Finalmente, tu directorio raíz debería incluir ahora estos nuevos archivos:
...
Ahora, los componentes de Astro pueden obtener datos de tu cuenta de Contentful usando el `contentfulClient` y especificando el `content_type`.

Por ejemplo, si tienes un tipo de contenido "blogPost" que tiene un campo de texto para el título y un campo de texto enriquecido para el contenido, tu componente podría verse así:
...
:::consejo
Si tienes un espacio de Contentful vacío, consulta la [configuración de un modelo de Contentful](#configurando-un-modelo-de-contentful) para aprender cómo crear un modelo básico de blog para tu contenido.
:::

Puedes encontrar más opciones de consulta en la [documentación de Contentful](https://contentful.github.io/contentful.js/contentful/9.1.34/ContentfulClientAPI.html).

## Crear un blog con Astro y Contentful
 
Con la configuración anterior, ahora puedes crear un blog que utiliza Contentful como CMS.

### Prerrequisitos

1. **Un espacio Contentful** - Para este tutorial, recomendamos comenzar con un espacio vacío. Si ya tienes un modelo de contenido, siéntete libre de usarlo, pero deberás modificar nuestros fragmentos de código para que coincidan con tu modelo de contenido.
2. **Un proyecto Astro integrado con el [SDK de Contentful](https://github.com/contentful/contentful.js)** - Consulta [integración con Astro](#integración-con-astro) para obtener más detalles sobre cómo configurar un proyecto Astro con Contentful.
...
¡Siéntete libre de agregar varios posts como quieras, luego cambia a tu editor de código favorito para comenzar a hackear con Astro!
...
### Generar publicaciones de blog individuales
...
Navega a http://localhost:3000/posts/astro-is-amazing/ para asegurarte de que tu ruta dinámica esté funcionando.

#### Renderizado en el lado del servidor (Server Side Rendering, SSR)

Si has [optado por el modo SSR](/es/guides/server-side-rendering/#habilitando-ssr-en-su-proyecto), usarás una ruta dinámica que usa un parámetro `slug` para obtener los datos de Contentful.
...
Si la entrada no fue encontrada, puedes redirigir al usuario a la página 404 usando [`Astro.redirect`](/es/guides/server-side-rendering/#astroredirect).
...
Para publicar tu sitio web, visita nuestras [guías de despliegue](/es/guides/deploy/) y sigue las instrucciones de tu proveedor de alojamiento preferido.

#### Reconstruir en cambios contenido en Contentful
