...
Si estás usando el modo estático predeterminado de Astro, usarás [rutas dinámicas](/es/basics/routing/#rutas-dinámicas) y la función `getStaticPaths()`. Esta función se llamará en tiempo de compilación para generar la lista de rutas que se convierten en páginas.
...
Si la entrada no fue encontrada, puedes redirigir al usuario a la página 404 usando [`Astro.redirect`](/es/basics/routing/#redirecciones-dinámicas).
