...
Si deseas obtener [IntelliSense de TypeScript para estas variables de entorno](/en/guides/environment-variables/#intellisense-for-typescript), puedes crear un nuevo archivo `env.d.ts` en el directorio `src/` y configurar `ImportMetaEnv` de esta manera:
