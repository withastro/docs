...
2. **Una base de datos de MongoDB** - PayloadCMS le pedirá una cadena de conexión de MongoDB al crear un nuevo proyecto. Puede configurar uno localmente o usar [MongoDBAtlas](https://www.mongodb.com/) para alojar una base de datos en la web de forma gratuita.
...
:::tip
Por defecto, tanto Astro como PayloadCMS utilizarán el puerto 3000. Es posible que desees cambiar el puerto de PayloadCMS en el archivo `src/server.ts`. No olvides actualizar el `serverURL` en `src/payload.config.ts` también.
:::
...
Crea una página `src/pages/posts/[slug].astro` para [generar dinámicamente una página](/es/core-conceptss/routing/#rutas-dinámicas) para cada publicación.
