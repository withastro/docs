...
Crea el archivo `src/pages/blog/[slug].astro` para [generar dinámicamente una página](/es/basics/routing/#rutas-dinámicas) para cada artículo.
...
Si has [optado por el modo SSR](/es/guides/server-side-rendering/) con `output: server` o `output: hybrid`, [genera tus rutas dinámicas](/es/basics/routing/#modo-servidor-ssr) utilizando el siguiente código:
