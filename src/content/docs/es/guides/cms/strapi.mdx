...
Si has [optado por el modo SSR](/es/guides/server-side-rendering/#habilitando-ssr-en-su-proyecto) con `output: server` o `output: hybrid`, [genera tus rutas dinámicas](/es/core-concepts/routing/#modo-servidor-ssr) utilizando el siguiente código:
