...
Si has [optado por el modo SSR](/es/guides/on-demand-rendering/) con `output: server` o `output: hybrid`, [genera tus rutas dinámicas](/es/guides/routing/#modo-servidor-ssr) utilizando el siguiente código:
