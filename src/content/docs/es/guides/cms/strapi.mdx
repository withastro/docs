...
[Strapi](https://strapi.io/) es un CMS headless de código abierto y personalizable.
...
En el modo estático predeterminado de Astro (SSG), utiliza [`getStaticPaths()`](/es/reference/routing-reference/#getstaticpaths) para obtener tu lista de artículos desde Strapi.
...
Si ha [optado por el renderizado bajo demanda con un adaptador](/es/guides/on-demand-rendering/) con `output: server` o `output: hybrid`, [genera tus rutas dinámicas](/es/guides/routing/#rutas-dinámicas-bajo-demanda) utilizando el siguiente código:
