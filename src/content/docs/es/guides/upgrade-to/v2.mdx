...
¿Necesitas actualizar un proyecto más antiguo a la v1? Ve nuestra [guía de migración anterior](/en/guides/upgrade-to/v1/).

## Actualiza Astro

Actualiza la versión de tu proyecto Astro a la última versión usando el manejador de paquetes. Si estás utilizando alguna integración, también acutalizalas a la última versión.
...
Checa que tanto tu entorno de desarrollo como tu entorno de producción estén usando **Node `16.12.0` o superior**.

1. Checa la versión local de Node usando:
...
2. Checa la propia documentación del [entorno de producción](/es/guides/deploy) para verificar que ellos soportan Node 16.
...
Astro v2.0 ahora incluye la API de Colecciones para organizar tus archivos Markdown y MDX en [colecciones de contenido](/en/guides/content-collections/). Esta API reserva `src/content/` como una carpeta especial.

#### ¿Qué debería hacer?

Renombra la carpeta existente `src/content/` para evitar conflictos. Esta carpeta, si es que existe, ahora solo se puede usar para [colecciones de contenido](/en/guides/content-collections/).
...
Elimina `extendDefaultPlugins: false` y agrega las flags para deshabilitar cada plugin individualmente en su lugar.
...
Checa cualquier plugin de remark y rehype que hayas escrito para ver si su comportamiento ha cambiado. Ten en cuenta que `data.astro.frontmatter` ahora es el frontmatter _completo_ del documento Markdown o MDX, en lugar de un objeto vacío.
...
## Astro v2.0 Flags Experimentales Eliminados

Elimina las siguientes flags experimentales de `astro.config.mjs`:
...
- [Prerenderizado individual de páginas a HTML estático](/es/guides/server-side-rendering/#renderizado-híbrido) cuando se usa SSR para mejorar la velocidad y la caché.
