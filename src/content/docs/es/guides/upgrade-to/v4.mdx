...
## Actualizar Astro \{#upgrade-astro\}
...
También puedes [actualizar tus integraciones de Astro manualmente](/es/guides/integrations-guide/#manual-upgrading) si es necesario, y puede que también necesites actualizar otras dependencias en tu proyecto.
...
Astro v4.0 incluye [cambios potencialmente importantes](#breaking-changes), así como la [eliminación de algunas funciones obsoletas](#previously-deprecated-features-now-removed).
...
## Astro v4.0 Banderas Experimentales Eliminadas \{#astro-v40-experimental-flags-removed\}
...
### Actualizado: Vite 5.0 \{#upgraded-vite-50\}
...
### Actualizado: dependencias unified, remark y rehype \{#upgraded-unified-remark-and-rehype-dependencies\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-1\}
...
### Renombrado: `entrypoint` (API de integraciones) \{#renamed-entrypoint-integrations-api\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-2\}
...
### Cambiado: Firma `app.render` en la API de integraciones \{#changed-apprender-signature-in-integrations-api\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-3\}
...
### Cambiado: Ahora los adaptadores deben especificar las caracteristicas compatibles \{#changed-adapters-must-now-specify-supported-features\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-4\}
...
### Eliminado: Propiedad `path` del lenguaje Shiki \{#removed-shiki-language-path-property\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-5\}
...
### Obsoleto: `handleForms` para eventos `submit` de View Transitions \{#deprecated-handleforms-for-view-transitions-submit-events\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-6\}
...
## Características previamente obsoletas ahora eliminadas \{#previously-deprecated-features-now-removed\}
...
### Eliminado: Retornar objetos simples desde endpoints \{#removed-returning-simple-objects-from-endpoints\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-7\}
...
### Eliminado: `build.split` y `build.excludeMiddleware` \{#removed-buildsplit-and-buildexcludemiddleware\}

En Astro v3.0, las opciones de configuración de construcción `build.split` y `build.excludeMiddleware` fueron marcadas como obsoletas y reemplazadas con [opciones de configuración de adaptadores](/es/reference/adapter-reference/#adapter-features) para realizar las mismas tareas.

Astro v4.0 elimina por completo estas propiedades.

#### ¿Qué debo hacer? \{#what-should-i-do-8\}
...
### Eliminado: `Astro.request.params` \{#removed-astrorequestparams\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-9\}
...
### Eliminado: Usando `rss` en `getStaticPaths()` \{#removed-using-rss-in-getstaticpaths\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-12\}
...
### Eliminado: Nombres de métodos HTTP en minúsculas \{#removed-lowercase-http-method-names\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-13\}
...
### Eliminado: Redirecciones 301 cuando falta un prefijo `base` \{#removed-301-redirects-when-missing-a-base-prefix\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-14\}
...
### Eliminado: Auto-conversión de `astro/client-image` \{#removed-astroclient-image-auto-conversion\}
...
#### ¿Qué debo hacer? \{#what-should-i-do-15\}
