...
Un script de precarga se agregará a todas las páginas de su sitio. Luego puede agregar el atributo `data-astro-prefetch` a cualquier enlace `<a />` en su sitio para optar por la precarga. Cuando pasa el mouse sobre el enlace, el script buscará la página en segundo plano.
...
- `hover` (default): Precarga cuando pasas el mouse o te enfocas en el enlace.
...
- Si un visitante está usando [data saver mode](https://developer.mozilla.org/en-US/docs/Web/API/NetworkInformation/saveData) o tiene una [conexión lenta](https://developer.mozilla.org/en-US/docs/Web/API/NetworkInformation/effectiveType), la precarga se desactivará.
...
La estrategia de precarga predeterminada al agregar el atributo `data-astro-prefetch` es `hover`. Para cambiarlo, puedes configurar [`prefetch.defaultStrategy`](/en/reference/configuration-reference/#prefetchdefaultstrategy) en tu archivo `astro.config.js`:
...
Si quieres precargar todos los enlaces, incluidos los que no tienen el atributo `data-astro-prefetch`, puedes establecer [`prefetch.prefetchAll`](/en/reference/configuration-reference/#prefetchprefetchall) en `true`:
