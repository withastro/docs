...
Con la bandera experimental [`clientPrerender`](/en/reference/experimental-flags/client-prerender/) activada, puedes usar la opción `eagerness` en `prefetch()` para sugerirle al navegador con qué nivel de anticipación debería realizar el *prefetch* o *prerender* de los enlaces de destino.

Esto sigue la misma API descrita en la [API Speculation Rules](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/script/type/speculationrules#eagerness) y, de forma predeterminada, se establece en `immediate` (la opción más agresiva). En orden decreciente de anticipación, las otras opciones son: `eager`, `moderate` y `conservative`.
