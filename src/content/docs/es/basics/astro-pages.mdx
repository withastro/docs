...
Los archivos `.html` pueden ser colocados en `src/pages/` y usados directamente como páginas en tu proyecto. Ten en cuenta que algunas funcionalidades clave de Astro no son compatibles con [Componentes HTML](/es/basics/astro-components/#html-components).
...
Para que una página de error 500 personalizada se muestre para las páginas que se [renderizan bajo demanda](/es/guides/server-side-rendering/#habilita-la-renderización-de-servidor-bajo-demanda), crea el archivo `src/pages/500.astro`. Esta página personalizada no está disponible para las páginas prerenderizadas y no puede ser prerenderizada.
