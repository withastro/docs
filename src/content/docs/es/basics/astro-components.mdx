...
<ReadMore>Lee más en la [referecia de sintaxis de Astro](/es/reference/astro-syntax/).</ReadMore>
:::
...
Cuando tu componente de Astro necesite interactividad en el lado del cliente, puedes añadir [etiquetas HTML estándar `<script>`](/es/guides/client-side-scripts/) o [componentes UI Framework](/es/guides/framework-components/#hidratando-componentes-interactivos) como "islas cliente".

En el caso de los componentes que necesitan renderizar contenido personalizado o dinámico, puede aplazar su renderización en el servidor añadiendo una [directiva de servidor](/es/reference/directives-reference/#directivas-del-servidor). Estas "islas de servidor" renderizarán su contenido cuando esté disponible, sin retrasar la carga completa de la página.
...
:::
El script del componente Astro es TypeScript, que permite añadir sintaxis adicional a JavaScript para las herramientas del editor y la comprobación de errores.

<ReadMore>Lee más acerca el [soporte integrado de TypeScript](/es/guides/typescript/) de Astro.</ReadMore>
