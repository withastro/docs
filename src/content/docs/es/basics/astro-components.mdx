...
Para pasar múltiples elementos HTML dentro del placeholder `<slot/>` de un componente sin un `<div>` envolvente, usa el atributo `slot=""` en el [componente `<Fragment/>` de Astro](/es/basics/astro-syntax/#fragments):
