...
## Modos de `output` del servidor \{#server-output-modes\}
...
Los sitios estáticos pueden incluir [islas de Astro](/es/concepts/islands/) para componentes UI interactivos (¡o incluso aplicaciones completas incrustadas renderizadas en el lado del cliente!) escritos en el [framework UI que elijas](/es/guides/framework-components/) en una página estática pre-renderizada.
...
### Renderizado bajo demanda \{#on-demand-rendered\}

Con [un adaptador SSR](/es/guides/server-side-rendering/), las otras dos opciones de salida de Astro se pueden configurar para habilitar el **renderizado bajo demanda de algunas o todas tus páginas, rutas o endpoints de API**:
...
<ReadMore>Lee más sobre las [características disponibles para ti con el renderizado bajo demanda y un adaptador](/es/guides/server-side-rendering/#características-de-renderización-bajo-demanda), como cookies, objetos Response y Request, streaming de HTML y más.</ReadMore>
...
Ambos modos de salida `server` e `hybrid` te permiten incluir [islas de Astro](/es/concepts/islands/) para la interactividad (¡o incluso aplicaciones completas incrustadas renderizadas en el lado del cliente!) en tu elección de [frameworks UI](/es/guides/framework-components/). Con [middleware](/es/guides/middleware/) y la [API de View Transitions](/es/guides/view-transitions/) de Astro para animaciones y preservación del estado a través de las navegaciones de ruta, incluso las aplicaciones altamente interactivas son posibles.

:::tip
El renderizado de servidor bajo demanda en Astro, especialmente cuando es combinado con [view transitions](/es/guides/view-transitions/) proporciona una verdadera experiencia de aplicación sin la sobrecarga de JavaScript de una aplicación de página única del lado del cliente.
:::
