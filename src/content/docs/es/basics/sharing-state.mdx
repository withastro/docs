...
Frameworks tales como React o Vue pueden alentar a usar [proveedores de "contexto"](https://react.dev/learn/passing-data-deeply-with-context) ("context" providers) para que sea consumido por otros componentes. Pero al [hidratar componentes parcialmente](/es/basics/framework-components/#hidratando-componentes-interactivos) dentro de Astro o en Markdown, no puedes usar esos contextos envolventes.
...
- Escribir en un store desde un archivo `.astro` o un [componente no-hidratado](/es/basics/framework-components/#hidratando-componentes-interactivos) _no_ afectará el valor recibido por un [componente del lado del cliente](/es/reference/directives-reference/#directivas-del-cliente).
