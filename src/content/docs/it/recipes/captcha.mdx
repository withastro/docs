...
Gli [endpoints del server](/it/core-concepts/endpoints/#endpoint-server-percorsi-api) possono essere utilizzati come endpoint API REST per eseguire funzioni come autenticazioni, accesso al database e verifiche senza esporre dati sensibili al client.

In questa ricetta, un percorso API viene utilizzato per verificare Google reCAPTCHA v3 senza esporre il secret ai client.

## Prerequisiti
- Un progetto con [SSR](/it/guides/server-side-rendering/) (`output: 'server'`) abilitato
