...
- Eliminare qualsiasi cosa da ogni singola pagina che ora viene gestita dal layout, incluso:
  ...
  - Regole CSS in un tag `<style>` (es. `<h1>` nella tua pagina About)
  - Tag `<script>`

:::note[Mantenimento degli stili della tua pagina About]
...
- [Astro `<slot />`](/it/basics/astro-components/#slots)
