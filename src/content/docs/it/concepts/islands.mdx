---
title: Isole Astro
description: "Le Isole Astro (noto anche come Isole Component) sono un modello di architettura web sviluppato da Astro. Il concetto di "architettura delle isole" è stato coniato per la prima volta dall'architetto frontend di Etsy, Katie Sylor-Miller, nel 2019, ed è stato ampliato dal creatore di Preact, Jason Miller."
i18nReady: true
---

import IslandsDiagram from '~/components/IslandsDiagram.astro';

**Isole Astro** (noto anche come Isole Component) sono un modello di architettura web sviluppato da Astro. L'idea di "architettura delle isole" è stata coniata per la prima volta dallo sviluppatore frontend di Etsy, [Katie Sylor-Miller](https://twitter.com/ksylor), nel 2019, ed è stata approfondita in [questo articolo](https://jasonformat.com/islands-architecture/) dal creatore di Preact, Jason Miller.

## Cosa sono le Isole Astro?

Il termine "Isola Astro" si riferisce a un componente UI interattivo all'interno di una pagina HTML. Su una pagina possono esistere più isole, e un'isola viene sempre renderizzata da sola. Pensate a esse come isole in un mare di HTML statico e non interattivo.
...
<IslandsDiagram>
  ...
  <Fragment slot="source">Fonte: [Architettura delle isole: Jason Miller](https://jasonformat.com/islands-architecture/)</Fragment>
</IslandsDiagram>

In Astro, è possibile utilizzare qualsiasi framework UI supportato (React, Svelte, Vue, ecc.) per renderizzare le isole nel browser. Tu puoi combinare diversi framework sulla stessa pagina o semplicemente scegliere il tuo preferito.
...
Con le Isole Astro, la stragrande maggioranza del tuo sito rimane in HTML e CSS. Nell'esempio sopra, hai appena aggiunto una singola e isolata **isola di interattività** senza modificare anche il resto della pagina.
...
Ancora meglio, puoi dire ad Astro esattamente come e quando renderizzare ciascun componente. Se quel carosello di immagini è davvero costoso da caricare, puoi aggiungere una [direttiva client](/it/reference/directives-reference/#client-directives) speciale che indica ad Astro di caricare il carosello solo quando diventa visibile sulla pagina. Se l'utente non lo vede mai, non viene mai caricato.

In Astro, è lo sviluppatore indicare esplicitamente ad Astro quali componenti sulla pagina devono essere eseguiti anche nel browser. Astro idraterà solo ciò di cui c'è bisogno sulla pagina e lascerà il resto del tuo sito come HTML statico.

**Le isole sono il segreto della storia delle prestazioni rapide di Astro!**