...
Per inserire elementi HTML dentro il placeholder `<slot/>` di un componente senza inserirlo dentro ad un `<div>`, una l'attributo `slot=""` nel [componente Astro `<Fragment/>`](/it/basics/astro-syntax/#fragments):
