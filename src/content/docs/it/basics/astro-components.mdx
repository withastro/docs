...
Per inserire elementi HTML dentro il placeholder `<slot/>` di un componente senza inserirlo dentro ad un `<div>`, una l'attributo `slot=""` nel [componente Astro `<Fragment/>`](/it/reference/api-reference/#fragment-):
...
📚 Scopri come utilizzare i [componenti di framework UI](/it/guides/framework-components/) nel tuo progetto Astro.
