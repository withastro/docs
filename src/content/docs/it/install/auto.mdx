...
Il comando `create astro` è il modo più rapido per iniziare un nuovo progetto di Astro partendo da zero. Ti guiderà attraverso ogni passo per impostare il tuo nuovo progetto di Astro. Ti permette di scegliere tra diversi template ufficiali già pronti oppure puoi [usare qualunque progetto esistente su GitHub con l'argomento `--template`](#starter-templates).
...
Esegui il seguente comando nel tuo terminale per eseguire il nostro utilie strumento di installazione:
...
Puoi eseguire il comando `create astro` ovunque nel tuo computer, perciò non serve creare una nuova cartella vuota per il tuo progetto prima di cominciare. Se ancora non hai una cartella vuota per il tuo progetto, lo strumento ne creerà automaticamente una al posto tuo.
...
Se hai saltato il comando `npm install` durante l'esecuzione dello strumento CLI, allora assicurati di installare le dependencies prima di continuare.

## 2. Fai partire Astro ✨

Astro arriva con un server di sviluppo integrato che ha tutto il necessario per il progresso del tuo progetto. Il comando `astro dev` farà partire il server di sviluppo locale così che tu possa vedere il tuo sito in azione per la primissima volta.
...
Se tutto va bene, ora Astro dovrebbe servire il tuo progetto all'indirizzo [http://localhost:3000/](http://localhost:3000/)!
...
Se non riesci ad aprire il tuo progetto nel browser, torna al terminale dove hai eseguito il comando `dev` e controlla se c'è stato un errore, o se il tuo progetto viene servito ad un indirizzo diverso da quello indicato qua sopra.

## Template di partenza

Puoi anche iniziare un nuovo progetto di Astro partendo da un [esempio ufficiale](https://github.com/withastro/astro/tree/main/examples) o dal ramo `main` di una qualunque repository di GitHub passando l'argomento `--template` al comando `create astro`.
...
Ecco alcuni argomenti che ti consigliamo di esplorare ora. Li puoi leggere in ordine sparso. Puoi anche uscire dalla nostra documentazione per un po' e andare a provare la codebase del tuo nuovo progetto di Astro, e tornare qua se dovessi avere problemi o domande.
...
📚 **Conosci la tua codebase:** Familiarizza con la struttura di un progetto di Astro nella nostra [guida alla Struttura del Progetto](/it/core-concepts/project-structure/).
