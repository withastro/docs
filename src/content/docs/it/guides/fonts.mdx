---
title: Utilizzare caratteri personalizzati
description: >-
...
Questa guida ti mostrerà come aggiungere caratteri personalizzati al tuo progetto e come utilizzarli nei tuoi componenti.

## Utilizzo di un file di caratteri locale
...
2. Aggiungi lo statement `@font-face` al tuo CSS. Puoi aggiungerlo in un file `.css` globale che importerai in seguito, in un blocco `<style is:global>` o in un blocco `<style>` nel componente o nel layout specifico in cui vuoi utilizzare questo carattere.
    ...
    ```css
    /* Registra il tuo font personalizzato e indica al browser dove si trova. */
    @font-face {
...
## Utilizzo di Fontsource

Il progetto [Fontsource](https://fontsource.org/) semplifica l'utilizzo di Google Fonts e altri caratteri open source. Fornisce moduli npm che puoi installare in base ai caratteri che decidi di utilizzare.

1. Cerca il carattere che vuoi utilizzare nel [catalogo di Fontsource](https://fontsource.org/). In questio esempio useremo [Twinkle Star](https://fontsource.org/fonts/twinkle-star).
...
## Registrare caratteri in Tailwind

Se stai utilizzando l'[integrazione Tailwind](/en/guides/integrations-guide/tailwind/), puoi utilizzare uno dei metodi precedenti per installare il tuo carattere, con alcuni accorgimenti. Per installare il carattere puoi aggiungere uno statement [`@font-face` se stai usando un carattere locale](#using-a-local-font-file) oppure puoi utilizzare la [strategia di importazione di Fontsource](#using-fontsource).

Puoi registrare il tuo carattere in Tailwind seguendo questo passi:
...
2. Aggiungi il nome del carattere a `tailwind.config.cjs`.
    ...
    Questo esempio aggiunge `InterVariable` e `Inter` allo stack di caratteri sans-serif, con i caratteri di fallback di default di Tailwind CSS.
...
Per maggiori informazioni, consulta la [configurazione di caratteri personalizzati in Tailwind](https://tailwindcss.com/docs/font-family#font-families).
