...
- Un progetto Astro con [server-side rendering (SSR)](/it/guides/server-side-rendering/#abilitare-ssr-nel-tuo-progetto) abilitato.
...
### Creazione dei endpoint del server per l'autenticazione
...
:::note
Questa è un'implementazione di base del endpoint per la disconnessione. Puoi aggiungere ulteriore logica a questo endpoint per adattarlo alle tue esigenze.
:::
...
:::note
Questa è un'implementazione di base del endpoint per la registrazione. Puoi aggiungere ulteriore logica a questo endpoint per adattarlo alle tue esigenze.
:::
...
L'esempio di `src/pages/register.astro` qui sotto include un form che invierà una richiesta `POST` al endpoint `/api/auth/register`. Questo endpoint creerà un nuovo utente utilizzando i dati del form e quindi reindirizzerà l'utente alla pagina `/signin`.
...
La pagina di esempio qui sotto contiene un form che invierà una richiesta `POST` al endpoint `/api/auth/signin` con il token ID generato dall'app client di Firebase.
...
- Un progetto Astro inizializzato con Firebase come descritto nella sezione: [Inizializzazione di Firebase in Astro](#initializing-firebase-in-astro).
...
Infine, `src/pages/dashboard.astro` mostrerà una lista degli amici. Ogni amic avrà un link alla loro pagina e un bottone che li manderà sulla pagina per modificare l'amico.
