...
## Abilitare il precaricamento \{#enable-prefetching\}
...
### Strategia di precaricamento predefinita \{#default-prefetch-strategy\}
...
### Precaricamento di tutti i link di default \{#prefetch-all-links-by-default\}
...
La strategia di precaricamento predefinita per tutti i link può essere cambiata con `prefetch.defaultStrategy` come mostrato nella sezione [Strategia di precaricamento predefinita](#default-prefetch-strategy).

## Precarica programmaticamente \{#prefetch-programmatically\}
...
## Usarlo con le View Transitions \{#using-with-view-transitions\}

Quando usi le [View Transitions](/it/guides/view-transitions/) in una pagina, il precaricamento verrà abilitato di default. Imposta una configurazione predefinita di `{ prefetchAll: true }` che abilita il [precaricamento per tutti i link](#prefetch-all-links-by-default) nella pagina.
...
## Supporto dei browser \{#browser-support\}
...
## Migrazione da `@astrojs/prefetch` \{#migrating-from-astrojsprefetch\}
