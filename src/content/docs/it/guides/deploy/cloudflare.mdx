...
Puoi effettuare il deploy del tuo progetto Astro su [Cloudflare Pages](https://pages.cloudflare.com/), una piattaforma per sviluppatori frontend che consente di collaborare e hostare siti web statici (JAMstack) e SSR.

Questa guida include:

- [Come effettuare il deploy tramite il Pannello di controllo di Cloudflare Pages](#come-effettuare-il-deploy-di-un-sito-con-git)
...
5. Seleziona il progetto git che desideri hostare e clicca su **Inizia configurazione**.
...
Dopo aver caricato i file del tuo progetto, Wrangler ti fornirà un URL per visualizzare l'anteprima del tuo sito per ispezionarlo. Quando accedi al pannello di controllo di Cloudflare Pages, vedrai il tuo nuovo progetto.
...
È possibile cambiare il comando per visualizzare l'anteprima con quello della cli `wrangler` invece del comando di anteprima della cli di Astro:
...
Puoi creare un sito Astro SSR e eseguire il deploy su Cloudflare Pages utilizzando l'adattatore [`@astrojs/cloudflare`](/it/guides/integrations-guide/cloudflare/).

Segui i passaggi qui sotto per configurare l'adattatore. Successivamente, puoi effettuare il deploy del tuo sito utilizzando uno dei metodi visti in precedenza.

### Installazione rapida

Aggiungi l'adattatore di Cloudflare per abilitare l'SSR nel tuo progetto Astro utilizzando il seguente comando: `astro add`. Questo installerà l'adattatore e apporterà le modifiche appropriate al tuo file `astro.config.mjs` in un solo passaggio.
...
📚 Leggi di più su [SSR in Astro](/en/guides/server-side-rendering/).
