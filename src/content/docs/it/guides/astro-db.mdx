...
I dati in Astro DB sono memorizzati utilizzando tabelle SQL. Le tabelle strutturano i tuoi dati in righe e colonne, dove le colonne impongono il tipo di ciascun valore di riga. Astro DB supporta i seguenti tipi di colonne:
...
Le tabelle potenziano il supporto automatico di TypeScript per i tuoi contenuti in Astro. Quando definisci una tabella, Astro genererà automaticamente e applicherà un'interfaccia TypeScript ad essa. Il risultato è il pieno supporto di TypeScript quando interroghi i tuoi dati, incluso l'autocompletamento delle proprietà e il controllo dei type.

Per configurare una tabella del database individuale, importa e usa le utilità `defineTable()` e `column` da `astro:db`.

Questo esempio configura una tabella `Comment` con colonne di testo obbligatorie per `author` e `body` e poi la rende disponibile al tuo progetto tramite l'esportazione `defineDb()`.
...
Astro DB può [collegarsi alla piattaforma Astro Studio](/it/recipes/studio/) per aggiungere rapidamente un database ospitato al tuo progetto. Puoi visualizzare, gestire e distribuire nuovi database ospitati direttamente dal portale web di Astro Studio.

Ci sono due modi per creare un progetto in Astro Studio:
1. tramite la [CLI di Astro Studio](/it/reference/cli-reference/#astro-studio-cli) usando `astro link`
2. tramite la [UI web di Astro Studio](https://studio.astro.build)

Per creare un nuovo database nella UI web di Astro Studio, clicca sul pulsante "crea progetto" nell'intestazione e segui le istruzioni presentate. Astro Studio ti guiderà attraverso i passaggi necessari e, al termine del processo, avrai un database ospitato configurato per il tuo progetto.
