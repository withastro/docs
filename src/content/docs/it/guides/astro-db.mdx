...
2. Aggiungi un nuovo schema di tabella (corrispondente esattamente alle proprietà della tabella esistente) con il nuovo nome:
    ...
    ```
3. [Pubblica su Astro Studio](#pubblicazione-gli-schemi-delle-tabelle) con `astro db push --remote`. Questo aggiungerà la nuova tabella e segnerà la vecchia come deprecata.
...
Inoltre, [la pubblicazione di eventuali modifiche allo schema delle tabelle](#pubblicazione-gli-schemi-delle-tabelle) (note anche come "migrazioni dello schema") deve essere gestita manualmente utilizzando questa variabile d'ambiente.
