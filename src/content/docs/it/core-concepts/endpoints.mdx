...
Tutto quanto descritto nella sezione endpoint file statici può essere utilizzato anche in modalità SSR: i file possono esportare una funzione `GET` che riceve un [oggetto contesto](/en/reference/api-reference/#endpoint-context) con proprietà simili a l´`Astro` globale.
...
Puoi anche esportare una funzione `ALL` per far corrispondere qualsiasi metodo che non abbia una funzione esportata corrispondente. Se è presente una richiesta senza metodo corrispondente, verrà reindirizzata alla [pagina 404](/en/core-concepts/astro-pages/#custom-404-error-page) del tuo sito.
