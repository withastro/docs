...
Le pagine, le route, e gli API endpoint possono essere [pre-renderizzati a build time](#pre-rendered) o [renderizzati on demand da un server](#on-demand-rendered) quando viene richiesta una route.
