...
Una pagina deve produrre un documento HTML completo. Se non incluso esplicitamente, Astro aggiungerà la dichiarazione `<!DOCTYPE html>` necessaria e il contenuto `<head>` a qualsiasi componente `.astro` situato all'interno di `src/pages/` per impostazione predefinita. Puoi disattivare questo comportamento in base al componente contrassegnandolo come pagina [parziale](#page-partials).
