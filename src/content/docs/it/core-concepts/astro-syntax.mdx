...
La sintassi del componente Astro è un superset di HTML. La sintassi è stata [progettata per risultare familiare a chiunque abbia esperienza nella scrittura di HTML o JSX](#differences-between-astro-and-jsx) e aggiunge il supporto per includere componenti ed espressioni JavaScript.
...
Vedi sotto per ulteriori esempi di [differenze tra Astro e JSX](#differences-between-astro-and-jsx).
:::
...
- **Le direttive di idratazione non sono supportate.** Quando si utilizzano le [direttive di idratazione `client:*`](/it/core-concepts/framework-components/#hydrating-interactive-components), Astro deve sapere quali componenti raggruppare per la produzione e il modello di tag dinamico ne impedisce il funzionamento.
