...
[Considere ativar o modo `server` ou `hybrid`](/pt-br/guides/server-side-rendering/#enable-on-demand-server-rendering) em seu projeto Astro se você precisar do seguinte:
...
:::tip
A renderização sob demanda no lado do servidor no Astro oferece uma verdadeira experiência de aplicação sem a sobrecarga de JavaScript de um aplicação de página única no lado do cliente.
:::
