...
:::tip
Enquanto esse guia descreve algumas convenções populares usadas na comunidade do Astro, as únicas pastas reservadas pelo Astro são `src/pages/`. Você é livre para renomear e reorganizar quaisquer outras pastas de uma forma que funciona melhor para você.
:::

### `src/pages`

Rotas de paginas são criadas para seu site adicionando [tipos de arquivos suportados](/pt-br/basics/astro-pages/#supported-page-files) para esse diretório.
...
Astro suporta varios formatos de arquivo para seu arquivo de configuração Javascript: `astro.config.js`, `astro.config.mjs`, `astro.config.cjs` e `astro.config.ts`. Nós recomendamos usar `.mjs` na maioria dos casos ou `.ts` se você quiser escrever código Typescript no seu arquivo de configuração.
