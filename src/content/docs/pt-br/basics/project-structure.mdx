...
Astro suporta vários formatos de arquivo para seu arquivo de configuração JavaScript: `astro.config.js`, `astro.config.mjs`, `astro.config.cjs` e `astro.config.ts`. Nós recomendamos usar `.mjs` na maioria dos casos ou `.ts` se você quiser escrever código TypeScript no seu arquivo de configuração.
