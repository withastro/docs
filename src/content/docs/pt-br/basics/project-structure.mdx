...
description: Uma introdução à estrutura básica de arquivos de um projeto Astro.
i18nReady: true
---
...
- `src/*` - O código-fonte do seu projeto (componentes, páginas, estilos, imagens, etc.)
...
- [Imagens para serem otimizadas e processadas pelo Astro](/pt-br/guides/images/)
...
:::tip
Enquanto esse guia descreve algumas convenções populares usadas na comunidade do Astro, a única pasta reservada pelo Astro é `src/pages/`. Você é livre para renomear e reorganizar quaisquer outras pastas de uma forma que funciona melhor para você.
:::
...
:::caution
`src/pages` é um subdiretório **obrigatório** no seu projeto Astro. Sem ele, seu site não vai ter paginas ou rotas!
:::
...
Assim como `src/components`, esse diretório é uma prática comum porém não obrigatória.
...
Este é um arquivo utilizado por gerenciadores de pacotes de JavaScript para gerenciar suas dependências. Ele também define scripts que são comumente usados para executar o Astro (por exemplo, `npm run dev` e `npm run build`).
...
Caso precise de ajuda em como criar um novo arquivo `package.json` para o seu projeto, veja as instruções de [instalação manual](/pt-br/install-and-setup/#configuração-manual).
...
Astro suporta vários formatos de arquivo para seu arquivo de configuração JavaScript: `astro.config.js`, `astro.config.mjs` e `astro.config.ts`. Nós recomendamos usar `.mjs` na maioria dos casos ou `.ts` se você quiser escrever código TypeScript no seu arquivo de configuração.
...
Consulte a [referência de configuração](/pt-br/reference/configuration-reference/) para ver todos os detalhes.
