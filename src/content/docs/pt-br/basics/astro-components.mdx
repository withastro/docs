...
Slots também podem renderizar **conteúdo de fallback**. Quando não existirem filhos correspondentes passados a um slot, um elemento `<slot />` irá renderizar os elementos filhoss que ele possui como placeholder.
