...
**Componentes Astro** são os elementos básicos de qualquer projeto Astro. São componentes de template com apenas HTML sem execução no lado do cliente e que usam a extensão de arquivo `.astro`.

:::note
Se você conhece HTML, já sabe o suficiente para escrever seu primeiro componente Astro.

<ReadMore>Saiba mais na [referência da sintaxe Astro](/pt-br/reference/astro-syntax/).</ReadMore>
:::

Componentes Astro são extremamente flexíveis. Um componente Astro pode ser tão pequeno quanto um trecho de HTML, como uma coleção de tags `<meta>` comuns que facilitam trabalhar com SEO. Componentes podem ser elementos de UI reutilizáveis, como um cabeçalho ou um cartão de perfil. Componentes Astro podem até mesmo conter o layout inteiro de uma página ou, quando estão na pasta especial `src/pages/`, ser uma página completa.

A coisa mais importante de entender sobre componentes Astro é que eles **não renderizam no lado do cliente**. Eles renderizam para HTML em tempo de build ou sob demanda. Você pode incluir código JavaScript dentro do frontmatter do seu componente, e todo esse código vai ser removido da página final enviada para o navegador dos seus usuários. O resultado é um site mais rápido, sem nenhum JavaScript adicionado por padrão.

Quando seu componente Astro precisar de interatividade no lado do cliente, você poderá adicionar [tags `<script>` tradicionais do HTML](/pt-br/guides/client-side-scripts/) ou [componentes de um Framework de UI](/pt-br/guides/framework-components/#hydrating-interactive-components) como "ilhas de cliente".

Para componentes que precisam renderizar conteúdo personalizado ou dinâmico, você pode adiar a renderização no servidor adicionando uma [diretiva do servidor](/pt-br/reference/directives-reference/#server-directives). Essas "ilhas de servidor" renderizarão seu conteúdo quando ele estiver disponível, sem atrasar o carregamento da página inteira.

## Estrutura do Componente

Um componente Astro é feito de duas partes principais: o **Script do Componente** e o **Template do Componente**. Cada parte desempenha uma função diferente, mas juntas elas fornecem um framework fácil de usar e expressivo o bastante para lidar com tudo o que você quiser criar.

```astro title="src/components/ComponenteVazio.astro"
---
// Script do Componente (JavaScript)
---
<!-- Template do Componente (HTML + Expressões em JS) -->
```

### O Script do Componente

Astro utiliza uma cerca de código (`---`) para identificar o script do seu componente Astro. Se você já escreveu Markdown antes, você já deve estar familiar com um conceito similar chamado *frontmatter*. A ideia do Astro de um script de componente foi diretamente inspirado por este conceito.
...
:::note
O script de um componente Astro é TypeScript, o que permite adicionar sintaxe extra ao JavaScript para oferecer ferramentas no editor e verificação de erros.

<ReadMore>Saiba mais sobre o [suporte integrado a TypeScript](/pt-br/guides/typescript/) do Astro.</ReadMore>
:::
...
Porém, a [sintaxe do template de componentes Astro](/pt-br/reference/astro-syntax/) também suporta **expressões JavaScript**, tags [`<style>`](/pt-br/guides/styling/#styling-in-astro) e [`<script>`](/pt-br/guides/client-side-scripts/) do Astro, **componentes importados** e [**diretivas especiais do Astro**](/pt-br/reference/directives-reference/). Dados e valores definidos no script do componente podem ser utilizados no template do componente para produzir HTML criado dinamicamente.
...
<!-- Adie a renderização do componente e forneça um conteúdo alternativo durante o carregamento: -->
<Avatar server:defer>
  <svg slot="fallback" class="avatar-generico" transition:name="avatar">...</svg>
</Avatar>
...
Props do componente podem receber valores padrão para serem utilizados quando nenhum é fornecido.
...
O elemento `<slot />` é um placeholder para conteúdo HTML externo, permitindo que você injete (ou "encaixe") elementos filhos de outros arquivos no template do seu componente.

Por padrão, todos os elementos filhos passados para o componente serão renderizados em seu `<slot />`.

:::note
Diferente de _props_, que são atributos passados para um componente Astro e ficam disponíveis em todo o seu componente com `Astro.props`, _slots_ renderizam elementos HTML filhos onde eles estsão escritos.
:::
...
Este padrão é a base de um [componente de layout Astro](/pt-br/basics/layouts/): uma página inteira de conteúdo HTML pode ser "envolvida" com tags `<AlgumComponenteDeLayout></AlgumComponenteDeLayout>` e enviada ao componente para ser renderizada dentro dos elementos comuns de página definidos nele.

<ReadMore>Consulte as [funções utilitárias de `Astro.slots`](/pt-br/reference/astro-syntax/#astroslots) para conhecer outras maneiras de acessar e renderizar o conteúdo de slots.</ReadMore>

### Slots Nomeados

Um componente Astro também pode ter slots nomeados. Isso permite que você passe apenas elementos HTML com o nome de slot correspondente para a localização do slot.
...
<div id="invólucro-do-conteúdo">
  <Cabecalho />
  <!-- filhos com o atributo `slot="depois-do-cabecalho"` virão para cá -->
  <slot name="depois-do-cabecalho" />
  <Logo />
  <h1>{titulo}</h1>
  <!-- filhos sem um atributo `slot`, ou com `slot="default"`, virão para cá -->
  <slot />
  <Rodape />
  <!-- filhos com o atributo `slot="depois-do-rodape"` virão para cá -->
  <slot name="depois-do-rodape" />
</div>
```

Para injetar conteúdo HTML dentro de um slot em particular, use o atributo `slot` em qualquer elemento filho para especificar o nome do slot. Todos os outros elementos filhos do componente serão injetados no `<slot />` padrão (não nomeado).
...
Para passar múltiplos elementos HTML para o placeholder `<slot />` de um componente sem ter uma `<div>` envolta, use o atributo `slot=""` no [componente `<Fragment />` do Astro](/pt-br/reference/astro-syntax/#fragmentos):
...
Injete múltiplas linhas e colunas de conteúdo HTML usando o atributo `slot=""` para especificar o conteúdo do `"cabecalho"` e `"corpo"`. Elementos individuais do HTML também podem ser estilizados:

```astro title="src/components/TabelaDeEstoque.astro" {5-7, 9-13} '<Fragment slot="cabecalho">' '<Fragment slot="corpo">' '</Fragment>'
---
...
:::note
Não é possível gerar dinamicamente o nome de um slot Astro, como dentro de uma função [`map`](https://developer.mozilla.org/pt-BR/docs/Web/JavaScript/Reference/Global_Objects/Array/map). Se essa funcionalidade é necessária dentro de componentes de frameworks de UI, talvez seja melhor gerar esses slots dinâmicos dentro do próprio framework.
:::

### Conteúdo de Fallback para Slots

Slots também podem renderizar **conteúdo de fallback**. Quando nenhum filho correspondente for passado a um slot, um elemento `<slot />` irá renderizar seus próprios filhos como conteúdo substituto.
...
Conteúdo de fallback só será mostrado quando não houver elementos correspondentes com o atributo `slot="nome"` sendo passados para um slot nomeado.
...
- Quaisquer tags `<script>` deixam de fazer parte do bundle, sendo tratadas como se tivessem uma [diretiva `is:inline`](/pt-br/reference/directives-reference/#isinline).
- Eles podem apenas [referenciar assets que estão no diretório `public/`](/pt-br/basics/project-structure/#public).

:::note
Um [elemento `<slot />`](/pt-br/basics/astro-components/#slots) dentro de um componente HTML irá funcionar assim como funcionaria em um componente Astro. Para utilizar o elemento [slot de Web Components do HTML](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/slot) no lugar, adicione `is:inline` ao seus elemento `<slot>`.
:::
