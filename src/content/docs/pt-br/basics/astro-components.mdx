...
:::note
Não é possível gerar dinamicamente um slot nomeado no Astro, como por exemplo dentro de uma função map. Se essa funcionalidade é necessária dentro de um componente de um framework de interface, o melhor é gerar esses slots dinâmicos dentro do próprio framework.
:::
