...
Para passar múltiplos elementos HTML para um placeholder `<slot/>` de um componente sem ter uma `<div>` envolta, use o atributo `slot=""` no [Componente `<Fragment/>` do Astro](/pt-br/basics/astro-syntax/#fragmentos):
