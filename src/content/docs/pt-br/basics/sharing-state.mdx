...
Frameworks de UI como React ou Vue podem encorajar [provedores de "contexto"](https://react.dev/learn/passing-data-deeply-with-context) para outros componentes consumirem. Porém, ao [parcialmente hidratar componentes](/pt-br/basics/framework-components/#hidratando-componentes-interativos) no Astro ou Markdown, você não pode utilizar esses invólucros de contexto.
...
- Escrever para uma store de um arquivo `.astro` ou [componente não-hidratado](/pt-br/basics/framework-components/#hidratando-componentes-interativos) _não_ irá afetar os valores recebidos por [componentes no lado do cliente](/pt-br/reference/directives-reference/#diretivas-de-cliente).
