...
Agora que você tem algumas postagens do blog com links para, vamos configurar a página Blog para criar uma lista de postagens automaticamente!
...
2. Para gerar a lista inteira de postagens dinamicamente, utilizando os títulos das postagens e URLs, substitua suas tags `<li>` individuais com o seguinte código Astro:
    ...
    Sua lista inteira de postagens do blog agora está sendo gerada dinamicamente, por mapear um array retornado por `Astro.glob()`.
...
Tente sozinho fazer todas as mudanças necessários ao seu projeto Astro para que você ao invés disso, utilize o seguinte código para gerar sua lista de postagens do blog:
...
4. Importe o novo componente em sua página Blog.
    ...
    <details>
    <summary>Mostra o código</summary>
    ```astro title="src/pages/blog.astro" ins={3}
    ---
    import LayoutBase from '../layouts/LayoutBase.astro';
