...
4. Visite `localhost:3000/rss.xml` e verifique que você pode ver texto (não formatado) na página com um `item` para cada um dos seus arquivos `.md`. Cada item deve conter informação sobre a postagem do blog como um `title`, `url`, e `description`.
