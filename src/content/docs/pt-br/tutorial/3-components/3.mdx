...
2. Verifique a pré-visualização do seu navegador e verifique que o seu cabeçalho é mostrado em todas as páginas. Ele não se parecerá diferente ainda, mas se você inspecionar sua pré-visualização utilizando ferramentas de desenvolvedor, você irá ver que agora você tem elementos como `<header>` e `<nav>` entre os seus links de navegação.
...
Redimensione sua tela e veja os diferentes estilos sendo aplicados em diferentes larguras de tela. Seu cabeçakho agora é **responsivo** ao tamanho da tela através do uso de `@media` queries.
