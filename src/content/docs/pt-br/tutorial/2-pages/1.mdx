...
Agora que você sabe que arquivos `.astro` são responsáveis pelas páginas no seu website, vamos criar uma!
...
4. Adicione `/sobre` ao final da URL de pré-visualização do seu website na barra de endereço e verifique que você consegue ver uma página carregando lá. (e.x. `http://localhost:3000/sobre`)
...
Para fazer mais fácil de pré-visualizar todas as suas páginas, adicione links de navegação de página HTML antes do seu `<h1>` noa topo de ambas as suas páginas (`index.astro` e `sobre.astro`):
...
:::note
Diferente de vários frameworks, Astro utiliza elementos `<a>` padrões do HTML para navegar entre páginas (também chamadas rotas_), com recarregamento tradicional de página.
:::
...
Você agora deve ter um website com três páginas que tem links de uma para a outra. Vamos adicionar algum conteúdo para a página Blog.
