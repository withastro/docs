...
2. Importe e adicione o componente `<ThemeIcon />` em `Header.astro` para que ele seja mostrado em todas as páginas. Envolva tanto `<ThemeIcon />` quanto `<Menu />` dentro de uma `<div>` para agrupá-los para estilização, e adicione a tag `<style>` como mostrado abaixo para alguns estilos básicos que melhoram o layout.
