...
- **Páginas protegidas**: Se você precisa restrigir o acesso a uma página baseado nas permissões do usuário, você pode habilitar SSR para lidar com o acesso do usuário no servidor.
...
Ambos os modos `server` e `hybrid` permitem páginas e endpoints renderizados pelo servidor e vão renderizar todas as páginas de acordo com seu modo padrão. Entretanto, ambos os modos permitem que você marque uma página individual para optar em sair desse comportamento padrão.
...
### Dividir a build SSR em múltiplos arquivos

Por padrão, o Astro constrói todas as páginas em um único arquivo chamado `entry.mjs`. Ao invés disso, você pode configurar [`split`](/pt-br/reference/configuration-reference/#buildsplit) nas suas opções de build para emitir um único arquivo para cada página:
...
Os arquivos dentro da pasta `pages/` da build vão espelhar a estrutura de diretórios dos seus arquivos de página no `src/pages/`, por exemplo:
