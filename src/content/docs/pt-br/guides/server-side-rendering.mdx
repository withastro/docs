...
Para habilitar funcionaludades do SSR em deploys de produção, atualize sua configuração `output` para `'server'` ou `'hybrid'` (introduzido em **v2.6.0**). Ambos os modos controlam quais [páginas](/pt-br/core-concepts/astro-pages/) ou [endpoints do servidor](/pt-br/core-concepts/endpoints/#server-endpoints-api-routes) devem ser renderizadas pelo servidor. Cada opção de configuração possui um comportamento padrão, e permite que rotas individuais se recusem do comportamento padrão de acordo:
