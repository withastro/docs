...
Renderização no lado do Servidor, também conhecido como SSR,se refere a gerar páginas GTML no servidor sob-demanda e então enviar elas para o cliente.

SSR permite você fazer:
...
Considere hablitar a renderização no lado do servidor em seu projeto do Astro se você precisa do seguinte:
...
- **Páginas protegidas**: Se você precisa restrigir o acesso a páginas baseado no privilégio de usuários, você precisa habilitar SSR para lidar com o acesso de usuários no servidor.
...
Para habilitar funcionaludades do SSR em deploys de produção, atualize sua configuração `output` para `'server'` ou `'hybrid'` (introduzido em **v2.6.0**). Ambos os modos controlam quais [páginas](/pt-br/core-concepts/astro-pages/) ou [endpoints do servidor](/pt-br/core-concepts/endpoints/#endpoints-do-servidor-rotas-de-api) devem ser renderizadas pelo servidor. Cada opção de configuração possui um comportamento padrão, e permite que rotas individuais se recusem do comportamento padrão de acordo:

- __`output: 'server'`__: Renderizado pelo servidor por padrão. Use esta opção quando a maior parte ou todo o seu site deve ser renderizado pelo servidor. Qualquer página individual ou endpoint pode *concordar* de ser pré-renderizada.
- __`output: 'hybrid'`__: Pré-renderiza para HTML por padrão. Use este opção quando a maior parte do seu site deve ser estático. Qualquer página individual ou endpoint pode *recusar* de ser pré-renderizada.
...
Você pode então recusar do comportamento de renderização padrão com uma declaração de exportação em qualquer página ou rota:
...
```
Veja mais exemplos de uso de [configurar rotas individuais](#configurando-rotas-individuais)
...
O Astro continuará sendo um gerador de sites estáticos por padrão. Porém, quando você habilita a renderização no lado do servidor e adiciona um adapter, algumas novas funcionalidades são disponibilizadas a você.

### Configurando rotas individuais

Ambos os modos `server` como `hybrid` permitem páginas e endpoints renderizados pelo servidor e vão renderizar todas as páginas de acordo com seu modo padrão. Entretanto, ambos os mobos permitem que você marque uma página individual para recusar esse comportamento padrão.

### Recusando a renderização pelo servidor

Para um app majoritariamente renderizado pelo servidor configurador como `output: server`, adicione `export const prerender = true` para qualquer página ou rota para pré-renderizar uma página ou endpoint:
...
### Recusando a pré-renderização

Para um app majoritariamente renderizado pelo servidor configurador como `output: hybrid`, adicione `export const prerender = false` a qualquer arquivo que deve ser renderizado pelo servidor:
...
Os cabeçalhos de uma requisição estão disponíveis em `Astro.request.headers`. Isso funciona como o [`Request.headers`](https://developer.mozilla.org/en-US/docs/Web/API/Request/header) do navegador. Ele é um objeto [Headers](https://developer.mozilla.org/en-US/docs/Web/API/Headers), parecido com um Map, onde você pode pegar cabeçalhos como o cookie.
...
O método HTTP usado na requisição está disponível em `Astro.request.method`. Isso funciona como o [`Request.headers`](https://developer.mozilla.org/en-US/docs/Web/API/Request/header) do navegador. Ele retorna a representação em string do método HTTP usado na requisição.
...
Os navegadores nativamente suportando streaming HTTP, onde um documento é quebrando em pedaços, enviado para a rede em ordem, e renderizado na página naquela ordem.

Durante esse processo, os navegadores consomem o HTML incrementalmente: analisando, renderizando para o DOM e desenhando. Isso acontece você tendo feito stream do seu HTML intencionalmente ou não. As condições da rede podem causar que documentos grandes sejam baixados lentamente, e esperar por requisições de dados pode bloquear a renderização da página.
...
### Dividir o build SSR em múltiplos arquivos

Por padrão, o Astro constrói todas as páginas em um único arquivo chamado `entry.mjs`. Você pode configurar o [`split`](/pt-br/reference/configuration-reference/#buildsplit) nas suas opções de build para ao invés emitir um unico arquivo para cada página:
...
Os arquivos da página emitida vão ser salvos na pasta especificada para [`outDir`](/pt-br/reference/configuration-reference/#outdir), dentro da nova pasta `pages/` criada.

Os arquivos dentro da pasta `pages/` do build vão espelhar a estrutura de pastas dos seus arquivos de páginas no `src/pages/`, por exemplo:
