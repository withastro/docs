...
Para importar imagens dinamicamente da pasta `src/`, veja a seguinte receita:
...
Ao utilizar imagens em sua proporção de tela original, o `width` e `height` são opcionais. Essas dimensões podem ser inferidas automaticamente de imagens localizadas na pasta `src/` e de imagens remotas com [`inferSize` igual a `true`](#infersize).
...
Permite você atribuir o valor original de `width` e `height` de uma imagem remota automaticamente.

Por padrão, o valor é `false` e você precisa manualmente especificar ambas as dimensões para a suas imagens remotas.

Adicione `inferSize` ao seu componente `<Image />` (ou `inferSize: true` para `getImage()`) para inferir os valores do conteúdo da imagem quando é requisitada. Isso é útil quando você não sabe a dimensão da imagem remota, ou se ela pode mudar:

```astro mark="inferSize"
---
import { Image } from 'astro:assets';
---
<Image src="https://example.com/gato.png" inferSize alt="Um gato dormindo no sol.">
```

`inferSize` pode buscar as dimensões de uma [imagem remota de um domínio que não foi autorizado](#autorizando-imagens-remotas), porém a imagem em si continuará não sendo processada.
...
Imagens em coleções de conteúdo serão processadas da mesma forma que em [Markdown](#imagens-em-arquivos-markdown) e [MDX](#imagens-em-arquivos-mdx) dependendo de qual tipo de arquivo você está usando.
...
[Sharp](https://github.com/lovell/sharp) é o serviço de imagem padrão utilizado em `astro:assets`. Você ainda pode configurar o serviço de imagem usando a configuração [`image.service`](/pt-br/reference/configuration-reference/#imageservice).
...
### Configurando um serviço sem operação

Se o seu [adaptador para modo `servidor` ou `híbrido`](https://astro.build/integrations/?search=&categories%5B%5D=adapters) não suporta a otimização incluída no Astro  com Squoosh e Sharp (e.x. Deno, Cloudflare), você pode configurar um serviço de imagem sem operação para permitir que você use os componentes `<Image />` e `<Picture />`. Note que o Astro não faz nenhuma transformação de imagem ou processamente nesse ambiente. No entanto, você ainda pode aproveitar os outros benefícios de usar `astro:assets`, incluindo a prevenção de Cumulative Layout Shift (CLS), a imposição do atributo `alt`, e uma experiência consistente no uso.

Configure o `passthroughImageService()` para evitar ambos processamentos de imagem, Squoosh e Sharp:
