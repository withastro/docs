...
## Habilitar o pré-carregamento \{#enable-prefetching\}
...
## Configuração de pré-carregamento \{#prefetch-configuration\}

A configuração de `prefetch` também aceita um objeto opcional para customizar o pré-carregamento.

### Estratégias de pré-carregamento \{#prefetch-strategies\}
...
### Estratégia padrão de pré-carregamento \{#default-prefetch-strategy\}
...
### Pré-carregar todos os links por padrão \{#prefetch-all-links-by-default\}
...
A estratégia de pré-carregamento padrão pode ser alterada para todos os links com `prefetch.defaultStrategy`, como mostrado na [seção Estratégia padrão de pré-carregamento](#default-prefetch-strategy).

## Pré-carregar programaticamente \{#prefetch-programmatically\}
...
## Utilizando com Transições de Visualização \{#using-with-view-transitions\}

Quando você usa [Transições de Visualização](/pt-br/guides/view-transitions/) em uma página, o pré-carregamento também será habilitado por padrão. Isso define a configuração padrão de `{ prefetchAll: true }`, que habilita o [pré-carregamento para todos os links](#prefetch-all-links-by-default) da página.
...
## Migrando de `@astrojs/prefetch` \{#migrating-from-astrojsprefetch\}
