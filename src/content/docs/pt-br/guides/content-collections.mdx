...
:::note
O arquivo `src/content/config.ts` é opicional. Entretanto, escolher não definir suas coleções vai desabilitar algumas das suas melhores funcinoalidades como validação do esquema do frontmatter ou tipagem automática do TypeScript.
:::
...
Both functions return content entries as defined by the [`CollectionEntry`](/pt-br/reference/api-reference/#tipo-da-entrada-da-coleção) type.
