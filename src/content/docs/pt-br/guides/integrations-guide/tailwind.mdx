...
*   O template [Astro Tailwind](https://github.com/withastro/astro/tree/latest/examples/with-tailwindcss?on=github) te ajuda a começar com uma base para o seu projeto que utiliza o Tailwind para estilização.
...
*   O [Astro Ink](https://github.com/one-aalam/astro-ink), [Sarissa Blog](https://github.com/iozcelik/SarissaBlogAstroStarter) e [Creek](https://github.com/robertguss/Astro-Theme-Creek) utilizam o Tailwind para estilização.
...
Ao usar a diretiva `@apply` em um componente Astro, Vue, Svelte ou outra integração de componente dentro da tag `<style>`, ela pode gerar erros sobre a não existência da sua classe personalizada do Tailwind e fazer com que a compilação falhe.
...
*   Se você editar e salvar um arquivo e não ver a atualização do seu site correspondente, tente atualizar a página.
*   Se a atualização da página não atualizar a visualização, ou se uma nova instalação não parecer estar funcionando, reinicie o servidor de desenvolvimento.
...
[astro-ui-frameworks]: /pt-br/core-concepts/framework-components/#using-framework-components

## Contribuição

Este pacote é mantido pela equipe principal do Astro. Você é bem-vindo para enviar um problema ou PR!
