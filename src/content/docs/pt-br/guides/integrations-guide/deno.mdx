---
title: '@deno/astro-adapter'
description: O adaptador Deno para o Astro
githubIntegrationURL: 'https://github.com/withastro/astro/tree/main/packages/integrations/deno/'
i18nReady: true
---
O adaptador Deno permite o Astro fazer o deploy do seu site SSR para um serviço que tem Deno como target, incluindo Deno Deploy.

## Changelog

O adaptador Deno anteriormente era mantido pelo time do Astro, mas agora é mantido pela própria equipe do Deno. O seu uso está documentado no [repositório do adaptador Deno](https://github.com/denoland/deno-astro-adapter).

Se você atualmente esta usando esse adaptador Astro você irá precisar migrar para a nova versão do Deno ou [para outro adaptador](/pt-br/guides/on-demand-rendering/) para continuar usando SSR em seu projeto.
