...
Ao usar SSR, variáveis de ambiente podem ser acessadas em momento de execução baseado no adaptador de SSR sendo usado. Com a maioria dos adaptadores você pode acessar variáveis de ambiente com `process.env`, para o adaptador do Den você tem que usar `Deno.env.get()`. Cloudflare possui sua [própria forma](/pt-br/guides/integrations-guide/cloudflare/#environment-variables) de tratar variáveis de ambiente.
