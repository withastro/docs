...
Astro vem com suporte integrado para [TypeScript](https://www.typescriptlang.org/). Você pode importar arquivos `.ts` e `.tsx` em seu projeto Astro, escrever código TypeScript dentro de seu [componente Astro](/pt-br/basics/astro-components/#the-component-script), e até mesmo utilizar um arquivo [`astro.config.ts`](/pt-br/guides/configuring-astro/#the-astro-config-file) se você quiser.

Usando TypeScript, você pode evitar erros em tempo de execução ao definir os formatos dos objetos e componentes no seu código. Por exemplo, se você usa TypeScript para definir o [tipo das props do seu componente](#component-props), você vai receber um erro no seu editor se você definir uma prop que seu componente não aceita.
...
O servidor de desenvolvimento do Astro não executará nenhuma checagem de tipos, mas você pode usar um [script separado](#type-checking) para checar erros de tipo pela linha de comando.
...
### Plugin TypeScript para editores \{#typescript-editor-plugin\}
...
### Frameworks de UI \{#ui-frameworks\}
...
## Importações de Tipos \{#type-imports\}
...
## Aliases de Importação \{#import-aliases\}
...
## Estendendo `window` e `globalThis` \{#extending-window-and-globalthis\}
...
## Propriedades de Componentes \{#component-props\}
...
### Padrões comuns de tipos de prop \{#common-prop-type-patterns\}
...
## Utilitários de Tipagem \{#type-utilities\}
...
### Atributos HTML integrados \{#built-in-html-attributes\}
...
### `ComponentProps` type \{#componentprops-type\}
...
### Tipo polimórfico \{#polymorphic-type\}
...
O exemplo abaixo implementa um componente totalmente tipado e polimórfico que pode renderizar como qualquer elemento HTML. O tipo [`HTMLTag`](#built-in-html-attributes) é usado para garantir que a prop `as` é um elemento HTML válido.
...
### Inferir tipos de `getStaticPaths()` \{#infer-getstaticpaths-types\}
...
### Erros ao fazer a tipagem de múltiplos frameworks JSX ao mesmo tempo \{#errors-typing-multiple-jsx-frameworks-at-the-same-time\}
