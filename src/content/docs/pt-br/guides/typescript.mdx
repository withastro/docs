...
O servidor de desenvolvimento do Astro não executará nenhuma checagem de tipo, mas você pode usar um [script separado](#checagem-de-tipos) para checar erros de tipo pela linha de comando.
...
Para checar que o plugin está funcionando, crie um arquivo `.ts` e importe um componente do Astro nele. Vocêe nao deve ter nenhuma mensagen de aviso do seu editor.
...
Se você só quer definir o tipo de uma propriedade no objeti `window`, forneça uma interface `Window` ao invés disso:
...
- Se seu componente precisa ser passado filhos para seu slot padrão, você pode impor isso usando `type Props = { children: any; };`.
...
O Astro fornece os tipos `HTMLAttributes` para checar que sua marcação está usando atributos HTML válidos. Você pode usar esses tipos para ajudar construir as props de componentes.

Por exemplo, se você está construindo um componente `<Link>`, você pode fazer o seguinte para espelhar os atributos padrão para tags `<a>` nos tipos da prop do seu componente.
...
O exemplo abaixo implementa um compomente totalmente tipado e polimófico que pode renderizar como qualquer elemento HTML. O tipo [`HTMLTag`](#atributos-html-integrados) é usado para garantir que a prop `as` é um elemento HTML válido.
...
O Astro inclui utilitários para trabalhar com os tipos retornados pelas suas funções [`getStaticPaths()`](/pt-br/reference/api-reference/#getstaticpaths) para rotas dinâmicas.
