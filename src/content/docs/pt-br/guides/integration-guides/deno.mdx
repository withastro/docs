...
Aprenda a realizar o deploy do seu site Astro em nosso [guia de deployment com Deno Deploy](/pt-br/guides/deploy/deno/).
...
Se você deseja [usar renderização no lado do servidor (SSR)](/pt-br/guides/server-side-rendering/), o Astro requer um adaptador que corresponda ao runtime do deployment.
...
Adicione o adaptador Deno para permitir o SSR no seu projeto Astro com o seguinte comando `astro add`. Isso irá instalar o adaptar e realizar as mudanças apropriadas para o seu arquivo `astro.config.js` em um único passo.
...
Se você preferir instalar o adaptador manualmente em vez disso, complete os dois passos seguintes:
...
Após [realizar uma build](/pt-br/guides/deploy/#fazendo-build-do-seu-site-localmente) haverá um módulo `dist/server/entry.mjs`. Você pode inicializar um servidor ao importar este módulo no seu app Deno:
...
Se você desativar isso, você precisa escrever o seu próprio servidor web do Deno. Importe e chame `handle` do script de entrada gerado para renderizar requests:
...
Este pacote é mantido pela equipe principal do Astro. Sinta-se livre para submeter uma issue ou PR!
