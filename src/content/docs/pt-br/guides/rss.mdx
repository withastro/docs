...
O pacote [`@astrojs/rss`](https://github.com/withastro/astro/tree/main/packages/astro-rss) fornece utilitários para a geração de feeds RSS utilizando [endpoints de API](/pt-br/basics/endpoints/#endpoints-de-arquivos-estáticos). Isso permite builds estáticas *e* geração sob demanda quando você estiver utilizando um [adaptador de SSR](/pt-br/guides/server-side-rendering/).
