...
O arquivo de exemplo abaixo `src/pages/rss.xml.js` vai cirar um feed RSS em `site/rss.xml`.
...
Opicional: substitua seus esquemas de coleções do blog para garantir essas propriedades de RSS esperadas.

Para garantir que cada entrada da coleção produz um item válido no feed RSS, você pode opicionalmente importar e aplicar `rssSchema` ao invés de definir cada propriedade individual do seu esquema.
