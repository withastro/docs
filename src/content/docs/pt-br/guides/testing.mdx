...
Na raiz do seu projeto, cria um arquivo `cypress.config.js` com o seguinte conteúdo:
...
2. Crie um arquivo `index.spec.js` na pasta `cypress/e2e`. Use o seguinte teste no arquivo para verificar que o título da página e o cabeçalho estão corretos.
    ...
    Você pode definir [`"baseUrl": "http://localhost:3000"`](https://docs.cypress.io/guides/end-to-end-testing/testing-your-app#Step-3-Configure-Cypress) no arquivo de configuração `cypress.config.ts` para usar `cy.visit("/")` ao invés de `page.goto("http://localhost:3000/")` para uma URL mais conveniente.
    :::
...
Promeiro, inicie o servidor de desenvolvimento para que o Cupress possa acessar seu site ao vivo.
...
1. Em `playwright.config.ts`, adicione o objeto `webServer` e atualize o valor de `command` para `yarn preview`.
...
1. Execute `yarn build`, e em seguida execute `yarn test:e2e` para rodar os testes do Playwright.
