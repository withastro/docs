...
2. Crie um arquivo `index.cy.js` na pasta `cypress/e2e`. Use o seguinte teste no arquivo para verificar que o título da página e o cabeçalho estão corretos.
    ...
    Você pode definir [`"baseUrl": "http://localhost:3000"`](https://docs.cypress.io/guides/end-to-end-testing/testing-your-app#Step-3-Configure-Cypress) no arquivo de configuração `cypress.config.js` para usar `cy.visit("/")` ao invés de `page.goto("http://localhost:3000/")` para uma URL mais conveniente.
    :::
