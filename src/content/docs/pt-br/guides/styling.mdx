...
Para usar o Tailwind no seu projeto, instale a [integração Tailwind oficial do Astro][tailwind] usando o comando `astro add` no seu gerenciador de pacotes:
...
A opção `'auto'` vai colocar em linha apenas as folhas de estilo que são abaixo do limite `vite.build.assetsInlineLimit`, reduzindo o número de requisições por folhas menores. Grandes folhas de estilo, como as globais usadas por todas as páginas, ainda serão buscadas externamente e armazenadas em cache. Essa opção fornece um balanço entre o número de folhas de estilos carregadas e os estilos que podem ser armazenados em cache entre páginas.
