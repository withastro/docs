...
Ao usar a estratégia de estilo isolado padrão, você também deve passar o atributo `data-astro-cid-*`. Você pode fazer isso passando o `...rest` das props para o componente. Se você alterou a propriedade `scopedStyleStrategy` para `'class'` ou `'where'`, a prop `...rest` não é necessária.
...
:::note[Estilos isolados de componentes pais]
...
Quaisquer métodos de estilização no Astro estão disponíveis em um [componente de layout Markdown](/pt-br/core-concepts/layouts/#layouts-markdownmdx), porém diferentes métodos terão diferentes efeitos na estilização da sua página.
...
Você também pode adicionar [integrações de CSS](#integrações-css), incluindo o [Tailwind](/pt/guides/guia-de-integracoes/tailwind/). Se você estiver usando o Tailwind, o [plugin de tipografia](https://tailwindcss.com/docs/typography-plugin) pode ser útil para estilizar o Markdown.
