...
Você pode publicar tanto projetos Astro totalmente estáticos quanto renderizados sob demanda no Clever Cloud. Independentemente do seu modo de `output` ([pré-renderizado ou sob demanda](/pt-br/basics/rendering-modes/)), você pode optar por publicar como uma **aplicação estática** que roda usando um _hook_ pós-build, ou como uma **aplicação Node.js**, que funciona prontamente com seu `package.json`.
