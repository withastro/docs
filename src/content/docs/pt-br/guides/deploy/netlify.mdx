...
Você pode fazer o deploy do seu projeto utilizando também a Netilify Edge Functions adicinando `edgeMiddleware: true` na configuração do adaptador Netilify.
    ```diff lang="js"
      // astro.config.mjs
      import { defineConfig } from 'astro/config';
      import netlify from '@astrojs/netlify/functions';
