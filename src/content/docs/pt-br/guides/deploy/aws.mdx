...
Usar `pnpm` vai exigir configurações ligeiramente diferentes para armazenar em cache o diretário de armazenamento ao invés de `node_modules`. Abaixo está a configuração de build recomendada:
