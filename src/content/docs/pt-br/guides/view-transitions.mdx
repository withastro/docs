...
Suporte para **transições de visualização, opcionais, por página** em projetos Astro pode ser habilitado com uma flag experimental. Transições de visualização atualizam o contaúdo de sua página sem a atualização normal, atualizando a página inteira, do navegador e oferece animações suaves entre páginas.
...
O suporte para transições de visialização do Astro é possível gracs à nova API de [Transições de Visualização](https://developer.chrome.com/docs/web-platform/view-transitions/) do navegador e também inclúi:

- Algumas [animações integradas](#animcações-integradas), como `slide` e `fade`.
- Suporte a animações de navegação tanto avançando quanto retornando.
- A habilidade de [customizar completamente todos os aspectos da animação de transição](#customizando-animcações), e construir suas próprias animações.
...
Importe e adiciona o componente `<ViewTransitions />` em seu `<head>` comum ou componente de layout. Astro criará animações padrões baseado nas similaridades entre a página antiga e a nova e também oferecerá um comportamento de fallback para navegadores sem suporte.
...
Use diretivas `transition:*` opcionais em elementas da página em seus componentes `.astro` para exercer um controle fino sobre o comportamento da transição de página durante a navegação.
...
Note que o nome em `transition:name` só pode ser utilizado uma vez em cada página. Define esse valor manualmente quando Astro não conseguir infer um nome apropriado sozinho, or para ter um controle mais fino sobre os os elementos associados.
...
Astro inclúi algumas animação integradas para sobrescrever a transição padrão `morph`. Adicione a diretiva `transition:animate` para experimentar as transições `slide` ou `fade` do Astro.
...
Para examinar uma animação integrada, primeiro importe a animação de `astro:transitions`, e então passe opções de customização.
...
export interface TransitionAnimation {
  name: string; // O nome do quadro chave.
  delay?: number | string;
...
O roteamento de `<ViewTransitions />` funciona melhor em navegadores que suportam Transições de Animação (ex.: Navegadores baseado no Chromium), mas também inclúi um fallback default para suportar outros navegadores. Mesmo se o navegador não suportar a API de Transições de Visualização, Astro ainda proverá navegação dentro do navegador utilizando uma das opções de fallback para uma experiência comparável.
