...
**Middleware** lhe permite interceptar chamadas e respostas e injetar comportamentos dinamicamente toda vez que uma página ou endpoint for ser renderizado.

Isso também permite definir e compartilhar informações específicas da chamada entre endpoints e páginas alterando o objeto `locals` que é disponibilizado em todos os componentes Astro e endpoints de API.
...
2. Dentro deste arquivo, exporte uma função [`onRequest()`](#onrequest). Esta exportação não pode ser um default export.
    ...
    export function onRequest ({ locals, request }, next) {
        // intercepta dados de uma chamada
        // opcionalmente, transforma a chamada modificando `locals`
...
You can store any type of data inside `Astro.locals`: strings, numbers, e até mesmo tipos complexos como funções e maps.

```js title="src/middleware.js"
export function onRequest ({ locals, request }, next) {
    // intercepta dados de resposta de uma chamada
...
async function validacao(_, next) {
    console.log("validação da chamada");
    const resposta = await next();
...
async function auth(_, next) {
    console.log("autenticação da chamada");
    const resposta = await next();
...
async function cumprimentos(_, next) {
    console.log("cumprimentos da chamada");
    const resposta = await next();
...
Uma função exportada obrigatória do arquivo `src/middleare.js` que será chamada antes da renderização de cada página ou rota da API.
...
Um objeto que inclúi informações a serem disponibilizadas para outro middleware, rotas de API e rotas `.astro` durante o processo de renderização.
...
Uma função que intercepta (lê e modifica) a resposta (`Response`) de uma chamada (`Request`) ou invoca o próximo middleware na sequência e retorna uma `Response`.
...
Esse objeto `locals` é repassado através do processo de processamento de chamada e é diponibilizado como uma propriedade em [`APIContext`](/pt-br/reference/api-reference/#contexto-de-endpoint) e [`AstroGlobal`](/pt-br/reference/api-reference/#global-astro).
