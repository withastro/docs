...
- Transições de Visualização para transições de página animadas e ilhas persistentes. Veja as [mudanças radicais da API de transições de visualização e dicas para atualização](/pt-br/guides/view-transitions/#upgrade-to-v30-from-v2x) se você estava utilizando essa flag experimental.
- Uma nova API de serviço de imagem `astro:assets` para utilizar imagens no Astro, incluindo um novo componente `<Image />` e a função `getImage()`. Por favor leia as detalhadas [dicas de atualização para imagens](/pt-br/guides/images/#upgrade-to-v30-from-v2x) **se você estava ou não utilizando essa flag experimental** para ver como isso pode afetar seu projeto.
...
2. Verifique a própria documentação do seu [ambiente de deployment](/pt-br/guides/deploy/) para verificar que ele suportam Node 18. 
...
Astro v3.0 atualize as predefinições de `tsconfig.json` para apenas suportar TypeScript 5.x. Astro agora assume que você utiliza TypeScript 5.0 (Março de 2023), ou que seu editor o inclui (e.x. VS Code 1.77).
...
Remova a integração `@astrojs/image` do seu projeto. Você irá precisará não só desinstalar a integração mas também atualizar ou remover quaisquer declarações de importação e quaisquer componentes `<Image />` e `<Picture />` existentes. Você também vai precisar configurar um serviço de processamento de imagem padrão de sua preferência.

Você irá encontrar [instruções passo a passo completas para remover a integração de imagens antiga](/pt-br/guides/images/#remove-astrojsimage) em nosso guia de Imagens.

Migrar para `astro:assets` também irá trazer algumas novas opções de imagem e funcionalidades que você pode desejar utilizar agora. Por favor veja as [dicas de atualização para imagens da v3.0](/pt-br/guides/images/#upgrade-to-v30-from-v2x) para mais detalhes!
...
No Astro v1.x, Astro depreciou opções da configuração assim como os tipos de atributo de script/style `global` e `hoist`. Esses foram mantidos por compatibilidade com versões anteriores.

Astro v3.0 remove essas APIs depreciadas completamente. Elas não podem mais ser usadas no seu projeto Astro.
...
No Astro 2.x, a API de coleções de conteúdo depreciou a exportação `image` do `astro:content` utilizada no esquema de suas coleções de conteúdo.

Astro 3.0 remove essa exportação completamente.

#### O que devo fazer?

Se você está utilizando o `image()` depreciado do `astro:content`, o remova, já que não ele não existe mais. Valide imagens através do [utilitário `image` de `schema`](/pt-br/guides/images/#atualize-esquemas-de-coleções-de-conteúdo) no lugar:
...
No Astro v2.x, [valores de `class:list`](/pt-br/reference/directives-reference/#classlist) eram passadis para componentes através de [`Astro.props['class:list']`](/pt-br/reference/api-reference/#astroprops).
...
No Astro v2.x, [variáveis CSS](/pt-br/guides/styling/#variáveis-css) camelCase passadoas para o atributo `style` eram renderizadas tanto como camelCase (assim como foi escrito) como em kebab-case (mantido por compatibilidade com versões anteriores).

Astro 3.0 remove a transformação para kebab-case de nomes de variáveis CSS em camelCase, e apenas a variável CSS original em camelCase é renderizada.
...
```js title="endpoint.json.ts" del={2} ins={3}
...
No Astro v2.x, Squoosh era o [serviço padrão de processamento de imagem](/pt-br/guides/images/#default-image-service).
...
No Astro v2.x, [métodos de requisição HTTP](/pt-br/core-concepts/endpoints/#métodos-http) eram escritos usando nomes de função em letras minúsculas: `get`, `post`, `put`, `all` e `del`.
...
Você pode com segurança removar qualquer código que utiliza `has()` para verificar se o valor de `Astro.cookies` é `undefined`:
...
Para [executar a CLI do Astro programaticamente](/pt-br/reference/cli-reference/#advanced-apis-experimental), utilize as novas APIs JavaScript experimentais:
