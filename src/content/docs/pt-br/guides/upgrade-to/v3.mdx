...
Se você está usando APIs da Web em componente renderizados pelo servidor, você vai precisar de ou fazer uso dessas APIs condicionalmente ou usar [a diretiva do client `client:only`](/pt-br/reference/directives-reference/#clientonly).
...
### Padrão alterado: `verbatimModuleSyntax` nas predefinições tsconfig.json
