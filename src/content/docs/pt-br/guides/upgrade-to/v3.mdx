...
No Astro v2.x, [métodos de requisição HTTP](/pt-br/basics/endpoints/#métodos-http) eram escritos usando nomes de função em letras minúsculas: `get`, `post`, `put`, `all` e `del`.
