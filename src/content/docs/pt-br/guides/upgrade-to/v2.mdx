...
```js del={4} ins={5} title="astro.config.mjs"
...
Na v1.x, assets erão construídos em várias localizações, incluindo `assets/`, `chunks/` e na raiz do resultado da build.
...
Na v1.x, Astro utilizava `markdown.extendDefaultPlugins` para rehabilitar os plugins padrões do Astro quando se adiciona seus próprios plugins Markdown.
...
Astro v2.0 modifica `markdown.extendDefaultPlugins: false` com opções Boolean sepoaradas para individualmente controlar cada um dos plugins Markdown padrões do Astro. Eles são habilitados por padrão e podem ser definidos como `false` independentemente.
...
```js del={5} ins={6-7} title="astro.config.mjs"
import { defineConfig } from 'astro/config';
...
```js del={7} title="astro.config.mjs"
import { defineConfig } from 'astro/config';
...
Substitua `extendPlugins: 'defaults'` com `extendMarkdownConfig: false` e adiciona as opções speradas para GitHub-Flavored Markdown e SmartyPants para habilitar esses plugins padrões individualmente no MDX.

```js del={7} ins={8-10} title="astro.config.mjs"
import { defineConfig } from 'astro/config';
import mdx from '@astrojs/mdx';
...
Verifique quaisuqer plugins remark e rehype que você escreveu para ver se seus comportamentos foram modificados. Note que `data.astro.frontmatter` agora é o frontmatter _completo_ do documento Markdown ou MDX, ao invés de ser um objeto vazio.
...
```js ins={2, 7, 9} title="src/pages/rss.xml.js"
import rss, {
  pagesGlobToRssItems
} from '@astrojs/rss';
...
Astro 2.0 requer um arquivo `svelte.config.js` em seu projeto se você estiver utilizando [a integração `@astrojs/svelte`](/pt-br/guides/integrations-guide/svelte/). Isso é necessário para fornecer preenchimento automático na IDE.
...
```js del={2-4} title="astro.config.mjs"
export default defineConfig({
	legacy: {
...
```js del={4-8} title="astro.config.mjs"
import { defineConfig } from 'astro/config';
...
- [Pré-renderizar páginas individuais para HTML estático](/pt-br/guides/server-side-rendering/#renderização-híbrida) ao utilizar SSR para melhorar a velocidade e cacheabilidade.
