...
A coisa mais importante de entender sobre componentes Astro é que eles **não renderizam no lado do cliente**. Eles renderizam para HTML tanto na hora do build ou sob demanda usando [renderização no lado do servidor (SSR, do inglês server-side rendering)](/pt-br/guides/server-side-rendering/). Você pode incluir código JavaScript dentro do frontmatter do seu componente, e todo ele vai ser removido da página final enviada para o navegador dos seus usuários. O resultado é um site mais rápido, com nenhum JavaScript por padrão.

Quando seu componente Astro precisa de interatividade no lado do cliente, você pode adicionar [tags `<script>` tradicionais do HTML](/pt-br/guides/client-side-scripts/) ou [componentes de um Framework UI](/pt-br/core-concepts/framework-components/#hidratando-componentes-interativos).
...
Porém, a [sintaxe do template do script do Astro](/pt-br/core-concepts/astro-syntax/) também suporta **expressões JavaScript**, tags do Astro  [`<style>`](/pt-br/guides/styling/#estilização-em-astro) e [`<script>`](/pt-br/guides/client-side-scripts/#utilizando-script-no-astro), **componentes importados**, e [**diretivas especiais do Astro**](/pt-br/reference/directives-reference/). Dados e valores definidos (em tempo de build da página) no script de componente podem ser utilizados no template do script para produzir HTML criado dinamicamente.
...
<!-- Inclua outros componentes de frameworks UI com a diretiva `client:` para hidratá-los: -->
