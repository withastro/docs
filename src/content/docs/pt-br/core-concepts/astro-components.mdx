...
Quando seu componente Astro precisa de interatividade no lado do cliente, você pode adicionar [tags `<script>` tradicionais do HTML](/pt-br/guides/client-side-scripts/) ou [componentes de um Framework UI](/pt-br/core-concepts/framework-components/#hydrating-interactive-components).
...
Porém, a [sintaxe do template do script do Astro](/pt-br/core-concepts/astro-syntax/) também suporta **expressões JavaScript**, tags do Astro  [`<style>`](/pt-br/guides/styling/#styling-in-astro) e [`<script>`](/pt-br/guides/client-side-scripts/#using-script-in-astro), **componentes importados**, e [**diretivas especiais do Astro**](/pt-br/reference/directives-reference/). Dados e valores definidos (em tempo de build da página) no script de componente podem ser utilizados no template do script para produzir HTML criado dinamicamente.
