...
```astro "<slot />" title="src/layouts/LayoutDoMeuSite.astro"
---
...
```astro /(?<!//.*){?frontmatter(?:\\.\w+)?}?/ "<slot />" title="src/layouts/LayoutBase.astro"
---
...
```astro "utilitarioSofisticadoJS" "titulo" title="src/layouts/LayoutBase.astro"
---
...
```astro /{?titulo}?/ /Astro.props[.a-z]*/ title="src/components/MeuLayout.astro"
---
...
```astro {2} /</?LayoutBase>/ /</?LayoutBase url={frontmatter.url}>/ title="src/layouts/LayoutPostagemBlog.astro"
---
