...
Parâmetros podem ser incluidos em partes separadas do caminho. Por exemplo, o arquivo `src/pages/[lingua]/[versao]/info.astro` com o mesmo `getStaticPaths()` acima irá gerar as rotas `/en/v1/info` e `/fr/v2/info`.
...
Você pode definir regras para [redirecionar usuários para páginas permanentemente movidas](#redirecionamentos-configurados-experimental) na sua configuração do Astro. Ou, [redirecionar usuários dinamicamente](#redirecionamentos-dinâmicos) enquanto eles utilizam seu site.

### Redirecionamentos Configurados (experimental)
<Since v="2.6.0" />

Depois de habilitar a flag experimental `redirects: true`, você pode especificar um mapeamento de redirecionamentos permanentes na sua configuração do Astro. Para a maioria dos redirecionamentos, isto é um mapeamento de uma rota antiga para uma nova rota:
...
Redirecionamento também segue as mesmas regras, mas são priorizados *por último*; se existe uma rota baseada em arquivo e um redirecionamento com o mesmo número de prioridade da rota, a rota baseada em arquivo é escolhida.
...
Você pode excluir páginas ou até mesmo diretórios inteiros da build ao prefixar seus nomes com um subtraço (`_`). Arquivos com o prefixo `_` não serão reconhecidas pelo roteador e não serão adicionadas ao diretório `dist/`.
