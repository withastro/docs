...
As variáveis locais podem ser usadas em funções do JSX para produzir elementos HTML gerados dinamicamente:
...
O Astro pode exibir condicionalmente o HTML usando operadores lógicos do JSX e expressões ternárias.
...
```astro title="src/components/TagsDinamicas.astro" /Element|(?<!Meu)Componente/
---
...
- **Diretivas de hidratação não são suportadas.** Ao usar as [diretivas de hidratação `client:*`](/pt-br/core-concepts/framework-components/#hidratando-componentes-interativos), o Astro precisa saber qual componente deve ser compôr o bundle para produção e o padrão de tags dinâmicas impede que isso funcione.

### Fragmentos

O Astro suporte usar tanto `<Fragment></Fragment>` ou a abreviação `<></>`.
...
---
const htmlString = '<p>Conteúdo HTML cru</p>';
---
...
A sintaxe de componente Astro é um superconjunto do HTML. Ela foi projetada para sentir familiar a qualquer um com experiência com HTML ou JSX, mas existem algumas diferenças importantes entre arquivos `.astro` e o JSX.
...
Um template de componente do Astro pode renderizar vários elementos sem a necessidade de envolver tudo em um único `<div>` ou `<>`, diferente do JavaScript ou JSX.
...
---
---
<!-- A sintaxe de comentário HTML é válida em arquivos .astro -->
{/* A sintaxe de comentário JS também é válida */}
```

:::caution
Os comentários no estilo HTML serão incluídos no DOM do navegador, enquanto os JS serão ignorados. Para deixar mensagens TODO ou outras explicações somente para desenvolvimento, você pode querer usar comentários no estilo JavaScript ao invés disso.
:::
