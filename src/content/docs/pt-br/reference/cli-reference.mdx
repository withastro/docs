...
Um dos comandos que você vai usar com mais frequência é `astro dev`. Esse comando inicia o servidor de desenvolvimento e dá a você uma pré visualização ao vivo e atualizado do seu site em um navegador conforme você trabalha:
...
Comandos 
                   ...
                   build  Constrói seu projeto e escreve ele no disco
               ...
               telemetry  Configura configurações de telemetria.
...
Quando você segue as instruções para [instalar o Astro manualmente](/pt-br/install/manual/#2-instale-astro), você é instruido a adicionar esses scripts você mesmo. Você também pode adicionar mais scripts para essa lista manualmente para qualquer comando você usa frequentemente.
...
<PackageManagerTabs>
  ...
  ```
  (O `--` a mais antess da flag `--port` é necessário para o `npm` passar suas flags para o comando `astro`.)
  </Fragment>
  <Fragment slot="pnpm">
  ```shell
  # executa o servidor de desenvolvimento na porta 8080 usando o script `start` no `package.json`
...
Esse estas flags para customizar o comportamento do comando.
...
Reporta informações úteis sobre sua ambiente do Astro atual. Útil para fornecer informação ao abrir uma issue.
