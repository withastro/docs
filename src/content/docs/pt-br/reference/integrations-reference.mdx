...
Adiciona uma [diretiva customizada do cliente](/pt-br/reference/directives-reference/#diretivas-customizadas-do-cliente) para ser usada em arquivos do Astro. Como isso é uma funcionalidade experimental, você também precisa habilitar a [opção de configuração `experimental.customClientDirectives`](/pt-br/reference/configuration-reference/#experimentalcustomclientdirectives) para usar essa API.

Note que entrupoitns de diretivas são agrupados somente através do esbuild e devem ser mantidos pequenos para que não deixem a hidratação de componentes mais lenta.
...
__astro-diretiva-clique/clique.js__
...
__astro-diretiva-clique/index.d.ts__
