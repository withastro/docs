...
`paginate()` recebe os seguintes arguments:
- `pageSize` - O número de itens exibido por página
...
A função `getImage()` tem a intenção de gerar imagens para serem usadas em lugares não diretamente no HTML, por exemplo em uma [Rota de API](/pt-br/core-concepts/endpoints/#endpoints-do-servidor-rotas-de-api). Ela também permite que você crie seu próprio componente `<Image />` customizado.
...
O objeto `context` que `defineCollection` para a função `shape` do `schema`. Esse tipo pode ser útil ao criar schemas reutilizáveis para múltiplas coleções.
...
Uma função exportada obrigatória do arquivo `src/middleware.js` que será chamada antes da renderização de cada página ou rota da API. Ela aceita dois argumentos opcionais: [context](#context) e [next()](#next). `onRequest()` deve retornar uma `Response`: ou diretamente, ou chamando `next()`.
