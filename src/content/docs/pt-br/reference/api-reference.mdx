...
Uma slug pronta para URLs para documentos Markdown ou MDX. Por padrão é o `id` sem a extensão de arquivo, mas pode ser sobrescrita ao definir [a propriedade `slug`](/pt-br/guides/content-collections/#defining-custom-slugs) no frontmatter de um arquivo.
