...
`context.locals` é um objeto disponível **somente em funções de middleware**. Você pode usar esse objeto para armazenar informações
arbitrárias durante o ciclo de vida de uma requisição.
...
As funções [`getCollection()`](#getcollection) e [`getEntryBySlug()`](#getentrybyslug) retornam entradas com o tipo `CollectionEntry`. Esse tipo está disponível como um utilitário de `astro:content`:
...
Uma slug pronta para URLs. Por padrão é o `id` sem a extensão de arquivo, mas pode ser sobreescrita ao definir [a propriedade `slug`](/pt-br/guides/content-collections/#defining-custom-slugs) no frontmatter de um arquivo.
