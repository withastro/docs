...
Veja também: [Passando Dados com `props`](/pt-br/reference/routing-reference/#data-passing-with-props)
...
Obtém o cookie como um objeto [`AstroCookie`](#astrocookie-type), que contém `value` e funções utilitárias para converter o cookie em tipos não-string.
...
Uma vez que um cookie é "deletado" (expirado), `Astro.cookies.has()` retornará `false` e `Astro.cookies.get()` retornará um [`AstroCookie`](#astrocookie-type) com `value` de `undefined`. Opções disponíveis ao deletador um cookie são: `domain`, `path`, `httpOnly`, `sameSite`, e `secure`.
...
export interface MarkdownInstance<T extends Record<string, any>> {
  /* Qualquer dado especificado no frontmatter YAML/TOML do arquivo */
	frontmatter: T;
  /* O caminho de arquivo absoluto este arquivo */
	file: string;
...
Outros arquivos podem ter várias interfaces diferentes, mas `Astro.glob()` aceita um TypeScript genérico se você souber exatamente o que um tipo de arquivo desconhecido contém.
