...
[Veja o guia `Coleções de Conteúdo`](/pt-br/guides/content-collections/#defining-collection-references) para um exemplo de uso.
...
// Obtém o erfil referenciado por `data.capitao`
const empreendimentoCapitaoPerfil = await getEntry(empreendimento.data.capitao);
---
```

Veja o guia `Coleções de Conteúdo` para exemplos de  [consultando entradas de coleção](/pt-br/guides/content-collections/#querying-collections).
...
`getEntries()` é uma função que recupera múltiplas entradas de uma mesma coleção. Isso é útil para [retornar um array de entradas referenciadas](/pt-br/guides/content-collections/#defining-collection-references) para acessar as propriedades `data`, `body` e `render()` associadas a elas.
...
const capitaoPerfil = await getEntries(empreendimentoPostagem.data.postagensRelacionadas);
---
```
...
[Veja o guia de `Coleções de Conteúdo`](/pt-br/guides/content-collections/#querying-collections) para exemplos de uso.
...
O nome de uma pasta de nível superior dentro de `src/content/` onde as entradas são localizadas. Este é o nome usado para referencias a coleção no seu esquema e em funções de consulta.
...
Uma slug pronta para URLs para documentos Markdown ou MDX. Por padrão é o `id` sem a extensão de arquivo, mas pode ser sobreescrita ao definir [a propriedade `slug`](/pt-br/guides/content-collections/#defining-custom-slugs) no frontmatter de um arquivo.
...
[Veja a seção guia de `getCollection()`](/pt-br/guides/content-collections/#rendering-content-to-html) para exemplos completos de uso.
