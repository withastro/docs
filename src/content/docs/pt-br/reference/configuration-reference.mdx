...
Para sites estaticamente gerados sem nenhum adapter instalado, isso vai produzir um redirecionamento no cliente usando uma [tag `<meta http-equiv="refresh">`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#http-equiv) e não suporta códigos de status.

Quando estiver usando SSR ou um adapter estático no modo `output: static`, códigos de status não são suportados.
O Astro irá server requisições GET redirecionadas com um status de `301`
e usar um status de `308` para qualquer outro método de requisição.

Você pode customizar o [codigo de status de redirecionamento](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status#redirection_messages) usando um objeto na configuração de redirecionamento:
...
Espeficica se os redirecionamentos são enviados como HTML durante o build.
Essa opção só se aplica para o modo `output: 'static'`; em SRR os redirecionamentos
...
de configuração especiais para redirecionamento a não precisa/querem redirecionamentos baseados em HTML.
...
Define se algum código de middleware SSR vai ser empacotado ou não quando o build for feito.

Quando habilitado, código de middleware não é empacotado e importado por todas as páginas durante o build. Para ao invés disso executar e importar códigos de middleware manualmente, defina `build.excludeMiddleware: true`:
...
Habilita o suporte experimental para a configuração de redirecionamento. Com isso habilitado
você pode definir redirecionamento através da propriedade `redirects` no nível superior. Para habilitar
essa funcionalidade, defina `experimental.redirects` para `true`.
```js
{
	experimental: {
		redirects: true,
