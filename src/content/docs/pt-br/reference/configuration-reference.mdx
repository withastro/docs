...
A opção de configuração da build `build.split` foi substituída pela opção de configuração do adaptador [`functionPerRoute`](/pt-br/reference/adapter-reference/#functionperroute).

Por favor veja a [documentação do seu adaptador SSR](/pt-br/guides/integrations-guide/#integrações-oficiais) para utilizar `functionPerRoute` para definir como seu código SSR deve passar por bundle.
...
A opção de configuração da build `build.excludeMiddleware` foi substituída pela opção de configuração do adaptador [`edgeMiddleware`](/pt-br/reference/adapter-reference/#edgemiddleware).

Por favor veja a [documentação do seu adaptador SSR](/pt-br/guides/integrations-guide/#integrações-oficiais) para utilizar `edgeMiddleware` para definir se algum ou nenhum código de middleware SSR irá passar por bundle durante a build.
...
Essa opção requer uma matriz de nomes de domínio individuais como strings. Coringas não são permitidos. Ao invés disso, use [`image.remotePatterns`](#imageremotepatterns-experimental) para definir uma lista de padrões de URL de origens permitidos.
