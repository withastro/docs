...
Permite integrações usarem a [API experimental `addClientDirective`](/pt-br/reference/integrations-reference/#addclientdirective-option) no hook `astro:config:setup` para adicionar diretivas customizadas do cliente nos arquivos do Astro.

Para habilitar essa funcionalidade, defina `experimental.customClientDirectives` para `true` na sua configuração do Astro:
...
Habilite o suporte experimental para SSR híbrido com pré-renderização habilitada por padrão.

Para habilitar essa funcionalidade, primeiro defina `experimental.hybridOutput` para `true` na sua configuração do Astro, e defina `output` para `hybrid`.
...
```
Então adicione `export const prerender =  false` em qualquer página ou endpoint que você quiser recusar a pré-renderização.
```astro
---
// pages/contato.astro
