...
Desde o Astro 2.6.0, integrações também podem adicionar diretivas `client:*` customizadas para alterar como e quanto os componentes devem ser hidratados.

Visite a página da [API `addClientDirective`](/pt-br/reference/integrations-reference/#opção-addclientdirective) para aprender mais sobre criar uma diretiva customizada do cliente.
