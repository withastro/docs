...
[Docker](https://docker.com) é uma ferramenta para construir, implantar e executar aplicações usando contêineres.

Imagens e contêineres Docker podem ser implantados em diversas plataformas, como AWS, Azure e [Google Cloud](/pt-br/guides/deploy/google-cloud/#cloud-run-ssr-and-static). Esta receita não explica como implantar seu site em uma plataforma específica, mas mostra como configurar o Docker para seu projeto.
...
O seguinte Dockerfile irá construir seu site e servi-lo usando Apache httpd na porta `80` com a configuração padrão.
...
2. Para rodar sua imagem como um contêiner local, use o seguinte comando. 
    ...
    Substitua `<local-port>` por uma porta aberta em sua máquina. Substitua `<container-port>` pela porta exposta pelo seu contêiner Docker (`4321`, `80` ou `8080` nos exemplos acima).
