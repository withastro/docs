...
Se você prefere que o língua padrão não seja visível na URL ao contrário de outras línguas, abaixo tem [instruções para ocultar o língua padrão](/pt-br/recipes/i18n/#hide-default-language-in-the-url).
...
2. Configure `src/pages/index.astro` para redirecionar para o seu língua padrão.
...
1. Crie um diretório em `src/content/` para cada tipo de conteúdo que você quer incluir e adicione subdiretórios para cada língua suportado. Por exemplo, para oferecer suporte a postagens de blog em inglês e francês:
...
3. Use [rotas dinâmicas](/pt-br/core-concepts/routing/#dynamic-routes) para buscar e renderizar conteúdo baseado em parâmetros `lang` e `slug`.
    ...
    Isso garante que a data e a hora sejam formatadas de acordo com o língua do usuário.
    :::

### Traduzir strings da UI 

Criar dicionários de termos para traduzir as labels dos elementos de UI em todo o seu site. Isso permite que seus visitantes experimentem seu site completamente no língua deles.
...
2. Crie duas funções: uma para detectar o língua da página baseado na URL atual, e outra para obter as strings de tradução para diferentes partes da UI em `src/i18n/utils.ts`:
    ...
    :::note[Você percebeu?]
    No passo 1, a string `nav.twitter` não foi traduzida para o francês. Você pode não querer traduzir todos os termos, como nomes próprios ou termos comuns da indústria. A função `useTranslations` irá retornar o valor do língua padrão se uma chave não for traduzida. Neste exemplo, os usuários franceses também verão “Twitter” na barra de navegação.
    :::
    
3. Importe as funções onde necessário e use-as para escolher a string de UI que corresponde ao língua atual. Por exemplo, um componente de navegação pode ser assim: 
...
4. Cada página deve ter um atributo `lang` no elemento `<html>` que corresponda ao língua da página. Neste exemplo, um [layout reutilizável](/pt-br/core-concepts/layouts/) extrai o língua da rota atual:
    ```astro
    ---
...
Criar links para os diferentes línguas que você oferece suporte para que os usuários possam escolher o língua que eles querem ler seu site.
...
### Ocultar o língua padrão na URL

1. Crie um diretório para cada língua, exceto o língua padrão. Por exemplo, armazene suas páginas do língua padrão diretamente em `pages/`, e suas páginas traduzidas em `fr/`:
...
3. Adicione uma função ao arquivo `src/i18n/utils.ts`, para traduzir caminhos baseado no língua atual:
   ```js
    // src/i18n/utils.ts
...
5. A função também pode ser usada para traduzir caminhos para um língua específico. Por exemplo, quando os usuários alternam entre línguas:
    ```astro
    ---
    // src/components/LanguagePicker.astro
    import { languages } from '../i18n/ui';
