...
:::tip
Você também pode [criar um novo projeto Astro a partir de qualquer repositório Astro existente no GitHub](/pt-br/install-and-setup/#usando-um-tema-ou-um-modelo-inicial) usando a marcação `--template`:
```bash
...
Como Bun possui [compatibilidade com a API Node.js](https://bun.sh/docs/runtime/nodejs-apis), você pode usar qualquer adaptador Astro para [renderização no lado do servidor](/pt-br/guides/server-side-rendering/) em seu projeto Astro:
...
Para criar seu primeiro teste Playwright, siga as instruções no [Guia de Testes Astro](/pt-br/guides/testing/#crie-seu-primeiro-teste-playwright).
