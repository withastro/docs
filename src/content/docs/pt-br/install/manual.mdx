...
:::note[Importante]
O Astro precisa ser instalado localmente, nao globalmente. Tenha certeza que você *não* está rodando `npm install -g astro` `pnpm add -g astro` ou `yarn add global astro`.
:::
