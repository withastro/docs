...
- **Descontinuação** de funcionalidades/opções existentes com um aviso de que elas serão removidas em uma próxima versão maior.
...
- As versões ímpares do Node.js podem ser depreciadas e/ou abandonadas quando a próxima versão par do Node.js for publicada. Essa alteração pode ocorrer em uma versão **menor** do Astro, após um período razoável de suporte estendido, conforme decidido pela equipe Core do Astro.
...
- O upgrade de versões menor ou maior do Node.js (**não** Manutenção LTS) ocorre apenas nas versões Maior do Astro.
  - Exceção de segurança: Se uma falha de segurança no Node.js que **afeta o Astro** for divulgada e corrigida, a equipe Core poderá aumentar a versão mínima em uma versão **menor** do Astro.
