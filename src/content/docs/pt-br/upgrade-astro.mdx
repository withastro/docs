...
Você pode encontrar uma lista exaustiva de todas as alterações no [changelog do Astro](https://github.com/withastro/astro/blob/main/packages/astro/CHANGELOG.md) e instruções importantes para atualizar para cada nova [versão major](#major-changes) em nossos [guias de atualização](#upgrade-guides).

## Atualize para a versão mais recente \{#upgrade-to-the-latest-version\}
...
### Atualização manual \{#manual-upgrading\}
...
### Instalar um número de versão específico \{#install-a-specific-version-number\}
...
### Guias de atualização \{#upgrade-guides\}

Após cada nova [versão major](#major-changes), você encontrará um **guia de atualização** com informações sobre alterações importantes e instruções para atualizar o código do seu projeto.
...
### Documentação antiga (sem manutenção) \{#older-docs-unmaintained\}
...
### Mudanças major \{#major-changes\}

As versões major incluirão mudanças significativas em pelo menos alguns códigos existentes. Essas alterações são sempre documentadas em um guia ["Atualize para vX"](#upgrade-guides) no Astro.
...
### Políticas de suporte e atualização do Node.js \{#nodejs-support-and-upgrade-policies\}
...
### Manutenção estendida \{#extended-maintenance\}
