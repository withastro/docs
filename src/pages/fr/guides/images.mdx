...
L'image sera traitée comme une image distante, donc une valeur pour `aspectRatio` est également requise pour s'assurer que la hauteur correcte puisse être calculée au moment du Build.
...
<!-- Image distante (le ration d'aspect est requis) -->
...
L'intégration d'image officielle modifiera les importations d'images pour renvoyer un objet plutôt qu'une chaîne source.
...
Une alternative consiste à ajouter `?url` à vos imports pour leur dire de renvoyer une chaîne source.
...
Pour utiliser des [images externes avec les composants `<Image />`](#remote-images) et [`<Picture />`](#remote-images-1) de l'intégration d'image d'Astro, vous devez spécifier les valeurs de dimension et de format pour travailler avec des images distantes.
...
Si l'image est simplement décorative (i.e. qu'elle ne contribue pas à la compréhension de la page), définissez `alt=""` pour que les lecteurs d'écran sachent ignorer l'image.
