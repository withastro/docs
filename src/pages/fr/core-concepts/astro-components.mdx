...
La syntaxe du composant Astro est un sur-ensemble de HTML. La syntaxe a été [conçue pour être familière à toute personne ayant de l'expérience dans l'écriture de HTML ou JSX](#differences-between-astro-and-jsx) , et ajoute la prise en charge de l'inclusion de composants et d'expressions JavaScript. Vous pouvez repérer un composant Astro par son extension de fichier : `.astro` .
...
// Accédez aux propriétés passées dans le composant, comme `<X title="Hello, World" />`
...
Vous pouvez définir des variables JavaScript locales à l'intérieur des barres de code du script du composant dans un composant Astro. Vous pouvez ensuite injecter ces variables dans le template HTML du composant à en utilisant des expressions de type JSX !

:::note[dynamic vs réactif] En utilisant cette approche, vous pouvez inclure des valeurs **dynamiques** qui sont calculées dans le frontmatter. Mais une fois incluses, ces valeurs ne sont pas **réactives** et ne changeront jamais. Les composants Astro sont des templates qui ne s'exécutent qu'une seule fois, au moment de la compilation.

Voir ci-dessous pour plus d'exemples de [différences entre Astro et JSX](#differences-between-astro-and-jsx) . :::
...
Les variables locales peuvent être utilisées entre accolades pour transmettre des valeurs d'attribut aux éléments HTML et <br> aux composants :
...
:::caution Les attributs HTML seront convertis en chaînes, il n'est donc pas possible de passer des fonctions et des objets aux éléments HTML. Par exemple, vous ne pouvez pas affecter un gestionnaire d'événements à un élément HTML dans un composant Astro :
...
- **Les directives d'hydratation ne sont pas prises en charge.** Lorsque vous utilisez [ les directives d'hydratation `client:*` ](/en/core-concepts/framework-components/#hydrating-interactive-components), Astro doit savoir quels composants regrouper pour la production, et le modèle de balise dynamique empêche cela de fonctionner.
...
Les fragments peuvent également être utiles pour éviter d'utiliser des éléments englobants lors de l'ajout [de directives `set:*`](/en/reference/directives-reference/#sethtml) , comme dans l'exemple suivant :
...
:::caution Les commentaires de style HTML seront inclus dans le DOM du navigateur, tandis que ceux en JS seront ignorés. Pour laisser des messages TODO ou d'autres explications réservées au développement, vous pouvez utiliser des commentaires de style JavaScript à la place. :::
...
Ce composant, lorsqu'il est importé et rendu dans d'autres composants standards,  Composants Layout ou pages Astro, peut recevoir ces props en tant qu'attributs :
...
:::note Contrairement aux *props* , qui sont des attributs passés à un composant Astro disponibles pour une utilisation dans votre composant avec `Astro.props` , *les emplacements* restituent les éléments HTML enfants là où ils sont placés. :::
...
Ce modèle est à la base d'un Composant Layout dans Astro : une page entière de contenu HTML peut être "enveloppée" avec des balises `<Layout></Layout>` et envoyée à au Composant Layout pour être rendu à l'intérieur des éléments de page communs.

### Les emplacements nommés

Un composant Astro peut également avoir des emplacements nommés. Cela vous permet de ne faire passer que des éléments HTML avec le nom d'emplacement (slot) correspondant, au niveau de l'emplacement.
...
Utilisez un attribut `slot="my-slot"` sur l'élément enfant que vous souhaitez transmettre à un emplacement `<slot name="my-slot" />` correspondant dans votre composant.

:::tip Les emplacements nommés peuvent aussi être passés aux [composants de framework](/en/core-concepts/framework-components/) ! :::

### Contenu de repli pour les emplacement

Les emplacements peuvent également restituer **du contenu de repli** . Lorsqu'il n'y a pas d'enfants correspondants passés à un emplacement, un élément `<slot />` fera le rendu de ses propres enfants.
...
<div id="content-wrapper">
  ...
  <h1>{title}</h1>
  <slot>
    <p>Ceci est mon contenu de repli, s'il n'y a pas d'enfants passés à l'emplacement</p>
  </slot>
  <Footer />
</div>
```
...
Les balises CSS `<style>` sont également prises en charge à l'intérieur du template du composant.

Ils peuvent être utilisés pour styliser vos composants, et toutes les règles de style sont automatiquement limitées à la portée du composant lui-même pour éviter les conflits CSS dans les applications volumineuses.
...
---
<style>
  /* limité à la portée du composant, les autres H1 de la page seront inchangés */
  h1 { color: red }
</style>
...
:::caution Les styles définis ici s'appliquent uniquement au contenu écrit directement dans le template du composant lui-même. Les enfants et tous les composants importés ne seront **pas** stylisés par défaut. :::
...
Par défaut, Astro traite et regroupe les balises `<script>` , ajoutant la prise en charge de l'importation de modules npm, de l'écriture de TypeScript, etc.

📚 Consultez notre [guide sur les scripts](/en/guides/client-side-scripts/) pour plus de détails.
...
- Ils ne peuvent [référencer que les assets qui se trouvent dans le dossier `public/`](/en/guides/images/#public) .

:::note Un [élément `<slot />`](/en/core-concepts/astro-components/#slots) à l'intérieur d'un composant HTML fonctionnera comme il le ferait dans un composant Astro. Pour utiliser l'élément [Slot des Web Components (composants web)](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot)  à la place, ajoutez `is:inline` à votre élément `<slot>` . :::

## Étapes suivantes

📚 En savoir plus sur [les composants intégrés d'Astro](/en/reference/api-reference/#built-in-components) .

📚 Apprendre comment utiliser [les composants de framework JavaScript](/en/core-concepts/framework-components/) dans votre projet Astro.
