...
La syntaxe du composant Astro est un sur-ensemble de HTML. La syntaxe a été [conçue pour être familière à toute personne ayant de l'expérience dans l'écriture de HTML ou JSX](#differences-entre-astro-et-jsx) , et ajoute la prise en charge de l'inclusion de composants et d'expressions JavaScript. Vous pouvez repérer un composant Astro par son extension de fichier : `.astro` .
...
Voir ci-dessous pour plus d'exemples de [différences entre Astro et JSX](#differences-entre-astro-et-jsx) . :::
...
:::tip Les emplacements nommés peuvent aussi être passés aux [composants de framework](/fr/core-concepts/framework-components/) ! :::
...
:::note Un [élément `<slot />`](#les-emplacements) à l'intérieur d'un composant HTML fonctionnera comme il le ferait dans un composant Astro. Pour utiliser l'élément [Slot des Web Components (composants web)](https://developer.mozilla.org/fr/docs/Web/HTML/Element/slot)  à la place, ajoutez `is:inline` à votre élément `<slot>` . :::
