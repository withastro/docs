...
- `remarkPluginFrontmatter` - El objeto frontmatter moficado después de cualquier [plugin remark o rehype ha sido aplicado](/es/guides/markdown-content/#modifying-frontmatter-programmatically). Configurado para el tipo `any`.
...
<PackageManagerTabs>
	<Fragment slot="npm">```shell npm install @astrojs/prism ```</Fragment>
	<Fragment slot="pnpm">```shell pnpm install @astrojs/prism ```</Fragment>
	<Fragment slot="yarn">```shell yarn add @astrojs/prism ```</Fragment>
</PackageManagerTabs>
