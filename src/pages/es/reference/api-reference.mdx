...
`schema` es un objeto Zod opcional para configurar el tipo y la forma del frontmatter del documento para una colección. Cada valor de objeto debe usar [un validador Zod](https://github.com/colinhacks/zod).
...
- `remarkPluginFrontmatter` - El objeto frontmatter moficado después de cualquier [plugin remark o rehype ha sido aplicado](/es/guides/markdown-content/#ejemplo-inyectando-frontmatter). Configurado para el tipo `any`.
