...
#### Modificación programática de frontmatter
...
1. Agrega una `customProperty` a la propiedad `data.astro.frontmatter` del argumento `file` de tu plugin:
    ...
    `data.astro.frontmatter` contiene todas las propiedades de un documento Markdown o MDX dado. Esto te permite modificar propiedades existentes del frontmatter, o calcular nuevas propiedades a partir de este frontmatter existente.
    :::
...
### Extendiendo la config de Markdown desde MDX
