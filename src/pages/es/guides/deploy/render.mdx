...
4. Asigna un nombre a tu sitio web, selecciona la rama y especifica el comando de compilación y el directorio de publicación
    ...
    - **Environment variables (advanced)**: Por defecto, Render usa Node.js 14.17.0, pero Astro [requiere una versión superior](/es/install/auto/#prerequisites). Añade una variable de entorno con una **Variable key** de `NODE_VERSION` y un **Value** de `16.12.0` o superior para indicarle a Render que use una versión compatible de Node.js. Alternativamente, añade un archivo [`.node-version`](https://render.com/docs/node-version) o [`.nvmrc`](https://render.com/docs/node-version) a tu proyecto para especificar una versión de Node.js.
