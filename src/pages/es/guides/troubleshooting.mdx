...
**Solución**: Intenta buscar errores en los componentes que estás intentando importar y renderizar, y asegúrate que estan funcionando correctamente. Considera abrir una plantilla de inicio de Astro desde [astro.new](https://astro.new) para intentar solucionar el problema de tu componente en un entorno mas sencillo
...
- Tu importación posee el mismo nombre que el del componente? (Chequea el nombre de tu componente y que [cumpla con la sintaxis de componentes `.astro`](/es/core-concepts/astro-components/#diferencias-entre-astro-y-jsx).)
...
Quizás observes que se importan las etiquetas `<script>` o `<style>` de un componente en tu código HTML incluso si dicho componente no aparece en el resultado final. Esto puede suceder, por ejemplo, cuando [componentes renderizados condicionalmente](/es/core-concepts/astro-components.mdx#html-din%C3%A1mico) no son mostrados. 
