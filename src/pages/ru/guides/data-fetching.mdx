...
Ваш задеплоенный Astro сайт выполнит запрос на получение данных **один раз, во время сборки**. Во время разработки, вы увидите запросы данных когда компонент обновляется. Если вам нужно обновлять данные несколько раз на стороне клиента, используйте [компоненты фреймворка](/ru/core-concepts/framework-components/) или [клиентские скрипты](/ru/core-concepts/astro-components/#client-side-scripts) в Astro компоненте.
:::
...
Astro компоненты могут получать данные с вашей CMS и затем рендерить контент на странице. Используя [динамические роуты](/ru/core-concepts/routing/#dynamic-routes), компоненты даже могут генерировать страницы основываясь на контенте с вашей CMS.
