...
[UIフレームコンポーネント](/ja/core-concepts/framework-components/)(ReactやSvelteなど)に画像を追加する場合、そのコンポーネントのフレームワークに沿った画像の文法を利用します。
...
Markdownファイルから利用する場合、[`public/`へ配置する](#public)か[CMSやCDN上の画像を利用](#cmsやcdn上の画像利用)することをお勧めします。
...
しかし、`/public`ディレクトリにあるファイルは常に処理されずにそのままで提供、コピーされます。Markdownファイル以外で画像を利用する場合はAstroが画像を変換・最適化・バンドルできるように、可能な限りローカル画像は`src`への保存をお勧めします。
...
Astroの公式な画像のインテグレーションでは、最適化された画像をレンダリングするための2つの`<Image />`と`<Picture />`コンポーネントを提供しています。これらのコンポーネントは全ての静的サイトと、[サーバーサイドレンダリングするデプロイホスト](/ja/guides/integrations-guide/image/#installation)をサポートしています。

[`@astrojs/image`のインストール](/ja/guides/integrations-guide/image/#installation)することで、`.astro`と`.mdx`などのAstroコンポーネントを利用できるファイル内でこの2つのコンポーネントを利用できます。

### `<Image />`

Astroの[`<Image />`コンポーネント](/ja/guides/integrations-guide/image/#image-)は1つの画像を最適化し、幅・高さ・アスペクト比を指定できます。また特定の出力フォーマットに画像を変換できます。

このコンポーネントはディスプレイ間で一貫したサイズを維持したい画像や、品質を厳密にコントロールしたい画像(ロゴなど)で有効です。
...
`alt`は必須ですが[その他の属性](/ja/guides/integrations-guide/image/#image-)はオプションで、指定が無ければ画像ファイルのプロパティが初期値として設定されます。
...
利用ユーザーのブラウザに最適な画像サイズ、解像度、そして画面サイズや帯域幅に基づいたファイルタイプを選択できます。また、メディアクエリに基づいてブラウザに従わせるルールを指定できます。
...
`<Picture />`コンポーネントの[`formats`](/ja/guides/integrations-guide/image/#formats)には指定がなければ、デフォルト値は元の画像のフォーマットに加えて`avif`と`webp`が含まれます。
...
CDNネットワーク上の画像をとAstroで利用するには、画像の完全なURLを`<img>`タグやMarkdownの`src`属性として利用します。
...
公式の[`@astrojs/image`](/ja/guides/integrations-guide/image/)インテグレーションに加え、Astroプロジェクトで画像の最適化や処理を行うためのサードパーティー製の[コミュニティー画像インテグレーション](https://astro.build/integrations/css+ui/?q=image)がいくつかあります。
