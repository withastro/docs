...
ただし、Astroのコンポーネントテンプレート構文は、**JavaScript式**、**インポートしたコンポーネント**、[**特別なAstroディレクティブ**](/ja/reference/directives-reference/)もサポートしています。コンポーネントスクリプトで（ページ構築時に）定義されたデータと値をコンポーネントテンプレートで使用し、動的に作成されたHTMLを生成できます。
...
<!-- `client:`ディレクティブを付けてコンポーネントをハイドレートします -->
...
Astroコンポーネントのfront-matterのコンポーネントスクリプト内で、ローカルJavaScript変数を定義できます。また、JSXに似た式を使用して、これらの変数をコンポーネントのHTMLテンプレートに挿入できます。
...
TypeScriptの`Props`型のインターフェイスでpropsを定義できます。Astroはfront-matter内の`Props`インターフェイスを自動的に検出し、型の警告やエラーを出します。propsは、`Astro.props`から分割代入する際に、デフォルト値を与えることもできます。
...
このパターンはAstroレイアウトコンポーネントの基本です。HTMLコンテンツのページ全体を`<Layout></Layout>`タグで「囲んで」レイアウトコンポーネントに送り、共通のページ要素の中にレンダリングさせられます。
...
スクリプトは、イベントリスナーの追加、解析したデータの送信、アニメーションの再生など、JavaScriptがWeb上で実行できるあらゆることに使用できます。
...
Astroはデフォルトで`<script>`タグを処理・バンドルし、npmモジュールのインポートやTypeScriptの記述などをサポートします。
...
- front-matterやサーバーサイドのインポート、動的な記法をサポートしません
