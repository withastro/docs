...
> 💡 _props_（Astroコンポーネントに渡される属性で、`Astro.props()`でコンポーネント全体で使用することができます）とは異なり、_slot_は子要素が書かれた場所にレンダリングされます。
...
> ⚠️ これは、他のAstroコンポーネントにスロットを渡す場合のみ機能します。他の[UI framework components](/ja/core-concepts/framework-components/) をAstroファイルに含めることについてはこちらをご覧ください。
...
> ⚠️ ここで定義されたスタイルは、そのコンポーネント自身のコンポーネントテンプレートに直接書かれたコンテンツにのみ適用されます。子コンポーネントやインポートされたコンポーネントは、デフォルトではスタイルが**適用されません**。
...
> **注意:** `<script>` タグに `type="module"` やその他の属性を追加すると、Astro のデフォルトのバンドル動作が無効になり、 `is:inline` ディレクティブがあるかのようにタグが処理されます。
