...
---
Since your site will be viewed on different devices, let's group your page navigation items into a new header component that can respond to multiple screen sizes!
...
1. Create a new Header component that is a `<header>` containing your existing `Navigation.astro` component inside a `<nav>` element.
...
2. Copy the following code into your component. This will represent your 3-line "hamburger" menu to open and close your navigation links on mobile. (You will add the new CSS styles to `global.css` later.)
    ...
    3. Import and add this new `<Hamburger />` component to `Header.astro`, just before your `<Navigation />` component inside the `<nav>`. 

    <details>
    <summary>Show me the code!</summary>

    ```astro title="src/components/Navigation.astro" ins={2,7}
    ---
...
Update `Navigation.astro` with a CSS class to create a responsive, mobile header.
...
2. Check your browser preview and verify that your header is displayed on every page.
...
Instead of writing your JavaScript directly in your component, you can instead move the contents of your `<script>` tag into its own `.js` file in your project.
...
:::note
You have already used some JavaScript to build parts of your site:

- mapping through a list of skills on the About page
- fetching information from your Markdown files
...
The JavaScript in a `<script>` tag is sent to the browser, and is available to run, and re-run, based on user interactions like refreshing a page or toggling an input.
:::
...
In Astro, JavaScript that is only meant to build your site is written between the || **code fences** ||, is run on a server (e.g. Netlify), and then discarded once your site is published. JavaScript that is meant to control interactive elements on your site is || **written or imported** || between opening and closing `<script>` tags.
