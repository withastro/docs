...
<Goals>
  - used `Astro.glob()` to access data from all your Markdown (`.md`) files at once
  - used the array returned by `Astro.glob()` to display a sorted list of posts on your Blog page
...
3. Add a new blog post by creating a new `post-4.md` file in `src/pages/posts/` and adding some Markdown content. Include frontmatter properties for `layout`, `title` and `date`, at a minimum. (You may choose to include more properties, or to copy and paste the entire file content of one of your previous posts.)
...
3. Returning a `<LastUpdated />` component for each item in `myPosts`, passing the blog post's `date` as props.

    || `myPosts.map((post) => <LastUpdated date={post.frontmatter.date} />)` ||
