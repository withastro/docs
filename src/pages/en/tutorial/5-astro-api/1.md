...
<Goals>
  - accessed data from all your Markdown (`.md`) files at once using `Astro.glob()`
  - display a sorted list of posts on your Blog page
  - created a `<PostItem />` component for each list item
</Goals>
...
3. Add a new blog post by creating a new `post-4.md` file in `src/pages/posts/` and adding some Markdown content. Be sure to include at least the following frontmatter properties.
