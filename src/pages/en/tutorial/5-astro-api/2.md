...
:::note[Takeaway]
To **give** information to your function to construct a page route, write it **inside** `getStaticPaths()` so that the function has access to it to create the routes.

To **use** information in the HTML template of a page route, write it **outside** `getStaticPaths()` to define the `props` and `params` for your template.
:::
