...
<Goals>
  - created a page to generate multiple pages using `[bracket]` notation
  - specified which page routes to build, and passed each page its own props using `getStaticPaths()`
</Goals>

## Dynamic Page Routing

You can create entire sets of pages dynamically using `.astro` files that export a `getStaticPaths()` function. This function returns an array of page routes, and all of the pages at those routes will use the same template defined in the file.
...
3. Values of `props` and `params` to be used by each individual page in the HTML template.
...
You have some tags that are used multiple times, but you will only want to create one page for each unique tag. The following line of JavaScript will create an array by looking at the contents of every blog posts' `tags` array, but only include each value once.
