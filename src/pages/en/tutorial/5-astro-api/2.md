...
- The `Astro.glob()` call to receive information about all your `.md` files to pass to each page route. _(inside)_

- The list of routes generated (returned) by `getStaticPaths()` _(inside)_

- Definitions of values to be used by each individual page in the component template. _(outside)_
...
1. parameter  -  a value that defines the name of a page route generated dynamically

2. psuedocode  -  an informal way of writing code, not meant to be executed by machines

3. dynamic routing - in Astro, the process of creating multiple page routes from one function

4. `getStaticPaths ()` - a function you can write inside an Astro component script that returns an array of page routes that can receive props.

5. `Set` - a JavaScript object that is a collection of unique, non-repeating items
