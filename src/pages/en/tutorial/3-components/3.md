...
<Goals>
  ...
  - used an Astro `<slot />` element to place page contents within a layout
...
2. To display a page title on the page, you can:
    ...
    <MultipleChoice>
      ...
      <Option>
        use a standard HTML element on the page referring to a variable defined in your component's frontmatter script (e.g. `<h1>{title}</h1>`)
      </Option>
      <Option>
