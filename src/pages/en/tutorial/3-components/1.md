...
PReviously, we added new `.astro` files to the `src/pages/` folder. This is because Astro automatically turns each file in that `/pages/` folder into a page on your website. To hold `.astro` files that will generate HTML but that will not become new pages on your website, you will need a new folder in your project's `src` outside of `/pages/`. 
...
2. You have already written some links to navigate between your pages (shown below). Copy these links from the top of any page and paste it into your new file, `Navigation.astro`:
