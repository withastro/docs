...
Notice that **all** of those HTML elements are things we want to appear on every page, so we can remove most of the content from `index.astro` and still show the same page to the reader via only the `<Layout />` component. 
...
|| This is useful for a layout component in particular because we often want a page layout to define most elements on our page _except_ for some specific page content, that is different on every page. A `<slot />` allows us to create one single page layout, and have different inner content on each page.  ||
