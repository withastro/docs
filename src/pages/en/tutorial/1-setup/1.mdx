...
For Astro to run on your system, you also need to have [**Node.js**](https://nodejs.org/en/) of version `v16.12.0` or later.
First, check whether you already have Node.js installed and check the installed version. To do this, run the following command in your terminal:
...
* If the command returns a version number higher than `v16.12.0`, you're good to go! 🎉 If not, you need to install the compatible Node.js version.
* If the command returns an error message like `Command 'node' not found`, Node.js is not installed in your system.

To install Node.js (or the compatible version), follow [this guide](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm).
