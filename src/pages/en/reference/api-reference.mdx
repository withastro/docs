...
**Type:** `() => Promise<RenderResult | RenderResultWithHtml>`
...
- **(Markdown only)** `html` - The rendered HTML string, [mirroring Astro's `compiledContent()` utility](/en/guides/markdown-content/#exported-properties).
...
const { Content, headings, remarkPluginFrontmatter, html } = await entry.render();
---
```
