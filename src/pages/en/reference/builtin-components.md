...
> `@astrojs/prism` is built-in as part of the `astro` package. No need to install as a separate dependency just yet! Still, note that we do plan to extract `@astrojs/prism` to a separate package in the future.
