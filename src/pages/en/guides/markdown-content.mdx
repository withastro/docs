...
For example, to disable GitHub-Flavored Markdown and apply a different set of remark plugins for MDX files only:
...
You may also prefer to disable Markdown config extension. For this, you can set [the `extendMarkdownConfig` option](/en/guides/integrations-guide/mdx/#extendmarkdownconfig) (enabled by default) to `false`:
