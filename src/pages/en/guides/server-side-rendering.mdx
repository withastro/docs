...
To enable **hybrid rendering**, [pages](/en/core-concepts/astro-pages/) and [server endpoints](/en/core-concepts/endpoints/#server-endpoints-api-routes) can opt-in to prerendering. These files will be statically rendered at build-time, similar to the default static [`output`](/en/reference/configuration-reference/#output) mode.
