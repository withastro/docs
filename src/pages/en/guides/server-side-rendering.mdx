...
On the `Astro` global, this interface allows to manipulate cookies. It allows to directly get them from requests and set them in responses. Whereas `Astro.request.headers.get('cookie')` gets all cookies and still need to be parsed, `Astro.cookies.get` gets a single cookie in an object.

More details about the API in the reference [Astro.cookies](/en/reference/api-reference/#astrocookies)

The example below shows how to update the value of a counter cookie.

```astro title="src/pages/index.astro" {4,10}
---
let counter = 0
...
if(cookie.value != undefined){
	counter = parseInt(cookie.value) + 1
}
