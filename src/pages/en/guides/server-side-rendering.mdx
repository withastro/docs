...
To enable **hybrid rendering**, any [`.astro` page](/en/core-concepts/astro-pages/) or [server endpoint](/en/core-concepts/endpoints/#server-endpoints-api-routes) can opt-in to prerendering. These files will be statically rendered at build-time, similar to the default static [`output`](/en/reference/configuration-reference/#output) mode.
...
[MDX pages](/en/guides/markdown-content/#using-exported-variables-in-mdx) can also be prerendered:

```astro title="src/pages/markdown.mdx" {2}
---
...
Plain `.md` Markdown pages do not support exporting variables, and so must be converted to `.mdx` before they can be prerendered.
