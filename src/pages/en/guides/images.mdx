...
For responsive images, or art direction, [use the `<Picture />` component](#picture--) instead.

#### Local Images in `src/`

Required attributes:
- [`src`](/en/guides/integrations-guide/image/#src): Import your image in frontmatter and pass it directly to the attribute
- [`alt`](/en/guides/integrations-guide/image/#alt)

[All other properties](/en/guides/integrations-guide/image/#image-) are optional and will default to the image file's original properties if not provided.

#### Remote Images

Required attributes:
- [`src`](/en/guides/integrations-guide/image/#src): A full image URL
- [`format`](/en/guides/integrations-guide/image/#format): An output format is required to transform your remote image as the `<Image />` component cannot determine the original file format of a remote image.
- Dimensions: Because the `<Image />` component does not know the dimensions of a remote image, one of the following must be provided to avoid content layout shifts:
  - [`width`](/en/guides/integrations-guide/image/#width) and [`height`](/en/guides/integrations-guide/image/#height)
  - One of the dimensions plus [`aspectRatio`](/en/guides/integrations-guide/image/#aspectratio)
- [`alt`](/en/guides/integrations-guide/image/#alt)

#### Local Images in `public/`

Required attributes:
- [`src`](/en/guides/integrations-guide/image/#src): A path relative to the public folder
- [`format`](/en/guides/integrations-guide/image/#format) : Required to transform your image
- Dimensions: Because the image will be treated as a remote image, one of the following is required:
  - [`width`](/en/guides/integrations-guide/image/#width) and [`height`](/en/guides/integrations-guide/image/#height)
  - One of the dimensions plus [`aspectRatio`](/en/guides/integrations-guide/image/#aspectratio)
- [`alt`](/en/guides/integrations-guide/image/#alt)
...
Required attributes:
- [`src`](/en/guides/integrations-guide/image/#src-1): Import your image in frontmatter and pass it directly to the attribute
- [`widths`](/en/guides/integrations-guide/image/#widths), [`sizes`](/en/guides/integrations-guide/image/#sizes), [`alt`](/en/guides/integrations-guide/image/#alt-1)

You must provide the component with guidance for image widths and screen sizes, but [all other properties](/en/guides/integrations-guide/image/#picture-) are optional. By default, the `<Picture />` component's [`formats`](/en/guides/integrations-guide/image/#formats) will include `avif` and `webp` in addition to the image's original format if not specified.


#### Remote Images 

Required attributes:
- [`src`](/en/guides/integrations-guide/image/#src-1): A full image URL
- [`aspectRatio`](/en/guides/integrations-guide/image/#aspectratio-1): Required to ensure the correct height can be calculated at build time for remote images
- [`widths`](/en/guides/integrations-guide/image/#widths), [`sizes`](/en/guides/integrations-guide/image/#sizes), [`alt`](/en/guides/integrations-guide/image/#alt-1)

You must provide the component with guidance for image widths and screen sizes, but [all other properties](/en/guides/integrations-guide/image/#picture-) are optional. By default, only `webp` and `avif` will be included in [`formats`](/en/guides/integrations-guide/image/#formats). The original image format of images in the `public/` folder is unknown and will not be included.

#### Local Images in `public/`

Required attributes:
- [`src`](/en/guides/integrations-guide/image/#src-1): A path relative to the public folder
- [`aspectRatio`](/en/guides/integrations-guide/image/#aspectratio-1): Required to ensure the correct height can be calculated at build time as the image will be treated as a remote image
- [`widths`](/en/guides/integrations-guide/image/#widths), [`sizes`](/en/guides/integrations-guide/image/#sizes), [`alt`](/en/guides/integrations-guide/image/#alt-1)
