...
Source for the original image file. The value depends on where the image is located in:

- Project's `src`: Use the file path relative to the `src` directory. (e.g. `src="../assets/source-pic.png"`)
- `public` directory: Use the URL path relative to the `public` directory. (e.g. `src="/images/public-image.jpg"`)
- Remote: Provide the full URL. (e.g. `src="https://astro.build/assets/blog/astro-1-release-update.avif"`)
