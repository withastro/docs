...
`getEntryBySlug()` is function that returns a specific entry in a collection by its slug (URL-ready). Both of these are required parameters.
...
`getCollection()` and `getEntryBySlug()` will return entries that include:
...
Querying your content files with `getCollection()`or `getEntryBySlug()` allows you to use frontmatter properties from an entry's `data` object in [JSX-like expressions](/en/core-concepts/astro-components/#jsx-like-expressions) or pass props to other components, such as a layout. You can optionally add type safety with a built-in utility.
...
If a page or component uses content from a `getCollection()` or `getEntryBySlug()` query, you can use the `CollectionEntry` utility to type its props:
...
import { getEntryBySlug } from 'astro:content';
const announcementPost = await getEntryBySlug('announcements', 'welcome');
...
<details>
<summary>**🙋 Why don't `getCollection()` and `getEntryBySlug()` contain these values?**</summary>
