...
These commands will also add a [reference path](https://www.typescriptlang.org/docs/handbook/triple-slash-directives.html#-reference-path-) to your [`src/env.d.ts` file](/en/guides/typescript/#setup) if one exists. You can add this entry manually like so:

```ts title="src/env.d.ts" ins={2}
// use a relative path `../` to the `.astro` directory
