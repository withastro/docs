...
:::note
If your Content Collections configuration file is a JavaScript file (uses the `.mjs` or `.js` file extension), you won't get intellisense in that config file by default, unless your project is using a `jsconfig.json` instead of a `tsconfig.json`. To get type checking and intellisense when using a `tsconfig.json`, you will need to update it to enable the `allowJs` compiler option.
