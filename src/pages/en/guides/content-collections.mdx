...
Every content entry generates a URL-friendly `slug` property from its file `id`. The file `id` is simply the filename including its extension (`.md` or `.mdx`), but the extension is stripped for the slug. The slug is used to query the entry directly from your collection. It is also useful when creating new pages and URLs from your content.
