...
5. Since we wrapped the content in a layout in the dynamic route, remove the `layout` definition in each post's frontmatter.
...
6. To fetch your content, use [`getCollection`](/en/reference/api-reference/#getcollection) or [`getEntryBySlug`](/en/reference/api-reference/#getentrybyslug) instead of `Astro.glob` or `import` statements. For example, our unique tags logic becomes:
