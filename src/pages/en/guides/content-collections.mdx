...
:::note
If your Content Collections configuration file is a JavaScript file (uses the `.mjs` or `.js` file extension), you won't get intellisense by default, unless you're using a `jsconfig.json` file instead of a `tsconfig.json`. To get type checking and intellisense when using a `tsconfig.json` file, you will need to set the `allowJs` compiler option to `true`.
