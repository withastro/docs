...
<ol start="7"><li>
Replace `Astro.glob()` with [`getCollection()`](/en/reference/api-reference/#getcollection) to fetch content and metadata from your Markdown files. You will also need to update references to the returned post object, since you will now find your frontmatter values on the `data` property.

The blog index page in the tutorial lists a card for each post:
...
The same logic appears in the tag index page:
