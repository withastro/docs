...
4. To [generate routes](/en/guides/content-collections/#generating-routes-from-content), create a `.astro` page with a [dynamic route](/en/core-concepts/routing/#dynamic-routes), then query and render your content. In our tutorial blog, which uses the static output mode, we'll use `getStaticPaths` to create a route for each page:
...
:::note
In this example, we provided the layout for our posts in the dynamic route, but you can also provide it directly in the Markdown frontmatter:
```astro title="src/content/post-1.md"
---
...
5. To fetch your content, use [`getCollection`](/en/reference/api-reference/#getcollection) or [`getEntryBySlug`](/en/reference/api-reference/#getentrybyslug) instead of `Astro.glob`. For example, our unique tags logic becomes:
