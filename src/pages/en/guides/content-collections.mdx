...
:::note
Inside a collection, Markdown and MDX files no longer automatically create pages using Astro's [file-based routing inside `src/pages/`](/en/guides/markdown-content/#file-based-routing). You will need to [generate routes from your collections](/en/guides/content-collections/#generating-routes-from-content) yourself.
:::

5. Create a `src/pages/posts/[...slug].astro`. This page will use [dynamic routing](/en/core-concepts/routing/#dynamic-routes) to generate a page for each collection entry. 
...
Render your post `<Content />` within the layout for your Markdown or MDX pages, since they can no longer rely on the `layout` frontmatter property to specify their own layout.
...
For example, the tutorial blog project dynamically generates a page for each tag. This page now becomes:
...
The tutorial blog project also includes creating an RSS feed. This file must also use `getCollection` and use the `data` object:
