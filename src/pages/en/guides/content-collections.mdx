...
:::note
When using JavaScript files in an Astro project using a `tsconfig.json`, you won't get intellisense or type-checking, unless your project is using a `jsconfig.json`. As such, when using a JavaScript file as your Content Collections configuration file, you won't get those benifits.
To get type checking and intellisense for JavaScript files in that case, you will need to update your `tsconfig.json` to enable the `allowJs` compiler option.
