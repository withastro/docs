...
Astro 2.0 now includes the Collections API for organizing your Markdown and MDX files into [content collections](/en/guides/content-collections/). This API reserves `src/content/` as a special folder.
...
In 1.x, Astro ensured the URL you set as `site` in `astro.config.mjs` always had a trailing slash when accessed using `Astro.site`.

Astro 2.0 no longer modifies the value of `site`. `Astro.site` will use the exact value defined, and a trailing slash must be specified if desired.
...
In 1.x, assets were built to various locations, including `assets/`, `chunks/` and the root of the build output.
...
In 1.x, Astro used `markdown.extendDefaultPlugins` to re-enable Astro's default plugins when adding your own Markdown plugins.

Astro 2.0 removes this configuration option entirely because its behavior is now the default in Astro.
...
GitHub-Flavored Markdown and Smartypants are now applied whether or not custom `remarkPlugins` or `rehypePlugins` are configured.


##### What should I do?

Remove `extendDefaultPlugins` in your configuration. This is now Astro's default behavior in 2.0, and you can delete this line without any replacement.
...
In 1.x, you could choose to disable both of Astro's default Markdown plugins (GitHub-Flavored Markdown and SmartyPants) by setting `markdown.extendDefaultPlugins: false`.

Astro 2.0 replaces `markdown.extendDefaultPlugins: false` with separate Boolean options to control each of Astro's built-in default Markdown plugins individually. These are enabled by default, and can be set to `false` independently.
...
In 1.x, the MDX integration’s `extendPlugins` option managed how your MDX files should inherit your Markdown configuration.

Astro 2.0 replaces the behavior controlled by `extendPlugins` with three new, independently-configurable options:

- **[`extendMarkdownConfig`](/en/guides/integrations-guide/mdx/#extendmarkdownconfig)** (`true` by default) to inherit all of your Markdown configuration or none of it.
- **`gfm`** (`true` by default) to enable or disable GitHub-Flavored Markdown in MDX.
- **`smartypants`** (`true` by default) to enable or disable SmartyPants in MDX.
...
Astro 2.0 allows you to now individually set [every available Markdown configuration option](/en/reference/configuration-reference/#markdown-options) (except `drafts`) separately in your MDX integration configuration.
...
In 1.x, remark and rehype plugins did not have access to user frontmatter. Astro merged plugin frontmatter with your file's frontmatter, without passing the file frontmatter to your plugins.
...
In 1.x, Astro's RSS package allowed you to use `items: import.meta.glob(...)` to generate a list of RSS feed items. This usage is now deprecated and will eventually be removed.

Astro 2.0 introduces a `pagesGlobToRssItems()` wrapper to the `items` property.
...
If you were using this feature in 1.x, we recommend [using the MDX integration](/en/guides/integrations-guide/mdx/) which allows you to combine components and JSX expressions with Markdown syntax.
