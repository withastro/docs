...
You may also wish to reuse code from Gatsby's `src/components/seo.js` to include additional site meta data. Notice that Astro uses neither `<Helmet>` nor `<Header>` but instead accesses `<head>` directly. You may create and import components, even within `<head>`, to separate and organize your page content.
...
```astro title="src/components/MyComponent" del={1-7} ins={9}
