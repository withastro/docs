...
- Astro has built-in support for Markdown and an integration for using MDX files. Also, you can configure and continue to use many of your existing [Markdown plugins](/en/guides/markdown-content/#configuring-markdown-and-mdx).
...
- Astro has support for installing NPM packages, including React libraries. You may be able to keep some or all of your existing React components and dependencies.
...
You may also wish to reuse code from Gatsby's `src/components/seo.js` to include additional site meta data. Notice that Astro uses neither `<Helmet>` nor `<Header>` but instead creates `<head>` directly. You may import and use components, even within `<head>`, to separate and organize your page content.
