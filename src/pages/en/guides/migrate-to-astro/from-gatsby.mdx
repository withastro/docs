...
Your existing Markdown and MDX files can be reused, but may require some adjustments to their frontmatter, such as adding [Astro's special `layout` frontmatter property](/en/core-concepts/layouts/#markdownmdx-layouts). They can also be placed within `src/pages/` to take advantage of automatic file-based routing.
...
```astro title="src/components/MyComponent" del={1-7} ins={9-11}
...
Also, there is no standard configuration and attributes must be added to each component individually. For greater flexibility you can [build your own image component](/en/guides/images/#setting-default-values) to configure reusable image components with default attributes and styling.

To continue using local images in Markdown using standard Markdown syntax (`![]()`), move your images into your `public/` folder. You may need to update the link to the relative URL. You can also use the standard `<img>` tag in these files.  Note that these [images in `public/` will not be optimized by Astro](/en/guides/images/#public).
...
Remove all references to GraphQL queries, and instead use [`Astro.glob()`](/en/guides/imports/#astroglob) to access data from your local files. Or, to use content collections for your content, query your files using [`getCollection`](/en/guides/content-collections/#getcollection) 
