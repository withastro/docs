...
- Option 2: **Update your existing Next project dependencies**
...
2. Change any [Next or JSX syntax to Astro](#eference-convert-nextjs-syntax-to-astro) (e.g. `<Link>`, `<Script>`, `{children}`, `className`, inline style objects) or to HTML web standards.
...
Your existing Next JSX (`.js`) pages will need to be [converted from JSX files to `.astro` pages](#convert-jsx-files-to-astro-files).
