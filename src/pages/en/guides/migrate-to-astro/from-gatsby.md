...
This is not a full, step-by-step walkthrough, but it will guide you through some changes you will have to make. 
...
- Astro is component-based, and you will continue to build your site by writing and importing UI components, which can include React components. 

- Astro has built-in support for Markdown, and [official integrations](#install-any-desired-integrations) for using MDX files and React components. Note that in Astro, React (JSX) files **must** have a `.jsx` or `.tsx` extention.
...
3. Identify and add needed JavaScript
  ...
  To conditionally render a header based on the page route and title in Astro, we need to provide these as props via `Astro.props`. (Remember: Your Astro templating accesses props from its frontmatter, not passed into a function.) 
  
  Use a ternary operator to show one heading if this is the home page, and a different heading otherwise. 

  Remember to replace Gatsby's `<Link/>` tags with `<a>` anchor tags, and use `class` instead of `className`. 

  Copy the existing Gatsby CSS stylesheet into your project, and import it into the layout.

  ```astro title="src/layouts/Layout.astro" ins={2-3} {15-24}
  ---
...
```jsx title="src/templates/blog-post.js" {11-32}
import * as React from "react"
...
```jsx title="src/pages/index.js" {21-56}
import * as React from "react"
import { Link, graphql } from "gatsby"
import Bio from "../components/bio"
import Layout from "../components/layout"
