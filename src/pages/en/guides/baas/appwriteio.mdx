...
:::caution
Please note that you can use the same JavaScript SDK `appwrite` to access both the serverside and clientside data. However in this example we are using `node-appwrite` to further illustrate the use of API keys. This might change in future releases of `appwrite` to allow for use of the API Key.
:::
...
With the setup above, you are now able to create a blog that uses Appwrite.io as your backend.

You can find the full example on https://github.com/appwrite/demos-for-astro.

### Prerequisites

Install Appwrite.io's CLI, if you want to use and additional method see [Appwrite CLI](https://appwrite.io/docs/command-line)
    ...
    "collections": [
        {
            ...
            "$createdAt": "2023-02-09T21:11:53.352+00:00",
            "$updatedAt": "2023-02-09T21:11:53.352+00:00",
            "$permissions": [],
...
Create a new interface called `BlogPost` and add it to your `appwrite.ts` file in `src/lib/`. This interface will match the attributes of your blog post content type in Appwrite.io. You will use it to type your blog post entries response.
...
Next, go to the Astro page `/src/pages/blog/index.astro` where you will fetch data from Appwrite.io's database.
