...
export default defineConfig({
  output: "server",
  integrations: [react(), tailwind()], //optional integrations
  adapter: node({
    mode: "standalone",
  }),
  server: {
...
FROM node:18-slim
WORKDIR /app
COPY ["package.json", "robots.txt","tailwind.config.cjs","astro.config.mjs", "tsconfig.json","./"]
COPY public ./public
COPY src ./src
RUN npm install
RUN npm run build
