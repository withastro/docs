...
Astro's build process works on the module graph: once a component is included in the template, its `<script>` and `<style>` tags are processed, optimized, and bundled, whether it appears in the final output or not (e.g. when [conditionally rendered](https://docs.astro.build/en/core-concepts/astro-components/#dynamic-html) components are not displayed). This does not apply to scripts when the `is:inline` directive is applied.
