...
Astro's build process works on the module graph: this means that once a component is used, its `<script>` and `<style>` tags are processed, optimized, and bundled, whether it is rendered or not. This does not apply if the `is:inline` directive is applied. 
