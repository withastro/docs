...
:::note[Replace variables]
Replace the following:

`SOURCE_IMAGE` is the local image name or image ID.
`HOSTNAME` is the registry host (`gcr.io`, `eu.grc.io`, `asia.gcr.io`, `us.gcr.io`).
`PROJECT` is your Google Cloud project ID.
`TARGET-IMAGE` is the name for the image when it's stored in the registry.
