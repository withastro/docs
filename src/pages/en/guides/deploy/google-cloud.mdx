...
- Locally using Docker
  - Requires you to build with `docker build` command and push to a remote using `docker push`. (That would be Artifact registry or container registry on Google Cloud)
- Using [Cloud Build](https://cloud.google.com/build)
  - To setup a pipeline with CloudBuild, you have to add a separate `cloudbuild.yaml` file with a description what the pipeline should do. [Check the Google Cloud Build documentation here](<https://cloud.google.com/build/docs/build-push-docker-image>).
