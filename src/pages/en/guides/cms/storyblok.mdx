...
To test the setup, create a new page in the `src/pages` directory named `index.astro` with the following content:
...
To create a route for a specific `page`, you can fetch its directly from the Storyblok API and pass it to the `Page` component. Create an `index.astro` file in `src/pages` to render the `home` page:
...
To dynamically render all of the blog posts, create a dynamic route. This approach varies depending on whether you're using **static site generation** (the default) or **server-side rendering**.
...
Create a new file in a `src/pages/blog` directory called `[...slug].astro` and add the following code:
