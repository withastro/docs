...
The page `src/pages/index.astro` will list post, with a description and link to its own page.
...
Import the API in the Astro frontmatter to use the `posts.browse()` method to access blog posts from Ghost. Set `limit: all` to retrieve all posts.
...
Fetching via the content API returns an array of object that includes the following properties for each post:
...
Import the API to access blog posts using `posts.browse()` and return a post as props to each of your dynamic routes. 
