...
3. Check that you are running **at least Node `16.12.0`**. In v2.0, Astro dropped support for Node `14`.
    ...
    If necessary, [update your version of Node](en/upgrade-astro/#updated-node-version-16120) in your local and deployment environments.

## Do I need to update my code?

Many projects will not need any changes to the codebase! Your upgrade may now be complete.  

But, this upgrade does bring some big, potentially breaking changes:
...
So, [you might see Houston](#error-overlay) a few times before you're done! 
...
- [Prerendering individul pages to static HTML](/en/guides/server-side-rendering/#prerendering) when using SSR to improve speed and cacheability.
- [Error Overlay](#error-overlay) improvements, with a complete redesign to help you understand and fix your errors.
...
Astro 2.0 enables per route, opt-in prerendering when using `output: 'server'`. 
...
But when you do, we hope that enjoy our new error overlay to give you more helpful, and easy-to-read messages. You can now expect:
...
- A full, syntax-colored code window when showing the location of your error.
