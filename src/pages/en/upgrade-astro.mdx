...
[Does this belong?]
[#5584](https://github.com/withastro/astro/pull/5584) - builds to `assets`. This is (?) changing to `_astro`? Not done yet?

Builds chunks into the `_astro` folder. This simplifies configuring immutable caching with your adapter provider as all files are now in the same `assets` folder.

### Changed: Markdown plugin configuration

#### Removed: `extendDefaultPlugins`

Astro 2.0 removes `markdown.extendDefaultPlugins` as applying remark and rehype plugins **no longer disables** Astro's default plugins. 
