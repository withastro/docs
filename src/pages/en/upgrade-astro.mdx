...
- **Vite 4**
  ...
  Astro *should* [handle everything](#updated-vite-4) for you under the hood! But still, stuff to know!

- **the complete removal of several previously-deprecated items**

  Using one of the [deprecated options mentioned below](#astro-20-breaking-changes)? That's got to go!
...
Astro 2.0 removes `markdown.extendDefaultPlugins` for controlling Astro's default plugin behavior. 
...
Astro 2.0 removes the `mdx.extendPlugins` option to manage inheriting Markdown plugin defaults in MDX. Previously, this configuration setting was used to indicate which Markdown plugins, all or just Astro's defaults, should be applied to MDX. 

This behavior is now configurable with three new options:

- [`mdx.extendMarkdownConfig`](/en/guides/integrations-guide/mdx/#extendmarkdownconfig) (`true` by default) to toggle Markdown config inheritance.
- `mdx.gfm` (`true` by default) to toggle GitHub-Flavored Markdown in MDX.
- `mdx.smartypants` (`true` by default) to toggle SmartyPants in MDX.
...
Replace `extendPlugins: 'defaults'` with `extendMarkdownConfig: false` if you do not wish to inherit any of your Markdown plugins in MDX. You can use the [individual options for GitHub-Flavored Markdown and SmartyPants](/en/guides/integrations-guide/mdx/#options-inherited-from-markdown-config) to reenable only Astro's default plugins.
...
And, with access to a file's frontmatter, you can now write plugins that use this data! For example, to compute a full image URL based on an `imageSrc` slug in your document frontmatter:
...
If you have an existing `src/content/` folder, it must be renamed to avoid conflicts. This special folder can now only be used for [content collections](/en/guides/content-collections/).
