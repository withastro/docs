...
const pages = Object.fromEntries(
	paths.map(({ id, slug, data }) => [id, { data, isRtl: rtlLanguages.has(getLangFromSlug(slug)) }])
);
...
export const { getStaticPaths, GET } = OGImageRoute({
	...
	getImageOptions: async (_, { data, isRtl }: (typeof pages)[string]): Promise<OGImageOptions> => ({
		title: data.title,
