...
Tudo descrito na seção de endpoints de arquivos estáticos também pode ser utilizado no modo SSR: arquivos podem exportar uma função `get` que recebe um [objeto de contexto](/en/reference/api-reference/#contexto-de-endpoint) com propriedades similares a global `Astro`.
