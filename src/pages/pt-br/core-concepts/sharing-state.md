...
Você deve ter notado que estamos chamando `itensCarrinho.get()` aqui ao invés de pegar o helper `useStore` de nossos exemplos em React / Preact / Solid / Vue. Isso é porque **useStore é feito para gatilhar re-renderizações de componente.** Em outras palavras, `useStore` deve ser utilizado sempre que o valor da store está sendo renderizado na UI. Já que estamos lendo o valor quando um **evento** é iniciado (`adicionarAoCarrinho` nesse caso) e nós não estamos tentando renderizar o valor, nós não precisamos de `useStore` aqui.
</details>
:::
