...
:::note
Adicionar `type="module"` ou qualquer atributo que não seja `src` a uma tag `<script>` irá desabilitar o comportamento padrão de bundling do Astro, tratando a tag como se tivesse uma diretiva `is:inline`.
:::
...
Astro irá fazer a build, otimizar e adicionar esses scripts a página para você, seguindo suas [regras de bundling de script](#bundling-script).
