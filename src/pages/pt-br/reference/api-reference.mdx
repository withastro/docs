...
**Tipo:** `(entry: { defaultSlug: string } & Omit<CollectionEntry, 'slug'>) => string | undefined`

`slug()` é uma função opcional que sobreescreve a slug padrão da entrada gerada pelo Astro. Essa função recebe `defaultSlug`, `id` da entrada, `body` do documento sem parse, e `data` com parse pelo seu schema configurado, se houver.
