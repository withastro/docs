...
Astro 组件的语法是 HTML 的超集。该语法[被设计成让所有拥有编写 HTML 或 JSX 经验的人都感到熟悉](/en/comparing-astro-vs-other-tools/#astro-vs-jsx)，并增加包括对组件和 JavaScript 表达式的支持。你可以通过文件扩展名 `.astro` 来创建新的 Astro 组件。
...
然而，Astro 的组件模板语法也支持 **JavaScript 表达式**、**导入的组件** 和 [** 特殊的 Astro 指令 **]（/en/reference/directives-reference/）。在组件脚本中定义的数据和值（在页面构建时）可以在组件模板中使用，以产生动态创建的 HTML。
...
> ⚠️ 这仅在你将插槽传递给其他 Astro 组件时才有效。 了解有关在 Astro 文件中包含其他 [UI 框架组件](/en/core-concepts/framework-components/)的更多信息。
...
📚 有关应用样式的更多信息，请参阅我们的 [样式指南](/en/guides/styling/)。

### 客户端脚本

在不使用[使用框架组件](/en/core-concepts/framework-components/)（React、Svelte、Vue、Preact、SolidJS、AlpineJS、Lit）或 [Astro 集成]（https： //astro.build/integrations/)（例如 astro-XElement）时，你可以在你的 Astro 组件模板中使用 `<script>` 标记使得该 JavaScript 可以在浏览器中使用。
...
📚 请参阅我们的[指令参考](/en/reference/directives-reference/#script--style-directives)页面以获取有关 `<script>` 标签上可用指令的更多信息。
...
📚 阅读 [Astro 的内置组件](/en/reference/api-reference/#built-in-components)。

📚 了解如何在你的 Astro 项目中使用 [JavaScript 框架组件](/en/core-concepts/framework-components/)。
